...
| `cyrup-ext-subagents/src/spawn/signal.rs:276,288,304` | `send_sigint`/`send_sigterm`/`send_sigkill` compile to no-ops; subagent termination kills nothing | `utils/shell.ts:200-212` `killProcessTree` → `taskkill /F /T /PID` |
