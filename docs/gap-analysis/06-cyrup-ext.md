...
| EXT-050 | low — **PARTIALLY CLOSED 2026-08-14** | upstream-drift | M | pi.events gained stale-context guarding and tracked unsubscribe; cyrup's bus has neither — **PARTIALLY CLOSED 2026-08-14**: sweep 1 — partially closed; residual unchanged. |
...
| EXT-061 | medium | not-ported | S | `ctx.getSystemPromptOptions()` has no counterpart, so a command can read the resolved system prompt but not the bag that built it — **filed 2026-08-14 (ext-rpc surface enumeration)**: the only genuinely unaccounted-for member of the whole 134-member extension API surface; `grep -rn 'system_prompt_options\|getSystemPromptOptions\|system-prompt-options' crates/` returns nothing workspace-wide. |
| EXT-062 | low | cyrup-original | S | `registration.add-autocomplete` is a cyrup-original registration call carrying no pi citation at any of its five sites — **filed 2026-08-14**: defensible (a WIT record cannot carry a closure) but undisclosed; needs a `CYRUP-DELTA` naming `types.ts:1166`. Same shape `working-start`/`working-stop` had before EXT-021 caught them. |
| EXT-063 | low | cyrup-original | S | `ManifestResources.agents` and the `cyrup` package.json key are cyrup-original additions to pi's four-key resource manifest — **filed 2026-08-14**: labelled in-code, low risk, filed so the invented half of the manifest surface is KNOWN. **FIX SITE: `crates/cyrup-resources/src/package/manifest.rs` (area 05).** |
| EXT-064 | medium | cyrup-original | M | `ui.set-header`/`set-footer` take a `string` where pi takes a component factory, and pi's `ReadonlyFooterDataProvider` has no cyrup analog at all — **filed 2026-08-14**: the collapse is a recorded decision (TUI-033) but the WIT carries no delta note at the signature, so a reader of `world.wit` alone sees a signature that silently contradicts the pi line it cites. |
| EXT-065 | medium | cyrup-original | S | `add-autocomplete-provider` sits on `interface registration`, not `interface ui`, so the manifest `ui` grant does not gate it — **filed 2026-08-14**: placement divergence with a capability consequence; the control EXT-054 built does not reach this call. |
| EXT-066 | medium | parity-bug | S | The live theme is the one theme a guest cannot read the colours of — `theme-get` returns `option<string>` (a NAME) while `theme-get-by-name` returns a serialized theme — **filed 2026-08-14**: the name-only return is documented, the asymmetry with `theme-get-by-name` is not. Pairs with EXT-006, which needs a theme on the render path. |
| EXT-067 | medium | parity-bug | S | `session_before_compact` and `session_before_tree` drop pi's `signal` payload field, and both WIT comments enumerate the other fields exhaustively so the drop reads as an oversight — **filed 2026-08-14**: `ctx-state.is-run-cancelled` is the plausible substitute but is never named as such. Sibling of EXT-015, which covers the other four lifecycle events. |
| EXT-068 | medium | parity-bug | S | `on-session-tree` carries one opaque `tree-json` blob and is the only event export in `world.wit` with no comment and no pi citation at all — **filed 2026-08-14**: pi's `SessionTreeEvent` has four named fields and nothing in the workspace states which payload the blob actually carries. |
| EXT-069 | low | cyrup-original | S | The three `on-tool-exec-*` exports abbreviate pi's `tool_execution_*` event names — **filed 2026-08-14**: the only place in the 33-event mapping where the export name is not a mechanical kebab-case of pi's, so a name-based cross-check reports three false gaps. Renaming is an export re-signing ⇒ bump `HOST_WORLD`. |
| EXT-070 | low | stale-port | S | The extension-manifest surface is split across two crates and `cyrup-ext/src/manifest.rs`'s doc comment conflates the two disjoint halves (and mis-cites `loader.ts:596`) — **filed 2026-08-14**: a reader of either file concludes the other half is missing. Comment-only. |
...
| EXT-072 | low | stale-port | M | ~50 in-tree pi citations across both `world.wit` copies and `cyrup-ext-sdk/src/ctx.rs` are stale by a **uniform offset**, in five distinct clusters — **filed 2026-08-14**: same class as EXT-036's `+8`/`~+7` clusters, in regions that sweep did not walk (`interface events` / `interface registration` / `ctx.rs` / the payload block). Includes the `:1257-1266` instance EXT-036 named by name and fixed only in `.rs`, and three self-undermining corrective notes. Wants a citation-lint test, not just a rewrite. |
| EXT-073 | medium | stale-port | S | Two **fabrications** survive in the WIT — a spliced `ctx.abort()` doc quote, and nine `subscribed at` citations naming a band that exists in no version — **PARTIALLY FIXED 2026-08-14**: the spliced quote is fixed at all four sites (both `world.wit` copies, `ctx.rs:225`/`:235`, `host/services.rs:119`/`:122`). **RESIDUAL, still open:** the nine `:1135-1161` `subscribed at` lines and the header's `session_info_changed … upstream :1203` (twice) — the same not-any-version signature EXT-036 filed and this area's third fabrication. |
