...
| EXT-013 | medium | parity-bug | M | Slash-command argument completions and autocomplete providers are dead — **2026-08-14, still open — but NOW A ONE-LINER**: sweep 6 re-verified — **the cyrup-ext side EXISTS and works**: `facade.rs::command_completions` and `host/live.rs::argument_completions` are implemented and reachable from tests. **The ONLY thing missing is `crates/cyrup-tui/src/commands.rs:348` hard-coding `has_arg_completion: false`.** A cyrup-tui owner can close it in one line plus a test. |
