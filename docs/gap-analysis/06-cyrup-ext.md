...
> ## RE-MEASURED 2026-09-24 — cyrup **`ea23ca2`**, pi **`v0.87.1`**. Supersedes the pin table above.
>
> | | now measured at | window read this pass | still unread |
> |---|---|---|---|
> | `cyrup/` | **`ea23ca2`** (2026-09-24) | `9aeba769..ea23ca2` under `crates/cyrup-ext` = 11 files, +1 139 / −70, from `838eac7` (UW-7: raw terminal-input key decoding in `contract.rs`, `has_terminal_input_subscribers`, a TUI consumer) and `df3e9a8` (the `HumanWaitGate` generalised to `SanctionedWaitGate`, `DEFAULT_INVOKE_BUDGET` made public, `HostServices::{editor_has_focus, registered_provider}`); `crates/cyrup-ext/wit/world.wit` and `crates/cyrup-ext-sdk` **unchanged**; `crates/cyrup-session-svc` 7 files, +413 / −6 (`host_services.rs` editor-focus mirror + provider swap, `session/inject.rs` inject-pump re-drain) | — |
> | `pi/` | **v0.87.1** | `packages/coding-agent/src/core/extensions/types.ts` diffed in full `v0.85.1..v0.87.1`; `runner.ts` read at `emitUserBash`, `emitBoundary`, `emitContext` and `snapshotEventHandlers` @v0.87.1 (and `emitUserBash` @v0.85.1); `loader.ts`'s `on()` @v0.87.1; coding-agent CHANGELOG 0.86.0–0.87.1; tag-by-tag presence checks for every item filed | `runner.ts` (+378) and `loader.ts` (+256) outside the hunks named; `v0.84.1..v0.85.1` of `core/extensions/` is still only diff-stat skimmed (it is +9 / −5 across `loader.ts`/`types.ts` — small, but unread); `packages/chord` and `src/experimental/` (see the 2026-09-14 census below) not re-opened |
>
> **What this pass did.** (1) Every open row re-verified at `ea23ca2`. The `cyrup-ext` hunks in the
> window are additive (two new `ExtensionHost` methods, new native-ctx types, a renamed wait gate) and
> none touches the surface any open row names — `reload`/`emit_*`, `http-client`, OAuth, provider
> `refreshModels`, header/footer, `ui_prompt_*`, `ctx.compact`, command shadowing — so each open row
> is RE-CONFIRMED and marked in its cell. Nothing closed. (2) **Four items filed**, each read on both
> sides: **`EXT-077`** (`user_bash` now fails CLOSED, v0.86.0 — medium), **`EXT-078`** (actionable
> `turn_end` + new `agent_before_settle`, v0.87.0), **`EXT-079`** (`context_with_system`, v0.87.0),
> **`EXT-080`** (`pi.on()` returns an unsubscribe, v0.86.0).
>
> **Leads from `v0.85.1..v0.87.1`, NOT filed** (upstream read from `types.ts` diff + CHANGELOG; cyrup
> side not opened unless stated): `cache_warming_decision` event (v0.86.0 — rides on a prompt-cache
> warmer cyrup has no counterpart for; area 01/08 first); `ctx.modelRegistry.stream()`/`streamSimple()`
> for extension model calls (v0.86.0 — compare with the new cyrup-original
> `HostServices::registered_provider`, `crates/cyrup-ext/src/host/services.rs`, which returns the
> session's provider to a NATIVE caller only); extension tools without a parameter schema rejected at
> registration (#9300, v0.86.0); `before_agent_start`'s `systemPromptOptions` became a mutable
> `NormalizedBuildSystemPromptOptions` observed by later handlers and `systemPrompt` read-only
> (v0.86.0 — bears on the closed `EXT-061`); `agent_settled` handlers' requested runs deferred until
> all settled handlers finish (v0.87.0); `ProviderConfig.streamSimple` now receives a
> `TranscriptContext` (v0.86.0 — `AGENT-039`'s shape reaching `EXT-052`'s seam);
> `ProviderModelConfig.{inputLimits,promptCache}` (v0.86.0/v0.87.0). **One cyrup-side addition with
...
| EXT-003 | **closed 2026-09-04** | Was `**still open** (medium)`; every clause of that note is false at HEAD and the correction is dated to the code, not to this edit. The store half landed first (`builder.rs:695-698` reads `TrustInputs.saved` from `SessionBuilder::trust_store`, `:704-713` persists a `remember`-ed verdict), and the `with_wasm` ordering half landed 2026-09-04 in `605f483c` — TUI-N02's commit, which carried this fix without naming EXT-003 anywhere but an inline comment, which is why this line survived. `pre_trust_extension_verdict` (`:2254`) now falls back to `ExtensionHost::new` (`:2293-2302`) and the native vote loop (`:2314-2321`) runs either way. See the row and the closure block under `## ~~EXT-003~~`. |
...
| EXT-006 | **closed 2026-09-04** | **CLOSED 2026-09-04 in `1739fcc4`.** Both remaining residuals landed. (1) `render-call`/`render-result` gained a third parameter `opts-json` in both `world.wit` copies (`cyrup:ext@0.8` → `0.9`, `HOST_WORLD` with it) carrying `cyrup_ext::RenderOptions` — the union of pi's `MessageRenderOptions {expanded, outputPad}` (`extensions/types.ts:1195-1199` @v0.84.4), `EntryRenderOptions {expanded}` (`:1209-1211`) and `ToolRenderResultOptions {expanded, isPartial}` (`:413-418`) plus the theme NAME; native extensions widen additively through `NativeExtension::render_{call,result,entry}_under`. (2) The render is no longer one-shot: `App::refresh_extension_renders` (`cyrup-tui/src/app/events.rs`) re-invokes every renderer whose text was produced under display inputs that no longer hold, called from `dispatch_run_action` and `ingest_session_event_owned`. Staleness is DERIVED, not flagged — `Rendered::Text(RenderedText)` carries a private `RenderSource {surface, key, payload, under}` stamped by `run_renderer`. Residual, low and named in the item body: the refresh covers only rows still addressable (active tool runs + committed-but-unflushed entries), not native scrollback or the retained altscreen `document`. The replayed-**tool**-row residual was **EXT-041**'s custom-ENTRY half, itself CLOSED 2026-09-05 in `104002d3`. |
...
| EXT-012 | **closed** | `register-entry-renderer` (`world.wit:290`), host import (`host/live.rs:137-142`), first-wins `entry_renderer_owner` table (`registry.rs:192`, `:353-361`, `:364-366`), `render_entry` + three-state `RenderOutcome` (`facade.rs:808-813`, `:1381-1394`), native `InitApi::register_entry_renderer` (`native.rs:295-297`), TUI consumer (`app/events.rs:38`, the `extension_render_entry` hop inside `ingest_event_with_extensions`), test `tests/entry_renderer.rs`. upstream `registerEntryRenderer` `types.ts:1279` @v0.83.0 (the auditor's `:1291` is the v0.84.1 line). Replay of custom **entries** was unwired → EXT-041, CLOSED 2026-09-05 in `104002d3` (`ReplayItem::CustomEntry` + `App::push_custom_entry`). |
...
| EXT-017 | **still open** (medium) | Unchanged in code, **worse in consequence**: since EXT-S02 closed, `command_descriptions`' unordered `HashMap` walk (`registry.rs:662-669`) feeds `slash_command_catalog` (`session.rs:2277,:2281`) which feeds the interactive `/` menu (`app/run_arms.rs:33` at boot, `:216` on session swap, `app/execute_misc.rs:217` on an `enableSkillCommands` flip). `resolved_commands`/`resolved_command_owner` remain production-dead. |
...
| EXT-024 | **closed 2026-09-04** | **CLOSED 2026-09-04 in `75532cee`** — the last unported piece, the TUI consumer of `Tool::render_kind`, landed; the WIT/SDK/host halves and `constrainedSampling` (PROV-011) were already in (`6d805fed`). See the `## Open items` row and the item body. |
...
| EXT-029 | **still open** (medium) | Unchanged, and **worse in practice**: since EXT-S03 closed, the spurious `onError` is now rendered into the interactive transcript as `Extension "<id>" error: cancelled` (`app/extension_ui.rs:267-271`, drained at `app/run_arms.rs:479`). |
...
| EXT-S02 | **closed** | `SlashCommand::name` is `Cow<'static, str>` (`cyrup-tui/src/commands.rs:46`); `dynamic_commands_from_catalog_gated` (`:436`) maps catalog rows to `CommandSource::{Extension,Prompt,Skill}` honouring `enableSkillCommands`; `CommandRegistry::with_dynamic` (`:214`) merges behind the builtins; installed at `app/run_arms.rs:33` (boot), `:216` (session swap) and `app/execute_misc.rs:217` (an `enableSkillCommands` flip). upstream `interactive-mode.ts:601-628`, skill gate `:610`. Three behaviours inside this seam remain open separately: EXT-017, EXT-013, EXT-053. |
| EXT-S03 | **closed** | `App::install_error_listener` (`app/extension_ui.rs:246-255`) forwards contained `ExtensionError`s onto an unbounded channel, drained at `app/run_arms.rs:473-481` into `show_extension_error` (`app/extension_ui.rs:267-271`), installed at boot (`app/run.rs:107`) and re-installed on session swap (`app/run_arms.rs:212`). upstream copy is byte-identical (`interactive-mode.ts:2545-2546`, bound at `:1701`). Documented residual: cyrup's `ExtensionError` has no `stack`, so pi's dimmed stack line is absent (stated at `app/extension_ui.rs:261-263`). |
...
| EXT-039 | **closed 2026-09-04** | Extension shortcuts bypass the reserved-keybinding refusal, emit no conflict diagnostics, and lose to built-ins — **CLOSED 2026-09-04 in `9c25f603`**: the gate is called from production, the precedence is inverted, and the diagnostics reach the `[Extension issues]` panel. The precedence half is no longer inferred — `custom-editor.ts:31-34` @v0.84.4 was read; see the `## Open items` row and `## Coverage`. Read side is TUI-N05 in area 07. |
| EXT-040 | **new this pass** — open (low) | `register-shortcut`'s description is discarded, so `/hotkeys` shows a raw key-id. |
| EXT-041 | **closed 2026-09-05** | Replayed tool calls and results lose their extension renderer — **CLOSED 2026-09-05**: the TOOL half in `a0134787`, the custom-ENTRY residual in `104002d3`. Split out of EXT-006's third residual so the custom-message replay closure is not re-opened; both halves are now landed and tested. |
...
> **RECOUNTED 2026-09-04, batch 2 (ledger audit); authoritative over every block below — counted set: 0 critical, 0 high, 5 medium, 9 low = 14 open.** The table carries **72 rows: 58 closed, 14 open** (`scripts/count_open_items.py`). Two rows moved on landed code, both independently reviewed: **`EXT-024` CLOSED** at `75532cee` (the TUI consumer of `Tool::render_kind`; see the row's Git-history note — `75532cee`/`651bac70` do not build alone, step to `0e8c62fa`) and **`EXT-041` PARTIALLY CLOSED** at `a0134787` (replayed tool calls/results get their extension renderer; the custom-ENTRY half stays open, low, fix site area 08). `EXT-039`'s residual (`resolve_shortcuts` has no production caller) was hit live by area 14's `FLUX-004` and is unchanged here.
>
> **RECOUNTED 2026-09-04 (baseline-sweep pass against cyrup HEAD `2571969`, `4fb5e40..HEAD`; authoritative over every block below) — counted set: 0 critical, 0 high, 6 medium, 9 low = 15 open.** The table carries **72 rows: 57 closed, 15 open.** `git log --oneline 4fb5e40..HEAD -- crates/cyrup-ext` surfaced 16 commits; the one that actually moved this area's open set was `dd69fea` ("fourteen medium pi parity gaps, wired end-to-end across crate boundaries"), a cyrup-tui-led pass that also edited cyrup-session-svc and closed several area-06 residuals in passing without this file's knowledge — the same "nobody told the ledger" pattern the 2026-08-15 pass found. **Five rows CLOSED, all with residuals verified gone by reading the current code, not the commit message:** `EXT-013` (the session-svc `argumentCompletions` producer and the cyrup-tui consumer both exist and are wired to a real production call site), `EXT-017` (the catalog now emits `invocation_name` and dispatch resolves the owner through `resolved_commands`), `EXT-019` (`App::apply_markdown_transformers` is reached from both the live and replay production paths), `EXT-031` (the DRIFT-033 two-slot prompt model landed and `prepare_next_turn` re-pushes `effective_system_prompt()`), and `EXT-059` (`AgentSession::load_wasm_extension` now takes a required `Capabilities` parameter). **One row PARTIALLY closed further:** `EXT-003` — the trust-store half (persist + read-back, wired to a real store in `crates/cyrup/src/session_launch.rs`) is done; the `with_wasm`-discards-native-votes-on-failure half is not. **One row narrowed without closing:** `EXT-025` — `command_completions` gained a real caller (the EXT-013 fix); the other four dead facade methods (`emit_before_agent_start`/`emit_input`/`emit_user_bash`/`reload`) did not. **One new item filed**, `EXT-075`, from the `v0.84.1..v0.84.4` diff-stat skim this file's own baseline table calls for: pi added `ui_prompt_start`/`ui_prompt_end` at v0.84.4 with no cyrup counterpart at all. `EXT-S04` gained a strengthening note (not a severity change): v0.84.4 independently added the `session_compact_failed` event this item's own Fix had proposed inventing, so cyrup now has an upstream-confirmed shape to port against — still not ported. **Eleven open rows were re-checked this pass and left exactly as found, because HEAD had not moved them:** `EXT-006`, `-022`, `-024`, `-026`, `-027`, `-039`'s residual, `-041`, `-051`'s residual, `-053`, `-058`, `-064`'s residual — each re-verified by grep/read against the specific symbol or file its row names, not re-derived from prose. **This pass did not run the surface-driven enumeration `## Coverage` axis 8 calls for** — the version-lag skim was scoped to a diff-stat of `packages/coding-agent/src/core/extensions/` only, and `runner.ts`'s `createExtensionAPI` transactional-loading rewrite (`v0.84.1..v0.84.4`, commit/discard semantics for a not-yet-active extension's registrations) was seen but not investigated for a cyrup analog — flagged, not filed, because cyrup's WASM instantiation is plausibly already atomic in a way that makes the concern moot, and confirming that needs a read this pass did not do.
>
> **RECOUNTED 2026-08-19 (post-`40821ed` citation repair) — counted set: 0 critical, 0 high, 10 medium, 9 low = 19 open.** The table carries **71 rows: 52 closed, 19 open.** One row moved: **`EXT-040` CLOSED** — its stated area-07 residual ("`cyrup-tui/main.rs` consuming `shortcut_specs`") is discharged at `crates/cyrup/src/main.rs:2017` and the description now reaches `/hotkeys`; the row has the per-hop evidence. **The severity split in the fifth-edition block below is wrong and is corrected here, not by editing it**: it reads "8 medium, 11 low" against a table that held 10 medium and 10 low before this pass — a bookkeeping slip, not a status change, and the kind the header's own "count this table" rule exists to catch. **Nothing else moved; this pass repaired citations.** `40821ed` split `crates/cyrup-tui/src/app.rs` into `crates/cyrup-tui/src/app/`, and every `app.rs:NNNN` in this file was re-pointed by SYMBOL and verified by reading the target. Two of the automated remaps had landed on text that matched but meant something else (`app/channels.rs` for `install_error_listener`/`show_extension_error`, which are `app/extension_ui.rs:246` and `:267`; `app/shell.rs` for `replay_session_with_extensions`, which is `app/session_bind.rs:145`), and fourteen occurrences carried a range whose END was still the pre-split number. **`EXT-047`'s detail section also gained a closure note** — the row closed on 2026-08-14 but its filing text still described the collapsed one-argument WIT and the single `extension_widget` slot, both of which are gone.
>
...
> **RE-AUDITED 2026-09-24 at cyrup `ea23ca2` against pi `v0.87.1`.** No row closed; every open row is
> RE-CONFIRMED and marked in its own cell (see the RE-MEASURED block at the top of this file for why
> the window's cyrup-ext changes reach none of them). **Four rows filed** (`EXT-077`…`EXT-080`, all
> `upstream-drift`), each read on both sides. This block states no count; derive one from the rows.
...
| ~~EXT-059~~ | ~~medium~~ **CLOSED 2026-09-04** | cyrup-original | S | `AgentSession::load_wasm_extension` is a full-authority manifest-less load reachable as public API — **CLOSED 2026-09-04**: `caps: &cyrup_ext::Capabilities` is now a REQUIRED parameter of `AgentSession::load_wasm_extension` (`crates/cyrup-session-svc/src/session/accessors.rs:399-407`), routed to `ExtensionHost::load_wasm_with_caps(id, bytes, services, caps)` exactly as the Fix asked, with an in-source note ("`caps` is REQUIRED, not implied... a byte-level load with a TOTAL grant, chosen silently by a function whose name says nothing about capabilities") recording the old behaviour it replaced. `Capabilities::host_granted()` is preserved as the caller's explicit choice for an embedder-supplied component, not a silent default. |
| EXT-058 | low | cyrup-original | S | Guest WASM `http-client` is not gated by `--offline` — **classification refuted 2026-08-13**: pi's flag is documented "startup network operations" verbatim (`args.ts:277` @v0.83.0) and pi has no extension network gate at all, so this is parity, not a defect. The control that was missing was EXT-054's `"net": false`, now enforced. Open as a decision — **2026-08-14, still open**: sweep 2 — correctly untouched: an open product decision whose defect classification was already refuted. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low; this row's unusual shape (code claim HOLDS, defect classification REFUTED, left open as a PRODUCT DECISION) survives intact, and nothing in the tree has decided it.** cyrup: `grep -rn offline crates/cyrup-ext/src crates/cyrup-ext/wit` is still zero, so the extension host still cannot consult the flag; the shipped help text is now `crates/cyrup/src/cli/help.rs:100` ("Disable startup network operations (same as CYRUP_OFFLINE=1)") — this row's `cli.rs:936` citation is dead. Upstream at **v0.85.1**, `packages/coding-agent/src/cli/args.ts:318` is the identical sentence with `PI_OFFLINE`, i.e. pi's flag scope has NOT widened two minor versions past the baseline. The 2026-08-13 refutation's load-bearing premise was re-tested at the CURRENT tag rather than taken on faith: `loader.ts` @v0.85.1 has zero hits for offline/capab/sandbox/permission/net, and `runner.ts` @v0.85.1 zero for offline/capabilit — pi still has NO extension network gate at all. The compensating control was read, not assumed: `guest.require_net()?` at `crates/cyrup-ext/src/host/live.rs:80`, implemented at `crates/cyrup-ext/src/host/services.rs:1777`, regression-pinned by `crates/cyrup-it/tests/ext/manifest_capabilities.rs:188 net_is_refused_when_the_manifest_denies_it`. So the observation holds, the refutation holds, and the decision is undecided. **Falsification condition (owed, because the disposition rests on an argument):** reopen as a DEFECT the moment either measurement changes — (a) `git show <newer-tag>:packages/coding-agent/src/cli/args.ts` shows help text whose scope is no longer the literal string "Disable startup network operations"; or (b) a grep of `core/extensions/loader.ts` and `runner.ts` at a newer tag for offline/capab/net returns a non-zero count, i.e. pi grows any extension-side network gate. A third, cyrup-side trigger: if `crates/cyrup/src/cli/help.rs:100`'s sentence is ever widened past "startup", the flag's own promise is broken and the code claim becomes a defect without upstream moving at all. **LEDGER DEFECT, recorded not silently rewritten:** the detail section's header line still reads `**Kind** cyrup-original · **Severity** medium` while BOTH status tables carry the low / product-decision re-rating; a correction line has been added directly under that header. **KIND RECLASSIFIED 2026-09-16 at `cc7818b`: `product-decision` → `cyrup-original`.** `product-decision` is not in this directory's taxonomy (`README.md`'s *Item format*: `parity-bug` · `not-ported` · `upstream-drift` · `stale-port` · `cyrup-original` · `test-defect`, plus a tail of `tooling` / `port-divergence`), so `scripts/count_open_items.py` flags this cell as UNCLASSIFIED and cannot class it. **`cyrup-original` is the correct kind and it is not a compromise choice.** The other six do not fit and were each tested against the row rather than eliminated by elimination: it is not `parity-bug` or `stale-port` (the 2026-08-13 refutation, re-proved at v0.85.1 above, establishes cyrup MATCHES pi here — pi has no extension network gate at all); not `not-ported` or `upstream-drift` (there is no upstream behaviour on either side of the ported tag to port or to lag behind); not `test-defect`; not `tooling`. What the row actually proposes is that cyrup ADD a gate **pi does not have** — a surface with no upstream basis, which is the literal definition of `cyrup-original`. The row's own detail-section header already read `**Kind** cyrup-original`, so this aligns the table with the body rather than inventing a third answer. **Severity `low` is unchanged and deliberately so** — it is already the floor, and the code claim is parity. **Recorded, not applied — a `tracker` reclassification is also defensible and needs an owner, not an auditor.** `README.md` defines `tracker` as a row proposing no schedulable work because it asks a scope question, and requires that each tracker record what would escalate it; this row asks exactly such a question and already carries three explicit falsification conditions. Moving it would take it out of the counted tally, which is a count change this pass is not entitled to make unilaterally — so the kind is fixed here and the tracker question is raised for the ranking stage, alongside `AGENT-028` and `SESS-038`, which are the precedents. **RE-CONFIRMED 2026-09-24 at `ea23ca2`** — no `http-client`/offline code in the `9aeba769..ea23ca2` cyrup-ext diff. |
| ~~EXT-003~~ | ~~medium~~ **CLOSED 2026-09-04** | not-ported | M | Project-trust store is unwired at the extension seam — **CLOSED 2026-09-04**: both halves are done, and the second one landed under another item's id, which is why this row is only being struck on 2026-09-05 (review pass). **Store half.** `SessionBuilder` carries `trust_store: Option<Arc<TrustStore>>` (`crates/cyrup-session-svc/src/builder.rs:432`) with a `.trust_store(store)` setter (`:527-529`), wired to a real store in production at `crates/cyrup/src/session_launch.rs:173`; `build()` reads it into `TrustInputs.saved` (`:695-698`, no longer a literal `None`) and persists a `remember`-ed extension verdict with `store.set(&cwd, Some(decision))` (`:704-713`). Upstream re-read at v0.84.4: `core/project-trust.ts:65-67` persists under `if (result.remember === true)`, `:72-75` reads the store back after the extension pass. **`with_wasm` ordering half — the residual this row carried — landed 2026-09-04 in `605f483c`,** whose subject and body name only TUI-N02; nothing but an inline code comment named EXT-003, so this row went on asserting the opposite for a day. `pre_trust_extension_verdict` (`builder.rs:2254`) no longer opens `ExtensionHost::with_wasm(host_config).ok()?`: it matches on `pre_trust_wasm_host(...)` (`:2293-2302`, helper `:2357-2370`) and on `Err` warns and continues with `ExtensionHost::new(host_config)`, so the native `decides_project_trust` voters at `:2314-2321` still run and `aggregate_project_trust` (`:2343`) still sees them. The wasm discovery scan is skipped in that state (`if wasm_ready`, `:2329`) because every `load_wasm_with_caps` on a runtime-less host returns `ExtError::WasmHostDisabled` — a tier that cannot load contributes no voters, it does not drop the others. That is upstream's own partial-failure rule, read at v0.84.4: `loadProjectTrustExtensions()` (`core/resource-loader.ts:380-386`) returns a `LoadExtensionsResult` whose per-extension faults live in `errors`, and `emitProjectTrustEvent` polls `extensionsResult.extensions` — the ones that DID load (`core/extensions/runner.ts:204-233`, the loop head at `:210`, the per-handler `catch` pushing to `errors` at `:223-230`). **Test:** `a_dead_pre_trust_wasm_runtime_does_not_discard_native_votes` (`crates/cyrup-session-svc/src/tests/project_trust_extension.rs:474`), which stages the failure through the `cfg(test)`-only `SessionBuilder::force_pre_trust_wasm_failure` (`builder.rs:445`, `:519-522`) because engine construction cannot be made to fail on demand — the production trigger is the pooling allocator failing to reserve its slabs (`cyrup-ext/src/host/engine.rs:19-26`), a property of the machine. RED before the change (`asked == 0`, the prompt runs and its `yes` trusts the project), GREEN after (the native's `no` decides, the prompt never runs). **RESIDUAL, low, unchanged and already filed:** EXT-043 — the `project_trust` event still carries no `cwd`, so a deciding extension has no well-defined key for the `remember` this row now persists. **Census effect**, measured with `docs/gap-analysis/scripts/count_open_items.py` against this directory before and after the strike: area 06 goes 12 open / 60 closed to 11 / 61 and its medium count 3 → 2; the workspace total goes 90 → 89 open and 16 → 15 medium, with above-medium still 0. **Citation note 2026-09-05:** this row's `crates/cyrup-session-svc/src/builder.rs` line anchors have drifted again since it was written (`CFG-078` and others moved the file); at HEAD `ba380e58` the symbols are `pre_trust_extension_verdict` (`:2286`), its `(host, wasm_ready)` match (`:2325`), the `if wasm_ready` scan gate (`:2361`) and the helper `pre_trust_wasm_host` (`:2389`). Every one was re-found by NAME, not shifted by an offset; prefer the symbol over the number, which will rot again. |
| ~~EXT-006~~ | ~~medium~~ **CLOSED 2026-09-04** | parity-bug | L | Renderers run without display options or theme, and only once — **CLOSED 2026-09-04 in `1739fcc4`**: the 2026-08-14 note was right that the WIT half had to land WITH the cyrup-tui move, and both did, in one commit. `render-call`/`render-result` take a third `opts-json` parameter in both `world.wit` copies (`cyrup:ext@0.8` → `0.9` with `HOST_WORLD`) carrying `cyrup_ext::RenderOptions` = `{expanded, outputPad, isPartial, theme}`, the union of pi's `MessageRenderOptions` (`extensions/types.ts:1195-1199` @v0.84.4), `EntryRenderOptions` (`:1209-1211`) and `ToolRenderResultOptions` (`:413-418`) plus the theme NAME (CYRUP-DELTA: a `Theme` object cannot cross a component boundary — a guest reads the palette with `ui.theme-get-json`, EXT-066). Native extensions widen ADDITIVELY through `NativeExtension::render_{call,result,entry}_under`, which default to the existing two-argument methods, so no in-tree native re-signs anything. The one-shot half: `App::refresh_extension_renders` re-invokes every renderer whose text was produced under display inputs that no longer hold, from `dispatch_run_action` and `ingest_session_event_owned`; staleness is DERIVED from a private `RenderSource {surface, key, payload, under}` that `run_renderer` stamps into every `Rendered::Text`, not from a dirty flag. Residual (low, in the item body): only rows still addressable refresh — active tool runs and committed-but-unflushed entries, not native scrollback (R-ARCH-TUI-003) nor the retained altscreen `document`. |
...
| ~~EXT-013~~ | ~~medium~~ **CLOSED 2026-09-04** | parity-bug | M | Slash-command argument completions and autocomplete providers are dead — **CLOSED 2026-09-04**: the corrected fix (one commit spanning both crates) landed in `dd69fea` ("fourteen medium pi parity gaps"). `AgentSession::slash_command_catalog` (`crates/cyrup-session-svc/src/session/commands.rs:174-278`) now carries the arg-completion signal — an opt-in `(owner, registered-name)` table (`ExtensionRegistry::command_autocomplete()`) written by both tiers and read into an `argumentCompletions` boolean key on extension rows only, explicitly citing `EXT-013 / TUI-012` and `interactive-mode.ts:753 @v0.84.3`. The consumer reads it: `crates/cyrup-tui/src/commands.rs:570-595` sets `arg_completion: ArgumentCompleter::Extension` from `row.get("argumentCompletions")`, gated on `kind == CommandSource::Extension`, and the async fetch reaches `ExtensionHost::command_completions` from `crates/cyrup-tui/src/app/execute_misc.rs:282-287`. |
...
| ~~EXT-017~~ | ~~medium~~ **CLOSED 2026-09-04 — residual verified gone** | parity-bug | S | Command listing is non-deterministic, drops name:N, and a colliding command is unexecutable — **CLOSED 2026-09-04**: the residual is discharged at HEAD. `slash_command_catalog` now emits `cmd.invocation_name` (the load-order, collision-suffixed `deploy:2` form), not the bare name — `entry.insert("name".into(), serde_json::Value::from(cmd.invocation_name));` (`crates/cyrup-session-svc/src/session/commands.rs:250`), sourced from `self.services.ext_host.registry().resolved_commands()` (`:225`). Dispatch matches: `ExtensionFacade::command_route` resolves `resolved_commands()....find(|r| r.invocation_name == invocation)` and hands the extension its own registered (unsuffixed) name back (`crates/cyrup-ext/src/facade.rs:1966-1973`), consulted by `live_for_command` (`:1977-1991`) — exactly the two-part fix the item's own Fix line specified ("swap session.rs:2277 to emit r.invocation_name… and change live_for_command… to consult resolved_command_owner"). |
...
| ~~EXT-024~~ | ~~medium~~ **CLOSED 2026-09-04** | parity-bug | M | renderShell/constrainedSampling unexpressible, and render_kind has zero consumers — **CLOSED 2026-09-04 in `75532cee`** (code; this row in the follow-up docs commit). All three halves are now in. **(1) Expressible** — landed earlier via `6d805fed`: `render-shell: option<string>` and `constrained-sampling: option<string>` on `tool-descriptor` in BOTH `world.wit` copies (`crates/cyrup-ext/wit/world.wit:113,122`, `crates/cyrup-ext-sdk/wit/world.wit:113,122`); the SDK lowers them with the EXT-023 exhaustiveness guard (`crates/cyrup-ext-sdk/src/guest.rs:78-91`); the host stores them (`crates/cyrup-ext/src/registry.rs:44,54`) and answers `WasmTool::render_kind` / `constrained_sampling` off the descriptor (`crates/cyrup-ext/src/host/live.rs:2127-2132,2140-2142`); `cyrup_core::Tool::constrained_sampling` exists (`crates/cyrup-core/src/tool.rs:226`) and the provider adapters consume it (PROV-011, `crates/cyrup-provider/src/utils/constrained_sampling.rs`). **(2) The consumer — this commit.** The producer this row said was missing now exists: `cyrup_session_svc::ToolInfo.render_kind` (`crates/cyrup-session-svc/src/tools.rs:41`, `#[serde(skip)]` so the EXT-038-pinned guest shape is untouched, read at `:223`). `AppState::known_tool_definitions` is a `HashMap<String, ToolRenderKind>` (`crates/cyrup-tui/src/app/state.rs:391`) filled by the live per-tool-start lookup (`app/events.rs:216-221`) and the bind-time refresh (`app/session_bind.rs:159`); both `push_tool_start_defined` writers hand the shell to the transcript (`app/events_fold.rs:191`, `app/session_bind.rs:370`) as `ToolRun::definition: Option<ToolRenderKind>` (`transcript/entry.rs:283`, replacing `has_definition: bool`). `tool_lines` resolves pi's three-way `getRenderShell()` (`transcript/tool_render.rs:111-121`) and `self_rendered_lines` (`:163`) emits the row bare — column 0, no state tint, nothing at all when the renderers drew nothing — with the built-in `edit`'s own `Box(1, 1, getEditHeaderBg)` drawn there around its rows (`:172`), so `edit` is byte-identical (X8 test unchanged) and the shell tail no longer special-cases `run.name == "edit"`. Upstream at the latest tag: `types.ts:467` @v0.84.4 (`:465` @v0.83.0, comment drift only); `components/tool-execution.ts:108-116` (`getRenderShell`), `:71-79` (bare `Container` vs `Box(1, 1, bgFn)` at construction), `:275-277` (no `setBgFn` on the container), `:237-259` (the `render()` branch); definition source `interactive-mode.ts:2067-2069` → `:3274`/`:3348`/`:3741`; `core/tools/edit.ts:330` `renderShell: "self"`, its box at `:258-281`. `v0.83.0..v0.84.4` changes only the result fallback in that file; the shell logic is identical. Visible beyond extensions: every MCP tool in the default `compact` mode (`crates/cyrup-mcp/src/registration.rs:2261-2266`) now draws without the tinted shell, which is what pi-mcp-adapter's `toolRenderShell` asks for. **Tests** (red-before verified by running the e2e file at HEAD before any code change: 2 of 3 failed with the row at column 1 under `toolPendingBg`; the Default control passed): `crates/cyrup-tui/src/tests/tool_render_shell.rs` `a_self_rendered_definition_omits_the_default_shell`, `a_default_definition_keeps_the_state_tinted_shell`, `the_live_tool_start_lookup_resolves_the_shell_without_a_bind_refresh` (a real `AgentSession` with `custom_tools` declaring each kind, driven through `refresh_known_tool_definitions` + `ingest_session_event_owned`, asserting on cell backgrounds); `crates/cyrup-tui/src/tests/tool_render.rs` `a_self_rendered_extension_row_has_no_shell`, `a_default_extension_row_keeps_the_shell`, `the_builtin_tier_answers_the_shell_for_a_legacy_push`. **Residual, low, hand-off to area 07 (built-in `edit` renderer fidelity, not the shell):** pi's edit `renderResult` returns a separate `Container` (`Spacer(1)` + `Text(output, 1, 0)`, `edit.ts:446-454`) OUTSIDE the header box; cyrup draws the result diff inside the header box — pre-existing, unchanged. **Git-history note (review fix 2026-09-04):** `75532cee` and its docs commit `651bac70` do not build in isolation — the code commit staged whole files (`crates/cyrup-tui/src/app/state.rs`, `crates/cyrup-tui/src/tests/mod.rs`) that carried the concurrent TUI-037 track's not-yet-committed hunks: `mod reload_implicit_trust;` (`tests/mod.rs:98` at that sha) names a file absent from that tree, and the swept `AppState::auto_trust_on_reload_cwd` doc links `App::set_auto_trust_on_reload_cwd`, which first exists in the very next commit `0e8c62fa` (TUI-037). At exactly those two shas `cargo test -p cyrup-tui` cannot compile and `cargo doc -D warnings` fails; the check results reported in this row were run on the shared working tree (i.e. the state `0e8c62fa` commits), not on `75532cee` checked out alone. Every commit from `0e8c62fa` onward builds and is green; the history was deliberately not rewritten (no rebase on the shared tree) — a bisect landing on either sha should step to `0e8c62fa`. The `:391` citation above replaces a stale `:384` (shifted by the later TUI-037/TUI-081 fields in `state.rs`). |
...
| ~~EXT-031~~ | ~~medium~~ **CLOSED 2026-09-04** | parity-bug | L | Turn-boundary refresh propagates tools but not the rebuilt system prompt — **CLOSED 2026-09-04**: the DRIFT-033 prerequisite this row named as blocking landed, and the refresh itself now uses it. `AgentSession` carries the split slots (`base_system_prompt: Mutex<String>` / `system_prompt_override: Mutex<Option<String>>`, `crates/cyrup-session-svc/src/session/mod.rs:169,180`) with `effective_system_prompt()` resolving `override.unwrap_or_else(base)` (`session/accessors.rs:175-179`). `PolicyHooks::prepare_next_turn` now populates the previously-unset slot: `update.system_prompt = Some(session.effective_system_prompt());` (`crates/cyrup-session-svc/src/hooks.rs:269`), read AFTER `next_turn_tools()` per the in-source note so a late-registered tool's snippet is in the resolved prompt sent with it — exactly the ordering and mechanism the Fix specified. |
...
| ~~EXT-039~~ | ~~medium~~ **CLOSED 2026-09-04** | not-ported | M | Extension shortcuts bypass the reserved-keybinding refusal, emit no conflict diagnostics, and lose to built-ins — **CLOSED 2026-09-04 in `9c25f603`** (code; this row in the follow-up docs commit). The 2026-08-14 partial closure landed the registry half (`resolve_shortcuts`, `RESERVED_KEYBINDINGS_FOR_EXTENSION_CONFLICTS`, `build_builtin_keybindings`, `shortcut_diagnostics`) and left three things, all now done. **(1) A production caller.** `App::install_extension_shortcuts(&ExtensionHost)` (`crates/cyrup-tui/src/app/shell.rs`) is pi's `setupExtensionShortcuts`, whose first statement is `extensionRunner.getShortcuts(this.keybindings.getEffectiveConfig())` (`modes/interactive/interactive-mode.ts:2078-2079` @v0.84.4). It runs at boot (`crates/cyrup/src/interactive.rs`, after the `keybindings.json` merge) and on a session swap (`crates/cyrup-tui/src/app/run_arms.rs`) — pi's own two call sites, `:1981` and `:5990`. Its input is the new `App::effective_keybindings` (`KeybindingsManager.getEffectiveConfig()`): pi keeps ONE manager over every definition, cyrup splits the same ids across a table per focus context, so the whole is the concatenation of six `Keymap::effective_config()`s plus the alt-screen table, each built from a new `id()` inverse of the existing `from_id` (`crates/cyrup-tui/src/keymap.rs`). Its output is the new `ExtensionRegistry::resolve_shortcut_specs` / `ExtensionHost::resolve_shortcut_specs` — `getShortcuts`'s real return value, whole `ExtensionShortcut` records (`extensions/types.ts:1611-1616`) rather than bare owners, because `/hotkeys` renders the same map the editor consults (`interactive-mode.ts:6364-6377`), so a refused key is also a key `/hotkeys` cannot advertise. **(2) Precedence inverted** at `crates/cyrup-tui/src/app/input.rs`: the extension tier resolves BEFORE the global `app.*` keymap (still after the selector / overlay / loader guards, matching pi's focus model — `onExtensionShortcut` hangs off the editor). **The `## Coverage` §Inferred caveat is DISCHARGED, not carried:** the dispatch site was read this pass — `CustomEditor.handleInput` opens with `// Check extension-registered shortcuts first` / `if (this.onExtensionShortcut?.(data)) { return; }` (`modes/interactive/components/custom-editor.ts:31-34` @v0.84.4), ahead of `app.clipboard.pasteImage`, `app.interrupt`, `app.exit` and the rest of the action table. Inverting is only safe because the gate refuses the reserved keys, which is why (1) had to land with it. **(3) Diagnostics surfaced.** `cyrup_tui::shortcut_diagnostics` (`crates/cyrup-tui/src/startup.rs`) projects each `ExtensionConflict` as a WARNING and the panel builder appends them to the same `[Extension issues]` block the load failures use — that builder is `cyrup_tui::StartupReport::from_session` (`crates/cyrup-tui/src/startup.rs:179`) since `605f483c` (TUI-N02) moved it out of `crates/cyrup/src/interactive.rs`, where it was the private `build_startup_report` this row was written against; `rg build_startup_report` finds no definition at HEAD, after them — pi's own order and severity (`interactive-mode.ts:1872-1892`; `{type: "warning", …}` at `runner.ts:549-553`). **Correction to this row's residual text:** they cannot go into `AgentSessionServices::startup_diagnostics.extensions` as it asked — that vector is built in the session builder, before any keybindings exist; the panel is the join point, exactly as upstream. Upstream re-verified at the latest tag: the four rules and the reserved list are UNCHANGED v0.83.0 → v0.84.4, only the line numbers moved (`runner.ts:544-586`, reserved list `:72-90` — declared at `:72` with entries `:73-90`; this row and the code comments first said `:71-89`, off by one, corrected 2026-09-05 — `getShortcutDiagnostics` `:589-591`). **Tests**, red-before proven by restoring `app/input.rs` from HEAD and re-running: `crates/cyrup-tui/src/tests/extension_shortcut.rs` `a_non_reserved_builtin_key_goes_to_the_extension` (red `Dequeue`, green `ExtensionShortcut("alt+up")`; it REPLACES `builtin_bindings_win_over_extension_shortcuts`, which asserted the pre-item ordering `custom-editor.ts:31-34` refutes), `a_reserved_builtin_key_is_refused_at_install_and_diagnosed` (red `ExtensionShortcut("ctrl+c")`; keeps the ungated `shortcut_specs()` install beside it as the contrast), `effective_keybindings_spans_every_reserved_namespace_and_follows_a_rebind`; `crates/cyrup-tui/src/tests/startup_resources_panel.rs` `shortcut_conflict_warnings_join_the_extension_issues_block`; `crates/cyrup-ext/src/tests/payload_and_seam_parity.rs` `resolve_shortcut_specs_keeps_descriptions_and_drops_the_refused_key`. The FLUX-004 comments that cited this residual as open are updated in the same commit. **RESIDUAL, low, area 07:** pi re-runs `setupExtensionShortcuts` inside `/reload` (`interactive-mode.ts:5990`); cyrup's `apply_lifecycle_outcome` (`crates/cyrup-tui/src/app/channels.rs`) reloads the keymaps but holds no host handle, so a `/reload` whose only change is `keybindings.json` re-gates via the session-swap arm rather than in pi's order. **Two deltas the closure did not declare, added 2026-09-05 in the review pass.** (a) — recorded here and as a `[CYRUP-DELTA]` on the code — `Action::id()` (`crates/cyrup-tui/src/keymap.rs:305-321`) emits `app.pageUp` / `app.pageDown`, which are cyrup inventions — upstream's `AppKeybindings` lists every `app.*` id at `core/keybindings.ts:14-57` @v0.84.4 and has no page-scroll entry (pi's page ids are all `tui.*`: `packages/tui/src/keybindings.ts:21-22`, `:40-41`, `:45-46`); `Action::from_id` already recorded the invention as TUI-028, but the `id()` inverse feeds `effective_keybindings()`, so an extension binding `pageup` earns an `[Extension issues]` warning pi would never emit. Non-reserved, so the key still reaches the extension: one extra warning, not a routing change. (b) `App::install_extension_shortcuts` replaces the installed set unconditionally where pi early-returns on `if (shortcuts.size === 0) return;` (`interactive-mode.ts:2080`) before re-attaching `onExtensionShortcut` — after a `/reload` that removes the last extension pi keeps firing the stale closure and cyrup stops. cyrup's behaviour is the better one; it was simply not recorded. (b) is recorded only here: `crates/cyrup-tui/src/app/shell.rs` was being edited concurrently by another batch-3 item when this pass ran, and staging a comment into it would have swept a foreign change into this commit. |
| ~~EXT-076~~ | ~~medium~~ **CLOSED 2026-09-05** | parity-bug | S | **A non-lowercase extension shortcut key is normalized on install but dispatched by exact match, so its handler is never found** — filed 2026-09-05 by the batch-3 ledger pass, both sides read, against the code `EXT-039` landed (`9c25f603`); the batch-3 second review raised it and the batch ended without answering it. **cyrup** `ExtensionRegistry::register_shortcut` (`crates/cyrup-ext/src/registry.rs`) stores the author's key VERBATIM, but `resolve_shortcuts_inner` emits `key.to_lowercase()` and `resolve_shortcut_specs` hands THAT to `App::install_extension_shortcuts`, so a press produces `AppAction::ExtensionShortcut(<lowercased>)` → `ExtensionHost::run_shortcut(<lowercased>)`. `ExtensionRegistry::shortcut_owner` has a case-insensitive fallback, so the OWNER still resolves and nothing fails early — but the handler lookup at the far end is an exact, case-sensitive compare: guests at `crates/cyrup-ext-sdk/src/api.rs::execute_shortcut` (`self.shortcuts.iter().find(\|s\| s.key == key)` → `Err(format!("no such shortcut: {key}"))`), natives in their own `execute_shortcut`. **Regression, not a pre-existing gap:** before `9c25f603` the production line installed the RAW keys from `shortcut_specs()`, so `"Ctrl+G"` worked. **upstream is structurally immune** — `setupExtensionShortcuts` captures the HANDLER off the normalized map and never re-looks-up by string (`modes/interactive/interactive-mode.ts:2119-2131` @v0.84.4 over `getShortcuts(...)`), so pi's `key.toLowerCase()` (`core/extensions/runner.ts`) has no far end to disagree with. cyrup dispatches by string, so normalizing at one end only is not a faithful port. **Impact** any extension registering `"Ctrl+G"`, `"Alt+Up"`, `"ctrl+shift+P"` — anything not already all-lowercase — gets an error surfaced through `shortcut_status_tx` instead of its handler. **Fix** normalize in ONE place: either lowercase inside `register_shortcut` (both maps), or dispatch the ORIGINAL registered spelling out of `resolve_shortcuts_inner`; the guest SDK lookup must agree with whichever is chosen. **Verify** the missing test is what let this land: register a mixed-case, NON-reserved key on a guest (or a native double), press it, assert the handler ran. The one mixed-case registration in `EXT-039`'s new tests uses `"Ctrl+C"`, which the reserved gate refuses, so the surviving path is never exercised at any case | **CLOSED 2026-09-05 at `79b0a578`.** `ExtensionApi::execute_shortcut` (`crates/cyrup-ext-sdk/src/api.rs`) now matches with `eq_ignore_ascii_case`, so the normalized key the host dispatches reaches a handler registered as `"Ctrl+G"`. That is upstream's own guarantee expressed at this boundary — `setupExtensionShortcuts` captures the handler off the normalized map rather than re-looking it up by the pressed key — and it changes neither the wire shape nor what the registry stores, so `shortcut_keys()` and `/hotkeys` still show the author's spelling. Rejected: normalizing in `register_shortcut` (rewrites what the author sees advertised back) and reverting EXT-039's move to normalized keys (restores the mirror-image bug, where the registry's case-insensitive owner fallback disagrees with an exact handler lookup). Test `a_shortcut_registered_with_capitals_still_dispatches_on_the_normalized_key` (`src/tests/ergonomic.rs`) fails before the change and passes after, verified by reverting only the fix. cyrup-ext-sdk 54/54; clippy clean on host and `wasm32-wasip2`; rustdoc clean. |
| ~~EXT-041~~ | ~~medium~~ **CLOSED 2026-09-05** | parity-bug | M | Replayed tool calls and results lose their extension renderer — **CLOSED 2026-09-05**, both halves. (1) The TOOL surface, which is this item's title, landed 2026-09-04 in `a0134787`: `App::replay_items_with_extensions` (`crates/cyrup-tui/src/app/session_bind.rs`) pre-resolves every replayed assistant `toolCall` through `extension_render_tool_call` and every `toolResult` through `extension_render_tool_result`, keyed by TOOL-CALL id, and the walk reads them into the two slots that were literal `None`; upstream `interactive-mode.ts:3725-3741`/`:3760`/`:3770-3775` @v0.84.4 plus `tool-execution.ts:84-101`, `:307-308`. (2) The custom-ENTRY residual landed 2026-09-05 in `104002d3` — the last open half of this row. `KnownEntry::Custom` now REACHES the replay stream: `cyrup_session::context::build_context_entries` (`crates/cyrup-session/src/context.rs`) is pi's `buildContextEntries` (`core/session-manager.ts:418-453` @v0.84.4) as its own function — `build_context_agent_messages_tagged` is now that function flat-mapped through `push_as_raw_tagged`, so the compaction-admission rule cannot drift between the message projection and the entry walk — `SessionManager::context_entries` is pi's method form (`:1277-1279`), `AgentSession::replay_items` (`crates/cyrup-session-svc/src/session/accessors.rs`) walks those ENTRIES and emits the new `ReplayItem::CustomEntry(Value)` (`session/types.rs`) carrying the serialized entry byte-identical to what the live `entry_appended` event carries, and `App::push_custom_entry` (`crates/cyrup-tui/src/app/events_fold.rs`) is pi's `addCustomEntryToChat` called by BOTH the live arm and the walk's new `ReplayItem::CustomEntry` arm, with the renderer resolved in the same pre-pass as the message and tool ones. Upstream: `renderSessionEntries` `if (entry.type === "custom") return [entry];` (`modes/interactive/interactive-mode.ts:3799-3801` @v0.84.4) out of `buildContextEntries()` (`:3910`), dispatched by `renderSessionItems` (`:3717-3719`) into the SAME method the live `entry_appended` arm calls (`:3217-3218`), body `:3570-3590` — `getEntryRenderer(entry.customType)` else nothing, `new CustomEntryComponent(entry, renderer)`, `hasContent()` else nothing, a throw ⇒ the failure box (`components/custom-entry.ts:47-52`). **Tests**, red-before established by running each pair against the rest of the change with only its own arm neutered: `crates/cyrup-session-svc/src/tests/replay_custom_entries.rs` `a_custom_entry_replays_between_the_messages_that_surround_it`, `compaction_admission_applies_to_custom_entries_too` (2/2 failed with no `CustomEntry` item in the stream); `crates/cyrup-tui/src/tests/extension_renderers.rs` `a_replayed_custom_entry_still_reaches_its_registered_renderer`, `a_replayed_unclaimed_entry_draws_what_the_live_path_draws_for_one`, `a_replayed_entry_whose_renderer_faults_still_draws_the_failure_box` (3/3 failed with the arm dropping the item). `cargo nextest run` -p cyrup-session 161 / -p cyrup-session-svc 356 / -p cyrup-tui 1425 all pass; clippy `-D warnings` and rustdoc `-D warnings` clean on all three; `cargo check -p cyrup` (the production callers, `crates/cyrup/src/main.rs` and `app/run_arms.rs`) clean. The shipped consumer that gets its card back on `/resume` is `cyrup-intercom`'s inbound-message entry renderer (`crates/cyrup-intercom/src/extension.rs:481`, appended through `append_entry` at `:414`). **No residual.** |
...
| ~~EXT-019~~ | ~~low~~ **CLOSED 2026-09-04 — residual verified gone** | not-ported | M | registerMarkdownTransformer has no counterpart (re-scoped to v0.84.1) — **CLOSED 2026-09-04**: the area-07 residual is discharged. `App::apply_markdown_transformers` (`crates/cyrup-tui/src/app/events.rs:102-165`) calls `ext_host.transform_markdown(&text, kind.as_pi_str(), is_streaming, available_width).await` for every pending committed entry plus the two live streaming partials (thinking + assistant), guarded by a sync `has_markdown_transformers()` pre-check so a session with no transformer pays only a rwlock read. It is reached from real event ingestion, not just a test: `app/events.rs:75` (the live per-event path) and `app/session_bind.rs:214` (`replay_items_with_extensions`, both production replay entry points) both call it. |
| EXT-022 | low | not-ported | M | ProviderConfig.refreshModels is not represented (re-scoped to the publish/persist contract) — **2026-08-14, still open**: sweep 2 — **RE-SCOPED from "add a marker + export" to the real v0.84.1 contract, which the item understates.** `RefreshModelsContext` is defined in packages/ai/src/models.ts:46-62 (NOT extensions/types.ts) and carries `credential`, a read-only `stored` snapshot, a generation-checked `publish(ModelsPublication): Promise<boolean>`, `allowNetwork`, `force` and an always-present `signal`; `ModelsPublication` is `{persist?: ModelsStoreEntry \| null, update?: () => void}` at :39-44. A faithful port therefore needs a `models.publish` host import and the persist/generation machinery, not just a `provider-refresh-models` export. **Consciously not started in sweep 2, and that was right** — landing the export alone would be another declared-surface-with-no-reader of exactly the EXT-023 kind. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low; nothing landed on either side, and the sweep-2 publish/persist re-scope is CONFIRMED still current rather than superseded.** cyrup: `ProviderConfig` (`crates/cyrup-ext/src/provider.rs:34-75`) has no refresh hook — the field list runs name/base_url/api/api_key/auth_header/headers/models/oauth and ends at `has_stream_simple`, exactly as filed; the WIT provider export block (`crates/cyrup-ext/wit/world.wit:222-226`) carries precisely five exports (provider-login, provider-refresh-token, provider-get-api-key, provider-modify-models, provider-stream-simple) with NO `provider-refresh-models`, and the `cyrup-ext-sdk` copy is byte-identical at the same lines — the guest export half is absent in BOTH copies. Upstream: `refreshModels?(context: RefreshModelsContext): Promise<ProviderModelConfig[]>` has DRIFTED AGAIN — `types.ts:1448` @v0.83.0 -> `:1469` @v0.84.1 -> **`:1539` @v0.85.1**. The publish/persist contract is unchanged at v0.85.1: `packages/ai/src/models.ts:39` `ModelsPublication {persist?, update?}`, `:46` `RefreshModelsContext {credential?, stored?, publish(), allowNetwork, force?, signal}`, `:55` `publish(publication): Promise<boolean>` — so the re-scoped `Fix` (this needs a `models.publish` host import plus generation machinery, not just an export) stands unchanged. The row's judgement that landing the export alone would be another declared-surface-with-no-reader is independently reinforced by the crate's own module doc: `crates/cyrup-ext/src/provider.rs:12-23` (the EXT-M07 note) records that NONE of the five existing `provider-*` exports has a production caller, so a sixth would join four dead siblings. **RE-CONFIRMED 2026-09-16 at `cc7818b` by identity:** `crates/cyrup-ext` is byte-identical across `9aeba769..cc7818b`, so the 2026-09-14 re-read's citations resolve unchanged. No re-grep performed and none owed. **RE-CONFIRMED 2026-09-24 at `ea23ca2`** — `provider.rs` untouched in the window. |
| EXT-025 | low | cyrup-original | S | reload() and four emit_* facade methods are dead code that has drifted — **2026-08-14, still open — BLOCKED, NOT UNREACHED, and the block is STRUCTURAL. RE-FILED with widened ownership: it needs ONE agent owning `crates/cyrup-ext` + `crates/cyrup-session-svc` + `crates/cyrup-it`, deleting and repointing in a SINGLE commit.** Sweeps 2 and 6 both confirmed: the preferred fix (make the live session-svc paths call the facade so there is one implementation) is entirely in cyrup-session-svc; the alternative (delete the four dead `emit_*` and `reload()`) reds tests in `crates/cyrup-it`, which are their only callers. **Sweep 6 added the fact that makes splitting it unsafe: `cyrup-it` is `required-features = ["it"]`, so the 16-second workspace gate does NOT build it — the breakage would land silently and be found by whoever next runs the gated suite.** Splitting this across three feature-agents cannot work. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low; both the dead-code claim and the DRIFT claim re-verified, the drift has WIDENED, and this row's test citations are DEAD in a way that strengthens its own structural argument.** The dead four are `emit_before_agent_start` (`crates/cyrup-ext/src/facade.rs:881`), `emit_input` (`:923`), `emit_user_bash` (`:1035`), `reload` (`:2438`). A workspace grep for `ext_host.` + those names + `emit_message_end` / `command_completions` returns exactly ONE production hit, the live exception this row already names: `crates/cyrup-session-svc/src/subscriber.rs:163` (`emit_message_end`). Every caller of the four is a test (`crates/cyrup-it/tests/ext/wasm_dispatch.rs:68,90,98,117,131,186,192`, `crates/cyrup-it/tests/ext/discover_load.rs:135`, `crates/cyrup-ext-subagents/src/tests/child_prompt_runtime_integration.rs:133`). **DRIFT WIDENED, both copies re-read rather than trusted:** the facade's `emit_user_bash` still hardcodes `exclude_from_context: false` and `std::env::current_dir()` (`facade.rs:1040-1047`), while the live inline copy `AgentSession::emit_user_bash_event` threads the caller's flag (`crates/cyrup-session-svc/src/session/bash.rs:239-242`, fed from `:203` with `options.exclude_from_context`) and the SESSION cwd (`bash.rs:253`, `self.services.cwd`) — and the live copy has since grown a whole second behaviour the facade copy lacks, the `user_bash_operations` backend resolution at `bash.rs:281-286`. The two implementations have drifted FURTHER apart, not converged. Upstream at **v0.85.1** still has exactly one emitter each and no parallel unused copy: `emitUserBash` at `runner.ts:1005` with its only callers `interactive-mode.ts:6480` and `rpc-mode.ts:564`; `emitInput` at `runner.ts:1246` with its only caller `agent-session.ts:1186`. The 2026-09-04 narrowing is re-confirmed rather than assumed: `command_completions` is genuinely live at `crates/cyrup-tui/src/app/execute_misc.rs:302`. **CITATION CORRECTION for the next pass:** `crates/cyrup-ext/tests/wasm_dispatch.rs` and `crates/cyrup-ext/tests/discover_load.rs` NO LONGER EXIST — both moved into `crates/cyrup-it/tests/ext/`. That MOVE strengthens this row's own structural argument: the ext test target is `[[test]] name = "ext"` with `required-features = ["it"]` (`crates/cyrup-it/Cargo.toml:198-200`), so 100% of the four methods' remaining callers are now behind the gate a default workspace build skips, and deleting them would break silently exactly as the row warns. **RE-CONFIRMED 2026-09-16 at `cc7818b` by identity** (`crates/cyrup-ext` unchanged in `9aeba769..cc7818b`). The recorded STRUCTURAL block is unaffected by the SCOPE sequence, which landed in `cyrup-ext-subagents`. **RE-CONFIRMED 2026-09-24 at `ea23ca2`** — `facade.rs`'s two hunks in the window are additive (`terminal_input` doc, `has_terminal_input_subscribers`); `reload`/`emit_*` unchanged and still without a production caller outside the crate. |
| EXT-026 | low — **PARTIALLY CLOSED 2026-08-14** | cyrup-original | M | A wasmtime-free cyrup-session-svc build cannot be produced — **PARTIALLY CLOSED 2026-08-14**: sweep 2 — **CONFIDENCE UPGRADED and the item re-scoped.** The item was "medium (static analysis only — no compile was run this pass either)". A compile WAS run and the cyrup-ext half was a HARD BUILD ERROR, not a warning: `LoadError` was imported under `#[cfg(feature = "wasm-host")]` while `discover_with_diagnostics` returns `Vec<LoadError>` in every build, so `cargo check -p cyrup-ext --no-default-features` failed outright — the feature had been broken for some time with nothing detecting it. Fixed; `--no-default-features --all-targets` is clean. **RESIDUAL: the item's actual target, a wasmtime-free cyrup-session-svc build (area 08).** Standing note: the no-default-features build needs a check in whatever gate the orchestrator runs, or it will rot again. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN; this row's self-declared weakness ("static analysis only — no compile was run this pass either") is now DISCHARGED BY MEASUREMENT, and the mechanism has changed for the worse.** I ran the second half of the `Verify` clause: `cargo tree -p cyrup-session-svc --no-default-features -e normal -i wasmtime` returns `wasmtime v46.0.1` reached as `cyrup-ext -> cyrup-session-svc`, plus wasmtime-wasi / wasmtime-wasi-io / wiggle. The `Verify` requirement ("the resulting dependency graph contains no wasmtime") FAILS by measurement, not by inference. **Worse by construction now:** `crates/cyrup-session-svc/Cargo.toml:53` reads `cyrup-ext = { workspace = true, features = ["wasm-host"] }` — the feature is declared HARD on the edge, so `--no-default-features` on session-svc can no longer drop wasmtime even if the workspace table were fixed; the row's original mechanism (inheriting cyrup-ext's `default = ["wasm-host"]` through an un-`default-features = false` workspace entry) is also still true at `Cargo.toml:127`. `mod host_services;` is still ungated at `crates/cyrup-session-svc/src/lib.rs:31` (row says `:29`), and `host_services.rs:17-19` still names `cyrup_ext::caps::http::HttpCaps`, `cyrup_ext::caps::proc::ProcCaps` and `cyrup_ext::host::{...}` with no cfg. The never-compiled arms have MULTIPLIED AGAIN: `cfg(not(feature = "wasm-host"))` appears SEVEN times in `facade.rs` (`:745`, `:1533`, `:1560`, `:1675`, `:2195`, `:2363`, `:2411`), against the THREE this row cites. **One half HAS genuinely improved, and it was audited rather than ignored:** `crates/cyrup-session-svc/Cargo.toml:24-28` now states in-manifest "It does NOT produce a wasmtime-free build and must not be described as one" and names EXT-026 by file and line. That closes the MIS-ADVERTISEMENT sub-claim — no manifest reader is now misled — but not the item, whose subject is that the build cannot be produced. It still cannot. No upstream side exists by construction (pi has no compile-time feature tiers), so `cyrup-original` remains the right Kind with no tag check needed. **RE-CONFIRMED 2026-09-16 at `cc7818b` by identity** (`crates/cyrup-ext` unchanged in `9aeba769..cc7818b`). **RE-CONFIRMED 2026-09-24 at `ea23ca2`** — no `Cargo.toml` feature change in `cyrup-ext`/`cyrup-session-svc` in the window. |
| EXT-027 | low | not-ported | L | pi's bundled llama.cpp router extension has no counterpart — **2026-08-14, still open**: sweep 2 — not reached; pi's bundled llama.cpp router extension, a whole extension, effort L, no work started. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low, unmoved, and this pass finally read the upstream side this row twice carried forward on stale evidence.** Upstream still ships the bundled tier at **v0.85.1**: `packages/coding-agent/src/extensions/index.ts` is four lines and reads `export const builtInExtensions: InlineExtension[] = [{ name: "llama.cpp", factory: llamaExtension, hidden: true }]`, backed by `extensions/llama/{client,huggingface,index,provider,ui}.ts`. It kept moving past this row's last measurement: `git diff --stat v0.83.0 v0.85.1 -- packages/coding-agent/src/extensions/` = 3 files, +78/-12 (client.ts +11, index.ts +13, provider.ts +66) — so the row's "+11/+28 over v0.83.0..v0.84.1" UNDERSTATES the current delta. cyrup: no bundled-extension tier exists. A grep for built_in_extensions / builtin_extensions / BUILTIN_EXTENSIONS / bundled across `crates/cyrup-ext/src/` matches only two unrelated doc comments, and every `llama` hit in the crate is a doc comment ABOUT pi, not an implementation: `crates/cyrup-ext/src/native.rs:573` (describing pi's inline-factory tier when justifying `is_ambient() -> false`), `crates/cyrup-ext/src/build/cache.rs:92-95`, `crates/cyrup-ext/src/tests/env_surface_records.rs:14`. Notably `native.rs:565-580` shows cyrup HAS thought about pi's inline tier in detail and deliberately mapped `NativeExtension` onto it — but no llama.cpp router is registered through it, so this row's "no counterpart" claim is exact. Lowest value in the area, unchanged. **RE-CONFIRMED 2026-09-16 at `cc7818b` by identity** (`crates/cyrup-ext` unchanged in `9aeba769..cc7818b`). Still effort `L`, still a whole unported extension. **RE-CONFIRMED 2026-09-24** — still no counterpart; upstream still ships the llama.cpp router at v0.87.1 (CHANGELOG 0.86.0 fixes its `enable_thinking` handling, #9528). |
...
| ~~EXT-040~~ | ~~low~~ **CLOSED 2026-08-19** | parity-bug | S | register-shortcut's description is discarded, so /hotkeys shows a raw key-id — ~~**PARTIALLY CLOSED 2026-08-14**: sweep 1 + 2 — the registry/facade half is done (`shortcut_specs` exists, and EXT-035's new test pins the description round-trip). **RESIDUAL (area 07): `cyrup-tui/main.rs` consuming `shortcut_specs`.**~~ **CLOSED 2026-08-19 — the area-07 residual is DISCHARGED and the chain is live end to end at HEAD `4fb5e40`**, found while repairing this item's `app.rs` citations. The binary consumes the rich form: `app.set_extension_shortcuts(session.services().ext_host.shortcut_specs())` (`crates/cyrup/src/main.rs:2017`, whose comment at `:2012-2016` states the choice and cites pi's `types.ts:1524-1529`), and every hop behind it carries the description — `host/live.rs:172-181` keeps `desc` and maps an EMPTY string to `None` (pi's absent field, `:179`), `ExtensionRegistry::register_shortcut` takes `description: Option<String>` (`registry.rs:833-846`) into `shortcuts: HashMap<String, (ExtensionId, Option<String>)>` (`:218`), `ExtensionHost::shortcut_specs()` (`facade.rs:1815`) hands out `(key, description)` pairs, `ShortcutSpec::from((String, Option<String>))` (`cyrup-tui/src/app/state.rs:435-439`) accepts them, and `/hotkeys` prints `spec.description.as_deref().unwrap_or(spec.id.as_str())` (`cyrup-tui/src/app/hotkeys.rs:115`). Pinned twice: `a_shortcut_description_survives_registration_and_falls_back_to_the_extension_id` (`crates/cyrup-ext/src/tests/payload_and_seam_parity.rs:501`, which also asserts pi's fallback is the extension **id**, never the key) and the EXT-040 assertion inside `ext035_a_native_registered_shortcut_actually_fires` (`crates/cyrup-ext/src/tests/native_dispatch.rs:1180-1187`). |
...
| EXT-051 | low — **PARTIALLY CLOSED 2026-08-14** | upstream-drift | S | Extension provider OAuth drifted: refreshToken gained a signal, oauth gained isSubscription — **PARTIALLY CLOSED 2026-08-14**: sweep 2 — narrowed and handed off. The signal half needs NO work (the item's own preferred fix — poll `ctx-state.is-run-cancelled` — landed with EXT-045 in sweep 1); the typed field now exists as `ProviderConfig::oauth_is_subscription()` / `ProviderRegistration::oauth_is_subscription()`. **RESIDUAL is the CONSUMER in `crates/cyrup-provider/src/auth/`, beside the existing `AuthProvider::is_subscription` overrides — re-filed under area 01.** **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN; the partially-closed status was verified in BOTH directions (the closed halves really are closed, not just claimed, and the residual really is still open).** **Signal half CONFIRMED DONE:** this row's preferred fix was to leave the export alone and let the guest poll `ctx-state.is-run-cancelled`, and that import exists end to end — `is-run-cancelled: func() -> bool;` (`crates/cyrup-ext/wit/world.wit:1133`), host impl `crates/cyrup-ext/src/host/live.rs:1411`, SDK wrapper `crates/cyrup-ext-sdk/src/ctx/base.rs:240`; the export itself is unchanged as intended (`provider-refresh-token: func(id: string, credentials-json: string) -> result<string, string>` at `world.wit:223`, both copies). **Typed-field half CONFIRMED DONE:** `ProviderConfig::oauth_is_subscription()` at `crates/cyrup-ext/src/provider.rs:92` and `ProviderRegistration::oauth_is_subscription()` at `:213`. **CONSUMER RESIDUAL CONFIRMED STILL OPEN**, by grep and by reading the one plausible consumer: `grep -rn oauth_is_subscription crates/ --include=*.rs` matches only those two definitions plus `crates/cyrup-ext/src/tests/provider.rs:167-202`, and `crates/cyrup-session-svc/src/guest_providers.rs:70 upsert_provider` takes a `&ProviderRegistration` and never reads it. The concept exists for BUILT-IN providers only — `fn is_subscription` default at `crates/cyrup-provider/src/auth/mod.rs:134` with seven overrides (`auth/oauth/anthropic.rs:691`, `auth/oauth/github_copilot.rs:813`, `auth/oauth/kimi_coding.rs:704`, `auth/oauth/openai_codex.rs:1040`, `auth/oauth/xai.rs:639`, `providers/github_copilot.rs:627`, `providers/openai_codex.rs:481`) — none of which is fed by a guest registration, so `Verify` assertion 2 ("an extension provider declaring isSubscription is described as subscription-backed in the auth listing") demonstrably fails. **`Verify` assertion 1 ALSO cannot pass, for a reason this row does not record:** `crates/cyrup-ext/src/provider.rs:12-16` states that `provider_refresh_token` has NO production caller at all (only `crates/cyrup-it/tests/ext/`), so a guest provider's refresh never runs in production and cannot be cancelled or not-cancelled. Kind `upstream-drift` RE-SETTLED AT THE TAGS per the directory rule: at v0.83.0 `refreshToken(credentials: OAuthCredentials)` is one-argument (`types.ts:1458`) and `isSubscription` is absent; at v0.84.1 both are present (`:1481`, `:1475`); at v0.85.1 they persist (`:1551`, `:1545`) — post-baseline, classification stands. **RE-CONFIRMED 2026-09-16 at `cc7818b` by identity** (`crates/cyrup-ext` unchanged in `9aeba769..cc7818b`). **RE-CONFIRMED 2026-09-24 at `ea23ca2`** — OAuth surface untouched in the window. |
| EXT-053 | low | parity-bug | S | An extension command shadowing a built-in is dropped from autocomplete with no diagnostic — **2026-08-14, still open**: sweep 2 — not reached (cyrup-tui). **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low, unmoved; the closing seam (EXT-S02 / EXT-017) that could plausibly have fixed this in passing was read rather than assumed, and it did not.** `CommandRegistry::with_dynamic` has MOVED to `crates/cyrup-tui/src/commands.rs:336-372` (this row cites `:166-178`) and still does exactly what the row describes: it builds `let existing: HashSet<String>` from the builtin names and filters with `.filter(\|c\| !existing.contains(c.name.as_ref()))` at `:347`, then returns `Self`. The signature is `-> Self` — there is no channel for a dropped name to escape on, so the drop cannot be reported even in principle without the signature change the `Fix` proposes. The panel the `Fix` would reuse exists and is live — `StartupDiagnostics` is constructed at `crates/cyrup-session-svc/src/builder.rs:1309` and fed extension load diagnostics at `:1409` and `:1449` — and receives nothing from this path. The comment at `:337-339` still cites pi's suffixing rule as justification while implementing neither suffix nor warning, exactly as filed. The live caller is now `crates/cyrup-tui/src/app/execute_misc.rs:149` (the row's `:282-287` is now `command_completions`, a different call). **This row's forward-looking claim that EXT-017's closure makes the second message form accurate is NOT yet realised here:** `with_dynamic` still keys off `c.name`, not an invocation name — `invocation_name` in `crates/cyrup-tui/src/commands.rs` matches only a doc comment at `:71`. Upstream unchanged at **v0.85.1**: `getBuiltInCommandConflictDiagnostics` at `packages/coding-agent/src/modes/interactive/interactive-mode.ts:619`, with BOTH message forms verbatim at `:628` and `:629`, pushed into the startup panel at `:1831`. Upstream still warns; cyrup still does not. **RE-RUN 2026-09-16 at `cc7818b`** (not carried forward: this row's fix site is `crates/cyrup-tui`, one of the three crates that changed in `9aeba769..cc7818b`). The `#138` cyrup-tui commits touched `keymap.rs`, `app/execute_misc.rs`, `app/submit.rs`, `app/input.rs`, `app/action.rs`, `app/backend.rs`, `error.rs` and `editor/wrap.rs` — the autocomplete/command-collision path is NOT among them, and `crates/cyrup-tui/src/commands.rs` is unchanged in this window. Still open, unchanged. **RE-CONFIRMED 2026-09-24 at `ea23ca2` by identity** — `crates/cyrup-tui/src/commands.rs` unchanged in `9aeba769..ea23ca2`. |
...
| EXT-S04 | low | not-ported | M | ctx.compact's onError path has no observable counterpart — **2026-08-14, still open**: sweep 2 — not reached: the assertActive-on-emit refusal stays unimplemented because `ExtensionHost::reload` is still production-dead (EXT-025). **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN; this pass traced the cyrup failure path END TO END rather than resting on the grep the 2026-09-04 note used, and the open half is now nailed to a specific line.** The closed half is real: `customInstructions` is ported — `crates/cyrup-ext/wit/world.wit:1156` `compact: func(opts-json: string) -> result<_, string>`, host import `crates/cyrup-ext/src/host/live.rs:1271-1284` parsing `customInstructions` and issuing `ControlOp::Compact`; note the row's `world.wit:558` / `host/live.rs:829` / `session.rs:2862` citations have all drifted. **THE OPEN HALF, LOCATED:** the `result<_, string>` the guest awaits is ONLY the ENQUEUE result — `live.rs:1282` returns `guest.services.control(ControlOp::Compact { custom_instructions })`, which goes to the sink at `crates/cyrup-session-svc/src/host_services.rs:1579` and returns as soon as the op is queued. The actual compaction runs later, in the control drain at `crates/cyrup-session-svc/src/session/control.rs:139-141`, and its error is SWALLOWED: `control.rs:188-190` does `if let Err(e) = outcome { self.report_control_failure(name, &e); }`, and `report_control_failure` at `:287-289` is a bare `tracing::warn!`. Nothing reaches the guest, no event is dispatched, no return value carries it. So this row's claim — the guest can observe success (`session_compact`, `world.wit:537`) and cannot observe failure — is exact, and the `Fix`'s preferred return-value route is still available, because the error text genuinely is being thrown away at a single identifiable line. `grep -rn session_compact_failed crates/` across the whole workspace returns zero. Upstream: the shape the 2026-09-04 note found at v0.84.4 SURVIVED to the current tag — `SessionCompactFailedEvent` at `packages/coding-agent/src/core/extensions/types.ts:618` (`type: "session_compact_failed"` at `:619`), in the `SessionEvent` union at `:678`, and on `ExtensionAPI.on` at `:1271` @v0.85.1. Kind `not-ported` RE-CONFIRMED rather than reclassified to drift: the gap was filed against `onError`, which exists at v0.83.0 (`interactive-mode.ts:1819-1829`), and `SessionCompactFailedEvent` is absent from `types.ts` at BOTH v0.83.0 and v0.84.1 (grep count 0), so the event is a confirming target, not the origin of the gap. **Ownership note for a next pass:** `03-cyrup-session.md`'s `SESS-050` is a second id on this same gap; `08-cyrup-session-svc-and-modes.md:110-122` already says this row owns it and that a second id must not be filed — the live question there is duplication, not the Kind field. **RE-CONFIRMED 2026-09-16 at `cc7818b` by identity** (`crates/cyrup-ext` unchanged in `9aeba769..cc7818b`). **RE-CONFIRMED 2026-09-24 at `ea23ca2`** — `ctx.compact` path untouched in the window. |
...
| EXT-064 | medium — **PARTIALLY CLOSED 2026-08-15** | cyrup-original | M | `ui.set-header`/`set-footer` take a `string` where pi takes a component factory — **PARTIALLY CLOSED 2026-08-15 (verified at HEAD, the row was stale)**: the `[CYRUP-DELTA]` the item asked for IS at the signature in both `world.wit` copies, naming BOTH collapses (factory→string, and `undefined`→`""` for "restore the built-in") against `types.ts:183-187`/`:190` @v0.83.0. **RESIDUAL, unchanged and stated in that note: `ReadonlyFooterDataProvider` (`types.ts:179-181`) still has no analog**, so an extension footer cannot render the git branch or other extensions' `setStatus` segments. Needs a data channel (a `ui.footer-data` import or extra `set-footer` parameters) and should batch with EXT-006's render-path widening — an import addition, which per EXT-061's finding needs no `HOST_WORLD` bump. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at medium; the 2026-08-15 partial closure is CONFIRMED by audit (what it does, not merely that it exists) and the residual is CONFIRMED STILL OPEN and has GROWN.** Comment half genuinely done: `crates/cyrup-ext/wit/world.wit:735-746` carries the `[CYRUP-DELTA]` at the signature naming both collapses (factory-to-string at `:735`, and the empty-string-restores overload), and the SDK copy is byte-identical at the same lines (`crates/cyrup-ext-sdk/wit/world.wit:735-746`) — so the "both copies" condition the closure was granted on actually holds; the note even states the residual against itself at `:740-744`. Surface half still absent: `set-footer: func(content: string);` at `world.wit:746` (both copies) takes no data parameter, and a search for any footer data channel across `crates/cyrup-ext` and `crates/cyrup-ext-sdk` (footer_data / footer-data / FooterData) matches ONLY that comment — no `ui.footer-data` import, no extra parameter, no host-side provider. cyrup does have the raw ingredient host-side (`GuestState::set_status` at `crates/cyrup-ext/src/host/services.rs:2126`, logged at `:1513`, imported at `world.wit:664`) but it is write-only from the guest's side; nothing reads the segments back out to a footer factory. **THE RESIDUAL HAS WIDENED — this is the finding of the re-audit.** The item was filed against a TWO-member provider (the git branch plus the setStatus segments). At **v0.85.1** it is FOUR members: `export type ReadonlyFooterDataProvider = Pick<FooterDataProvider, "getGitBranch" \| "getExtensionStatuses" \| "getAvailableProviderCount" \| "onBranchChange">` at `packages/coding-agent/src/core/footer-data-provider.ts:385-388`. Two of those — `getAvailableProviderCount` and the `onBranchChange` SUBSCRIPTION — are not covered by this row's `Fix` at all, and `onBranchChange` in particular is a PUSH channel, not a poll, so "a `ui.footer-data` import returning json" as written would not satisfy it; the `Fix`'s surface half should be re-scoped to four members including a subscription. **CITATION WARNING:** the upstream file MOVED (`modes/interactive/` -> `core/`), which is why `git cat-file -e v0.85.1:packages/coding-agent/src/modes/interactive/footer-data-provider.ts` FAILS — a next pass greping the old path would wrongly conclude the type was deleted. Kind `cyrup-original` re-settled at the baseline tag: `ReadonlyFooterDataProvider` is imported at `types.ts:53` and used at `:185` @v0.83.0, so the collapse was made at port time. Severity `medium` stands. **RE-READ 2026-09-16 at `cc7818b`. Disposition UNCHANGED (still partial, the `ReadonlyFooterDataProvider` half still open), but every WIT line number in this row and in its body is stale by ~177 lines and the `Fix`'s first half is DONE — the body still asks for work that has landed.** `crates/cyrup-ext` did not change in `9aeba769..cc7818b`, so this drift predates this window; it is corrected here because a `Fix` that asks for a comment already written is how a scheduled agent burns a pass. **Comment half — DONE, in BOTH copies, verified by opening each rather than by grep count:** the `[CYRUP-DELTA]` sits at `crates/cyrup-ext/wit/world.wit:735-744` and `crates/cyrup-ext-sdk/wit/world.wit:735-744`, identical text, and it names all three collapses the `Fix` enumerated — the factory→string flattening, the empty-string-restores overload (`:739`, *"`string` has no such value here, so the EMPTY STRING carries 'restore the built-in'"*), and the dropped `footerData` argument (`:740-742`, citing `:179-181`). It even names the remaining half and its routing (`:743-744`, *"needs a data channel (a `ui.footer-data` import or extra `set-footer` parameters) and should batch with EXT-006's render-path widening"*) — i.e. the WIT now carries this row's own `Fix` text. **Citations to repair:** `world.wit:568-569` → **`:745-746`** (`set-header`/`set-footer`, both copies); the `Impact`'s *"the WIT itself carries **no** delta note at `:566-570` — only a citation-correction comment"* is now **FALSE** and is the sentence most likely to be re-derived. **Surface half — still fully open, re-measured:** `git grep -in 'footer.data\|ReadonlyFooterData' -- crates/cyrup-ext/` returns only the two lines of that comment (`world.wit:741`, `:743`) in each copy — no import, no host impl, no SDK surface. An extension footer still cannot read the git branch or any `setStatus` segment. **Effort drops from M to S-plus-M-residual in practice:** what is left is the export re-signing (`HOST_WORLD` bump) and the data channel, with the documentation work already banked. **RE-CONFIRMED 2026-09-24 at `ea23ca2`** — both `world.wit` copies unchanged in `9aeba769..ea23ca2`; still no footer-data channel. |
...
## ~~EXT-003~~ — ~~Project-trust store is unwired at the extension seam~~ **CLOSED 2026-09-04**

> **CLOSED 2026-09-04 — both halves are wired at HEAD; the body below is kept for traceability and is STALE from its
> first sentence.** The store half closed earlier on 2026-09-04 (the paragraph that follows this block, kept verbatim).
> The `with_wasm` ordering residual it left behind was fixed the same day by `605f483c` — a commit whose subject and
> body announce TUI-N02 and name EXT-003 nowhere except an inline code comment, so this section went on advertising a
> residual that no longer existed. Caught by the batch-3 review; struck 2026-09-05.
>
> **What is true at HEAD.** `pre_trust_extension_verdict` (`crates/cyrup-session-svc/src/builder.rs:2254`) takes
> `force_wasm_failure: bool` and resolves the host through `pre_trust_wasm_host` (`:2357-2370`); on `Err` it logs a
> `tracing::warn!` and continues with `ExtensionHost::new(host_config)` (`:2293-2302`) instead of returning `None`, so
> the native `decides_project_trust` loop at `:2314-2321` still votes and `aggregate_project_trust` (`:2343`) still
> sees the result. The wasm discovery scan is the only thing skipped (`if wasm_ready`, `:2329`), because on a
> runtime-less host every `load_wasm_with_caps` returns `ExtError::WasmHostDisabled` and the scan would buy a
> directory walk to produce nothing but errors.
>
> **Upstream, read at v0.84.4 rather than carried from the v0.83.0 filing below.** pi has no runtime to construct, but
> it has the matching partial-failure rule and it is the opposite of fail-everything:
> `ResourceLoader.loadProjectTrustExtensions()` (`core/resource-loader.ts:380-386`) returns a `LoadExtensionsResult`
> whose per-extension faults live in `errors`, and `emitProjectTrustEvent` iterates `extensionsResult.extensions` —
> the ones that DID load — pushing handler faults onto its own `errors` and continuing
> (`core/extensions/runner.ts:204-233`; loop head `:210`, `catch` `:223-230`). Part of the extension subsystem failing
> never silences the part that still works; here the natives are exactly that part. The store half's citations move
> the same way at this tag: persist `core/project-trust.ts:65-67`, read-back `:72-75`.
>
> **Test.** `a_dead_pre_trust_wasm_runtime_does_not_discard_native_votes`
> (`crates/cyrup-session-svc/src/tests/project_trust_extension.rs:474`). The failure is staged with the
> `cfg(test)`-only `SessionBuilder::force_pre_trust_wasm_failure` (`builder.rs:445`, `:519-522`): engine construction
> cannot be made to fail on demand from inside the process, and the production trigger — the pooling allocator failing
> to reserve its slabs (`cyrup-ext/src/host/engine.rs:19-26`, the reason `build_engine_on_demand` exists) — is a
> property of the machine. RED before the fix (`asked == 0`; the tiers fall through to the prompt, whose `yes` trusts
> the project), GREEN after (the native votes `no` and the prompt never runs). Production compiles the helper down to
> the bare `with_wasm` call.
>
> **Residual, low, already filed as its own item:** EXT-043 — the `project_trust` event carries no `cwd`, so the
> deciding extension has no well-defined key for the `remember` this item now persists.

**Superseded text follows.** **PARTIALLY CLOSED 2026-09-04 — the store is wired; the `with_wasm` ordering residual survives.** `SessionServiceBuilder::trust_store()` is called with a real `TrustStore` in production (`crates/cyrup/src/session_launch.rs:163`), `build()` reads it back into `TrustInputs.saved` and persists a `remember`-ed extension verdict, exactly as the Fix's first sentence asked. `pre_trust_extension_verdict`'s `ExtensionHost::with_wasm(host_config).ok()?` (`builder.rs:2165`) still discards every native vote on a wasm-construction failure instead of falling back to `ExtensionHost::new`, per the Fix's second sentence — unchanged.

**Kind** not-ported · **Severity** ~~medium~~ · **Effort** M · **Confidence** high · **Status** **CLOSED 2026-09-04**
...
## ~~EXT-006~~ — ~~medium~~ **CLOSED 2026-09-04** — Renderers run without display options or theme, and only once

**CLOSED 2026-09-04 in `1739fcc4`.** Both residuals landed; what follows is the state at closure, then the original finding.

**What landed.** (1) **Signature.** `render-call`/`render-result` take a third parameter `opts-json` in **both** `world.wit` copies, and the package moved `cyrup:ext@0.8.0` → `0.9.0` with `HOST_WORLD` (`crates/cyrup-ext/src/manifest.rs`, whose history block records the bump; `wit_world_sync.rs` ties the two copies and the constant together). The bag is `cyrup_ext::RenderOptions` (`crates/cyrup-ext/src/render.rs`) — `{expanded, outputPad, isPartial, theme}` — the union of pi's three option bags, because cyrup keeps ONE export pair keyed by an opaque custom-type and tells the surfaces apart by the host's registry table. Two CYRUP-DELTAs are written into the world file and the type: the theme crosses as a NAME (a `Theme` object cannot cross a component boundary; a guest reads the palette with `ui.theme-get-json`, EXT-066), and native extensions get the real `&dyn RenderTheme` through the richer `RenderedComponent`/`RenderCtx` tier. Native extensions widen **additively**: `NativeExtension::render_{call,result,entry}_under` default to the existing two-argument methods, so no in-tree native extension re-signs anything. (2) **One-shot.** `App::refresh_extension_renders` (`crates/cyrup-tui/src/app/events.rs`) re-invokes every renderer whose text was produced under display inputs that no longer hold, called from `dispatch_run_action` (`app/run_action.rs` — Ctrl+O, `/theme`, `/settings`, a selector, an extension shortcut) and from `ingest_session_event_owned` (an extension handler moving the inputs from an event).

**Design decision (recorded per the batch-3 rule).** The invariant is *rendered text is only valid for the display inputs it was produced under*, and it first becomes known at the moment of the call. `Rendered::Text(String)` became `Rendered::Text(RenderedText)`, pairing the flattened rows with a **private** `Option<Box<RenderSource>>` — `{surface, key, payload, under}` — built in exactly one place, `run_renderer`, from the call it has just made. So a `Rendered::Text` cannot exist without recording its own provenance, and `TranscriptView::stale_extension_renders` is a pure comparison over the view with no clock, no host and no I/O (functional core; the async shell is `refresh_extension_renders`). What is now impossible: producing extension-rendered text with unrecorded inputs, and deciding staleness from anything but the recorded options. What still needs runtime tests: that the refresh is *called* from the two run-loop seams, that the guest wire really carries the bag, and that a re-render replaces rather than duplicates a row. **Review correction 2026-09-05 (code `824a539e`): the first of those three was NOT covered when this closed** — the commit body of `1739fcc4` said "All three are covered below", but every test named calls `refresh_extension_renders` directly, so nothing would have noticed either arm dropping the call and the renderers silently going back to one-shot. Two source guards now cover it, the way TUI-004 and TUI-N02 cover theirs: `cyrup-tui/src/tests/extension_renderers.rs::the_action_arm_still_refreshes_extension_renders` (reads `app/run_action.rs`'s `dispatch_run_action`) and `::the_events_arm_still_refreshes_extension_renders` (reads `app/events.rs`'s `ingest_session_event_owned`, and additionally pins that the refresh runs AFTER the fold so the row the event just created is itself refreshable). Both were verified to FAIL when their call site is deleted. Read from source because neither arm is constructible here — `dispatch_run_action` takes a `RunCtx` owning a runtime, an event stream and nine channels. Alternatives rejected: a `renders_dirty` flag on `AppState` (the TUI-N02 "nine other things and never this one" failure, and it cannot express per-row staleness — a tool result's `isPartial` is a property of the row, not the frame); a typestate `Fresh`/`Stale` (the state is decided by external events and inspected dynamically per row, which is the enum/data case, not the typestate case); widening `NativeExtension`'s three existing methods (same behaviour, but re-signs a trait implemented in four other crates for extensions that never read the options); leaving the options to the ambient `ui.get-tools-expanded`/`ui.theme-get-json` imports (not what upstream does, racy against the render, and still one-shot). Migration cost: `RenderedText` replaced `String` at three producers and five readers, all inside `cyrup-tui`; the boxed source keeps `Entry`'s largest variant inside clippy's `large_enum_variant` ratio; the host's five `render_*` methods and `LiveExtension::render_{call,result}` gained one parameter (call sites: `cyrup-tui` and four `cyrup-it` tests). No serde format and no persisted shape changed.

**Residual (low, named rather than filed as a new id).** The refresh covers rows still addressable — the active tool runs and the committed-but-unflushed entries. A row already flushed to native scrollback cannot be repainted by anyone under R-ARCH-TUI-003; that is the same boundary `set_pending_markdown` works within, and it is why the tool surface (where `Ctrl+O` actually means something) is the one that moves. The retained altscreen `document` is deliberately not re-invoked: a toggle would fan out to every extension-rendered row in it, each behind the 2 s `EXTENSION_RENDER_TIMEOUT`. Also unchanged: `cyrup-ext-subagents`' `render_result` still draws pi's COMPACT tier only — now a choice (it can override `render_result_under`) rather than a limit, and its doc comment says so.

**DOCUMENTATION HALF OF THE ABI BUMP — missing at closure, landed 2026-09-05 in `824a539e`.** This item took `HOST_WORLD` `cyrup:ext@0.8 -> 0.9` and DRIFT-004 took it `0.9 -> 0.10`, and neither updated the author-facing documents. Since `ExtensionManifest::check_world` requires `gmaj == hmaj && gmin >= hmin` (`crates/cyrup-ext/src/manifest.rs`), the sample manifest the shipped guide told authors to copy — `"world": "cyrup:ext@0.8"` at `docs/guide/extensions/authoring.md:104`, with the rule restated at `:125-126`, plus `docs/guide/extensions/overview.md:27` and `README.md:68` — named a world the host **REFUSES** with `ExtError::WorldVersion`. All four are corrected, together with `manifest.rs`'s own `e.g. cyrup:ext@0.8` doc example. Nothing caught it because the in-tree fixtures interpolate `HOST_WORLD`; the new `cyrup-ext/src/tests/wit_world_sync.rs::the_authoring_docs_declare_the_current_host_world` ties every `cyrup:ext@…` literal in those three documents to `HOST_WORLD` (MAJOR.MINOR, the pair `check_world` compares), so a future bump either updates them or fails — verified RED against the pre-fix `@0.8` sample.

**Verified** — `cyrup-tui tests::extension_renderers::{a_renderer_receives_the_display_options_and_the_theme, toggling_expansion_re_invokes_the_renderer_with_the_new_options, switching_the_theme_re_invokes_the_renderer, an_unchanged_frame_does_not_re_invoke_anything, staleness_is_derived_from_the_options_the_text_was_produced_under}`, `cyrup-ext render::tests::*`, `cyrup-ext-sdk tests::ergonomic::{a_renderer_sees_the_display_options_and_the_theme_name, the_hosts_opts_json_parses_into_the_guest_mirror}`, and the guest end-to-end `cyrup-it ext::wasm_renderer_routing::a_guest_renderer_sees_the_display_options_and_the_theme` (the in-tree SDK example's `DemoToolRenderer::render_result` branches on `opts.expanded`, `DemoEntryRenderer` names `opts.theme`). The three App-level tests were re-confirmed red by stubbing `refresh_extension_renders` to an early return on the finished branch. **They prove the FUNCTION works, not that it is called** — the two run-loop call sites are covered by the source guards added 2026-09-05 (see the design block above). **One further review note, checked and NOT a defect:** `App::render_options` builds `is_partial: false` while `stale_extension_renders` wants `partial(!run.done)`, which looks like it would schedule an immediate wasted re-invocation per tool row. It cannot: `TranscriptView::push_tool_end_rendered` is the ONLY writer of `ToolRun::rendered_result` and sets `done = true` in the same statement, so a result render always exists under `done` — i.e. under `is_partial == false`. The invariant is now written at `app/events.rs::render_options`. `cyrup-it --features it` 494/494.
...
**cyrup** — the replay half for custom messages is now closed (`replay_session_with_extensions`, `crates/cyrup-tui/src/app/session_bind.rs:145-167`, two production callers). Two residuals remain. (1) **Signature**: `render-call: func(custom-type: string, call-json: string) -> option<string>` (`crates/cyrup-ext/wit/world.wit:169`) carries neither display options nor a theme, and `render-result` matches. (2) **One-shot**: the render is computed once at ingest — `let rendered = extension_render(ext_host, ev).await;` at `app/events.rs:30`, fed to `ingest_event_rendered` at `:42` — so the text is baked and an expand/collapse or a theme change never re-invokes the renderer. The third residual, replayed **tool** rows, is filed separately as EXT-041.

**upstream** — `MessageRenderer = (message, options, theme) => Component | undefined` at `pi/packages/coding-agent/src/core/extensions/types.ts:1146`, with `EntryRenderOptions {expanded}` at `:1142` @v0.83.0; pi re-invokes the renderer from the draw path, so options and theme are live inputs rather than ingest-time constants. **Re-read at v0.84.4 for the port**: `MessageRenderer` is `:1213-1217`, `EntryRenderer` `:1219-1223`, `ToolDefinition.renderCall`/`renderResult` `:491-498`; `setToolsExpanded` re-broadcasts the flag to every child on every toggle (`modes/interactive/interactive-mode.ts:4032-4048`) and `:3437` seeds it into a freshly added `CustomEntryComponent`. **Tag-to-tag difference**: at the ported baseline v0.83.0 the MESSAGE surface took the same `EntryRenderOptions {expanded}` the entry surface takes — `MessageRenderOptions` and its `outputPad` field (`:1195-1199`) are a post-baseline addition, carried here because ADR-0006 makes the latest tag the parity target.
...
**CLOSED 2026-09-04 — the corrected fix site (session-svc + tui, one commit) landed in `dd69fea`.** `AgentSession::slash_command_catalog` now emits an `argumentCompletions` presence bit for extension rows (`crates/cyrup-session-svc/src/session/commands.rs:174-278`), sourced from an opt-in `(owner, name)` table both tiers write; `crates/cyrup-tui/src/commands.rs:570-595` reads it into `ArgumentCompleter::Extension`, and the async fetch reaches `ExtensionHost::command_completions` from `crates/cyrup-tui/src/app/execute_misc.rs:282-287`.
...
**CLOSED 2026-09-04 in `75532cee`.** The TUI consumer the Fix called "the part that matters most" landed; the other two pieces were already in at HEAD (via `6d805fed`) and are re-verified here. **Expressible:** `render-shell: option<string>` / `constrained-sampling: option<string>` on `tool-descriptor` in both WIT copies (`crates/cyrup-ext/wit/world.wit:113,122`, `crates/cyrup-ext-sdk/wit/world.wit:113,122`), lowered by `lower_tool_descriptor` under the EXT-023 exhaustiveness guard (`crates/cyrup-ext-sdk/src/guest.rs:78-91`), stored on the host descriptor (`crates/cyrup-ext/src/registry.rs:44,54`), answered by `WasmTool::render_kind`/`constrained_sampling` (`crates/cyrup-ext/src/host/live.rs:2127-2132,2140-2142`); `constrainedSampling` is consumed provider-side (PROV-011, `cyrup_core::Tool::constrained_sampling` at `crates/cyrup-core/src/tool.rs:226`). **Consumed:** `cyrup_session_svc::ToolInfo.render_kind` (`crates/cyrup-session-svc/src/tools.rs:41,223`, `#[serde(skip)]`) is pi's `getToolDefinition(name).renderShell`; `AppState::known_tool_definitions: HashMap<String, ToolRenderKind>` (`crates/cyrup-tui/src/app/state.rs:391`) is filled by the live lookup (`app/events.rs:216-221`) and the bind refresh (`app/session_bind.rs:159`) and read into `ToolRun::definition: Option<ToolRenderKind>` (`transcript/entry.rs:283`) by both writers (`app/events_fold.rs:191`, `app/session_bind.rs:370`); `tool_lines` resolves `getRenderShell()`'s three-way `??` (`transcript/tool_render.rs:111-121`) and `self_rendered_lines` (`:163-195`) draws the `"self"` branch of `tool-execution.ts:237-259` @v0.84.4 — bare rows at column 0, no state tint, nothing when empty, the built-in `edit`'s own `Box(1, 1, getEditHeaderBg)` drawn around its rows (`:172-178`) so its output is byte-identical. Verify line satisfied: `crates/cyrup-tui/src/tests/tool_render_shell.rs` registers a native `Tool` declaring `SelfRendered` on a real session and asserts the drawn row carries no `toolPendingBg` and no padding column, beside a `Default` control that keeps both (red-before: 2 of 3 failed at HEAD). Design decision recorded in the commit body: one `Option<ToolRenderKind>` answers both `hasRendererDefinition()` and `getRenderShell()` rather than a `bool` + kind pair. **Residual (low, area 07):** pi's edit `renderResult` places its diff in a separate unboxed `Container` (`edit.ts:446-454`); cyrup keeps it inside the header box — a built-in-renderer fidelity gap, pre-existing and out of this item's scope. Falsification: `grep -rn render_kind crates/cyrup-tui/src` returning zero, or the e2e test's `SelfRendered` row drawing at column 1, reopens this.

**Git-history note (review fix 2026-09-04).** The landing commit `75532cee` and its docs commit `651bac70` do not build when checked out alone. The code commit staged whole files rather than hunks, and two of them — `crates/cyrup-tui/src/app/state.rs` and `crates/cyrup-tui/src/tests/mod.rs` — carried the concurrent TUI-037 track's in-progress edits: `git show 75532cee:crates/cyrup-tui/src/tests/mod.rs` has `mod reload_implicit_trust;` at line 98 while `git ls-tree 75532cee crates/cyrup-tui/src/tests/reload_implicit_trust.rs` is empty (E0583 on any `cargo test -p cyrup-tui` at that sha), and the swept `AppState::auto_trust_on_reload_cwd` field's doc comment links `App::set_auto_trust_on_reload_cwd`, which does not exist until the very next commit `0e8c62fa` (TUI-037; broken intra-doc link under `RUSTDOCFLAGS='-D warnings'`). The checks quoted in this closure (1354 passed, rustdoc clean) were run on the shared working tree, which already held the TUI-037 files — they describe the state `0e8c62fa` commits, not `75532cee` in isolation. From `0e8c62fa` onward every commit builds and is green (the batch-2 review verified this at `89b82681`: `cargo nextest run -p cyrup-tui` 1372 passed / 1 skipped, `cargo clippy --all-targets -- -D warnings` and `RUSTDOCFLAGS='-D warnings' cargo doc` clean). The two shas were not rewritten — the brief forbids rebasing the shared tree — so a bisect landing on `75532cee` or `651bac70` should step forward to `0e8c62fa`. Also corrected here: the `state.rs` citation for `known_tool_definitions` read `:384` when the row was written and is `:391` at HEAD after TUI-037's `auto_trust_on_reload_cwd` (`:228`) and TUI-081's `pending_import` (`:283`) landed above it.
...
**CLOSED 2026-09-04.** DRIFT-033's two-slot model landed (`base_system_prompt`/`system_prompt_override` on `AgentSession`, `crates/cyrup-session-svc/src/session/mod.rs:169,180`, resolved by `effective_system_prompt()`), and `PolicyHooks::prepare_next_turn` now re-pushes it: `update.system_prompt = Some(session.effective_system_prompt());` (`crates/cyrup-session-svc/src/hooks.rs:269`), read after `next_turn_tools()` so a late tool's snippet is in the same turn's resolved prompt — the exact split and ordering the Fix specified.
...
**Verify** — point `DiscoveryRoots.configured` at (a) a nonexistent path and (b) an existing non-extension file; assert each produces exactly one `errors` entry naming the path, and that the already-closed `.wasm` case still loads. Extend `crates/cyrup-ext/src/tests/loader_direct_file.rs`.
...
## ~~EXT-039~~ — ~~medium~~ **CLOSED 2026-09-04** — Extension shortcuts bypass the reserved-keybinding refusal, emit no conflict diagnostics, and lose to built-ins
...
> **CLOSED 2026-09-04** in `9c25f603`. Everything after this block is the filing text and
> describes HEAD as it was before the 2026-08-14 partial closure; the *Caveat* at the end is
> discharged, see below.
>
> **What was already in** (2026-08-14, sweep 1, re-verified in sweep 2): the registry gate itself —
> `ExtensionRegistry::resolve_shortcuts`, `RESERVED_KEYBINDINGS_FOR_EXTENSION_CONFLICTS`,
> `build_builtin_keybindings` and `shortcut_diagnostics` (`crates/cyrup-ext/src/registry.rs`), a
> rule-for-rule port of `getShortcuts`. That sweep also **rewrote this item's precedence claim**:
> the Fix below asked for two extensions on one key to stop "silently collapsing", but pi is
> LAST-wins there — `extensionShortcuts.set(normalizedKey, shortcut)` runs unconditionally after
> the warning, and the warning's own text is `Using ${shortcut.extensionPath}`. Only the DIAGNOSTIC
> was missing. It also corrected the Fix's placement: pi gates in `getShortcuts` against the
> RESOLVED keybindings, not in `registerShortcut`, which is why `register_shortcut` is documented
> in-source as deliberately ungated.
>
> **What landed now** — the three things the residual named.
>
> 1. **A production caller.** Nothing had ever called `resolve_shortcuts`; the two install sites
>    passed `ExtensionHost::shortcut_specs()`, the raw per-extension table.
>    `App::install_extension_shortcuts(&ExtensionHost)` (`crates/cyrup-tui/src/app/shell.rs`) is
>    pi's `setupExtensionShortcuts`, whose first statement is
>    `const shortcuts = extensionRunner.getShortcuts(this.keybindings.getEffectiveConfig());`
>    (`modes/interactive/interactive-mode.ts:2078-2079` @v0.84.4). It runs at boot
>    (`crates/cyrup/src/interactive.rs`, deliberately AFTER the `keybindings.json` merge) and on a
>    session swap (`crates/cyrup-tui/src/app/run_arms.rs`) — pi's own two call sites, `:1981` and
>    `:5990`.
> 2. **Its input.** `App::effective_keybindings` is `KeybindingsManager.getEffectiveConfig()`. pi
>    keeps one manager over every registered definition, so its config is naturally whole; cyrup
>    splits the same ids across a binding table per focus context, so the whole is the
>    concatenation of `Keymap`, `SelectKeymap`, `TreeKeymap`, `SessionKeymap`, `ModelsKeymap`,
>    `EditorKeymap` and `AltScreenKeymap`'s new `effective_config()`. Each is built by a new
>    `id()` — the inverse of the `from_id` each action enum already had
>    (`crates/cyrup-tui/src/keymap.rs`). `AutocompleteKeymap` is deliberately excluded and says why
>    in-source: upstream has no `tui.autocomplete.*` family, the popup reuses `tui.select.*` and
>    `tui.input.tab`, so every id it could contribute is already contributed.
> 3. **Its output.** `ExtensionRegistry::resolve_shortcut_specs` (and the facade twin) is
>    `getShortcuts`'s real return value — whole `ExtensionShortcut` records
>    (`extensions/types.ts:1611-1616` @v0.84.4) rather than the bare owner
>    `resolve_shortcuts` yields — because pi renders the SAME map in `/hotkeys`
>    (`const description = shortcut.description ?? shortcut.extensionPath;`,
>    `interactive-mode.ts:6364-6377`). That is what makes a refused key un-advertisable, which is
>    the "listed by `/hotkeys` as if live" half of the Impact. Both shapes project one private
>    `resolve_shortcuts_inner`, so the rules and the diagnostics cannot drift, and
>    `resolve_shortcuts`'s signature is unchanged.
> 4. **Precedence inverted** at `crates/cyrup-tui/src/app/input.rs`: the extension tier resolves
>    BEFORE the global `app.*` keymap, still after the selector / overlay / loader guards — which
>    matches pi's focus model, since `onExtensionShortcut` hangs off the EDITOR and cannot preempt
>    a focused picker there either. Safe only because the reserved keys are now refused upstream of
>    it.
> 5. **Diagnostics.** `cyrup_tui::shortcut_diagnostics` (`crates/cyrup-tui/src/startup.rs`)
>    projects each `ExtensionConflict` as a WARNING and the panel builder — `build_startup_report`
>    (`crates/cyrup/src/interactive.rs`) when this was written, `cyrup_tui::StartupReport::from_session`
>    (`crates/cyrup-tui/src/startup.rs:179`) since `605f483c` (TUI-N02) gave the panel its second
>    call site — appends them to the same `[Extension issues]` block the load failures use, after them — pi's order and severity (`interactive-mode.ts:1872-1892`;
>    `{type: "warning", message, path: extensionPath}` at `runner.ts:549-553`).
>    **Correction to the residual's wording:** they cannot be threaded into
>    `AgentSessionServices::startup_diagnostics.extensions`, because that vector is assembled in
>    the session builder, before a TUI or a keybinding config exists. The panel is the join point,
>    which is exactly where upstream joins them.
>
> **The *Caveat* below is DISCHARGED, not carried.** pi's dispatch site was read this pass:
> `CustomEditor.handleInput` opens with `// Check extension-registered shortcuts first` /
> `if (this.onExtensionShortcut?.(data)) { return; }`
> (`modes/interactive/components/custom-editor.ts:31-34` @v0.84.4), ahead of
> `app.clipboard.pasteImage`, `app.interrupt`, `app.exit` and the rest of the action table. The
> inference was correct. `## Coverage` §Inferred is updated.
>
> **Upstream at the latest tag.** The four rules, the reserved list and the diagnostics accessor
> are UNCHANGED v0.83.0 → v0.84.4; only line numbers moved (`getShortcuts` `runner.ts:544-586`,
> `buildBuiltinKeybindings` `:95-114`, `RESERVED_KEYBINDINGS_FOR_EXTENSION_CONFLICTS` `:72-90`,
> `getShortcutDiagnostics` `:589-591`).
>
> **Tests** — red-before proven by restoring `crates/cyrup-tui/src/app/input.rs` from HEAD and
> re-running: `crates/cyrup-tui/src/tests/extension_shortcut.rs`
> `a_non_reserved_builtin_key_goes_to_the_extension` (red `Dequeue`, green
> `ExtensionShortcut("alt+up")`), `a_reserved_builtin_key_is_refused_at_install_and_diagnosed` (red
> `ExtensionShortcut("ctrl+c")`), `effective_keybindings_spans_every_reserved_namespace_and_follows_a_rebind`;
> `crates/cyrup-tui/src/tests/startup_resources_panel.rs`
> `shortcut_conflict_warnings_join_the_extension_issues_block`;
> `crates/cyrup-ext/src/tests/payload_and_seam_parity.rs`
> `resolve_shortcut_specs_keeps_descriptions_and_drops_the_refused_key`. The first REPLACES
> `builtin_bindings_win_over_extension_shortcuts`, which asserted the pre-item ordering that
> `custom-editor.ts:31-34` refutes — it was rewritten to the corrected contract, not removed.
>
> **RESIDUAL, low, area 07.** pi re-runs `setupExtensionShortcuts` inside `/reload`
> (`interactive-mode.ts:5990` @v0.84.4, right after `this.keybindings.reload()`); cyrup's
> `apply_lifecycle_outcome` (`crates/cyrup-tui/src/app/channels.rs`) reloads the keymaps but holds
> no extension-host handle, so a `/reload` whose only change is `keybindings.json` re-gates via the
> session-swap arm rather than in pi's order. Fix: give that arm the host, or move the
> `reload_keybindings_in` step beside the swap rebind.

**cyrup** — registration has no gate at all: `ExtensionRegistry::register_shortcut` (`crates/cyrup-ext/src/registry.rs:515-523`) is a bare `g.shortcuts.insert(key.into(), owner)` — no lowercase normalization, no comparison against the keymap, no diagnostic, and two extensions claiming the same key silently collapse to whichever registered last. `ExtensionHost::shortcut_keys()` (`crates/cyrup-ext/src/facade.rs:1233-1235`) hands the raw keys to the TUI, which parses and stores them (`set_extension_shortcuts`, `crates/cyrup-tui/src/app/state.rs:375-381`, wrapped by `App::set_extension_shortcuts` at `app/shell.rs:124-129`). Dispatch consults the **built-in** action table first (`return self.apply_action(action)` at `app/input.rs:82`) and extension shortcuts only afterwards (`:86-93`), with an in-tree comment stating that as the intent. There is no counterpart to `getShortcutDiagnostics` anywhere in `crates/`.
...
## EXT-076 — A non-lowercase extension shortcut key is normalized on install but dispatched by exact match, so its handler is never found

**Kind** parity-bug · **Severity** medium · **Effort** S · **Confidence** confirmed (both sides read at HEAD `ba380e58` / `v0.84.4`)
**Filed** 2026-09-05 by the batch-3 ledger pass. **Provenance:** raised by the batch-3 second review of `EXT-039` and left unanswered — the four commits after that review (`9de4254b`, `0a697d62`, `ce81dba9`, `824a539e`) touch no shortcut path. A defect in the code `EXT-039` landed, not a re-opening of `EXT-039`, whose own subject (the reserved-key gate and the precedence inversion) is genuinely closed.

**cyrup** — the shortcut key exists as three stringly-typed copies with three different normalization policies:
* `ExtensionRegistry::register_shortcut` (`crates/cyrup-ext/src/registry.rs`) stores the key **verbatim** in `shortcuts` and `shortcut_order`.
* `ExtensionRegistry::resolve_shortcuts_inner` emits `let normalized = key.to_lowercase();` and pushes THAT; `resolve_shortcut_specs` hands the normalized ids to `App::install_extension_shortcuts`, which stores them as `ShortcutSpec.id`.
* On a press `crates/cyrup-tui/src/app/input.rs` returns `AppAction::ExtensionShortcut(spec.id.clone())` and `app/run_action.rs` spawns `ExtensionHost::run_shortcut(key)` (`crates/cyrup-ext/src/facade.rs`). `ExtensionRegistry::shortcut_owner` DOES have a case-insensitive fallback, so the owner resolves and nothing fails earlier — which is exactly why the defect is silent. The **handler** lookup is then an exact, case-sensitive compare: `crates/cyrup-ext-sdk/src/api.rs::execute_shortcut` is `self.shortcuts.iter().find(|s| s.key == key)` falling to `Err(format!("no such shortcut: {key}"))`, and a native's own `execute_shortcut` compares its constant the same way (the trait default in `crates/cyrup-ext/src/native.rs` likewise errors).

**upstream** — `modes/interactive/interactive-mode.ts:2119-2131` @v0.84.4: `setupExtensionShortcuts` captures the HANDLER off the map `extensionRunner.getShortcuts(...)` returns and calls `shortcut.handler(createContext())`. pi never re-looks-up by string, so its `key.toLowerCase()` in `core/extensions/runner.ts` has no far end to disagree with. **Normalizing at one end only is not a faithful port of `key.toLowerCase()` when the other end dispatches by string.**

**Regression, not a pre-existing gap.** Before `9c25f603` the production line installed the RAW keys from `ExtensionHost::shortcut_specs()` (`crates/cyrup/src/interactive.rs` at `9c25f603^` — `app.set_extension_shortcuts(session.services().ext_host.shortcut_specs())`), so `"Ctrl+G"` reached its handler.

**Impact** — an extension registering any spelling that is not already all-lowercase (`"Ctrl+G"`, `"Alt+Up"`, `"ctrl+shift+P"`) had a working shortcut before `EXT-039` and now gets `Err("no such shortcut: ctrl+g")` surfaced through `shortcut_status_tx` instead of its handler.

**Fix** — normalize in ONE place. Either lowercase inside `register_shortcut` so both maps and the guest's own `Shortcut.key` agree, or carry the ORIGINAL registered spelling out of `resolve_shortcuts_inner` and dispatch that. The guest SDK lookup must agree with whichever is chosen; a `KeyId` newtype whose only constructor normalizes would make the third copy unrepresentable, and is the design opportunity `EXT-039`'s own design note missed (it reasoned about `Action::id()`, whose id is a compile-time constant per variant, and explicitly ruled out a newtype "for the binding id" — true of the ACTION id, false of the SHORTCUT key, which crosses three boundaries).

**Verify** — the ABSENT test is what let this land: `resolve_shortcut_specs_keeps_descriptions_and_drops_the_refused_key` (`crates/cyrup-ext/src/tests/payload_and_seam_parity.rs`) is the only mixed-case registration in `EXT-039`'s new tests and it registers `"Ctrl+C"`, a RESERVED key the gate refuses — so the surviving path is never exercised at any case. Register a mixed-case, non-reserved key on a live guest (or a native double), press it, and assert the handler ran.


## ~~EXT-041~~ — ~~Replayed tool calls and results lose their extension renderer~~ **CLOSED 2026-09-05**

**Kind** parity-bug · **Severity** medium · **Effort** M · **Confidence** high

> **CLOSED 2026-09-05 — both halves, no residual.** The TOOL surface (this item's title) closed
> 2026-09-04 in `a0134787`; the custom-ENTRY half that block left open closed 2026-09-05 in
> `104002d3`. Everything after these two blocks is the filing text and describes HEAD as it was
> before `a0134787`.
>
...
>
> **The residual this block used to name — replayed custom ENTRIES — CLOSED 2026-09-05 in
> `104002d3`.** It was the Fix's last sentence ("do the same for replayed custom entries via
> `has_entry_renderer`/`render_entry`") and EXT-012's hand-off, and it was not a rendering seam:
> `KnownEntry::Custom` never entered the replay stream at all, because `ReplayItem` was pi's
> `RenderSessionItem` "minus its `custom` entry variant" and `push_as_raw`
> (`crates/cyrup-session/src/context.rs`) has no arm for it — so there was nothing for the TUI to
> ask a renderer about, and `cyrup-intercom`'s inbound card (`extension.rs:481`, appended through
> `append_entry` at `:414`) vanished on `/resume`.
>
> **What landed, three layers.** (1) `cyrup-session`: `build_context_entries` (`context.rs`) is pi's
> `buildContextEntries` (`core/session-manager.ts:418-453` @v0.84.4) as its own function — the
> compaction at the head, the entries kept from `firstKeptEntryId`, then everything after — and
> `build_context_agent_messages_tagged` is now that function flat-mapped through
> `push_as_raw_tagged`, so the admission rule cannot drift between the message projection and the
> entry walk; `SessionManager::context_entries` (`manager/context.rs`) is pi's method form
> (`:1277-1279`). (2) `cyrup-session-svc`: `ReplayItem::CustomEntry(Value)`
> (`session/types.rs`) is the `custom` arm of pi's `RenderSessionItem` union
> (`interactive-mode.ts:216`), and `AgentSession::replay_items` (`session/accessors.rs`) walks the
> context ENTRIES — emitting the serialized entry for a `KnownEntry::Custom` and flat-mapping
> `raw_context_messages` for everything else, which reproduces `build_context_raw_tagged` exactly.
> (3) `cyrup-tui`: `App::push_custom_entry` (`app/events_fold.rs`) is `addCustomEntryToChat`, now
> called by BOTH the live `entry_appended` arm and the walk's new `ReplayItem::CustomEntry` arm
> (`app/session_bind.rs`), with the renderer resolved in the same pre-pass as the message and tool
> ones — a fourth `ReplayRenders` map, keyed by ITEM index.
>
> **Upstream, at the latest tag.** `renderSessionEntries` passes a custom entry through
> unprojected — `if (entry.type === "custom") return [entry];`
> (`modes/interactive/interactive-mode.ts:3799-3801` @v0.84.4) — out of
> `this.sessionManager.buildContextEntries()` (`:3910`); `renderSessionItems` dispatches it with
> `if (isCustomSessionEntry(item)) this.addCustomEntryToChat(item)` (`:3717-3719`) into the SAME
> method the live `entry_appended` arm calls (`:3217-3218`); the body (`:3570-3590`) is
> `getEntryRenderer(entry.customType)` else nothing, `new CustomEntryComponent(entry, renderer)`,
> `hasContent()` else nothing, and a throw becomes the failure box
> (`components/custom-entry.ts:47-52`).
>
> **Design decisions** (recorded in the commit body). The item travels as the SERIALIZED ENTRY,
> byte-identical to what the live `entry_appended` event carries, so the front-end's live and replay
> arms are ONE arm and a renderer sees one payload shape; a typed `{custom_type, data}` pair would
> have been a second wire format for the same thing. `ReplayRenders.entries` is keyed by ITEM index,
> not entry id: pi resolves this renderer inside the walk while holding the entry, so there is no
> upstream key to mirror, and the item's position is the one address that exists whatever the
> serialized entry carries. The three-state outcome is carried WHOLE (`Rendered`, not flattened
> text) because `Failed` is pi's own third answer for this surface and must not read as "no renderer
> claimed the type" — which is exactly the collapse X15 was filed for.
>
> **Tests** — red-before established by running each pair against the rest of the change with only
> its own arm neutered. `crates/cyrup-session-svc/src/tests/replay_custom_entries.rs`:
> `a_custom_entry_replays_between_the_messages_that_surround_it` (the entry replays in branch order
> between the two turns, carrying `type`/`customType`/`data`/`id`),
> `compaction_admission_applies_to_custom_entries_too` (a custom entry before `firstKeptEntryId` is
> admitted no more than the message beside it, while the kept and post-compaction ones survive) —
> 2/2 failed with no `CustomEntry` item in the stream. `crates/cyrup-tui/src/tests/extension_renderers.rs`:
> `a_replayed_custom_entry_still_reaches_its_registered_renderer`,
> `a_replayed_unclaimed_entry_draws_what_the_live_path_draws_for_one` (the mirror: the receipt, no
> renderer consulted), `a_replayed_entry_whose_renderer_faults_still_draws_the_failure_box` — 3/3
> failed with the arm dropping the item. `cargo nextest run` -p cyrup-session 161 / -p
> cyrup-session-svc 356 / -p cyrup-tui 1425 all pass; clippy `-D warnings` and rustdoc `-D warnings`
> clean on all three; `cargo check -p cyrup` (the production callers, `crates/cyrup/src/main.rs` and
> `app/run_arms.rs`) clean. **Falsification:** load an extension registering an entry renderer,
> append a custom entry, resume the session, and find the `entry appended → …` receipt instead of
> the card — the first TUI test above is that observation.
...
**Verify** — load two projects in one process with a native that decides trust; assert its handler receives each project's own cwd and that a per-directory allowlist yields different verdicts. Extend the trust assertions in `crates/cyrup-ext/src/tests/aggregation.rs`.
...
> **CLOSED 2026-08-14 at both ends. Everything below is the filing text and no longer describes
> HEAD** — re-verified 2026-08-19 while repairing this section's `app.rs` citation. The WIT is now
> `set-widget: func(key: string, content-json: option<string>, opts-json: string);`
> (`crates/cyrup-ext/wit/world.wit:648`), with the before/after and the reason for each of the three
> arguments recorded in place at `:639-647`. The TUI half landed as `TUI-014`: the single
> `AppState::extension_widget` slot is now the keyed collection `extension_widgets`
> (`app/state.rs:227`); the `UiEffect::SetWidget` arm removes-then-inserts by key
> (`app/extension_ui.rs:336-346`) and an empty `lines` REMOVES rather than re-mounts (`:343-345`),
> which is exactly the `{"key": …, "content": null}` case the paragraph below says stayed occupied;
> and there are real readers — `widget_rows` (`app/layout.rs:30-36`) sizes the above/below bands and
> `render_extension_widgets` (`app/render.rs:211`) draws them from two call sites (`:142`, `:145`).
> `UiEffect::SetWidget`'s own doc (`crates/cyrup-session-svc/src/host_services.rs:160-167`) now names
> pi's three keys instead of admitting the collapse.

**cyrup** — `set-widget: func(widget-json: string);` (`crates/cyrup-ext/wit/world.wit:326`) — one opaque payload where pi takes three arguments. The host forwards it verbatim (`crates/cyrup-ext/src/host/live.rs:245-250`) into `UiEffect::SetWidget { widget: Value }` (`crates/cyrup-session-svc/src/host_services.rs:154`), whose own doc at `:148-153` **admits** the collapse and says there is "no cyrup-side convention to re-derive them from". The TUI then stores the whole blob in one slot (`AppState::extension_widget`, assigned in the pre-split `app.rs` at the arm that is `crates/cyrup-tui/src/app/extension_ui.rs:336` today). A de-facto convention already exists and is unenforced: the shipped subagents extension hand-rolls `{"key": FLEET_STATUS_WIDGET_KEY, "content": null}` to mean *clear* (`crates/cyrup-ext-subagents/src/extension.rs:9489-9492`, `:9979-9982`, the latter commented "pi `ctx.ui.setWidget(FLEET_STATUS_WIDGET_KEY, undefined)`") — and that clear still leaves `extension_widget = Some({key, content: null})`, not `None`.
...
**Verify** — an extension blocking **every** call in a two-call batch with terminate: assert the agent loop ends after the batch. Assert that blocking only one of two does **not** terminate (upstream's `every()` rule). Add beside the fail-closed assertions in `crates/cyrup-ext/src/tests/ext_fail_closed.rs`.
...
**CLOSED 2026-09-04 — residual verified gone.** `App::apply_markdown_transformers` (`crates/cyrup-tui/src/app/events.rs:102-165`) calls `ExtensionHost::transform_markdown` for every pending committed entry and both live streaming partials, reached from the live per-event path (`app/events.rs:75`) and the replay path (`app/session_bind.rs:214`) — the two production callers this row's residual needed.
...
**2026-09-04 — narrowed, not closed.** `ExtensionHost::command_completions` (`facade.rs:1895`) is no longer test-only: `crates/cyrup-tui/src/app/execute_misc.rs:282-287` now calls `session.services().ext_host.command_completions(&name, &argument)` from the live extension slash-argument-completion path landed with EXT-013. The other four (`emit_before_agent_start`, `emit_input`, `emit_user_bash`, `reload`) are unchanged — `grep -rn 'ext_host\.\(reload\|emit_before_agent_start\|emit_input\|emit_user_bash\)(' crates/ --include=*.rs` is still empty outside tests — so the "two implementations of the same seam" defect and the structural blocker (needs one owner spanning cyrup-ext + cyrup-session-svc + cyrup-it) both still hold for the remaining four.
...
**Fix** — widen `shortcuts` to `HashMap<String, (ExtensionId, Option<String>)>` (`registry.rs:144`), thread `desc` through `register_shortcut` (`registry.rs:515`) and the host import (`host/live.rs:98`), and add `shortcut_specs() -> Vec<(String, Option<String>)>` on the facade beside `shortcut_keys()` (`facade.rs:1233`). The TUI already accepts the richer form — `ShortcutSpec`'s `From<(String, Option<String>)>` impl at `crates/cyrup-tui/src/app/state.rs:435-439` exists for precisely this — so the only other edit is `crates/cyrup/src/main.rs:2017`. *(The pre-split citation this replaced pointed at the `/hotkeys` label block rather than at the impl — it was wrong before the split, not because of it.)* Fall back to the extension **id**, not the key, when `desc` is empty. Land with EXT-039 and EXT-035, which rewrite the same registrar.

**Verify** — a guest registering `ctrl+t` with description "Toggle X": assert `/hotkeys` renders `Toggle X` in the Action cell. Extend `crates/cyrup-tui/src/tests/extension_shortcut.rs`.
...
**Verify** — register an extension command named `settings`; assert it is absent from `/` autocomplete **and** that exactly one `[Extension issues]` warning naming it is shown. After EXT-017, assert it is reachable as `/settings:2` and that the message says so. Extend `crates/cyrup-tui/src/tests/commands.rs`.
...
**Verify** — two extensions registering a renderer for the same tool name: assert the first-loaded one renders and that exactly one conflict diagnostic names both. Add beside the renderer assertions in `crates/cyrup-ext/src/tests/entry_renderer.rs`.
...
**2026-09-04 — upstream independently confirms the Fix's event-route shape, not yet ported.** `v0.84.4` (past this file's swept `v0.84.1` baseline) adds exactly the event this item's Fix proposed: `SessionCompactFailedEvent {type: "session_compact_failed", reason: "manual"|"threshold"|"overflow", errorMessage?: string, aborted: boolean, willRetry: boolean, fromExtension: boolean}` (`pi/packages/coding-agent/src/core/extensions/types.ts:617-629` @v0.84.4), added to `SessionEvent` (`:678`) and `ExtensionAPI.on` (`:1271`). `grep -rn session_compact_failed crates/cyrup-ext/` is still empty at HEAD — not ported, severity/impact unchanged (this is the SAME gap, now with an upstream-confirmed shape to port against rather than a design to invent).

## EXT-059 — `AgentSession::load_wasm_extension` is a full-authority, manifest-less load reachable as public API

**CLOSED 2026-09-04.** `caps: &cyrup_ext::Capabilities` is now a required parameter (`crates/cyrup-session-svc/src/session/accessors.rs:399-407`), routed to `load_wasm_with_caps`, with an in-source note recording the manifest-less behaviour it replaced.
...
> **HEADER CORRECTION 2026-09-14 (re-audit at `9aeba769`) — the line above is the STALE half; both status tables are authoritative.** This item's current classification is **Kind `product-decision` · Severity `low`**, re-rated 2026-08-13 when the DEFECT classification was refuted (see `## Open items` and the status table). The header line is left unedited as the record of what it said at filing; do not quote its `cyrup-original` / `medium` when counting or scheduling — `scripts/count_open_items.py` reads the table, not this line.
...
## EXT-077 — pi v0.86.0 made `user_bash` fail closed; cyrup's `emit_user_bash` still fails open and accepts any handled value
...
**cyrup** — `crates/cyrup-ext/src/facade.rs::ExtensionHost::emit_user_bash` (`:1035-1057`) runs
`dispatch_block_mutate` and maps `Reduced::Pass` → `UserBashReduction::Continue` (execute locally).
In `crates/cyrup-ext/src/dispatch.rs::Dispatcher::block_mutate_chain` (`:432-…`) a contained fault
(returned error, trap, panic, budget timeout, (de)serialization failure) fails CLOSED only when
`EventKind::fails_closed()` — which is `tool_call` alone (`crates/cyrup-ext/src/event.rs:177-179`);
every other kind, `user_bash` included, "degrades to no-mutation and the chain continues" (the
comment at `dispatch.rs:462-463`, which cites pi's per-handler `catch { continue }`). A handled result
is carried open-shaped as `UserBashReduction::Handled(Value)` (`facade.rs:61-71`) with no check that it
holds exactly one of `operations`/`result`.

**upstream** — `packages/coding-agent/src/core/extensions/runner.ts` @v0.87.1: `emitUserBash`
(`:1154-1183`) returns on the first handler whose result is not `undefined`, but first validates it
with `isUserBashEventResult` (`:136-…`: exactly one of `operations` — an object with an `exec`
function — or `result`), throwing `"Invalid user_bash handler result: …"` otherwise; the `catch`
reports via `emitError` and then **re-throws** (`:1177`). At v0.85.1 the same function (`:1005-1032`)
caught, reported and continued to the next handler, and accepted any truthy result. `types.ts`
@v0.87.1 narrows `UserBashEventResult` to that exclusive union (`:1229-…`). coding-agent CHANGELOG
0.86.0 *Breaking*: "`user_bash` now fails closed: errors or invalid defined results abort the command
without invoking later handlers or executing locally" (#9068).

**Impact** — An extension that routes `!` commands somewhere other than the host (a container, a
remote box, a policy gate) and faults: pi v0.86+ runs nothing; cyrup runs the command on the host.
The user typed the command, so this is not a model-driven permission bypass — hence medium, not
critical — but it is a sandbox escape of the user's own command on exactly the path a routing
extension exists to guard, and it is silent apart from the error report. A malformed handled value
(both keys, neither, a non-callable `operations`) is accepted rather than refused.

**Fix** — Add `EventKind::UserBash` to `fails_closed()` (the `Blocked` arm then carries
"Extension failed, blocking execution: …"; check the TUI/RPC callers render it as an aborted command,
not as a block reason), and validate `Handled` in `emit_user_bash` — exactly one of
`operations`/`result` — mapping an invalid value to the same aborted outcome. Stop the chain on the
first fault (pi does not invoke later handlers).

**Verify** — A native `user_bash` handler that panics: the command is not executed and the error is
reported; a handler returning `{operations, result}` is refused; a handler returning `undefined`
still falls through to local execution.

---

## EXT-078 — pi v0.87.0 made `turn_end` actionable and added `agent_before_settle`; cyrup's `on-turn-end` is notify-only

**Kind** upstream-drift · **Severity** low · **Effort** M · **Confidence** confirmed

> **Filed 2026-09-24.** Kind settled by presence: `agent_before_settle` occurs 0 times in `types.ts`
> at v0.86.1 and 2 at v0.87.0. Depends on `AGENT-038` (`finishTurn`'s `continue`) and `SESS-052`
> (the `context_edit` draft).

**cyrup** — `crates/cyrup-ext/wit/world.wit:459`
`on-turn-end: func(turn-index: u32, message-json: string, tool-results-json: string);` — no return
value; `HostEvent::TurnEnd { turn_index, message, tool_results }` (`crates/cyrup-ext/src/event.rs:340-344`);
no `agent_before_settle` anywhere in `crates/cyrup-ext`, either WIT, `crates/cyrup-ext-sdk` or
`crates/cyrup-session-svc/src` (grep, `ea23ca2`).

**upstream** — `extensions/types.ts` @v0.87.1: `TurnEndEvent extends BoundaryState` (`:853-…`) and
gains `messageEntryId`/`toolResultEntryIds`; `BoundaryState` carries `entries`, `continue`, a
`context` preview (`contextEntries`, `contextMessages`, `llmMessages`, `pendingMessages`,
`canContinue`) and `outcome`; `AgentBeforeSettleEvent extends BoundaryState` (`:818-…`) "Fired before
final settlement. May append entries and ensure one next provider request"; handlers return
`BoundaryResult { entries?, continue? }` (`:812-…`) whose drafts are `custom`, `custom_message`,
`context_edit` or `compaction` (`SessionBoundaryDraft`, `:791-…`). `runner.ts::emitBoundary`
(`:928-980`) folds results across handlers, rebuilding the preview after each and discarding
everything if any draft set fails to build. `ExtensionRunner.emit()` no longer accepts `turn_end`
(CHANGELOG 0.87.0 *Breaking*).

**Impact** — A cyrup guest cannot persist structural entries at a turn boundary in order, cannot
request "one more provider request" at `turn_end` or before settlement, and cannot see the entry ids
of the turn it observes. Capability absence; nothing wrong happens.

**Fix** — After `AGENT-038`: a new `on-turn-end` shape returning a boundary result (WIT export
addition plus a `HOST_WORLD` consideration, per `EXT-061`'s finding), an `on-agent-before-settle`
export, the draft type, and a session-svc `emit_boundary` that appends drafts through
`SessionManager` and feeds `continue` into `finish_turn`.

**Verify** — A guest returning `{continue: true}` from `turn_end` on a tool-less final turn causes
exactly one more assistant request; a `custom` draft lands in the session file before the next turn's
entries.

---

## EXT-079 — `context_with_system` (pi v0.87.0) has no counterpart

**Kind** upstream-drift · **Severity** low · **Effort** S (after `AGENT-039`) · **Confidence** confirmed

> **Filed 2026-09-24.** `context_with_system` occurs 0 times in `types.ts` at v0.86.1 and 2 at
> v0.87.0.

**cyrup** — One request-time transform: `on-context: func(messages-json: string) -> hook-outcome;`
(`crates/cyrup-ext/wit/world.wit:406`), dispatched as `EventPatch::Context`. The transcript it sees
has no system messages because cyrup has none (`AGENT-039`), so the `context` half of the upstream
change is already equivalent by construction.

**upstream** — `runner.ts::emitContext` @v0.87.1 (`:1190-…`): `context` handlers see the conversation
with system messages filtered out, and pi re-attaches the prompt and tool state after each
(`snapshotEventHandlers` loop at `:1194-…`, helper doc at `:273-280`); then every
`context_with_system` handler (`:1221-…`) sees the full transcript including system messages and its
output is sent as returned — "the handler owns the prompt and tool declarations"
(`types.ts::ContextWithSystemEvent`, `:708-…`). CHANGELOG 0.87.0 records the reason: `context`
handlers that filtered messages were dropping the prompt and tool declarations (#9789, #9822).

**Impact** — A guest cannot rewrite the system prompt or tool declarations per request. Capability
absence only; blocked until system messages exist in cyrup's transcript.

**Fix** — With `AGENT-039`: an `on-context-with-system` export dispatched after the `context` chain,
its result used verbatim.

**Verify** — A guest that appends a section via `context_with_system` changes the next request's
system prompt and nothing else.

---

## EXT-080 — pi v0.86.0's `pi.on()` returns an unsubscribe; cyrup subscriptions cannot be dropped

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed

> **Filed 2026-09-24.** Every `on(...)` overload in `ExtensionAPI` returns `void` at v0.85.1 and
> `() => void` at v0.86.0.

**cyrup** — Guest subscriptions are declared once, as a bitset, by
`registration.subscribe: func(event-kinds: list<u8>)` (`crates/cyrup-ext/wit/world.wit:627`); native
ones by `InitApi::subscribe` (`crates/cyrup-ext/src/native.rs:457`) during `NativeExtension::init`.
There is no removal call for either. The one exception is raw terminal input, which already has
`unsubscribe-terminal-input` (`world.wit:681`), and the bus, which has its own unsubscribe.

**upstream** — `extensions/types.ts` @v0.87.1: `on(event: "project_trust", …): () => void` (`:1354`)
and every sibling overload; `loader.ts` @v0.87.1 `on(event, handler): () => void` (`:256-…`,
returning the remover at `:263`). Dispatch snapshots each extension's handler list before iterating
(`runner.ts::snapshotEventHandlers`, `:265-267`), so "handlers added or removed during a dispatch
apply to later dispatches, not the current one" (CHANGELOG 0.86.0, #8967).

**Impact** — A guest that wants to stop handling an event (a one-shot `agent_end` listener, a mode
toggle) must keep its subscription and no-op in the handler, paying one guest call per event.
Capability absence; nothing wrong happens.

**Fix** — A `registration.unsubscribe: func(event-kinds: list<u8>)` import (import addition, no
`HOST_WORLD` bump per `EXT-061`) clearing bits in the registry, applied between dispatches — the
dispatcher already snapshots `subscribers_for(kind)` per dispatch, which gives pi's semantics.
...
- **EXT-017 / EXT-037 non-determinism.** Both rest on `HashMap` iteration order being observably unstable between runs. Rust's default hasher is randomly seeded per process, so this is sound in principle, but no reorder was observed — the claim is derived from `command_descriptions` iterating `.commands.iter()` (`registry.rs:662`) and that value reaching `crates/cyrup-tui/src/app/run_arms.rs:33` unsorted.
- **EXT-039's precedence inversion.** ~~pi's **editor** was not read, so "upstream lets a non-reserved extension binding win" is inferred from the reserved-list design and the warning text, not from the dispatch site.~~ **DISCHARGED 2026-09-04 in `9c25f603`** — the dispatch site was read at v0.84.4: `CustomEditor.handleInput` opens with `// Check extension-registered shortcuts first` / `if (this.onExtensionShortcut?.(data)) { return; }` (`modes/interactive/components/custom-editor.ts:31-34`), i.e. the extension tier is consulted before `app.clipboard.pasteImage`, `app.interrupt`, `app.exit` and the rest of the action table. The inference was correct; it is now evidence. `getShortcuts`, `setupExtensionShortcuts` and cyrup's ordering at `app/input.rs` were already read, and the refusal and diagnostics halves always were.
...
## EXT-075 — `ui_prompt_start`/`ui_prompt_end` (pi v0.84.4) have no counterpart, so a guest cannot tell when another extension is blocking on its own UI prompt

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** high · **filed 2026-09-04, from the `v0.84.1..v0.84.4` diff-stat skim this file's own baseline table calls for**

**cyrup** — `grep -rin "ui_prompt" crates/cyrup-ext/ crates/cyrup-ext-sdk/` returns nothing: no WIT event, no `on-ui-prompt-*` export, no `ExtensionUIContext`-wrapping mechanism of any kind. `ctx.rs`'s `Ui::select`/`confirm`/`input`/`editor`/`custom` wrappers call the host import directly.

**upstream** — landed after this file's already-swept `v0.84.1` version-lag baseline: `UIPromptStartEvent`/`UIPromptEndEvent` (`pi/packages/coding-agent/src/core/extensions/types.ts:748-753`/`:756-761` @v0.84.4), with `UIPromptKind = "select" | "confirm" | "input" | "editor" | "custom"` at `:745`. `ExtensionRunner.wrapUIPromptContext` (`runner.ts:441-450`) wraps every guest-facing `ExtensionUIContext` method — `select`/`confirm`/`input`/`editor`/`custom` — through `withUIPrompt` (`:453-476`), which tracks a re-entrancy depth (`uiPromptDepth`) so a prompt nested inside another prompt does not emit a spurious extra pair, fires `ui_prompt_start` once on the OUTER call only (`:456-458`), and `ui_prompt_end` once the outermost `run()` settles or throws (`:461-475`, in a `finally`/`catch`-and-`finish` pair so a rejected prompt still ends the window). Both events are dispatched via `queueMicrotask` (`emitUIPromptEvent`, `:482-486`), matching pi's fire-and-forget event convention elsewhere in the runner.

**Impact** — narrow: a second extension has no way to know that a UI prompt is currently blocking the interactive session (e.g. to defer its own competing prompt, or to show a passive "waiting on <ext>" indicator) — the two events exist precisely so extensions can coordinate around cyrup's/pi's single-prompt-at-a-time terminal. No existing cyrup item covers this pair; it is a clean absence, not a drifted shape.

**Fix** — add `on-ui-prompt-start: func(kind: string, title: option<string>)` and `on-ui-prompt-end: func(kind: string, title: option<string>)` to the event-catalog export list in both `world.wit` copies (an export addition ⇒ bump `HOST_WORLD`, batch with the next such bump per ADR-0002's one-bump-per-pass rule); wrap the five `ui.*` guest-facing prompt calls in `crates/cyrup-ext/src/host/live.rs` with the same outer-call-only depth tracking `withUIPrompt` uses, keyed per `GuestState` since cyrup's WASM guests do not share pi's single in-process runner instance; dispatch through the existing bus/facade emit path. `NativeExtension`'s equivalent five call sites need the same wrap for native-guest parity.

**Verify** — a guest whose `execute_command` calls `ctx.ui.select(...)`: assert exactly one `ui_prompt_start{kind:"select"}` then one `ui_prompt_end{kind:"select"}` cross the bus, and that a prompt nested inside another (if reachable) does not double-fire.
