...
| EXT-003 | medium — **PARTIALLY CLOSED 2026-09-04** | not-ported | M | Project-trust store is unwired at the extension seam — **PARTIALLY CLOSED 2026-09-04**: the store half is done. `SessionServiceBuilder` gained a `trust_store: Option<Arc<TrustStore>>` field and a `.trust_store(store)` setter (`crates/cyrup-session-svc/src/builder.rs:434,509-511`); `build()` now reads `saved = self.trust_store.as_ref()....nearest(&cwd)....` into `TrustInputs.saved` (`:674-677,704`, no longer a literal `None`) and, on a `remember`-ed extension verdict, persists it with `store.set(&cwd, Some(decision))` (`:683-693`) rather than only warning. Wired to a REAL store in production: `crates/cyrup/src/session_launch.rs:163` calls `.trust_store(crate::prelaunch::trust_store_for(dirs))`. **RESIDUAL, unchanged: the `with_wasm` ordering.** `pre_trust_extension_verdict` (`builder.rs:2165`) still opens `let host = ExtensionHost::with_wasm(host_config).ok()?;` under `#[cfg(feature = "wasm-host")]`, discarding every native vote on a wasm-construction failure rather than falling back to `ExtensionHost::new(host_config)` (which IS what the `#[cfg(not(feature = "wasm-host"))]` twin at `:2167` does) — the Fix's second sentence is not done. |
...
>    (`extensions/types.ts:1547-1552` @v0.84.4) rather than the bare owner
...
>    projects each `ExtensionConflict` as a WARNING and `build_startup_report`
>    (`crates/cyrup/src/interactive.rs`) appends them to the same `[Extension issues]` block the
>    load failures use, after them — pi's order and severity (`interactive-mode.ts:1872-1892`;
...
> `buildBuiltinKeybindings` `:94-113`, `RESERVED_KEYBINDINGS_FOR_EXTENSION_CONFLICTS` `:71-89`,
