...
| CFG-038 | medium | parity-bug | S | One unparseable key spec discards the whole `keybindings.json` — and applies it partially first — **2026-08-14, still open; VERIFIED at HEAD by sweep 6 and this is the single highest-value unlanded item in area 05.** **FIX SITE: `crates/cyrup-tui/src/keymap.rs` (+ `crates/cyrup/src/main.rs:1624-1629`), NOT cyrup-config.** All four `merge_json` bodies (`:597-604`, `:684-691`, `:787-794` and siblings) still do `self.set_action(action, parse_key_values(&value)?)` INSIDE the entry loop, so one bad key spec aborts after earlier entries were already applied, and `main.rs:1624-1629` then prints "ignoring {path}", which is false. **Recipe (~30 lines + one test):** (a) skip-and-continue per entry with a collected failure list, (b) `App::load_keybindings_json` returns that list, (c) `main.rs` names the offending ids. **The whole-document error must stay ONLY for `keybindings_object`** (pi's `loadRawConfig` undefined path), which is where CFG-048's read-time half already sits. pi `keybindings.ts:275-288`, `:328-336`, `:350-355` @v0.83.0; `packages/tui/src/keybindings.ts:243-256`. |
| ~~CFG-045~~ | ~~medium~~ **CLOSED 2026-08-14 — already-done** | not-ported | S | `doubleEscapeAction` is inert — the Escape handler has no double-escape and no bash-mode-exit branch — **CLOSED 2026-08-14**: sweep 8 read `crates/cyrup-tui/src/app.rs` at HEAD instead of the row. **BOTH halves this row calls missing are present**, landed under `TUI-009` as a port of `interactive-mode.ts:2569-2595` @v0.83.0: the **bash-mode-exit** branch at `:2263-2267` (cyrup derives the mode from the buffer the way pi's `onChange` does at `:2621-2622`, so clearing the buffer *is* leaving the mode — the in-source comment says so) and the **500 ms double-Escape window** reading `doubleEscapeAction` at `:2272-2290`, with the `last_escape` field at `:440`, the `tree`/`fork`/`none` dispatch, and the `last_escape = None` reset on fire so a third press starts a new pair. `EffectiveSettings::double_escape_action` (`cyrup-config/src/settings.rs:845`) now has live consumers at `app.rs:2273` and `app.rs:4668-4669` — so the "a `/settings` row is not a consumer" finding this row generated is discharged as well as recorded. **The status line below is stale.** — ~~**2026-08-14, still open**: sweeps 2 and 6 — unchanged at HEAD.~~ **FIX SITE: the `Action::Interrupt` arm in `crates/cyrup-tui/src/app.rs` plus a `last_escape_time` field** (pi `interactive-mode.ts:2570-2596` @v0.83.0). Outside cyrup-config/cyrup-resources. |
...
| CFG-042 | low — **PARTIALLY CLOSED 2026-08-14** | upstream-drift | M | `FileModelsStore` does not normalize its path, cache by file revision, or accept cancellation — **PARTIALLY CLOSED 2026-08-14**: sweeps 1 + 2 landed path normalization and the revision-checked snapshot; **sweep 6 landed the insertion-ordered-map half.** `crates/cyrup-config/src/models_store.rs` gained a private `OrderedObject(Vec<(String, Value)>)` with the four JS object operations (`current[id]`, `current[id] = v` replacing IN PLACE, `delete current[id]`, and a `MapAccess` `Deserialize` that keeps a duplicate key's FIRST position with its LAST value) plus a `JSON.stringify(x, null, 2)` writer; `read_all`/`read_latest`/`update_read_state`/`write_all` all use it and `write_all` no longer re-sorts provider ids alphabetically (pi `models-store.ts:120-149` @v0.84.1, stringify at `:130`/`:141`). **Sweep 5's recorded blocker is VOID: `indexmap` in the root `[workspace.dependencies]` was not needed at all** — the swap is a ~60-line local type and no new dependency. **RESIDUAL — ONE half, not two: the `CancelToken`/`signal` parameter on the `ModelsStore` trait** (pi's `options?.signal?.throwIfAborted()` at `:120-122`, `:127-137`, `:139-149`), which lives in **cyrup-provider**. Tests: `a_write_preserves_the_files_provider_order_and_appends_new_ones` (the fixture is RAW JSON TEXT on purpose — `serde_json::json!` builds a `BTreeMap` here and would have alphabetized the input before it ever reached the code under test, making the test vacuous), `stringify_matches_json_stringify_with_two_space_indent`, `a_duplicate_key_takes_the_last_value_at_the_first_position`. |
...
> wrong about the code.** `crates/cyrup-tui/src/app.rs:2227-2290` is a faithful port of pi's four
> mutually-exclusive `else if` arms (`interactive-mode.ts:2569-2595` @v0.83.0), and it carries **both**
> halves this item calls missing: the bash-mode-exit branch (`:2263-2267`) and the 500 ms
> double-Escape window reading `doubleEscapeAction` (`:2272-2290`, `last_escape` field at `:440`,
> tree/fork/none dispatch, `last_escape` reset on fire). Landed under `TUI-009`.
> `EffectiveSettings::double_escape_action` (`cyrup-config/src/settings.rs:845`) has live consumers at
> `app.rs:2273` and `app.rs:4668-4669`. **The row's status line said "sweeps 2 and 6 — unchanged at
