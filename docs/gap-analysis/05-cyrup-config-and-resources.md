...
> | `ASYNC_EVENTS_MAX_BYTES` | `runs/background/subagent-runner.ts:307`, `maxAsyncEventsBytes` `:318-324` | **PORTED.** `background/runner_main.rs` `ASYNC_EVENTS_MAX_BYTES_ENV` (+ `_PI_ALIAS`), `resolve_async_events_cap_bytes`; `open_run_events` → `BoundedJsonlWriter::create_with_cap`. Residual, low (SUBA-063's third part): the `subagent.events.truncated` marker event with its 512-byte reserve (`:308-309`, `:351-367`) is still not written; the writer stays a silent no-op at the cap. |
> | `RUNTIME_ACKNOWLEDGED_EXTENSIONS` | `runs/shared/runtime-acknowledged-extensions.ts:6`; child write `subagent-prompt-runtime.ts:117-130`; parent read `foreground/execution.ts:1386`, `background/subagent-runner.ts:1859` | **OPEN.** The acknowledgement protocol (`subagent:acknowledge-extension` event → `0600` file → parent read-back into the result) does not exist in cyrup (`rg -i acknowledg crates/cyrup-ext-subagents/src` → 0 at `pub` level). **SUBA-063**'s second half; owned there. |
