...
> |---|---|---|---|
> | `cyrup/` | **`ea23ca2`** (2026-09-24) | `9aeba769..ea23ca2` (18 non-merge commits, 485 files, +164 754 / −4 932 under `crates/`). **`crates/cyrup-config` and `crates/cyrup-resources` are byte-identical across it**, as are `cyrup-provider`, `cyrup-modes`, `cyrup-tools` and `cyrup-sdk` (`git diff --name-only 9aeba769..ea23ca2 -- <those>` is empty), so every row whose evidence lives there carries forward by identity. The rows whose evidence lives in a crate that DID move were re-run individually: `CFG-063`…`CFG-066` (`cyrup-tui`: five `src/app/` files changed, none of them the clipboard, keymap, footer or env-read sites these rows cite — re-grepped, all four still open), `CFG-067` and `CFG-074` (`cyrup-ext-subagents`: re-grepped, still open, see their cells), `CFG-068` (`crates/cyrup/src/cli/help.rs` unchanged — still no `CYRUP_HOME`), and `CFG-073`, **closed this pass** | — |
> | `pi/` | **v0.87.1** (was v0.85.1; new tags v0.86.0 · v0.86.1 · v0.87.0 · v0.87.1) | **`v0.85.1..v0.87.1` over this area's paths, both sides:** `core/settings-manager.ts` (+81), `core/model-resolver.ts`, `core/model-config.ts`, `core/prompt-templates.ts`, `core/resource-loader.ts`, `core/model-registry.ts`, `core/model-runtime.ts`, `migrations.ts`, `config.ts`, and the coding-agent `CHANGELOG.md` entries 0.86.0–0.87.1. Five items filed (`CFG-081`…`CFG-085`) | **CLOSED by the second pass below.** *First-pass text:* `v0.84.1..v0.85.1`: the 2026-09-14 lead list below is still UNVERIFIED — nothing in it was re-read this pass |
> | siblings | pi-subagents **v0.71.0** · pi-intercom **v0.14.0** · pi-mcp-adapter **v2.37.0** · code_puppy_core_plugins **v0.0.62**; pi-permission-system v0.8.0 and pi-acp v0.0.33 unchanged | only as far as `CFG-073` needed (`src/watchdog/lsp-diagnostics.ts` at v0.64.0 and v0.71.0) | **CLOSED by the second pass below** (re-walked; `CFG-067` narrowed to one var, `CFG-074` to six). *First-pass text:* `CFG-067`/`CFG-074`'s env-var sets were not re-walked against pi-subagents v0.71.0 or pi-intercom v0.14.0 |
>
> **Leads from `v0.85.1..v0.87.1` read on the pi side only and deliberately NOT filed** (each needs
> a cyrup-side read or an owner this file is not):
> - **`cacheWarming` setting** (`settings-manager.ts:157`, `getCacheWarmingMode` `:955`, global-only;
>   consumer `core/cache-warmer.ts`, 453 new lines) and the model `promptCache` metadata it reads —
>   a whole feature (idle/streaming prompt-cache refresh, `/session` diagnostics, the
>   `cache_warming_decision` extension event). `grep -rn 'cache_warming\|cacheWarming' crates/` is
>   empty. The settings key is area 05's; the mechanism is area 03/08's and the event area 06's.
>   `CFG-085` carries only the `promptCache` models.json field.
> - **models.json compat keys REMOVED upstream**: `deferredToolsMode`, `supportsToolSearch`,
>   `supportsAdditionalTools` and `supportsToolReferences` leave `model-config.ts`'s compat schemas in
>   this window, while cyrup still models
>   them (`crates/cyrup-provider/src/api/compat.rs:363`, `:415`; consumers in `api/openai_completions/`
>   and `providers/openai_codex.rs`). Whether they moved into the catalog or were retired was not
>   traced — **area 01's**, recorded so it is not lost.
> - **`/bug` + `crash-log.ts`** add a `pi.bug-report` custom session entry and crash records; the
>   settings/redaction side touches this area but the feature is area 07/08's.
>
> ### RE-MEASURE — 2026-09-24, second pass (the unread windows above, closed)
>
> **Read this pass:** every entry of the 2026-09-14 `v0.84.1..v0.85.1` lead list below, both sides,
> each now carrying a disposition in place; `core/skills.ts`, `utils/text.ts` and its 20+ call sites,
> `core/settings-diagnostics.ts`, `core/auth-storage.ts`, `core/keybindings.ts`,
> `core/provider-composer.ts`, `core/model-resolver.ts` (`findInitialModel`) and `core/sdk.ts` at the
> tags named in each item; `core/cache-warmer.ts` in full. **Siblings re-walked for `CFG-067` and
> `CFG-074`** at pi-subagents **v0.71.0** and pi-intercom **v0.14.0** by literal env-name census (the
> blocks at the top of each body). Items filed: `CFG-086`…`CFG-093`.
>
> **Header leads above, resolved:** `cacheWarming` → **`CFG-093`** (the key) + **`SEAM-131`** (the
> warmer, area 08; the `promptCache` field stays `CFG-085`). Removed compat keys
> (`deferredToolsMode`, `supportsToolSearch`, `supportsAdditionalTools`, `supportsToolReferences`) →
> area 01's **`PROV-083`**, which records that v0.86.0 replaced them with mid-conversation system
> messages. `/bug` + `crash-log.ts` → tracker **`SEAM-132`** (area 08).
>
> **Two of the first pass's rows duplicate items other areas filed the same day** — kept, because
> each names the config half and the other the consumer, but they must ship together: `CFG-081` ↔
> area 12's `DRIFT-057` (the `maxAgentDelayMs` cap), `CFG-082` ↔ area 03's `SESS-055`
> (`compaction.modelOverrides`).
>
> **Still unread after this pass:** nothing in this area's `v0.84.1..v0.87.1` window. The sibling
> re-walk covered env-var NAMES; the semantics of the four still-shared `pi-subagents` vars were not
> re-read beyond `TOOL_TIMEOUT_MS`.
...
- **Root `.md` files must declare skill frontmatter** — S. `packages/agent/src/harness/skills.ts:259-263,272,280 @v0.85.1`;
  ...
  **— NO LONGER A LEAD: FILED 2026-09-24 (second pass) as `CFG-086`** (followed to
  `Skill::load_with_diagnostics`; the diagnostics are confirmed). Reference re-pinned to the
  coding-agent copy, `core/skills.ts:277-345` @v0.87.1.
- **Skills loader error handling and frontmatter type guards** — M. `core/skills.ts:117-121,279-345 @v0.85.1`.
  ...
  **— NO LONGER A LEAD: folded into `CFG-086`** (read failure → warning, parse warning only for
  `SKILL.md`, string type guards). cyrup already warns on a read failure (`skill.rs:179` `?` →
  `scan.rs:319`), so only the two other limbs are owed.
- **Skills prompt section gated on read OR bash** — S. `core/system-prompt.ts:45-46,66-67,161-162 @v0.85.1`.
  ...
  **— STRUCK 2026-09-24 (second pass): duplicate of area 03's `SESS-059`,** filed the same day with
  both sides read. The system prompt is area 03's surface.
...
- **BOM tolerance on every config / JSON / text read path** — M. New
  ...
  **— NO LONGER A LEAD: FILED 2026-09-24 (second pass) as `CFG-087`** (added v0.84.3).
- **Settings load errors carry the file path; startup diagnostics deduplicated and routed** — M.
  ...
  **— NO LONGER A LEAD: FILED 2026-09-24 (second pass) as `CFG-088`** (all three behaviours, one row;
  added v0.84.3).
- **`auth.json` permissions applied on creation only** — S. `core/auth-storage.ts:24,63-66,104-107,185-188 @v0.85.1`.
  ...
  **Upstream read at both tags; cyrup side only grepped** — `cyrup-config/src/auth.rs:2` documents
  ...
  write was not read.
  **— NO LONGER A LEAD: FILED 2026-09-24 (second pass) as `CFG-089`.** Read: it does, and via
  temp+rename, so the old inode's mode and ACL are both lost (`lock.rs:336-373`). Upstream removed the
  `chmodSync` calls at v0.84.3.
- **Terminal capability overrides (`terminal.hyperlinks` / `.images` / `.trueColor`)** — M.
  ...
  **— NO LONGER A LEAD: FILED 2026-09-24 (second pass) as `CFG-090`** (v0.84.4; area 07 owns the seam).
- **WSL keybinding defaults and `app.thinking.save`** — S. `core/keybindings.ts` (v0.84.1..v0.85.1
  ...
  **— NO LONGER A LEAD: FILED 2026-09-24 (second pass) as `CFG-091`** (WSL arm v0.84.4,
  `app.thinking.save` v0.85.1).
...
- **Per-model thinking level in `findInitialModel`** — S. `core/model-resolver.ts:629,640,665-671,679-682 @v0.85.1`,
  ...
  **— REFUTED 2026-09-24 (second pass).** pi's `findInitialModel` thinking level is dead on the live
  path: `sdk.ts` @v0.87.1 (`:212-228`) keeps only `result.model`, then resolves thinking as explicit →
  restored → `getModelThinkingLevel(model)` → global default (`:232-248`); `main.ts` sets
  `options.thinkingLevel` for scoped models only when the scope entry carries one (`:502-509`). cyrup's
  single consumer does exactly that order — `crates/cyrup-session-svc/src/builder.rs:2332-2350`
  (explicit/parsed → restored → `model_thinking_level` → settings default). `select.rs`'s
  unused thinking output matches pi's unused one. Nothing to file.
- **`findModelDefaults` for models.json / extension model definitions** — S.
  ...
  **— NO LONGER A LEAD: FILED 2026-09-24 (second pass) as `CFG-092`** (v0.85.0). Only `api` and
  `baseUrl` come from the defaults row on either side (`provider-composer.ts:164-170` @v0.87.1;
  `compose.rs:209-228`), so the wrong row means a wrong endpoint.
- **model-config compat schema: five new per-model flags** — S. `core/model-config.ts:78,112,123,125,137 @v0.85.1`.
  ...
  **— RESOLVED 2026-09-24 (second pass): every key is owned in area 01.** `ModelCompat` has no
  `deny_unknown_fields` (`crates/cyrup-provider/src/api/compat.rs:294-296`), so nothing is a hard
  failure — an unported key is silently ignored. `supportsFinishReason` is ported; `vllmPriority` →
  `PROV-100`; `supportsMaxOutputTokens` → `PROV-093`; `supportsMidConvoEffort` → `PROV-091`;
  `supportsAdditionalTools` was removed from `model-config.ts` again by v0.87.1 and re-anchored on
  mid-conversation system messages → `PROV-083`. Nothing filed here.
- **Curated default model table — three rows still lagged** — S. `core/model-resolver.ts:35-38 @v0.85.1`:
  ...
  **— SUBSUMED 2026-09-24 by `CFG-084`**, filed by the first pass against v0.87.1 (six providers).
...
- **`PI_*` environment aliases deleted workspace-wide** — S. Commit `dd44b3c` removed every
  ...
  and the row is left as it is.
  **— STRUCK 2026-09-24 (second pass): not a parity gap.** pi never reads a `CYRUP_*` name and cyrup
  never promised to read pi's; the `PI_*` fallbacks were themselves cyrup-original (the
  `R-07-028` they cited is unverifiable, as noted). Verified at `ea23ca2`:
  `crates/cyrup-config/src/paths.rs:335` `ENV_AGENT_DIR_KEYS = ["CYRUP_AGENT_DIR",
  ENV_CODING_AGENT_DIR]` with `ENV_CODING_AGENT_DIR = "CYRUP_CODING_AGENT_DIR"` (`:344`).
  **`CFG-076` item 2's sentence "Both original spellings are retained as lower-precedence
  fallbacks" is FALSE at `ea23ca2`** — read that row's item 2 as history. The same hard-rename
  policy covers `bash`'s `CYRUP_CODING_AGENT` (area 04's disposition). Whether the deletion is itself a divergence to file depends on an
...
- **`xai` default chased to grok-4.6, with a named-exception test** — S.
  ...
  **— RESOLVED 2026-09-24 (second pass): no new row.** The default table's divergence is `CFG-084`
  (read at v0.87.1, where pi has moved `xai` on again, to grok-4.7). The stale framing in `PROV-054`
  is area 01's text and is left to that file.
- **`AppMode::Acp` and `decide_trust` gated on a new `can_prompt`** — S. `cyrup-config/src/trust.rs`:
  ...
  **— STRUCK 2026-09-24 (second pass): not a gap in this area.** The variant and its `can_prompt`
  split are area 15's (`ACP-002`), with a written `[CYRUP-DELTA]`; nothing in 05's rows depends on
  `AppMode` having three values.
...
>
> ### 2026-09-04 — audit against cyrup HEAD `2571969` (this block is authoritative over every block above it)
>
> **`git log --oneline 4fb5e40..HEAD -- crates/cyrup-config crates/cyrup-resources`** (210 commits ahead
> of this file's `4fb5e40` re-audit baseline) surfaced the fullscreen/alt-screen TUI landing
> (`dbcf59a`), a fourteen-item TUI parity sweep (`dd69fea`), and a home/agent-dir path-ladder
> consolidation (`3f9380f`) as the three commits with area-05-relevant fallout. **Four rows CLOSED, all
> read and re-derived at HEAD, not inferred from commit subjects:** `CFG-021` (`tuiMode` /
> `fullscreenScrollbar` — the alt-screen renderer this row was waiting on shipped, with both keys
> modelled and live consumers in the new `crates/cyrup-tui/src/altscreen/` tree), `CFG-014`
> (`showCacheMissNotices` gained real gating consumers across `crates/cyrup-tui/src/app/*`, not a
> `/settings` row), `CFG-053` (`max_retry_delay_ms` now has a direct test on `retry_delay_ms`, the
> function `open_sse`'s retry loop actually calls) and `CFG-076` (the path-ladder consolidation is
> exactly the fix this row's item 3 asked for — `resolve_agent_dir_from` now routes through the shared
> `ENV_AGENT_DIR_KEYS`, and the new code's own doc comment names this row by id). **One row NARROWED**
> (`CFG-067` — three of twelve `pi-subagents` env vars landed: `STEER_CAPABILITY`, `STEER_ACK_DIR`,
> `CAPABILITY_CEILING_V1`; nine remain unported). **One row's citation found stale but left open**
> (`CFG-073` — the `CI`-read citation no longer resolves after an `exec/acceptance.rs` split; could not
> determine in this pass whether the behaviour moved or was removed). **`CFG-068` and `CFG-066` got a
> "still open, here is why adjacent new code does not close this" note** so neither is mistaken for
> done by a reader who greps the new path-ladder or clipboard-write-chain modules. **Two rows FILED**
> from a lightweight `git diff --stat v0.84.1..v0.84.4` skim of this area's upstream paths (this file's
> own recorded latest is v0.84.1; the drift window `v0.84.1..v0.84.4` was never measured before now):
> `CFG-078` (`fullscreenExitOutput` / `fullscreenCopyOnSelect`, both alt-screen-only v0.84.4 additions)
> and `CFG-079` (`defaultTools`, a v0.84.4 addition). **No ID renumbered, merged or deleted.**
>
> **RECOUNTED — counted set: 0 critical, 0 high, 4 medium, 10 low = 14.** The table carries **70 rows
> (66 `CFG-NNN` + the four legacy `CFG-S0N` surface-sweep rows already closed): 56 closed, 14 open.**
> *(Previous edition: 0 / 0 / 4 / 12 = 16, 68 rows.)*
>
> **Left open, unverified this pass, exactly as recorded — one sentence each per README's "publish
> what you excluded":** `CFG-015` (four unconsumed `cyrup-config` accessors) — re-confirmed unconsumed
> anywhere in `crates/cyrup-tui/src` this pass, unchanged, no edit needed. `CFG-020` (no `ModelRuntime`
> type) — `grep -rn 'struct ModelRuntime' crates/` is still empty; unchanged, not reached this pass, and
> still genuinely L (three crates). `CFG-063`/`CFG-064`/`CFG-065` (`PI_TUI_DEBUG`/`PI_DEBUG_REDRAW`,
> `isWindowsTerminalSession`, `isWslEnvironment`) — each re-greppped across `crates/cyrup-tui/src` with
> zero hits beyond an unrelated `WT_SESSION` image-protocol check; unchanged. `CFG-070` (AWS/`APPDATA`
> reads, correct-as-written) — re-confirmed present and unchanged; no action needed by the row's own
> instruction. `CFG-074` (nine invented sibling-port env vars) — not re-walked this pass; the item
> spans three crates outside this file's pair (`cyrup-ext-subagents`, `cyrup-intercom`,
> `cyrup-permission-system`) and a full re-verification of all nine was out of this pass's budget.
> `CFG-077` (recursive prompt-root scan, a deliberate record not a defect) — untouched, correctly so
> per the row's own instruction not to "fix" it.
...
| CFG-020 | medium | not-ported | L | No `ModelRuntime` type and no availability snapshot — **2026-08-14, still open**: sweeps 2 and 6 — not reached; L, three crates, requires reading `model-runtime.ts` at v0.84.1 (+356 lines). Not tail-sized. **Its prerequisite mechanism — the revision-checked snapshot in `FileModelsStore` — is now complete except for CFG-042's cancellation parameter.** **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN, claim holds in full.** No `struct ModelRuntime` exists anywhere in the workspace (every hit is rustdoc naming pi's type). `full_model_registry()` has MOVED (`session.rs:2680` -> `crates/cyrup-session-svc/src/session/model.rs:199-244`) but still recomposes the whole registry on EVERY call: it rebuilds `base` from `provider.current().models().to_vec()`, folds guest providers, folds `cyrup_provider::default_models(..)`, then runs `model_config.compose(&base)` — no caching, no invalidation queue. `available_model_catalog` (`session/model.rs:253-257`) calls it and filters per query. `ApiKeyAuth` (`crates/cyrup-provider/src/auth/mod.rs:61-104`) now exposes `name`, `supports_login`, `login` and `resolve` — the login half landed for CFG-005 — but pi's status-query half (`check`) still has NO counterpart. Upstream at **v0.85.1** is unchanged in substance and slightly larger: `snapshot.configuredProviders` (`model-runtime.ts:61,144,281,301-309`), `queueAvailabilityRefresh` (`:316`), `refreshProviderAvailability` (`:331-367`), `getAvailableSnapshot()` (`:422`), `hasConfiguredAuth` reading the snapshot (`:466-467`), `enqueueCredentialOperation` (`:494`) and `CredentialSynchronizationError` (`:94-106`) are all present. **TWO CORRECTIONS to this row's own text, recorded rather than rewritten:** (a) the parenthetical "+356 lines at v0.84.1" is WRONG — `model-runtime.ts` is 595 lines at v0.83.0, 787 at v0.84.1 (+192) and 795 at v0.85.1; (b) the `Verify` instruction to read at v0.84.1 rather than v0.83.0 is still right, and v0.85.1 adds only one unrelated method (`streamDeferred`, the sole 11-line insertion in the `v0.84.1..v0.85.1` diff), so the port target is effectively the v0.84.1 shape plus that. |
...
| ~~CFG-014~~ | ~~low~~ **CLOSED 2026-09-04** | not-ported | M | `showCacheMissNotices` and prompt-cache-miss tracking absent — **CLOSED 2026-09-04: the CONSUMER half this row's last edit said was still owed has landed.** `crates/cyrup-tui/src/app/state.rs:209-210` carries the field, and `EffectiveSettings::show_cache_miss_notices()` now has live consumers across the app module: `app/session_bind.rs:243`/`:251`, `app/events.rs:330`, `app/channels.rs:256`, `app/tree_nav.rs:211`, `app/events_fold.rs:309`, with `app/run_arms.rs:80`/`:267` doing the settings→state read on session (re)build and `app/execute_misc.rs:696` handling the `/settings` toggle write. This is real gating logic (`if self.state.show_cache_miss_notices { … }`), not a display row. The `cyrup-config` accessor itself was already correct and is unchanged. |
| CFG-015 | low | not-ported | M | Five unconsumed settings accessors, incl. `lastChangelogVersion` and `collapseChangelog` — **NARROWED 2026-08-14**: sweep 6 — `warnings().anthropic_extra_usage` now HAS real consumers (`cyrup-tui/src/app/selectors.rs:290`, `app/execute_misc.rs:209`, `app/run_arms.rs:76-77` and `:253`) and comes off the unconsumed list. **FOUR remain: `code_block_indent`, `last_changelog_version`, `collapse_changelog` (a `/settings` display row only) and `npm_command`.** **FIX SITE: every remaining consumption site is in `crates/cyrup-tui` — NOT cyrup-config.** Landing the accessors alone is the exact "a /settings row is not a consumer" failure this area's Coverage section records. (pi `settings-manager.ts:55`, `:59`, `:84`, `:99`, `:102` @v0.83.0.) **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low, but the enumeration NARROWS BY ONE and that narrowing REFUTES this row's literal claim rather than closing the item.** `code_block_indent` (now `crates/cyrup-config/src/settings/effective.rs:472`), `last_changelog_version` (`:706`) and `npm_command` (`:374`) still have ZERO consumers outside `crates/cyrup-config/src/` — **three, not four**. `collapse_changelog` (`:711`) still has exactly one consumer, the display row (`crates/cyrup-tui/src/app/settings_rows.rs:181`), i.e. a `/settings` toggle over a value nothing reads. The fifth key, `warnings().anthropic_extra_usage` (`settings/effective.rs:600-603`, field `settings/types.rs:146`), NOW HAS CONSUMERS — at different lines than the 2026-08-14 narrowing recorded: `crates/cyrup-tui/src/app/run_arms.rs:87-88` and `:331-332` cache it into `AppState::warn_anthropic_extra_usage` (`state.rs:203`), `selectors.rs:683-691` renders the nested Warnings submenu from it, `execute_misc.rs:833-834` writes it back — so the phrase "four accessors have zero consumers anywhere outside cyrup-config/src" is FALSE for that key. **The substantive gap survives the refutation:** the warning the setting gates is never emitted. pi's `maybeWarnAboutAnthropicSubscriptionAuth` (`modes/interactive/interactive-mode.ts:4908-4935` @v0.85.1) returns early on `getWarnings().anthropicExtraUsage === false` and otherwise shows `ANTHROPIC_SUBSCRIPTION_AUTH_WARNING`; cyrup has no such producer anywhere in `crates/`, so setting the key to `false` still suppresses nothing. All five keys are still declared upstream at v0.85.1 (`settings-manager.ts:65,70,95,114,115`; getters/setters `:703-708`, `:990-995`, `:1000-1005`, `:1347`). PB-6 is unchanged: `/changelog` is still hardcoded at `crates/cyrup-tui/src/app/submit.rs:125-128` and no parser exists. Severity `low` stands. |
...
| CFG-063 | low | not-ported | S | `PI_TUI_DEBUG` and `PI_DEBUG_REDRAW` — the two upstream render-debug env vars — have no counterpart, so the cursor/viewport bug class has no instrument — **filed 2026-08-14** (env-var surface). **FIX SITE: `crates/cyrup-tui` (area 07).** Sibling of `TUI-040`. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low; cyrup side unchanged, but EVERY UPSTREAM CITATION IN THIS ROW IS NOW DEAD and one variable was RENAMED — whoever schedules the port must not copy them.** cyrup: zero hits for `TUI_DEBUG`, `DEBUG_REDRAW` or `debug_redraw` across all of `crates/`. Upstream at **v0.85.1** both instruments still exist but have MOVED out of `packages/tui/src/tui.ts` into `packages/tui/src/tui-main-screen.ts`: `PI_TUI_DEBUG` is now `tui-main-screen.ts:569` (the field list is unchanged plus `newLines.length` / `previousLines.length` and the three dumped arrays, still written to `/tmp/tui/render-<ts>-<rand>.log`); `PI_DEBUG_REDRAW` was **RENAMED to `PI_TUI_DEBUG_REDRAW`** (`tui-main-screen.ts:321`), its log file is now `pi-tui-debug.log` rather than `pi-debug.log`, and per `packages/tui/CHANGELOG.md:17` (#8699) redraw logging is DISABLED outright when the host supplies no log directory (`const redrawLogDirectory = process.env.PI_TUI_DEBUG_REDRAW === "1" ? this.logDirectory : undefined;` then `if (redrawLogDirectory === undefined) return;`, `:322-323`). Fix site (`crates/cyrup-tui`, area 07, sibling of `TUI-040`) is unaffected; severity `low` stands. **RE-CONFIRMED STILL OPEN 2026-09-16 at `cc7818b`.** Re-run rather than carried forward, because this row's fix site is `crates/cyrup-tui`, one of the three crates that changed in `9aeba769..cc7818b`: `git grep -in 'TUI_DEBUG\|DEBUG_REDRAW' -- crates/` is still **0**. Neither instrument exists, and the `#140` window is the reason to say so out loud — the one cyrup-tui defect that pass fixed (`app/backend.rs`'s timing-bounded cursor assertion, now filed as `TUI-N14`) is precisely a render-path defect that had to be diagnosed without either of these logs. |
| CFG-064 | low | not-ported | S | `isWindowsTerminalSession()` is unported — `SSH_CLIENT` / `SSH_CONNECTION` / `SSH_TTY` are read nowhere — so Ctrl+Backspace degrades to Backspace on Windows Terminal, and the bug direction flips over SSH — **filed 2026-08-14** (env-var surface). **FIX SITE: `crates/cyrup-tui` (area 07).** **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low (already the floor); the absence claim holds and upstream is byte-stable, but this row's cyrup grep is STALE and its `Fix` describes a port that cannot be written as stated.** Upstream unchanged at **v0.85.1**: `isWindowsTerminalSession()` at `packages/tui/src/keys.ts:715-718` (`Boolean(process.env.WT_SESSION) && !process.env.SSH_CONNECTION && !process.env.SSH_CLIENT && !process.env.SSH_TTY` on `:717`), consumed at `:733` (`matchesRawBackspace`) and `:1287` (`if (data === "\x08") return isWindowsTerminalSession() ? "ctrl+backspace" : "backspace"`). cyrup still has no such predicate — but the row's `grep SSH_TTY/SSH_CLIENT/SSH_CONNECTION -> 0` is NO LONGER TRUE: `crates/cyrup-tui/src/clipboard.rs:82` reads `SSH_CONNECTION` / `SSH_CLIENT` for pi's `isRemoteSession` (`clipboard.ts:22-24`), a DIFFERENT predicate; `SSH_TTY` is still read nowhere, and `WT_SESSION` is still read only for image capabilities (`crates/cyrup-tui/src/image.rs:825`). **What the `Fix` cannot do:** cyrup has no raw-byte pre-parse seam, so `keys.ts:1287` has no site to route through — `crates/cyrup-tui/src/escape_reassembly.rs:44-50` carries its own `[CYRUP-DELTA]` stating exactly this (crossterm's `parse_event` is `pub(crate)`, `EventSource` is private, and `event::poll` drains the tty into crossterm's own parser, so nothing in cyrup can hand bytes to it), and cyrup's reassembly runs AFTER crossterm on shredded events. The `Fix` should be rewritten to target crossterm's decoded `KeyEvent` rather than "the two `keys.ts` decisions". Confidence on the BEHAVIOURAL half is medium: whether Ctrl+Backspace actually degrades under crossterm on Windows Terminal is a crossterm question that was not measured this pass. **RE-CONFIRMED STILL OPEN 2026-09-16 at `cc7818b`, re-run because the fix site is `crates/cyrup-tui`. No cell changes — the 2026-09-14 correction already caught this row's stale grep, and it is recorded accurately above.** Re-measured: `isWindowsTerminalSession` has no counterpart (`git grep -in 'windows_terminal' -- crates/` = 0); `SSH_TTY` is still read nowhere; `SSH_CONNECTION` / `SSH_CLIENT` are still read only by `clipboard.rs:82` for the DIFFERENT `isRemoteSession` predicate; `WT_SESSION` is still read only at `image.rs:825` for image capabilities. The `escape_reassembly.rs` blocker the 2026-09-14 note identified is untouched by `#138`'s keymap work, which changed `keymap.rs` only for `TUI-073`'s `clear` diagnostic. |
| CFG-065 | low | not-ported | S | `isWslEnvironment()` (`WSL_DISTRO_NAME` / `WSL_INTEROP`) and its git-HEAD polling fallback are unported, so the footer branch indicator goes stale on `/mnt/<drive>` repos where inotify never fires — **filed 2026-08-14** (env-var surface). **FIX SITE: `crates/cyrup-tui` (area 07).** **RE-CONFIRMED STILL OPEN 2026-09-16 at `cc7818b`; this row carries no 2026-09-14 marker, so it was re-measured.** `git grep -in 'WSL_DISTRO_NAME\|WSL_INTEROP\|is_wsl' -- crates/` is **0** — neither half of the predicate (the env test or the `/mnt/<drive>` path test) exists, and the footer still has no polling fallback. Unchanged on the cyrup side since filing; upstream not re-read this pass. |
| CFG-066 | low | not-ported | S | The clipboard backend's two load gates — `TERMUX_VERSION` and `hasDisplay` (`DISPLAY` / `WAYLAND_DISPLAY`) — are unported, so the backend is attempted unconditionally on headless Linux and under Termux — **filed 2026-08-14** (env-var surface). Distinct from the known clipboard-TEXT gap at `12-upstream-drift-pi-core.md:1039-1047`. **STILL OPEN 2026-09-04 — NOT closed by adjacent new work, stated so it is not mistaken for done:** `crates/cyrup-tui/src/clipboard.rs` is new since the baseline (a full port of pi's `copyToClipboard` write chain) and DOES read `TERMUX_VERSION`/`WAYLAND_DISPLAY`/`DISPLAY` (`ClipboardEnv::from_process`, `clipboard.rs:79-88`) — but only to pick which platform CLI to shell out to (`clipboard_write_plan`), the port of `clipboard.ts:104-160`. It does not gate whether the **native** backend (`arboard`) is constructed at all, which is this item's actual claim (pi `clipboard-native.ts:31`'s `loadClipboardNative()` guard) — `crates/cyrup-tui/src/app/event_extract.rs:106` still calls `arboard::Clipboard::new()` unconditionally for the Ctrl+V image-paste read path, with no Termux/headless-Linux gate. Different upstream file, different cyrup call site; this row is unaffected by the write-chain port. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at low, Kind `not-ported` correct; the literal claim holds, but the `Impact` as written does not transfer to the port mechanism and this row should say so.** Upstream unchanged at **v0.85.1**: `packages/coding-agent/src/utils/clipboard-native.ts:16` `const hasDisplay = process.platform !== "linux" \|\| Boolean(process.env.DISPLAY \|\| process.env.WAYLAND_DISPLAY);` and `:31` `const clipboard = !process.env.TERMUX_VERSION && hasDisplay ? loadClipboardNative() : null;`. cyrup: the 2026-09-04 note's line numbers have drifted — `ClipboardEnv::from_process` is now `crates/cyrup-tui/src/clipboard.rs:80-88` (`termux`, `wayland_display`, `wayland_session`, `x11_display`) and the unconditional `arboard::Clipboard::new()` is `crates/cyrup-tui/src/app/event_extract.rs:108`. There is a SECOND ungated site this row does not name: `clipboard.rs:351`, the `ClipboardRead::Native` step, which `clipboard_read_plan` (`:262-269`) pushes on EVERY platform. The WRITE half is now STRICTER than upstream, not looser — `clipboard_write_plan:112-114` pushes `Native` only on `macos`/`windows` — so the write half is moot. **What does NOT transfer:** upstream's gate exists because `require("@mariozechner/clipboard")` THROWS at import time on Termux and the addon is a dynamically loaded `.node`; `arboard` is statically linked and `Clipboard::new()` is a fallible constructor whose `Err` is handled at both sites (`.ok()?` / `.ok()`), so the headless/Termux outcome is a silent degrade identical to pi's `clipboard === null`. Neither environment was run this pass, so "the attempt is free" is an argument, not a measurement. **Falsification condition (owed, because the disposition rests on an argument):** reopen as a real defect — rather than a name-parity record — if a measurement shows `arboard::Clipboard::new()` on Linux with neither `DISPLAY` nor `WAYLAND_DISPLAY` set, or under Termux, does anything other than return `Err` promptly: blocks, spawns a surviving thread, writes to stderr, or panics. Close it outright if such a measurement shows the attempt is free AND this row is rewritten as a `[CYRUP-DELTA]` record. **RE-CONFIRMED STILL OPEN 2026-09-16 at `cc7818b` — and this row is the pass's worked example of a grep that looks like a closure and is not.** `crates/cyrup-tui` is one of only three crates that changed in `9aeba769..cc7818b`, so this row's evidence was re-run rather than carried forward. Running the row's ORIGINAL 2026-08-14 grep today gives a false green: `TERMUX_VERSION`, `WAYLAND_DISPLAY` and `DISPLAY` are all read, at `crates/cyrup-tui/src/clipboard.rs:79-91` (`ClipboardEnv::from_process`). The 2026-09-14 re-read already established why that does not close the row and it holds verbatim: those reads only choose which platform CLI to shell out to (`clipboard_write_plan`, `:97-140`, the port of `clipboard.ts:104-160`), and the native `arboard` backend is still constructed with **no** Termux/headless gate. **Only the citation moves:** `crates/cyrup-tui/src/app/event_extract.rs:106` → **`:108`**, `arboard::Clipboard::new().ok()?` in `read_clipboard_image_to_temp`, still the Ctrl+V image-paste READ path, still ungated. The write chain has two further ungated `arboard::Clipboard::new()` sites (`clipboard.rs:168`, `:351`), but those are reached only on `macos`/`windows`, where pi's `hasDisplay` is trivially true (`platform !== "linux"`), so they are NOT additional instances — recorded so the next pass does not file them as such. Note also that `crates/cyrup-tui/src/tests/clipboard.rs:162` (`headless_linux_has_no_local_step`) pins the WRITE plan's headless behaviour and can be mistaken for this row's `Verify`; it is not — this row's `Verify` is about the backend being CONSTRUCTED. |
| CFG-067 | ~~medium~~ medium — **PARTIALLY CLOSED 2026-09-04, NARROWED AGAIN 2026-09-05** | not-ported | M | Twelve `pi-subagents` env vars have no `CYRUP_` counterpart — three of them are budget/ceiling caps and one is a security kill switch — **NARROWED 2026-09-04: nine of twelve remain unported; three landed since this row was filed.** `git log 4fb5e40..HEAD -- crates/cyrup-ext-subagents` surfaced work that ports `PI_SUBAGENT_STEER_CAPABILITY` and `PI_SUBAGENT_STEER_ACK_DIR` verbatim (`crate::prompt_runtime::STEER_CAPABILITY_ENV` / `STEER_ACK_DIR_ENV`, consumed in `exec/agent_config.rs`, `exec/spawn_plan.rs`) and `PI_SUBAGENT_CAPABILITY_CEILING_V1` — one of the three named budget/ceiling caps — as `crate::exec::capability_ceiling::CAPABILITY_CEILING_ENV = "CYRUP_SUBAGENT_CAPABILITY_CEILING_V1"`, with the PI spelling honoured as a read-side compat alias (`CAPABILITY_CEILING_ENV_PI_ALIAS`, `capability_ceiling.rs:60-64`, `:424-425`). `grep -rn` for the remaining nine spellings (`TOOL_TIMEOUT_MS`, `TASK_DELIVERY`, `RUN_FANOUT_BUDGET`, `MAX_SPAWNS_PER_RUN`, `TOOL_BUDGET_ZERO_AUTH`, `ASYNC_EVENTS_MAX_BYTES`, `RUNTIME_ACKNOWLEDGED_EXTENSIONS`, `LLM_INTENT_ARBITER`, `PACKAGE_ROOT`) across `crates/cyrup-ext-subagents/src` and `crates/cyrup-intercom/src` still returns nothing — this row stays open for those nine. **FIX SITE: `crates/cyrup-ext-subagents` (area 09); this row exists so the enumeration is not lost while area 09 has no item for any of them.** **PARTIALLY CLOSED 2026-09-04 — commit `91ca02e` (`feat(subagents): CFG-067 …`), three of the nine ported, six re-scoped; the row stays OPEN at medium for the six.** Ported, each at pi-subagents **v0.64.0** (ADR-0006 latest tag) with a `CYRUP_` spelling and — where an operator sets it — the `PI_` spelling as read-side alias: **(1)** `PI_SUBAGENT_TASK_DELIVERY` (`runs/shared/pi-args.ts:76-102`, consumed `:822`; new at v0.48.0, `darwin` leg at v0.63.0) → `crates/cyrup-ext-subagents/src/spawn/mod.rs` `TASK_DELIVERY_ENV` / `TaskDelivery` / `resolve_task_delivery` / `should_deliver_task_via_file` / `ChildSpawnSpec::resolve_task_arg_with`, the shell `resolve_task_arg` reading env + `cfg!(target_os = "macos")`; **(2)** `PI_SUBAGENT_TOOL_BUDGET_ZERO_AUTH` (`runs/shared/tool-budget.ts:5,16-24,74-80`; child read `subagent-prompt-runtime.ts:693`; parent write `pi-args.ts:1032`) → `exec/tool_budget.rs` `TOOL_BUDGET_ZERO_AUTH_ENV` / `HardMinimum::{One,Zero}` / `HardMinimum::from_env` / `validate_tool_budget_config_with` / `decode_tool_budget_env(value, minimum)`, wired at the child-side decode in `prompt_runtime.rs` (`HardMinimum::from_env(get)`) — **security note (review 2026-09-04):** with the gate ported, the pre-existing decode-error policy in `prompt_runtime.rs` (a malformed `CYRUP_SUBAGENT_TOOL_BUDGET` payload is `tracing::warn`ed and DROPPED, the child runs UNBUDGETED) now also covers an UNAUTHORISED `{"hard":0}` — upstream's `decodeToolBudgetEnv` throws out of module init and the child never starts (`tool-budget.ts:74-80`), cyrup starts it with no budget at all. Pre-existing divergence (the parent is the only writer, so it needs a tampered environment), recorded here as the security-relevant half of it; `prompt_runtime::tests::a_zero_budget_is_honoured_only_with_the_parents_authorisation` pins the current behaviour; **(3)** `PI_SUBAGENT_ASYNC_EVENTS_MAX_BYTES` (`runs/background/subagent-runner.ts:306-324`; v0.31.0) → `background/runner_main.rs` `ASYNC_EVENTS_MAX_BYTES_ENV` / `resolve_async_events_cap_bytes`, and `open_run_events` now opens `events.jsonl` through `BoundedJsonlWriter::create_with_cap` with it (it called `create`, which has no cap input, before). Tests: `spawn::tests::{task_delivery_is_file_only_for_the_trimmed_lowercased_word_file, the_pi_alias_is_consulted_only_when_the_cyrup_spelling_is_unset, should_deliver_task_via_file_has_upstreams_three_legs, file_delivery_spills_even_a_short_task, macos_spills_every_task_regardless_of_length_or_env}`, `exec::tool_budget::tests::{a_zero_hard_budget_is_rejected_unless_the_parent_authorised_it, zero_authorisation_requires_the_exact_string_one, a_zero_budget_blocks_the_first_browsing_call}`, `prompt_runtime::tests::a_zero_budget_is_honoured_only_with_the_parents_authorisation`, `background::runner_main::async_events_cap_tests::{the_cap_override_follows_pis_number_coercion, the_pi_alias_is_consulted_only_when_the_cyrup_spelling_is_unset}`; crate suite 2679/2679. **Still open — six, each re-derived at v0.64.0 in the detail section:** `TOOL_TIMEOUT_MS`, `RUN_FANOUT_BUDGET`, `MAX_SPAWNS_PER_RUN`, `RUNTIME_ACKNOWLEDGED_EXTENSIONS`, `LLM_INTENT_ARBITER` — every one names a MECHANISM cyrup lacks (a per-tool child timeout, a run-fan-out claims ledger, an extension-acknowledgement protocol, an LLM intent arbiter), so a bare env constant would be a knob wired to nothing — and `PI_CODING_AGENT_PACKAGE_ROOT`, which is **inapplicable** (Node package-root resolution; cyrup's child is the `cyrup` binary via `CYRUP_SUBAGENT_BINARY`). **Cross-ref 2026-09-04 (batch-2 ledger audit):** the unauthorised-`{"hard":0}` decode-drop divergence this row records (child runs UNBUDGETED where pi's child refuses to start) is now its own row, `CFG-080` (low), so the census counts it — **`CFG-080` CLOSED 2026-09-04 at `9a7c0fdb`: the decode error is refused rather than dropped, so the description of the drop policy earlier in this row is history, not HEAD.** **NARROWED AGAIN 2026-09-05 — commits `063cfe6f` + `5a4d456e`, two of the six remaining ported, so this row is now open for FOUR.** The two that landed are the pair the 2026-09-04 disposition table had blocked on ONE missing mechanism: `RUN_FANOUT_BUDGET` ("Porting the var without the ledger is impossible") and `MAX_SPAWNS_PER_RUN` ("deliberately not ported ahead of the ledger"). `063cfe6f` ports the ledger — the whole of pi-subagents **v0.64.0** `src/runs/shared/run-fanout-budget.ts` as `crates/cyrup-ext-subagents/src/exec/run_fanout_budget.rs`: `RunFanoutBudgetDescriptor`/`Snapshot`/`Rejection` (`shared/types.ts:760-778`), `create_run_fanout_budget_in` (`:79-89`, the `0o700` ledger dir + exclusive `manifest.json` + `safeRootRunId`'s `[^A-Za-z0-9._-] -> _` scrub at `:40-42`), `validate_run_fanout_budget_descriptor_in` (`:91-106`, incl. `validateDirectory`'s canonicalising managed-root containment check `:65-77`), read/write/encode/decode of the descriptor (`:108-136`, base64url being the spawn-boundary form), `run_fanout_budget_snapshot_in` (`:209-213`) over `claimCount`'s `/^\d{6}\.json$/` slots (`:200-207`), `claim_run_fanout_batch_in`/`_with_commit_in` (`:220-272`) — all-or-nothing admission under `withAdmissionLock` (`:168-198`: exclusive-`mkdir` lock, `owner.json`, pi's `[10,25,50,100,200,500,1000,2000,4000]` ladder, stale-owner reclaim gated on a `kill(pid, 0)` liveness probe `:156-166`, token-checked release) — and the two verbatim formatters (`:274-280`); plus `MAX_SPAWNS_PER_RUN_ENV = "CYRUP_SUBAGENT_MAX_SPAWNS_PER_RUN"` (+ the `PI_` spelling as a read-side alias) and `resolve_max_spawns_per_run` from `shared/types.ts:2807-2818`. `5a4d456e` gives both vars the operator-visible consumer this row's `Verify` clause names: pi `formatRunFanoutSection` (`extension/doctor.ts:180-193`) as `RunFanoutDoctor` + `MaxSpawnsPerRunSource`, rendered as `buildDoctorReport`'s `"Run fan-out budget"` block (`:256-258`) by `registration/doctor.rs`, and `SubagentExtensionConfig.max_subagent_spawns_per_run` (pi `ExtensionConfig.maxSubagentSpawnsPerRun?`, `shared/types.ts:2550`) as the resolver's middle rung. **Deltas, each at its match site:** `RunFanoutLimitError` (a subclass callers `instanceof`) becomes the `RunFanoutError::{Limit, Invalid}` enum; `process.kill(pid, 0)` becomes `nix::sys::signal::kill(pid, None)` (`#![forbid(unsafe_code)]`), `EPERM` still meaning alive; the retry ladder is upstream's UNCLAMPED base, since `PI_SUBAGENT_FS_RETRY_MAX_TOTAL_MS` is not one of this row's twelve names; every filesystem root is a parameter of an `_in` function so no rule reads the process environment or the real temp root. **Asymmetry worth naming:** `0` means UNLIMITED on the per-SESSION cap and means "fall through to the next rung" on the per-RUN one — upstream's, via `normalizeMaxSubagentSpawnsPerRun`'s `> 0` filter, documented now at both resolvers. Tests: 27 in `exec::run_fanout_budget::tests` plus the `Run fan-out budget` assertions in `registration::doctor::tests::build_doctor_report_has_four_filesystem_dirs_and_per_source_counts`; RED established by mutation (eight rules replaced with their plausible naive forms across the two commits, nine tests failing and all passing after). `cargo nextest run -p cyrup-ext-subagents` 2833/2833; clippy `--all-targets -D warnings` and `RUSTDOCFLAGS='-D warnings' cargo doc --no-deps` clean. **Still open — four:** `TOOL_TIMEOUT_MS` (needs the `effectiveToolTimeoutMs` / `DEFAULT_FAST_TOOL_TIMEOUT_TOOLS` subsystem and its parent-side per-tool-call timer, `runs/shared/tool-timeout.ts` + `foreground/execution.ts:1316-1333`, still absent), `RUNTIME_ACKNOWLEDGED_EXTENSIONS` and the parent-side WRITER of `TOOL_BUDGET_ZERO_AUTH` (both **SUBA-063**'s, owned there), and `LLM_INTENT_ARBITER` (needs a model-backed `createTaskMutationArbiter` to switch off; cyrup has only the heuristic `exec/task_intent.rs::classify_task_mutation_intent`). `PI_CODING_AGENT_PACKAGE_ROOT` remains **inapplicable**, recorded not ported. **RESIDUALS of this pass (each low, none blocking):** the ledger is not yet CREATED or CLAIMED against on any live run path — no executor entry calls `create_run_fanout_budget`, `spawn_plan.rs` does not yet write `RUN_FANOUT_BUDGET_ENV` into the fan-out-authorized child overlay (pi `pi-args.ts:942-943`), and no dispatch calls `claim_run_fanout_batch_in` (pi `subagent-executor.ts:6477`, `:5229`, `background/subagent-runner.ts:3991`); the mechanism and its doctor surface exist, the enforcement wiring is the next slice and needs an area-09 item. **The doctor block now SAYS so (batch-4 review, `RunFanoutDoctor::lines`):** it renders a `[CYRUP-DELTA]` line — *"- enforcement: NOT WIRED — no run creates or claims against this budget yet, so the limit above is reported but not applied"* — because until that slice lands `- configured limit: 64 (default)` describes a cap no run applies and `- usage: available after a run starts` can never become true, which is exactly the "misleading doctor surface" this row's own 2026-09-04 disposition refused to ship the resolved number ahead of. Disclosing it in the ledger alone is not disclosing it to the operator reading `/subagents-doctor`. Two defects in the same pass's own output were fixed with it: `with_admission_lock` released the lock on a straight-line statement, so a panic in the caller-supplied `commit` closure of `claim_run_fanout_batch_with_commit_in` leaked `admission.lock` under a LIVE pid — never reclaimed by `admission_lock_is_stale`, so every later admission for that run timed out — where upstream releases in a `finally` (`run-fanout-budget.ts:201-204`); it is a drop guard now, pinned by `a_panic_inside_the_commit_closure_still_releases_the_admission_lock` (RED without the guard). And `lines()` degraded the usage line to EMPTY via `strip_prefix(...).unwrap_or_default()` if the formatter's prefix ever changed; it degrades to the whole sentence now. Also unported with it: pi's `Spawn budget` doctor section (the per-session budget IS ported, only its doctor block is not), the `SteeringRecoveryDescriptor.runFanoutBudget` recovery leg, and `PI_SUBAGENT_FS_RETRY_MAX_TOTAL_MS`'s clamp on the shared retry ladder. **UNFIXED AT MERGE, caught by the batch-4 regression gate after the review round closed and recorded rather than filed:** the CLAIM rollback in the same function (`run_fanout_budget.rs:888-902`) got none of the panic-safety the LOCK got — it rolls back only under `if outcome.is_err()`, so a panicking (or inner-`Err`-returning) `commit` permanently burns the batch's slots where upstream's `catch` at `run-fanout-budget.ts:257-262` unlinks them, and `033e8c79`'s own test asserts the divergent `used == 2` (upstream: 1) as correct. See the detail block. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN at medium for the four; three findings, one of which DELETES AN OPEN DEFECT recorded above and one of which warns that the enumeration's premise is superseded upstream.** **(1) The `UNFIXED AT MERGE` block above is RESOLVED — read it as history.** `claim_run_fanout_batch_with_commit_in` (`crates/cyrup-ext-subagents/src/exec/run_fanout_budget.rs:849-925`) now uses the drop guard the block asked for: `struct RollbackOnDrop { created, armed }` with an `impl Drop` unlinking every slot (`:889-907`), armed at creation and disarmed only on `outcome.is_ok()` (`:920-922`), so a panic out of the caller's `commit` rolls the batch's claims back. The test was corrected with it — `a_panic_inside_the_commit_closure_releases_the_lock_and_rolls_back_its_claims` (`:1535`) now asserts `snapshot.used == 1` with the comment "the panicking batch's claim must not survive its own unwind", where the block's complaint was that it asserted `2`. Only the block's second sub-claim survives, and it is NOT a divergence: a `commit` returning `T = Result<_, Err>` still disarms, but upstream's `commit` returning a rejected VALUE is not a `throw` either, so upstream does not roll back there. **(2) The three still-open vars verify exactly as this row says.** `TOOL_TIMEOUT_MS` has no resolver — the only related symbol is `discovery/runtime_registry.rs:180` `default_tool_timeout_ms`, which is parsed and then DISCARDED, listed by name in `UNREPRESENTABLE_FIELDS` (`:110-124`, the `defaultToolTimeoutMs` entry) with zero consumers outside that file. `RUNTIME_ACKNOWLEDGED_EXTENSIONS` has no acknowledgement protocol — every `acknowledg*` hit in `cyrup-ext-subagents/src` is the unrelated STEER ack in `background/control.rs`. `LLM_INTENT_ARBITER` has only the heuristic `exec/task_intent.rs:1274 classify_task_mutation_intent` and no model-backed arbiter to switch off. **(3) THE ENUMERATION'S PREMISE IS SUPERSEDED UPSTREAM — read this before scheduling any of it.** At pi-subagents **v0.67.0**, SEVEN of the twelve names no longer exist at all (`TASK_DELIVERY`, `STEER_CAPABILITY`, `STEER_ACK_DIR`, `CAPABILITY_CEILING_V1`, `RUN_FANOUT_BUDGET`, `TOOL_BUDGET_ZERO_AUTH`, `RUNTIME_ACKNOWLEDGED_EXTENSIONS`); only `TOOL_TIMEOUT_MS` (`runs/background/async-execution.ts:205,277`), `MAX_SPAWNS_PER_RUN`, `ASYNC_EVENTS_MAX_BYTES` (`runs/background/subagent-runner.ts:328`), `LLM_INTENT_ARBITER` (`runs/shared/llm-intent-arbiter.ts:20,248`) and `PI_CODING_AGENT_PACKAGE_ROOT` remain. The whole `PI_SUBAGENT_*` child-env overlay collapsed into ONE variable, `PI_SUBAGENT_RUNNER_CONFIG`, an absolute path to a JSON config (`runs/background/binary-bootstrap.ts:14-17`; `runs/shared/child-runtime-config.ts`). The v0.64.0 citations in this row remain correct AS HISTORY and the row stays open on that baseline — but the standing instruction to "close it by reference when area 09 files per-var items" now needs an owner decision this row cannot make, because those items would be filed against a contract upstream has deleted. **RE-READ 2026-09-16 at `cc7818b`. Disposition UNCHANGED (still partial), but the 2026-09-04 disposition table carries four claims that are now FALSE, and they are the kind that read as extra coverage.** `cyrup-ext-subagents` took ~142 of the ~162 files in `9aeba769..cc7818b`, so every cell was re-run rather than carried forward. **(1) The four `(+ _PI_ALIAS)` parentheticals are dead.** `TASK_DELIVERY_ENV`, `RUN_FANOUT_BUDGET_ENV`, `MAX_SPAWNS_PER_RUN_ENV` and `ASYNC_EVENTS_MAX_BYTES_ENV` are each documented above as shipping a `PI_`-spelled alias beside the `CYRUP_` name; `git grep -n '_PI_ALIAS' -- crates/cyrup-ext-subagents/src` is **0**. `dd44b3c` (*"drop every `PI_*` env-var alias in favour of `CYRUP_`-only"*, 2026-09-11) removed them workspace-wide and flipped the matching tests to assert the `PI_` spelling is inert. This is a deliberate hard rename, not a regression — but the cells claim a compatibility surface the binary no longer has, and an operator migrating from `pi` reads them. **(2) The three OPEN vars are still open, re-measured not assumed:** the full `CYRUP_SUBAGENT*` name set at `cc7818b` is 64 strings and contains no `TOOL_TIMEOUT_MS`, no `RUNTIME_ACKNOWLEDGED_EXTENSIONS` and no `LLM_INTENT_ARBITER`. `git grep -in 'intent_arbiter' -- crates/` is 0 workspace-wide. `toolTimeoutMs` now has a FRONTMATTER validator (`crates/cyrup-ext-subagents/src/discovery/frontmatter.rs:1214-1228`, SUBA-086) that range-checks the agent-level key and discards it — so the value is parsed and thrown away, with still no resolver and no enforcement; that is a smaller gap than the cell describes and the cell should say so. **(3) `RUN_FANOUT_BUDGET`'s residual is unchanged and is the one worth re-reading:** `git grep -n 'claim_run_fanout_batch\|create_run_fanout_budget_in\|RUN_FANOUT_BUDGET_ENV' -- crates/cyrup-ext-subagents/src` outside `exec/run_fanout_budget.rs` itself is **0 hits**. The whole ledger — including the drop-guarded admission lock and the doctor block — still has **no production caller**, which makes it this codebase's fifth instance of tested machinery with no production reader. The SCOPE sequence (`#137`/`#139`/`#140`) landed in this exact crate and did NOT wire it. **2026-09-23 (TAIL batch): `RUNTIME_ACKNOWLEDGED_EXTENSIONS` is PORTED — the acknowledgement protocol exists now (SUBA-063 CLOSED; see the table row below and area 09) — and the parent-side `TOOL_BUDGET_ZERO_AUTH` writer is re-scoped to `SUBA-022`, whose delegation bridge is upstream's only producer. This row stays open for `TOOL_TIMEOUT_MS` and `LLM_INTENT_ARBITER` only.** **RE-WALKED 2026-09-24 at pi-subagents v0.71.0 (second pass): `LLM_INTENT_ARBITER` no longer exists upstream (removed v0.71.0, `7c98a696`), so that limb CLOSES; the row stays open for `TOOL_TIMEOUT_MS` ONLY. See the re-walk block at the top of the body.** |
| CFG-068 | medium — **PARTIALLY CLOSED 2026-09-16** | cyrup-original | S | `CYRUP_HOME` is invented, live in shipped builds, and takes precedence over `$HOME` at four sites at once — undocumented in `--help` and described in-source as a test knob — **filed 2026-08-14** (env-var surface). Needs an owner decision: promote it or confine it to test builds. **STILL OPEN 2026-09-04, no owner decision made:** `3f9380f` consolidated the mechanism into one place (`cyrup_config::paths::ENV_HOME` / `cyrup_home_dir_from`, `crates/cyrup-config/src/paths.rs:320`), but its own doc comment still reads *"Nothing in this workspace sets it outside tests, where it is the sandbox lever that keeps a run's artifacts out of the developer's real home"* — i.e. still live in production code, still not `#[cfg(test)]`-gated, and `grep -rn CYRUP_HOME crates/cyrup/src` is still zero, so `--help` still does not document it. The row's underlying claim is unchanged; only its citation moved. **PARTIALLY CLOSED 2026-09-16 at `cc7818b` — the row was stale, and it was stale BEFORE `9aeba769`; this row carries no 2026-09-14 marker, which is why the 2026-09-14 pass did not catch it.** The `Fix` offered two branches — **promote** (document it, define its precedence against `CYRUP_AGENT_DIR`, list it in `--help`) or **confine** (`#[cfg(test)]`). The PROMOTE branch was taken and is **two thirds landed**, in commits that all predate this ledger's own baseline. **(a) Documented — DONE.** `docs/guide/reference/environment.md:56-75` carries a section titled *"The three directory variables that are not synonyms"* (landed `e4d5e63`), a three-row table giving `CYRUP_HOME`'s reader set (*"subagents, intercom and the permission system only"*), its default (`$HOME`) and what it relocates, plus the paragraph *"`CYRUP_HOME` does not move your configuration"* at `:68-71`; `crates/cyrup-ext-subagents/resources/docs/configuration.md:113` and `agents.md:115` repeat it for the extension. **(b) Precedence against `CYRUP_AGENT_DIR` — DONE, and this is the half the row said was "undefined anywhere".** That is now false: `crates/cyrup-config/src/paths.rs:322` (`ENV_HOME`), `:344` (`ENV_CODING_AGENT_DIR`), `:337` (`ENV_AGENT_DIR_KEYS`), the ladder `cyrup_home_dir_from` at `:373` and `cyrup_dir_override_from` at `:402` define one ordering in one place, and `:395-401` states in terms the row asked for that `home` is a parameter precisely so a `~/…` agent-dir override and a home override cannot land in different directories — the concrete hazard this row named. **The `Verify` clause is executed, not designed** — three tests in `crates/cyrup-config/src/paths.rs`: `the_home_ladder_is_one_ladder` (`:451`, `CYRUP_HOME` beats `HOME`, blank-is-unset), `every_agent_dir_spelling_reaches_the_one_ladder` (`:485`) and `a_tilde_agent_dir_anchors_on_the_supplied_home_not_an_ambient_one` (`:521`). **STILL OPEN, and it is exactly one clause of one branch: `--help` does not list it.** `crates/cyrup/src/cli/help.rs:22-23` names `CYRUP_AGENT_DIR` and `CYRUP_SESSION_DIR` and nothing else; `git grep -n CYRUP_HOME -- crates/cyrup/src` is **0**, and `crates/cyrup/src/cli/tests/help.rs:9` asserts only the `CYRUP_AGENT_DIR` string. The confine branch was NOT taken either — none of the reader sites is `#[cfg(test)]`, so the var is still live in shipped builds, which the `paths.rs:320-322` doc still describes as a test sandbox lever. **Do not round this to closed:** the operator-discoverability half of the complaint (`--help`) is the half a shipped binary exposes. **Landing shas:** `da970e8` (`cyrup_home_dir_from` + the one ladder), `e4d5e63` (the guide section), `dd44b3c` (the `PI_*` alias drop that made the ladder single-spelling). **Closes when** `cyrup --help` names `CYRUP_HOME` and `cli/tests/help.rs` asserts it — or when an owner records the confine decision instead. |
...
| CFG-070 | low | cyrup-original | S | `AWS_CONFIG_FILE`, `AWS_SHARED_CREDENTIALS_FILE` and `APPDATA` are read by cyrup's hand-rolled credential resolvers and appear nowhere in pi's source, which inherits them from `@aws-sdk` / `google-auth-library` — **filed 2026-08-14** (env-var surface). **Correct as written — do NOT "fix" it by removing the reads.** Recorded so a later fidelity pass does not read them as unexplained branches. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN by design ("this row is a record, not work") and the record is still accurate, but TWO OF THE THREE CITATIONS ARE DEAD.** `bedrock_converse_stream.rs` became a module: the AWS pair is now at `crates/cyrup-provider/src/api/bedrock_converse_stream/config.rs:256` (`AWS_SHARED_CREDENTIALS_FILE`) and `:260` (`AWS_CONFIG_FILE`), with the boundary doc line this row quotes verbatim at `:247-249` ("Honors AWS_SHARED_CREDENTIALS_FILE and AWS_CONFIG_FILE, exactly as the SDK does. Role assumption / SSO / IMDS are not ported"). `APPDATA` moved one line, to `crates/cyrup-provider/src/auth/google_adc.rs:228`, inside `resolve_source`, with the Windows well-known path documented at `:99`. Upstream half re-verified at **v0.85.1**: neither AWS name appears anywhere in `packages/`; `APPDATA` appears only in a markdown doc (`packages/coding-agent/docs/terminal-setup.md:120`), never in product source; and `packages/ai/src/env-api-keys.ts:61` still hardcodes only the POSIX ADC path. **"Do NOT fix this by removing the reads" still applies.** One addition this pass, which this row does not cover and its own rationale invites: `crates/cyrup-tools/src/path.rs:169` is a THIRD cyrup `APPDATA` read — the Windows roaming-dir resolver, with a documented win32 known-folder fallback at `:158-169`. It is not a credential resolver and is out of this row's scope, but a name-level parity diff will flag it for the same reason it flags these three, so it needs an owner in whichever area covers `cyrup-tools`. |
...
| ~~CFG-073~~ | ~~low~~ **CLOSED 2026-09-24 — REFUTED (the surviving write is a faithful port of pi-subagents)** | cyrup-original | S | `NO_COLOR` and `CI` are read where pi reads neither — behaviour that changes under CI and not locally is the divergence class that hides — **filed 2026-08-14** (env-var surface). **NOT VERIFIED THIS PASS, left open with a note per README's "publish what you excluded":** the row's `CI` citation (`crates/cyrup-ext-subagents/src/exec/acceptance.rs:2702`) no longer resolves — `acceptance.rs` was split (`aad9ba0`, "decompose discovery.rs and git_url.rs, clear hygiene debt", touched adjacent modules) and an exhaustive `grep -rn '"CI"'` / `env::var("CI")` over `crates/cyrup-ext-subagents/src` now returns only unrelated test-fixture JSON (`exec/acceptance/lattice/lowering.rs:423`/`:441`, a `verify[].env` test value, not a process-env read). I could not determine in this pass whether the production `CI` read was removed outright, folded into the new lattice/verify-contract machinery under a different name, or simply relocated somewhere my grep missed — that needs someone reading the whole `exec/acceptance/` module tree, not a citation chase. The `NO_COLOR` half is unchanged: `crates/cyrup-tui/src/watchdog/lsp_diagnostics.rs`'s equivalent moved to `crates/cyrup-ext-subagents/src/watchdog/lsp_diagnostics.rs:904`, still an unconditional `.env("NO_COLOR", "1")` set on the spawned LSP child (a write to the child's environment, not a read of cyrup's own — a narrower claim than the row's headline makes, worth a future correction). **FIX SITE: `crates/cyrup-ext-subagents` (area 09).** **PARTIALLY CLOSED 2026-09-16 at `cc7818b` — the question this row explicitly left open is now SETTLED, in the direction of closure, and the surviving half needs re-characterising.** **The `CI` half is CLOSED.** The prior pass could not tell whether the production `CI` read had been removed, renamed or relocated, and said so. One decisive measurement answers it: `git grep -nE 'var(_os)?\("CI"\)' -- crates/` is **0 files** workspace-wide — there is no process-environment read of `CI` anywhere in cyrup. The three surviving `"CI"` string literals are all non-reads and were each opened: `exec/acceptance/lattice/lowering.rs:423` is a `"env": {"CI": "1"}` **test fixture**, `:448` is that same fixture's `env.get("CI")` **assertion** over the lowered JSON (a map lookup, not `std::env`), and `workflows/host_step.rs:276` is a `"label": "CI"` display string. The divergence is gone, not hidden. **The `NO_COLOR` half survives but this row MISDESCRIBES it, and the misdescription is the reason it reads worse than it is.** The single site is `crates/cyrup-ext-subagents/src/watchdog/lsp_diagnostics.rs:950` — `.env("NO_COLOR", "1")` on a `tokio::process::Command` builder. That is cyrup **WRITING** `NO_COLOR` into a spawned language-server child, not **READING** it, so the row's framing (*"behaviour that changes under CI and not locally is the divergence class that hides"*) does not apply: nothing here branches on the ambient environment, and the value is a constant. **Re-file the residual against its real class.** A var written unconditionally into every child of a given kind, which upstream never writes, is the `CFG-069` (`AI_AGENT`) class, not this one — same Kind (`cyrup-original`), same severity floor, but a different `Fix` (a `[CYRUP-DELTA]` at the spawn site naming why the LSP child is forced monochrome) and a different `Verify` (assert the delta line, not an ambient-env behaviour flip). **Closes when** that delta line exists at `lsp_diagnostics.rs:950`. The `CI` limb needs no further work. **CLOSED 2026-09-24 at `ea23ca2` — REFUTED; no code change.** Both halves re-measured on both sides. **`CI`:** `grep -rn '"CI"' --include=*.rs crates/` outside tests still yields only the lattice fixture (`exec/acceptance/lattice/lowering.rs:440`, `:465`) and two display labels (`workflows/host_step.rs:276`, `background/async_status_snapshot/project.rs:1090`); no process-environment read exists. **`NO_COLOR`:** the one site, `crates/cyrup-ext-subagents/src/watchdog/lsp_diagnostics.rs:950` `.env("NO_COLOR", "1")` on the LSP child, is **a faithful port, not a `cyrup-original`**: pi-subagents writes the same variable into the same child — `src/watchdog/lsp-diagnostics.ts:450` `env: { ...process.env, NO_COLOR: "1" }` @v0.64.0 and unchanged @v0.71.0. The 2026-09-16 re-classification into the `CFG-069` class assumed "upstream never writes" it, which was true of `pi` and false of the sibling upstream that owns this code — the env-var surface was walked against `pi` only, the blind spot README's *Surfaces left to walk* names. The `[CYRUP-DELTA]` that cell asked for would therefore be wrong. **Falsification:** reopen if a `std::env::var("CI"\|"NO_COLOR")` read appears under `crates/`, or if a later pi-subagents tag drops the `NO_COLOR` write from `lsp-diagnostics.ts`. |
| CFG-074 | medium | cyrup-original | M | Nine invented env vars across the three sibling ports — `CYRUP_PERMISSION_SYSTEM` (an opt-in over a SECURITY gate), two permission-forwarding knobs upstream keeps as compile-time constants, three `CYRUP_INTERCOM_*` transport/broker vars, `CYRUP_SUBAGENT_AGENT_NAME`, `CYRUP_HOOK_WARMUP` and `CYRUP_SUBAGENTS_TEMP_ROOT` — **filed 2026-08-14** (env-var surface). Each is defensible; none is currently KNOWN, and each wants a `[CYRUP-DELTA]` naming the upstream file:line it replaces. **RE-CONFIRMED STILL OPEN 2026-09-16 at `cc7818b`; this row carries no 2026-09-14 marker, so all nine were re-measured rather than carried forward.** All nine invented names are still live: `CYRUP_PERMISSION_SYSTEM`, `CYRUP_PERMISSION_SYSTEM_FORWARDING_AGENT_DIR`, `CYRUP_PERMISSION_FORWARDING_TIMEOUT_MS`, `CYRUP_INTERCOM_TRANSPORT`, `CYRUP_INTERCOM_TCP`, `CYRUP_INTERCOM_BROKER_BINARY`, `CYRUP_SUBAGENT_AGENT_NAME`, `CYRUP_HOOK_WARMUP` and `CYRUP_SUBAGENTS_TEMP_ROOT` each still resolve in `crates/`. **One upstream-facing premise of this row STRENGTHENED rather than decayed, and it is worth stating because it cuts the other way from what a reader expects.** The row's `Note for the record` observes that `PI_PERMISSION_SYSTEM_POLICY_AGENT_DIR` is carried as a legacy alias and is NOT fabricated. That alias is gone: `dd44b3c` dropped every `PI_*` spelling workspace-wide, so the sibling ports now present a `CYRUP_`-only surface with **no** migration path from pi's real names — which makes the nine inventions more visible to an operator, not less, and makes the `Fix`'s per-var `[CYRUP-DELTA]` requirement the only remaining record of what each one replaces. **`CYRUP_SUBAGENT_AGENT_NAME` was re-checked against the SCOPE diff specifically**, since `#137`/`#139`/`#140` rewrote ~142 files of `cyrup-ext-subagents`: it is still set by the spawn overlay and still has no `PI_SUBAGENT_AGENT_NAME` counterpart upstream. **FIX SITES unchanged: areas 09, 10 and 11** — none of the nine is area-05 work, which is why this row has survived four passes without moving. **RE-WALKED 2026-09-24 at pi-subagents v0.71.0 / pi-intercom v0.14.0 (second pass): three of the nine are REFUTED — `CYRUP_INTERCOM_TRANSPORT`/`CYRUP_INTERCOM_TCP` rename pi-intercom's real `PI_INTERCOM_TRANSPORT`/`PI_INTERCOM_TCP` (present v0.9.0…v0.14.0) and `CYRUP_SUBAGENTS_TEMP_ROOT` renames the real `PI_SUBAGENTS_TEMP_ROOT` (v0.64.0 and v0.71.0). Six remain invented; see the re-walk block in the body.** |
...
| ~~CFG-076~~ | ~~low~~ **CLOSED 2026-09-04 for item 3, the row's one real defect; items 1 and 2 remain correct-as-written records** | cyrup-original | S | Three `PI_`→`CYRUP_` rename exceptions, one of which is a live inconsistency INSIDE cyrup: `CYRUP_AGENT_DIR` (short) in `cyrup-config` vs `CYRUP_CODING_AGENT_DIR` (long) in `cyrup-ext-subagents` — **CLOSED 2026-09-04: `3f9380f` ("one home ladder and one agent-dir ladder for the workspace") is exactly the fix this row asked for.** `cyrup_config::paths::ENV_AGENT_DIR_KEYS` (`crates/cyrup-config/src/paths.rs:328-329`) now lists all three spellings — `CYRUP_AGENT_DIR`, `CYRUP_CODING_AGENT_DIR`, `PI_CODING_AGENT_DIR` — in precedence order, and `crates/cyrup-ext-subagents/src/paths.rs::resolve_agent_dir_from` (`:128-133`) routes through it via `cyrup_config::paths::cyrup_agent_dir_from`, replacing the crate's own hard-coded long-form-only read. The doc comment on that function names this exact row's defect verbatim: *"cyrup_config has read it since CFG-076, this function read only the outer two, and the gap meant an operator setting the long spelling moved the binary's layout while this crate's agent memory, run history, settings, prompts and sessions stayed in the old tree."* Items 1 (`PI_SUBAGENT_PI_BINARY` → `CYRUP_SUBAGENT_BINARY`) and 2 (`PI_CODING_AGENT_DIR`/`_SESSION_DIR` noun-shortening) were never defects and are untouched. **Note:** `native_supervisor.rs::intercom_agent_dir_from` still reads `CYRUP_CODING_AGENT_DIR` alone by design — its own doc explains this is `getAgentDirPath`'s cwd-relative contract, a genuinely different question from the home-anchored ladder above, so it is not a residual of this closure. |
| CFG-077 | low | cyrup-original | S | Prompt roots are scanned **recursively** and named by root-relative path, where pi's `loadTemplatesFromDir` is non-recursive and names by basename — **filed 2026-08-19 as a RECORD, not a defect.** `/flux/*` exists only because of this (`cyrup-flux/src/extension.rs:128-131` says so in as many words), so a parity sweep that "restores" pi's flat scan deletes fifteen shipped commands and renames every remaining template. **Correct as written — do NOT close it by removing the recursion.** Already pinned by nine `npt_*` tests. **RE-READ 2026-09-14 at `9aeba769` — STILL OPEN exactly as written ("this row IS the fix"); the one stated residual is unchanged and has grown a sixth citing site, and the cyrup citations have MOVED.** Upstream re-verified at **v0.85.1** and byte-stable in substance: `packages/coding-agent/src/core/prompt-templates.ts:134-136` still carries the docstring "Scan a directory for .md files (non-recursive) and load them as prompt templates." over `loadTemplatesFromDir`, and `loadTemplateFromFile` (`:104`) still derives the name from the basename with the `.md` suffix stripped (`:109`). cyrup still diverges and every rule this row enumerates holds — but `discovery.rs` became `discovery/scan.rs`, so `scan_prompt_root` is now `scan.rs:331` (handing off to the recursive `scan_prompt_dir` at `:349`), the `[CYRUP-DELTA]` is `:328-333`, `MAX_PROMPT_NAMESPACE_DEPTH = 8` is `:326`, the per-directory sort `:363`, the `.` / `_` / `node_modules` dir skip `:383-385`, the depth-cap warning `:386-393`, and the symlink policy (directory symlinks never followed, file symlinks loading when the target is a regular `.md`) `:372-378`. **THE RESIDUAL PERSISTS AND HAS GROWN A SITE:** `spec/namespaced-prompt-templates.md` still does not exist in the tree (`git ls-tree 9aeba769 spec/` has no match), while SIX source sites cite it as their governing spec — `crates/cyrup-resources/src/discovery/scan.rs:323`, `:329`, `prompt.rs:15`, `:34` (inside a `[CYRUP-DELTA]` whose stated authority IS that file), `:57`, and `crates/cyrup-resources/tests/resources/prompt_namespaces.rs:14` — plus `spec/flux.md:63`, which still notes the absence in passing. This row's own framing applies unchanged: a `[CYRUP-DELTA]` whose authority is a file nobody can read is the shape the directory README says to treat as an unverifiable claim rather than a decision of record. Either write the spec or re-point the six citations at this row. Severity `low` stands. |
| ~~CFG-078~~ | ~~low~~ **CLOSED 2026-09-05 — FIXED THIS PASS** | upstream-drift | S | `fullscreenExitOutput` and `fullscreenCopyOnSelect` (v0.84.4 additions) are not modelled — **filed 2026-09-04** (version-lag skim, `v0.84.1..v0.84.4`). **CLOSED 2026-09-05 — commit `69f6c4e1` (`feat(config,tui): CFG-078 …`), `crates/cyrup-config` + `crates/cyrup-tui` + `crates/cyrup`.** Both sides re-read at HEAD and at pi **v0.84.4**: `packages/coding-agent/src/core/settings-manager.ts:38` (`export type FullscreenExitOutput = "transcript" | "resume-hint";`), `:143` / `:145` (the two keys, defaults `"transcript"` and `true`, both "no effect in regular TUI mode"), getters `:1212-1214` / `:1233-1235`, setters `:1216-1220` / `:1237-1241`; consumers `modes/interactive/interactive-mode.ts:378` (`copyOnSelect` into `createInteractiveTui`), `:1995` (`applyRuntimeSettings`), `:4750-4760` (the two `/settings` handlers), `:6556` → `:836-843` (`stop()` reading the exit output per call), `components/settings-selector.ts:680-686` / `:694-700` (the two rows), and `packages/tui/src/tui-alt-screen.ts:205,229,246-248,1035` (the renderer flag and its ONE read site). **Config half** — `FullscreenExitOutput` in `settings/types.rs`, `EffectiveSettings::fullscreen_exit_output` / `::fullscreen_copy_on_select` in `settings/effective.rs` with pi's degrade-don't-reject contract (an unknown spelling and an absent key both answer `transcript`; `?? true`, so an explicit `false` is honoured), and `SettingsManager::set_fullscreen_exit_output` / `::set_fullscreen_copy_on_select` writing the GLOBAL scope with upstream's own spellings. **TUI half — the row's `Fix` line asked for both, and both landed:** `AltScreen::copy_on_select` + `set_copy_on_select` gate `selection::release` exactly where `:1035` gates upstream, so the selection stays anchored and highlighted and only the clipboard write is withheld; and the session-exit teardown (`app/run.rs`) now passes `App::preserve_screen_on_exit()` rather than a hardcoded `false`, so `resume-hint` skips `altscreen::exit::repaint`. **One stale note in this file is corrected by that:** `altscreen/mod.rs` claimed the exit repaint was unreachable (`ALT_SCREEN_EXIT_REPAINT_UNREACHABLE`) because "the only construction site passes `true`" — at HEAD both teardowns passed `false` and the repaint was live; the module doc is rewritten to say `preserve_screen` is decided per teardown. The `/settings` grid gains pi's two rows (ids `fullscreenExitOutput` / `fullscreenCopyOnSelect`, descriptions verbatim), replacing the in-code note at `app/settings_rows.rs` that said the exit row was deliberately absent at v0.84.1 — the note this row existed to retire. **Design decisions** (three, recorded in the commit body): the exit output is a two-variant enum in BOTH crates and the tui-side copy is deliberately not the config type (same split ADR-0005 §A-3 made for `TuiMode` vs `TuiRenderMode`), read through the named `preserve_screen_on_exit()` because the boolean inverts; `copy_on_select` is a plain field defaulted `true` at construction rather than a new `AltScreen::enter` parameter (upstream's own shape, and no existing call site or test changes); `selection::route` takes the flag as an argument rather than storing it in `SelectionState`. **Tests** (each RED before, by mutation as well as by the API not existing): `cyrup-config settings::tests::getters::{fullscreen_exit_output_and_copy_on_select_degrade_to_pis_defaults, fullscreen_exit_output_and_copy_on_select_round_trip_through_the_global_scope}`; `cyrup-tui tests::alt_screen::copy_on_select_disabled_keeps_the_selection_and_withholds_the_copy` (pi's own `test/tui-alt-screen.test.ts:1003-1007` case); `cyrup-tui tests::fullscreen_scrollback::the_exit_output_setting_decides_whether_the_excursion_reaches_scrollback`; `cyrup-tui tests::settings_trust_selectors::the_settings_grid_offers_the_two_v0_84_4_alt_screen_rows`; and the new `cyrup-tui/src/tests/fullscreen_settings.rs` (three cases, end-to-end through `App::execute_command` + `App::handle_input` against a faux-provider session). `cargo nextest run` 228/228 (`-p cyrup-config`), 1394/1394 (`-p cyrup-tui`), 236/236 (`-p cyrup`); `cargo fmt --all -- --check`, clippy `-D warnings --all-targets` and `RUSTDOCFLAGS=-D warnings cargo doc --no-deps` clean on all three. **Residuals (two, both low).** (1) `app/run.rs`'s exit tail is still not reachable from a unit test — it lives inside `App::run`'s `select!` loop — so what is pinned is the DECISION (`preserve_screen_on_exit`), not that `run.rs` passes it; a literal `false` there would still compile. (2) pi ALSO changed `app.message.copy` (Ctrl+X) at v0.84.4 to prefer the active selection when copy-on-select is off (`interactive-mode.ts:2910`, `:6117-6127`), while `/copy` does not (`:3022`); cyrup funnels both into one `AppCommand::Copy` whose arm prefers a live selection UNCONDITIONALLY (the ADR-0005 §B-11 decision, unchanged by this pass). Splitting them needs a second command id and is filed as its own residual rather than smuggled in here. |
| ~~CFG-079~~ | ~~low~~ **CLOSED 2026-09-05 — FIXED THIS PASS** | upstream-drift | S | `defaultTools` (v0.84.4 addition) — an initial built-in tool selection — is not modelled — **filed 2026-09-04** (version-lag skim, `v0.84.1..v0.84.4`). **CLOSED 2026-09-05 — commit `0a697d62` (`feat(config,session-svc): CFG-079 …`), `crates/cyrup-config` + `crates/cyrup-session-svc`.** Both sides re-read at HEAD and at pi **v0.84.4**: `packages/coding-agent/src/core/settings-manager.ts:128` (`defaultTools?: string[]; // Initial built-in tool selection`), getter `:1273-1276` (`const tools = this.settings.defaultTools; return tools ? [...tools] : undefined;` — a defensive copy, no validation), and the whole contract at `src/core/sdk.ts:256-263`: `options.tools ?? (options.noTools ? [] : (configuredDefaultToolNames ?? defaultActiveToolNames))` then `.filter(name => !excludedToolNameSet?.has(name))`. The result is `initialActiveToolNames`, handed to `AgentSession` (`:397`) and applied at construction with `includeAllExtensionTools: true` (`agent-session.ts:406-410` → the `:2742-2745` branch of `_refreshToolRegistry`), so extension and SDK custom tools stay active whatever the setting lists. `docs/settings.md:226-244` names the eight built-ins and both edge rules: an empty array starts with no built-ins while preserving extension/SDK tools, and a project array REPLACES the global one. **Tag-to-tag (ADR-0006, ported at the latest tag):** the key landed at `4d9aa837c` ("add configurable default tools") as an `allowedToolNames` allowlist that ALSO suppressed extension and SDK custom tools; `541045ae0` ("preserve extension tools with defaults") narrowed it to the initial built-in selection before v0.84.4 shipped — v0.84.4's selection semantics are what is ported, not the first landing's allowlist. **Config half** — `EffectiveSettings::default_tools() -> Option<Vec<String>>` (`crates/cyrup-config/src/settings/effective.rs`), shaped like its sibling `enabled_models`: a plain passthrough, `None` for unset and `Some(vec![])` for an explicit empty list, with project-over-global replace falling out of `deep_merge`'s existing array rule. **Consumer half — the row's `Fix` line guessed `cyrup-session-svc`'s builder and that is exactly where it landed:** `select_active_tools` (`crates/cyrup-session-svc/src/builder.rs`) takes `default_tools: Option<&[String]>` and consults it on the ONE arm where neither `tools` nor `noTools` is set, replacing `DEFAULT_BUILTIN_TOOLS` there; the `\|\| !ALL_BUILTIN_TOOLS.contains(name)` leg is untouched, which is what keeps extension/embedder tools active (upstream's `includeAllExtensionTools`). The single call site reads `settings.effective().default_tools()`. Two in-code comments in that file already named `defaultTools` as the only route to `powershell` — a route that did not exist until this commit. **Design decisions** (recorded in the commit body): the value stays an `Option` end to end rather than being defaulted to a `Vec` of the four names at the config layer, so "unset" and "explicitly empty" cannot be confused — collapsing them makes `{"defaultTools": []}` mean read/bash/edit/write, the exact inverse of what it asks for (the same decision `CFG-056` records for `defaultThinkingLevel`); it is threaded as an ARGUMENT to `select_active_tools` rather than added to `SessionConfig`, because `SessionConfig` is the declarative CLI/caller input while `defaultTools` is a settings read upstream also performs inside `createAgentSession` — a field there would hand a caller a silent override and a second place the value can disagree with the file; and no new type/enum/state machine, since a `ToolSelection` enum over `{Default, Configured(Vec<String>)}` is isomorphic to the `Option` and buys no protection. **Tests**, each RED before by mutation as well as by the API not existing: `cyrup-config settings::tests::getters::default_tools_round_trips_and_distinguishes_unset_from_empty` (unset/empty/populated, an unvalidated unknown name, and the project-replaces-global merge — RED with the getter mutated to answer `Some(vec![])` for an absent key); `cyrup-session-svc builder::tests::{the_default_tools_setting_replaces_pis_four_built_ins, explicit_tool_flags_still_outrank_the_default_tools_setting}`, ported case for case from upstream's own `test/default-tools-setting.test.ts` at v0.84.4 — RED with the arm restored to ignore the parameter. `cargo nextest run` 229/229 (`-p cyrup-config`), 336/336 (`-p cyrup-session-svc`); `cargo fmt --all -- --check`, clippy `--all-targets -D warnings` and `RUSTDOCFLAGS=-D warnings cargo doc --no-deps` clean on both. **Residuals (two, both low).** (1) The port is pinned at `select_active_tools` and at the getter, not through a live session: no process was launched with a `defaultTools` in `settings.json` to watch the resulting tool array reach a provider request. (2) **A separate defect found while reading this arm and NOT fixed here, because it is not this row's subject:** `--no-builtin-tools` (`NoTools::Builtin`) keeps `grep`/`find`/`ls`/`powershell` active — the arm is `!DEFAULT_BUILTIN_TOOLS.contains(name)`, so it drops only pi's four — where pi's `options.noTools ? []` (sdk.ts:262) starts with NO built-in active and only extension/SDK tools on. It deserves its own row. |
| ~~CFG-080~~ | ~~low~~ **CLOSED 2026-09-04 — FIXED THIS PASS** | port-divergence | S | An UNAUTHORISED `{"hard":0}` in `CYRUP_SUBAGENT_TOOL_BUDGET` ran the child with NO tool budget, where pi's child never gets the runtime at all — **CLOSED 2026-09-04 — commit `9a7c0fdb` (`fix(subagents): CFG-080 …`), the row's fix option (a), the faithful one.** Both sides re-read at the latest tags: pi-subagents **v0.64.0** `src/runs/shared/tool-budget.ts:74-80` (`decodeToolBudgetEnv` ends `if (normalized.error) throw new Error(normalized.error);`) called at registration in `src/runs/shared/subagent-prompt-runtime.ts:693`, and pi core **v0.84.4** `packages/coding-agent/src/core/extensions/loader.ts:545-587` — the throw is caught, `load.discard()` (`:559`) drops every registration the factory made and the loader records `Failed to load extension: …` (`:587`), so no child can run with the budget it was handed silently removed. cyrup at `85e6e902` did the inverse in `crates/cyrup-ext-subagents/src/prompt_runtime.rs`: the decode `Err` was `tracing::warn!`ed and the runtime built with `tool_budget: None`, so the one payload `CYRUP_SUBAGENT_TOOL_BUDGET_ZERO_AUTH` exists to gate gave the child UNLIMITED tool calls. **Fixed by making the refusal a value, not a log line:** `prompt_runtime_from_env`, `prompt_runtime_extension_from` and `prompt_runtime_extension_for_env` now return `Result<Option<…>, String>` carrying pi's own validation message, and `crates/cyrup/src/session_launch.rs`'s `attach_native_extensions` propagates it with `?` into the same `anyhow` chain that already carries the intercom extension's build failure, so the child exits before its first turn. `Ok(None)` ("not a subagent child") and `Err` ("this environment is not trustworthy") stop sharing one `None`; the fail-open arm is no longer expressible. **[CYRUP-DELTA]**, recorded at the match site: pi loses only this extension and its child keeps running; cyrup has no per-extension quarantine at its attach point, so the refusal ends the launch — stricter, in the safe direction. **Blast-radius note added 2026-09-05 (batch-3 review), row text and match-site comment both:** the delta is WIDER than a subagent child. `prompt_runtime_from_env` decodes the budget before its inertness check and `attach_native_extensions` calls `prompt_runtime_extension_for_env()` unconditionally (`crates/cyrup/src/session_launch.rs:130`), so a stray or stale `CYRUP_SUBAGENT_TOOL_BUDGET` aborts a TOP-LEVEL interactive launch too, where pi would merely have lost one extension. The decision stands — only a parent writes that variable, so its presence in a top-level environment is already evidence the environment is not the one this process was handed — but the row previously described the radius as the child's alone. Tag-to-tag: the `throw` is byte-identical at v0.34.0 (the ported baseline) and v0.64.0; only `options.allowZero` was added between them (v0.36.0, ported by CFG-067). Tests: new `prompt_runtime::tool_budget_runtime_tests::an_unauthorised_zero_budget_refuses_the_runtime_instead_of_running_unbudgeted` (unauthorised `{"hard":0}`, non-JSON, and `soft > hard` each refuse with pi's message; the authorised `{"hard":0}` still builds an armed runtime), RED before (with the old drop arm restored it fails on the first case) and green after; `a_zero_budget_is_honoured_only_with_the_parents_authorisation` keeps the authorised path. `cargo nextest run -p cyrup-ext-subagents` 2794/2794, `-p cyrup` 236/236, `-p cyrup-it --features it` 493/493; clippy `-D warnings` and `RUSTDOCFLAGS=-D warnings cargo doc` clean on both crates. **Residual (low):** the launch-path leg is verified by construction (the `?` and the compiler) plus the crate suites — no live child was spawned with a tampered `CYRUP_SUBAGENT_TOOL_BUDGET` to watch the process exit. |
| CFG-081 | low | upstream-drift | S | **NEW 2026-09-24.** `retry.maxAgentDelayMs` (pi v0.86.0, default 60 s) is not modelled, so agent-level retry backoff is uncapped — see the body. |
| CFG-082 | low | upstream-drift | S | **NEW 2026-09-24.** `compaction.modelOverrides` (per-model `reserveTokens`/`keepRecentTokens`, pi v0.86.0) is not modelled, and pi's invalid-value throws are absent — see the body. |
| CFG-083 | low | parity-bug | S | **NEW 2026-09-24.** A prompt template with malformed YAML frontmatter is LOADED (as `{}`) with no warning; pi drops it at every tag since v0.83.0 and, from v0.87.0, reports a resource warning. The in-source comment claiming pi treats the fault as `{}` is wrong — see the body. |
| CFG-084 | low | upstream-drift | S | **NEW 2026-09-24.** `default_model_per_provider` differs from pi v0.87.1 on six providers (`cerebras`, `zai`, `zai-coding-cn`, `xai`, `radius`, and `meta`, which cyrup does not have) — see the body. |
| CFG-085 | low | upstream-drift | M | **NEW 2026-09-24.** models.json `inputLimits` (image resize profile, per-message/per-request image caps, `maxRequestBytes`), `promptCache` and anthropic `compat.allowedFallbackModels` (pi v0.86.0–v0.87.0) are silently ignored — see the body. |
| CFG-086 | low | upstream-drift | S | **NEW 2026-09-24 (second pass; two 2026-09-14 leads).** The skills loader reports every loose root `.md` (README, AGENTS…) as a broken skill and fails a skill whose `name` is not a string; pi v0.84.3 only diagnoses files named `SKILL.md` and type-guards `name` — see the body. |
| CFG-087 | medium | upstream-drift | S | **NEW 2026-09-24 (second pass; was a 2026-09-14 lead).** No config reader strips a UTF-8 BOM, so a Windows-authored `settings.json`/`auth.json`/`trust.json`/`keybindings.json`/`models.json` fails to parse; pi v0.84.3 strips it at every read site — see the body. |
| CFG-088 | low | upstream-drift | S | **NEW 2026-09-24 (second pass; was a 2026-09-14 lead).** Settings load diagnostics keep v0.84.1's `(<context>, <scope> settings) <msg>` form: no file path, no dedup across the two managers, and printed pre-TUI in interactive mode; pi v0.84.3 names the file, dedups, and hands them to the TUI — see the body. |
| CFG-089 | low | upstream-drift | S | **NEW 2026-09-24 (second pass; was a 2026-09-14 lead).** Every `auth.json` write is temp-file + rename with a forced `0600`, replacing an administrator-set mode/ACL; pi v0.84.3 writes in place and applies `0600` only when creating the file — see the body. |
| CFG-090 | low | upstream-drift | S | **NEW 2026-09-24 (second pass; was a 2026-09-14 lead).** `terminal.hyperlinks`/`terminal.images`/`terminal.trueColor` capability overrides (pi v0.84.4) are not modelled; the TUI always auto-detects. Co-owned with area 07 — see the body. |
| CFG-091 | low | upstream-drift | S | **NEW 2026-09-24 (second pass; was a 2026-09-14 lead).** Default keybindings have no WSL arm (pi v0.84.4 treats `WSL_DISTRO_NAME`/`WSL_INTEROP` as Windows for five defaults) and no `app.thinking.save` action (pi v0.85.1, `ctrl+s` in the thinking selector) — see the body. |
| CFG-092 | medium | upstream-drift | S | **NEW 2026-09-24 (second pass; was a 2026-09-14 lead).** A models.json model declared under an existing provider inherits `api`/`baseUrl` from the provider's FIRST model; pi v0.85.0 `findModelDefaults` prefers the same id, then the same `api`, then `openai-completions` — see the body. |
| CFG-093 | low | upstream-drift | S | **NEW 2026-09-24 (second pass; was a header lead).** The global-only `cacheWarming` setting (`off`/`streaming`/`idle`, default `streaming`, pi v0.86.0) is not modelled — the config half of `SEAM-131` — see the body. |
...
**cyrup**    — `crates/cyrup-config/src/settings/effective.rs:650` `retry_settings()` carries `enabled`/`max_retries`/`base_delay_ms` only. The agent-level retry in `crates/cyrup-session-svc/src/session/retry.rs:113-115` computes `retry_base_delay_ms.saturating_mul(2^(attempt-1))` with no ceiling, and the summarization retry in `crates/cyrup-provider/src/utils/retry.rs:271-273` does the same over `RetryPolicy` (`:147-173`), which has no cap field.
**upstream** — `packages/coding-agent/src/core/settings-manager.ts:45` (`maxAgentDelayMs?: number; // default: 60000`) and `:932-939` `getRetrySettings()` @v0.87.1; the cap is applied by `retryDelayMs` (`packages/ai/src/utils/retry.ts:111-117`: `Math.min(safeDelay, policy.maxAgentDelayMs ?? 60_000)`), called from `core/agent-session.ts:3393` and from `ai/src/utils/retry.ts:209`. New at v0.86.0 (CHANGELOG, #8826).
**Impact**   — none at the defaults (3 retries × 2 s base peaks at 8 s). A user who raises `retry.maxRetries` gets pi's 60 s ceiling upstream and an exponential wait in cyrup — attempt 10 sleeps ~17 minutes — and a `maxAgentDelayMs` written into a shared `settings.json` is silently ignored.
**Fix**      — add `max_agent_delay_ms` (default 60 000) to `RetrySettings`/`EffectiveSettings::retry_settings`, to `RetryPolicy`, and to the session's retry fields (`session/mod.rs:101`, `builder.rs:2106`); clamp at both computation sites. Fix site spans areas 05, 08 and 01 (`cyrup-provider`), one commit.
**Verify**   — with `maxRetries: 10`, `baseDelayMs: 2000`, the tenth `AutoRetryStart` carries `delay_ms == 60_000`; with `maxAgentDelayMs: 5000` the third carries 5 000.

## CFG-082 — `compaction.modelOverrides` is not modelled

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24)
**cyrup**    — `crates/cyrup-config/src/settings/effective.rs:248`/`:255` `compaction_reserve_tokens()`/`compaction_keep_recent_tokens()` and `:641` `compaction_settings()` take no model and read no per-model map; `grep -rn modelOverrides crates/cyrup-config/src/settings` is empty (the only `modelOverrides` in the crate is models.json's, `model/compose.rs`).
**upstream** — `packages/coding-agent/src/core/settings-manager.ts:13-28` (`CompactionModelOverride`, `modelOverrides?: Record<string, CompactionModelOverride>` keyed by exact `provider/modelId`), `:859-894` `getCompactionTokenSetting` (override → ordinary → default, and a THROW on any non-safe-integer or negative value, ordinary or override), `:896` `getCompactionSettings(model?)` @v0.87.1; the model is passed at `core/agent-session.ts:589`, `:2419`, `:2604`. New at v0.86.0.
**Impact**   — a `compaction.modelOverrides` block is ignored, so a small-context model compacts on the global budget. Secondary: pi refuses an invalid `reserveTokens`/`keepRecentTokens` value by throwing where cyrup silently uses it.
**Fix**      — model the map in `settings/types.rs`, give `compaction_settings` a `(provider, model_id)` argument, resolve override → ordinary → default, and surface pi's two error strings as load errors. Callers are area 03's compaction entry points.
**Verify**   — with `{"compaction":{"reserveTokens":16384,"modelOverrides":{"anthropic/claude-opus-5-5":{"reserveTokens":4096}}}}`, `compaction_settings("anthropic","claude-opus-5-5").reserve_tokens == 4096` and any other model gets 16 384; `reserveTokens: -1` is a reported error.

## CFG-083 — A prompt template with malformed frontmatter is loaded as `{}` instead of dropped and reported

**Kind** parity-bug (the drop half, present at the ported tag) + upstream-drift (the warning half, v0.87.0) · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; not run)
**cyrup**    — `crates/cyrup-resources/src/prompt.rs:390-403` `parse_frontmatter` does `serde_yml::from_str(..).unwrap_or_default()` under the comment *"Pi silently treats a YAML parse fault as `{}` (prompt-templates.ts:129-131)"*, so `PromptTemplate::load_with_root` (`:60`) returns `Ok` with an empty frontmatter and the post-fence body.
**upstream** — **the comment is wrong at every tag.** `packages/coding-agent/src/utils/frontmatter.ts`'s `parseFrontmatter` calls `yaml`'s `parse`, which throws; at the ported baseline `core/prompt-templates.ts:104-133` @v0.83.0 wraps the whole load in `try { … } catch { return null; }`, so a malformed template is DROPPED. At v0.87.0 (@v0.87.1 `:105-150`) the throw is caught separately (`:121-127`) and pushed as a `{type:"warning", message, path}` resource diagnostic, then merged into the prompt diagnostics by `core/resource-loader.ts:700-710`. CHANGELOG 0.87.0 (#9830).
**Impact**   — a template whose YAML is broken still appears as a `/name` command in cyrup, with its `description`/`argument-hint` silently lost; pi shows no such command and, since v0.87.0, tells the user why.
**Fix**      — make `parse_frontmatter` return `Result` and map a YAML fault to `ResourceError::Manifest`. Nothing else is needed: every loader already turns a load `Err` into a `ResourceWarning` (`discovery/scan.rs:414-420`, and the `load` sites at `:276`, `:520`, `discovery/blocking.rs:472`), which is exactly pi's v0.87.0 shape. Delete the wrong comment.
**Verify**   — a `prompts/bad.md` with `---\ndescription: [unclosed\n---\nbody` yields no `bad` template and one prompt warning naming the path.

## CFG-084 — `default_model_per_provider` differs from pi v0.87.1 on six providers

**Kind** upstream-drift · **Severity** low · **Effort** S (M where the catalog lacks the id) · **Confidence** confirmed (mechanical diff 2026-09-24)
**cyrup**    — `crates/cyrup-config/src/model/defaults.rs:8-63`: `cerebras → zai-glm-4.7`, `zai → glm-5.1`, `zai-coding-cn → glm-5.1`, `xai → grok-4.6`, `radius → auto`, no `meta` arm. The in-source note at `:24-33` records that only `xai` was chased to v0.85.1, the other v0.85.1 rows being blocked on ids the embedded catalog lacks, and the `CHASED` constant in the test (`:320`) pins that.
**upstream** — `packages/coding-agent/src/core/model-resolver.ts` `defaultModelPerProvider` @v0.87.1: `radius: "balanced"` (`:27`), `xai: "grok-4.7"` (`:35`), `cerebras: "gpt-oss-120b"` (`:37`), `zai`/`"zai-coding-cn": "glm-5.3"` (`:38-39`), `meta: "muse-spark-1.3"` (`:52`). v0.85.1 already carried `cerebras`/`zai`/`zai-coding-cn` as now; v0.87.1 adds `xai`, `radius` and `meta`. Extraction: the `defaultModelPerProvider` object literal from `git show v0.87.1:…/model-resolver.ts`, sorted and diffed against the `match` arms — exactly six lines differ, no other.
**Impact**   — a fresh launch with only that provider's credentials picks a different default than pi. `meta` is a new provider (Meta Muse, v0.86.1) that cyrup does not ship at all — that half is area 01's.
...
**upstream** — `packages/coding-agent/src/core/model-config.ts` @v0.87.1: `ImageResizeSchema`/`ModelInputLimitsSchema` (`maxRequestBytes`, `images.{resize{maxWidth,maxHeight,maxBytes,jpegQuality}, maxPerMessage, maxPerRequest}`) and `ModelPromptCacheSchema` (`short`/`long` > 0), accepted on both model definitions (`:196`, `:198`) and `modelOverrides` (`:211`, `:221`); `allowedFallbackModels` (≤3 `{provider, model, cost}`) on the Anthropic compat schema (`:170`). Consumers: `core/tools/read.ts:112-118` (`resizeOptions: ctx?.model?.inputLimits?.images?.resize`), attachments and tool-result images (CHANGELOG 0.87.0, #9631); `promptCache` feeds `core/cache-warmer.ts`. New at v0.86.0 (`allowedFallbackModels`, `promptCache`) and v0.87.0 (`inputLimits`).
**Impact**   — a user who lowers a model's image resize profile, caps images per request, or disables Anthropic server-side fallbacks in models.json gets none of it in cyrup, with no diagnostic. Distinct from `CFG-043` (cyrup reports schema faults differently): here a VALID file is under-applied.
**Fix**      — schema first (both structs, both levels, pi's minimums as validation), then the consumers: the `read` tool's resize and the attachment path (area 04 / 08), `allowedFallbackModels` in the Anthropic request builder (area 01). `promptCache` has no consumer until cache warming exists (header lead).
**Verify**   — a model with `"inputLimits":{"images":{"resize":{"maxWidth":512}}}` makes `read` of a 2 000 px PNG return a ≤512 px image.

## CFG-086 — The skills loader diagnoses loose root `.md` files and rejects non-string `name`

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; not run)
**cyrup**    — `crates/cyrup-resources/src/discovery/scan.rs:147-151` loads every direct `.md` child of a skills root through `load_one_skill` (`:300-320`) → `Skill::load_with_diagnostics` (`crates/cyrup-resources/src/skill.rs:174-232`), which has no notion of a declared skill: a `README.md` with no frontmatter gets `validate_description`'s "description is required" warning; a YAML fault in ANY `.md` becomes a `ResourceWarning` (`?` at `:182-185`); and `SkillFrontMatter.name` is a typed `Option<String>`, so `name: 123` fails the serde parse and drops the whole skill with a warning.
**upstream** — `core/skills.ts:277-345` @v0.87.1 `loadSkillFromFile` (from v0.84.3, #7805): `isDeclaredSkill = basename(filePath) === "SKILL.md"`; a frontmatter parse error is reported only for a declared skill; a non-`SKILL.md` file with no string description is skipped SILENTLY (`if (!isDeclaredSkill && !hasDescription) return { skill: null, diagnostics }`); `frontmatter.name` is used only when `typeof === "string"`, else the parent directory name; a read failure is a warning.
**Impact**   — a repository whose skills root holds a `README.md` or notes shows spurious "broken skill" warnings at every start in cyrup and none in pi; a skill with a numeric `name` loads in pi (named after its directory) and is missing in cyrup.
**Fix**      — thread `is_declared = file_name == "SKILL.md"` into `load_with_diagnostics`: gate the parse-error warning and return `Ok((None, vec![]))` for an undeclared file without a string description before any validation; deserialize `name`/`description` as `serde_yml::Value` and keep them only when strings. Update the comment at `scan.rs` that cites the pre-fix range.
**Verify**   — a skills root with `README.md` (no frontmatter) and `bad.md` (malformed YAML) yields no skill and no diagnostic; `x/SKILL.md` with malformed YAML yields one warning; `y/SKILL.md` with `name: 123` loads as skill `y`.

## CFG-087 — No config reader tolerates a UTF-8 byte-order mark

**Kind** upstream-drift · **Severity** medium · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; serde_json's rejection of a leading U+FEFF is its documented behaviour, not run here)
**cyrup**    — every JSON config read hands the raw text to `serde_json`: `crates/cyrup-config/src/settings/layer.rs:50`, `auth.rs:61`, `trust.rs:116`, `keybindings.rs:321`, `models_store.rs:250`; `grep -rni 'feff\|bom' crates/cyrup-config/src crates/cyrup-resources/src` is empty. The only BOM handling in the workspace is the `edit` tool's and shell-output decoding (`cyrup-tools`).
**upstream** — `utils/text.ts:1-9` (`splitBom`/`stripBom`, new at v0.84.3) applied at every read site @v0.87.1: `settings-manager.ts:424`, `:645`; `auth-storage.ts:216`, `:366`, `:501`; `trust-manager.ts:105`; `keybindings.ts:363`; `model-config.ts:297`; `models-store.ts:63`; `migrations.ts:36`, `:51`, `:162`; `resource-loader.ts:61`, `:82`; `package-manager.ts:1504`, `:1886`; `pi-manifest.ts:19`; `utils/frontmatter.ts:12`; `cli/file-processor.ts:77`; theme JSON (`theme.ts:410-411`, `:499`).
**Impact**   — a `settings.json` saved by Notepad (or any editor that writes a BOM) is a load error in cyrup: the scope is treated as unreadable and, per `CFG-030`'s write latch, later `/settings` writes to it are refused. A BOM'd `auth.json` makes every stored credential unreadable; a BOM'd `models.json` drops the user's providers. pi reads all of them.
**Fix**      — one `strip_bom(&str) -> &str` in `cyrup-config` applied before each `from_str` above, plus frontmatter parsing in `cyrup-resources` and the theme loader (area 07); write paths keep the file BOM-free as pi's do.
**Verify**   — each of the five files, written with a leading `EF BB BF`, loads identically to its BOM-less twin.

## CFG-088 — Settings load diagnostics keep v0.84.1's shape, lack the path, are not deduplicated, and print before the TUI

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; not run)
**cyrup**    — `crates/cyrup/src/bootstrap.rs:108-122` `collect_settings_diagnostics` formats `({context}, {scope} settings) {message}`; `ScopedError` (`crates/cyrup-config/src/settings/manager.rs:33`) carries `{scope, message}` and no path. The startup and runtime managers are drained separately and both lists are reported, and the report happens on stderr before any mode starts.
**upstream** — `core/settings-diagnostics.ts` @v0.87.1 (new at v0.84.3): `Invalid settings file <path>: <msg>` (or `Invalid <scope> settings: <msg>` with no path), and `deduplicateDiagnostics` on `type`+`message`. `main.ts` @v0.85.1 `:893-900`: startup and runtime diagnostics are merged and deduplicated; they are printed only when not interactive or when a runtime error aborts, and otherwise handed to `InteractiveMode({startupDiagnostics})`; `--help` and `--list-models` print the startup set before exiting (`:854`, `:863`).
**Impact**   — a broken settings file produces a warning that does not say which file, twice (once per manager), scrolled away as the TUI takes the screen.
**Fix**      — carry the path on `ScopedError`, format pi's two strings, dedup the merged list, and route it to the TUI's startup notices in interactive mode (area 07's surface) while keeping stderr for print/json/rpc.
**Verify**   — with an invalid project `settings.json`, `cyrup -p hi` prints exactly one `Invalid settings file <abs path>: …` line; the interactive TUI shows it once in the transcript and nothing on stderr.

## CFG-089 — `auth.json` is replaced on every write, discarding an administrator-set mode

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; not run)
**cyrup**    — `crates/cyrup-config/src/auth.rs:342` writes through `crate::lock::write_atomic(path, bytes, secret: true)` (`crates/cyrup-config/src/lock.rs:336-373`): a temp file created `0600`, `set_permissions(0600)`, then `rename` over `auth.json`. Every write yields a new inode with mode `0600` and no ACL, whatever the old file had.
**upstream** — `core/auth-storage.ts` @v0.87.1: `AUTH_FILE_WRITE_OPTIONS = { encoding: "utf-8", mode: 0o600 }` (`:25`) passed to `writeFileSync` at `:65`, `:106`, `:187` — in place; Node applies `mode` only when it creates the file. The three post-write `chmodSync(authPath, 0o600)` calls were removed at v0.84.3 (present at v0.84.1).
**Impact**   — an operator who made `auth.json` group-readable for a service account, or put an ACL on it, has that undone by the next login or token refresh in cyrup; pi leaves it alone. Security-neutral for the default case (both end at `0600` for a new file).
**Fix**      — preserve the existing file's permissions across the atomic replace (read `metadata().permissions()` before, apply to the temp file) and apply `0600` only when the file did not exist. Keeping temp+rename is fine — it is a mechanism difference with a reason (crash safety); preserving the ACL needs an `xattr` copy on Linux and is the part that may be declared instead.
**Verify**   — `chmod 640 auth.json`, then a credential write: the mode is still `640`; a fresh `auth.json` is created `600`.

## CFG-090 — `terminal.hyperlinks` / `terminal.images` / `terminal.trueColor` are not modelled

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24)
**cyrup**    — `EffectiveSettings` has the four v0.84.1 terminal keys (`showImages`, `imageWidthCells`, `clearOnShrink`, `showTerminalProgress`) and nothing else under `terminal`; `grep -rni 'hyperlinks\|true_color\|truecolor' crates/cyrup-config/src` is empty; the TUI decides hyperlink and image support by sniffing the terminal (`crates/cyrup-tui/src/image.rs`).
**upstream** — `core/settings-manager.ts` @v0.87.1 `TerminalSettings` `:57-59` (`hyperlinks?: boolean | "auto"`, `images?: "kitty" | "iterm2" | "auto" | false`, `trueColor?: boolean | "auto"`), `getTerminalCapabilityOverrides()` `:1195-1203` (`images:false` → `{images:null}`, `"auto"` → no override), applied at startup by `main.ts` `setCapabilityOverrides(…)` (`:849` @v0.85.1). New at v0.84.4.
**Impact**   — a user on a terminal that mis-reports its capabilities (tmux, some SSH setups) cannot force images off, force Kitty graphics, or disable hyperlinks/true colour from settings; the key is silently ignored.
**Fix**      — model the three keys in `cyrup-config` with pi's mapping; the override seam and its consumers are area 07's (`cyrup-tui` capability detection).
**Verify**   — `{"terminal":{"images":false}}` makes the TUI render no inline images on a Kitty terminal; `"auto"` behaves as unset.

## CFG-091 — Default keybindings: no WSL arm and no `app.thinking.save`

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; not run)
**cyrup**    — the default table (`crates/cyrup-tui/src/keymap.rs`, action ids mirrored in `crates/cyrup-config/src/keybindings.rs:85`, `:156`) has no WSL detection (`grep -rn 'WSL_DISTRO_NAME\|WSL_INTEROP' crates/cyrup-config/src crates/cyrup-tui/src` is empty) and no `app.thinking.save` id.
**upstream** — `core/keybindings.ts` @v0.87.1: `useWindowsKeybindings()` (`:62-67`, v0.84.4) is `win32 || (linux && (WSL_DISTRO_NAME || WSL_INTEROP))` and selects the Windows default for `tui.editor.undo` (`alt+z`), `tui.altScreen.previousPrompt`/`nextPrompt` (`ctrl+up`/`ctrl+down`), `tui.altScreen.search` (`ctrl+f`) and `app.model.cycleBackward` (`alt+p`); `app.thinking.save` (`:104-107`, `ctrl+s`, v0.85.1) is consumed by `components/thinking-selector.ts:131` ("set as default").
**Impact**   — under WSL cyrup keeps the Linux chords that pi moved off because the terminal reserves them (CHANGELOG, #8372: "avoid terminal-reserved shortcuts for … model cycling, editor undo, fullscreen transcript navigation and search"), so those actions can be unreachable by default; and the thinking selector has no key to save a level as the default.
**Fix**      — add the WSL predicate beside the existing platform arms and the `app.thinking.save` action; the selector behaviour is area 07's.
**Verify**   — with `WSL_DISTRO_NAME=Ubuntu` on Linux, `app.model.cycleBackward`'s default is `alt+p`; `app.thinking.save` resolves to `ctrl+s`.

## CFG-092 — A declared models.json model inherits `api`/`baseUrl` from the provider's first model

**Kind** upstream-drift · **Severity** medium · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; not run)
**cyrup**    — `crates/cyrup-config/src/model/compose.rs:178-181`: `defaults = existing.map_or_else(|| models.first(), |i| models.get(i))`, and `model_from_json` (`:204-228`) falls back to `defaults.api` and `defaults.base_url`.
**upstream** — `core/provider-composer.ts:198-205` @v0.87.1 `findModelDefaults(models, id, definition.api ?? config.api)`: same id, else the first model with that `api`, else the first `openai-completions` model, else `models[0]`; used at `:239` and `:257`. New at v0.85.0 (absent at v0.84.4).
**Impact**   — on a built-in provider whose models span wire APIs, a model added in models.json with only `"api": "anthropic-messages"` gets `models[0]`'s `baseUrl` — another API's endpoint — so every request goes to the wrong URL; with no `api` at all it also inherits `models[0]`'s API instead of the completions one. pi composes both correctly.
**Fix**      — port `findModelDefaults` verbatim and pass `definition.api.or(config.api)`.
**Verify**   — provider with `[{id:a, api:openai-responses, baseUrl:X}, {id:b, api:anthropic-messages, baseUrl:Y}]`; a declared `{id:c, api:"anthropic-messages"}` composes with `baseUrl == Y`.

## CFG-093 — The `cacheWarming` setting is not modelled

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24)
**cyrup**    — no key: `grep -rn 'cache_warming\|cacheWarming' crates/` is empty.
**upstream** — `core/settings-manager.ts` @v0.87.1: `CACHE_WARMING_MODES` / `CacheWarmingMode` (`:78`), `cacheWarming?: CacheWarmingMode // default: "streaming"; global only because each refresh costs money` (`:157`), `getCacheWarmingMode()` reading `globalSettings` only (`:955-958`), `setCacheWarmingMode` (`:960-962`); read by `CacheWarmer` and `AgentSession.setCacheWarmingMode`. v0.86.0.
**Impact**   — none until `SEAM-131` lands; recorded so the warmer is built against the right key, the global-only scope and the `streaming` default.
**Fix**      — model the key GLOBAL-only (a project-scope value must be ignored, as upstream does), default `streaming`; ship with `SEAM-131`.
**Verify**   — a project `settings.json` with `cacheWarming:"off"` does not change the effective mode; the global one does.
...
## CFG-067 — Twelve `pi-subagents` env vars have no `CYRUP_` counterpart — **PARTIALLY CLOSED 2026-09-04, NARROWED AGAIN 2026-09-05**

> **RE-WALKED 2026-09-24 (second pass) at pi-subagents v0.71.0.** Literal-name census
> `git grep -h -o -E '\b(PI_SUBAGENTS?_[A-Z0-9_]+|PI_[A-Z0-9_]+)' <tag> -- src | sort -u`: 83 names
> at v0.64.0, 48 at v0.71.0. **Of this row's twelve, only four still exist at v0.71.0:**
> `TOOL_TIMEOUT_MS` (`src/runs/shared/tool-timeout.ts:1`, read as the lowest-precedence override at
> `src/runs/background/async-execution.ts:240`/`:318`/`:381`), `MAX_SPAWNS_PER_RUN`,
> `ASYNC_EVENTS_MAX_BYTES` and `PI_CODING_AGENT_PACKAGE_ROOT`. The others were removed upstream as
> children moved in-process — `TASK_DELIVERY` and `RUN_FANOUT_BUDGET` are gone from v0.65.0 on; the
> capability ceiling is now a `Symbol.for("pi-subagents.capability-ceiling.v1")` registry key
> (`src/runs/shared/capability-ceiling.ts:3-4`), not an env var; `LLM_INTENT_ARBITER` is gone at
> v0.71.0 (`7c98a696`, "remove inferred no-edit completion failures", #2356).
> - **`LLM_INTENT_ARBITER` limb: CLOSED — removed upstream.** There is no kill switch left to port.
>   *Falsification:* reopen if the name reappears at a later tag.
> - **`TOOL_TIMEOUT_MS` limb: still OPEN.** cyrup has the per-agent frontmatter default
>   (`crates/cyrup-ext-subagents/src/discovery/runtime_registry.rs:184` `default_tool_timeout_ms`) but
>   no env override: `grep -rn 'TOOL_TIMEOUT_MS' crates/cyrup-ext-subagents/src` is empty.
> - **The vars cyrup ported and upstream then removed** (`TASK_DELIVERY`, `RUN_FANOUT_BUDGET`,
>   `TOOL_BUDGET_ZERO_AUTH`, `RUNTIME_ACKNOWLEDGED_EXTENSIONS`) stay, because cyrup's children are still
>   OS processes (the `[CYRUP-DELTA]` on `RUNTIME_ACKNOWLEDGED_EXTENSIONS` says so); whether cyrup
>   follows upstream in-process is area `09b`'s question, not re-litigated here.
> - **New at v0.71.0 with no `CYRUP_` counterpart, not filed here:** `PI_SUBAGENT_CACHE_RETENTION`
>   (v0.68.0, `src/shared/child-cache-retention.ts`) — already a lead in `09b` (`:388`), so not filed
>   twice; `PI_SUBAGENT_RUNNER_CONFIG` / `PI_ASYNC_NATIVE_RUNNER` (`async-execution.ts:735-736`,
>   `background/binary-bootstrap.ts:21`) select a Node runner/bootstrap — inapplicable to a Rust child;
>   `PI_SUBAGENTS_HERDR_BRIDGE` (`src/runs/shared/herdr-pi-protocol.ts:5`) — herdr is reached through
>   cyrup's own client (`crates/cyrup-herdr`; the sibling `CYRUP_SUBAGENTS_HERDR_RUNTIME_DIR` /
>   `_RUN_ID` exist in `placement/native.rs`) and herdr itself is not a port target, so this is
>   recorded, not filed.

> **UNFIXED AT MERGE — A DEFECT IN THIS ITEM'S OWN 2026-09-05 CODE (tenth-edition ledger pass).**
> The batch-4 regression gate ran after the review round had closed, so this did not reach a fix
> commit. Recorded here rather than filed as a row, per the batch-4 rule.
>
> **The claim rollback in `claim_run_fanout_batch_with_commit_in`
> (`crates/cyrup-ext-subagents/src/exec/run_fanout_budget.rs:888-902`) is not panic-safe, and does not
> fire when `commit` fails.** Upstream (`run-fanout-budget.ts:238-262` @v0.64.0, read at the tag) wraps
> slot creation AND `return commit(getRunFanoutBudgetSnapshot(valid))` in ONE `try`, whose `catch` at
> `:257-262` unlinks every slot in `created` before rethrowing — a `commit` that throws ROLLS THE
> BATCH'S CLAIMS BACK. cyrup runs the body in a closure and rolls back only under
> `if outcome.is_err()`, which a panic unwinding out of the caller-supplied `commit` skips entirely;
> and because `commit` returns a plain `T`, a caller whose commit fails by returning an inner `Err`
> (`T = Result<_, E>`, the natural Rust idiom) never triggers it either. Claims are permanent by
> design — this module's own reset boundary is "cumulative claims are never released" — so a
> panicking or failing commit irrevocably burns N slots of the run's cap where upstream returns them.
>
> Two things make this worse than a plain miss. **(1)** The review fix `033e8c79` applied exactly the
> right mechanism — a drop guard, `impl Drop for ReleaseOnDrop` at `:1083` — to the admission LOCK,
> and did not apply it to the claims that lock protects, which is the same shape as upstream's nested
> `try`/`catch`. **(2)** That commit's own new test
> `a_panic_inside_the_commit_closure_still_releases_the_admission_lock` asserts
> `assert_eq!(snapshot.used, 2)` after the panicking batch, with the comment "the claim the panicking
> batch wrote stands" — upstream would be 1. A review-fix commit pinned the divergence as correct
> instead of declaring it. The function's rustdoc at `:853-854` over-promises in the same direction:
> "When `commit` is not reached the claims this call created are removed again, so a failed admission
> group leaves the ledger exactly as it found it."
>
> **Fix:** make the `created` rollback a drop guard disarmed on success, symmetric with the lock's, and
> correct the test's assertion, its comment and the rustdoc — or, if the leak is deliberate, declare it
> as an explicit `[CYRUP-DELTA]` beside this module's other four and say so in the test. Nothing here
> changes the item's disposition: the ledger mechanism and its doctor surface are ported and proven,
> the four remaining vars are unchanged, and the row stays **partial**.

> **PARTIALLY CLOSED 2026-09-04 — commit `91ca02e`, cyrup-ext-subagents.** Re-derived at
> **pi-subagents v0.64.0** (all twelve names still exist there; `git -C tmp/pi-subagents grep -n
> <name> v0.64.0 -- src`). Disposition per var, so an auditor can re-verify each without this pass:
>
> | var | v0.64.0 declaration | disposition |
> |---|---|---|
> | `TOOL_TIMEOUT_MS` | `runs/shared/tool-timeout.ts:1` (resolver `:61-88`, env lowest precedence `:73`) | **OPEN.** cyrup has no per-tool child timeout at all (`rg -i 'tool_timeout\|ToolTimeout' crates/cyrup-ext-subagents/src` → 0 — the whole `effectiveToolTimeoutMs` / `DEFAULT_FAST_TOOL_TIMEOUT_TOOLS` subsystem is absent), so the env var has no resolver to feed. Needs an area-09 item for the subsystem. |
> | `TASK_DELIVERY` | `runs/shared/pi-args.ts:83` (`resolveSubagentTaskDelivery` `:87-93`, `shouldDeliverTaskViaFile` `:95-102`, consumed `:822`) | **PORTED.** `spawn/mod.rs` `TASK_DELIVERY_ENV` (+ `_PI_ALIAS`), `TaskDelivery`, `TaskDeliveryHost`, `resolve_task_delivery`, `should_deliver_task_via_file`, `ChildSpawnSpec::resolve_task_arg_with`. Residual, low: upstream's SIGKILL startup-retry that flips delivery to `file` on a zero-activity kill (`foreground/execution.ts:2122-2123`, `background/subagent-runner.ts:2071-2072`) is not ported — the operator override is. |
> | `STEER_CAPABILITY`, `STEER_ACK_DIR`, `CAPABILITY_CEILING_V1` | — | closed before this pass (see the 2026-09-04 narrowing in the row). |
> | `RUN_FANOUT_BUDGET` | `runs/shared/run-fanout-budget.ts:12` (the 280-line file-claims ledger; encoded into the child env at `pi-args.ts:942-943`, inherited at `foreground/subagent-executor.ts:4849`, `:6473`) | **PORTED 2026-09-05 (`063cfe6f`), mechanism and decode side.** The whole ledger is `exec/run_fanout_budget.rs`: `RUN_FANOUT_BUDGET_ENV` (+ `_PI_ALIAS`), `create_run_fanout_budget_in`, `validate_run_fanout_budget_descriptor_in`, read/write/encode/`decode_run_fanout_budget_descriptor_in`, `run_fanout_budget_snapshot_in`, `claim_run_fanout_batch_in`/`_with_commit_in` under `with_admission_lock` — whose release is a DROP GUARD (batch-4 review fix), because upstream releases in a `finally` (`:201-204`) and a panic in the caller-supplied `commit` closure would otherwise leak `admission.lock` under a live pid that `admission_lock_is_stale` never reclaims. Residual, low: no production path CREATES or CLAIMS against one yet, and `spawn_plan.rs` does not yet write the var into the fan-out-authorized child overlay (`pi-args.ts:942-943`) — the enforcement wiring needs an area-09 item. |
> | `MAX_SPAWNS_PER_RUN` | `shared/types.ts:2807-2818` (`resolveMaxSubagentSpawnsPerRun`: env → config → `DEFAULT_MAX_SUBAGENT_SPAWNS_PER_RUN = 64`), `extension/doctor.ts:189-193` | **PORTED 2026-09-05 (`063cfe6f` + `5a4d456e`), with the ledger above.** `MAX_SPAWNS_PER_RUN_ENV` (+ `_PI_ALIAS`), `resolve_max_spawns_per_run`, `MaxSpawnsPerRunSource`, `SubagentExtensionConfig.max_subagent_spawns_per_run`, and the `"Run fan-out budget"` doctor block (`RunFanoutDoctor`, `registration/doctor.rs`) — which is this row's `Verify` clause about doctor. The block carries one `[CYRUP-DELTA]` line upstream does not (`- enforcement: NOT WIRED …`, batch-4 review fix), because the limit it reports is not applied by anything yet and the operator reading `/subagents-doctor` is owed that, not just the ledger reader. A fourth `[CYRUP-DELTA]` was added beside the module's three (batch-4 review): `normalize_max_spawns_per_run` is `u32::from_str` on the trimmed string, while upstream's `normalizeNonNegativeInteger` (`types.ts:2758-2762`) is `Number(value)` + `Number.isInteger`, so `"5.0"`/`"0x10"`/`"1e2"` are accepted upstream and rejected here (falling through to the config rung, never misread). `0` falls THROUGH here rather than meaning unlimited, unlike the per-session sibling; that is upstream's `> 0` filter and is documented at both resolvers. |
> | `TOOL_BUDGET_ZERO_AUTH` | `runs/shared/tool-budget.ts:5`, `:16-24` (`minimumHard: 0 \| 1`), `:74-80`; child `subagent-prompt-runtime.ts:693`; parent `pi-args.ts:1032` | **PORTED (child side, the gate).** `exec/tool_budget.rs` `HardMinimum`, `validate_tool_budget_config_with`, `decode_tool_budget_env(value, minimum)`; `prompt_runtime.rs` decodes with `HardMinimum::from_env(get)`. Residual, low: no parent-side WRITER — upstream sets it only from `delegatedAllowZeroToolBudget` on the owned-delegation path (`subagent-executor.ts:6937-6943`), a surface cyrup does not have; this is **SUBA-063**'s first half and that row (area 09, untouched here) should be closed by reference on it. **2026-09-23:** re-verified at v0.68.0 — the sole producer is `executeDelegated` (`subagent-executor.ts:7455-7475`), called only by the prompt-template delegation bridge (`extension/index.ts:747-755`); cyrup has no such surface (`git grep "prompt-template:subagent\|execute_delegated" -- crates` = 0), so the writer is owed with **SUBA-022**, not here. SUBA-063 is CLOSED on that basis. |
> | `ASYNC_EVENTS_MAX_BYTES` | `runs/background/subagent-runner.ts:307`, `maxAsyncEventsBytes` `:318-324` | **PORTED.** `background/runner_main.rs` `ASYNC_EVENTS_MAX_BYTES_ENV` (+ `_PI_ALIAS`), `resolve_async_events_cap_bytes`; `open_run_events` → `BoundedJsonlWriter::create_with_cap`. Residual, low (SUBA-063's third part): the `subagent.events.truncated` marker event with its 512-byte reserve (`:308-309`, `:351-367`) is still not written; the writer stays a silent no-op at the cap. **CLOSED at `2f4d0a5`:** `events.jsonl` is a `jsonl::RunEventLog` — lifecycle lines never capped, diagnostics capped at `cap - 512` with one `subagent.events.truncated` marker (`jsonl::tests::the_first_overflowing_diagnostic_line_writes_one_marker_and_stops`); 2026-09-23 moved the detached-workflow reconciler's journal onto it too (`extension/executor/workflow_detach/mod.rs`, `a_full_event_log_still_receives_the_workflows_terminal_completion_line`). |
> | `RUNTIME_ACKNOWLEDGED_EXTENSIONS` | `runs/shared/runtime-acknowledged-extensions.ts:6`; child write `subagent-prompt-runtime.ts:117-130`; parent read `foreground/execution.ts:1386`, `background/subagent-runner.ts:1859` | ~~**OPEN.**~~ **PORTED 2026-09-23 (SUBA-063).** `exec/runtime_acknowledged_extensions.rs` `RUNTIME_EXTENSION_ACK_PATH_ENV = "CYRUP_SUBAGENT_RUNTIME_ACKNOWLEDGED_EXTENSIONS"`: written for every child by `exec/spawn_plan.rs`, read by the child's prompt runtime (`prompt_runtime.rs` `with_runtime_acknowledgements`, collecting `subagent:acknowledge-extension` bus events and writing the `0600` capture at `agent_end`), read back and sanitized by the parent at the child's close (`exec/attempt_runner.rs`) onto `SingleResult`/`StepStatus`/`RunStatus`/`ResultFile`. [CYRUP-DELTA]: the variable was removed upstream at v0.65.0 when children went in-process; cyrup's are OS processes, so the file stays the channel. Tests in the SUBA-063 row. *Was:* The acknowledgement protocol (`subagent:acknowledge-extension` event → `0600` file → parent read-back into the result) does not exist in cyrup (`rg -i acknowledg crates/cyrup-ext-subagents/src` → 0 at `pub` level). **SUBA-063**'s second half; owned there. |
> | `LLM_INTENT_ARBITER` | `runs/shared/llm-intent-arbiter.ts:20`, `:229` (`=== "0"` → `undefined`) | **OPEN.** cyrup has the heuristic classifier (`exec/task_intent.rs::classify_task_mutation_intent`) but no model-backed `createTaskMutationArbiter` to switch off; the kill switch needs the arbiter first. Needs an area-09 item. |
> | `PI_CODING_AGENT_PACKAGE_ROOT` | `shared/utils.ts:12`; used to read the pi package's `package.json` `piConfig.configDir` (`:45-75`), to locate the pi entrypoint (`runs/shared/pi-spawn.ts:136`) and to import `typebox/compile` (`runs/shared/structured-output.ts:105-115`) | **INAPPLICABLE — recorded, not ported.** All three uses are Node module-resolution concerns; cyrup's child is the `cyrup` binary itself (`spawn/mod.rs` `CYRUP_SUBAGENT_BINARY`), its config dir is `cyrup-config::paths`, and its schema compiler is compiled in. Nothing to port. |
>
> **Design decisions (DESIGN-GUIDANCE):** `TaskDelivery`/`TaskDeliveryHost` and `HardMinimum` are
> two-variant domain enums mirroring upstream's literal unions (`"auto" | "file"`, `0 | 1`), chosen over
> bools/integers that would admit states upstream cannot express; every decision is a pure function
> over an injected `get: &dyn Fn(&str) -> Option<String>` with the process-env read confined to the
> shell (`resolve_task_arg`, `open_run_events`, the child runtime's existing `get`). Rejected:
> reading `std::env` inside the rules (untestable), a `u32` floor (admits `7`), and a new
> `resolve_task_arg` signature (it keeps its two arguments; `_with` is the explicit form).
>
> **Note on the crate's rustdoc gate, pre-existing and out of this item's scope:** `RUSTDOCFLAGS='-D
> warnings' cargo doc -p cyrup-ext-subagents --no-deps` failed at this commit on
> `discovery/runtime_registry.rs:18` (then `crate::extension::executor::SubagentExecutor` — `mod executor`
> is private at `extension/mod.rs:64`), unchanged by this commit; the links this commit adds resolve.
> **Superseded the same day:** `SUBA-088` (`ba24e5e5`) repointed that link to
> `crate::extension::SubagentExecutor` and the gate is clean at HEAD (review 2026-09-04, `6cf2cb9f`).
...
| `PI_SUBAGENT_TOOL_BUDGET_ZERO_AUTH` | `src/runs/shared/tool-budget.ts:5` | the escape hatch authorizing a zero tool budget; the `budget == 0` case has no gate — ported 2026-09-04; see the row's security note on the unauthorised-zero decode path |
...
> **PARTIALLY CLOSED 2026-09-16, re-read at `cc7818b`. The PROMOTE branch of the `Fix` was taken and
> is two thirds landed; only the `--help` clause is outstanding.** Full disposition in the Open-items
> row. Two corrections this body needs, because both of its own citation blocks are now wrong:
>
> **(1) All four `cyrup` citations below are DEAD, and none was a line drift — the functions moved
> crates.** `extension.rs:5168` (`dirs_home`) names a function that no longer exists anywhere in the
> tree, and `crates/cyrup-ext-subagents/src/discovery/skills.rs:106` says so in place; `agent_dir_from`
> is now `native_supervisor::intercom_agent_dir_from`; and `background/mod.rs`'s `temp_root_dir_from`
> split into `background/artifact_roots.rs:260` and `spawn/nested_events.rs:101`. The four hand-rolled
> ladders the row counted were **collapsed into one** at `da970e8`: `cyrup_config::paths::ENV_HOME`
> (`crates/cyrup-config/src/paths.rs:322`) with `cyrup_home_dir_from` (`:373`),
> `cyrup_home_override_from` (`:388`) and `cyrup_dir_override_from` (`:402`). **The consolidation is
> not itself the closure** — it moved the mechanism without deciding the question — but it is what made
> the precedence answerable at all.
>
> **(2) The `Impact` sentence *"its precedence against `CYRUP_AGENT_DIR` is undefined anywhere"* is
> now FALSE and must not be re-derived.** It is defined twice over: in prose at
> `docs/guide/reference/environment.md:56-75`, and in code at `paths.rs:337` (`ENV_AGENT_DIR_KEYS`,
> short spelling first) with `:395-401` stating that `home` is a parameter *precisely* so the two
> overrides cannot resolve against different homes — the concrete hazard this row raised. Three tests
> pin it (`paths.rs:451`, `:485`, `:521`).
>
> **What is genuinely left is one clause: `--help`.** `crates/cyrup/src/cli/help.rs:22-23` names
> `CYRUP_AGENT_DIR` and `CYRUP_SESSION_DIR` only; `git grep -n CYRUP_HOME -- crates/cyrup/src` is 0.
> The confine branch was not taken either — no reader site is `#[cfg(test)]`-gated, so the var remains
> live in shipped builds while `paths.rs:320-322` still describes it as a test sandbox lever. The
> `Verify` below is therefore **half executed**: the precedence assertions exist, the `--help`
> assertion does not.
...
> **RE-WALKED 2026-09-24 (second pass) at pi-subagents v0.71.0, pi-intercom v0.14.0 and
> pi-permission-system v0.8.0 (unchanged).** Three of the nine rows below are **REFUTED** — the
> upstream name exists and cyrup's var is a documented `PI_`→`CYRUP_` rename, not an invention:
> - `CYRUP_INTERCOM_TRANSPORT` / `CYRUP_INTERCOM_TCP` — pi-intercom `broker/paths.ts:52`
>   (`env.PI_INTERCOM_TRANSPORT`, `"tcp"`) and `:57` (`env.PI_INTERCOM_TCP`, the legacy `1`/`true`
>   opt-in) @v0.14.0, present at every tag v0.9.0…v0.14.0, i.e. at the ported v0.9.2 too. cyrup's
>   `crates/cyrup-intercom/src/transport/target.rs:31-50` ports both with pi's semantics and says so.
>   The "full set" this row quotes for pi-intercom was simply incomplete.
> - `CYRUP_SUBAGENTS_TEMP_ROOT` — `PI_SUBAGENTS_TEMP_ROOT` is read at `src/shared/types.ts:2688`
>   @v0.64.0 and `:2808` @v0.71.0 (`process.env.PI_SUBAGENTS_TEMP_ROOT?.trim()`).
>
> **Six remain invented at the latest tags:** the three permission-system vars, `CYRUP_INTERCOM_BROKER_BINARY`,
> `CYRUP_SUBAGENT_AGENT_NAME` and `CYRUP_HOOK_WARMUP` (none in the v0.71.0 48-name set or the v0.14.0
> set). pi-intercom v0.14.0's set gained `HERDR_PANE_ID` over v0.13.0 (`index.ts:527`) — herdr
> integration, served by cyrup's own herdr client; not an invention question and not filed.
...
> assertions are all pinned by tests. The original filing is kept unedited beneath this block.
>
> **Upstream, re-read at v0.84.4 (ADR-0006's parity target).** The two keys and their accessors are
> as the filing recorded them (`core/settings-manager.ts:38`, `:143`, `:145`, `:1212-1214`,
> `:1216-1220`, `:1233-1235`, `:1237-1241`). What the filing did not carry, and what decided the
> shape of the port, is where they are CONSUMED:
> `modes/interactive/interactive-mode.ts:378` passes `copyOnSelect` into every
> `createInteractiveTui` (fed at `:586`, and again for the incoming renderer of a live mode switch
> at `:871`); `:1995` re-pushes it from `applyRuntimeSettings`; `:4750-4752` / `:4757-4760` are the
> two `/settings` handlers, of which only the copy-on-select one touches the live renderer;
> `:6556` evaluates `getFullscreenExitOutput()` as `stop()`'s DEFAULT ARGUMENT, i.e. per call, and
> `:836-843` is what it selects — `if (renderer.mode === "fullscreen" && output === "transcript")`
> switch back to regular and `renderNow()`, then `ui.stop({preserveScreen: renderer.mode ===
> "fullscreen"})`. `components/settings-selector.ts:680-686` / `:694-700` are the rows, `:916-923`
> their dispatch. On the renderer, `packages/tui/src/tui-alt-screen.ts:205` is the field, `:229` its
> `?? true` seed, `:242-248` the getter/setter pair, and `:1035` its ONE read — `if
> (this.copyOnSelect) void this.copySelectionToClipboard();`, with the `requestRender()` under it
> unconditional. Tag-to-tag: none of this exists at v0.84.1, so there is no earlier behaviour to
> reconcile — the whole surface is the v0.84.1→v0.84.4 delta.
>
> **What changed.** *Config:* `FullscreenExitOutput` in `settings/types.rs`;
> `EffectiveSettings::fullscreen_exit_output` / `::fullscreen_copy_on_select` in
> `settings/effective.rs`, degrading rather than rejecting exactly as `tui_mode` /
> `fullscreen_scrollbar` do; `SettingsManager::set_fullscreen_exit_output` /
> `::set_fullscreen_copy_on_select` on the GLOBAL scope. *TUI:* `AltScreen::copy_on_select` +
> `set_copy_on_select`, with `selection::release` gated where `:1035` gates upstream — the
> selection stays anchored, focused and highlighted and ONLY `PointerOutcome::Copy` is withheld,
> which is what keeps `/copy` able to reach it; `App::{set_fullscreen_exit_output,
> set_fullscreen_copy_on_select, preserve_screen_on_exit}`; `adopt_fullscreen_renderer` seeding
> each screen it builds (upstream's `:378`); the two `AppCommand::ApplySetting` arms (upstream's
> `:4750-4760`); pi's two `/settings` rows. *Composition root:* both settings mapped onto the
> tui-side types in `crates/cyrup/src/interactive.rs`, UNCONDITIONALLY rather than inside the
> boot-fullscreen branch, because a session that switches into fullscreen mid-run must not run on
> renderer defaults.
>
> **The exit-output port is not a line-for-line copy, and the reason is structural.** pi gets
> `"transcript"` by swapping back to the regular renderer and letting it re-render the shared chat
> container; cyrup's committed entries have already left the app for the terminal's own scrollback
> (R-ARCH-TUI-003), so its inline renderer has nothing to re-render and the transcript reaches the
> main screen through `altscreen::exit::repaint` instead. The setting therefore selects that
> repaint: `transcript` → `preserve_screen = false` (today's behaviour), `resume-hint` →
> `preserve_screen = true`, leaving the restored screen plus the line `resume_hint.rs` already
> prints. The MODE-SWITCH teardown keeps its `false` unconditionally — it is not an exit, and the
> inline renderer resuming over a blank screen is not one of pi's two outcomes.
>
> **Two stale in-code notes corrected in the same commit,** both of which this row's evidence pass
> exposed: `altscreen/mod.rs` said the exit repaint was unreachable (`preserve_screen` "fixed at
> construction", "the only construction site passes `true`", filed as
> `ALT_SCREEN_EXIT_REPAINT_UNREACHABLE`) — at HEAD both teardowns passed `false` and the repaint was
> live, so the note was simply wrong; and `app/settings_rows.rs` carried the v0.84.1 scope boundary
> this row exists to retire.
>
> **Tests**, each RED before the change under mutation, not only because the API was absent:
> inverting the degrade default fails `fullscreen_exit_output_and_copy_on_select_degrade_to_pis_defaults`;
> stubbing `preserve_screen_on_exit()` to `false` fails
> `the_exit_output_setting_decides_whether_the_excursion_reaches_scrollback`; disabling either
> `ApplySetting` arm or the adopt-time seed fails all three cases in the new
> `crates/cyrup-tui/src/tests/fullscreen_settings.rs`; making `set_copy_on_select` a no-op fails
> `copy_on_select_disabled_keeps_the_selection_and_withholds_the_copy`, which is pi's own
> `packages/tui/test/tui-alt-screen.test.ts:1003-1007` case. Suites: 228/228 `-p cyrup-config`,
> 1394/1394 `-p cyrup-tui`, 236/236 `-p cyrup`; fmt, clippy `-D warnings --all-targets` and
> `RUSTDOCFLAGS=-D warnings cargo doc --no-deps` clean on all three.
>
> **Residual 1 (low).** `app/run.rs`'s exit tail still cannot be entered by a unit test — it is
> inside `App::run`'s `select!` loop — so the DECISION is pinned but not that `run.rs` consults it;
> a literal `false` there would compile. This is the same gap `fullscreen_scrollback.rs` has
> always recorded for that call site, narrowed from "the argument is a literal" to "the argument is
> the right method".
>
> **Residual 2 (low), new and NOT part of this row's scope.** v0.84.4 also re-pointed
> `app.message.copy`: Ctrl+X is now `handleCopyCommand({flashConfirmation: true, preferSelection:
> true})` (`interactive-mode.ts:2910`) and prefers the active fullscreen selection **only when
> copy-on-select is off** (`:6117-6127`), while the `/copy` command still passes no
> `preferSelection` (`:3022`) and always copies the last assistant message. cyrup routes BOTH to one
> `AppCommand::Copy` (`app/input.rs:505`) whose arm prefers a live selection unconditionally — the
> ADR-0005 §B-11 decision, taken against v0.84.1 where `handleCopyCommand` had no selection leg at
> all. Closing it needs the two entry points split, which is a `cyrup-tui` command-surface change,
> not a settings one.

**Kind** upstream-drift · **Severity** ~~low~~ **closed** · **Effort** S · **Confidence** confirmed (both sides re-read 2026-09-05 at pi v0.84.4)

**upstream** — neither key exists at v0.84.1, this file's recorded upstream-drift ceiling. Both are new in the `v0.84.1..v0.84.4` window: `pi/packages/coding-agent/src/core/settings-manager.ts` @v0.84.4 declares `export type FullscreenExitOutput = "transcript" | "resume-hint";` (`:38`) and the two `TerminalSettings`-adjacent fields `fullscreenExitOutput?: FullscreenExitOutput` (`:143`, default `"transcript"`) and `fullscreenCopyOnSelect?: boolean` (`:145`, default `true`), with degrading getters `getFullscreenExitOutput()` (`:1212-1214`, degrades anything but `"resume-hint"` to `"transcript"`, the same non-throwing contract `tuiMode`/`fullscreenScrollbar` already use) and `getFullscreenCopyOnSelect()` (`:1234`, `?? true`) plus matching setters (`:1216-1218`, `:1238-1239`). Both keys are declared "no effect in regular TUI mode" in-source, i.e. they are alt-screen-only, the same gate `fullscreenScrollbar` (CFG-021, closed) carries.

**cyrup** — `grep -rn 'fullscreenExitOutput\|fullscreenCopyOnSelect\|fullscreen_exit_output\|fullscreen_copy_on_select' crates/cyrup-config/src crates/cyrup-tui/src` resolves only to a documentation line: `crates/cyrup-tui/src/app/settings_rows.rs:290` already carries an in-code note (written while landing the ADR-0005 alt-screen renderer) stating verbatim that `fullscreenExitOutput` does not exist at v0.84.1, the tag that work was deliberately measured against — so the absence is a stated scope boundary, not an oversight, and this row is what turns that boundary into a version-lag item now that upstream latest has moved past it.

**Impact** — low. `fullscreenCopyOnSelect` governs whether the alt-screen renderer's text-selection feature (ADR-0005 §B-8, already ported per CFG-021's closure) auto-copies on select or waits for an explicit copy action; `fullscreenExitOutput` governs whether leaving fullscreen mode dumps the retained transcript or a short resume hint. Both are alt-screen-only and off by default reach (fullscreen is not the default `tuiMode`), so the blast radius is the same opt-in population CFG-021 already described.

**Fix** — add `fullscreen_exit_output` / `fullscreen_copy_on_select` to `EffectiveSettings` alongside `tui_mode`/`fullscreen_scrollbar` (`crates/cyrup-config/src/settings/effective.rs`), with the same degrade-don't-reject contract; wire the two into `crates/cyrup-tui/src/altscreen/` (exit-output at the alt-screen teardown path already producing a transcript dump per ADR-0005 §B-1 retention; copy-on-select into `altscreen/selection.rs`, which already has the copy mechanism CFG-021's landing built).
...
> pinned by tests. The original filing is kept unedited beneath this block.
>
> **Upstream, re-read at v0.84.4 (ADR-0006's parity target).** The key and its getter are as the
> filing recorded them (`core/settings-manager.ts:128`, `:1273-1276`). What the filing did not
> carry, and what decided the shape of the port, is the CONSUMER — one expression at
> `core/sdk.ts:256-263`:
>
> ```ts
...
> ```
>
> Note what `defaultTools` is NOT: it never reaches `allowedToolNames`, so it is a SELECTION, not
> an allowlist. `initialActiveToolNames` goes to `AgentSession` (`sdk.ts:397`) and is applied at
> construction with `includeAllExtensionTools: true` (`agent-session.ts:406-410`), which
> `_refreshToolRegistry` honours at `:2742-2745` by pushing every extension/SDK tool into the
> active set regardless of the configured list. `docs/settings.md:226-244` states both edge rules
> in prose: an empty array "starts with no built-in tools while preserving extension and SDK custom
> tools", and "a project `defaultTools` array replaces the global array".
>
> **Tag-to-tag difference, and why it matters here.** The key landed at `4d9aa837c` ("add
> configurable default tools") with `allowedToolNames = options.tools ?? (options.noTools === "all"
> ? [] : options.noTools === undefined ? configuredDefaultToolNames : undefined)` — i.e. as a real
> allowlist, which DROPPED extension and SDK custom tools. `541045ae0` ("preserve extension tools
> with defaults") deleted that leg, narrowing the setting to the initial built-in selection, before
> v0.84.4 shipped. Porting the earlier semantics would have been a defensible reading of the
> CHANGELOG line alone; ADR-0006's latest-tag rule is what makes the selection reading correct.
>
> **Change.** `EffectiveSettings::default_tools() -> Option<Vec<String>>`
> (`crates/cyrup-config/src/settings/effective.rs`), a plain passthrough with no validation, shaped
> like its sibling `enabled_models`: `None` for unset, `Some(vec![])` for an explicit empty list.
> Project-over-global replace needs no code — `deep_merge`'s array rule
> (`settings/merge.rs:20-21`, "arrays + primitives: `over` wins") already is upstream's rule.
> `select_active_tools` (`crates/cyrup-session-svc/src/builder.rs`) gains a
> `default_tools: Option<&[String]>` parameter consulted on the one `(None, None)` arm — neither
> `--tools` nor `--no-tools`/`--no-builtin-tools` — where it REPLACES `DEFAULT_BUILTIN_TOOLS`. The
> arm's `|| !ALL_BUILTIN_TOOLS.contains(name)` leg is deliberately untouched: that leg is cyrup's
> spelling of `includeAllExtensionTools`, and it is what makes an empty configured list keep the
> extension tools alive. The one call site reads `settings.effective().default_tools()`.
>
> **Design decisions** (three; the commit body carries the same list).
> 1. **The `Option` survives end to end** instead of being defaulted to a `Vec` of the four names at
>    the config layer. The invariant is that "unset" and "explicitly empty" cannot be confused —
>    collapsing them makes `{"defaultTools": []}` mean read/bash/edit/write, the exact inverse of
>    what it asks for. `CFG-056` records the same decision for `defaultThinkingLevel`, and
>    `enabled_models` already carries it. What is still a runtime question: whether the CONSUMER
>    reads the distinction correctly — pinned by the empty-list case in the builder test.
> 2. **Threaded as an argument, not a `SessionConfig` field.** `SessionConfig` is the declarative
>    CLI/caller input; `defaultTools` is a settings read upstream also performs inside
>    `createAgentSession` rather than accepting through `options`. A field there would give a caller
>    a silent override of the setting and a second place the value can disagree with the file.
>    Migration cost: one signature, one call site.
> 3. **No new type.** A `ToolSelection` enum over `{Default, Configured(Vec<String>)}` is
>    isomorphic to the `Option` and makes nothing new impossible, so it is ceremony.
>
> **Tests**, each RED before the change by MUTATION, not only because the API was absent:
> `cyrup-config settings::tests::getters::default_tools_round_trips_and_distinguishes_unset_from_empty`
> covers unset / explicit `[]` / populated, an unvalidated unknown name, and the
> project-replaces-global merge — with the getter mutated to answer `Some(vec![])` for an absent key
> it fails on the unset assert. `cyrup-session-svc
> builder::tests::the_default_tools_setting_replaces_pis_four_built_ins` and
> `::explicit_tool_flags_still_outrank_the_default_tools_setting` are ported case for case from
> upstream's own `test/default-tools-setting.test.ts` at v0.84.4 (the configured list as the initial
> selection, `powershell` instead of `bash`, extension tools kept, and `tools`/`excludeTools`/
> `noTools` precedence); with the arm restored to ignore the parameter both fail. Suites: 229/229
> `-p cyrup-config`, 336/336 `-p cyrup-session-svc`; `cargo fmt --all -- --check`, clippy
> `--all-targets -D warnings` and `RUSTDOCFLAGS=-D warnings cargo doc --no-deps` clean on both.
>
> **Residual 1 (low).** Verified at the seam, not through a live process: nothing launched a
> session with a `defaultTools` in `settings.json` and observed the tool array in a provider
> request. The two tests pin `select_active_tools`'s output and the getter's value; the wire
> between them is the one call site and the compiler.
>
> **Residual 2 (low) — a separate defect found while reading this arm, deliberately NOT fixed
> here.** `--no-builtin-tools` (`NoTools::Builtin`) is `!DEFAULT_BUILTIN_TOOLS.contains(name)`, so
...
**upstream** — absent at v0.84.1. New at v0.84.4: `pi/packages/coding-agent/src/core/settings-manager.ts:128` declares `defaultTools?: string[]; // Initial built-in tool selection` on `Settings`, with getter `getDefaultTools()` (`:1273-1276`, `const tools = this.settings.defaultTools; return tools ? [...tools] : undefined;`) — a defensive copy, no validation.

**cyrup** — `grep -rn 'defaultTools\|default_tools' crates/cyrup-config/src crates/cyrup-tui/src` → 0.

**Impact** — low. Lets a user pre-select which built-in tools a session starts with (distinct from `enabledModels`, the sibling model-cycling list already ported); its absence means cyrup has no equivalent startup-time tool filter sourced from settings.

**Fix** — add `default_tools` to `EffectiveSettings` (`crates/cyrup-config/src/settings/effective.rs`) returning `Option<Vec<String>>`, ported as a plain passthrough (no validation, matching pi); consume it wherever the initial built-in tool set is assembled at session start (likely `cyrup-session-svc`'s builder, outside this crate pair).
...
> invents a `hard: 1` budget the parent never wrote. Re-derived at the latest tags before the
> change: the `throw` in `decodeToolBudgetEnv` is byte-identical at pi-subagents **v0.34.0** (this
> crate's ported baseline) and **v0.64.0**; only `options.allowZero` was added between them
> (v0.36.0, ported by `CFG-067`). What upstream's throw actually costs the child was read this pass
> and is narrower than this section's original wording: pi core **v0.84.4**
> `packages/coding-agent/src/core/extensions/loader.ts:545-587` CATCHES it — `load.discard()`
> (`:559`) drops every registration `registerSubagentPromptRuntime` had already made and the loader
> records `Failed to load extension: …` (`:587`) — so pi's child process still starts, but with the
> whole subagent prompt runtime (budget, steering inbox, permission gate, structured output,
> watchdog) absent. **The row's "the child never runs" was an over-statement; "the runtime never
> registers" is exact.** Either way upstream never runs a budgeted child with its budget silently
> removed, which is the divergence this row named.
>
> **Change.** `prompt_runtime_from_env` / `prompt_runtime_extension_from` /
> `prompt_runtime_extension_for_env` return `Result<Option<…>, String>`; the decode result is now
> `?`-propagated instead of matched-and-warned, and `attach_native_extensions`
> (`crates/cyrup/src/session_launch.rs`) maps it into the `anyhow` chain that already carries the
> intercom extension's build failure, so the child exits before its first turn. The `[CYRUP-DELTA]`
> the row asked for sits at the decode site: cyrup has no per-extension quarantine at its
> native-extension attach point, so where pi loses one extension cyrup ends the launch — stricter,
> in the safe direction.
>
> **Tests.** `prompt_runtime::tool_budget_runtime_tests::an_unauthorised_zero_budget_refuses_the_runtime_instead_of_running_unbudgeted`
> — the unauthorised `{"hard":0}`, a non-JSON payload and a `soft > hard` payload each refuse with
> pi's own message, and the authorised `{"hard":0}` still builds an armed runtime. RED before /
> green after: with the old warn-and-drop arm restored under the new signature the test fails on
> the first case (`an undecodable tool budget must refuse the runtime: "{\"hard\":0}"`) while the
> other two tool-budget tests still pass. `cargo nextest run -p cyrup-ext-subagents` 2794/2794,
> `-p cyrup` 236/236, `-p cyrup-it --features it` 493/493.
>
> **Residual (low).** The launch-path leg is verified by construction (the `?`, the compiler and
> the crate suites); no live child was spawned with a tampered `CYRUP_SUBAGENT_TOOL_BUDGET` to
> watch the process exit.

**Kind** port-divergence · **Severity** low · **Effort** S · **Confidence** confirmed (both sides read 2026-09-04; filed by the batch-2 ledger audit from the `CFG-067` review's non-blocking finding, so the census sees it)

**cyrup** — `crates/cyrup-ext-subagents/src/prompt_runtime.rs:2222-2235`: `decode_tool_budget_env(get(TOOL_BUDGET_ENV), HardMinimum::from_env(get))` is matched `Ok(budget) => budget, Err(message) => { tracing::warn!("{message} — running this child with no tool budget"); None }`. The comment above it declares the choice ("running unbudgeted beats an unexplained child crash"; the parent is the var's only writer). With `CFG-067`'s `CYRUP_SUBAGENT_TOOL_BUDGET_ZERO_AUTH` ported, a `{"hard":0}` payload WITHOUT the `=1` authorisation is exactly such a decode error — so the one payload the authorisation exists to gate is the one that disables the budget entirely. `a_zero_budget_is_honoured_only_with_the_parents_authorisation` (`prompt_runtime.rs:2510`) pins the divergent arm behind `if let Some(ext)`.

**upstream** — `pi-subagents` v0.64.0 `src/runs/shared/subagent-prompt-runtime.ts:693`: `registerToolBudget(pi, decodeToolBudgetEnv(process.env[TOOL_BUDGET_ENV], { allowZero: process.env[TOOL_BUDGET_ZERO_AUTH_ENV] === "1" }))` runs at module init; `src/runs/shared/tool-budget.ts:74-80` `decodeToolBudgetEnv` → `validateToolBudgetConfig(parsed, TOOL_BUDGET_ENV, options.allowZero ? { minimumHard: 0 } : undefined)` and `if (normalized.error) throw new Error(normalized.error);` — the child extension fails to load, so the child never runs.

**Impact** — Only reachable if the parent writes a payload it did not validate, or the environment is altered between spawn and child start (the parent validates before writing — hence low, not medium). When reached, the failure mode is inverted relative to pi: pi fails closed (no child), cyrup fails open (child with unlimited tool calls) and the only trace is a `warn`.

**Fix** — either (a) fail closed like pi: return an error from the runtime registration so the child exits before its first turn, carrying `message`; or (b) keep the drop but for the `hard: 0`-unauthorised case specifically, fall back to `HardMinimum::One`'s validated budget rather than `None`. (a) is the faithful port; record whichever is chosen with a `[CYRUP-DELTA]` at the match arm.
...
6. **One closure verified structurally, not end to end; the other's residual since closed.** CFG-022's shared predicate is verified identical at both call sites, but it was not confirmed that a models.json-only provider actually STREAMS (the same caveat the previous pass recorded). **CFG-006's residual — `max_retry_delay_ms` reaching the retry loop — was re-filed as `CFG-053` and closed 2026-09-04**: `crates/cyrup-provider/src/utils/provider_retry.rs::a_server_delay_over_the_ceiling_fails_with_pis_exact_sentence` drives `retry_delay_ms`, the exact function `crates/cyrup-provider/src/stream/sse.rs:467`'s retry loop calls, with the settings-sourced ceiling and asserts the ceiling behaviour.
