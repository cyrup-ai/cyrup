...
## CFG-067 — Twelve `pi-subagents` env vars have no `CYRUP_` counterpart — **PARTIALLY CLOSED 2026-09-04**
...
> | `RUN_FANOUT_BUDGET` | `runs/shared/run-fanout-budget.ts:12` (the 280-line file-claims ledger; encoded into the child env at `pi-args.ts:942-943`, inherited at `foreground/subagent-executor.ts:4849`, `:6473`) | **OPEN.** No run-fan-out claims ledger exists in cyrup (`rg -i 'fanout_budget\|FanoutBudget\|spawns_per_run'` → 0); the var carries a base64url descriptor of a directory that ledger owns. Porting the var without the ledger is impossible. |
> | `MAX_SPAWNS_PER_RUN` | `shared/types.ts:2807-2818` (`resolveMaxSubagentSpawnsPerRun`: env → config → `DEFAULT_MAX_SUBAGENT_SPAWNS_PER_RUN = 64`), `extension/doctor.ts:189-193` | **OPEN, with the ledger above.** The resolver is pure and trivially portable, but its only consumers are `createRunFanoutBudget(runId, …)` and doctor's "configured limit" line; a resolved number nothing enforces would be a misleading doctor surface, so it is deliberately not ported ahead of the ledger. Also blocks this row's `Verify` clause about doctor. |
