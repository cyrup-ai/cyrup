...
| CFG-080 | low | port-divergence | S | An UNAUTHORISED `{"hard":0}` in `CYRUP_SUBAGENT_TOOL_BUDGET` runs the child with NO tool budget, where pi's child refuses to start — **filed 2026-09-04 (batch-2 ledger audit, from the `CFG-067` review)**. Both sides read: cyrup `crates/cyrup-ext-subagents/src/prompt_runtime.rs:2222-2235` drops a `decode_tool_budget_env` error with a `tracing::warn!` and `None`; pi `src/runs/shared/subagent-prompt-runtime.ts:693` @v0.64.0 calls `decodeToolBudgetEnv(...)` at module init and `tool-budget.ts:74-80` `throw`s on a validation error, so the child never registers. Declared in the code comment as a deliberate divergence (the parent is the var's only writer); recorded as a row because it is the security-flavoured half of that divergence now that `CYRUP_SUBAGENT_TOOL_BUDGET_ZERO_AUTH` exists. |
...
## CFG-080 — An unauthorised `{"hard":0}` tool budget runs the child UNBUDGETED where pi's child refuses to start
