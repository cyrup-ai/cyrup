...
| CFG-045 | medium | not-ported | S | `doubleEscapeAction` is inert — the Escape handler has no double-escape and no bash-mode-exit branch — **2026-08-14, still open**: sweeps 2 and 6 — unchanged at HEAD. **FIX SITE: the `Action::Interrupt` arm in `crates/cyrup-tui/src/app.rs` plus a `last_escape_time` field** (pi `interactive-mode.ts:2570-2596` @v0.83.0). Outside cyrup-config/cyrup-resources. |
