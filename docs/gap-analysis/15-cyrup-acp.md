...
> ### RE-MEASURED 2026-09-24 — pins moved; ten §6 rows closed on both-sides reads
>
> **Current pins:** cyrup **`ea23ca2`**; `pi-acp` **v0.0.33** — still the newest tag (re-checked with
> `git tag --sort=-v:refname`), so **there is no new upstream window to census and no
> `upstream-drift` row is filed.** The clone's HEAD is now `v0.0.33-8-g42926cc`: past the tag,
> `src/acp/agent.ts` +12/−1, `src/acp/session.ts` +68/−24, `src/pi-rpc/process.ts` +75/−18 (commits
> `b21894c` "report Pi context window usage", `64f680c` "scope stats timeout to context usage",
> `e96c7e8` Windows session-path launch). **Untagged, so not filed** per the tag-only rule; recorded
> here as a lead for when v0.0.34 is cut — note cyrup already emits `UsageUpdate` at settle
> (`turn.rs`, `TurnActor::on_event`'s `Settled` arm), so the context-usage change may already be
> matched.
>
> **cyrup side:** the `824a539e..b28d3ff` gap the 2026-09-14 correction named is closed for the rows
> struck in §6, and `crates/cyrup-acp` is byte-identical from `9aeba769` (and from `b28d3ff`) to
> `ea23ca2` — none of the 46 commits in the window touched it. The census section below is still a
> worklist of UNVERIFIED leads for the 93 rows this pass did not re-read; its line citations into
> `crates/cyrup-acp` remain valid because the crate has not moved.
>
> **RE-MEASURE — 2026-09-24, pass 2 (the unread window closed).** Read in full: all 17 `src/` files
> of `pi-acp` at `v0.0.33` (`git show v0.0.33:<path>`), all production code in
> `crates/cyrup-acp/src/*.rs` at `ea23ca2`, the ACP arms of `crates/cyrup/src/{main,run,signals,
> acp_host,acp_terminal_login_cmd}.rs` and `cli/config_map.rs`, `ext_mode` in
> `crates/cyrup-session-svc/src/builder.rs`, and — for `ACP-014`/`ACP-281` — the relevant parts of
> `agent-client-protocol-schema-1.7.0` and `agent-client-protocol-2.1.0` in the cargo registry.
> Every one of the 93 open §6 rows was re-read on both sides: **90 closed, 3 left open with a
> 2026-09-24 stamp, 2 items filed (`ACP-297`, `ACP-298`)** — see §6's pass-2 paragraph. The census
> section's leads are resolved one by one in its own head note. Test names cited in closures were
> checked to exist (`grep 'fn <name>'`); no test was run (`cargo` not invoked by this pass).
>
> **Remaining unread window: none on either side at this pin.** Stated limits, not gaps in the
> window: `@agentclientprotocol/sdk` 0.26 is not vendored in `tmp/pi-acp`, so what upstream's SDK
> answers for an unimplemented optional method (relevant to `ACP-014`) was not read; upstream's
> `test/` tree was read only where a row cites a fixture by name.
>
> **Post-tag leads (pi-acp v0.0.33..42926cc) — NOT items; cited by commit, since README cites
> upstream only at tags.** `git -C tmp/pi-acp diff v0.0.33..42926cc -- src/` was read in full
> (`agent.ts` +12/−1, `session.ts` +68/−24, `process.ts` +75/−18):
> 1. **`b21894c` / `64f680c` — context-window `usage_update`.** Upstream now emits
>    `{sessionUpdate:'usage_update', used, size}` from `get_session_stats().contextUsage`
>    (`toUsageUpdate`, null when `tokens` is null or not a safe integer, or `size <= 0`) at four
>    points: before resolving every `session/prompt` (`settleTurn`), after the `session/new`
>    response and after `session/load` (both before the `available_commands_update`), after
>    `unstable_setSessionModel`, and after a `model` `set_config_option`. cyrup matches the **first**
>    point: `TurnActor::on_event`'s `Settled` arm emits `RuntimeAgent::usage` →
>    `usage_update(stats)` (`crates/cyrup-acp/src/turn.rs`, `None` when `context_usage` or `tokens`
>    is `None`), forwarded by `AcpTurnAgent::usage` since `cb290d1`. cyrup does **not** emit it after
>    `session/new`, after `session/load`, or on a model change (`config_pump`'s `ModelChanged` arm
>    sends only `config_option_update`), so the client's meter is empty until the first settle and
>    shows the old window size after a model switch until the next settle. cyrup additionally sends
>    `cost` (USD), which upstream does not. Would become an `upstream-drift` item when v0.0.34 is cut.
> 2. **`64f680c` — stats timeout (1 000 ms) and response routing** (`PiRpcProcess.request`'s
>    `timeoutMs`, responses for unknown ids no longer broadcast as events): subprocess-RPC plumbing
>    with no in-process counterpart. Not applicable.
...
> Kept verbatim as history; read the dispositions first.
>
> - **"No status axis" (structural finding)** — *superseded*: §6 now carries status inline
>   (`~~sev~~ **CLOSED <date>**` rows with evidence, open rows re-stamped), which is what the lead
>   asked a STATUS file for. The split into `15-cyrup-acp-STATUS.md` is a layout proposal, not a
>   defect, and is not filed.
> - **Census result (145 of 150 built)** — *confirmed and sharpened* by the row-by-row re-read: of
>   the 93 rows still open before pass 2, 90 close on read code; open are `ACP-014` (a gap the
>   census missed: three ungated v1 methods unregistered), `ACP-294`, `ACP-296`.
> - **`ACP-059` / `ACP-060` / `ACP-220`** — *promoted to closures* under `ACP-Q7`, each with a
>   falsification condition. **`ACP-055`** — closed as duplicate. **`ACP-294`** — left open
>   (partial). **`ACP-125`** — closed as superseded; the census's "no written trace on either side"
>   is **refuted**: `crates/cyrup-acp/src/startup.rs`'s module-level `ACP-068` CYRUP-DELTA names
>   upstream's `startupInfoSent` flag and why it has no counterpart.
> - **README's four "not ported" claims** — *not applicable to this file*; README.md is not edited
>   by this pass and its area-15 rows already read "ported as `crates/cyrup-acp`".
> - **§5 (38 of 46 questions decided in code)** — *confirmed*; applied as a status note at the head
>   of §5 with the undecided remainder listed.
> - **§8 (every phase built)** — *confirmed*; applied as a note at the head of §8.
> - **Five `NEW` units with no ids; 168 vs 150** — *struck, no schedulable work*: the five are built
>   and tested (`cb290d1`), cited in code as `(NEW, area 4d/4e)`; 150 live + 18 refuted = 168 is
>   arithmetic, not a contradiction.
> - **`ACP-018` ↔ `ACP-066` tension** — *resolved*, see `ACP-018`'s closure (no themes are
>   discovered under ACP, so the prelude's `Themes` block renders nothing — upstream's net output).
> - **`ACP-281` (schema derive attributes unread)** — *resolved*, see its closure: the schema was
>   read and the reject-the-turn divergence holds at 1.7.0.
> - **`ACP-015` ("auth-required.ts not read line by line")** — *resolved*: all 37 lines were read at
>   v0.0.33; the eleven-substring ladder maps onto `AcpFailure::classify` (typed, three variants)
>   plus `classify_terminal` exactly as that closure states.
> - **"ninth command" factual error** — *confirmed*, recorded as a residual on `ACP-070`'s closure.
> - **Calibration datum (`single-source` rows proved true)** — *confirmed again* (every
>   `single-source` row re-read in pass 2 was a real unit; all but `ACP-294`/`ACP-296` closed on
>   built code); a README-level observation, not
>   applied there by this pass.
...
> **Status 2026-09-24 (pass 2), by `grep -rn 'ACP-Q<n>' crates/` at `ea23ca2` plus reading each
> site.** *Decided in code* (the site carries `ACP-Q<n>`, most with `decided`): Q1 (root
> `Cargo.toml`), Q2, Q3, Q5, Q7, Q9, Q12, Q15, Q16, Q17 ("settled by this", `commands.rs`), Q18,
> Q19, Q20, Q23, Q24, Q25 ("answered", `ledger.rs`), Q26, Q27, Q30–Q38, Q40–Q46. *Moot*: Q14 (no
> `models` field, `ACP-065`), Q21 (eviction is structural, `ACP-061`). *Deferred in code without a
> decision*: Q13 (boolean config options, a superset enhancement), Q22 (the queue `_meta` is emitted
> anyway), Q39 (per-command capability filter). *Unanswered anywhere, needing an observed client
> or an owner*: Q4, Q6, Q8, Q10, Q11 (answered in practice as byte-parity, see `ACP-058`'s
> closure), Q28, Q29. The table below is kept as written.
...
**Re-audit 2026-09-24 at cyrup `ea23ca2` / pi-acp `v0.0.33`.** `git log 9aeba769..ea23ca2 --
crates/cyrup-acp` is empty and `crates/cyrup-it/tests/bin/acp_*.rs` is unchanged, so no row moved
because of new code; the crate's only external movement is additive (`crates/cyrup/src/predispatch.rs`
gained `__subagent-inspector` and still classifies `--terminal-login` LAST, which is `ACP-001`'s
invariant; `HostServices` gained `editor_has_focus`/`registered_provider` with defaulted
implementations). **This pass personally re-read both sides of all five open criticals and five
highs and closes ten rows** — `ACP-121`, `ACP-145`, `ACP-209`, `ACP-219`, `ACP-291`, `ACP-005`,
`ACP-056`, `ACP-122`, `ACP-140`, `ACP-221` — each struck below with the Rust symbol, the landing
commit, the upstream lines and a stated residual. Nothing was built or run. **Current arithmetic:
150 rows, 57 closed, 93 open — 0 critical, 9 high, 48 medium, 36 low.** **The other 93 rows were NOT
re-read this pass**; they carry the 2026-09-14 status by identity of the code, and the census
section above (145 of 150 built at `b28d3ff`) says most of them are closable by the same kind of
read. The nine open highs are `ACP-022`, `ACP-059` (superseded by `ACP-Q7`, not built),
`ACP-060` (built-unreachable), `ACP-061`, `ACP-123`, `ACP-126`, `ACP-144`, `ACP-153`, `ACP-155`.

**Re-audit 2026-09-24, pass 2 — every one of those 93 rows re-read on both sides.** Upstream
`pi-acp` v0.0.33's 17 `src/` files were read in full (`git show v0.0.33:<path>`), and the whole of
`crates/cyrup-acp/src` production code plus the ACP-bearing binary files (`crates/cyrup/src/{run,
main,signals,acp_host,acp_terminal_login_cmd}.rs`, `cli/config_map.rs`, `ext_mode` in
`crates/cyrup-session-svc/src/builder.rs`) at `ea23ca2`; three schema claims were checked in
`agent-client-protocol-schema-1.7.0` / `agent-client-protocol-2.1.0` from the cargo registry. Nothing
was built or run. **90 rows are struck `CLOSED 2026-09-24 (pass 2)`** with the Rust symbol, the
landing commit and the upstream lines; four of them close on a decision rather than code
(`ACP-059`, `ACP-060`, `ACP-220` under `ACP-Q7`; `ACP-281` accepted divergence) and carry a
falsification condition, and `ACP-055`/`ACP-125` close as duplicate/superseded. **Three stay open
with a re-read stamp**: `ACP-014` (medium — re-described: `session/resume`, `session/close` and
`logout` are ungated v1 methods with no handler, and an unclaimed session-scoped message is retained
unanswered), `ACP-294` (low, partial) and `ACP-296` (low, pin test absent). **Two items filed**:
`ACP-297` and `ACP-298`, both low, below the table. **Current arithmetic: 152 rows, 147 closed,
5 open — 0 critical, 0 high, 1 medium, 4 low.**
...
| ~~critical~~ **CLOSED 2026-09-24** | ACP-121 | A prompt resolves only on `agent_settled` — **CLOSED 2026-09-24.** BUILT. `TurnActor::on_event` (`crates/cyrup-acp/src/turn.rs`) reaches `finish` with a normal outcome only from the `TurnSignal::Settled` arm; `translate` maps `AgentEnd` to `TurnSignal::RunEnded` and `AgentSettled` alone to `Settled` (`translate.rs`, the `AgentSessionEvent::AgentSettled` arm), and `Turn::settle` returns `None` on a second call. Pinned by `turn.rs::a_prompt_resolves_only_on_agent_settled_and_exactly_once` (AgentStart / AutoRetryStart / AgentEnd{will_retry} / AgentStart / AgentEnd, then two `AgentSettled`s → exactly one `EndTurn`). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:474-510 (`startTurn`), :826-860 (`turn_end`/`agent_end` do not resolve; `agent_settled` resolves after `flushEmits`). Residual: the row's Verify names a faux provider that fails once and retries; the pin feeds events to the actor instead — the mechanism is the same seam, the end-to-end form is not written. Other non-settle exits (`Wake::Event(None)` → `Replaced`, teardown → `Cancelled`) answer `cancelled`, never a fabricated `end_turn`. Falsification: reopen if any arm other than `Settled` calls `finish(TurnOutcome::Settled)`. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | M | 4c | verified |
| ~~critical~~ **CLOSED 2026-09-24** | ACP-145 | Select: option ids and the strict round-trip — **CLOSED 2026-09-24.** BUILT, by the type the row prescribes. `DialogOptionTable::mint` (`crates/cyrup-acp/src/permission.rs`) mints `choice-<n>` ids and stores id→string in one vector; `DialogOptionTable::choose` resolves only inside `if let RequestPermissionOutcome::Selected`, by exact id lookup in THIS dialog's table, so a fabricated, stale or non-`Selected` outcome yields `None` → `reply_for` → `deny_default(UiKind::Select)` (`UiReply::Text(None)`). `DialogChoice` has a private constructor. Non-permission selects keep upstream's blanket `allow_once`; only the exact four-string list `LocalAskChannel::confirm` builds gets per-option kinds (a `CYRUP-DELTA`, rendering hint only — the reply is the string). Pinned by `selecting_the_second_option_returns_the_second_string`, `a_fabricated_option_id_can_never_produce_a_choice` (`choice-01`, `choice-1.0`, `choice--1`, past-end, past-safe-integer), `an_empty_select_never_reaches_the_client`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:913-935 (`handleExtensionSelect`), :993-1005 (`optionIndex`). Falsification: reopen if `choose` gains a path that is not an exact lookup in the minting table, or if `PERMISSION_DIALOG_OPTIONS` drifts from `crates/cyrup-permission-system/src/ask.rs`'s four option constants. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | S | 4c | verified |
...
| ~~critical~~ **CLOSED 2026-09-24** | ACP-209 | Single-flight restore — **CLOSED 2026-09-24.** BUILT. `RestoreGate::enter` (`crates/cyrup-acp/src/sessions.rs`) re-takes the live check INSIDE a `tokio::sync::Mutex`, and `SessionManager::restore_session` passes `self.get(id)` (id-filtered slot read) as the live check and `build_and_install` (which installs before returning) as the build — so the second of two racing prompts observes the first's install. `session/load` goes through `RestoreGate::rebuild` under the same lock (`ACP-225`). A failed build is not cached. Pinned by `two_concurrent_restores_build_exactly_once`, `prompt_short_circuits_on_live_and_load_does_not`, `a_failed_restore_is_retried_rather_than_remembered`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:181-234 (`restoreSession` + `restoringSessions`). Residual: the pin is gate-level with a counting stand-in; the row's wire-level Verify (two `session/prompt`s, JSONL holds exactly two user entries) is not written. The same race stays open for `cyrup-tui`'s `/resume` (handed to areas 07/08, recorded in the census above). Falsification: reopen if `restore_session`'s build closure stops installing before it returns, or any id-bearing handler builds a runtime without going through the gate. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | M | 4d | verified |
| ~~critical~~ **CLOSED 2026-09-24** | ACP-219 | `session/delete` of the session that is currently live — **CLOSED 2026-09-24.** BUILT. `SessionManager::delete_session` (`crates/cyrup-acp/src/sessions.rs`) takes the live session out of the slot (`take_live`), unbinds its turn actor and config pump (`cb290d1`), and obtains the path to unlink ONLY via `LiveSession::dispose_and_take_path`, which consumes and disposes the session — so dispose-before-unlink is a property of the type. The row's Verify is written verbatim as `crates/cyrup-it/tests/bin/acp_session.rs::deleting_the_live_session_removes_its_file_and_leaves_no_stub` (`{}` response, file gone, a following prompt is `-32602 Unknown sessionId`, no stub after a 500 ms drain, repeat delete idempotent). Landed `0aefd08` + `cb290d1`. Upstream read at `v0.0.33`: src/acp/agent.ts:1111-1134 (`deleteSession`, no active-session guard). Not executed this pass (no cargo). Falsification: reopen if a second route to the live session's path appears that does not dispose. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | S | 4d | verified |
| ~~critical~~ **CLOSED 2026-09-24** | ACP-291 | `/export` composes a path from client input and writes it unguarded — **CLOSED 2026-09-24.** BUILT. Every id-bearing handler parses through `AcpSessionId::parse` → `cyrup_session::ids::validate_session_id` (alphanumeric ends, `[A-Za-z0-9._-]` inside — no `/`, `\`, NUL or leading `.`), and `AcpSessionId::export_path_in` (`crates/cyrup-acp/src/ids.rs`) is the only constructor of `ExportPath`, re-checking `normalize(path).parent() == Some(dir)`; `commands.rs::export` can only hand an `ExportPath` to `export_to_html`, and `export_dir` (`cb290d1`) takes the session's own cwd. Pinned by `ids.rs::a_hostile_session_id_never_becomes_a_path`, `export_path_is_always_a_direct_child_of_the_session_cwd`, `commands.rs::the_export_path_is_agent_composed_and_contained`, and on the wire `acp_session.rs::an_unknown_session_is_invalid_params_and_the_connection_survives` (`session/delete` with `../../etc/passwd` → `-32602`, validator message). Landed `0aefd08` + `cb290d1`. Upstream read at `v0.0.33`: src/acp/agent.ts:788-789 (`safeSessionId` regex + `join(session.cwd, …)`). Residual: the Verify's exact wire form (`session/load` with a hostile id, temp-root scanned for no write) is not written — the wire test uses `session/delete`, and `prepare_load` reaches the same `locate` → `parse`. Falsification: reopen if `ExportPath` gains a second constructor or `validate_session_id` admits a path separator. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | S | 4e | single-source |
...
| ~~high~~ **CLOSED 2026-09-24** | ACP-005 | Stdin EOF and close terminate the process, and dispose first — **CLOSED 2026-09-24.** BUILT. `crates/cyrup-acp/src/connection.rs` calls `conn.sessions().shutdown().await` on every exit out of `connect_to`, before propagating the result; `SessionManager::shutdown` takes the slot, unbinds, and awaits `runtime.dispose()` (idempotent). Pinned by `crates/cyrup-acp/tests/wire_gaps.rs::the_transport_ending_disposes_the_live_session` (one dispose, reason `quit`) and the real-binary stdin-EOF path in `crates/cyrup-it/tests/bin/acp_transport.rs` (exit 0). Landed `cb290d1` (+ test `8de7460`). Upstream read at `v0.0.33`: src/index.ts:54-73 (`shutdown` on stdin `end`/`close`, SIGINT/SIGTERM). Residual: the Verify's "no child process group survives" half is not asserted end to end. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | S | 4a | verified |
...
| ~~high~~ **CLOSED 2026-09-24** | ACP-022 | A mid-turn provider 401/403 is not an `Err` — classify at the settle boundary — **CLOSED 2026-09-24 (pass 2).** BUILT. `RunTermination::of` (`crates/cyrup-acp/src/translate.rs`) reads the terminal `AssistantMessage` off each `AgentEnd`; `AcpFailure::classify_terminal` (`error.rs`) accepts only anchored `http 401:`/`http 403:`, the three `AuthError` prefixes, and the delimited `: 401: `/`: 403: ` token; `TurnActor::on_event` holds the last verdict until `Settled`. Pinned by `the_terminal_classifier_is_anchored_and_never_reads_a_digit_run` (the `214031` row) and `a_mid_turn_401_asks_the_client_to_authenticate`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:489-500, src/acp/auth-required.ts:9-36. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4a | single-source |
| ~~high~~ **CLOSED 2026-09-24** | ACP-056 | `session/new` rejects a non-absolute `cwd` — **CLOSED 2026-09-24.** BUILT. `SessionManager::new_session` opens with `AbsCwd::parse(req.cwd.clone())?` before `build_runtime`, message `cwd must be an absolute path: <p>` → `INVALID_PARAMS_CODE` (`error.rs`). Pinned in `sessions.rs` tests (`relative/dir` → exact message, host never asked). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:272-275. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | XS | 4b | verified |
| ~~high~~ **CLOSED 2026-09-24** | ACP-059 | Zero available models means unauthenticated — **CLOSED 2026-09-24 (pass 2).** CLOSED — SUPERSEDED by decision `ACP-Q7`, recorded at `SessionManager::decorate_new_session` (`crates/cyrup-acp/src/sessions.rs`): a zero-model session is returned with no `model` option and a leading `Warning:` chunk (`ACP-017`), not refused. Upstream read at `v0.0.33`: src/acp/agent.ts:330-339. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Falsification: reopen if `ACP-Q7` is reversed; the wire shape to build is then `AcpFailure::AuthRequired` through the existing `new_session` `Err` arm. | S | 4b | verified |
| ~~high~~ **CLOSED 2026-09-24** | ACP-060 | The destructive rollback on a normal error path — **CLOSED 2026-09-24 (pass 2).** CLOSED — BUILT-UNREACHABLE by decision `ACP-Q7`. `SessionManager::cleanup_failed_new_session` (dispose, then `purge_partial_session_file`: containment-checked direct `remove_file`, no trash) is wired to `new_session`'s `Err` arm, which cannot fire because `decorate_new_session` is infallible (`#[allow(clippy::unnecessary_wraps)]`, stated in its doc). Mechanism pinned by `a_partial_session_file_is_purged_only_from_inside_the_sessions_root`. Upstream read at `v0.0.33`: src/acp/agent.ts:141-158. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Falsification: reopen, with `ACP-220`'s assertion, the day `decorate_new_session` gains a failing path. | S | 4b | verified |
| ~~high~~ **CLOSED 2026-09-24** | ACP-061 | One live session per connection — **CLOSED 2026-09-24 (pass 2).** BUILT. One slot: `SessionManager::install` (`sessions.rs`) replaces `live`, shuts the old `SessionBinding` down and disposes the outgoing runtime; `TurnActor::on_prompt` answers `Unknown sessionId` for an id not bound; `AcpTurnAgent::rebound` and `config_pump` re-install the ui sinks. Recorded divergence: eviction is `session_shutdown{reason:"quit"}`, not a generation bump/`SessionReplaced` (per-session runtime), CYRUP-DELTA on `install`. A prompt for the evicted id restores it from disk, which is upstream's behaviour too. Pinned by `bind_repoints_the_actor_at_the_newly_installed_session`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:372-377, :955-957; src/acp/session.ts:168-185. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4b | verified |
...
| ~~high~~ **CLOSED 2026-09-24** | ACP-122 | `lastEmit` ordering: the response never overtakes a notification — **CLOSED 2026-09-24.** BUILT. `TurnActor::finish` (`crates/cyrup-acp/src/turn.rs`) emits the last `session_info_update` and then `action.deliver()` with nothing between; `TurnSink::notify` for `ConnectionTo<Client>` is a synchronous enqueue whose `Err` is discarded (`let _ = self.send_notification(..)`), so a failed notification cannot stop the response. Pinned by `turn.rs::the_response_never_overtakes_a_notification` and on the real binary by `crates/cyrup-it/tests/bin/acp_session.rs` (chunk position < response position). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:401-418 (`emit` chain + `flushEmits`), :839-845. Residual: the `send_notification`-returns-`Err` half is pinned by the code shape, not a test. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | S | 4c | verified |
| ~~high~~ **CLOSED 2026-09-24** | ACP-123 | `cancelRequested` and the `StopReason` mapping — **CLOSED 2026-09-24 (pass 2).** BUILT. `session/cancel` is a notification handler calling `SessionManager::request_cancel` synchronously; `Turn::request_cancel` flags the turn, `TurnActor::on_cancel` aborts (and latches a cancel that lands during admission), and `Turn::settle` alone maps the stop reason from that turn's flag. Pinned by `the_stop_reason_is_a_function_of_the_outcome_and_this_turns_own_cancel_flag`, `a_cancelled_turn_settles_as_cancelled_on_its_own_agent_settled`, `a_cancel_during_preflight_is_not_swallowed`, and `acp_session.rs` (cancel dispatched before the response). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:375-399, src/acp/agent.ts:885-899. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
| ~~high~~ **CLOSED 2026-09-24** | ACP-126 | The prompt-failure path: flush, auth detection, queue clearing — **CLOSED 2026-09-24 (pass 2).** BUILT. `TurnActor::admit` answers a preflight `Err` with `AcpFailure::classify` as a JSON-RPC error for that submission only (queued/folded ones keep riding the run); `finish` sends the last notification before the response; `TurnHandle::fail` discharges a turn a host tears down. Pinned by `a_refused_preflight_answers_with_an_error_and_the_actor_survives`, `the_host_can_fail_an_in_flight_turn_and_failing_an_idle_one_is_a_no_op`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:489-513. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
...
| ~~high~~ **CLOSED 2026-09-24** | ACP-140 | `bashOutputDelta`: the append-only terminal delta — **CLOSED 2026-09-24.** BUILT, and deliberately better than upstream. `TerminalAppender::push` (`crates/cyrup-acp/src/ledger.rs`) returns `Push::{Nothing, Append, Resync, Desynced}`; a slid window resyncs on the overlap instead of upstream's `: next` full re-send, and a desynced FINAL push falls back to a text content block. Pinned by `a_growing_snapshot_yields_only_the_suffix`, `a_slid_window_resyncs_on_the_overlap`, `a_sliding_window_never_repeats_or_drops_a_line` (20 lines through a 4-line window, concatenated data == whole output, each line once), `the_tail_of_a_desynced_final_push_survives_as_content`, and end to end by `crates/cyrup-acp/tests/end_to_end.rs::a_bash_call_streams_terminal_output`. Landed `0aefd08` + `cb290d1` (`Resync`). Upstream read at `v0.0.33`: src/acp/translate/bash.ts:79-81 (`bashOutputDelta`), src/acp/session.ts:442-450. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | M | 4c | verified |
| ~~high~~ **CLOSED 2026-09-24** | ACP-144 | `extension_ui_request` dispatch and the catch that always answers — **CLOSED 2026-09-24 (pass 2).** BUILT. `PermissionBridge::run_with_caps` (`crates/cyrup-acp/src/permission.rs`) spawns one `serve_dialog` per request; every path sends exactly one `UiReply` (client error → `deny_default`; guest gone → `reply.closed()` wins); sinks installed by `SessionSinks::install` on bind and re-install. Pinned by the `ACP-144`/`ACP-150` tests in `permission.rs`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:771-781, :871-911. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4c | verified |
...
| ~~high~~ **CLOSED 2026-09-24** | ACP-153 | Use `prompt`'s run-scoped stream, not `subscribe()` — **CLOSED 2026-09-24 (pass 2).** BUILT. `RuntimeAgent::start_run` calls `AgentSession::prompt_run`, returning the run-scoped stream and the `PromptAccepted` verdict; `Handled` is answered directly; session-wide `subscribe()` is used only by `config_pump`, which settles nothing. Pinned by `a_settle_unbinds_the_run_scoped_stream`, `a_submission_an_extension_handled_is_answered_rather_than_parked`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:474-514. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4c | single-source |
...
| ~~high~~ **CLOSED 2026-09-24** | ACP-155 | Never await a client round trip on the event pump — **CLOSED 2026-09-24 (pass 2).** BUILT. `TurnSink::notify` is a plain `fn`; dialogs run on `PermissionBridge`'s own detached tasks; admission is a future polled by the actor's `select!` rather than awaited (`TurnActor::pump_admission`). Pinned by `the_pump_keeps_draining_while_a_submission_is_being_admitted` and `permission.rs`'s `ACP-155` test (an unanswered dialog does not block the next). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:771-781. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual: the tabled >1 024-event backpressure run is not written. | M | 4c | single-source |
...
| ~~high~~ **CLOSED 2026-09-24** | ACP-221 | `restoreSession`'s failure mapping, and the `cx.spawn` trap — **CLOSED 2026-09-24.** BUILT. `serve_prompt` (`crates/cyrup-acp/src/sessions.rs`) answers a failed `restore_session` with `responder.respond_with_error(failure.into())` and returns `()` — there is no `Err` to propagate out of the spawned task; `AcpFailure::classify` maps `SessionServiceError::MissingSessionCwd` to an internal error, explicitly not auth-required (`error.rs`). Pinned by `restore_maps_an_unknown_id_and_a_failed_build_to_distinct_errors` and, for connection survival after bad requests, `acp_session.rs::an_unknown_session_is_invalid_params_and_the_connection_survives`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:196-210 (the spawn try/catch → `internalError`). Residual: a wire test for specifically a deleted recorded cwd followed by a next request is not written. Re-read at `ea23ca2` (crate byte-identical to `9aeba769`). | S | 4d | single-source |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-004 | A failed stdout write is a clean exit, not an error — **CLOSED 2026-09-24 (pass 2).** BUILT. `run_acp_dispatch` (`crates/cyrup/src/run.rs`) maps `AcpError::Transport` to `Ok(())` when `is_client_hangup` matches: code `-32603` and the `BrokenPipe`/`NotConnected`/`UnexpectedEof` `io::ErrorKind` display text found in `data` (where `into_internal_error` puts it) or `message`. Unit-pinned in `run.rs` tests (hang-up kinds true, `EIO` and impostor codes false). Landed `cb290d1` (the predicate previously read `message` only and could never match). Upstream read at `v0.0.33`: src/index.ts:24-40, :76-82. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual: the tabled `cyrup-it` stdout-closed assertion is not written; the predicate is unit-tested only. | S | 4a | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-012 | Zed's `_meta["terminal-auth"]` compat shape, gated on the client's probe — **CLOSED 2026-09-24 (pass 2).** BUILT. `ClientView::from_request` (`crates/cyrup-acp/src/connection.rs`) evaluates the probe with `Value::as_bool` (strict `=== true`); `auth_methods` (`config_options.rs`) attaches `_meta["terminal-auth"]` with `label: "Launch cyrup"` only when `!auth_terminal && terminal_auth_meta` (CYRUP-DELTA: typed `auth.terminal` preferred). Pinned by `the_legacy_terminal_auth_shim_is_gated_on_the_probe_and_suppressed_by_the_typed_flag`, `a_truthy_non_boolean_probe_does_not_produce_the_shim`, `the_terminal_auth_probe_is_strict`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/auth.ts:13-42, src/acp/agent.ts:251-253. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4a | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-013 | The terminal-auth launch spec must name this executable — **CLOSED 2026-09-24 (pass 2).** BUILT. `auth_methods` (`config_options.rs`) names `std::env::current_exe()` with a `"cyrup"` fallback and `args: [TERMINAL_LOGIN_ARG]` (one element); upstream's node/.js sniff is cut as specified. Asserted non-empty in the shim test above. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/auth.ts:44-59. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4a | verified |
| medium | ACP-014 | `authenticate` is a successful no-op — **re-read 2026-09-24 (pass 2) — stays OPEN, re-described.** The `authenticate` half is BUILT (`connection.rs` inline handler returning `AuthenticateResponse::new()`; pinned on the wire by `acp_transport.rs` `a_later_request_is_answered_while_session_new_is_still_building` and `acp_session.rs`). The unit's own verify, the handler-completeness test over the schema's method names, **fails if written**: `agent-client-protocol-schema-1.7.0` `src/v1/agent.rs:4761-4765` declares `session/resume`, `session/close` and `logout` **ungated** (no feature flag), and `connection.rs`'s builder registers none of them. `agent-client-protocol-2.1.0` `src/role/acp.rs:308-332` returns `Handled::No { retry: message.has_session_id() }` for an unclaimed v1 message, and `incoming_actor.rs`'s pending list keeps it until a dynamic handler claims it, so a `session/close`/`session/resume` is retained unanswered forever — the hang `connection.rs`'s own module doc warns about, contradicting its claim that every served method is registered. Reachability: none of the three capabilities is advertised (`agent_capabilities`), so only a client that ignores capabilities reaches it. Upstream read at `v0.0.33`: src/acp/agent.ts:432-436 (no close/resume handler either; what the TS SDK answers for them was not readable here). Fix: register the three and answer `-32601`, plus the enumeration test. Severity kept `medium` on what it does when reached (an unanswered request). Mechanism read in the SDK source, not run. | XS | 4a | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-016 | The `AUTH_REQUIRED` payload: data shape and message string — **CLOSED 2026-09-24 (pass 2).** BUILT. `AcpFailure::into_error` (`crates/cyrup-acp/src/error.rs`) builds `Error::new(-32000, AUTH_REQUIRED_MESSAGE).data({authMethods, detail})`; `From<AcpFailure> for Error` attaches `auth_methods_for_error()` (typed method only, no `_meta` shim: the recorded resolution of the asymmetry). Pinned by `the_auth_required_payload_is_byte_exact` and `turn.rs`'s `a_mid_turn_401_asks_the_client_to_authenticate` (one method in `data.authMethods`). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/auth-required.ts:31-36, src/acp/agent.ts:335-346. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4a | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-017 | Zero available models is treated as unauthenticated — **CLOSED 2026-09-24 (pass 2).** BUILT in the reversed form `ACP-Q7` chose (option b). `SessionConfigKnob::advertise` omits the `model` option for an empty catalog; `startup_updates` (`crates/cyrup-acp/src/startup.rs`) emits `Warning: <model_fallback_message>` as the first follow-up chunk. Pinned by `a_modelless_session_leads_with_the_model_fallback_warning` and `the_model_option_is_membership_checked_and_omitted_for_an_empty_catalog`. Landed `cb290d1` (the banner half). Upstream read at `v0.0.33`: src/acp/agent.ts:330-339. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Falsification: reopen if `ACP-Q7` is reversed. | M | 4a | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-023 | `spawn_abort_on_signal` needs a runtime the lazy build does not have yet — **CLOSED 2026-09-24 (pass 2).** BUILT as decided: `AcpHost::runtime_ready` (`crates/cyrup-acp/src/sessions.rs`, doc) arms the watcher on the first `session/new`; `RuntimeSlot` (`crates/cyrup/src/signals.rs`) re-points one watcher at later runtimes. Landed `0aefd08`; slot `dc52eeb`-era. Upstream read at `v0.0.33`: src/index.ts:54-73. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). The tabled verify (SIGTERM before any `session/new` runs the watcher) is **not** satisfied by design: that window takes tokio's default disposition. Falsification: reopen if anything can start a tracked detached child or announce an extension before the first `session/new`. | S | 4a | single-source |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-050 | `initialize` clamps the requested protocol version — **CLOSED 2026-09-24 (pass 2).** BUILT. `clamp_protocol_version` (`connection.rs`) is total and returns `V1`; the SDK-imposed parse divergence is a CYRUP-DELTA there. Pinned by `every_requested_protocol_version_clamps_to_v1` and `acp_transport.rs`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:237-243. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4b | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-054 | `authMethods` and the conditional `_meta` shim, from `initialize` — **CLOSED 2026-09-24 (pass 2).** BUILT. `auth_methods(&view)` at the `initialize` handler (`connection.rs`), gated as `ACP-012`. Both tabled JSON assertions exist in `config_options.rs` tests. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:251-253. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-058 | The auth-required / internal-error paths of `session/new` — **CLOSED 2026-09-24 (pass 2).** BUILT. Branches 1/2/4 collapse into `build_runtime`'s `Err` → `AcpFailure::from` → `classify` (`NoConfiguredAuth`/`AuthPreflightRefused`/`NoModelSelected` → `-32000` with `data.authMethods`, else `-32603`); branch 3 is `ACP-059`. `ACP-Q11` answered byte-parity: `message` is upstream's sentence, the specific remedy rides in `data.detail`. Pinned by `typed_auth_states_classify_and_nothing_else_does`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:318-347. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-062 | The ACP mode list is the thinking-level ladder — **CLOSED 2026-09-24 (pass 2).** BUILT. `SessionConfigKnob::mode_state` over `available_thinking_levels()` with `thinking_level_id`/`thinking_level_name` (`config_options.rs`); seven rungs incl. `max`, model-dependent, current level read not guessed. Pinned by `a_reasoning_model_yields_the_whole_ladder_and_the_real_current_level`, `a_non_reasoning_model_yields_exactly_off_and_the_surface_still_exists`, `cyrup_has_a_thinking_level_pi_does_not`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1200-1241. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-063 | The advertised model list and current selection — **CLOSED 2026-09-24 (pass 2).** BUILT. `SessionConfigView::model_current_value` is membership-checked with a first-entry fallback and no `'default'` sentinel; empty catalog omits the option. Pinned by `the_model_option_is_membership_checked_and_omitted_for_an_empty_catalog` and `a_missing_or_stale_current_model_falls_back_to_the_first_entry_never_to_default`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1319-1384. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). See new `ACP-298` for the set-side bare-id gap. | S | 4b | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-065 | `NewSessionResponse` has no `models` field — **CLOSED 2026-09-24 (pass 2).** BUILT. No `models` key and no `_meta` shim on `NewSessionResponse`/`LoadSessionResponse` (`decorate_new_session`, `prepare_load`); decided in `config_options.rs`'s module doc (`ACP-065`/`ACP-Q14`). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:379-389, :1065-1074. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-066 | The markdown startup prelude — **CLOSED 2026-09-24 (pass 2).** BUILT. `StartupInventory::of` reads the live registries and `render_markdown` (`crates/cyrup-acp/src/startup.rs`) is the pure renderer; the four tabled cases are `the_prelude_is_headings_and_bullets_and_an_empty_section_emits_nothing`, `diagnostics_alone_are_enough_to_produce_a_prelude`, `quiet_startup_keeps_the_diagnostics_and_drops_the_listing`, `a_bare_project_produces_no_prelude_at_all`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1488-1644. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual filed separately as `ACP-297` (the dropped version header and update notice carry no CYRUP-DELTA). | M | 4b | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-069 | `available_commands_update` is also deferred past the response — **CLOSED 2026-09-24 (pass 2).** BUILT. `commands::available_commands_update` is the one function both `decorate_new_session` (as `follow_up`, after the response via `respond_then_notify`) and `prepare_load` (after the response in `handle_load`) call; extension commands included per `ACP-269`. Landed `0aefd08`; the load half fixed at integration (see `prepare_load`'s comment). Upstream read at `v0.0.33`: src/acp/agent.ts:395-427, :1076-1106. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-070 | The eight headless built-ins — **CLOSED 2026-09-24 (pass 2).** BUILT as seven of eight: `pub const BUILTINS: [Builtin; 7]` (`crates/cyrup-acp/src/commands.rs`) with `name`/`description`/`hint`/`parse` on one enum, so advertisement and dispatch cannot diverge; `/changelog` cut under a CYRUP-DELTA with a re-add condition. Pinned by `the_advertised_list_is_upstreams_fixture`, `every_builtin_name_round_trips`. Landed `0aefd08`; arms `cb290d1`. Upstream read at `v0.0.33`: src/acp/agent.ts:64-104, :658-734. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual (doc only): that CYRUP-DELTA says upstream advertises "a ninth command"; `agent.ts:64-104` has eight and `changelog` is the eighth. | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-073 | `session/set_config_option` routes `model` and `thought_level` — **CLOSED 2026-09-24 (pass 2).** BUILT. `SessionConfigKnob::parse` (`config_options.rs`) produces upstream's three strings (`Expected string value…`, `Unknown thinking level: …`, `Unknown config option: …`); `apply_config_option` returns the re-derived set; the two notifications come from `config_pump`'s `ThinkingLevelChanged` arm in the pinned order (`current_mode_update` then `config_option_update`), per `ACP-Q20`. Pinned by `an_unknown_config_id_is_invalid_params_with_the_exact_message`, `a_boolean_value_is_refused_with_upstreams_typeof_message`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1167-1197. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). See `ACP-298`. | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-075 | `emitConfigOptionsUpdate` re-derives the whole option set — **CLOSED 2026-09-24 (pass 2).** BUILT. `config_options_update`/`apply_config_option` re-derive from `SessionConfigView::of(session)` after the mutation; never patched from the request. Pinned by `the_config_update_carries_the_whole_option_set`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1386-1402. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-077 | Push config/mode updates on session-originated changes — **CLOSED 2026-09-24 (pass 2).** BUILT. `config_pump` / `session_updates_for` (`sessions.rs`) subscribe session-wide `ModelChanged`/`ThinkingLevelChanged`/`SessionInfoChanged` and are the single emitter; setters emit nothing except `ACP-072`'s no-op echo. Pinned by `acp_session.rs` `set_mode_answers_empty_and_the_pump_reports_the_applied_level`. Landed `0aefd08`. No upstream (latent upstream defect). Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual: no test drives a non-ACP model change (the tabled verify) end to end. | S | 4b | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-078 | The `Unknown sessionId` gate every setter opens with — **CLOSED 2026-09-24 (pass 2).** BUILT. Both setters open with `SessionManager::live_session` → `restore_session` (`sessions.rs`), whose `locate` answers `unknown_session` (`Unknown sessionId: <id>`, `-32602`) before anything is built or mutated. Pinned by `restore_maps_an_unknown_id_and_a_failed_build_to_distinct_errors`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:181-195, :1143-1144, :1167-1168. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4b | single-source |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-079 | The setters do real blocking work and must leave the dispatch loop — **CLOSED 2026-09-24 (pass 2).** BUILT. `connection.rs` runs `set_mode` and `set_config_option` inside `cx.spawn(..)` and `respond_then_notify` returns `Ok(())` on the failure arm. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1143-1197. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4b | single-source |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-120 | SessionManager: registry, lookup error string, one-live-session collapse — **CLOSED 2026-09-24 (pass 2).** BUILT. `SessionManager::unknown_session` (`sessions.rs`) is the one `Unknown sessionId: <id>` builder, used by `TurnActor::on_prompt`, `serve_prompt` and `locate`; the registry is the one-slot `live`. Pinned by `the_unknown_session_message_is_byte_exact`, `a_prompt_for_an_unknown_session_is_rejected_by_the_turn`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:230-234. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-124 | The turn queue and the `_meta` queue-depth publication — **CLOSED 2026-09-24 (pass 2).** BUILT with the recorded divergence. `RuntimeAgent::fold_into_run` folds a second prompt as `StreamingBehavior::FollowUp`; `TurnActor::admit` emits `Queued message (position N).` then the `cyrupAcp` queue `_meta`; N folded requests settle together on one `AgentSettled` (decided on `RunningTurn::responders`), so the tabled "second response after the second settle" does not hold by design. Pinned by `two_overlapping_prompts_fold_into_one_run_and_settle_together`, `folding_numbers_the_queue_the_way_upstream_does`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:337-373, :839-864. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-127 | `message_update`: text and thinking deltas to chunks — **CLOSED 2026-09-24 (pass 2).** BUILT. `message_update` (`translate.rs`) emits one chunk per `TextDelta`/`ThinkingDelta` and nothing for the other `StreamEvent` arms; `message_id` left `None` for parity. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:520-538. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-128 | Early tool-call surfacing from streaming deltas — **CLOSED 2026-09-24 (pass 2).** BUILT with decided divergence `ACP-Q23`: `ToolCallStart` announces `pending`, `ToolCallDelta` emits nothing, `ToolCallEnd` emits one update with complete args (`translate.rs` `message_update`/`surface_tool_call`); the `{partialArgs}` wrapper has no input. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:540-607. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-129 | Monotonic tool-call status and the first-vs-update decision — **CLOSED 2026-09-24 (pass 2).** BUILT. `ToolCallLedger::announce` is the only `tool_call` producer and `update` takes `max(status)` over `ToolStatus` (no backward transition), so a second `tool_call` or a downgrade is unconstructible (`crates/cyrup-acp/src/ledger.rs`). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:280, :567-603. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-131 | `tool_execution_start` (non-bash): snapshot capture and the transition emit — **CLOSED 2026-09-24 (pass 2).** BUILT. `snapshot_needed` (`translate.rs`) requests the read; `snapshot_through` (`turn.rs`) reads through the session's own `FsOps` and classifies read/absent/unreadable; `tool_execution_start` attaches the snapshot and announces or transitions. Pinned by `the_snapshot_classifier_reads_a_real_file_through_the_sessions_own_fs`, `a_confined_session_snapshots_an_outside_path_as_absent_and_emits_no_diff`. Landed `0aefd08`, classifier test `cb290d1`. Upstream read at `v0.0.33`: src/acp/session.ts:613-686. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-136 | `toolResultToText`: the diff → content → stdout → JSON ladder — **CLOSED 2026-09-24 (pass 2).** BUILT with the stdout ladder cut under a CYRUP-DELTA: `tool_result_to_text` (`translate.rs`) is `details.diff` → joined text blocks → pretty JSON, `Null` → empty. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/translate/pi-tools.ts:1-51. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-139 | `emitBashToolCall` and the `terminal_info` `_meta` protocol — **CLOSED 2026-09-24 (pass 2).** BUILT. `ToolCallLedger::announce` is the only producer of `content[0].terminalId` and `_meta.terminal_info {terminal_id, cwd}`; updates cannot carry them. Exercised end to end by `crates/cyrup-acp/tests/end_to_end.rs` `a_bash_call_streams_terminal_output`. Landed `0aefd08`, e2e `772ed8d`. Upstream read at `v0.0.33`: src/acp/session.ts:420-440, src/acp/translate/bash.ts:83-92. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-141 | `bashExitCode` and the `terminal_exit` `_meta` — **CLOSED 2026-09-24 (pass 2).** BUILT, option (c): the bash tool's non-zero-exit arm sets `BashDetails { exit_code: Some(code) }` (`crates/cyrup-tools/src/tools/bash.rs`, field in `details.rs`) and `bash_exit_code` (`translate.rs`) probes `details.exitCode` first, then upstream's other three keys, then `is_error as i32`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/translate/bash.ts:72-77, :98-100. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual (recorded in code): timeout/kill report 1. | S | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-143 | `auto_compaction_start` / `_end` status chunks — **CLOSED 2026-09-24 (pass 2).** BUILT. `status_updates` (`turn.rs`) emits the two byte-exact strings for `Threshold`/`Overflow`, nothing for `Manual`, and gates the end string on `!aborted && error_message.is_none()` (CYRUP-DELTA). Pinned by `the_compaction_strings_are_byte_exact_and_the_success_string_is_gated`, `the_status_arms_have_exactly_one_producer`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:799-819. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-147 | Input and editor: cancellation with a visible fallback message — **CLOSED 2026-09-24 (pass 2).** BUILT. `ask_elicitation` (`permission.rs`) sends `elicitation/create` when the client advertised form-mode elicitation (`ClientView.elicitation` reads `.form`), else one chunk (`This client cannot display an extension … dialog; cancelling it.`) and `Text(None)`. Pinned by the four `ACP-147` tests in `permission.rs`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:888-898. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4c | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-158 | Prompt images and non-text content blocks reach the queued turn — **CLOSED 2026-09-24 (pass 2).** BUILT. `prompt_to_user_input` (`commands.rs`) returns `(text, images)` and `serve_prompt` builds one `UserInput { text, images, source: InputSource::Rpc, expand_templates: true }`; `/cmd` with an image is not intercepted. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:337-342, src/acp/agent.ts:441-445. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | single-source |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-205 | Title fallback chain, and the `(no messages)` sentinel — **CLOSED 2026-09-24 (pass 2).** BUILT. `title_of` (`sessions.rs`): name → first message clipped to 80 `char`s → `None`, sentinel mapped to `None` (CYRUP-DELTA: char vs UTF-16 count, multi-block join). Pinned by `the_title_ladder_clips_to_eighty_and_never_leaks_the_sentinel`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:114-189, :230-263. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-207 | `listSessions` defaults its cwd filter to `lastSessionCwd` — **CLOSED 2026-09-24 (pass 2).** BUILT. `list_sessions` (`sessions.rs`) uses `req.cwd` else `last_cwd()`, filter applied before the page slice. Pinned by `the_default_cwd_filter_is_the_last_session_cwd` (port of `session-list-scoped.test.ts`). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:901-908. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-208 | Numeric-offset opaque cursor, page size 50 — **CLOSED 2026-09-24 (pass 2).** BUILT with two recorded deltas: `parse_cursor` reproduces `parseInt` leniency; `nextCursor` is omitted rather than `null` on the last page (schema `skip_serializing_none`), `_meta: {}` kept; `ACP-Q32` keeps the offset cursor. Pinned by `every_malformed_cursor_means_page_one`, `pagination_is_fifty_rows_with_a_numeric_offset_cursor`, `the_last_page_omits_the_cursor_and_keeps_meta`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:910-927. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4d | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-213 | `AppMode::Acp` must persist sessions — **CLOSED 2026-09-24 (pass 2).** BUILT. `persists(no_session, explicit_session, mode)` (`crates/cyrup/src/cli/config_map.rs`) includes `AppMode::Acp`, and both former call sites route through it. Landed `0aefd08`. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-215 | Replay: synthetic completed tool-call pairs — **CLOSED 2026-09-24 (pass 2).** BUILT. `replay_tool_result` (`sessions.rs`) announces with the persisted `ToolCallId` then finishes through the ledger (no completed→failed flicker; `ACP-Q34` richer kinds). Pinned by `a_replayed_tool_result_is_a_pair_sharing_the_persisted_id`, `replayed_tool_kinds_are_the_live_classifiers_kinds`, `a_replayed_edit_shows_its_persisted_diff`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:999-1060. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4d | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-218 | `session/delete` is idempotent and deletes the file — **CLOSED 2026-09-24 (pass 2).** BUILT with the trash-first CYRUP-DELTA: `delete_session` (`sessions.rs`) is idempotent (`{}` when both lookups miss), resolves the live path via `dispose_and_take_path` else `find_stored`, and swallows a failed unlink with a `warn!`. Pinned by `delete_removes_the_file_and_is_idempotent`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1111-1135. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4d | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-220 | Remove the session file for a `session/new` that never returned an id — **CLOSED 2026-09-24 (pass 2).** CLOSED — verify UNSATISFIABLE by decision `ACP-Q7` (stated in `decorate_new_session`'s doc): the post-build failure arm this unit cleans up after cannot fire. The mechanism (`purge_partial_session_file`: containment check, direct `remove_file`, `NotFound` is success) is pinned by `a_partial_session_file_is_purged_only_from_inside_the_sessions_root`. Upstream read at `v0.0.33`: src/acp/agent.ts:141-158. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Falsification: identical to `ACP-060`'s. | S | 4d | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-224 | `deleteSession` leaves the ACP session live and usable — **CLOSED 2026-09-24 (pass 2).** BUILT, semantics pinned as `ACP-219`'s shape: deleting the live session disposes it (`LiveSession::dispose_and_take_path`) and unbinds it, a mid-turn prompt settles `cancelled`, and a later prompt for that id gets `Unknown sessionId`. Pinned by `acp_session.rs` `deleting_the_live_session_removes_its_file_and_leaves_no_stub` and `deleting_a_session_tears_down_the_binding_that_names_it`. Landed `0aefd08`; unbind `cb290d1`. Upstream read at `v0.0.33`: src/acp/agent.ts:1111-1135. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4d | single-source |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-267 | Project `slash_command_catalog()` rows into `AvailableCommand`s — **CLOSED 2026-09-24 (pass 2).** BUILT. `project_catalog` (`commands.rs`): nameless dropped (`ACP-Q38`), first-wins de-dup, description fallback, skill gate. Pinned by `the_projection_drops_nameless_rows_dedupes_and_never_emits_an_empty_description`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-commands.ts:22-59. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4e | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-269 | Reverse pi-acp's `source === 'extension'` exclusion — **CLOSED 2026-09-24 (pass 2).** BUILT (reversal decided, CYRUP-DELTA on `project_catalog`): extension rows are advertised. Pinned by `extension_commands_are_advertised`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-commands.ts:30, :46; src/acp/agent.ts:402-405. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual: the verify's execution half (`/deploy x` → notify chunk, no model call) is not asserted in the crate. | S | 4e | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-279 | `resource` → `[Embedded Context]` in three shapes — **CLOSED 2026-09-24 (pass 2).** BUILT. `prompt_to_user_input`'s `Resource` arm (`commands.rs`) renders the three shapes with upstream's mime defaults and `base64_decoded_len` (Node's `byteLength` rule). Pinned by `an_embedded_resource_renders_in_three_shapes`, `the_blob_byte_count_is_the_decoded_length_including_padding`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/translate/prompt.ts:37-55. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4e | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-281 | Unknown content blocks reject the turn where pi-acp dropped the block — **CLOSED 2026-09-24 (pass 2).** CLOSED — divergence DECIDED and recorded. Re-read `agent-client-protocol-schema-1.7.0`: `src/v1/agent.rs:3050` `pub prompt: Vec<ContentBlock>` carries no `VecSkipError`, and `src/v1/content.rs:34-38` `ContentBlock` has no `#[serde(other)]`, so an unknown block rejects the whole request. `prompt_to_user_input`'s CYRUP-DELTA (`commands.rs`) decides to accept the rejection rather than shim a `Vec<Value>` parser. Pinned by `an_unknown_block_type_is_rejected_by_deserialization_not_dropped`. Upstream read at `v0.0.33`: src/acp/translate/prompt.ts:64-66. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Falsification: reopen if a client is observed sending a non-schema block type. | XS | 4e | verified |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-282 | The built-in dispatch gate and argument split — **CLOSED 2026-09-24 (pass 2).** BUILT. `intercept` (`commands.rs`): no image, `trim_start().starts_with('/')`, split on the first literal space (CYRUP-DELTA), whole-name `Builtin::parse`; arguments tokenised by `cyrup_resources::parse_command_args`. Pinned by `the_interception_gate_matches_upstream_exactly`, `a_tab_after_the_command_name_is_not_a_command`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:445-450. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4e | verified |
...
| ~~medium~~ **CLOSED 2026-09-24** | ACP-290 | The advertised list is in nondeterministic order — **CLOSED 2026-09-24 (pass 2).** BUILT (CYRUP-DELTA on `project_catalog`): a stable sort within provenance groups by name, extension load order kept. Pinned by `the_projection_is_deterministic_whatever_order_the_catalog_arrives_in`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/slash-commands.ts:98-109, src/acp/agent.ts:106-118. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4e | single-source |
| ~~medium~~ **CLOSED 2026-09-24** | ACP-292 | Built-ins bypass the turn queue, and `/compact` aborts an in-flight turn — **CLOSED 2026-09-24 (pass 2).** BUILT, decided: refuse. `compact_refusal(session.is_run_active())` (`commands.rs`) answers `COMPACT_BUSY_MESSAGE` as a chunk (cyrup-original string, CYRUP-DELTA). Pinned by `compact_refuses_a_running_turn_and_says_so_in_one_chunk`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:452-476. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4e | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-006 | SIGINT and SIGTERM shut the ACP host down — **CLOSED 2026-09-24 (pass 2).** BUILT. `BinaryAcpHost::runtime_ready` (`crates/cyrup/src/acp_host.rs`) arms `spawn_abort_on_signal_slot(.., AppMode::Acp)` once per connection via `claim_watcher`; `signals.rs`'s host-array test iterates `[Rpc, Print, Json, Acp]`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/index.ts:72-73. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4a | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-018 | The ACP host disables theme discovery — **CLOSED 2026-09-24 (pass 2).** BUILT. `config.no_themes = self.no_themes \|\| mode == AppMode::Acp` (`crates/cyrup/src/cli/config_map.rs`), themes only; asserted in `crates/cyrup/src/cli/tests/config_map.rs`. The census's tension with `ACP-066` is resolved on reading: `disc.enable_themes = !cfg.no_themes` (`crates/cyrup-session-svc/src/builder.rs`) empties the discovered set, so the prelude's `Themes` block renders nothing under ACP, which is upstream's net output (`agent.ts:1642` excludes themes). Landed `0aefd08`. Upstream read at `v0.0.33`: src/pi-rpc/process.ts:137. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4a | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-024 | The stdin read-failure path is the unfiled half of ACP-004 — **CLOSED 2026-09-24 (pass 2).** BUILT. The split is pinned in `run_acp_dispatch`'s doc item 3 and `is_client_hangup` (`crates/cyrup/src/run.rs`): clean EOF and read-side `UnexpectedEof`/`BrokenPipe`/`NotConnected` exit 0, `EIO` stays non-zero; unit tests in `run.rs`. Landed `cb290d1`. Upstream read at `v0.0.33`: src/index.ts:42-48, :68-69. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4a | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-025 | `ext_mode` telling extensions the ACP host is `rpc` is a wire-visible decision — **CLOSED 2026-09-24 (pass 2).** BUILT. `ext_mode` (`crates/cyrup-session-svc/src/builder.rs`) maps `AppMode::Acp => (ExtMode::Rpc, true)` under a CYRUP-DELTA; pinned by `acp_reports_itself_to_extensions_as_rpc_with_a_ui`. `serve_prompt` uses `InputSource::Rpc` to agree. Landed `0aefd08`. No upstream (pi-acp's child is `pi --mode rpc`). Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4a | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-026 | `--terminal-login` must not bypass the TTY guard — **CLOSED 2026-09-24 (pass 2).** BUILT. `acp_terminal_login_cmd::refuse_when_not_a_terminal` then `strip` (`crates/cyrup/src/main.rs`, `Internal::AcpTerminalLogin` arm); pinned by `crates/cyrup-it/tests/bin/acp_transport.rs` `terminal_login_writes_no_frames_and_refuses_without_a_tty`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/index.ts:5-22. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4a | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-051 | `agentInfo` name / title / version — **CLOSED 2026-09-24 (pass 2).** BUILT. `AGENT_NAME = "cyrup"`, `AGENT_VERSION = env!("CARGO_PKG_VERSION")` (`connection.rs`, CYRUP-DELTA on the name); `title` is not sent (upstream's `pi ACP adapter` has no cyrup counterpart). Pinned by `agent_info_is_compile_time_identity`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:244-248, :1646-1666. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4b | verified |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-055 | `authenticate` answers success — **CLOSED 2026-09-24 (pass 2).** CLOSED as DUPLICATE of `ACP-014`'s built half: the same handler (`connection.rs`, `authenticate` arm) answers success for any `methodId`; wire-asserted by `acp_session.rs` (`ACP-014: authenticate is a successful no-op`). Upstream read at `v0.0.33`: src/acp/agent.ts:432-436. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4b | verified |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-081 | `buildStartupInfo` can never return an empty string — **CLOSED 2026-09-24 (pass 2).** BUILT, decided the better way: `render_markdown` returns `None` for an empty inventory and `startup_updates` emits no chunk (CYRUP-DELTA in `startup.rs`). Pinned by `a_bare_project_produces_no_prelude_at_all`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1643. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4b | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-082 | `lastSessionCwd` is connection-scoped state `session/new` writes — **CLOSED 2026-09-24 (pass 2).** BUILT. `SessionManager::last_cwd` is written by `new_session` and `build_and_install` (load and prompt-restore, after a successful build per `ACP-226`) and read by `list_sessions`. Pinned by `last_cwd_round_trips`, `the_default_cwd_filter_is_the_last_session_cwd`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:134, :222, :277, :907, :940. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4b | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-125 | Startup-info deferral: set / send-if-pending — **CLOSED 2026-09-24 (pass 2).** CLOSED — SUPERSEDED by mechanism: the prelude rides `HandlerOutcome::follow_up`, drained once by `respond_then_notify` (`crates/cyrup-acp/src/lib.rs`), so a second send is unexpressible. The census's "no written trace" is refuted: `startup.rs`'s module-level `ACP-068` CYRUP-DELTA states that upstream's `startupInfoSent` flag has no counterpart and why. Upstream read at `v0.0.33`: src/acp/session.ts:317-335, src/acp/agent.ts:369-370, :393. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-130 | `toToolCallLocations`: path probing and cwd resolution — **CLOSED 2026-09-24 (pass 2).** BUILT. `tool_path` + `locations_of` (`translate.rs`) resolve through `AbsCwd::resolve` (`ids.rs`, lexical normalize); a missing path yields no locations; `line: None` omits the key. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:78-83, :142-148. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | verified |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-133 | `getParsedEdits` / `getEditOldTexts`: current and legacy edit schemas — **CLOSED 2026-09-24 (pass 2).** BUILT. `edit_old_texts` (`translate.rs`) reads top-level `oldText` then `edits` (array or JSON string), de-duplicated first-wins; order pinned by `every_edit_shape_yields_its_needles_in_a_pinned_order`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:88-140. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-134 | `tool_execution_update`: partial output and file-mutation suppression — **CLOSED 2026-09-24 (pass 2).** BUILT. `tool_execution_update` (`translate.rs`) routes terminals to `terminal_progress` and suppresses both `content` and `rawOutput` for `ToolClass::Mutation`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:688-710. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4c | verified |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-138 | `isBashTool` and `bashCommand`: the tool-call title — **CLOSED 2026-09-24 (pass 2).** BUILT with the twelve-key probe cut (CYRUP-DELTA): `title_of` (`translate.rs`) reads `command` for terminal classes with upstream's trim guard; `ToolClass::of` is ASCII-case-insensitive, so `Bash` is recognised. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/translate/bash.ts:23-44. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-142 | `auto_retry_start` / `_end` status chunks and their exact strings — **CLOSED 2026-09-24 (pass 2).** BUILT. `format_auto_retry_message` (`turn.rs`) with the sub-second bump; `error_message` never emitted; the end string gated on `success` (CYRUP-DELTA). Pinned by `the_retry_message_is_byte_exact_including_the_sub_second_bump`, `no_retry_chunk_leaks_the_error_message`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:783-797, :1007-1020. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | verified |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-151 | `toToolKind`: the tool-name → `ToolKind` map — **CLOSED 2026-09-24 (pass 2).** BUILT. `ToolClass::of`/`acp_kind` (`ledger.rs`): bash/powershell → Execute, edit/write → Edit, read → Read, grep/find/ls → Search, else Other (CYRUP-DELTA). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:1022-1034. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-159 | `cancel()` resolves queued turns without flushing — **CLOSED 2026-09-24 (pass 2).** BUILT, pinned in `TurnActor::on_cancel`'s doc: under the folded queue there is no separate queue to resolve, so the order is flag → abort → one settle answering every folded responder `cancelled`. Pinned by `cancel_is_idempotent_and_legal_when_idle`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:375-395. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4c | single-source |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-200 | Sessions-directory resolution — **CLOSED 2026-09-24 (pass 2).** BUILT. `AcpHost::sessions_root` returns `ConfigDirs::session_dir` with the CLI > env > settings ladder applied by the binary (`crates/cyrup/src/main.rs` ACP arm); `session_dirs` (`sessions.rs`) refuses a relative root (`ACP-229`). Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:16-42. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-201 | sessionId → (file, cwd) resolution, local then cross-project — **CLOSED 2026-09-24 (pass 2).** BUILT. `find_stored` (`sessions.rs`) scans the root and one level below, orders by the filename hint and confirms by header, so local and cross-project resolve in one pass. Pinned by `resolution_crosses_projects_and_reads_no_session_bodies`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:160-179, src/acp/pi-sessions.ts:326-329. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-203 | Listing scan: header, title, updatedAt, ordering — **CLOSED 2026-09-24 (pass 2).** BUILT. `list_rows` (`sessions.rs`) delegates to `cyrup_session::listing::list_in_dir` per directory and sorts newest-first. Pinned by `updated_at_follows_the_last_message_not_the_header`, `a_leading_blank_line_does_not_hide_a_session`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:265-324. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-204 | `updatedAt` is a JS-`toISOString`-compatible string — **CLOSED 2026-09-24 (pass 2).** BUILT. `iso8601_millis`/`now_iso8601_millis` (`sessions.rs`) emit `YYYY-MM-DDTHH:MM:SS.mmmZ` for both `session/list` rows and `session_info_update`. Pinned by `updated_at_is_a_js_to_iso_string`, `the_two_updated_at_producers_agree_on_precision`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:191-228, :294-301. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-206 | ACP `SessionInfo.cwd` is a required absolute path — **CLOSED 2026-09-24 (pass 2).** BUILT. `to_acp_row` (`sessions.rs`) drops a row whose cwd is empty or relative. Pinned by `a_session_with_no_absolute_cwd_is_listed_by_cyrup_and_not_by_acp`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:101-112. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-210 | Unknown sessionId is `-32602` with the exact text — **CLOSED 2026-09-24 (pass 2).** BUILT. `unknown_session` builds `-32602` / `Unknown sessionId: <id>` by hand (not via strum). Pinned by `the_unknown_session_message_is_byte_exact`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:193-195, :942-945; src/acp/session.ts:232. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-211 | `cwd` must be absolute on `session/new` and `session/load` — **CLOSED 2026-09-24 (pass 2).** BUILT. `prepare_load` opens with `AbsCwd::parse` before any filesystem work. Pinned by `load_rejects_a_relative_cwd_with_upstreams_message`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:931-933. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-212 | `session/load` tears down the live session before restoring — **CLOSED 2026-09-24 (pass 2).** BUILT with the recorded eviction delta (`session_shutdown{quit}` instead of `SessionReplaced`, CYRUP-DELTA on `install`). `prepare_load` uses `RestoreGate::rebuild`, so an already-live id still builds exactly once and re-advertises commands; validation precedes teardown (`ACP-226`). Pinned by `prompt_short_circuits_on_live_and_load_does_not`, wire test `tests/wire_gaps.rs` `session_load_on_the_live_id_rebuilds_once_and_evicts_the_outgoing_runtime`. Landed `0aefd08`; manager half `cb290d1`, wire test `8de7460`. Upstream read at `v0.0.33`: src/acp/agent.ts:935-957. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-214 | Replay: user and assistant text — **CLOSED 2026-09-24 (pass 2).** BUILT. `replay_updates` (`sessions.rs`) emits user/assistant text via `joined_text`, nothing for an empty message. Pinned by `replay_emits_user_and_assistant_text_in_order_and_nothing_for_an_empty_message`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:966-997, src/acp/translate/pi-messages.ts:1-17. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | M | 4d | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-216 | Replay: the bash terminal variant — **CLOSED 2026-09-24 (pass 2).** BUILT. Terminal class replays via `terminal_finish` with `terminal_exit.signal: null` and no `terminal_output` for empty output. Pinned by `a_replayed_bash_result_is_a_terminal_and_an_exit`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/agent.ts:1005-1033. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4d | verified |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-228 | An explicit `session_info` clear erases the title in cyrup — **CLOSED 2026-09-24 (pass 2).** BUILT as a recorded delta: `listing::scan_file`'s newest `session_info` wins including a clear, surfaced as `title: null`; `session_info_update(None, ..)` sends `title: null`. Pinned by `clearing_a_session_name_clears_the_title`, `a_rename_sends_a_value_and_only_a_clear_sends_null`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:114-131. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-229 | A relative `sessionDir` anchors to the agent dir upstream — **CLOSED 2026-09-24 (pass 2).** BUILT, decided: a relative sessions root is refused (empty listing, `warn!`), not anchored (`session_dirs`, `sessions.rs`). Pinned by `a_relative_sessions_root_scans_nothing_at_all`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:22-37. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4d | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-230 | Symlink handling is the reverse of the cut's stated rationale — **CLOSED 2026-09-24 (pass 2).** BUILT, decided: symlinked files are listed and resolved (`is_dir` follows links; `find_stored` doc). Pinned by `a_symlinked_session_file_is_listed_and_resolvable`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-sessions.ts:44-59. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). Residual (recorded at `SessionFile::resolve`): containment is lexical, so a symlink inside the root pointing outside passes. | XS | 4d | single-source |
| ~~low~~ **CLOSED 2026-09-24** | ACP-263 | Provenance in the advertised description — **CLOSED 2026-09-24 (pass 2).** BUILT. `describe_fallback` (`commands.rs`) renders `(source:scope)` from `sourceInfo.scope` (CYRUP-DELTA: `location` does not exist), else `(command)`. Pinned by `the_provenance_fallback_has_upstreams_shape`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/pi-commands.ts:11-20. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4e | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-266 | cyrup-acp must not expand prompt templates — **CLOSED 2026-09-24 (pass 2).** BUILT as an invariant: `serve_prompt` submits raw text with `expand_templates: true`, and nothing in the crate looks a template up. Pinned by `nothing_here_expands_a_template`. Landed `0aefd08`. Upstream read at `v0.0.33`: src/acp/session.ts:338-339, src/acp/slash-commands.ts:185-197. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4e | verified |
...
| ~~low~~ **CLOSED 2026-09-24** | ACP-287 | `/follow-up` — **CLOSED 2026-09-24 (pass 2).** BUILT. `queue_mode`/`queue_mode_updates` with `QueueKind::FollowUp` writing `set_follow_up_mode` (`commands.rs`). Pinned by `the_queue_mode_arm_has_three_branches_and_three_strings`. Landed `0aefd08`, `cb290d1`. Upstream read at `v0.0.33`: src/acp/agent.ts:611-656. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | XS | 4e | verified |
| ~~low~~ **CLOSED 2026-09-24** | ACP-289 | `/autocompact` — **CLOSED 2026-09-24 (pass 2).** BUILT. `autocompact_target`/`autocompact_text` (`commands.rs`) with the eight-word table and toggle-on-anything-else preserved. Pinned by `autocompact_accepts_eight_words_and_toggles_on_everything_else`. Landed `0aefd08`, `cb290d1`. Upstream read at `v0.0.33`: src/acp/agent.ts:855-882. Re-read at `ea23ca2` (crate byte-identical since `9aeba769`). | S | 4e | verified |
...
| low | ACP-294 | `promptCapabilities.embeddedContext` has no cyrup env name — **re-read 2026-09-24 (pass 2) — stays OPEN, partial.** The pure predicate half is BUILT (`embedded_context_enabled(raw)` takes the value; the env read is in the `initialize` handler, `connection.rs`). The declaration half is not: `EMBEDDED_CONTEXT_ENV` still lives in `crates/cyrup-acp/src/config_options.rs`, and `grep` over `crates/` finds no `cyrup_config` env-key entry for it. Upstream read at `v0.0.33`: src/acp/agent.ts:260. | XS | 4e | single-source |
...
Both found while re-reading the rows above; both sides personally read. `verified` in the table
means both sides were read by this pass, not that an adversary pass ran.

**ACP-297 — The startup prelude drops upstream's version header and update notice with no `CYRUP-DELTA`.**

**Kind** parity-bug · **Severity** low · **Effort** S · **Confidence** confirmed
**cyrup**    — `crates/cyrup-acp/src/startup.rs` `render_markdown` / `StartupInventory::of` — the prelude is only `## <block>` sections; there is no product/version line and no `---` separator. The module doc's CYRUP-DELTA lists three behaviour changes (names not paths, de-duplication, diagnostic blocks) and neither cut; the update-notice cut is mentioned only in a test doc comment (`quiet_startup_keeps_the_diagnostics_and_drops_the_listing`). `connection::AGENT_VERSION` is available in-crate.
**upstream** — `git -C tmp/pi-acp show v0.0.33:src/acp/agent.ts` lines 1497-1511 open the prelude with `pi v<installed>` then `---` (from `pi --version`), 1636-1640 append `---` plus `buildUpdateNotice()` (1459-1486, `npm view`), and 359-362 keep the update notice even under `quietStartup`.
**Impact** — a Zed user sees no agent version at session start; and because the divergence is unmarked, a later byte-parity audit re-files it or "fixes" it blind. The npm update notice is legitimately cut (no subprocess), but that too is undocumented at the site.
**Fix** — in `render_markdown`, when `show_listing` and at least one section renders, prepend `cyrup v{AGENT_VERSION}` and `---` (upstream's shape); and add a CYRUP-DELTA to `startup.rs`'s module doc recording the update-notice cut and its reason. Either half alone is acceptable if the other is recorded as a decision.
**Verify** — `render_markdown(&fixture())` starts with `cyrup v` followed by `---`; `a_bare_project_produces_no_prelude_at_all` still passes (no header on an empty prelude — `ACP-081`).

**ACP-298 — `session/set_config_option` `model` rejects a bare model id that upstream resolves by catalog lookup.**

**Kind** parity-bug · **Severity** low · **Effort** S · **Confidence** confirmed
**cyrup**    — `crates/cyrup-acp/src/config_options.rs` `SessionConfigKnob::apply`, `Model` arm — the value must equal `model_value_id(provider, id)` for some catalog entry, else `-32602 Unknown modelId: <value>`. Its CYRUP-DELTA justifies exactness against `set_model(pattern)`'s *substring* leniency, and does not mention that upstream's own bare-id form is lost.
**upstream** — `git -C tmp/pi-acp show v0.0.33:src/acp/agent.ts` lines 1404-1434 `setSessionModel`: a value containing `/` splits provider/rest; a value **without** `/` is resolved by exact `m.id` match over `getAvailableModels()` (first hit supplies the provider). The comment names both forms as accepted.
...
> **Status 2026-09-24 (pass 2): every phase below is built** — `AppMode::Acp` and the first branch
> of `resolve_app_mode`, the `require_model` override in `main`'s ACP arm, the turn actor
> (`crates/cyrup-acp/src/turn.rs`), the permission bridge, the session manager and the command
> surface all landed in `0aefd08`…`dd44b3c` (see §6). Kept as the historical plan; **do not use
> "the first three units a porter should pick up" as a worklist** — all three are done.
