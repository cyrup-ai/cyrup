...
> ### RECOUNTED 2026-09-04 (baseline sweep against `4fb5e40..2571969`, plus a v0.84.1→v0.84.4 diff-stat skim of `modes/rpc/` and `modes/json-event.ts` — authoritative over every block below)
>
...
## SEAM-116 — pi's `clear_queue` RPC verb (v0.84.4) has no cyrup counterpart, though the session-layer capability already exists unused
