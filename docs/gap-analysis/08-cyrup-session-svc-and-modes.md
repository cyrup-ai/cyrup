...
| SEAM-015 | medium | not-ported | M | RPC bash ignores the `operations` backend override — **2026-08-14, still open**: sweeps 1, 2 and 6 — Fix CORRECTED: the blocker is NOT `BashOptions`' shape and it is **NOT in cyrup-modes** — a WASM guest has no way to return a callable `operations` backend across the WIT boundary. `rpc.rs:1232` still passes a literal `None`. **NEEDS: a `BashOperations` trait in `crates/cyrup-tools` (area 04) PLUS an extension-capability decision in area 06. Do not schedule this against area 08.** Its sibling DRIFT-004 needs the same trait first. |
...
| SEAM-S03 | medium | not-ported | M | No detached-child registry: `setsid`-detached bash children survive teardown — **2026-08-14, still open**: sweeps 1, 2 and 6 — `crates/cyrup/src/signals.rs:24-64` documents the exact residual and why it cannot land in the bin: pi's two tracking call sites are the spawn and its `finally` **inside the bash tool**, which in cyrup is `cyrup-tools` — a crate the `cyrup` bin depends on, so a registry living in the bin can never be written to. **The drain point already exists; NEEDS the `crates/cyrup-tools` half ONLY (area 04).** |
