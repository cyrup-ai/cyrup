...
> does not explain the repeated bash calls, so the row stays open and stays critical.
>
...
| SEAM-112 | **critical** | port-bug | M | **`/resume` produces a broken session: nothing renders, and bash tool calls repeat endlessly.** Owner report, live use 2026-08-15. **The repetition is the load-bearing clue and must drive the investigation:** a pure RENDER fault would run each command ONCE and fail to display it. Running the same command **over and over** means the model is not receiving its tool results — so this is at minimum a context/session-rebuild fault, possibly two faults sharing one cause in the swap path, not the display bug it resembles. **Wiring verified at HEAD — do not begin by re-deriving it:** `AgentSessionRuntime::switch_session_with` (`crates/cyrup-session-svc/src/runtime.rs:513`) emits `session_before_switch` with reason `"resume"`, builds a fresh session via `factory.build(SessionTarget::Resume(path), cwd)`, and calls `install` → `install_inner` (`:387`/`:398`) which bumps the runtime `generation`; the TUI watches that generation, re-subscribes via `*events = new_session.subscribe()` (`crates/cyrup-tui/src/app/run_arms.rs:158`) and re-binds at `App::rebind_session` (`app/session_bind.rs:4`). The mechanism exists, so the defect is inside it. **CANDIDATE (2) IS NO LONGER A CANDIDATE — a defect of exactly this shape was found and fixed on 2026-08-18 at `879eb4e`, and this row must be re-observed live before any further analysis.** The `session_swapped` arm sat LAST under `biased;` while the events arm bound an IRREFUTABLE `maybe_ev = events.next()`; `Fanout::invalidate` (`cyrup-session-svc/src/subscriber.rs:89-93`) drops every sender the instant a replacement lands, so the disposed session's stream went permanently `Ready(None)`, won every poll, and starved the swap arm — no re-subscribe, no `rebind_session()`, the loop still bound to the OLD session. At HEAD the events arm is refutable (`app/run.rs:344`, `Some(ev) = events.next()`) so a closed stream DISABLES the branch, and the swap arm is hoisted directly below the input arm (`app/run.rs:293`); the rebind is extracted to `App::on_session_swapped` (`app/run_arms.rs:138`) with a generation guard and is ALSO run from the input arm's pre-dispatch reconcile, and `src/tests/run_loop_swap_arm_reachable.rs` pins both structurally. The in-source rationale at `app/run.rs:284-292` names this row's first symptom verbatim — "the TUI up, dead". **That accounts for "nothing renders"; it does NOT obviously account for the repeated bash calls, which is why the row stays open and why the next step is one live `/resume`, not more tracing.** **Candidates, cheapest first, with the one explaining BOTH symptoms first:** (1) the rebuilt session's tool-result path is not re-wired, so results never re-enter the message list; (2) the generation bump fires before the new session is fully installed, so the TUI subscribes to a stream that is then replaced; (3) `rebind_session` resets the transcript but nothing drives the new subscription. **Read these three tests first to see what they assume, since none caught it:** `cyrup-tui/src/tests/runtime_swap.rs`, `cyrup-tui/src/tests/extension_ui_reset_on_swap.rs`, `cyrup-session-svc/src/tests/session_start_lifecycle.rs`. Reproduce by logging at tool-result append and at TUI subscribe/rebind, run ONE resume, read the log — do not characterise by re-running. — **FILED 2026-08-15 (live use)** |
| SEAM-113 | **high** | port-bug | M | **A model chosen with `/model` does not survive into the next session — it reverts to the catalog/settings default.** Owner report, live use 2026-08-15: repeatedly selecting `moonshotai/Kimi-K3` and finding `zai-org/GLM-5.2` on the next session. **Establish which of two contracts is broken before touching anything, because the fixes are in different crates:** (a) `/model` is supposed to PERSIST the choice (upstream writes the selection so a later session resolves to it) and the write is missing or lands in a layer the next session does not read; or (b) `/model` is correctly session-scoped upstream too, and the defect is that the *default* resolution picks `GLM-5.2` rather than the user's last/declared model. Read pi's `/model` handler and its settings write, then cyrup's, and say which contract applies. **Related, and worth checking in the same pass:** `defaultThinkingLevel` had exactly this shape — a settings getter folding "unset" into a type's zero value, so every session silently ran with reasoning off (fixed in `c06bb0c`); a `defaultModel`-shaped equivalent would produce precisely this report. **Also check the `models.json` interaction:** a user-declared model (the reporter's `Kimi-K3` is one) may resolve differently from a built-in at session build, since it enters through a different path — if so the bug is narrower than it looks and belongs with `TUI-089`. — **FILED 2026-08-15 (live use)** |
...
| SEAM-115 | medium | test-defect | S | The three `context_usage` correctness fixes shipped with no test at all — **FILED 2026-08-19, open.** `git show --stat 2086366` touches `crates/cyrup-session-svc/src/session.rs` and one deleted markdown file; `grep -rn 'context_usage' crates/cyrup-session-svc/src/tests/` returns nothing, and the four test files elsewhere that mention `ContextUsage` cover the JSON shape, `from_last_assistant`'s clamping and the intercom projection — **none drives `AgentSession::context_usage` over a branched session.** All three bugs are wrong NUMBERS produced by a correct-looking call, so each regresses silently; the commit's own "305 passed" would not have gone red for any of them. Three cases named in the body, each RED at `2086366~1`. |
...
**Verify** — **NOT DONE — see `SEAM-115`.** `git show --stat 2086366` touches exactly two files, `crates/cyrup-session-svc/src/session.rs` and the deleted task file: **no test was added for any of the three correctness fixes**, and `grep -rn 'context_usage' crates/cyrup-session-svc/src/tests/` returns nothing.

## SEAM-115 — The three `context_usage` correctness fixes shipped with no test at all
