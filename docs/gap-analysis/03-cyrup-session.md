...
| SESS-027 | **closed this pass** | Documentation-only. The three per-role deserializer docs in `cyrup-core/src/message/content.rs` already carried the corrected rationale; the surviving falsehood was the `Content` enum rustdoc at the top of the same file (`:5-17`) — the first doc rustdoc renders — which claimed an off-union block "is REJECTED on deserialize, exactly as Pi's typed unions reject it". It now states the real contract: the deserializers are READ-TOLERANT by design because pi's per-role unions are compile-time TypeScript only and its session read path is `JSON.parse(line) as FileEntry` skipping only MALFORMED JSON (`parseSessionEntryLine`, `core/session-manager.ts:503-511` @v0.83.0), with an explicit "do NOT restore a rejection pi never had" warning naming R-00-013. No code, no serde bytes, no test edited. |
...
| ~~SESS-027~~ | ~~low~~ **CLOSED 2026-08-22** | stale-port | S | Content-deserializer docs promise validation their bodies removed — **CLOSED 2026-08-22**: documentation-only, in `cyrup-core/src/message/content.rs` (this row previously cited the flat `cyrup-core/src/message.rs`, which the content block and its three deserializers have since moved out of). The three per-role deserializer docs already carried the corrected rationale; the surviving falsehood was the `Content` enum rustdoc at the top of the same file (`:5-17`), the first doc rustdoc renders for `Content`. It now states the real contract — the deserializers are READ-TOLERANT by design because pi's unions are compile-time TypeScript only and its read path is a bare `JSON.parse` that skips only malformed JSON (`parseSessionEntryLine`, `core/session-manager.ts:503-511` @v0.83.0) — plus an explicit "do NOT restore a rejection pi never had" warning naming the R-00-013 wire-interop regression it invited. `cargo test -p cyrup-core --lib` still 36 passed; no test edited. It was the SAME mechanism class as SESS-045 — pi's TypeScript field types are compile-time only and its readers re-validate by hand — which is what finally carried it over the bar. |
...
| SESS-S05 | low — **PARTIALLY CLOSED 2026-08-14** | not-ported | S | `TreeNode` drops pi's `labelTimestamp`; the `t` toggle switches an always-empty column of the wrong shape — **PARTIALLY CLOSED 2026-08-14**: sweep 1 — piece (1) of three is done: `cyrup_session::TreeNode.label_timestamp` exists (`manager.rs:39`), `SessionManager::label_timestamp()` is public (`:621-622`) and populated at `:668`; the 19-line comment at `cyrup-tui/src/app/tree_nav.rs:25-43` conceding there is no producer is STALE. **RESIDUAL: pieces (2) the cyrup-tui producer and (3) the inline render + `formatLabelTimestamp` port — area 07.** |
...
`manager.rs:30-40` and carries `pub label_timestamp: Option<String>` at `:39`,
`SessionManager::label_timestamp()` is public at `:621-622`, `labels: HashMap<EntryId, (String,
String)>` is `:51`, and `build_node` populates the field at `:668`. **So the producer this paragraph
blames the data layer for is now a one-line read in THIS crate**, and the comment at
`app/tree_nav.rs:36-43` is the stale half. Render shape also open: `tree_selector.rs:738-752` emits
...
(`manager.rs:39`, populated at `:668`)**;
(2) produce it at `app/tree_nav.rs:44` from `TreeNode::label_timestamp`, which `tree_node_from_dag`
does not currently receive — it reads `SessionDagNode` (`cyrup-session-svc/src/session.rs:147-166`),
whose only `timestamp` is the ENTRY's (`:165`), so the label timestamp has to reach the DAG
projection first; (3) rewrite `tree_selector.rs:738-752` to compose inline after the label rather
than as a right column, and port `formatLabelTimestamp`'s three-branch format from
