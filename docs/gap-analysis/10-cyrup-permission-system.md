...
> ### RE-MEASURE 2026-09-24 — cyrup `ea23ca2`, pi-permission-system **v0.8.0** (re-checked, still the newest tag)
>
> **Pin: cyrup `ea23ca2` (2026-09-23) × `pi-permission-system` v0.8.0 (`9affcc9`). No blind window
> upstream.** `git -C tmp/pi-permission-system tag --sort=-v:refname | head -1` → `v0.8.0`;
> `describe --tags` → `v0.8.0` exactly; `git diff --stat v0.8.0..HEAD` is **empty** (clone HEAD =
> the tag, 2026-07-03). Nothing to census; **zero** `upstream-drift` rows filed, as a measurement.
>
> **cyrup side: `crates/cyrup-permission-system` is byte-identical across `9aeba769..ea23ca2`**
> (`git diff --stat` over the crate is empty; no commit in the 46-commit window names it), and so
> are `crates/cyrup-agent` and `crates/cyrup-provider`, which hold `PERM-032`'s narrowed path and
> `PERM-012`'s closure. The window's churn is in `cyrup-ext-subagents`, `cyrup-herdr`, `cyrup-it`
> — the one seam from there into this area, `PERM-001`'s detached-runner anchor, was spot-read and
> still holds (`crates/cyrup-ext-subagents/src/background/spawn_detached.rs:178` passes
> `parent_anchor::detached_runner_env_overlay()` into the detached `Command`).
>
> **Open set unchanged: one row, `PERM-032` (low, *unclassified — lead*, BLOCKED on a live provider
> call).** Re-read at `ea23ca2`; see the row. Nothing closed, nothing filed, nothing reopened. The
> Coverage section's blind spots (the `formatToolInputForPrompt` structured-edit literals vs
> `gate.rs`, `manager.rs` layer construction) were **not** walked this pass and remain blind.
...
>
> **~~`PERM-008`'s Verify recipe was one-third wrong and the test pins the correction.~~ STRUCK
> 2026-08-14 (sweep 6) — THE REFUTATION WAS ITSELF WRONG.** The claim above was: the recipe asks for a
> `permission_forwarding.warning` on the malformed-request leg, upstream emits none, so the test should
> pin the SILENCE. **The observation about the CALLER is correct and is kept** — pi's
> `if (!request) { safeDeleteFile(...); continue; }` at index.ts:1144-1147 has no log call above it,
> unlike every branch below it. **The conclusion is struck: upstream logs one frame DOWN**, inside
> `readForwardedPermissionRequest` — `:942` (catch → `Failed to read forwarded permission request`)
> and `:928` (field-ladder rejection → `Ignoring invalid forwarded permission request format`). The
> test that resulted had pinned cyrup-invented SILENCE as if it were parity, and is now
> `an_off_session_request_and_a_malformed_one_each_warn`, asserting two warnings in scan order.
> **The lesson generalises and belongs with the citation hazards: a test that pins ABSENCE is only as
> good as the FRAME DEPTH of the citation behind it — absence claims need the callee opened too.**
>
...
> |---|---|
> | `PERM-011` | **Not area-10 work** — as this area's own ROUTING note already states. Both halves land in `crates/cyrup-ext` (area 06): half A needs a **native-extension runtime-API registry** so a second extension can call `yolo_mode`/`set_yolo_mode`/`toggle_yolo_mode`; half B needs an **event accessor on `SharedBus`**. **Neither seam exists**, so this is not "wire it up". |
> | `PERM-012` | Lands **entirely in `crates/cyrup-provider`** (`registerModelOptionCompatibilityGuard` / temperature stripping). Its HEAD citations are already refreshed in the row. |
> | `PERM-022` | **Ownership follows the code: the file MOVED.** It is now `crates/cyrup-it/tests/permission/forwarding_spawn_env.rs` (the 8 000 bound at `:288`, its 20 000 sibling at `:345`), so this is a **cyrup-it** item, not a `cyrup-permission-system` one. |
> | `PERM-032` | **NOT ATTEMPTED, deliberately, for a third sweep.** The item forbids scheduling work before a decisive experiment that requires a **live provider call**, and `TOGETHER_API_KEY`/`TOGETHER_AI_API_KEY` are exported in this environment — spawning the binary risks a real network call. **When it IS scheduled, the request-body diff must be RE-BASELINED against the current block shape: `HookOutcome::Block` gained a `terminate` field (`EXT-049`) after this item was filed.** |

> **SUPERSEDED — RECOUNTED 2026-08-14 (sweeps 3-6 reconciliation) — counted set: 0 critical, 0 high, 2 medium, 2 low = 4.** 18 rows are now marked CLOSED, including the area's critical (`PERM-009`), its high (`PERM-023`) and `PERM-033` (filed and closed by sweep 6). `PERM-017` moved to `## Trackers` and is no longer counted. *(Previous edition: 0 / 0 / 2 / 3 = 5, 16 closed.)*
>
> **The v0.7.1→v0.8.0 delta sweep was RUN IN FULL by sweep 6** — every `src/` file diffed, plus a string-literal census of all 7155 lines of v0.8.0 `src/` against the whole cyrup crate. **The premise that area 10 has ZERO drift items filed against it is stale**: sweeps 4/5 had already absorbed most of it, and sweep 6 verified rather than re-filed. `extension-config.ts`'s three v0.8.0 save behaviours are all present and documented (`ext_config.rs:423-428` corrupt-file refusal with pi's literal message, `:527-529` BOM strip, `:545-556` symlink write-through, mirror tests at `:1199`, `:1228`, `:1303`, `:1323`); `wildcard-matcher.ts`'s 500-unit cap is `PERM-015`; `common.ts`'s `isPrototypePollutionKey` is `PERM-016`; `permission-manager.ts`'s delta is a pure refactor (`findLatestTrustedPatternMatching` extraction) with no behaviour change; `zellij-modal.ts` is covered by `PERM-007`'s `config_modal.rs` port; and `permission-prompts.ts` (new, 329 lines, read in full) is a MOVE out of `index.ts` whose every formatter is in `gate.rs` — including the one genuine divergence, `format_missing_tool_name_reason`'s dropped `from pi.getAllTools()` clause, which already carries an explicit `CYRUP-DELTA` at `gate.rs:665-676`. **`PERM-033` is the only genuine gap the sweep produced.**
>
> **ROUTING — none of the four counted rows is area-10 work.** `PERM-011` needs two seams that do not exist in `crates/cyrup-ext` (a native-extension runtime-API registry so a second extension can call `yolo_mode`/`set_yolo_mode`/`toggle_yolo_mode`, and an event accessor on `SharedBus`); `PERM-012` lands entirely in `crates/cyrup-provider`; `PERM-022` follows its code to `crates/cyrup-it/tests/permission/forwarding_spawn_env.rs`; `PERM-032` is unclassified and blocked on a live-provider experiment.
...
| PERM-032 | low | *unclassified — lead* | M | A permission-**denied** tool result breaks the next provider request on `together/openai/gpt-oss-20b` (3/3), while two other models handle it fine — **new, observed 2026-08-13, low confidence** — **2026-08-15, still open (BLOCKED), but NARROWED offline**: the item's suspicion is "the shape of the message cyrup synthesises to represent a denied tool call". That half can be checked without a network call, and it does not hold. `BeforeOutcome::Block` reaches `cyrup-agent`'s `immediate_error` (`agent.rs:1120-1136`, `:1156-1194`), which builds pi's `createErrorToolResult` verbatim — `content: [text(reason)]`, `details: {}` (the empty-object literal, AGENT-009), `usage: None`, `added_tool_names: []`, `is_error: true` — and a THROWING TOOL takes the sibling arm in `finalize` (`:1234-1240`) producing the **identical field set**. So a denied call and a failed `read` differ on the wire only in the TEXT of one content block, which is exactly the control the item reports as working. The remaining hypotheses are therefore about the CONTENT (the gate's denial sentence) or about Together-side gpt-oss-20b harmony handling — not about a divergent block-path shape. **Still blocked on the live half** (dump the real request body for the turn after a block on that model), for the reason three prior sweeps gave: `TOGETHER_API_KEY`/`TOGETHER_AI_API_KEY` are exported here, so spawning the binary risks a real network call. — **2026-08-14**: sweep 1 + 2 — unclassified, and unchanged for the same reason both passes gave: the item forbids scheduling work before a decisive experiment that requires a live provider call, and TOGETHER_API_KEY/TOGETHER_AI_API_KEY are exported in this environment, so spawning the binary risks a real network call. Note `HookOutcome::Block` gained a `terminate` field (EXT-049) since it was filed, so the request-body diff must be re-baselined against the current block shape. — **2026-09-04 (area-10 pass, cyrup HEAD `2571969`)**: re-verified, still open, no closure. `crates/cyrup-agent/src/agent.rs` no longer exists (split into `crates/cyrup-agent/src/agent/`, the same shape as this directory's documented `app.rs` trap); `immediate_error` is now `crates/cyrup-agent/src/agent/run/tools/preflight.rs::immediate_error`, and its `ToolResultMessage` shape (`content`, `details: Some(empty_details())`, `usage: None`, `added_tool_names: []`, `is_error: true`) is unchanged from the description above — read in full, not assumed. `HookOutcome::Block`'s `terminate` field changed again, from `bool` to `cyrup_core::TerminateHint` (commit `4b7097b`, task CLTR_1); every plain-block call site in `crates/cyrup-permission-system/src/extension/{decide,prompt}.rs` still passes `TerminateHint::Unspecified`, so this is a mechanical retype with no wire-shape change — re-baseline against `TerminateHint`, not `bool`, next time the decisive experiment runs. No commit in `4fb5e40..HEAD` touches Together-specific request-body construction in `crates/cyrup-provider`. This session has no `TOGETHER_API_KEY`/`TOGETHER_AI_API_KEY` exported (unlike the passes that filed and re-confirmed this item), but driving the shipped binary against a live provider is a scope expansion beyond this documentation-only pass and was not attempted — the item stays open on the same missing decisive experiment, not on the specific env-var reason. — **2026-09-14 re-audit at `9aeba769`: CANNOT SETTLE — the blocker is unchanged and it is the LIVE half.** What blocked the reading: the item's own gate ("Do not schedule work from this item until that experiment is run") requires capturing the serialized request body for the turn FOLLOWING a block against a live `together/openai/gpt-oss-20b` call; this session has no `TOGETHER_API_KEY`/`TOGETHER_AI_API_KEY` exported (checked), and driving the shipped binary at a real provider is outside a read-both-sides audit. Severity and kind therefore stay exactly where they are — that is the item's own instruction, not a default. **The OFFLINE half DID move, in three ways.** (1) The SHAPE hypothesis stays refuted after the `agent.rs` → `agent/run/tools/` split, re-read in full rather than carried forward from the 2026-08-15 narration: the block arm (`crates/cyrup-agent/src/agent/run/tools/preflight.rs:107-125`, with `immediate_error` at `:150-180`) and the throwing-tool arm (`.../finalize.rs:21-47`, the `impl From<Result<ToolResult, ToolError>> for Executed` arm, folded at `:108-145`) produce the identical field set — `content: [text]`, `details: Some(...)` (the empty-details literal on the block arm, `e.details` on the throwing arm), `usage: None`, `added_tool_names: []`, `is_error: true`, `timestamp`. (2) The `terminate` re-baseline this file demanded twice is now **DONE**: all eleven production `HookOutcome::Block` sites pass `TerminateHint::Unspecified` (`crates/cyrup-permission-system/src/extension/decide.rs:75,:93,:160,:247,:290,:296,:356,:398,:403` and `extension/prompt.rs:269,:312`), which puts no key on the wire — the `bool` → `TerminateHint` retype (CLTR_1) is confirmed **wire-neutral**. (3) NEW — the CONTENT hypothesis, the only cyrup-side suspect the 2026-08-15 narrowing left standing, is ALSO refuted: `crates/cyrup-permission-system/src/gate.rs:243-251`, `:255-274` and `:278-295` (`hard_stop_hint`, `format_deny_reason`, `format_user_denied_reason`) reproduce `pi-permission-system v0.8.0:src/permission-prompts.ts:48-54`, `:56-78` and `:80-95` — part list, truthiness guards and BOTH hard-stop string literals — verbatim, so the sentence cyrup sends for a denied call is pi's own text and pi on the same model would put the same bytes on the wire. **Every cyrup-side suspect this item named is now dead on inspection; the sole surviving hypothesis is the one the item itself listed as the close-and-record-a-negative-result branch — a Together-side gpt-oss-20b harmony constraint** — and that is a conclusion the offline half cannot reach. Two measurements settle it: **(a)** run the fixture twice on that model, once denied and once with a `read` of a nonexistent path, and dump the serialized request body for the FOLLOWING turn — if the two bodies differ in anything but the text of one content block, the offline refutation above is wrong somewhere and this becomes a `parity-bug`; if they are identical modulo that text, close as a provider constraint and record the negative result. **(b)** Cheaper, and it needs no cyrup at all: send the exact denial sentence — "is not permitted to run 'bash' command '<cmd>'. Hard stop: this permission denial is policy-enforced. Do not retry or investigate bypasses; report the block to the user." — as a tool-result content block to that model through any client; a 400 there proves the failure is Together-side and independent of cyrup's block path, and closes the item without ever building the binary. Re-baseline note for whoever runs it: `HookOutcome::Block.terminate` is `cyrup_core::TerminateHint`, not `bool`, and `Unspecified` emits no key. — **RE-READ 2026-09-24 at `ea23ca2`: still open (BLOCKED), unchanged.** `crates/cyrup-agent` and `crates/cyrup-permission-system` are both unchanged over `9aeba769..ea23ca2`, so the offline narrowing stands; **citation correction:** pi's `createErrorToolResult` port now lives at `crates/cyrup-agent/src/agent/run/tools/preflight.rs:154` (`immediate_error`: `content:[text]`, `details: Some(empty_details())`, `usage: None`, `added_tool_names: []`, `is_error: true`, plus the `terminate` hint), not `agent.rs:1120-1136`. The decisive live-provider experiment was again NOT run (no network calls this pass). |
| ~~PERM-034~~ | ~~**critical**~~ **CLOSED 2026-08-29 — REFUTED** | port-bug | M | **"Allow Always" does not stick — the same tool/command is re-prompted repeatedly within one session.** Owner report from live use 2026-08-15. This makes the permission system unusable: a user who approves an operation permanently is asked again on the next identical call. **NOT the dead-seam class — both halves are wired, so do not re-derive that.** Write: `extension/prompt.rs:301-307` and `extension/decide.rs:384-390` call `SessionApprovalStore::approve_always(&check.tool_name, &subject)` (pi `index.ts:610`). Read: `get_rules()` at `extension/decide.rs:126`, `:296` and `extension/agent_start.rs:198`, funnelled through `gate::apply_pattern_approval_state` (`gate.rs:209`) (pi `index.ts:571`). **— MEASURED 2026-08-29; BOTH ORIGINAL CANDIDATES AND THE LATER "REAL SUSPECT" ARE NOW REFUTED, AND THE ROW IS RELOCATED OUT OF THIS AREA.** Method: instrument the write, the read and both clear sites with the extension INSTANCE identity, drive the gate end-to-end on a real assembled `AgentSession`, read the trace. **(a) SUBJECT MISMATCH — DEAD, empirically, not by reading.** Both write sites and the single read funnel call the same `gate::get_pattern_approval_subject`, so identical calls produce byte-identical subjects by construction; confirmed on three command shapes, each now pinned by a test in `cyrup-it/tests/permission/human_dialog.rs`: the owner's own `rm -rf ./tmp/test` (`perm034_allow_always_sticks_for_the_reported_command`) and a COMPOUND `rm -rf ./tmp/test && echo done` (`perm034_allow_always_sticks_for_a_compound_command`) — the compound case matters because `b3e1a6d` made the bash arm decompose into units via tree-sitter and `pick_most_restrictive` returns ONE unit's result, so the subject is the winning unit rather than the typed string; it still round-trips. Trace: `READ n=0 state=Ask` → `WRITE n=1 [bash::rm -rf ./tmp/test]` → `READ n=1 state=Allow`. **(b) OVER-EAGER CLEARING — REFUTED AS A DIVERGENCE.** `clear()` has exactly two triggers, `SessionStart` and `SessionShutdown` (`extension/native.rs`, both arms, unconditional), and **pi clears on exactly the same two, also unconditionally** (`pi-permission-system` `index.ts:1828-1831` and `:1862-1865` @v0.8.0); on both sides only the `resources_discover` reload spares the store (pi `:1844-1859`), clearing the dedup cache alone. A reload therefore DOES wipe every always-grant — reproduced, and it reproduces the reported symptom exactly, which makes it an attractive and **wrong** explanation. Pinned as upstream behaviour by `perm034_a_reload_wipes_always_grants_exactly_as_upstream_does`; **do not file a port bug against it.** **(c) THE "REAL SUSPECT" FROM `d0fe154` — PER-INSTANCE STORE — IS ALSO REFUTED.** That analysis (unmerged, on `origin/claude/largest-crate-src-files-4wfqz9`) reasoned from `session_approvals: Mutex<SessionApprovalStore>` being a plain `Mutex` rather than an `Arc` that a write could land on one instance and the read run on another. It cannot: `SessionFactory::build_with_parent` (`cyrup-session-svc/src/factory.rs:138`) clones `base_config`, which clones the `Vec<Arc<dyn NativeExtension>>`, so **one instance is shared by every session in the process** — and the instrumented instance id was identical across every write, read and clear in every run. **WHAT IS LEFT, AND IT IS NOT IN THIS AREA.** The handlers are a faithful port; if cyrup re-prompts where pi does not, the divergence is in **WHEN the two dispatch `SessionStart`/`SessionShutdown`**, not in what the handlers do with them. Note `emit_session_start` is latched once per session by `start_announced` (`cyrup-session-svc/src/session/lifecycle.rs:193`) but `ExtensionFacade::reload` (`cyrup-ext/src/facade.rs:2142-2199`) dispatches BOTH events to extensions directly, bypassing that latch — so the next step is to enumerate cyrup's dispatch sites against pi's and find one cyrup fires that pi does not. **CLOSED 2026-08-29 AS REFUTED — NOT AS FIXED. No code changed; the port is faithful and this was never a gap.** Every mechanism that produces the reported symptom was traced to behaviour pi has too, and the one mechanism that would have been a genuine divergence (a per-instance store) is structurally impossible. Closing on an argument plus a measurement of the mechanism, NOT on a reproduction of the original observation — so the closure states its own falsification condition, per this directory's closure rule. **REOPEN THIS ROW IF, AND ONLY IF, someone observes a re-prompt for a byte-identical tool call inside one session with NO `/reload`, no extension reload, and no session switch (`/new`, `/resume`, fork, model-change-induced rebuild) between the two prompts.** That is the one shape none of the evidence here covers, because a reload or a session switch legitimately clears the store on both sides. Capture the sequence of `SessionStart`/`SessionShutdown` dispatches alongside the two prompts — that is the discriminator, and it is cheap: the probe used to close this row was six `eprintln!` calls gated on an env var, reverted in the same change. **CITATIONS REPOINTED** — the originals (`extension.rs:1717`/`:2023`/`:1464`/`:1634`/`:2211`/`:2617`/`:2705`) all died when `4151cd5` split `extension.rs` into `extension/`. **Note the store is intentionally SESSION-ONLY and in-memory on both sides** (`stores.rs:13` records pi's own change to that behaviour), so "does not survive a restart" is CORRECT and is not this bug. — **FILED 2026-08-15 (live use). NARROWED 2026-08-29.** **Renumbered `PERM-033` → `PERM-034` on 2026-08-19 to resolve an id collision** — the struck forwarding-audit row below claimed `PERM-033` first (sweep 6, filed and closed 2026-08-14, commit `e5c6933`) and this row took the same id a day later in `4a05330`, so every mention of `PERM-033` in history at or before 2026-08-18 that says *"Allow Always"* is this row. |
...
| ~~PERM-033~~ | ~~medium~~ **FILED AND CLOSED 2026-08-14** | not-ported | S | Sixteen of upstream's twenty-eight forwarding audit sites are unported, so every filesystem and binding failure on the spool path is silent — **FILED AND CLOSED 2026-08-14**: sweep 6, found by the assigned pi-permission-system v0.7.1→v0.8.0 delta sweep. `PERM-008` landed the `AuditTrail` mechanism and ELEVEN forwarding call sites; upstream has ~28, and sixteen error paths in `crates/cyrup-permission-system/src/forwarding.rs` swallowed their failure silently. **The missing set:** the fs helpers (chmod / mkdir / inspect / rmdir / unlink), BOTH reader diagnostics, and BOTH response-binding rejections. **The last of those is the security-relevant one** — a forged or misaddressed response was discarded leaving only an all-null `response_received` entry, so nothing named it. Ported, each as pi's literal string — **every line number in the run below is pi `index.ts` @v0.8.0 paired with its cyrup landing site at HEAD, spelled out because the clause above anchors on `forwarding.rs` and a bare `:NNNN` would misread as that file**: `set_restrictive_mode` now takes a description and warns `Failed to restrict {description} permissions for '{path}'` (`index.ts:746-752` → `forwarding.rs:229-239`); new `ensure_directory_exists` warns/errors `Failed to create {description} directory '{path}'` (`index.ts:754-763` → `forwarding.rs:254-266`); `ensure_location` rewritten onto it with pi's three literal descriptions AND **pi's non-short-circuiting evaluation** — `index.ts:803-805` are three unconditional bindings ANDed at `index.ts:808`, so a broken spool reports ALL THREE causes, which `Iterator::all` would have reduced to one (hence the fold at `forwarding.rs:284`, inside `ensure_location` at `:277-287`, whose closure puts the `ensure_directory_exists` call on the LEFT of the `&&` so every directory is still attempted); new `try_remove_directory_if_empty` warns `Failed to inspect …` / `Failed to remove empty …` while ignoring ENOENT/ENOTEMPTY (`index.ts:823-849` → `forwarding.rs:532-563`, its two warnings at `:541` and `:558`), driven by its caller `cleanup_location_if_empty` (`index.ts:851-855` → `forwarding.rs:571-587`); new `safe_delete_file` warns `Failed to delete {description} file '{path}'` on non-ENOENT (`index.ts:857-867` → `forwarding.rs:590-600`), now backing both the temp cleanup in `write_json_atomic` (`index.ts:983-987` → `forwarding.rs:418-422`) and the malformed-request delete in `process_forwarded_requests`; new `response_is_bound_logged` raises pi's TWO distinct rejection warnings — `is not bound to request` / `does not match target session` (`index.ts:879-898` → `forwarding.rs:342-374`, the two at `:354` and `:365`). `write_json_atomic`, `ensure_location` and `cleanup_location_if_empty` gained an `Option<&AuditTrail>` (pi's logger is module-scope; cyrup's is an owned handle and the crate's own tests hold none — every production path passes `Some`). `read_request`/`read_response` (`index.ts:906-948`, `index.ts:950-977`) kept their signatures for the out-of-crate caller in `cyrup-it` and gained `_with_audit` twins (`forwarding.rs:496-501`, `:512-517`), both delegating to the shared ladder `read_forwarded_json` (`:440-483`) that raises the two reader diagnostics at `:456` and `:474`. Tests: `crates/cyrup-permission-system/src/tests/forwarding_audit_trail.rs::a_structurally_invalid_request_warns_about_its_format`, `::a_forged_and_a_misaddressed_response_are_each_named_in_the_trail`, plus the corrected `an_off_session_request_and_a_malformed_one_each_warn` (a deliberate RED→GREEN: it asserted ONE entry before the fix and TWO after — **if the gate goes red on that name, the fix did not take, rather than the test being wrong**). |
