...
| TUI-091 | **high** | port-bug | M | **Reasoning blocks never render, though the provider streams them and the setting says show.** Owner report, live use 2026-08-15, on `together`/`Kimi-K3` at `defaultThinkingLevel: high`. **Three layers verified CORRECT at HEAD — start below them, not at them:** (1) the SETTING — `hideThinkingBlock` is absent from the user's `settings.json`, pi's default is `false` (`settings-manager.ts:855`, `hideThinkingBlock ?? false`) and `crates/cyrup-tui/src/app.rs:7842` reads `settings.effective().hide_thinking_block()` faithfully, so the default is SHOW; (2) the PROVIDER — Together emits real reasoning text for this model, confirmed by a live streaming request returning `"delta":{"reasoning_content":"The"}` frames; (3) the DECODER — `first_reasoning_delta` (`crates/cyrup-provider/src/api/openai_completions.rs:2111-2121`) checks `reasoning_content` first and it heads `REASONING_FIELDS` at `:1630`. **So reasoning text is parsed and the view is configured to show it, and it still does not appear.** Candidates, unexamined: the decoded reasoning block is not converted into a `Entry::Thinking` on the transcript (`transcript.rs:682` pushes one with `hidden: self.hide_thinking`), the entry is pushed but never painted, or `ThinkingFormat::Together` routes the block somewhere the renderer does not read. **Check TUI-090 FIRST and rule it out cheaply** — that item has a full screen of whitespace emitted after every turn, so a block that renders correctly could simply be scrolled above the fold; if the reasoning text is present in native scrollback, this row is a duplicate of TUI-090 and should be closed as such rather than fixed. Verify in a REAL terminal: `TestBackend` cannot distinguish "not rendered" from "rendered off-screen", which is the same blind spot TUI-090 names. — **FILED 2026-08-15 (live use)** |
