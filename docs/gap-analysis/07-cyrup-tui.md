...
>   and **F8 (by-value ingest) was never written, although `e6f298d` deleted its task file and its subject says it
>   landed**. That is the finding to carry forward: a deleted task file is not a landed fix, and a commit subject
>   is not evidence. The row is open for F8 plus one post-round-2 live measurement.
...
| TUI-091 | **still open — but with ZERO live hypotheses** | Re-audited 2026-08-19. The row's one remaining candidate ("the accumulated `thinking` buffer is never in the painted region") is REFUTED: `transcript.rs:1272-1298` renders the live block above the answer and `git blame` dates it to 2026-08-02/2026-08-09, before the filing; `cached_render` (`:1228-1249`) is invalidated by `push_thinking_delta`'s generation bump (`:706`); and `cargo test -p cyrup-tui --lib thinking::` is 6/6 green. Next step is a live re-observation, not more tracing. |
| TUI-092 | ~~**critical**~~ **high — still open for F8 only** | De-escalated 2026-08-19: `Ctrl+D` is bound and kills (`keymap.rs:655` → `app/input.rs:126-129` → `app/run_action.rs:16`), `TUI-088` is closed, and the reader-thread escape hatch (`app/input_reader.rs:147-206`) makes a wedge escapable. Round 2 is 7/8 landed; **F8 (by-value ingest) was never written** though `e6f298d` deleted its task file — `app/run_action.rs:281-282` still passes by reference and `ingest_session_event_owned` has zero hits. |
