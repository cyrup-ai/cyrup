...
>   day this was filed) as the leading explanation.
...
| TUI-015 | medium — **PARTIALLY CLOSED 2026-08-19** | cyrup-original | M | ~~No render coalescing — one draw per streaming event~~, no frame budget — **the COALESCING half landed as `TUI-092` F3 (`f22efab`)**: all three producing arms now drain everything already queued and paint ONE frame per wakeup — `app/run_action.rs:217-220` (input), `:267-270` + the single `self.draw_synchronized()?` at `:298` (session events), and `app/run_arms.rs:351-355` (bash) — and the two per-frame costs this item named are memoised as well, the live-region render by F2 (`transcript.rs:1228-1249`, keyed on `render_generation`) and the image raster by F7 (`image.rs:58` + `:204-224`). **What is left is the TIME floor only:** `rg 'MIN_RENDER_INTERVAL|needs_render|request_render' crates/cyrup-tui/src` is still zero, so a stream that wakes the loop once per token still draws once per token. See the item. |
...
## TUI-091 — Reasoning blocks never render — **DOES NOT REPRODUCE HEADLESSLY AT HEAD; RELOCATED OUT OF THE RENDER PATH 2026-08-19**

**Kind** port-bug · **Severity** high · **Effort** M · **Confidence** confirmed (the report), **relocated** (the mechanism) · **filed 2026-08-15 (live use)**
...
**(3) Is the commit path intact, and does anything overwrite it? INTACT, AND NO.** `finalize_assistant_message` (`app/events.rs:178`) commits `thinking_text(&message.content)` **before** the answer; `commit_thinking` (`transcript.rs:724`) pushes `Entry::Thinking`; `entry_lines` renders it (`transcript.rs:2940`); `flush_committed` (`app/draw.rs:128-180`) emits it through `insert_before` (`:178`). The probe saw **exactly one** committed reasoning block, ordered above the answer, present in the reconstructed pty scrollback after the turn — no double-commit from `AgentEnd`'s `commit_thinking(None)` (`app/events_fold.rs:51`), because finalize had already cleared the buffer. `hide_thinking_block` was `false` throughout and is `false` by default (`crates/cyrup-config/src/settings.rs:580`).
