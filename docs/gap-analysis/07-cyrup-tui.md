...
> ## PROVENANCE — RE-MEASURED 2026-09-24 at cyrup `ea23ca2`, pi **v0.87.1**. Supersedes the 2026-09-14 pin block below as the CURRENT pin; that block stays as history.
>
...
> **@v0.84.3**, `status_indicator.rs` at **@v0.84.2**, `clipboard.rs` at **@v0.84.2** for the read side.
> Every item filed 2026-09-24 was classified `upstream-drift` by `git cat-file`/`git show` presence at
> `v0.84.3` (absent) and `v0.87.1` (present), not by date.
>
> **Other upstreams, re-checked 2026-09-24 (none is this area's):** `pi-subagents` v0.71.0,
> `pi-intercom` v0.14.0, `pi-mcp-adapter` v2.37.0, `code_puppy_core_plugins` v0.0.62,
> `pi-acp` v0.0.33 (unchanged), `pi-permission-system` v0.8.0 (unchanged). The UW-7 roster cites
> `pi-subagents/src/tui/fleet-status.ts` @v0.68.0 — that surface is area 09's, not this file's.
...
## Census 2026-09-24 — pi `v0.85.1..v0.87.1` (`packages/tui` + `modes/interactive/`), at cyrup `ea23ca2`

**Filed** (both sides read; rows in `## Open items`, detail sections before `## Coverage`):
`TUI-098` Zed capability detection · `TUI-099` LaTeX join symbols + legacy font switches ·
`TUI-100` skill slash-command ranking on the `skill:` prefix · `TUI-101` CJK-punctuation
autocomplete boundaries · `TUI-102` clipboard reports success after an unverifiable OSC 52 on a
desktop session · `TUI-103` status spinners are a band above the editor, not embedded in its top
...
- **Click toggling of branch-summary / compaction-summary / skill-invocation entries** (CHANGELOG
  ...
  Rides on the component mouse protocol lead below (`handleMouse`), which cyrup's `Component` trait
  does not have. File only together with, or after, that lead.
- **Fullscreen reserves a blank row for a custom footer that renders zero rows** (#8919, CHANGELOG
  [0.86.0]). cyrup's fullscreen footer path was not opened.
- **`/tree` navigation racing an active compaction and replacing its progress UI** (#9179,
  `interactive-mode.ts`). cyrup side not read; `TUI-055`'s compaction indicator is the neighbouring code.
- **`/bug`** (`modes/interactive/bug-report.ts`, net-new 298 lines; CHANGELOG [0.86.0]/[0.86.1]/[0.87.0]).
  Uploads to pi's own Radius service or exports a zip; crash log `~/.pi/agent/crashes.json`. Absent in
  cyrup (`commands.rs` has no `bug`). **A product decision precedes any item** — the upload target is
  pi's infrastructure — so this is a tracker candidate, not a parity bug.
- **New session entry types reaching the UI** — `footer.ts` totals `entry.type === "usage"` entries,
  `tree-selector.ts` hides `usage` and renders `context_edit` (`[context omit|replace: <id>]`). Both
  depend on entry types (cache-warming `usage`, `ContextEditEntry`) whose owner is area 03/08, not
  this file; cyrup side not read.
- **`Cache warming` `/settings` row** (`settings-selector.ts`, `cache-warming-mode`). Settings surface
  is area 05's; the feature behind it is not ported anywhere this pass looked.
- **Async Kitty image conversion replacing newer partial tool-output images** (#8743,
  `components/tool-execution.ts` `convertedImages` keyed by source data) and **fullscreen Kitty images
  erased by later row clears in WezTerm** (#9169, `tui-alt-screen.ts` +44). cyrup's image path
  (`image.rs`, transcript tool blocks) was not re-read for either.
- **LaTeX centred `cases` layouts and vertically laid-out unsupported/nested display scripts**
  (#9564, #7929; `latex.ts` `ScriptNode`, `parseScripts`, `scriptDepth`). Upstream read; cyrup's
  `markdown/latex.rs` script path NOT read, so these stay out of `TUI-099`.
- **`--resume` progressive results** (`session-selector.ts`, +120/−) — skimmed only.

**Read, nothing to file (negative results).**
- `fuzzy.ts` (#9267): the per-character loop became `indexOf(queryChar, lastMatchIndex + 1)`; the
  scoring terms are unchanged and the match is the same greedy first occurrence, so it is a latency
  change with no ranking change. cyrup's `fuzzy.rs` needs nothing.
- `native-platform.ts` / `native/**` (bundled macOS/Windows/X11 clipboard readers, `win32-platform`
  replacing `win32-console-mode`): substrate. cyrup reads the clipboard through `arboard` and has no
  native addon. The *write-side semantics* change that shipped alongside it is `TUI-102`.
- `terminal.ts` (−15): `enableWindowsVTInput` moved behind `getNativePlatformHelper()`; no
  behaviour change.
...
> **RE-VERIFIED 2026-09-24 at cyrup `ea23ca2` against pi v0.87.1 — nothing closed, six filed; authoritative over every block below.**
> `git diff --name-only cc7818b..ea23ca2 -- crates/cyrup-tui` is five files, all UW-7 fleet-roster
> wiring (`app/{terminal_input,run_action,state,extension_ui,mod}.rs`, all in `838eac7`; `dc52eeb`
> touched only `crates/cyrup/src/main.rs`'s `__subagent-inspector` pre-dispatch arm).
> **No open row cites any of them**, and UW-7's change is additive — a `terminal_input` fold that runs
> ahead of `handle_input` and passes events through untouched when nothing subscribes
> (`app/run_action.rs`, `offer_input_to_extensions`) — so it cannot close a row by side effect. The
> spot re-greps were re-run at `ea23ca2` rather than inherited, and each still holds: `TUI-003`
> (`rg 'Session compacted|compaction_count' crates/ -g '*.rs'` → zero), `TUI-011` (stub still at
> `app/submit.rs:133`), `TUI-040` (`rg -i 'write_log' crates/cyrup-tui/src` → zero), `TUI-046`
> (`keyboard_protocol.rs:145` still `DISAMBIGUATE_ESCAPE_CODES` alone), `TUI-047` (`stray_reply.rs`
> has no DCS/APC arm), `TUI-064` (`.attach_image(` → three call sites, all `tests/image.rs`),
> `TUI-071` (`keymap.rs:853-859` still binds `ctrl+v` and `alt+v` unconditionally, `:800` `ctrl+z`
> ungated). Every other open row's cited files are byte-identical to `cc7818b`, where the 2026-09-16
> block below re-ran or carried them. **UW-7 is not a TUI-area parity item** — the roster is
> `pi-subagents`' surface (area 09) and the seam is `EXT-021` (area 06); this file records only that it
> landed in `cyrup-tui` without touching any row here.
>
> **Open set after this pass (hand-counted from the table): 27 rows — 0 critical, 0 high, 1 medium
> (`TUI-102`), 26 low**, i.e. the 21 of 2026-09-16 (including record-only `TUI-095` and the two
> partials `TUI-048`, `TUI-062`) plus `TUI-098`…`TUI-103`. `TUI-102` is the first medium in this area
> since 2026-09-04.
...
> **SHAPE OF THE OPEN SET, 2026-09-16 at `cc7818b` — written for the ranking stage, which has to judge
> 21 rows that are all `low` and therefore cannot be ordered by severity.** This is a characterisation of
> the residue, not a re-rating: no severity cell moved, and nothing below is a new claim about any row.
>
> **The set is 21 open rows + 1 newly filed and already-closed (`TUI-N14`), and it is the largest open set
> in the directory. Every counted row is `low`.** That is not an accident of grading — it is what is LEFT
> after the mediums and highs were worked: `TUI-093` (high) closed 2026-08-17, `TUI-046` was re-rated
> medium → low, `TUI-N11` (medium) closed 2026-09-04, and `#138`/`#140` closed `TUI-073`, `TUI-097` and
> `TUI-060`. **A flat `low` set ranks by shape, and there are five shapes here:**
>
> 1. **Missing instruments (3) — `TUI-040`, plus `CFG-063`'s two, which are area-05 ids with an area-07 fix
>    site.** No `PI_TUI_WRITE_LOG`, no `PI_TUI_DEBUG`, no `PI_DEBUG_REDRAW`. **Schedule these first, and the
>    argument is not their user impact — it is that they are the diagnostic for the other shapes.** The one
>    cyrup-tui defect fixed in this window (`TUI-N14`) was a render-path problem diagnosed with none of
>    them; so was `TUI-093`. They are `S`, they are independent, and they compound.
> 2. **Unbuilt surfaces (6) — `TUI-011`, `TUI-018`, `TUI-064`, `TUI-N06`, `TUI-N07`, `TUI-S02`.** Real
>    absences, each `M`/`L`, none blocking anything else. `TUI-018` is the one with a live inconsistency
>    rather than a plain absence: its closed half advertises a `ctrl+o` affordance its open half has not
>    built.
> 3. **The invented-vocabulary census (4) — `TUI-065`, `TUI-066`, `TUI-071`, `TUI-095`.** Not defects the
>    user hits; they are the record of where cyrup's keybinding/chrome vocabulary diverges from pi's. They
>    are cheap, they are `S`, and they decay fastest — `TUI-066` gained a token in THIS window (`TUI-073`'s
>    `clear` rejection). **A census row that is not re-read is worse than no census row**, so these are
>    either kept current or consciously converted to trackers.
> 4. **Input-decode fidelity (4) — `TUI-045`-dependent `TUI-050`, plus `TUI-047`, `TUI-048`, `TUI-046`.**
>    These share a single structural blocker that is recorded in the source rather than only here:
>    `crates/cyrup-tui/src/escape_reassembly.rs:44-50`'s `[CYRUP-DELTA]` states that crossterm's
>    `parse_event` is `pub(crate)`, `EventSource` is private, and `event::poll` drains the tty into
>    crossterm's own parser — so cyrup has **no raw-byte pre-parse seam**. **Rank these as one item, not
>    four**: any fix is the same seam, and scheduling them separately will produce four blocked passes.
>    `CFG-064` (area 05, cyrup-tui fix site) is a fifth member of this family.
> 5. **Low-confidence live observations (2) — `TUI-056`, `TUI-057`.** Both filed 2026-08-13 from watching
>    the binary, both flagged low-confidence at filing, and **neither has been independently re-checked in
>    any pass since**, including this one. They are the two rows here whose STATUS is least trustworthy in
>    either direction. `REPRO-LOG.md` is the right instrument and neither has an entry.
>
> **Two rows are residuals owned elsewhere and should not be scheduled against a cyrup-tui-only agent:**
> `TUI-062` (the `showWarning` prefix, one of three callers) and `TUI-076`/`TUI-077` (the command-dispatch
> pair). `SESS-S05`'s pieces (2) and (3) are area-03 ids whose fix sites are both in this crate, and its
> real blocker is one rung lower still — `SessionDagNode` in `cyrup-session-svc`.
>
> **What this pass could NOT settle, stated so it is not read as verified.** All 21 were triaged; 6 were
> re-run against code that changed in `9aeba769..cc7818b` and are marked in their own cells; the remaining
> 15 are carried forward by the identity argument above (their cited files are byte-identical to the sha
> the 2026-09-14 pass read them at). **`TUI-056` and `TUI-057` are carried forward by identity and that is
> weaker for them than for the others**, because their claims are about observed runtime behaviour rather
> than about code text, and identical code can still be wrongly described. They need a live run, not a diff.
...
| ~~TUI-096~~ | ~~low~~ **CLOSED 2026-09-05** | parity-bug | S | ~~**A theme that fails to load repaints dark in silence, and the active name keeps the broken theme's name**~~ — filed 2026-09-05 by the batch-3 ledger pass against the code `TUI-004` landed (`1f53b8bd`) — **CLOSED 2026-09-05 at `b897afe8` + `8950604b`.** Both halves of pi's `applyThemeName` failure path (`modes/interactive/theme/theme-controller.ts:126-135` @v0.84.4) are ported. (1) The active name: `App::reapply_theme_from_settings` (`crates/cyrup-tui/src/app/shell.rs`) now calls `ThemeController::fall_back_to_dark` (`crates/cyrup-tui/src/theme.rs`) — pi `this.activeThemeName = result.success ? themeName : "dark"` (`:127`) — so `active_name()`, the theme-file watcher binding and the `/settings` row name the theme that is actually painted. (2) The notice: the verbatim sentence ``Failed to load theme "<name>": <error>\nFell back to dark theme.`` goes to `TranscriptView::push_error`, which is `showError`'s `Spacer(1)` + `Text(theme.fg("error", …), outputPad, 0)` (`interactive-mode.ts:4258-4262`); `showError` is `true` on both `applyFromSettings` branches that name a theme (`:64`, `:70`), i.e. on every path that reaches this seam. `<error>` is pi's own `result.error` for the failure — `loadThemeJson`'s `Theme not found: <name>` (`theme/theme.ts:623`, caught by `setTheme` at `:903-911`). Detecting the failure needed one new seam: `UiTheme::builtin_named` returns `Option`, so a miss is a value rather than `UiTheme::builtin`'s silent dark palette. It is the FALLBACK behind the session's `ResourceRegistry`, not ahead of it — a recorded [CYRUP-DELTA] vs `loadThemeJson`, which checks `builtinThemes` first (`theme.ts:607-610`): every other by-name theme resolution in cyrup goes through `ResourceSet::get_name` (`theme_access.rs:144`, `:164`; `build_theme_watcher`), so putting the built-ins first HERE alone would repaint one theme while the watcher watched another's file (that is what `8950604b` corrects in `b897afe8`). **REVIEW FIX `096a4dcf` (same batch, not a new row):** `8950604b` shipped that reversal with NO test that could tell it from the order it replaced — all five TUI-096 tests passed under both, because `registry(Vec::new())` already seeds the built-ins and the only custom theme any of them registered was `midnight`, which is not a built-in name. `a_discovered_theme_that_shadows_a_builtin_name_beats_the_builtin` now registers a discovered theme literally named `dark` with accent `#ff0000`, reapplies `Some("dark")` and asserts the painted accent is the user's (and that the fixture colour is not the compiled-in dark's, so it cannot pass by accident). Verified red under the replaced order — `left: Some(Rgb(138, 190, 183))`, `right: Some(Rgb(255, 0, 0))`, with the other sixteen tests green under both. The [CYRUP-DELTA] comment at the seam now names that test. The same commit makes `App::reapply_theme_from_settings` `pub`: `ThemeApply` is re-exported `pub` from `lib.rs`, so a `pub(crate)` sole producer left a public type no public API could return. The load result reaches the caller as `ThemeApply` — pi's `ThemeResult` (`:16`) plus a `NoController` arm for the harness `App` upstream cannot have. Tests: 5 added at `b897afe8` in `crates/cyrup-tui/src/tests/theme_reapply_on_reload.rs`, a sixth at `096a4dcf` (17 in the file). Red-before was observed: the probe asserting `active_name() == "dark"` after a `/reload` onto an unresolvable name got `Some("midnight")`. One more delta is recorded at the call site: cyrup has ONE `<error>` string where pi has two, because a malformed theme file is rejected by `cyrup-resources` at discovery (`discovery/scan.rs:551-558` turns it into a `ResourceWarning` the startup diagnostics panel renders) and never reaches the registry, so this seam cannot tell it from a deleted one and says `Theme not found` where pi says `Failed to parse theme <label>: …` (`theme.ts:600-604`) — the user is told either way, once here and once in the panel. **Residual, NOT closed by this item and not a regression of it:** the BOOT path is still silent and still cannot load a file-backed theme — `crates/cyrup/src/interactive.rs:122-123` projects `controller.theme()`, i.e. `UiTheme::builtin(active_name)`, with no `ResourceRegistry` in reach, where pi boots through the same `applyFromSettings` that now reports on reload. |
| TUI-097 | ~~low~~ **CLOSED 2026-09-14** | port-divergence | S | **Ctrl+X and `/copy` are one command, so `/copy` prefers a live fullscreen selection where pi always copies the last assistant message** — `CFG-078`'s residual 2, given the id its own closure asked for, 2026-09-05. **upstream** `modes/interactive/interactive-mode.ts:2910` @v0.84.4 binds `app.message.copy` to `handleCopyCommand({flashConfirmation: true, preferSelection: true})`, and `:6117-6127` prefers the active fullscreen selection ONLY when copy-on-select is off; the `/copy` command passes no `preferSelection` (`:3022`) and always copies the last assistant message. **cyrup** both entry points route to one `AppCommand::Copy` (`crates/cyrup-tui/src/app/input.rs`) whose arm prefers a live selection unconditionally — the ADR-0005 §B-11 decision, taken against v0.84.1 where `handleCopyCommand` had no selection leg at all, and now a divergence rather than a match. **Impact** `/copy` with an active selection copies the selection instead of the last assistant message. **Fix** split the two entry points into distinct commands and give only the keybinding the `preferSelection` leg, gated on `fullscreenCopyOnSelect` being off (which `CFG-078` has now modelled). **Verify** with a live selection and copy-on-select off: Ctrl+X copies the selection, `/copy` copies the last assistant message — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN, and the divergence is WIDER than this row states.** Both entry points still collapse to one command (`Action::MessageCopy => AppAction::Command(AppCommand::Copy)`, `app/input.rs:505`; `"copy" => cmd(C::Copy)`, `app/submit.rs:101`), and the guarded arm `C::Copy if self.altscreen.as_ref().and_then(AltScreen::selection_text).is_some()` (`app/execute_misc.rs:931-950`) carries NO discriminator for which entry point fired and NO copy-on-select gate; the unguarded `C::Copy` at `:953` copies the last assistant message. pi v0.85.1 passes `{flashConfirmation: true, preferSelection: true}` from the keybinding (`interactive-mode.ts:2895-2896`) and NOTHING from `/copy` (`:3007-3008`), and its selection leg additionally requires `!this.ui.getCopyOnSelect()` (`:6138-6141`). So cyrup diverges on TWO axes, not one: `/copy` prefers a selection it should never prefer (the filed claim), AND Ctrl+X prefers a selection even when copy-on-select is ON, which pi refuses — the Fix already anticipates the second axis but the Impact does not state it. **KIND RE-CHECKED and `port-divergence` CONFIRMED, settled by presence not by date:** `preferSelection` is ABSENT at the ported baseline v0.83.0 (`handleCopyCommand(): Promise<void>`, no options, `:5617`) and still absent at v0.84.1 (`:5949`, `{flashConfirmation?}` only), first appearing at v0.84.4 (`:6117-6121`) — but at v0.83.0 BOTH entry points already always copied the last assistant message, so cyrup's unconditional selection preference matched no upstream version at any tag. It is cyrup's own ADR-0005 §B-11 invention. Do NOT reclassify to `upstream-drift`. Only the line pins were stale. — **CLOSED 2026-09-14, and the RE-AUDIT's SECOND axis is closed with it.** **upstream, read at the tag** — the path is `packages/coding-agent/src/modes/interactive/interactive-mode.ts` (not the row's `src/` pin): `handleCopyCommand(options: { flashConfirmation?: boolean; preferSelection?: boolean } = {})` at `:6134-6136`, and the guard at `:6137-6145` is a FOUR-part conjunction — `options.preferSelection && this.ui instanceof TuiAltScreen && !this.ui.getCopyOnSelect() && this.ui.hasActiveSelection()`. `grep -n handleCopyCommand` @v0.85.1 returns exactly three lines: `:2896` (the keybinding, `{flashConfirmation: true, preferSelection: true}`), `:3008` (`/copy`, bare) and the definition — so there is no third entry point the port could have missed. `tui-alt-screen.ts:285-302` confirms `getCopyOnSelect()` is a field read and `hasActiveSelection()` is `getActiveSelectionText() !== undefined`, making cyrup's `!fullscreen_copy_on_select` and `selection_text().is_some()` literal equivalents rather than adjacent behaviour. **cyrup** `crates/cyrup-tui/src/app/action.rs:201` gives `AppCommand::Copy` a `prefer_selection: bool` — pi's options bag, kept as ONE command with a flag because that is the closer port, not split into two as the row's Fix loosely suggested; `app/input.rs:513` passes `true` from `Action::MessageCopy` and `app/submit.rs:105` passes `false` from `"copy"`; `app/execute.rs` routes on `C::Copy { .. }`; `app/execute_misc.rs:36` `prefers_active_selection` reproduces all four upstream clauses and `:988` is the guard arm. The gate is reachable in production, not wired to a constant: `crates/cyrup/src/interactive.rs:238` seeds it from effective settings, `execute_misc.rs:887` rewrites it from `/settings`, `mode_switch.rs:484` pushes it into each alternate screen. `flashConfirmation` was deliberately NOT folded in. **Tests** the new `crates/cyrup-tui/src/tests/copy_entry_points.rs` — `:125` `copy_on_select_off_ctrl_x_copies_the_selection`, `:151` `copy_on_select_off_slash_copy_copies_the_last_message` (the filed defect), `:177` `copy_on_select_on_ctrl_x_copies_the_last_message` (the RE-AUDIT's second axis), `:201` `no_altscreen_ctrl_x_copies_the_last_message`. All four drive the real App/altscreen/mouse/execute path against a faux-provider session with NO assistant message, so pi's empty-`getLastAssistantText()` line distinguishes the two legs on a headless host with no clipboard. Restoring the old one-clause guard fails tests 2 and 3 and leaves the two negative controls green. `tests/command_exec.rs` and `tests/app_global_actions.rs` were updated to assert the entry-point discriminator off a real `submit("/copy")` and a real `ctrl('x')` through `handle_input`, so the discriminator is driven from the entry points rather than asserted at the command. **KIND UNCHANGED** — `port-divergence` is confirmed by the closure: at v0.83.0 `handleCopyCommand()` took no options and BOTH entry points always copied the last assistant message, so cyrup's unconditional selection preference matched no upstream tag. **RE-CLOSED 2026-09-14 — the work is back in the tree, re-verified independently, and the workspace is green.** The reverted batch was re-applied TUI-097-only: every hunk in `0d653d2..baeef5d` was attributed to a row id by the marker the batch wrote into its own code comment, and only this row's were taken. Left behind: the `Action::ThinkingToggle` comment rewrite in `app/input.rs` (TUI-N06, refused as a proven visual no-op — `hide_thinking` is absent from `DocumentKey` at `altscreen/document.rs:141-176`); the `"changelog"` arm in `app/submit.rs` (TUI-011, which depends on a new 711-line `crates/cyrup-tui/src/changelog.rs` that is not this row's); both `push_warning` → `show_warning` hunks in `app/execute_misc.rs` (TUI-062, the `Warning: ` prefix channel); the `changelog_block_mirrors_the_hotkeys_envelope` test (TUI-011); and the `name_command_warnings` / `platform_keybindings` test modules (TUI-062 / TUI-071) — only `mod copy_entry_points;` was added to `tests/mod.rs`. A defect the batch itself carried was fixed on re-application: the four-part-conjunction doc comment had been mis-attached to `has_active_selection_for_test` and ended in a dangling fragment; it now sits on `prefers_active_selection`, the test helper has its own, and the `tui-alt-screen.ts` pins below were added from a personal read. **upstream, re-read at the tag** (`git -C tmp/pi show v0.85.1:packages/coding-agent/src/modes/interactive/interactive-mode.ts`) — `handleCopyCommand(options: { flashConfirmation?: boolean; preferSelection?: boolean } = {})` at `:6134-6136`; the FOUR-part conjunction at `:6137-6145`, counted line by line: `options.preferSelection` (`:6138`), `this.ui instanceof TuiAltScreen` (`:6139`), `!this.ui.getCopyOnSelect()` (`:6140`), `this.ui.hasActiveSelection()` (`:6141`); the last-message leg `const text = this.session.getLastAssistantText(); if (!text) { this.showError("No agent messages to copy yet."); return; }` at `:6147-6151`. `grep -n handleCopyCommand` @v0.85.1 returns exactly three lines — `:2896` (the keybinding, `{flashConfirmation: true, preferSelection: true}`), `:3008` (`/copy`, bare) and `:6134` (the definition) — so there is no third entry point the port could have missed. `packages/tui/src/tui-alt-screen.ts` @v0.85.1 (NOT the `coding-agent` path the reverted closure pinned, which does not exist at this tag): `:285-287` `getCopyOnSelect()` is a plain field read, `:293-295` `hasActiveSelection()` is `getActiveSelectionText() !== undefined`. At the ported baseline, `grep -n handleCopyCommand` @v0.83.0 returns `:2612`, `:2698` and `:5617 private async handleCopyCommand(): Promise<void>` — no options parameter at all, so BOTH entry points always copied the last assistant message and cyrup's unconditional selection preference matched no upstream tag. **KIND UNCHANGED — `port-divergence`.** **cyrup, line pins re-derived from the shipped tree** — the pins in the reverted closure above came from the abandoned snapshot and are several lines off; these are the real ones. `crates/cyrup-tui/src/app/action.rs:199-202` `Copy { prefer_selection: bool }` — pi's options bag kept as ONE command with a flag (the closer port), with `flash_confirmation` deliberately not folded in; `app/input.rs:509` passes `prefer_selection: true` from `Action::MessageCopy`; `app/submit.rs:105` passes `prefer_selection: false` from `"copy"`; `app/execute.rs:52` `\| C::Copy { .. }` routes; `app/execute_misc.rs:28-37` `prefers_active_selection(&self, prefer_selection: bool) -> bool` reproduces all four upstream clauses in upstream order; `app/execute_misc.rs:977` is the guard arm `C::Copy { prefer_selection } if self.prefers_active_selection(prefer_selection) =>`, with the unguarded `C::Copy { .. }` last-message leg following. The gate is reachable in production and was grepped, not assumed: `crates/cyrup/src/interactive.rs:179` seeds it from effective settings at boot (the snapshot's `:238` is stale), `app/execute_misc.rs:889` rewrites it from `/settings` (the snapshot's `:852`/`:887` are stale), `app/mode_switch.rs:484` pushes it into each alternate screen. **Tests** `crates/cyrup-tui/src/tests/copy_entry_points.rs:125` `copy_on_select_off_ctrl_x_copies_the_selection`, `:151` `copy_on_select_off_slash_copy_copies_the_last_message` (the FILED defect), `:177` `copy_on_select_on_ctrl_x_copies_the_last_message` (the RE-AUDIT's second axis), `:201` `no_altscreen_ctrl_x_copies_the_last_message` (the `instanceof TuiAltScreen` clause) — all four drive the real App/altscreen/mouse/execute path against a faux-provider session with NO assistant message, so pi's empty-`getLastAssistantText()` line tells the two legs apart on a headless host with no clipboard. The entry-point discriminators are asserted off the REAL entry points rather than at the command: `tests/app_global_actions.rs:75` (a real `ctrl('x')` through `handle_input` yields `prefer_selection: true`) and `tests/command_exec.rs:99` (a real `submit("/copy")` yields `prefer_selection: false`). **Red-without / green-with, re-run in this tree rather than inherited.** Weakening `prefers_active_selection` back to its pre-TUI-097 one-clause form (dropping `prefer_selection &&` and `!self.fullscreen_copy_on_select &&`, leaving only the selection check) and changing nothing else, `cargo test -p cyrup-tui --lib copy_entry_points` gives `copy_on_select_off_slash_copy_copies_the_last_message ... FAILED`, `copy_on_select_on_ctrl_x_copies_the_last_message ... FAILED`, `no_altscreen_ctrl_x_copies_the_last_message ... ok`, panicking at `copy_entry_points.rs:155:5` with `` `options.preferSelection` is pi's FIRST clause (`:6138`) `` and at `:181:5` with `` `!getCopyOnSelect()` fails, so the conjunction fails ``; `test result: FAILED. 2 passed; 2 failed; 0 ignored; 0 measured; 1430 filtered out`. Exactly the two axes the RE-AUDIT names fail, and both negative controls stay green, so the tests are not vacuous. Restoring the guard: `4 passed; 0 failed`. **Workspace gates, all green at the closing tree**: `cargo fmt --all -- --check` exit 0; `cargo clippy --workspace --all-targets -- -D warnings` exit 0; `cargo nextest run --workspace --features test-fixtures` — `Summary [125.959s] 9926 tests run: 9926 passed, 9 skipped`. |
...
| TUI-046 | ~~medium~~ **low** | parity-bug | M | ~~cyrup pushes Kitty keyboard flag 1, pi pushes 7 — and neither guard flag 7 requires exists, so raising it alone would duplicate characters and leak CSI-u text~~ — **PARTIALLY FIXED 2026-09-04 at `8bb0d22f`.** The three push sites are now one: `crate::keyboard_protocol::push_flags()` writing `DESIRED_FLAGS` = `DISAMBIGUATE_ESCAPE_CODES` alone (`\x1b[>1u`, pi's `\x1b[>7u` minus bits 2 and 4). ~~Alternate keys was the half with a user-visible effect (a shifted key on a non-US layout now resolves to the character that layout produces, `crossterm-0.29.0/src/event/sys/unix/parse.rs:597-606`).~~ **REVERTED 2026-09-05 at `824a539e`** — pushing bit 4 was a REGRESSION on every shift chord; see the detail section. Both `REPORT_EVENT_TYPES` and `REPORT_ALTERNATE_KEYS` are withheld as argued `[CYRUP-DELTA]`s (`keyboard_protocol.rs` module docs). Bit 2 buys cyrup nothing — `map_event_on` discards every `Release` — and both guards it requires filter RAW BYTES, which cyrup's seam is below. `drain.rs:11-16` corrected. **Stays open (low) for one residual only: the `pendingKittyPrintableCodepoint` dedup, unportable while cyrup filters crossterm events rather than bytes** — re-verified 2026-09-05 by pty measurement rather than by reading crossterm, and a third instance of the item's founding drift (the push-site comment still describing a one-bit withheld set) fixed at `836c6006`. — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN at low.** The closing code from the prior pass was audited for what it DOES, not merely that it exists: `DESIRED_FLAGS` is `DISAMBIGUATE_ESCAPE_CODES` alone (`keyboard_protocol.rs:144`, pinned by its own test at `:369`, `assert_eq!(DESIRED_FLAGS.bits(), 0b1)`), `PI_DESIRED_FLAGS` the three-bit union (`:156-159`, test-pinned to 7 at `:393`), and `WITHHELD_FLAGS = PI_DESIRED_FLAGS.difference(DESIRED_FLAGS)` (`:166`) so the delta is a compiler subtraction; `push_flags` (`:178-180`) is the sole writer and all three call sites are `crate::keyboard_protocol::push_flags(&mut out)` (`app/crossterm.rs:63`, `:146`, `:242`) with the surrounding comment naming only `WITHHELD_FLAGS` (`:58`) and no individual bit — **the third-occurrence drift fixed at `836c6006` has not regressed**, and the `:434-438` source-scanning test that enforces it is still present. **The residual that keeps this row open is unchanged on BOTH sides:** pi v0.85.1 still asks for 7 (`packages/tui/src/terminal.ts:15`, `KITTY_KEYBOARD_PROTOCOL_QUERY` at `:17`) and still runs the `pendingKittyPrintableCodepoint` dedup (`stdin-buffer.ts:288`, set at `:406`, consumed at `:401-402`, cleared at `:335`/`:356`/`:369`/`:422`/`:434`); cyrup has no counterpart at all. The blocker is also unchanged: `escape_reassembly.rs:40-48` is the post-crossterm EVENT seam, not the byte seam this needs. — **RESIDUAL: DO NOT IMPLEMENT AT CYRUP'S SEAM; LEFT OPEN 2026-09-14 pending the umbrella scope call.** Everything this row was filed for is closed and was re-verified at the recording HEAD rather than trusted: `DESIRED_FLAGS = KeyboardEnhancementFlags::DISAMBIGUATE_ESCAPE_CODES` alone (`keyboard_protocol.rs:144-145`), `PI_DESIRED_FLAGS` the three-bit union with its `terminal.ts:15` cite (`:156-159`), `WITHHELD_FLAGS = PI_DESIRED_FLAGS.difference(DESIRED_FLAGS)` (`:166`) so the delta is a compiler subtraction, `push_flags` the sole writer (`:178-180`), and all three call sites spelled `crate::keyboard_protocol::push_flags(&mut out)` (`app/crossterm.rs:63`, `:146`, `:242`) with no individual bit restated — the third-occurrence drift fixed at `836c6006` has not regressed and `the_push_site_does_not_restate_the_flag_set_it_pushes` (`:423`) still enforces it. The ONE residual is `pendingKittyPrintableCodepoint` (`stdin-buffer.ts:288`, matcher `:186-192`, consumed and re-armed in `emitDataSequence` `:396-406`, cleared at `:335`/`:356`/`:369`/`:422`/`:434`, contract pinned by `packages/tui/test/stdin-buffer.test.ts:284-303` @v0.85.1), and PORTING IT AT CYRUP'S SEAM IS A MEASURED REGRESSION, not a port: upstream filters RAW BYTES and can tell `\x1b[224u` from a typed `à`, whereas crossterm has already collapsed the two into byte-identical `KeyEvent` values by the time cyrup sees them (measured in a real pty 2026-09-05 and recorded as a `[CYRUP-DELTA]` at `keyboard_protocol.rs:69-96`; the producing equality re-read this pass at `parse.rs:596-616` versus `:118-135`). At the event level pi's guard degenerates to "drop the second of two identical printable presses" and eats the second `l` of `hello`, and over SSH — where two real keystrokes routinely batch into one `read(2)` — no timing or same-burst heuristic rescues it. The two cases pi's guard must NOT fire on, cyrup already gets right with no guard at all. A partial dedup covering only the split-sequence path `escape_reassembly` owns bytes for would be arbitrary coverage rather than a port, and was rejected on that basis. DISPOSITION: strike the residual from this row, close it as ported-with-a-reasoned-delta (the delta is already written and test-pinned in `keyboard_protocol.rs`), and carry the residual on the single umbrella item together with TUI-050 and TUI-045's remainder so one scope decision governs all three. Upstream cites re-pinned to v0.85.1 and confirmed resolving: `terminal.ts:15`/`:17`, `stdin-buffer.ts:186-192`/`:288`/`:396-406`. |
...
| TUI-003 | low | parity-bug | S | Replay omits the compaction-count status — **re-verified 2026-09-04, unchanged: `rg 'Session compacted\|compaction_count' crates/ -g '*.rs'` is still zero hits.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN, claim holds verbatim.** No compaction-count status exists anywhere in the workspace (`git grep -E 'Session compacted\|compaction_count' 9aeba769 -- 'crates/*.rs'` → 0 hits); the swap arm reaches the replay at `app/run_arms.rs:381-387` (`replay_items()` → `replay_items_with_extensions`), then `render_project_trust_warning_if_needed` (`:389-393`) and the re-entrancy guard (`:397`), pushing no status of any kind between them, and the boot path is the same projection. Upstream is unchanged in BEHAVIOUR and only MOVED: `renderInitialMessages()` ends by filtering `getEntries()` for `e.type === "compaction"` and, when non-zero, calls `this.showStatus(...)` — `interactive-mode.ts:3871-3885` @v0.85.1 (this row's `:3706` is the v0.84.x line; it was `:3492` @v0.83.0, so the emit PREDATES the ported baseline and `parity-bug` remains correct, not `upstream-drift`). **One correction to this row's Verify clause:** pi pluralizes (`1 time` / `N times`, `:3882-3883`), so the expected string is `Session compacted 2 times`, not the literal `Session compacted 2`. — **CALL-SITE HALF SKIPPED, HELD OPEN 2026-09-14.** `AgentSession::compaction_count()` has to be added to `crates/cyrup-session-svc/src/session/accessors.rs` — counting `Entry::Known(KnownEntry::Compaction { .. })` over `SessionManager::entries()`, which already excludes the header line and so IS `getEntries()` — and that file is outside the working territory. No existing public API substitutes: `replay_items()` and `raw_context_messages()` are the `buildContextEntries()` branch PROJECTION, so a superseded compaction is gone there, which is exactly what upstream counts; and `session_dag()`'s `SessionDagKind::Compaction` conflates `compaction` with `branch_summary` (`crates/cyrup-session-svc/src/session/transcript.rs:355`, `:368`, `:371`), which upstream excludes. Wiring is one line per call site once the accessor lands. |
...
| TUI-011 | low | not-ported | M | `/changelog` is a hardcoded stub; no "What's New" startup notice — **re-verified 2026-09-04, unchanged: `app/submit.rs:117-118` is still the verbatim `push_block("What's New", "No changelog entries found.")` stub.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN**, now at `app/submit.rs:125-129` (this row's `:111-114`/`:117-118` shifted); the arm is still literally `"changelog" => { self.state.transcript.push_block("What's New", "No changelog entries found."); AppAction::Redraw }`. Both dead-settings halves also hold: `collapseChangelog` is a live `/settings` toggle (`app/settings_rows.rs:181-182`) and `lastChangelogVersion` a live accessor (`cyrup-config/src/settings/effective.rs:705-707`) whose ONLY occurrences in the tree are its own definition — nothing reads either. Upstream at v0.85.1 is unchanged and complete: the parser import (`interactive-mode.ts:103`, `getChangelogPath, getNewEntries, normalizeChangelogLinks, parseChangelog`), `getChangelogForDisplay()` gating on `getLastChangelogVersion()` and persisting `setLastChangelogVersion(VERSION)` (`:1209-1233`), the `collapseChangelog` condensed-vs-full render (`:758-776`), and `/changelog` at `:3022-3023`. **Note for whoever lands this:** `crates/cyrup-tui/src/tests/command_exec.rs:115-141` pins the stub's rendering as if it were correct behaviour — it will need DELETING, not adapting. — **ATTEMPTED AND HELD OPEN 2026-09-14.** `crates/cyrup-tui/src/changelog.rs` is a close, well-cited port of `utils/changelog.ts` (splitLocalTarget / resolveRepositoryPath / isDirectoryTarget / encodeURI / `URL_SCHEME_RE` / `INLINE_MARKDOWN_LINK_RE`, the `parts[i] \|\| 0` degradation, and the reverse-for-`/changelog` versus file-order-for-startup asymmetry all reproduced), `Entry::ChangelogNotice` is correctly justified as distinct from `Entry::Block` and matches `:762-779`, and the three-branch `getChangelogForDisplay` decision including the record-but-show-nothing asymmetry is right. IT DOES NOT CLOSE because the feature is DEAD AT RUNTIME and every user-visible symptom the row lists survives. (1) There is no `CHANGELOG.md` in this repo — `git ls-files` outside `tmp/pi` returns nothing — so `changelog_path()` (`changelog.rs:57-59`, `cyrup_config::asset_dir()/CHANGELOG.md`) resolves to a file that does not exist, `changelog_markdown(&[])` always returns the literal `"No changelog entries found."` (`:369-372`) — byte-identical to the stub it replaced — and the "What's New" notice can never fire on any boot under any setting. (2) The "no packaging blocker" claim is wrong in both directions: pi's published npm package ships `CHANGELOG.md` inside `packages/coding-agent`, while `asset_dir()` (`cyrup-config/src/paths.rs:184-203`) is `CYRUP_ASSET_DIR`, else the exe dir IF it holds `README.md`, else the nearest ancestor with a `Cargo.toml` — for `~/.cargo/bin/cyrup` none of those hit, and there is no `include` rule, build script or release step that puts one next to an installed binary. (3) The Verify's app half is absent: `crates/cyrup/src/interactive.rs:39-81 show_startup_changelog_notice` — the only code reading `last_changelog_version()`/`collapse_changelog()`, gating on `session.messages()` and persisting `lastChangelogVersion` — has ZERO coverage, and `startup_changelog_action`'s `has_messages` argument is hard-coded `false` at the call site, so nothing proves the resume gate, the persist, the scope or the second-boot silence. The stated excuse for dropping the filesystem tests is also false: `parse_changelog_at` takes `Option<&Path>`, so a tempdir test needs no `set_var`. (4) The rewritten envelope test in `tests/command_exec.rs` derives its expectation from the same two calls the arm under test makes, so it is self-fulfilling AND vacuous — it is satisfied by the OLD stub — and it breaks the moment a real CHANGELOG.md exists, because `rows[top+4]` is then a rendered markdown heading. (5) PARSER DIVERGENCE with the doc comment asserting the opposite: upstream matches `/##\s+\[?(\d+)\.(\d+)\.(\d+)\]?/` against the WHOLE line (`changelog.ts:136`), so `## Version 1.2.3` and `## Unreleased (since 1.4.0)` do NOT match and their sections are DROPPED at `:144-148`; cyrup strips `"## "` and runs the pattern unanchored over the remainder (`changelog.rs:120-131`), turning both into real entries, while `changelog.rs:127` states upstream is unanchored — factually wrong. The existing test uses `## Unreleased`, which has no digits, so it cannot catch this. (6) The condensed notice drops pi's bold `/changelog` (`interactive-mode.ts:769`) — `transcript/render.rs:433` renders `Line::raw` while its own comment two lines above claims the bold is there — with no `[CYRUP-DELTA]`, and the test compares `line_text` only. TO CLOSE: add a `CHANGELOG.md` and a packaging story that survives `cargo install`, add the app test the Verify names, anchor the header regex, and either bold `/changelog` or record the loss. **RE-RUN 2026-09-16 at `cc7818b` — still open; ONE citation drifts.** `crates/cyrup-tui/src/app/submit.rs` is one of the 15 cyrup-tui files changed in `9aeba769..cc7818b`, so this row was re-read rather than carried forward. The stub is unchanged in substance and the citation moves: `app/submit.rs:117-118` → **`:133`**, still the verbatim `.push_block("What's New", "No changelog entries found.")`, and still pinned as correct behaviour by `crates/cyrup-tui/src/tests/command_exec.rs:145`. The `#140` edit to `submit.rs` was 7 lines and belongs to the `/copy` selection gate (`TUI-097`), not to `/changelog`. |
| ~~TUI-013~~ | ~~low~~ **CLOSED 2026-08-14** | parity-bug | S | Quoted paths with spaces break `@`-mention autocomplete — **CLOSED 2026-08-14**: sweep 1. |
| TUI-018 | low | not-ported | M | Startup header has no logo/version line and no expanded body — **re-verified 2026-09-04, unchanged: `chrome.rs:126-130` still states outright that no logo is drawn.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: BOTH open halves STILL HOLD; the partial closure (compact_onboarding + `STARTUP_ONBOARDING`) is intact and correct.** (a) LOGO: `compact_hint_entries` (`chrome.rs:169-207`) returns exactly six entries — blank(4), the hint bar, the compact-onboarding line, blank(2), `STARTUP_ONBOARDING`, blank(5) — with no logo entry, and `chrome.rs:135-138` still states in its own words that cyrup does not draw the `logo` part ("so 1 + 4 + 1 = 6"), with `COMPACT_HINT_ROWS: u16 = 6` at `:141` encoding that. The app name and version appear nowhere in the running UI: `CARGO_PKG_VERSION` resolves only at `crates/cyrup/src/main.rs:354`, the `--version` println. (b) EXPANDED BODY: `show_startup_hints` is still a plain bool — initialised true (`app/state.rs:481`), read by the layout/render guards (`app/layout.rs:190`, `app/render.rs:68`), and only ever CLEARED (`app/submit.rs:18`). There is no expand state, so the `Press {ctrl+o} to show full startup help` line cyrup NOW prints (`chrome.rs:107-114`) has nothing to toggle — **the closed half advertises an affordance the open half has not built.** Upstream unchanged: `logo = theme.bold(theme.fg("accent", APP_NAME)) + theme.fg("dim", ` v${this.version}`)` (`:913`), `expandedInstructions` (`:918`), and both ExpandableText bodies (`:955-956`) contain the logo, so its absence is unconditional. — **ATTEMPTED AND HELD OPEN 2026-09-14.** Two of the three moving parts landed and are proven: `chrome.rs` `logo_line` reproduces `interactive-mode.ts:913` @v0.85.1 exactly (bold+accent `APP_NAME` span, DIM — not bold — `" v{version}"` span), `expanded_hints` ports all nineteen `expandedInstructions` rows (`:918-938`) in exact order with exact description strings and the correct `keyHint`/`rawKeyHint` split, and `COMPACT_HINT_ROWS` 6→7 is right (`ExpandableText(…, paddingY = 0)` at `:954-960` framed by `Spacer(1)` either side at `:963-965`, collapsed body 5 parts at `:955`). The drop-rank arithmetic is sound and the tests genuinely fail without the change. IT DOES NOT CLOSE because the expansion STATE is ported at half, and the dropped half is the only one ever true at boot: `getStartupExpansionState()` (`:1298-1300`) is `this.options.verbose \|\| this.toolOutputExpanded`, it is the CONSTRUCTOR SEED (`:957`, not something `setToolsExpanded` drives — that calls `activeHeader.setExpanded(expanded)` directly at `:4213-4228`), and `toolOutputExpanded` is unconditionally `false` at construction (`:434`) — so the entire non-trivial content of that function is the `this.options.verbose` disjunct, which is the term the change drops. `app/layout.rs:191-200` and `app/render.rs:73` pass only `state.transcript.tool_expanded()` while citing `:1298-1300`, even though cyrup already holds the term: `AppState::verbose_startup` (`app/state.rs:229-235`), `set_verbose_startup` (`app/session_bind.rs:584`), armed from the real `--verbose` flag at `crates/cyrup/src/interactive.rs:365`. OBSERVABLE: `pi --verbose` boots with the startup header ALREADY expanded; `cyrup --verbose` boots with the five-item compact bar, and no test exercises `expanded == true` at boot. The same seed feeds the loaded-resources sections at `:1673`, so this is not a one-off. TO CLOSE: pass `state.verbose_startup \|\| state.transcript.tool_expanded()` at both sites per `:1299`, with a test that `App` + `set_verbose_startup(true)` boots showing "to queue follow-up" with no key press; if cyrup's live-per-frame read is meant to differ from pi's construct-once seed, that needs a `[CYRUP-DELTA]` naming `:957` and `:4217-4220` rather than silence. TWO PRE-EXISTING OBSERVATIONS THAT BLUNT THIS ROW'S STATED IMPACT, recorded so the next pass does not re-derive them: (a) upstream's `builtInHeader` is added to `headerContainer` at `:963-965` and never removed (only replaced by `customHeader` at `:2354-2375`), so the logo persists into scrollback, whereas cyrup's block lives only in the bottom-pinned live viewport (`app/render.rs:70-97`, never `insert_before`) and `app/submit.rs:16-19` clears `show_startup_hints` on the first non-empty submission — the comment there asserts "Pi drops compactInstructions once the conversation begins" and no upstream code doing that was found; (b) upstream gates the WHOLE header, logo included, behind `if (this.options.verbose \|\| !this.settingsManager.getQuietStartup())` (`:912`, substituting `new Text("", 0, 0)` at `:968`), and cyrup models `quietStartup` (`cyrup-config/src/settings/effective.rs:387`) but does not apply it here — so a `quietStartup: true` cyrup now prints an app name and version pi suppresses. NON-FINDING: threading `&EditorKeymap` instead of `Keymap` is CORRECT, because `keyHint("tui.editor.deleteToLineEnd", …)` at `:923` resolves through the editor keymap. |
...
| TUI-040 | low | not-ported | S | No `PI_TUI_WRITE_LOG` equivalent — no escape-sequence write log — **re-verified 2026-09-04, unchanged: `rg 'WRITE_LOG\|write_log' crates/cyrup-tui/src` is zero.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN at low; claim holds on both sides, but the UPSTREAM PIN IS DEAD and must be re-pointed.** cyrup: `git grep -iE 'WRITE_LOG\|write_log' 9aeba769 -- crates/` is zero workspace-wide — no tee, no alias, nothing. Upstream the feature is alive but has MOVED OUT of `tui.ts`: `packages/tui/src/terminal.ts:152-165` is the `writeLogPath` IIFE reading `process.env.PI_TUI_WRITE_LOG`, testing `statSync(env).isDirectory()` and deriving `tui-${ts}-${process.pid}.log`, and `:490-492` is `fs.appendFileSync(this.writeLogPath, data, { encoding: "utf8" })` on the write path; documented at `packages/tui/README.md:899-902`. **This row's `tui.ts:1375-1384` / `:1723` resolve to nothing at v0.85.1** — a reader who checks only those will conclude the feature was REMOVED upstream and close this by mistake. Severity low is right; the argument for its outsized usefulness is if anything stronger now, since TUI-046's residual is the one item this pass could settle only by a pty harness and still owes a live kitty/ghostty run. — **PARTIAL, HELD OPEN 2026-09-14 for a territory reason, not a technical one.** The write-log funnel landed and carries mode changes, the Kitty push, the OSC-0 title, the drain-time disables and the whole panic/exit restore sequence, but NINE of the fifteen writing sites are in files owned by sibling agents and were left alone: `terminal_query.rs:484`, `terminal_progress.rs:135`, `clipboard.rs:366`, `altscreen/out.rs:50` and `:57`, `startup_selector.rs:46` and `:72`, `app/backend.rs:219` and `:444`. (The two `is_tty()` probes at `drain.rs:163` and `terminal_query.rs:470` are correctly untouched.) The one site inside the working territory that is still off the funnel is `app/crossterm.rs:92`, the `InlineBackend` construction: `InlineBackend<W>` is generic in its writer, so moving it onto `write_log::TuiStdout` also rewrites `impl App<InlineBackend<Stdout>>` in `app/backend.rs:213`, `app/run.rs:29`, `app/run_arms.rs:13` and `app/run_action.rs:6` — four sibling-owned files. It is marked `WRITE-LOG-PENDING` at the call site and the ratchet test admits that one marker deliberately. CONSEQUENCE: frame CONTENT is not teed yet, and the row's LIVE Verify (one session, one log file containing the OSC-0 title, the keyboard push and the drain-time disables in order) is unmet because the `CSI ? u` negotiation QUERY write lives in `terminal_query.rs:484`. Two `[CYRUP-DELTA]`s are recorded in the module doc and should be read before the follow-up: (a) the variable is `CYRUP_TUI_WRITE_LOG` with NO `PI_TUI_WRITE_LOG` alias, because `cyrup-config/src/env.rs:25` documents a HARD rename (R-07-028) and an alias would reintroduce the last `PI_` name in the workspace — the row's own Fix text asked for that alias and is wrong; (b) the directory stamp is UTC, because std has no local-time source and `time` 0.3 refuses `OffsetDateTime::now_local()` in a multi-threaded process. Also owed: the crate README line mirroring `packages/tui/README.md:899-902` (no `crates/cyrup-tui/README.md` exists, so the module doc carries it for now). |
...
| TUI-047 | low | not-ported | M | A late or unsolicited DCS/APC frame is shredded into ~20 typed characters — `stray_reply.rs` recognises only OSC 11 — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN at low.** `stray_reply.rs` still fixes the tail to `const TAIL: [char; 3] = ['1', '1', ';']` (`:88`) over the five-state machine at `:73-85` (Idle/Esc/Tail/Payload/PayloadEsc); there is no DCS or APC arm. **A new module landed nearby WITHOUT touching this — exactly the case where a closure could be wrongly inferred, so it was checked:** `escape_reassembly.rs` (TUI-045's closure) does NOT generalise the machine and says so in its own scope section at `:57-63` — only `ESC [` (CSI, incl. bracketed paste) and `ESC O` (SS3) are claimed, while `ESC ]` (OSC), `ESC P` (DCS) and `ESC _` (APC) are replayed untouched, with `TUI-047` named as the generalisation. So a DCS/APC frame still reaches crossterm's Alt-fallback and is shredded. Upstream unchanged: `stdin-buffer.ts:57-64` classifies both families as first-class sequence types and `:149-179` holds `isCompleteDcsSequence` / `isCompleteApcSequence`, so pi emits one `data` sequence where cyrup emits ~20 key events. Low remains right — reachability is still narrow (tmux passthrough; cyrup's own kitty writes set `q=2`). — **ATTEMPTED AND HELD OPEN 2026-09-14.** The mechanism is right and the new tests are genuinely RED (no `P`/`_` arm exists at HEAD, so Alt+Shift+P takes `Step::Forward` and `out.is_empty()` fails), but the closure is partial on four counts. (1) The Verify names `every_event_of_a_non_matching_burst_is_accounted_for` (`stray_reply.rs:908`) as the proof the wider machine cannot eat a real key, and its alphabet was left unchanged — it contains no Alt+Shift+P, Alt+`_`, bare `_`, P+SHIFT, Alt+`\` or `\`, so not one of its 512 bursts can reach `State::StringPayload`; it is vacuous for the new arms, which are the only holds in the module with no discriminating tail and an alphabet of essentially all ASCII against OSC 11's 22 characters. (2) THREE NARROWINGS AGAINST UPSTREAM CARRY NO `[CYRUP-DELTA]`: cyrup's hold is bounded by `HELD_FLUSH_INTERVAL = 20ms` (`app/input_reader.rs:25`) and `MAX_HELD = 48`, where pi has neither — an incomplete DCS parks behind a 50ms `setTimeout` and `flush()` (`stdin-buffer.ts:388-397`, `:412-421`) emits the PARTIAL buffer as one ESC-leading string that dies on `editor.ts:980`'s `charCodeAt(0) >= 32` guard, so upstream inserts nothing while cyrup replays ~12 typed characters; a DCS split across reads more than 20ms apart is therefore still shredded, which is exactly the tmux path this row names as THE reachable source (`REPRO-LOG.md:1306` measured a 60ms inter-read gap); and `step_string_payload` replays, and so shreds, any frame whose payload holds TAB/CR/C0/DEL/non-ASCII, where pi constrains DCS/APC payload content not at all. (3) The Verify's MANDATORY live tmux run was not done — there is no TUI-047 entry in `REPRO-LOG.md`. (4) Callers and record left stale: `app/input_reader.rs:343-345` still documents the filter as removing only "a complete, terminated OSC 11 frame" and that file is not in the change set, `stray_reply.rs:1` still opens "Swallow a late terminal OSC 11 reply", `tests/input_pipeline.rs` gained no DCS/APC case although the split-opener claim depends on reassembler behaviour the unit test bypasses, and this row's body at `:2029` carries no FIXED banner. |
| TUI-048 | low — **partially fixed 2026-08-13** | parity-bug | M | Word navigation classifies by character class instead of Unicode word segmentation — CJK word motion jumps whole runs. Class-run motion replaced by UAX#29 + pi's punctuation sub-boundaries; the ICU dictionary pass for unspaced scripts remains — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: the partial closure is VERIFIED AS REAL and the open residual is UNCHANGED.** cyrup has the three-branch pi shape ported into `crates/cyrup-tui/src/editor/word_nav.rs` (`:42-45` backward takes the LAST `PUNCTUATION_REGEX` match, `:95` forward the FIRST, `:138` the regex ported as a `matches!` set, `:190-191` `byte_word_segments` over `UnicodeSegmentation`), reached from `InputEditor::word_segments` (`editor/motion.rs:87`) which `word_left_target`/`word_right_target` consume at `:158` and `:195`; the old `is_word_char` character-class run is gone. The residual is still exactly true and self-documented: the `[CYRUP-DELTA]` at `editor/motion.rs:81-86` says the base segmenter is UAX#29 and that parity needs an ICU-class word segmenter (`icu_segmenter` + its CJK/Thai data), a new workspace dependency — and `crates/cyrup-tui/Cargo.toml:121` still lists only `unicode-segmentation`. Upstream unchanged: `packages/tui/src/word-navigation.ts:1-3` still imports `getWordSegmenter` (an `Intl.Segmenter`, i.e. ICU) and branches on `isWordLike` at `:47`/`:100` with `PUNCTUATION_REGEX` sub-boundaries at `:50`/`:102`. The `你好世界` column-2-vs-3 divergence stands. — **NEEDS A DECISION, NOT AN IMPLEMENTATION; LEFT OPEN 2026-09-14.** The question for the workspace owner is narrower than this row states and is NOT "take a new workspace dependency": the ICU4X 2.x family (`icu_normalizer` 2.2.0, `icu_properties` 2.2.0, `icu_provider` 2.3.1, `icu_collections`, `icu_locale_core`) is ALREADY in `Cargo.lock` via `idna_adapter`, so `icu_segmenter = "2"` is version-compatible and adds only `icu_segmenter` + `icu_segmenter_data`. The real question is whether we accept the multi-MB compiled CJ-dictionary / LSTM blob in `icu_segmenter_data` in EVERY shipped cyrup binary in order to move the Alt+B / Alt+F / Ctrl+W caret one ideograph in unspaced scripts. Severity is low and there is no data-loss path, and there is no cheaper option — any provider configuration that drops the dictionary degrades to UAX#29, which is what HEAD already does — so it is strictly binary. IF APPROVED the change is mechanical: add `icu_segmenter = { version = "2" }` to `[workspace.dependencies]` beside `unicode-segmentation` (`Cargo.toml:191`) and `icu_segmenter = { workspace = true }` beside `crates/cyrup-tui/Cargo.toml:121`; in `crates/cyrup-tui/src/editor/motion.rs:87-123` `word_segments`, replace the `joined.split_word_bounds()` loop with a walk of `WordSegmenter::new_auto(..)`'s boundary iterator over `&joined` (`new_auto` is the dictionary-for-CJ + LSTM-for-Thai/Lao/Khmer/Burmese mode, the closest match to V8's `Intl.Segmenter`), taking `word_like` from `iter.word_type().is_word_like()` — pi's `isWordLike` bit rather than a recompute — leaving the marker-merge arms at `:97-120` untouched; make the same two swaps in `crates/cyrup-tui/src/editor/word_nav.rs:190-200` `byte_word_segments` so the single-line `text_input::Input` search field does not diverge; hold the segmenter in a module-static `std::sync::OnceLock` via `get_or_init` (construction loads compiled data and must not run per keystroke; no `unsafe`, no `unwrap`); and delete the now-false `[CYRUP-DELTA]`s at `motion.rs:81-86` and `word_nav.rs:23-27`. IF DECLINED the row closes as an accepted, documented divergence and nothing in the code changes. Upstream, read at the tag: `packages/tui/src/word-navigation.ts:1-3` @v0.85.1 imports `getWordSegmenter` from `./utils.ts`, whose `:5` is `new Intl.Segmenter(undefined, { granularity: "word" })`, and both walks branch on ICU's `isWordLike` directly (`:47`, `:100`) before sub-dividing with `PUNCTUATION_REGEX`; `word-navigation.ts` is unchanged v0.83.0 → v0.85.1. The divergence is confined to the dictionary pass — ICU segments `你好世界` as `你好`/`世界`, so pi's Alt+B from column 4 lands at 2 where cyrup lands at 3. The shared walks `find_word_backward`/`find_word_forward` (`word_nav.rs:47-135`) are already faithful and are NOT what diverges. |
...
| TUI-050 | low | not-ported | S | An 8-bit meta byte is silently dropped instead of being converted to `ESC` + char (depends on TUI-045) — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN — but this row's stated DEPENDENCY is now MISLEADING and should be reworded.** cyrup's reader is `event::poll(wait)` / `event::read()` (`app/input_reader.rs:373-374`) delivering `crossterm::event::Event` values; nothing on the path inspects a raw byte, so a lone `0xE1` is consumed inside crossterm's parser exactly as described. Upstream unchanged: `stdin-buffer.ts:303-308` @v0.85.1 still carries the high-byte conversion ("If buffer has single byte > 127, convert to ESC + (byte - 128)"). **THE CORRECTION:** this row says "dependent on TUI-045 … if that pre-parser lands, port `stdin-buffer.ts:294-306` into it". TUI-045 IS closed, but it landed as `escape_reassembly.rs`, whose own `# Why this operates on events rather than on bytes` section states that `parse_event` is `pub(crate)`, `EventSource` is private, `event::poll` drains the tty into crossterm's parser, and therefore *"there is no point at which cyrup can hand bytes to crossterm"* (`:40-48`) — the reassembler runs AFTER crossterm, on already-shredded events. The seam this row waits on was NOT delivered by the thing it names as its blocker, so a reader checking "is TUI-045 done?" will wrongly conclude this is now schedulable. Reword the dependency to name the unbuilt BYTE-LEVEL pre-parser directly. — **DO NOT IMPLEMENT AS FILED; LEFT OPEN 2026-09-14 with the reasoning recorded so the next pass does not re-derive it.** This row's premise — "port it into TUI-045's pre-parser" — is FALSE at HEAD. TUI-045 is closed, but its closure landed as `escape_reassembly.rs`, an EVENT-level repair that runs AFTER crossterm, and that module says so itself at `:40-48`: `parse_event` is `pub(crate)` (`parse.rs:26`), `EventSource` is private, and `event::poll` drains the tty into crossterm's own parser, so "there is no point at which cyrup can hand bytes to crossterm". Here crossterm emits NO EVENT AT ALL, and absence of events is not observable at the event level; a side-channel raw read of fd 0 is not available either, because `event::poll`'s reader thread already owns that fd. The only thing that closes this row is the byte-level reader described in the cluster note, which does not exist and is a workspace-owner scope call. ACTIONABLE NOW, DOCS ONLY: reword this row's Fix, which currently reads "dependent on TUI-045 … if that pre-parser lands, port `stdin-buffer.ts:294-306` into it" — a reader who checks "is TUI-045 done?" gets yes and wrongly concludes this is schedulable. Re-point it at the UNBUILT byte-level reader by name, re-pin the upstream cite to `stdin-buffer.ts:301-312` @v0.85.1 (verified present and unchanged: the single-byte `> 127` conversion at the head of `process()`, which keys on the raw one-byte CHUNK — a property of the read, with no event-level shadow), and fold the row into the umbrella with TUI-046 and TUI-045's remainder. Also record that the real impact is one notch WIDER than filed: crossterm 0.29's `parse_utf8_char` (`parse.rs:825-862`) returns `Ok(None)` for `[0xE1]`, `Parser::advance` keeps the buffer (`tty.rs:253-256`), and the next ordinary byte fails the continuation check and clears it (`tty.rs:261-265`) — so the 8-bit meta byte is dropped AND so is the keystroke that follows it. That does not change severity (`metaSendsEscape` has defaulted on for twenty years) but it is the honest statement. |
| ~~TUI-060~~ | ~~low~~ **CLOSED 2026-09-14** | parity-bug | M | The wrap / visual-line map is not paste-marker aware, so a marker can be torn across visual rows — pi passes a marker-merged `preSegmented` to `wordWrapLine` — **new, found 2026-08-13** — **re-verified 2026-09-04, unchanged: `editor/wrap.rs`'s `word_wrap_line` still takes a plain `&[char]`; a nearby CYRUP-DELTA note covers CJK/emoji grapheme width only, not paste-marker atomicity across a wrap boundary.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN, and cyrup's own source states the defect.** `word_wrap_line(line: &[char], width: usize) -> Vec<(usize, usize)>` (`editor/wrap.rs:513`) still takes a plain char slice; its only production caller is the visual-line map builder at `:34`. Its `[CYRUP-DELTA]` at `:506-512` says outright that upstream's recursive over-wide-segment branch exists *"because its segmenter merges a whole `[paste #N …]` marker into one atomic segment. cyrup's segments are plain extended grapheme clusters — never composite"* — i.e. the wrap map has no marker atomicity, precisely this row's claim. **Do NOT mistake `marker_grapheme_boundaries` (`:93-101`, `:149`) for a fix: it is on the CURSOR-snap path only and `word_wrap_line` never consults it.** Upstream confirms the counterpart: `wordWrapLine(line, maxWidth, preSegmented?)` (`packages/tui/src/components/editor.ts:117-121`) uses `preSegmented ?? [...graphemeSegmenter.segment(line)]` at `:132`, and both real callers pass `[...this.segment(line, "grapheme")]` (`:1020`, `:1837`) where `Editor.segment` is `segmentWithMarkers(text, graphemeSegmenter, this.validPasteIds())` (`:376-379`) — marker-merged by construction. — **CLOSED 2026-09-14.** **upstream, read at the tag** — `git -C tmp/pi show v0.85.1:packages/tui/src/components/editor.ts`: `wordWrapLine(line, maxWidth, preSegmented?)` at `:121`, `preSegmented ?? [...graphemeSegmenter.segment(line)]` at `:132`, `isWs = !isPasteMarker(grapheme) && isWhitespaceChar(grapheme)` at `:147`, the two break-opportunity arms at `:196` (`isPasteMarker(next.segment) \|\|`) and `:199-206` (`!isPasteMarker` gating each CJK test), and the over-wide recursion at `:170-186`. BOTH real callers pass the marker-merged list — `layoutText` `:1020` and `buildVisualLineMap` `:1837`. `v0.83.0:114-206` was diffed against `v0.85.1:121-213` and is byte-identical, so this is a port omission at cyrup's own ported baseline, not tag drift. **cyrup**, all in `crates/cyrup-tui/src/editor/wrap.rs`: `WrapSeg<'a> { start, text, atomic }` at `:563-573` (pi's `Intl.SegmentData` plus the `isPasteMarker` bit); `grapheme_wrap_segments` at `:577-590` (the `:132` default, `atomic: false` throughout); `word_wrap_line(line, width, pre_segmented: Option<&[WrapSeg<'_>]>)` at `:636-640`; `InputEditor::wrap_segments` at `:103-145`, the grapheme twin of `word_segments`, merging a registered paste marker into ONE segment off `paste::marker_spans` — with an equivalence note explaining why cyrup's validIds-gated `atomic` coincides with pi's syntactic `isPasteMarker` re-test. Both callers were changed together — `visual_line_map` at `:39` and `visual_line_count` at `:75` each build `joined` + `wrap_segments` and pass `Some(&segs)` — so the box is SIZED from the same segmentation it is DRAWN from. The three marker guards are `:679` `let is_ws = !seg.atomic && is_whitespace_seg(seg.text);` and the opportunity block at `:755-771`, restored to upstream's TWO arms rather than the merged predicate clippy's `if_same_then_else` used to force. The over-wide branch at `:705-737` recurses `word_wrap_line(sub, width, None)` for a composite segment; the single-cluster terminating branch is KEPT and its `[CYRUP-DELTA]` at `:628` rewritten to say exactly what remains delta and why — `wordWrapLine("👨", 1)` re-enters upstream with the same input and recurses forever, verified empirically against pi's own tagged source. **Tests** `crates/cyrup-tui/src/editor/tests/wrap.rs:202` `a_paste_marker_is_never_torn_across_visual_lines` (RED at HEAD: `map[1].start` was 22, not 7), `:224` `an_over_wide_paste_marker_is_re_wrapped_not_dropped` (rows tile the line contiguously and every row fits; RED with the `seg.atomic` recursion disabled AND red with only `visual_line_count` left on plain graphemes, which is what pins the second caller), `:260` `the_marker_guards_decide_where_the_break_lands` (each guard verified RED individually, and a third assert shows an UNREGISTERED marker falls back to plain-grapheme wrapping, pi's validIds gate). A differential harness built from pi's verbatim tagged source agreed byte-for-byte over 13,748 ASCII and CJK/Hiragana/Hangul cases. |
...
| ~~TUI-N05~~ | ~~low~~ **CLOSED 2026-09-14** | parity-bug | S | Extension shortcuts can never override a built-in key; no conflict reported — **not independently re-checked this pass; left open.** — **CLOSED 2026-09-14 with NO CODE CHANGE: superseded by EXT-039, which landed BOTH halves.** **upstream, read at the tag** — `git -C tmp/pi show v0.85.1:packages/coding-agent/src/core/extensions/runner.ts`: `RESERVED_KEYBINDINGS_FOR_EXTENSION_CONFLICTS` at `:72-89`, eighteen ids — `app.interrupt`, `app.clear`, `app.exit`, `app.suspend`, `app.thinking.cycle`, `app.model.cycleForward`, `app.model.cycleBackward`, `app.model.select`, `app.tools.expand`, `app.thinking.toggle`, `app.editor.external`, `app.message.copy`, `app.message.followUp`, `tui.input.submit`, `tui.select.confirm`, `tui.select.cancel`, `tui.input.copy`, `tui.editor.deleteToLineEnd`; `buildBuiltinKeybindings` at `:95-116` with the reserved-wins tie-break at `:104-106` (`if (existing?.restrictOverride && !restrictOverride) continue;`); and the dispatch precedence at `modes/interactive/components/custom-editor.ts:31-34` — "Check extension-registered shortcuts first". **cyrup** (1) PRECEDENCE INVERTED — `crates/cyrup-tui/src/app/input.rs:135-142` resolves `state.extension_shortcuts` and returns `AppAction::ExtensionShortcut(spec.id)` BEFORE `self.state.keymap.action_for(key)` at `:152`, with a 25-line comment at `:110-134` citing pi's dispatch site and naming EXT-039 as the row that inverted it. (2) THE GATE — `crates/cyrup-ext/src/registry.rs:1377-1396` is the 18-id const, compared entry-by-entry against pi's list this pass: identical strings, identical order. `build_builtin_keybindings` (`:1413-1436`) implements the reserved-wins tie-break at `:1419-1425`; `resolve_shortcuts_inner` (`:1031-1092`) implements pi's rules 2/3/4 with pi's warning strings verbatim — reserved skips, a NON-reserved built-in is WON by the extension with a warning naming both, ext-vs-ext lets the last win. (3) THE DIAGNOSTIC CHANNEL — `shortcut_diagnostics()` (`:1099`) is pi's `getShortcutDiagnostics()`; production installs the GATED map via `App::install_extension_shortcuts` → `ExtensionHost::resolve_shortcut_specs` (`crates/cyrup-tui/src/app/shell.rs:188-200`) and `crates/cyrup-tui/src/startup.rs:176`, `:261`, `:362` fold the warnings into the `[Extension issues]` heading. **EVIDENCE CORRECTION**: this row's index line at `:519` cites `rg 'RESERVED_KEYBINDINGS\|restrict_override' crates/` → zero; at the recording HEAD it returns 20+ hits, and `app/input.rs:86-94` is no longer the precedence site. **CROSS-AREA HANDOFF DISCHARGED for N05**: the note at `:3281` ("TUI-006's and TUI-N05's remaining diagnostic sources need new fields on `StartupDiagnostics` in area 08") is settled for this row — the shortcut diagnostics reach `[Extension issues]` through the EXISTING `startup_diagnostics.extensions` channel, so no area-08 work is owed on N05's account. TUI-006's own needs are unaffected and stay open. |
| TUI-N06 | low | parity-bug | L | `Entry::Thinking` freezes hide/show at commit time — **re-verified 2026-09-04, still open BY DESIGN and cited as such at three call sites** (`status.rs:114`, `thinking_selector.rs:77`, `tests/footer_chrome_fidelity.rs:938`); `fda714d`'s `hideThinkingBlock` visibility fix explicitly left this residual untouched. — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN; cyrup is unchanged in substance, only RELOCATED.** `commit_thinking` now lives at `crates/cyrup-tui/src/transcript/stream.rs:128-143` (this row's `transcript.rs:631-643`) and still stamps `Entry::Thinking { text: t, hidden: self.hide_thinking }` at `:139-141`, with its own doc at `:127` saying "The `hideThinkingBlock` choice is frozen into the entry here" and `set_hide_thinking_block` at `:145-151` conceding "already-flushed scrollback is immutable"; the toggle path (`app/input.rs:484-492` → `app/state.rs:432-433`) only flips the flag for future commits. **THE UPSTREAM MECHANISM CHANGED AND THE PROPERTY DID NOT — recorded so a reader diffing pi does not mistake it for a closure:** at v0.84.x pi did `chatContainer.clear(); rebuildChatFromMessages();`; at v0.85.1 `toggleThinkingBlockVisibility` (`:4241-4246`) instead calls `updateThinkingBlockVisibility` (`:4232-4239`), which walks `this.chatContainer.children` and calls `child.setHideThinkingBlock(...)` on every `AssistantMessageComponent` — cheaper, and still RETROACTIVE over the whole rendered conversation. pi's user-visible behaviour is identical and cyrup's divergence is unchanged; only this row's `:4050-4066` citation is dead. The structural constraint (committed entries are drained to `Terminal::insert_before`) is also unchanged, so effort L and the "decide, do not patch" framing stand. — **ATTEMPTED AND HELD OPEN 2026-09-14.** The alternate-screen document CACHE KEY omits `hide_thinking`, so the toggle is a proven visual no-op on that path, while the inline path is unchanged in practice because `flush_committed` runs every frame. Neither added test covers that seam, so the change does not close what the row claims. |
| TUI-N07 | low | parity-bug | L | Mid-session `/resume` cannot erase the previous session's scrollback — **not independently re-checked this pass; left open.** — **RE-AUDIT 2026-09-14 — this row was flagged "not independently re-checked this pass"; it is now CHECKED at `9aeba769` against v0.85.1 and STILL OPEN.** The swap arm (`crates/cyrup-tui/src/app/run_arms.rs`) runs, in order: settings re-reads (`:320-335`), `install_extension_shortcuts` (`:340`), `push_session_loaded_resources` (`:365`), `refresh_known_tool_definitions` (`:376`), the replay (`:381-387`), the trust banner (`:393`), the generation guard (`:397`) — there is no clear and no boundary rule anywhere on that path, and the only marker is the bare `push_status("session replaced")` at `app/session_bind.rs:39`. No scrollback-clear primitive exists to have been wired: `git grep -E 'ESC\[3J\|x1b\[3J\|clear_scrollback' 9aeba769 -- crates/cyrup-tui/src` → 0 hits. Upstream still owns its viewport and really clears — `this.chatContainer.clear()` at `interactive-mode.ts:1899`, `:2004`, `:3420`, `:3923` (`rebuildChatFromMessages`) and `:5305` (tree/fork navigate) @v0.85.1. **Interaction this row already anticipates, now confirmed:** TUI-N02's closure makes the loaded-resources panel APPEND a second copy on each swap (pushed at `:365` into linear scrollback), adding to the very pile-up described here rather than relieving it. Severity low and effort L stand; this remains an ADR-class decision, not a patch. — **NEEDS A DECISION; LEFT OPEN 2026-09-14, with the three questions stated so the call is cheap.** (1) This row's own "cheapest honest improvement" — a full-width rule plus the new session's id/name/branch in place of `session replaced` — is behaviour pi DOES NOT HAVE. Checked at v0.85.1: `chatContainer.clear()` at `:1899`, `:2004`, `:3420`, `:3923` and `:5305`, with no separator and no session banner at ANY of the five clear sites — the seam is invisible upstream because the old content is simply gone. Taking it would violate the porting rule this batch is bound by; it needs an explicit PRODUCT decision to invent it, not a porting decision. (2) A true inline clear needs a raw `ESC[3J` issued outside the ratatui buffer, which destroys the user's PRE-cyrup scrollback, not just cyrup's own — a data-loss action a session swap must not take unprompted. If taken at all it is a new OPT-IN setting, i.e. a cyrup-only surface pi has nothing to port for. (3) TUI-019 has closed since this row was written (`cyrup --tui-mode fullscreen`, `crates/cyrup-tui/src/altscreen/`, wired at `crates/cyrup/src/interactive.rs:136`), so the premise now holds only for the INLINE renderer; in fullscreen cyrup owns the viewport and `TranscriptView::clear_document()` (`transcript/view.rs:226-232`) is an exact `chatContainer.clear()`. RECOMMENDED FRAMING: close this row as a permanent `[CYRUP-DELTA]` for the inline renderer — the rustdoc at `app/session_bind.rs:75-83` already says so and is accurate, and no scrollback-clear primitive exists on that path (`git grep -E 'ESC\[3J\|x1b\[3J\|clear_scrollback' -- crates/cyrup-tui/src` → 0 hits), because committed entries leave the app through `Terminal::insert_before` into the terminal's own immutable scrollback (R-ARCH-TUI-003) — land the fullscreen half, and file (2) separately as a cyrup-only opt-in if anyone wants it. **TUI-N07-A — CARVED OUT DURING PLANNING, PARTIALLY LANDED, AND IT HAS NO LEDGER ROW OF ITS OWN: file one.** The fullscreen half above was implemented in `app/session_bind.rs`, but the `retained_dropped` half of its fix (test `a_session_swap_keeps_retained_dropped_monotonic`) is NOT done: it needs `TranscriptView::adopt_renderer_state_from` in `crates/cyrup-tui/src/transcript/view.rs`, outside the working territory, and cannot be done from `app/session_bind.rs` at all — `retained_dropped` is private to the `transcript` module with a getter at `view.rs:214` and NO setter, and no public API seeds it on a new view. Nothing regresses: the counter was already reset to 0 by the very line that changed, so the skipped scroll-offset reconcile (`altscreen/mod.rs:592-600`) is HEAD's behaviour, not a new defect. The residual is recorded in-code at `session_bind.rs:36-42`, naming the exact arithmetic the follow-up needs (`prev.retained_dropped() + prev.document().len()`, against `clear_document`'s body at `view.rs:226-233`). |
...
| ~~TUI-N10~~ | ~~low~~ **CLOSED 2026-09-14** | test-defect | S | `bash_overlay`'s two hotkeys tests hard-code the non-macOS `alt` spelling — **fixed this pass** — **CLOSED 2026-09-14 (verified re-audit at `9aeba769`).** This row already claimed "fixed this pass" but was never struck, so the CODE was verified rather than the claim. Both `bash_overlay` hotkeys tests now derive the modifier spelling at compile time and interpolate it — `let alt = if cfg!(target_os = "macos") { "Option" } else { "Alt" };` (`crates/cyrup-tui/src/tests/bash_overlay.rs:397-401`) asserting `` \| `Ctrl+X/{alt}+E` \| Edit message in external editor \| `` (`:404-406`), and again at `:471-475` asserting `` \| `{alt}+Shift+K` \| Kill the ring \| `` (`:477`). No hard-coded `alt`/`Alt` literal remains in either expectation; the only bare `alt` left (`:377`, `:460`) is INPUT — the keybindings JSON and the registered shortcut spec — which is correct and must stay lowercase. **The reason this cannot drift is STRUCTURAL, and was checked rather than assumed:** the renderer under test, `format_key_part` (`crates/cyrup-tui/src/chrome.rs:42-55`), branches on the identical `cfg!(target_os = "macos") && part.eq_ignore_ascii_case("alt")` and then title-cases, so test and renderer are compiled from the same predicate and cannot disagree on any host. Both match pi v0.85.1's `formatKeyPart` — `process.platform === "darwin" && part.toLowerCase() === "alt" ? "option" : part`, then capitalize under `{capitalize: true}` (`packages/coding-agent/src/modes/interactive/components/keybinding-hints.ts:12-15`, used by `formatKeyText` at `:17-26`). **FALSIFICATION CONDITION** (this closure rests on reading a `cfg!` branch, not on a darwin run — no macOS host was available): run `cargo nextest run -p cyrup-tui --lib tests::bash_overlay` on a darwin host and REOPEN if `hotkeys_global_key_cells_resolve_from_the_live_keymap` or the extensions-section test fails on the modifier cell; also REOPEN if `format_key_part` (`chrome.rs:42`) ever stops branching on `cfg!(target_os = "macos")` — e.g. moves to a runtime platform probe — since the compile-time coupling is what makes the two sides unable to disagree. **Citation hygiene for the next pass:** cyrup's in-source comments and this ledger both cite `keybinding-hints.ts` without the `components/` directory, a path that exists at no tag. |
...
| ~~TUI-N14~~ | ~~low~~ **CLOSED 2026-09-14 (landed `7e41cf9`, `#140`) — FILED RETROACTIVELY 2026-09-16** | test-defect | S | `get_cursor_position_returns_the_tracked_anchor_without_querying_the_terminal` bounded `Instant::elapsed()` at 250 ms, asserting a scheduling outcome it cannot control — red under a full-workspace `cargo nextest` run, repeatedly, across sessions. **Landed with no row**, exactly as `TUI-093` did, and found the same way: by grepping the code for what moved rather than by reading the ledger. **The defect.** `crates/cyrup-tui/src/app/backend.rs:358-366` (pre-fix) asserted `started.elapsed() < Duration::from_millis(250)` on the theory that only a real DSR round-trip could approach crossterm's 2000 ms timeout. That reasoning is sound about I/O and wrong about SCHEDULING: under `cargo nextest run --workspace` this process competes with ~10 others, and a descheduled thread blows a 250 ms wall-clock bound without anything having queried the terminal. It is the exact class `README.md`'s taxonomy names — *"a test asserting a timing outcome it cannot control"* — and this area's fourth instance after `TUI-N09`, `TUI-N11` and `TUI-N13`. **Why it mattered more than a flake usually does:** it guarded `TUI-093`, a closed `high` whose whole fix is *the backend ANSWERS instead of ASKING*; a red that never indicates a defect trains the next reader to re-run rather than investigate, on the one assertion that would catch a genuine regression of that fix. **The fix replaces a proxy with the property itself.** `backend.rs:359-380` now reads the SharedBuf and asserts `!written.contains("\x1b[6n")` — the DSR cursor-position request. A backend that answers from its tracked anchor never emits those bytes; one that asks must. Deterministic, host-independent, and it fails on a real regression where the timing bound only probably would. `assert_eq!(got, anchor)` is retained. **This row exists because the closure is not self-documenting:** the commit is titled for the subagents SCOPE work (*"capacity, status, retention, scheduled runs"*) and the cyrup-tui hunk is 23 lines inside a 45 415-line diff. **Escalation/reopen condition:** reopen if any assertion in `crates/cyrup-tui/src/app/backend.rs` is ever re-expressed as a wall-clock bound; the sibling `crates/cyrup-tui/src/tests/terminal_theme_query.rs:220-224` `< 1s` bound is recorded under *Rejected, with reasons* as monotone-safe, and that rejection is NOT disturbed by this row — but its stated rationale is narrower than the one this defect falsified, so re-read it rather than citing it. |
| TUI-S02 | low | not-ported | S | No dead-terminal (EIO/EPIPE/ENOTCONN) emergency exit path (panic-hook half closed) — **not independently re-checked this pass; no `DEAD_TERMINAL_ERROR_CODES`/`emergencyTerminalExit` counterpart turned up in the symbol sweep, left open rather than guessed at.** — **RE-AUDIT 2026-09-14 — this row was flagged "not independently re-checked this pass"; both mechanisms are now READ on both sides at `9aeba769` / v0.85.1 and it is STILL OPEN.** UPSTREAM PRESENT AND MOVED: `DEAD_TERMINAL_ERROR_CODES = new Set(["EIO","EPIPE","ENOTCONN"])` at `interactive-mode.ts:243`, `isDeadTerminalError` at `:245-250`, `emergencyTerminalExit()` at `:3994-4000` (body verbatim: `isShuttingDown = true; unregisterSignalHandlers(); killTrackedDetachedChildren();`, then the comment "The terminal is gone. Do not run normal shutdown because TUI and extension cleanup can write restore sequences and re-trigger EIO.", then `process.exit(129)`), armed at `:4062-4067` by a `terminalErrorHandler` installed on BOTH `process.stdout` and `process.stderr` — plus a new-since-baseline comment at `:4051-4054` making the coupling explicit ("SIGHUP no longer hard-exits … A genuinely dead terminal surfaces as an EIO on the restore writes, which the stdout/stderr error handler converts into emergencyTerminalExit (see #4144, #5080)"). **Upstream has if anything STRENGTHENED its dependence on this path since this row was filed.** CYRUP STILL ABSENT: no classifier anywhere in cyrup-tui's draw/flush/restore path, and the crate's own prose still concedes it (`crates/cyrup/src/signals.rs:66-70`). **That prose is now HALF-STALE and is itself the asymmetry this row records:** the `uncaughtException` analog DID land (`crates/cyrup-tui/src/panic_hook.rs`, chained at `:88-99`, installed at `app/crossterm.rs` before `enable_raw_mode`) — the panic half closed, the dead-terminal half did not. The cross-area dependency (SEAM-008 owns cyrup's exit codes) is unchanged. — **ATTEMPTED AND HELD OPEN 2026-09-14.** Real work landed — `is_dead_terminal_error` is a faithful port of pi's three-code set, `emergency_terminal_exit` writes nothing and exits 129, and `restore_terminal_best_effort` is on an armed funnel — but six parts are unaddressed or unproven. (1) THE GATE IS UNSOUND IN BOTH DIRECTIONS AND UNTESTED IN EITHER: `dead_terminal.rs:97-99` `raw_mode_enabled()` is crossterm's process-global `TERMINAL_MODE_PRIOR_RAW_MODE` flag set off `tty_fd()` (stdin or `/dev/tty`) and says NOTHING about stdout, while `crates/cyrup/src/interactive.rs:190` calls `App::into_stdout` with no `stdout().is_tty()` gate and `app/crossterm.rs:53` enables raw mode unconditionally — so `cyrup \| head` with a tty stdin takes the EPIPE to `emergency_terminal_exit()` and exit 129, the exact regression the delta doc says it exists to prevent; the doc's claim that this is "the same precondition `drain::stdin_is_drainable` uses" is false (`drain.rs:161-166` additionally requires both `is_tty()` probes), and the test hand-passes `raw_mode_on: false`, a value no production path produces in the scenario it names. (2) The same gate disarms the TAIL of the cited site: `panic_hook.rs:83` disables raw mode, then `:84` writes `cursor::Show` and `:87` flushes on the funnel, by which point `raw_mode_enabled()` is false — where `terminal.ts:439-484` @v0.85.1 restores raw mode LAST (`:481-483`) and `terminalErrorHandler` has no precondition at all. No delta records the narrowing. (3) A restore-path write pi arms is off cyrup's funnel: `panic_hook.rs:78-80` → `terminal_progress.rs:135` opens raw `std::io::stdout()`, matching `terminal.ts:440-442`, and is absent from the ratchet list. (4) The row's Fix names "the draw/flush AND restore paths"; only restore is armed — every frame write and `self.terminal.clear()` (`app/crossterm.rs:158`) fault into a discarded error. (5) Half the cited arming site is unported: `interactive-mode.ts:4062-4063` @v0.85.1 installs the handler on BOTH `process.stdout` AND `process.stderr`; cyrup arms stdout only, with no delta. And `emergencyTerminalExit` (`:3994-4001`) calls `this.unregisterSignalHandlers()` at `:3996`, simply absent from `emergency_terminal_exit` and undocumented — with it the cross-path effect (suppressing the concurrent normal shutdown writing restore sequences into a dead fd) is not reproduced, and `IN_EMERGENCY` is read by nothing outside its own module where pi's `isShuttingDown` is read by the signal handler at `:3560` and by `shutdown()`. (6) The row's Verify is unmet: no test drives a real failing write through `on_write_error` (`every_failed_write_reaches_the_error_arm_exactly_once` substitutes `counting_arm`), `the_emergency_exit_code_is_pis` asserts `129 == 129` without comparing against `ShutdownSignal::Hangup.exit_code()` (`crates/cyrup/src/signals.rs:159`), and `the_emergency_exit_writes_nothing_and_has_exactly_one_caller` scans only `include_str!("dead_terminal.rs")` while the function is `pub(crate)`. A subprocess re-exec asserting status 129 is the standard way to get what the Verify asks for and was not written. **RE-RUN 2026-09-16 at `cc7818b` — still open (still partially closed), evidence refreshed.** `git grep -in 'DEAD_TERMINAL\|emergency_terminal_exit\|ENOTCONN' -- crates/cyrup-tui/src` returns **two hits and neither is this path** — `keyboard_protocol.rs:603` and `terminal_query.rs:831` are both a test named `a_dead_terminal_is_skipped_not_awaited`, about a bounded startup PROBE declining to wait, not about an emergency exit on a write failure. `DEAD_TERMINAL_ERROR_CODES` and `emergencyTerminalExit` still have zero counterparts. Recorded precisely because the grep is no longer zero and a future pass reading only the hit count would mis-close it. |
| ~~TUI-S10~~ | ~~low~~ **CLOSED 2026-09-14** | not-ported | S | Shift+Ctrl+D global debug chord absent — `/debug` reachable only by typing into the editor — **re-verified 2026-09-04, unchanged: no `on_debug`/reserved-chord handling in `keymap.rs`.** — **CLOSED 2026-09-14 with NO CODE CHANGE: the chord is IMPLEMENTED and this row's evidence was a grep aimed at the wrong file.** **upstream, read at the tag** — `git -C tmp/pi show v0.85.1:packages/tui/src/tui.ts` `:1036-1040`, inside `handleTerminalInput`, after the input-listener loop and BEFORE the overlay-visibility fixups and the `this.focusedComponent?.handleInput` dispatch: `// Global debug key handler (Shift+Ctrl+D)` then `if (matchesKey(data, "shift+ctrl+d") && this.onDebug) { this.onDebug(); return; }`. Same code at `:796` @v0.83.0 — unchanged across the whole window. Wired at `interactive-mode.ts:2889` `this.ui.onDebug = () => this.handleDebugCommand();`, preserved across UI swaps at `:809`/`:828`, reaching the same `handleDebugCommand` (`:6419-6449`) that `/debug` reaches at `:3080`. It is NOT a `Keybindings` member — `grep -i debug` over `packages/tui/src/keybindings.ts` @v0.85.1 returns zero — confirming upstream hardcodes it. **cyrup** `crates/cyrup-tui/src/app/input.rs:12-27`: the FIRST thing `App::handle_input` does after the key-release guard and ahead of every routing branch (overlay, altscreen, selector, loader, extension shortcuts, global keymap, editor) is `if key.code == KeyCode::Char('d') && key.modifiers.contains(KeyModifiers::CONTROL \| KeyModifiers::SHIFT) { return self.run_command("debug", None); }`, dispatching to the same `/debug` handler the typed command reaches (`app/session_bind.rs:217`). The comment there is already labelled TUI-S10 and explains why the chord sits OUTSIDE `Keymap`. **Test** `crates/cyrup-tui/src/tests/escape_chain.rs:202-232` `shift_ctrl_d_dumps_debug_even_with_a_selector_focused`, which opens a selector first and then asserts a `Debug` block reaches the transcript. **EVIDENCE CORRECTION, recorded rather than silently dropped**: the index-table row at `:537` and this row's body cite `rg 'debug' crates/cyrup-tui/src/keymap.rs` → zero as proof of absence. That grep is EXPECTED to return zero and inverts the design — `keymap.rs` is precisely where this must not be, because upstream does not make it a configurable id. One cosmetic residual: the in-code comment still cites `tui.ts:850` @v0.83.0; the same check is `:1037` @v0.85.1. Retarget if that file is opened for another reason. |
| TUI-056 | low | parity-bug | S | The context-usage meter resets to `0.0%` after an aborted turn while the conversation is still in the transcript — **new, observed 2026-08-13** — **not independently re-checked this pass (needs a live aborted-turn observation, not a static read); left open.** — **RE-AUDIT 2026-09-14 — STILL OPEN, AND THE CAUSE THIS ROW COULD NOT ISOLATE IS NOW ISOLATED — on the opposite side of the seam from where the Fix sends the reader.** This row says "the producing side was not isolated" and its Fix says to trace `set_context_usage` from the `Interrupt` arm at `app/input.rs:130-213`; **that Fix is aimed at the wrong crate.** The TUI half is a faithful pass-through: `AgentEnd` is in `context_usage_may_have_moved` (`app/event_extract.rs:10-20`), so an abort triggers `refresh_context_usage` (`app/events.rs:355-370`), which simply mirrors whatever `session.stats_context_usage()` answers. The divergence is entirely inside that answer. UPSTREAM, read at BOTH tags and byte-identical: `getContextUsage` ends `const estimate = estimateContextTokens(this.messages); const percent = (estimate.tokens / contextWindow) * 100` (`core/agent-session.ts:3418-3424` @v0.85.1; `:3199-3205` @v0.83.0), and `estimateContextTokens` (`core/compaction/compaction.ts:202-229`) walks BACKWARD for the last *valid* assistant usage via `getAssistantUsage`, whose doc comment is decisive — *"Skips aborted, error, and all-zero usage messages as they don't have valid usage data"* (`:151-166`), returning `undefined` unless `stopReason !== "aborted" && stopReason !== "error" && calculateContextTokens(usage) > 0`. So after an abort pi falls through to the PREVIOUS good assistant and the meter HOLDS its pre-abort value — exactly the behaviour observed as missing. CYRUP: `AgentSession::context_usage` (`crates/cyrup-session-svc/src/session/stats.rs:187-231`) takes the last assistant on the branch filtered ONLY by `.find(\|a\| a.stop_reason != StopReason::Deferred)` — `Aborted` and `Error` are NOT excluded and there is no all-zero guard — then `ContextUsage::from_last_assistant` (`crates/cyrup-session-svc/src/state.rs:302-316`) sums `input+cache_read+cache_write+output`, so an aborted turn whose usage never arrived gives `used=0`, `fraction=0.0`; `stats_context_usage` (`stats.rs:96-122`) then returns `percent: Some(0.0)` with the window intact, because its only `None`-percent escape hatch is `has_post_compaction_usage()`, which fires only when a compaction exists on the branch. That reproduces the captured `0.0%/131k` exactly and is distinct from the `?` branch, so this row's scope note stands untouched. **NOTE: the abort/error filter cyrup needs is ALREADY WRITTEN twenty lines away in the same file, inside `has_post_compaction_usage` (`stats.rs:152-168`), which DOES exclude `Aborted\|Error`; the defect is that the ungated path does not apply it.** — **PARTIAL, HELD OPEN 2026-09-14.** RED is genuine (reverting the predicate fails exactly the three positives and leaves the negatives green) and the abort/error clauses are faithful, but the change reproduces only a SUB-CLAUSE of the upstream path and introduces a new divergence. (1) pi's third clause is `calculateContextTokens(usage) > 0`, i.e. `totalTokens \|\| sum-of-four` (`compaction.ts:146-148` @v0.85.1); `stats.rs:250` hand-rolls the bare four-field sum and IGNORES `total_tokens`. Proven with a throwaway test: a settled assistant with `totalTokens: 40000` and zero components is now SKIPPED and the meter falls back to the previous turn where pi reports 40000 — a NEW skip the old `!= Deferred` predicate did not have, reachable via bedrock/google/mistral and via `pi_messages.rs:657` reading `totalTokens` from a pi-written session file (the R-00-013 case). (2) The exact port already exists in-tree and public — `cyrup-provider/src/utils/estimate.rs:41 calculate_context_tokens`, re-exported at `lib.rs:176-179`, with `cyrup-session-svc` already depending on `cyrup-provider` (`Cargo.toml:33`), and `estimate.rs:121-149 last_assistant_usage_info` already implements the whole `getAssistantUsage` filter — so there are now TWO in-tree ports of one upstream function that disagree, and session-svc calls the wrong one. (3) The `[CYRUP-DELTA]` in `a_first_turn_abort_still_reads_zero` states "cyrup has no estimator", which is false — `estimate.rs` is a full 1:1 port of pi's `estimate.ts` and `estimate_message_tokens` is public and re-exported — so it pins `used_tokens == 0` on a constraint that does not exist. (4) `getContextUsage` returns `usageTokens + trailingTokens` (`compaction.ts:216-228`), and after an abort the trailing messages are the new user prompt and the aborted partial, so pi reads strictly MORE than the pre-abort value; `an_aborted_turn_holds_the_previous_context_reading` asserts `held == after_a1` exactly, locking in a number pi never produces in this row's own scenario. (5) The row's Verify has a SECOND assertion — "the CH segment is unchanged" — that both the diff and the claim ignore. It is the VERIFY that is wrong, not cyrup: `footer.ts:95-98` @v0.85.1 sets `latestCacheHitRate = undefined` when `latestPromptTokens === 0`, so `status.rs:166-177` is upstream-faithful — but that rebuttal was never written into the row, so its disproven "one shared reset rather than two" hypothesis still stands as filed. Amend the Verify when this is picked up. |
| TUI-057 | low | port-divergence | M | Slash-command palette submission is inconsistent — sometimes one Enter, sometimes two, sometimes a trailing space suppresses it — **new, observed 2026-08-13, low confidence** — **not independently re-checked this pass (originally a live-use finding, and this pass had no pty); left open.** — **RE-AUDIT 2026-09-14 — CANNOT SETTLE the observed behaviour. BLOCKED ON: no pty/terminal in this pass.** This row was filed from a tmux-driven live session, its own `cyrup` paragraph concedes no call site was identified, and its Verify demands a stubbed-completion harness plus a ten-invocation live run; nothing is guessed here. **Two by-products from code that WAS read, both usable by whoever works this.** (1) THE ROW'S OWN DECLARED BLOCKER IS NOW CLEARED — its `upstream` paragraph says pi's palette submission semantics *"must be settled before the item is worked, or the fix risks pinning cyrup-original behaviour"*: settled. pi's confirm branch is `packages/tui/src/components/editor.ts:777-800` @v0.85.1 — on `tui.select.confirm` with an item selected it calls `applyCompletion(...)`, then `if (this.autocompletePrefix.startsWith("/")) { this.cancelAutocomplete(); /* Fall through to submit */ } else { this.cancelAutocomplete(); onChange(); return; }` — ONE Enter for a slash item. cyrup's `A::AcceptSubmit` arm (`crates/cyrup-tui/src/editor/keys.rs:112-142`) is the same shape, testing `context == CompletionContext::Slash` where pi tests the prefix string, with an in-source paragraph at `:113-124` arguing they agree by construction on the slash surface and naming the one residual where they do not (an absolute-path popup, prefix `/usr/lo`, satisfies pi's string test and not cyrup's context test — pi submits, cyrup keeps editing). (2) THE RACE HYPOTHESIS INVERTS AND THE TRAILING-SPACE SYMPTOM IS REFUTED AT THE DISPATCH LAYER: pi is the side with async population (`editor.ts:2360-2400`, `getSuggestions` awaited and re-validated through `isAutocompleteRequestCurrent`), while cyrup's command-NAME popup is computed synchronously (`editor/completion.rs:80-118` → `autocomplete.rs:148-182`, `:507-538`) with no await on that path and `/compact` owns no argument completer, so Enter-before-population is an unlikely explanation for the two-Enter symptom; and `CommandRegistry::dispatch` trims FIRST — `let trimmed = text.trim();` (`commands.rs:395`) — before `strip_prefix('/')` and `match_command`, so `"/compact "` is indistinguishable from `"/compact"` by the time matching happens. Whatever swallowed that Enter, it was not command matching: **start ABOVE the editor's `E::Submit` arm, not below it.** — **PARTIAL, HELD OPEN DELIBERATELY 2026-09-14.** What landed is the one STATICALLY provable one-Enter-vs-two divergence inside this row's file territory (RED/GREEN verified by swapping `src/editor/keys.rs` to `git show HEAD:` — 5 of 6 pass at HEAD, 6 of 6 after). It does NOT satisfy this row's own Verify, which asks for a stubbed-completion harness plus ten live invocations through a pty, and the live `/compact` two-Enter symptom the row was filed for remains uncharacterised. RECOMMENDATION: re-scope this row to the live symptom or split it, rather than close it. One behaviour widening wants a human's eyes before any closure: after this change, Enter on a completed ABSOLUTE path popup submits the whole line (pi `editor.ts:794`), which may surprise a user who typed `cat /usr/lo<Tab>`. |
| TUI-062 | low — **PARTIALLY CLOSED 2026-08-14** | parity-bug | S | `showWarning`'s `Warning: ` prefix lives INSIDE pi's function but OUTSIDE cyrup's renderer, making it a per-caller obligation that one of three callers had dropped — **FILED AND PARTIALLY CLOSED 2026-08-14 (sweep 6, found while fixing `CFG-051`; the work was done from another partition and the design half belongs to whoever owns cyrup-tui).** pi's `showWarning` builds `Warning: ${warningMessage}` inside the function (`interactive-mode.ts:3885-3889` @v0.83.0), while cyrup's `Entry::Warning` renders its text **verbatim**, so every cyrup caller must supply the prefix. Two of three did (`app/extension_ui.rs:307`, `app/run_arms.rs:536`; the trust banner at `app/session_bind.rs:135` is a deliberate no-prefix case, documented in place); `main.rs`'s `modelFallbackMessage` push did not, so **a credential-less first run rendered a bare "No models available…" where pi renders "Warning: No models available…"**. That call site is fixed. **RESIDUAL, two halves, both cyrup-tui's:** (a) the in-tree citation at `crates/cyrup-tui/src/transcript.rs:2965-2971` cites `interactive-mode.ts:3956-3960` for `showWarning`; at v0.83.0 it is `:3885-3889` — a wrong cite of the version-lag kind, uncorrected; (b) the DESIGN question — prefix in the renderer (pi's shape, one place) vs. at each caller (cyrup's current shape, N places, already broken once). **This is the general class "a mechanism ported one level up or down, so the invariant quietly becomes optional"** — see the JS→Rust register in `00-residual-ledger.md`. — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.83.0: BOTH RESIDUALS STILL OPEN. (a) was partly acted on and the action introduced a NEW error; (b) has demonstrably REGRESSED AGAIN, which is this row's own thesis proving itself.** RESIDUAL (a) — WORSE, NOT FIXED: the wrong `interactive-mode.ts:3956-3960` cite survives at TWO sites (`transcript/entry.rs:81`, `transcript/notices.rs:33`). A third site was rewritten (`transcript/render.rs:346-364`) with a paragraph narrating the re-read — and the rewrite is itself off by one AND wrongly convicts this row's correct figure. Counted line by line at v0.83.0: `:3883` is showError's `}`, `:3884` BLANK, `:3885` `showWarning(warningMessage: string): void {`, `:3886` `Spacer(1)`, `:3887` the `Warning: ${warningMessage}` Text, `:3888` `requestRender()`, `:3889` `}` — so **`:3885-3889` (this row's proposed correction) is EXACTLY the function**; `render.rs:347`'s `:3884-3888` starts on the blank line, `render.rs:358`'s `:3886` for the template is one short (it is `:3887`), and `render.rs:352-355`'s assertion that this row is off by one is simply WRONG. That same paragraph also cites `app.rs:3626` and `app.rs`'s `render_project_trust_warning_if_needed`; `crates/cyrup-tui/src/app.rs` does not exist at `9aeba769` (`git cat-file -e` fails) — the real sites are `app/run_arms.rs:696` and `app/session_bind.rs:137-141`. RESIDUAL (b) — DESIGN QUESTION UNSETTLED AND THE PREDICTED FAILURE HAS RECURRED TWICE: `Entry::Warning(text)` still renders `text` verbatim (`transcript/render.rs:365-367` — `vec![Line::default()]` then `text_lines(text, width, output_pad, theme.warning_style())`, no prefix), so the prefix remains a per-caller obligation; the caller count has grown from 3 to **8 production sites** and the compliance ratio has FALLEN — only `app/extension_ui.rs:358`, `app/run_arms.rs:696` and `crates/cyrup/src/interactive.rs:332` supply `Warning: `. **Two NEW non-compliant sites are direct ports of `showWarning` calls:** `app/execute_misc.rs:922` pushes bare `Usage: /name <name>` against pi `:5640` `this.showWarning("Usage: /name <name>")`, and `app/execute_misc.rs:894-901` pushes a bare `Session name was normalized from X to Y` against pi `:5649` — pi prefixes both. The `:922` site is the sharper instance: its own comment at `:913-915` reasons explicitly about the severity CHANNEL ("a `showWarning` … not a neutral status") and gets the channel right while silently losing the prefix that channel carries. `app/channels.rs:199` and `crates/cyrup/src/interactive.rs:323` are two further unprefixed sites; `app/session_bind.rs:141`'s `PROJECT_UNTRUSTED_WARNING` remains the correct deliberate exception. — **ONE SITE DROPPED, HELD OPEN 2026-09-14, exactly as the plan permits.** `app/run_arms.rs:696` `push_warning(format!("Warning: {warning}"))` was NOT migrated to `show_warning`: `app/run_arms.rs` is inside a concurrently-working sibling agent's declared surface (git status confirms it and roughly fifteen other untouched files were modified in the tree during this batch), and the substitution is cosmetic with byte-identical output. A follow-up should change that one line AND `app/run_arms.rs:416`, a SECOND `Warning: `-inline site this row does not list, once that agent has landed. |
...
| TUI-064 | low | not-ported | M | The attached-image strip has no production callers — **filed 2026-08-14**; `attach_image`/`attach_image_path` are called only from three unit tests, so the strip is dead surface and `TUI-017` cannot be observed end to end. — **re-verified 2026-09-04, unchanged: `rg '\.attach_image\(' crates/cyrup-tui/src` still returns only `tests/image.rs`.** — **DO NOT IMPLEMENT AS FILED; LEFT OPEN 2026-09-14 with a recommended disposition, because closing it requires an upstream-paragraph amendment this pass is not authorised to make unilaterally.** THIS ROW'S UPSTREAM SENTENCE IS FALSE and was refuted at BOTH named tags. (a) pi has no attachment strip: the interactive dock is `document, pendingMessages, status, widgetsAbove, editor, widgetsBelow, footer` (`interactive-mode.ts:876-898` @v0.85.1 — the same `TuiLayouts.VStack` cyrup's `app/layout.rs:118-130` comment quotes), with no image region. (b) The paste path attaches nothing: `handleClipboardPaste` writes the temp file then calls `this.editor.insertTextAtCursor?.(filePath)` and RETURNS (`:2934-2965`, insert at `:2944` @v0.85.1; byte-identical body at `:2635-2659`, insert at `:2645` @v0.83.0). (c) pi's `Image` component has exactly two production consumers @v0.85.1 — `earendil-announcement.ts:41` and `tool-execution.ts:388` — neither editor-adjacent. (d) The ONLY images pi attaches to an interactive prompt are CLI `@file` args (`cli/file-processor.ts:61-66` → `initialImages` → `:1113`), i.e. message CONTENT for the first turn only, which cyrup already ports separately and correctly at `crates/cyrup/src/input.rs:493` → `Inputs::images` → `crates/cyrup/src/run.rs:317`. DO NOT WIRE: this row's Fix ("call `attach_image_path` from the clipboard-paste handler") contradicts upstream AND turns a green test red — `App::insert_clipboard_image_path` (`app/shell.rs:386-397`) is the deliberate port of `insertTextAtCursor(filePath)`, and `crates/cyrup-tui/src/tests/clipboard.rs:312-317` asserts `app.pending_images().is_empty()` with the message "clipboard paste must not embed an image block into pending_images"; the never-skip-a-test rule makes the Fix simply unavailable. There is also no `@`-mention image path to wire — cyrup's mention acceptance inserts text, exactly as pi's does. DO NOT DELETE either: the strip is parity-pure but the deletion is disproportionate and destructive — it would remove `pending_images` (`app/state.rs:101`, `:472`), three methods (`app/shell.rs:369-381`, `:441-448`), the `want_images` branch (`app/layout.rs:105-113`), the `Strip` construction (`app/draw.rs:215-218`), `render_images` (`app/render.rs:184-280`), the `Strip` type and `place` in `altscreen/images.rs` with its call at `altscreen/mod.rs:762`, and with them `ImageRenderer::render`/`cell_size`/`protocol_cache`/`ImageCacheKey` (`image.rs:56-80`) — cyrup's ONLY negotiated Kitty/iTerm2 emission path, which `image.rs:388-396` explicitly records as the reason tool-result images are allowed to downgrade to half-blocks. THE DEAD-SURFACE CLAIM HOLDS at the recording HEAD with drifted numbers: `attach_image` is `app/shell.rs:371` (row says `:294`), `attach_image_path` is `:377` (row says `:300`), and `clear_images` at `:441` is dead too — the row missed it; `rg attach_image` over `crates/` returns exactly five hits, the two definitions and three calls in `tests/image.rs:44`, `:84`, `:136`, and `clear_images` has zero callers anywhere. RECOMMENDED DISPOSITION: replace the false sentence at `app/shell.rs:370` ("The `@`-mention of an image file and clipboard-image paste both land here") — which is what made this look like a wiring bug — with a `[CYRUP-DELTA]` on both `attach_image` and `attach_image_path` recording that this is a cyrup-ORIGINAL surface pi does not have, citing `interactive-mode.ts:876-898`, `:2944` @v0.85.1 / `:2645` @v0.83.0 and `cli/file-processor.ts:61-66`, and stating that it is kept rather than deleted because `ImageRenderer::render` is the only negotiated-protocol draw path (`image.rs:388-396`); then amend this row's **upstream** paragraph to say the claim was refuted at both tags and close it as a recorded finding. |
| TUI-065 | low | cyrup-original | S | `app.pageUp` / `app.pageDown` are cyrup-invented ids, and each now resolves in **two** keymaps at once, so one config entry rebinds two actions — **filed 2026-08-14** (sweep 10, keybinding-id surface). TUI-028's closure said these "are listed as a new gap"; **no row was ever added** — this is it. — **not independently re-checked this pass; left open.** — **RE-AUDIT 2026-09-14 — this row was flagged "not independently re-checked this pass"; it is now CHECKED at `9aeba769` against v0.85.1 and is NARROWED TO HALF — still open.** THE NEWER HALF ("each now resolves in TWO keymaps at once, so one config entry rebinds two actions") IS CLOSED, by this row's own Fix option (a): `EditorAction::from_id` is now `keymap.rs:467-468` and reads `"tui.editor.pageUp" \| "editor.pageUp" \| "pageUp" => E::PageUp` (and the mirror for pageDown) — the `app.pageUp`/`app.pageDown` EDITOR aliases cited at `:403-404` are GONE. What remains at `:463-466` is a **STALE COMMENT that now contradicts the code beneath it** (it still says "The `app.*` spellings stay as aliases for the same reason the `editor.*` ones do"). The other maps were checked: `AltScreenAction::from_id` takes no `app.*` id (`:1985-1990`, doc: "No aliases"), and `git grep 'app\.pageUp' 9aeba769 -- crates/cyrup-tui/src` resolves only to `Action::from_id:273`, `Action::id:321`, the CYRUP-DELTA doc at `:305-314`, the stale comment at `:463` and a comment at `:1741` — one map. THE ORIGINAL HALF STANDS: `app.pageUp`/`app.pageDown` remain cyrup-invented ids with no upstream counterpart, now documented as `[CYRUP-DELTA]` at `keymap.rs:305-314` (which also names the user-visible consequence: both are default-bound at `:790-791`, so they enter `effective_keybindings()` and an extension registering `pageup` earns an `[Extension issues]` warning pi would never emit). Upstream absence confirmed independently: pi's page ids at v0.85.1 are `tui.editor.pageUp`/`pageDown` (`packages/tui/src/keybindings.ts:114-115`), and `coding-agent/src/core/keybindings.ts` has no `app.page*` at v0.83.0 or v0.85.1. **TWO RESIDUALS THIS ROW ASKED FOR AND DID NOT GET:** (i) its Verify's second clause — `Keymap::default()` and `EditorKeymap::default()` must not both claim bare `PageUp` — still FAILS (`keymap.rs:790-791` global vs `:1744-1747` editor; which fires is decided by App routing order, not by any upstream rule); (ii) its closing instruction to record the surviving cyrup-only id in TUI-066's list is NOT done. — **ATTEMPTED AND HELD OPEN 2026-09-14.** A keymap pass added `tui065_app_page_up_moves_exactly_one_map` and `tui065_bare_page_up_is_bound_in_both_tables_on_purpose`, but the TUI-065 part of that diff is 100% COMMENTS — `git show HEAD:crates/cyrup-tui/src/keymap.rs` has the same four sites byte-identical (`"app.pageUp" => Some(Action::PageUp)`, the `tui.editor.pageUp` arm with no `app.*` alternative, and both `Key::plain(PageUp)` default rows), so both new tests pass on the old code and are regression pins for `9aeba769`'s alias deletion, not evidence. Neither named residual moved: (i) the closing instruction "kept out of user documentation" is still violated at `docs/guide/reference/keybindings.md:350-352`, which teaches `app.pageUp`/`app.pageDown` to users inside the section that opens "There are 73 ids in four namespaces", and `docs/` was not in the change set; (ii) "recorded in TUI-066's list" was discharged in a Rust const (`CYRUP_ONLY_IDS` in `tests/keymap.rs`) rather than in TUI-066's row, and the two records now DISAGREE on cardinality — the const pins 24 `editor.*` aliases, TUI-066's row still says 19. The second Verify clause was refused on defensible merits (upstream really does collide bare keys across tiers) but the row still records it as "still FAILS", and the refusal was never written down. Two further defects to fix when this is picked up: the new `[CYRUP-DELTA]` at `keymap.rs:724-735` asserts the ten extra `Key::parse` tokens are "never documented" when all ten are documented at `docs/guide/reference/keybindings.md:321-325`; and the shipped comments at `keymap.rs:899-902` cite `keybindings.ts:139`/`:158`/`:157` where v0.85.1 has `:149`/`:160-163`/`:159` — off by ten, landing the next auditor on `tui.select.cancel`. **RE-RUN 2026-09-16 at `cc7818b` — still open, unchanged.** Re-read rather than carried forward because `crates/cyrup-tui/src/keymap.rs` changed in this window; the change is `TUI-073`'s `clear` diagnostic arm (`#138`, `fd316e7`) and touches no page-key row. `app.pageUp`/`app.pageDown` still resolve, and still resolve in two keymaps at once. |
| TUI-066 | low | cyrup-original | S | The cyrup-only keybinding-id and key-spec vocabulary TUI-028's closure deliberately preserved — 5 `tui.autocomplete.*`, 19 bare `editor.*` aliases, 10 `Key::parse` tokens pi's `KeyId` has no word for — is tracked nowhere — **filed 2026-08-14** — **not independently re-checked this pass; left open.** — **RE-AUDIT 2026-09-14 — this row was flagged "not independently re-checked this pass"; it is now CHECKED at `9aeba769` against v0.85.1 and is UNCHANGED — still open.** All three invented vocabularies are present in cyrup, all three are absent upstream at the CURRENT tag, and this row's Fix (a recording task, not a code change) is NOT done: `git grep 'TUI-066' 9aeba769 -- crates/` returns ZERO, so `grep TUI-066` over the tree still resolves to nothing and the alias blocks still cite TUI-028, the row whose closure created the untracked state. (1) `tui.autocomplete.*`, five ids: `AutocompleteAction` (`keymap.rs:1105-1120`), `from_id` (`:1138-1144`) accepting `previous\|next\|accept\|acceptSubmit\|cancel` alongside pi's `tui.select.up\|down\|confirm\|cancel` and `tui.input.tab`, merged at `:1201`, with an honest doc at `:1127-1134`; `git show v0.85.1:packages/tui/src/keybindings.ts \| grep 'tui.autocomplete'` → ZERO. (2) The bare `editor.*` namespace survives as the middle alternative of each `\|`-arm across `keymap.rs:~420-475` (`"tui.editor.deleteToLineEnd" \| "editor.deleteToLineEnd" \| "deleteToLineEnd"`, `"tui.editor.yank" \| "editor.yank" \| "yank"`, `"tui.input.newLine" \| "editor.newLine" \| "newLine"`, …) — matching neither pi's modern `tui.editor.*` nor pi's legacy bare spelling, a cyrup-only THIRD spelling, as filed. (3) The extra `Key::parse` tokens survive verbatim at `keymap.rs:630-663`: `"ctrl"\|"control"` (`:631`), `"alt"\|"option"\|"meta"` (`:633`), `"super"\|"cmd"\|"command"` (`:634`), `"backtab"` (`:637`), `"delete"\|"del"` (`:647`), `"pageup"\|"pgup"` (`:651`), `"pagedown"\|"pgdn"` (`:652`), `"insert"\|"ins"` (`:662`) — against pi's `ModifierName = "ctrl" \| "shift" \| "alt" \| "super"` (`keys.ts:142`) and `SpecialKey` (`keys.ts:109-138`), which have no word for `backtab`, `del`, `ins`, `pgup` or `pgdn`, so a cyrup-valid `keybindings.json` is still rejectable by pi's types. **One detail STRONGER than this row states, for whoever records it:** `keymap.rs:1648-1649` maps `"pageUp" => "pgup"` / `"pageDown" => "pgdn"` in the LABEL direction, so cyrup does not merely ACCEPT the invented tokens — it EMITS them, a label round-trip pi's `KeyId` cannot parse. — **ATTEMPTED AND HELD OPEN 2026-09-14.** The CODE half landed and is correct: `PI_IDS` (90) is set-exact against v0.85.1 (43 `app.*` + 47 `tui.*`), `PI_LEGACY_IDS` (59) set-exact against `KEYBINDING_NAME_MIGRATIONS`, `CYRUP_ONLY_IDS` (31) set-exact as accepted-minus-both, the scanner truncates nothing, and both guards were verified RED in both directions. Correction (a) — 24 `editor.*` aliases, not 19 — is right, and so is correction (c). But the Fix's PRIMARY clause, "record the three lists in one place — this item is that place", was NOT done: this row is untouched, still says 19, still omits `app.pageUp`/`app.pageDown`, so TUI-065's residual (ii) stays open and the lists went into a Rust const instead. The Verify asks for an assertion against THIS ITEM's list; the test asserts against `CYRUP_ONLY_IDS` with nothing tying the two together, so the row can rot further with the suite green. And the Fix's "the aliases stay accepted and undocumented" clause is violated at HEAD and unguarded: `docs/guide/reference/keybindings.md:322-325` teaches all ten tokens as ordinary grammar and `:351-353` teaches the `editor.*` prefix, `app.pageUp`/`app.pageDown` and `tui.autocomplete.*`, while the change writes "they stay accepted and stay undocumented" into `keymap.rs:471` and `:1275` and "never documented" into `:741` — three shipped statements the guide contradicts. Minor: `accepted_ids()` harvests comment text as well as match arms, so the new TUI-065 comment quoting `"app.pageUp"` passes only by coincidence, and an odd number of quotes in a future in-body comment flips literal parity for the rest of that span. **RE-RUN 2026-09-16 at `cc7818b` — still open, and the cyrup-only vocabulary GREW BY ONE in this window, which this row should own.** `keymap.rs` changed for `TUI-073`, whose closure adds a cyrup-specific REJECTION of the pi `KeyId` `clear` — `"clear" => return Err(TuiError::UnsupportedKey(...))` — plus a new `TuiError::UnsupportedKey` variant (`crates/cyrup-tui/src/error.rs`). That is a deliberate and well-documented `[CYRUP-DELTA]` (crossterm's `KeyCode` enumerates no counterpart, and no upstream default binds `clear`), not a defect — but it is one more token where cyrup's key-spec vocabulary and pi's diverge, and this row is the census that is supposed to know about it. |
...
| TUI-071 | low | port-divergence | S | Three platform-conditional upstream defaults are bound unconditionally: `app.clipboard.pasteImage` (both `ctrl+v` and `alt+v` everywhere), `app.suspend` (no win32 gate), `app.tree.foldOrUp`/`unfoldOrDown` (key order fixed at alt-first) — **filed 2026-08-14**; none carries the `CYRUP-DELTA` cite a forced difference requires — **re-verified 2026-09-04 for the `pasteImage` clause, unchanged: `keymap.rs:704-709` still binds `ctrl+v`/`alt+v` unconditionally on every platform (an explanatory comment was added, but no platform gate); the `app.suspend`/`foldOrUp` clauses were not independently re-checked.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1, with the classification re-checked at the v0.83.0 baseline: ALL THREE CLAUSES UNCHANGED, still open.** (1) `app.clipboard.pasteImage` — `Keymap::default` still binds BOTH keys on every platform (`keymap.rs:841` `(Key::ctrl('v'), Action::ClipboardPasteImage)` and `:842-848` the `Char('v')`+ALT twin), with no `cfg!` anywhere on the path; upstream @v0.85.1 `core/keybindings.ts:144-147` is `defaultKeys: windowsKeybindings ? "alt+v" : "ctrl+v"` — exactly one key, still. **PARTIAL IMPROVEMENT worth recording precisely, because this row's wording turns on it:** the complaint that "the comment at `keymap.rs:666-669` declares the intent but cites no upstream line" is now SATISFIED — the comment (now `:837-840`) cites `core/keybindings.ts:106-109` ("`ctrl+v` everywhere, `alt+v` on Windows"). It is still not marked `CYRUP-DELTA`, and the behaviour is unchanged. (2) `app.suspend` — `keymap.rs:787` `(Key::ctrl('z'), Action::Suspend)`, unconditional; `git grep 'cfg!(windows)\|cfg(windows)\|target_os' 9aeba769 -- crates/cyrup-tui/src/keymap.rs` returns NOTHING, so the whole file is platform-blind. Upstream `:96-99`: `defaultKeys: process.platform === "win32" ? [] : "ctrl+z"`. (3) `app.tree.foldOrUp`/`unfoldOrDown` — `keymap.rs:1594-1597` still alt-first unconditionally against upstream `:150-157`'s `process.platform === "darwin" ? ["alt+left","ctrl+left"] : ["ctrl+left","alt+left"]`: key SET right on both platforms, display ORDER wrong on non-darwin, as filed. **Baseline check done rather than assumed:** at v0.83.0 `core/keybindings.ts` has exactly four `process.platform` sites — `:70` (suspend), `:112` (pasteImage), `:120`/`:124` (tree) — i.e. precisely this row's three names and nothing else, so all three are genuine port-divergences against the ported tag, not drift. The much larger `useWindowsKeybindings` surface upstream has grown SINCE is a separate, later window and is deliberately not folded in here. — **ATTEMPTED AND HELD OPEN 2026-09-14.** Code for all three clauses landed, is green, and every upstream citation was verified at v0.83.0 and v0.85.1; the five `tui071_*` tests are genuinely RED at HEAD because the three constructors do not exist there. Four things are unaddressed. (1) cyrup's shipped user docs still advertise the removed behaviour on ALL three clauses — `docs/guide/reference/keybindings.md:31` "Ctrl+V / Alt+V", `:19` "Ctrl+Z" with no Windows qualifier, `:157-158` alt-first fold order, and `docs/guide/guides/tui.md:195` "Ctrl+V (or Alt+V)" — where pi's own doc carries the qualifiers (`docs/keybindings.md:128`, `:130`, `:172`), and TUI-068's closure set the precedent that keybinding fixes update this doc. (2) The `[CYRUP-DELTA]` on JS `Boolean("")` is claimed-tested but is not: the assertion carrying it is `use_windows_keybindings_for(false, true, &env_with(&[]))`, identical in effect to the `&none` assertion six lines earlier, so the real emptiness probe (`var_os(..).is_some_and(\|v\| !v.is_empty())`) is never exercised. (3) `tui071_alt_v_is_not_a_global_action_off_windows` is gated `#[cfg(not(windows))]` but asserts behaviour gated on `use_windows_keybindings()`, which is TRUE on Linux-under-WSL — so it runs and FAILS on a WSL host, the exact environment the new branch exists for; its sibling in the same file gets this right. (4) The SCOPE FENCE excludes the seven other v0.85.1 `windowsKeybindings` sites as "a later upstream window", but `pasteImage`'s WSL arm is from that same window, so on WSL cyrup now matches v0.85.1 for `pasteImage` while still shipping v0.83.0 for `dequeue`/`followUp`/`cycleBackward`/`undo` — a profile matching neither tag. **RE-RUN 2026-09-16 at `cc7818b` — still open, unchanged.** Re-read because `keymap.rs` changed in this window (for `TUI-073` only). All three platform-conditional defaults are still bound unconditionally. |
| ~~TUI-072~~ | ~~low~~ **CLOSED 2026-09-14** | upstream-drift | S | Four editor key sets carry their v0.84.1 extras (`ctrl+home`, `ctrl+end`, `ctrl+pageup`, `ctrl+pagedown`) against a v0.83.0 baseline, changing the `/hotkeys` cells — **filed 2026-08-14**; disclosed in-code, never in the ledger — **CLOSED 2026-09-14 with NO CODE CHANGE, exactly as this row's own Fix predicted ("when the baseline moves to v0.84.1 this row closes with no code change"). The reference tag is now v0.85.1 and upstream moved TO cyrup's sets; the four extras are not forward-ported, they are current.** **upstream, read at the tag** — `git -C tmp/pi show v0.85.1:packages/tui/src/keybindings.ts`: `"tui.editor.cursorLineStart": { defaultKeys: ["home", "ctrl+home", "ctrl+a"] }` at `:98-99`, `"tui.editor.cursorLineEnd": { defaultKeys: ["end", "ctrl+end", "ctrl+e"] }` at `:102-103`, `"tui.editor.pageUp": { defaultKeys: ["pageUp", "ctrl+pageUp"] }` at `:114` and `"tui.editor.pageDown": { defaultKeys: ["pageDown", "ctrl+pageDown"] }` at `:115`. At v0.83.0 the same file had `["home","ctrl+a"]` / `["end","ctrl+e"]` and single-key `"pageUp"` / `"pageDown"` — which is the drift this row was filed against, resolved by upstream, not by cyrup. **cyrup** `crates/cyrup-tui/src/keymap.rs` `EditorKeymap::default()` matches at the current tag EXACTLY, order included: `:1904` `(Key::plain(Home), E::CursorLineStart)`, `:1909` `(ctrl_code(Home), …)`, `:1910` `(ctrl('a'), …)`; `:1911` `(Key::plain(End), E::CursorLineEnd)`, `:1912` `(ctrl_code(End), …)`, `:1913` `(ctrl('e'), …)`; `:1918-1919` `PageUp` then `ctrl_code(PageUp)`; `:1920-1921` `PageDown` then `ctrl_code(PageDown)` — bare, then the ctrl-modifier, then the emacs chord, which is upstream's order. (The line numbers in the planning note were taken before TUI-071 edited this file and have drifted by ~174; these are the numbers at the recording HEAD.) **Residual hygiene, not required for this closure**: three in-code comments still frame these as lag — `keymap.rs` near the `cursorLineStart`/`cursorLineEnd` sets ("joined the line-start/line-end key sets in v0.84.1 … Version lag, not a port bug"), the matching `ctrl+pageUp`/`ctrl+pageDown` note, and the module doc of `crates/cyrup-tui/src/tests/editor_page_actions.rs:10-12`. Retarget them to `keybindings.ts:98-99`/`:114-115` @v0.85.1 and drop the "version lag" framing the next time that file is opened. |
| TUI-073 | ~~low~~ **CLOSED 2026-09-14** | parity-bug | S | `clear` is a valid pi `KeyId` that `Key::parse` rejects with a generic parse failure — **filed 2026-08-14**; the residual of a chord-parse audit in which **all 58 upstream default key specs parse**, so no default chord is dead — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: UNCHANGED ON BOTH SIDES, still open, including the load-bearing qualifier.** cyrup: `Key::parse`'s token match (`keymap.rs:630-680`) still has no `"clear"` arm and says so deliberately in place — the comment at `:659-660` reads "`clear` (`keys.ts:119`) is deliberately absent: crossterm's `KeyCode` has no counterpart, so there is nothing to map it to." The consequence is intact: `"clear"` is multi-character, so it misses the single-char fallback `(Some(c), None) => code = Some(KeyCode::Char(c))` at `:675-676` and reaches `_ => return Err(TuiError::KeySpec(s.to_string()))` at `:677` — and the diagnostic that produces is still the generic one this row objects to, `#[error("invalid key spec: {0}")] KeySpec(String)` (`crates/cyrup-tui/src/error.rs:24-25`), so a user who writes `{"app.interrupt": "clear"}` gets `invalid key spec: clear` and cannot tell a typo from an unsupported key. **This row's one-line Fix (keep rejecting it but say `unsupported key "clear"` instead of a generic parse failure) is NOT done.** Upstream: `"clear"` is still a `SpecialKey` member (`packages/tui/src/keys.ts:109-138` @v0.85.1, between `"insert"` and `"home"`), so pi's types still accept the binding cyrup rejects. **AND THE QUALIFIER THAT KEEPS THIS AT LOW WAS RE-CHECKED, NOT INHERITED:** no upstream DEFAULT uses `clear` — `grep '"clear"'` over `packages/tui/src/keybindings.ts` @v0.85.1 returns zero, and the only `clear` in `coding-agent/src/core/keybindings.ts` @v0.85.1 is `:273 clear: "app.clear"`, a `KEYBINDING_NAME_MIGRATIONS` entry mapping a legacy binding ID, not a key spec. So no default chord is dead and the failure stays confined to a hand-written config that names the key, exactly as filed. The sibling claim this row rests on (the f1-f12 and `insert` family DO parse) also still holds, at `keymap.rs:662-671`. — **CLOSED 2026-09-14.** **upstream, read at the tag** — `git -C tmp/pi show v0.85.1:packages/tui/src/keys.ts` puts `clear` in the `SpecialKey` union at `:119`, between `"insert"` and `"home"`, with sequence tables at `:379`/`:399`/`:413`, reverse-lookup rows at `:429-432` (`\x1bOe` → `ctrl+clear`, `\x1b[e` → `shift+clear`) and a `matchesKey` arm at `:990-994`, so pi genuinely binds `{"app.interrupt": "clear"}`. The low-severity qualifier was re-checked, not inherited: `grep '"clear"'` over `packages/tui/src/keybindings.ts` @v0.85.1 returns **zero**, so no upstream DEFAULT is dead and only a hand-written config reaches this. crossterm 0.29 enumerates no `KeyCode::Clear`, so the row's FIRST Fix option (map it) has no honest target and was deliberately not taken; the row's actual one-line Fix (keep rejecting, but say so precisely) is what landed. **cyrup** `crates/cyrup-tui/src/error.rs:27-41` adds `UnsupportedKey(String)` with `#[error("unsupported key \"{0}\"")]`, placed after `KeySpec`, carrying the `[CYRUP-DELTA]` that names `keys.ts:119 @v0.85.1` and crossterm's variant list and states why inventing an approximation (`KeyCode::Null`) would be behaviour pi does not have. `crates/cyrup-tui/src/keymap.rs:733` is the arm `"clear" => return Err(TuiError::UnsupportedKey("clear".to_string()))`, sited before the `"insert" \| "ins"` arm and well before the generic fallback, and it reports the literal TOKEN rather than `s`, so `"ctrl+clear"` also reads `unsupported key "clear"` instead of blaming the whole chord. **Non-breaking**: `git grep 'invalid key spec' -- crates/` finds only `error.rs` itself, and `TuiError` is never matched exhaustively anywhere in the workspace. **Tests** `crates/cyrup-tui/src/tests/keymap.rs:280` `tui073_clear_is_rejected_with_an_unsupported_key_diagnostic` and `:324` `tui073_a_clear_entry_is_reported_once_and_does_not_break_the_document` — the second pins that one bad entry is reported once and leaves the rest of the document loading. **RE-CLOSED 2026-09-14 — the work is back in the tree, re-verified independently, and the workspace is green.** The reverted batch was re-applied TUI-073-only, its hunks identified by the row-id markers the batch wrote into each comment and cross-checked against the row bodies here. Left behind in `keymap.rs`: `use_windows_keybindings_for` / `use_windows_keybindings`, the whole `Keymap::default` → `Keymap::for_platform(windows_keybindings, is_win32)` rewrite with its platform-gated `app.clipboard.pasteImage` binding and `if !is_win32` `app.suspend` gate, and the `TreeKeymap::default` → `TreeKeymap::for_platform(is_darwin)` rewrite (all TUI-071); the `[CYRUP-DELTA]` blocks on `Action::id`, `EditorAction::from_id` and `AutocompleteAction::from_id` and the rewritten `app.pageUp`/`app.pageDown` comment (TUI-065 / TUI-066, the original TUI-028 comment kept verbatim); and the TUI-066 cyrup-only-token delta block the batch had interleaved INTO this row's own diff region between the `"clear"` arm and the `"insert" \| "ins"` arm, split out by hand. Left behind in `tests/keymap.rs`: the TUI-065 page tests, the whole TUI-066 id-inventory section (`KEYMAP_SRC`, `PI_IDS`, both `tui066_*` tests) and the TUI-071 platform tests — 464 of the snapshot's 540 added lines; only the two `tui073_*` tests (76 lines) were taken. `grep "TUI-065\|TUI-066\|TUI-071\|TUI-N06\|use_windows_keybindings\|for_platform"` over `error.rs`, `keymap.rs` and `tests/keymap.rs` now returns nothing. **upstream, re-read at the tag** — `git -C tmp/pi show v0.85.1:packages/tui/src/keys.ts` puts `\| "clear"` in the `SpecialKey` union at `:119`, between `"insert"` (`:118`) and `"home"` (`:120`); the union actually spans `:109-139` (`\| "f12";` is at `:139`), correcting the `:109-138` span the RE-AUDIT above carries — the load-bearing pin at `:119` is exact either way. Reverse-lookup rows read verbatim at `:429-432` (`"\x1b[E": "clear"`, `"\x1bOE": "clear"`, `"\x1bOe": "ctrl+clear"`, `"\x1b[e": "shift+clear"`), so `ctrl+clear` and `shift+clear` are real upstream specs; the `matchesKey` arm at `:990-994` (`case "clear": if (modifier === 0) { return matchesLegacySequence(data, LEGACY_KEY_SEQUENCES.clear); } return matchesLegacyModifierSequence(data, "clear", modifier);`) means pi genuinely binds AND matches it. The LOW-severity qualifier was re-derived, not inherited: `git show v0.85.1:packages/tui/src/keybindings.ts \| grep -c '"clear"'` returns **0**, so no upstream DEFAULT chord is dead and only a hand-written config reaches this. crossterm-0.29.0 `src/event.rs` `pub enum KeyCode` (`:1226-1288`) was enumerated personally and `grep -n "Clear" src/event.rs` returns nothing, so the row's FIRST Fix option (map it) has no honest target and the second (keep rejecting, say so precisely) is what landed. **cyrup, line pins re-derived from the shipped tree** — the `keymap.rs:733` pin in the reverted closure above came from the abandoned snapshot and is 60 lines high (exactly the excluded TUI-071 hunks). The shipped arm is at **`crates/cyrup-tui/src/keymap.rs:673`**: `"clear" => return Err(TuiError::UnsupportedKey("clear".to_string()))`, sited immediately before the `"insert" \| "ins"` arm at `:674` and well before the single-char fallback and the `_ => Err(TuiError::KeySpec(s.to_string()))` generic arm; it reports the literal TOKEN rather than `s`, so `ctrl+clear` reads `unsupported key "clear"` instead of blaming the whole chord. `crates/cyrup-tui/src/error.rs:42` is `UnsupportedKey(String)` with `#[error("unsupported key \"{0}\"")]`, placed directly after `KeySpec` (`:24-25`), carrying the `[CYRUP-DELTA]` that names `keys.ts:119 @v0.85.1` and crossterm-0.29 `event.rs:1226-1288`. **Non-breaking, re-derived**: `git grep -n "TuiError::" -- crates/ ':!crates/cyrup-tui'` returns nothing — no crate outside `cyrup-tui` matches `TuiError` at all, exhaustively or otherwise — and `git grep -n "invalid key spec" -- crates/` returns only `error.rs:24` plus the two intentional test strings. **Unbound-not-reverted is real behaviour, not a claim**: `merge_entries` (`keymap.rs:119-150`) pushes a `KeybindingIssue { id, reason: e.to_string() }` at `:141-145` and still calls `apply(action, keys)` at `:147` with the surviving (here empty) key list, so the action ends up unbound — pi's own never-matching-`KeyId` outcome — rather than falling back to its `escape` default (a genuine default at `:787`). The reason reaches the user verbatim through `KeybindingIssue`'s `Display` (`keymap.rs:66-70`) and `crates/cyrup/src/interactive.rs:231-241`, and `App::load_keybindings_json` (`app/shell.rs:231-243`) fans the document across six maps but `"app.interrupt"` is owned only by `Keymap::action_from_id` (`keymap.rs:270`), so "reported once" holds at the user-facing level too. **Tests** `crates/cyrup-tui/src/tests/keymap.rs:280` `tui073_clear_is_rejected_with_an_unsupported_key_diagnostic` (covers `clear`, `ctrl+clear`, `shift+clear`, and pins that `clrea` still reads `invalid key spec: clrea` so a typo and an unsupported key stay distinguishable — the row's actual complaint) and `:324` `tui073_a_clear_entry_is_reported_once_and_does_not_break_the_document` (one bad entry, exactly one issue, `app.exit: f7` still applied). **Red-without / green-with, re-run in this tree rather than inherited.** Removing ONLY the `keymap.rs:673` arm and leaving everything else — so the `UnsupportedKey` variant still exists and the crate still compiles, making the failure behavioural rather than a build break — `cargo test -p cyrup-tui --lib tui073` gives `panicked at crates/cyrup-tui/src/tests/keymap.rs:332:5: assertion \`left == right\` failed / left: "invalid key spec: clear" / right: "unsupported key \\"clear\\""`, with `test result: FAILED. 0 passed; 2 failed; 0 ignored; 0 measured; 1432 filtered out`. That `left` value is exactly the generic diagnostic this row objects to. Restoring the arm: `2 passed; 0 failed`. **Workspace gates, all green at the closing tree**: `cargo fmt --all -- --check` exit 0; `cargo clippy --workspace --all-targets -- -D warnings` exit 0; `cargo nextest run --workspace --features test-fixtures` — `Summary [125.959s] 9926 tests run: 9926 passed, 9 skipped`. |
...
| TUI-076 | low | parity-bug | S | The builtin-collision filter keys on `invocation_name`, so a suffixed duplicate (`model:1`) survives into the menu as a command `dispatch_names` will never route — **filed 2026-08-14**; the diagnostic half is TUI-006, do not double-book — **not independently re-checked this pass; left open.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: UNCHANGED ON BOTH SIDES, still open — and the producer was TRACED rather than taken on this row's word.** `CommandRegistry::with_dynamic` (`crates/cyrup-tui/src/commands.rs:336-370`) still builds `existing` from the builtins' `c.name` and then filters the dynamic rows with `.filter(\|c\| !existing.contains(c.name.as_ref()))`; the function has grown a TUI-075 display-order sort since filing, but the filter predicate is untouched. **The producer confirms the field really is the invocation name, and it has MOVED FILE** — this row's `cyrup-session-svc/src/session.rs:2506` is now `crates/cyrup-session-svc/src/session/commands.rs`, where `slash_command_catalog` writes `entry.insert("name".into(), serde_json::Value::from(cmd.invocation_name))` at `:258`. Decisively, the ORIGINAL name is still available on the same object and still not emitted: two lines later the autocomplete opt-in check uses it — `autocomplete_opt_in.contains(&(cmd.owner.as_str().to_string(), cmd.name.clone()))` at `:282` — so `cmd.name` and `cmd.invocation_name` are BOTH in hand where the row is built and only the latter crosses the seam. That makes this row's Fix (carry the original `name` alongside `invocation_name` on the catalog row) a small, well-sited change, and it is not done. Upstream is still the two-step, verbatim: `const builtinCommandNames = new Set(slashCommands.map((c) => c.name));` then `.getRegisteredCommands().filter((cmd) => !builtinCommandNames.has(cmd.name)).map((cmd) => ({ name: cmd.invocationName, ... }))` (`interactive-mode.ts:703-711` @v0.85.1) — filter on the original, map to the invocation name, in that order. The failure stands: two extensions both registering `model` become `model:1`/`model:2`, pi drops BOTH from autocomplete, cyrup keeps both, and `match_command` will never route them (`commands.rs:430-448` matches only against `dispatch_names`, which `with_dynamic` deliberately does not extend — its own comment at `:331-335`), so they fall to `Dispatch::Prompt` and are sent to the model as literal text. The instruction not to double-book with TUI-006 is unaffected. — **PRODUCER HALF NOT DONE, HELD OPEN 2026-09-14.** The consumer filter landed in `cyrup-tui` and is tested (swapping `src/commands.rs` to `git show HEAD:` gives 1 of 2 failing; both pass after), but step 1 of the plan edits `crates/cyrup-session-svc/src/session/commands.rs:258`, outside the working territory, and NOTHING in-tree emits the `registeredName` key today — `unwrap_or(name)` reproduces HEAD behaviour exactly when the key is absent, so a real `model:1` row still survives and the landed filter is INERT in production. To finish: immediately BEFORE the `entry.insert("name", … cmd.invocation_name)` line at `commands.rs:258` insert `if cmd.name != cmd.invocation_name { entry.insert("registeredName".into(), serde_json::Value::from(cmd.name.clone())); }`, plus the producer-side test under `crates/cyrup-session-svc/src/tests/`. The plan's risk note stands and is this row's second obligation: `registeredName` is a cyrup-ORIGINAL addition to the `get_commands` RPC payload (pi's `rpc-mode.ts:682-713` @v0.85.1 emits `invocationName` alone) and must be recorded in `docs/gap-analysis/08-cyrup-session-svc-and-modes.md` alongside the outstanding `argumentHint` record — see TUI-095, which asks for the same entry. |
| TUI-077 | low | parity-bug | S | A slash-argument context falls through to path completion (pi's slash branch is terminal), and the name/argument split uses Unicode whitespace where pi splits on a literal space — **filed 2026-08-14**; land with TUI-012 — **not independently re-checked this pass; left open.** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: NARROWED BUT NOT CLOSED.** DIVERGENCE (2), THE SPLIT RULE — **CLOSED**: both split sites are now literal-space, `slash_context` gating on `if before.contains(' ')` (`autocomplete.rs:271`) and `argument_completer` doing `let space = rest.find(' ')?` (`:292`), so a tab-containing `/mo<TAB>del` stays in the command-name branch, matching pi's `textBeforeCursor.indexOf(" ")` (`autocomplete.ts:314` @v0.85.1); that half of the Verify is satisfiable. DIVERGENCE (1), THE FALLTHROUGH — **STILL OPEN, narrowed**: `Autocomplete::compute` (`autocomplete.rs:148-182`) now has a terminal guard, but a CONDITIONAL one — after `slash_context` misses it returns `None` only `if argument_completer(registry, &before).is_some()` (`:174-176`) — and the code states the remaining hole itself at `:177-178` ("A `/`-line with NO completer still falls through, which is a pre-existing cyrup deviation and the reason `/export ./sr<Tab>` completes a path."). Both of this row's observable examples therefore still reproduce, each traced end to end: `/export ./sr` — space present, `argument_completer` returns `None` (export has `ArgumentCompleter::None`), the guard does not fire, `path_context` (`:563-610`) takes `./sr`, `looks_pathy` is true via the `.` prefix and a FILE list pops, where pi returns `null` at `autocomplete.ts:351-353`; and `/Users/dav` — no space, `command_name_context` (`:507-538`) returns `None` on empty fuzzy at `:511-513`, the guard does not fire, `path_context` sees `/` and a path list pops, where pi returns `null` at `:336`. Upstream's branch is unconditionally terminal at v0.85.1: every path out of `if (!options.force && textBeforeCursor.startsWith("/"))` (`autocomplete.ts:313-363`) returns — `:336`, `:337-341`, `:351-353`, `:356-357`, `:358-362` — and `extractPathPrefix` at `:365` is reached only outside it. **This row's landing note is now partly overtaken:** TUI-012's argument branch HAS landed (`CompletionContext::SlashArgument` at `autocomplete.rs:330`, three builtin completers plus the extension tag at `:310-334`), so what is left is the narrow no-completer case alone and can be fixed independently — make the guard unconditional on `before.starts_with('/') && !force`. — **ATTEMPTED AND HELD OPEN 2026-09-14.** The guard landed exactly as the Fix words it — `crates/cyrup-tui/src/autocomplete.rs:197` is `if !force && before.starts_with('/') { return None; }`, after the slash arm (`:172`) and the completer-owned terminal arm (`:180`) and before `path_context` (`:200`) — and it is genuinely load-bearing: replacing it with `let _ = force;` fails `a_slash_line_with_no_argument_completer_does_not_fall_through_to_paths` (`crates/cyrup-tui/src/tests/autocomplete.rs:576`), `an_absolute_path_on_a_slash_line_closes_the_popup` (`:587`), `a_tab_separated_slash_line_stays_terminal` (`:618`) and the TUI-057 test at `:639`. Divergence (2) is confirmed still closed (`:283` `before.contains(' ')` and `:304` `let space = rest.find(' ')?`, matching `textBeforeCursor.indexOf(" ")` at `autocomplete.ts:314` @v0.85.1), and there is no `@`-mention regression because `compute_mention()` runs before BOTH `Autocomplete::compute` call sites (`editor/completion.rs:88`, `:143`), mirroring pi's `extractAtPrefix` branch at `:298-311`. It is held open because the guard lands on top of a MISSING piece of pi's editor and thereby replaces the filed divergence with a new, proven behavioural regression against v0.85.1 that the added tests are shaped not to catch; secondarily, four of the six upstream line citations in the new comment block are wrong, and this row is carried entirely by line citations. **RE-RUN 2026-09-16 at `cc7818b` — still open, unchanged.** Re-read because `keymap.rs` changed in this window; the change is `TUI-073`'s `clear` arm and does not touch the slash-argument completion path or the name/argument split. |
...
| TUI-095 | low | cyrup-original | — (record-only) | **The persistent command-token highlight and the argument-hint ghost are cyrup-original chrome pi does not render, and until now had no row — only the ad-hoc marker `CMDHINT_01`.** Landed at `0b7c4f4` + `bae24f5` (2026-08-17). **What ships.** While the first buffer line is a real command prefix, the `/name` token stays highlighted after the user types past it, and the command's declared `argument-hint` is drawn as dim ghost text after the space: `InputEditor::command_highlight` (`crates/cyrup-tui/src/editor.rs:238` doc, `:255` fn), the zone splitter `:2196`, the restructured cursor overlay `:2242`, the clipped ghost span `:2311`, and the matcher `autocomplete.rs:177` (`is_command_prefix`). **Verified cyrup-original, not drift.** The code says so in its own words at `editor.rs:211-213` — "CMDHINT_01 — cyrup-original; pi renders neither. Its `argumentHint` reaches exactly one site upstream, the popup description at `tui/src/autocomplete.ts:315`" — corroborated by `spec/CMDHINT_01.md:10-15`, which records that pi's `packages/tui/src/components/editor.ts` and coding-agent's `custom-editor.ts` were both read and do neither half. **One deliberate design choice worth pinning so it is not "fixed" later:** the matcher is NOT the fuzzy one `slash_context` uses. `autocomplete.rs:170-176` states the reason — fuzzy is right for a suggestion LIST and wrong for a signal that claims "what you typed is literally the start of a real command", because `/fa` fuzzy-matches `flux/aug` and must not be highlighted as a real path segment, and a bare `/` returns everything. **Why a row rather than nothing:** `cyrup-original` is a first-class counted class rated by reachability, and this is maximally reachable — it changes what every user sees on every keystroke of every slash command. It is also the `EXT-M03` situation the README legislated for: `CMDHINT_01` is cited thirteen times in shipped source (`cyrup-session-svc/src/session.rs:2628`; `cyrup-tui/src/editor.rs:26`, `:212`, `:238`, `:2196`, `:2242`, `:2311`, `:2945`; `autocomplete.rs:170`; `commands.rs:470`; plus three test files) with no ledger row, so `grep CMDHINT` resolved to nothing. **`CMDHINT_01` is the in-source marker; `TUI-095` is the id** — do not adopt the ad-hoc spelling into the ledger. **Two loose ends, neither a defect in this row:** `spec/CMDHINT_01.md`'s frontmatter still reads `stage: new, status: todo` although both commits landed, and the same change added an `argumentHint` key to the `get_commands` RPC payload that pi's `rpc-mode.ts` never emits — **that belongs in area 08 and is NOT recorded there** (`grep -n 'argumentHint\|CMDHINT' 08-cyrup-session-svc-and-modes.md` → zero hits), which is exactly what `TUI-078`'s own Fix instructed. — **FILED 2026-08-19 (surface sweep of the 27-commit round-2 batch)** — **RE-AUDIT 2026-09-14 — re-read at `9aeba769` against v0.85.1: STILL OPEN AS FRAMED (record-only); the core classification is re-confirmed against the CURRENT tag and BOTH named loose ends are unchanged.** CLASSIFICATION RE-CONFIRMED — the part that could have decayed, since `cyrup-original` is only safe if upstream has not since grown either half: `packages/tui/src/components/editor.ts` and `packages/coding-agent/src/modes/interactive/components/custom-editor.ts` at v0.85.1 both return ZERO hits for ghost / placeholder / argumentHint / commandHighlight / highlight, so pi renders neither the persistent command-token highlight nor the argument-hint ghost anywhere across the v0.83.0..v0.85.1 window, and the in-source claim at `editor/mod.rs:262` ("cyrup-original; pi renders neither") survives. THE FEATURE STILL SHIPS, at RELOCATED sites: `InputEditor::command_highlight` is `crates/cyrup-tui/src/editor/completion.rs:22-73`, the zone splitter `editor/render.rs:52`, the cursor overlay `editor/render.rs:100`, the clipped ghost span `editor/render.rs:172`, the matcher `autocomplete.rs:544-557` (`is_command_prefix`). The deliberate design choice this row asked to pin is intact and still argued in place at `autocomplete.rs:544-553` — the matcher is a literal `starts_with` prefix test, NOT `fuzzy::filter`, because `/fa` fuzzy-matches `flux/aug` and a bare `/` returns everything. **Do not "fix" it into the fuzzy one.** LOOSE END 1 — UNCHANGED: `spec/CMDHINT_01.md` frontmatter still reads `stage: new` / `status: todo` / `updated: 2026-08-16 22:10`, although both commits landed. LOOSE END 2 — UNCHANGED, and this row's one live obligation: `git show 9aeba769:docs/gap-analysis/08-cyrup-session-svc-and-modes.md \| grep -n 'argumentHint\\|CMDHINT'` returns ZERO, so the `argumentHint` key is still a cyrup addition to the `get_commands` RPC payload with no record in the area that owns that payload — it is emitted at `crates/cyrup-session-svc/src/session/commands.rs:299-303`, so area 08 still carries an unrecorded RPC-shape delta that a future parity pass there will read as drift. That is exactly what TUI-078's Fix instructed and what this row was created to carry. **ONE CORRECTION TO THIS ROW'S OWN TEXT (not a verdict change):** its thirteen in-source citations are listed against `cyrup-tui/src/editor.rs:26, :212, :238, :2196, :2242, :2311, :2945` — that file no longer exists, having been split into `editor/`; `git grep 'CMDHINT_01' 9aeba769 -- crates/` finds thirteen live references, at the new paths above. — **NO CODE; LEFT OPEN 2026-09-14 because its ONE live obligation is a record in ANOTHER area's file.** The classification was re-confirmed at the current tag, since `cyrup-original` decays if upstream grows the feature: `git -C tmp/pi show v0.85.1:packages/tui/src/components/editor.ts` and `…:modes/interactive/components/custom-editor.ts` — neither renders a persistent command-token highlight nor argument-hint ghost text. The feature ships at the RELOCATED sites: `InputEditor::command_highlight` (`crates/cyrup-tui/src/editor/completion.rs:21-73`), the matcher `is_command_prefix` (`crates/cyrup-tui/src/autocomplete.rs:544-557`, a literal `starts_with` over `registry.commands()` with its own argument for why it is not `fuzzy::filter`), and the render sites in `crates/cyrup-tui/src/editor/render.rs`; `git grep -c CMDHINT_01 -- crates/` returns thirteen live references across ten files. TWO LOOSE ENDS REMAIN, both outside `crates/`: (1) `docs/gap-analysis/08-cyrup-session-svc-and-modes.md` must record that cyrup's `get_commands` payload carries cyrup-original keys pi's `rpc-mode.ts:682-713` @v0.85.1 never emits — `argumentHint` (`crates/cyrup-session-svc/src/session/commands.rs:299-303`, consumed by `cyrup-tui/src/commands.rs dynamic_commands_from_catalog_gated`) and `argumentCompletions` (`:283-285`) — with the reason (cyrup's interactive registry is built from this catalog, pi's is not), so a future area-08 parity pass does not read them as drift; that file has exactly one `argumentHint` hit, at `:2394`, and it is about `/model`'s hint drift, not the RPC key. If TUI-076 lands, fold `registeredName` into the SAME entry rather than filing a second one. (2) `spec/CMDHINT_01.md` frontmatter still reads `stage: new` / `status: todo` / `updated: 2026-08-16 22:10` although both commits (`0b7c4f4`, `bae24f5`) landed. Also correct this row's own stale citation list: `cyrup-tui/src/editor.rs:26, :212, :238, :2196, :2242, :2311, :2945` no longer exists, the file having been split into `editor/`. Corroborating read this pass: `argumentHint` reaches exactly one upstream consumer, the popup description composed at `packages/tui/src/autocomplete.ts:319-326` @v0.85.1, which cyrup mirrors at `autocomplete.rs:520-526`. |
| TUI-098 | low | upstream-drift | S | Zed is not recognised as a capable terminal: cyrup falls through to the conservative profile (no OSC-8 hyperlinks, truecolor only via `COLORTERM`) where pi v0.85.0+ grants `{trueColor, hyperlinks}` — **filed 2026-09-24**, promoted from the 2026-09-14 lead list; both sides re-read at `ea23ca2` / v0.87.1 |
| TUI-099 | low | upstream-drift | S | `markdown/latex.rs` (a v0.84.1 port) lacks the seven relational-algebra join symbols (v0.85.0) and the legacy font switches `\bf \cal \it \rm \sf \sl \tt` (v0.86.0), so expressions using them fall back to raw source — **filed 2026-09-24**; the centred-`cases` and vertical-script halves of the same v0.86.0 entry are NOT in this row (cyrup side unread) |
| TUI-100 | low | upstream-drift | S | The `/` menu fuzzy-ranks skills on the full `skill:<name>` string, so typing a skill's bare name ranks it on the `skill:` prefix — pi v0.86.0 ranks on the bare name unless the query itself starts with `skill:` — **filed 2026-09-24** |
...
| ~~TUI-104~~ | ~~high~~ **CLOSED 2026-09-24** | upstream-drift | S | `/tree` navigation is not refused while a compaction (or another summarizing `/tree`) runs: neither the UI (`app/tree_nav.rs:107`) nor `navigate_tree` (`cyrup-session-svc/src/session/forking.rs:114`; session half owned by area 08's `SEAM-124`) checks `is_compacting()`, so the compaction entry is appended under the NEW leaf with a `first_kept_entry_id` on the abandoned branch, and the rebuilt context keeps nothing before it — pi v0.86.0 throws in `navigateTree` and re-checks in the UI (#9179) — **filed 2026-09-24 (second pass)**; static, race not observed — **CLOSED 2026-09-24** (`0820d97`): `app/tree_nav.rs::begin_tree_navigation` re-checks `is_compacting()` before the branch-summary indicator would replace another operation's and shows pi's error; its stop-the-response step now uses `is_run_active` + `abort_and_settle` (pi's awaited `abort()`). The session half landed with it as `SEAM-124`, so the severity disagreement no longer needs reconciling. Verify: `tests/tree_branch_summary.rs` `tree_navigation_is_refused_while_a_compaction_runs` (red with the check removed). |
...
>
> **SECOND CORRECTION 2026-09-05 (batch 4), code `836c6006` — the founding defect had recurred a
> third time, and the residual is now measured rather than argued. Still open (low).**
>
> **(a) The drift came back where the item said it would.** TUI-046 exists because
> `keyboard_protocol.rs`'s module doc described `ESC [ > 7 u` while the caller pushed `ESC [ > 1 u`.
> `8bb0d22f` single-sourced the VALUE into `DESIRED_FLAGS` but left the PROSE at
> `App::into_stdout`'s push site restating the delta — "minus `REPORT_EVENT_TYPES`" — and the
> `824a539e` bit-4 revert widened the withheld set from one bit to two without touching it. From
> 2026-09-05 until `836c6006` that site therefore documented `ESC [ > 5 u` at the exact line that
> writes `ESC [ > 1 u`: the item's own defect, third occurrence, introduced by the item's own fix.
> A single-sourced constant does not help if the neighbouring comment re-states what it means.
> The delta is now a SUBTRACTION the compiler performs — `PI_DESIRED_FLAGS`
> (pi's `DESIRED_KITTY_KEYBOARD_PROTOCOL_FLAGS = 7`, `packages/tui/src/terminal.ts:15` @v0.84.4)
> and `WITHHELD_FLAGS = PI_DESIRED_FLAGS.difference(DESIRED_FLAGS)`, never written out — and
> `app/crossterm.rs` names those constants and no individual bit at all.
> Tests: `keyboard_protocol.rs::the_push_site_does_not_restate_the_flag_set_it_pushes`, RED before
> (the pre-change `app/crossterm.rs` was restored and the test re-run: 1 failed) and green after;
> `::the_withheld_set_is_exactly_pis_flags_minus_cyrups`, red in the strongest sense (neither
> constant existed). `cargo nextest run -p cyrup-tui` 1422 passed; `cargo fmt --all -- --check`,
> `clippy -p cyrup-tui --all-targets -D warnings` and `RUSTDOCFLAGS='-D warnings' cargo doc -p
> cyrup-tui` all clean.
>
> **(b) The residual is unportable — measured, not read.** The 2026-09-04 argument that
> `pendingKittyPrintableCodepoint` cannot be expressed at cyrup's seam was drawn entirely from
> crossterm's source. It has now been run. crossterm 0.29's real `event::read()`, driven through a
> pty (`python3` `pty.fork`, probe enabling raw mode and printing every `Event::Key`):
>
> | bytes written to the pty master | events crossterm delivered |
> |---|---|
> | `\x1b[224u` + `\xc3\xa0` in one write (pi `packages/tui/test/stdin-buffer.test.ts:284-287`) | `Char('à') NONE Press NONE`, `Char('à') NONE Press NONE` |
> | the same, split across two writes 350 ms apart (shape of pi's cross-chunk case, `:289-293`) | identical to the row above |
> | `ll` in one write | `Char('l') NONE Press NONE`, `Char('l') NONE Press NONE` |
> | `\x1b[97u` + `b` (`:295-298`) | `Char('a') NONE Press NONE`, `Char('b') NONE Press NONE` |
> | `\x1b[64;3u` + `@` (`:300-303`) | `Char('@') ALT Press NONE`, `Char('@') NONE Press NONE` |
>
> Rows 1 and 3 are the same two events. The duplicate pi drops and an ordinary double letter are
> **the same bytes** at this seam, so any event-level form of the guard eats the second `l` of
> `hello` — over SSH, where two real keystrokes routinely batch into one `read(2)`, a timing or
> same-burst heuristic does not save it either. Rows 4 and 5 are the two cases pi's guard must NOT
> fire on, and cyrup already gets both right with no guard at all (distinguishable by codepoint and
> by modifier respectively). **The measurement confirms the row rather than closing it:** the
> doubling hazard is real at cyrup's seam and pi fixes it, so the gap stands; what changed is that
> "unportable" is now an observation. Closing it still needs the byte-level pre-parser
> `TUI-045`/`TUI-047`/`TUI-050` share, and that is a replacement of crossterm's input layer
> (`event::poll`/`read` own the tty fd, `parse_event` is `pub(crate)`, and the source cannot be
> injected: `UnixInternalEventSource::from_file_descriptor` is `pub(crate)`
> (`crossterm-0.29.0/src/event/source/unix/mio.rs:40`) and `tty_fd()` chooses fd 0 or `/dev/tty`
> on its own, `terminal/sys/file_descriptor.rs:124-139`), i.e. resize/mouse/focus/paste decoding
> as well as keys — far outside this item's effort M. It was NOT attempted this pass and no half
> of it was landed; a partial dedup covering only the split-sequence path `escape_reassembly`
> already owns the bytes for would be arbitrary coverage, not a port, and was rejected for that.
> A real kitty/ghostty/WezTerm run is still owed and is still unavailable in this environment; the
> pty measurement replaces the crossterm-source reading, not that live run.
>
...
## ~~TUI-N05~~ — ~~Extension shortcuts can never override a built-in key; no conflict reported~~ **CLOSED 2026-09-14**
...
## ~~TUI-N14~~ — ~~`get_cursor_position_returns_the_tracked_anchor_without_querying_the_terminal` bounds `Instant::elapsed()` at 250 ms, asserting a scheduling outcome it cannot control~~ **CLOSED 2026-09-14 (landed `7e41cf9`) — FILED RETROACTIVELY 2026-09-16**

**Kind** test-defect · **Severity** low · **Effort** S · **Confidence** confirmed · **Status** **CLOSED 2026-09-14**, landed at `7e41cf9` (`#140`); filed retroactively 2026-09-16 during the `cc7818b` reconciliation

**cyrup (pre-fix)** — `crates/cyrup-tui/src/app/backend.rs:358-366` — `let started = std::time::Instant::now();` … `assert!(started.elapsed() < std::time::Duration::from_millis(250), "get_cursor_position took {:?} — a backend that ANSWERS never approaches crossterm's 2000ms DSR timeout", started.elapsed())`. The assertion's premise is stated in the message and is the thing that is wrong: it treats *elapsed wall-clock time* as a proxy for *did an I/O round-trip happen*. Nothing on the path does I/O — that part of the reasoning was correct — but a bound on wall-clock time is a bound on the SCHEDULER, and `cargo nextest` runs one process per test against a shared CPU pool. The fix commit's own replacement comment records the observed consequence: *"under a full-workspace `cargo nextest` run this process competes with ~10 others, and the assertion lost that race repeatedly across sessions — a red that never indicated a defect and cost a re-run every time."*

**cyrup (at `cc7818b`)** — `crates/cyrup-tui/src/app/backend.rs:359-380` — the buffer is cloned before the call (`let buf = SharedBuf::new(); let mut backend = InlineBackend::with_anchor(buf.clone(), anchor);`), and the assertion is now over the BYTES: `let written = String::from_utf8(buf.bytes()).expect("the backend writes UTF-8"); assert!(!written.contains("\x1b[6n"), "get_cursor_position issued a DSR cursor-position request instead of answering from its tracked anchor: {written:?}")`. `assert_eq!(got, anchor)` is unchanged.

**upstream** — none, and the absence is structural rather than an omission. `TUI-093` records the decision in full: pi drives its TTY writer directly and never asks the terminal where the cursor is, so `CrosstermBackend::get_cursor_position`'s `CSI 6 n` round-trip — and therefore both this test and the defect it guards — exist only in the Rust port's ratatui seam. There is no pi test to port and no pi behaviour to compare against.

**Impact** — two costs, and the second is the one that justifies a row for an already-fixed flake. (1) A recurring red in the workspace gate that indicates no defect, on a suite (`cargo nextest run --workspace --features test-fixtures`) whose green is the precondition for every other closure in this directory. (2) It was the guard on `TUI-093`, a closed `high` — *"A commit flush, a window resize or a live-region height change stalls and then ENDS THE SESSION with `cursor position could not be read`"* — whose entire fix is that the backend answers from a tracked anchor instead of querying. An assertion that goes red for reasons unrelated to the property it guards teaches the reader to re-run rather than investigate, and it does so on the one assertion that would catch a real regression of that fix. This is the fourth instance of the class in this area (`TUI-N09` exact countdown, `TUI-N11` ambient `TERM_PROGRAM`, `TUI-N13` long `TMPDIR`), and the first whose proxy was time rather than environment.

**Fix** — *applied at `7e41cf9`.* Assert the property, not a proxy for it. The DSR request is the three-byte-plus sequence `ESC [ 6 n`; a backend that answers from its tracked anchor never writes it, and one that asks necessarily does — so its absence in the write buffer is exactly the claim the test's own name makes, and it is deterministic on every host at every load. The 23-line hunk also carries an in-source comment explaining why the timing bound was wrong, which is what makes the closure legible without this row; the row exists because the commit that carries it is titled for unrelated subagents work.

**Verify** — mutation-checked by construction rather than by wall clock: restore `CrosstermBackend`'s querying `get_cursor_position` and the assertion fails on the emitted `\x1b[6n`, on any host, at any load — whereas the 250 ms bound would fail only probabilistically and only on a fast machine. The pre-fix assertion's failure mode is not reproducible on demand, which is precisely the property that made it a defect.

**Ledger note, recorded because it is the recurring mechanism and not a one-off.** This is the second time area 07 has had work land with no row (`TUI-093`, 2026-08-17, was the first and was also filed retroactively). Both were found by reading the CODE for what moved, not by reading the ledger. `00-residual-ledger.md`'s standing rule — *grep the SOURCE for `AREA-NNN` citations at every reconciliation* — would not have caught this one, because the fix commit cites no item id at all: there was none to cite. The generalisation is that a reconciliation must diff the code over the window as well as grep it for ids.
...
## ~~TUI-S10~~ — ~~Shift+Ctrl+D global debug chord absent~~ **CLOSED 2026-09-14**
...
## ~~TUI-060~~ — ~~The wrap/visual-line map is not paste-marker aware, so a marker can be split across visual rows~~ **CLOSED 2026-09-14**
...
## ~~TUI-072~~ — ~~four editor key sets carry their v0.84.1 extras against a v0.83.0 baseline~~ **CLOSED 2026-09-14**
...
## ~~TUI-073~~ — ~~`clear` is a valid pi `KeyId` that `Key::parse` rejects~~ **CLOSED 2026-09-14**
...
## TUI-098 — Zed is not recognised as a capable terminal

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (static read, both sides, 2026-09-24)
**cyrup**    — `crates/cyrup-tui/src/image.rs:771` `detect_capabilities_on_platform` at `ea23ca2`: the chain is kitty → ghostty → wezterm → warp → iterm → `:825` `if has("WT_SESSION") || term_program == "vscode" || term_program == "alacritty"` → jetbrains → Windows console → `conservative(has_true_color)` (`:552`). `rg -i zed crates/cyrup-tui/src/image.rs` → zero. Under `TERM_PROGRAM=zed` cyrup lands on `conservative`: hyperlinks off, truecolor only when `COLORTERM` is `truecolor`/`24bit`.
**upstream** — `git -C tmp/pi show v0.87.1:packages/tui/src/terminal-image.ts`, `:113` `if (termProgram === "alacritty" || termProgram === "vscode" || termProgram === "zed")` → `{ images: null, trueColor: true, hyperlinks: true }`. Absent at v0.84.3 (`grep -c zed` → 0); landed v0.85.0 (#8828).
**Impact**   — In Zed's integrated terminal, OSC-8 links are never emitted and colours may drop to 256 where pi renders both.
**Fix**      — add `|| term_program == "zed"` to the `:825` arm; extend the capability-matrix test beside it with a `zed` case.
**Verify**   — unit test: `TERM_PROGRAM=zed`, no `COLORTERM` → `hyperlinks: true, true_color: true, images: None`.

## TUI-099 — `latex.rs` lacks the join symbols and the legacy font switches

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed for the two halves named; one-sided for the rest of the v0.86.0 entry
**cyrup**    — `crates/cyrup-tui/src/markdown/latex.rs` declares itself a port of `latex.ts` **v0.84.1** (`:1`). `rg 'bowtie|ltimes|rtimes|outerjoin|"Join"'` over it → zero, so none of the seven names is in `SYMBOLS` (`:39`). `IGNORED_COMMANDS` (`:531-538`) holds six style commands and no font switch; `rg '"bf"|"rm"|"sl"|FONT_SWITCH'` → zero. An unknown command reaches the tail of the command parser, `self.supported = false; format!("\\{command}")` (`:1358-1359`), which drops the whole expression back to raw source.
**upstream** — `git -C tmp/pi show v0.87.1:packages/tui/src/latex.ts`: `:66-72` `bowtie`/`Join` ⋈, `ltimes` ⋉, `rtimes` ⋊, `leftouterjoin` ⟕, `rightouterjoin` ⟖, `fullouterjoin` ⟗ in the symbol table, and the same names in the command list (`:305`, `:310`, `:313`, `:334`, `:342`, `:362`, `:365`) — v0.85.0, #9050. `:526` `FONT_SWITCH_COMMANDS = new Set(["bf", "cal", "it", "rm", "sf", "sl", "tt"])` and `:1050-1055` consume the switch plus trailing whitespace and emit nothing — v0.86.0, #8827. Both absent at v0.84.3 (`grep -c` → 0 each).
**Impact**   — `$R \bowtie S$` and `${\bf x}$` render as raw LaTeX source in cyrup, as Unicode in pi.
**Fix**      — add the seven `SYMBOLS` rows and command-list names; add a `FONT_SWITCH_COMMANDS` set checked before `IGNORED_COMMANDS` that skips whitespace and returns `String::new()`.
**Verify**   — renderer tests for `R \bowtie S` → `R ⋈ S` and `{\bf x}` → `x`.
**Not in this row** — the same v0.86.0 CHANGELOG line also fixes centred `cases` layouts (#9564) and vertical layout of unsupported/nested display scripts (#7929; `ScriptNode`, `parseScripts`, `scriptDepth`). Upstream was read; cyrup's script path was not, so those are a lead in the 2026-09-24 census, not part of this item. The 2026-09-14 lead's control-space / newline half (v0.84.2) is also still unread on the cyrup side.

## TUI-100 — Skill commands are fuzzy-ranked on their `skill:` prefix

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (static read)
**cyrup**    — `crates/cyrup-session-svc/src/session/commands.rs:316` names every skill row `format!("skill:{}", s.name)`; `crates/cyrup-tui/src/autocomplete.rs:507` `command_name_context` then runs `fuzzy::filter(commands, query, |c| c.name.as_ref())` (`:510`) over those full names. Typing `/review` scores `skill:review` with the match starting at index 6 behind a `:` boundary, so it pays the gap and late-position penalties against every builtin and prompt that matches earlier.
**upstream** — `git -C tmp/pi show v0.87.1:packages/tui/src/autocomplete.ts`, `:335-338`: the key function is `!prefix.startsWith("skill:") && item.name.startsWith("skill:") ? item.name.slice("skill:".length) : item.name` — the bare name is scored unless the user typed `skill:` themselves. The inserted value is still `item.name`. Absent at v0.84.3 and v0.85.1 (`grep -c 'skill:'` → 0); v0.86.0, #9120.
...
**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed (static read)
**cyrup**    — `crates/cyrup-tui/src/autocomplete.rs:560` `PATH_DELIMS = [' ', '\t', '"', '\'', '=']`; `trailing_token` (`:666`) splits only on those, `is_token_start` (`:631`) accepts only index 0 or a `PATH_DELIMS` predecessor, and `mention_query` (`:725`) requires the trailing token to begin with `@`. The mention completion quotes only on `path.contains(char::is_whitespace)` (`:233`). So in `看看，@src` the token is `看看，@src`, `strip_prefix('@')` fails and no popup opens; a completed path containing `，` is inserted unquoted.
**upstream** — `git -C tmp/pi show v0.87.1:packages/tui/src/utils.ts:57-63` adds `cjkPunctuationRegex`, `autocompleteSeparatorRegex` (whitespace or CJK punctuation) and `autocompleteBoundaryRegex`; `autocomplete.ts:47-57` `findLastDelimiter` and `:75-77` `isTokenStart` treat a separator as a delimiter, `:116` quotes when `autocompleteSeparatorRegex.test(path)`, `:516` offers an empty prefix after CJK punctuation; `components/editor.ts:255-274` rebuilds the trigger/debounce patterns on the same boundary and `:1240` auto-triggers on CJK characters. None of the three symbols exists at v0.84.3 (`grep -c autocompleteSeparator` → 0); v0.86.0, #9746.
**Impact**   — CJK-language users get no `@`/path completion directly after `，`/`。`-class punctuation, and a completed path containing such punctuation is inserted where it will be re-split.
**Fix**      — port the three regexes (Unicode `Punctuation` ∩ the `cjkBreakRegex` scripts, plus the explicit list) into `autocomplete.rs`; use them in `trailing_token`, `is_token_start`, the mention quoting at `:233`, and the editor's trigger check.
**Verify**   — `看看，@sr` opens the mention popup; completing `a，b.md` inserts `@"a，b.md"`.
**Neighbouring, NOT filed:** the bare-path arm `CompletionContext::Path` (`autocomplete.rs:218-222`) inserts `completion.value` with no quoting at all, where upstream's `buildCompletionValue` (`:116`) quotes non-`@` paths too. Seen while reading for this item and not traced further — a lead, since it predates this window.

## TUI-102 — `/copy` reports success when no clipboard received the text

**Kind** upstream-drift · **Severity** medium · **Effort** M · **Confidence** confirmed (static read; not observed live)
**cyrup**    — `crates/cyrup-tui/src/clipboard.rs` ports `copyToClipboard` as of v0.84.x. `copy_to_clipboard` (`:213`) runs the plan, then `if osc52_required(env.remote, copied)` — `remote || !copied` (`:158-160`) — writes OSC 52 and sets `copied = true`. So on a desktop Linux session (`DISPLAY` set) where `xclip`/`xsel` are missing or fail, cyrup emits OSC 52, returns `true`, and `app/execute_misc.rs:1000-1003` prints `copied last message (N chars)` whether or not the terminal honoured OSC 52. On failure the only message is the generic `Failed to copy to clipboard` (`:1009`). `rg -i wsl crates/cyrup-tui/src/clipboard.rs` → zero.
**upstream** — `git -C tmp/pi show v0.87.1:packages/coding-agent/src/utils/clipboard.ts`, `copyToClipboard` `:74-137`: `:118` `const headless = p === "linux" && !env.DISPLAY && !env.WAYLAND_DISPLAY && !env.TERMUX_VERSION`; `:120-123` OSC 52 is emitted only `if (!osc52Emitted && (isRemoteSession(env) || (!copied && headless)))`, with the comment "OSC 52 cannot be verified, so a desktop session with a display reports the failure instead (#9618)"; `:124-137` throws actionable messages — `text exceeds the OSC 52 size limit`, `install the Termux:API app…`, `install wl-clipboard…`, `install xclip or xsel, or check X11 access`. `:29-52` `copyViaWindowsClipboard` and `:110-114` add a WSL path (OSC 52 under `WT_SESSION`, else PowerShell `Set-Clipboard` via `wslpath`). At v0.85.1 the gate was `if (remote || !copied)` (`:167`) — exactly cyrup's. v0.86.0 (#9618) and v0.86.1 (#9688).
...
**cyrup**    — `crates/cyrup-tui/src/status_indicator.rs:1-6`: a fixed 2-row band between the active turn and the editor for all four kinds (`working | retry | compaction | branchSummary`), cited @v0.84.2. `rg -il 'in_border|render_in_border'` over `crates/cyrup-tui/src` → zero; the editor's top border never carries a status.
**upstream** — `git -C tmp/pi show v0.87.1:packages/coding-agent/src/modes/interactive/components/custom-editor.ts`: `embedWorkingStatus` (`:5-8`, "Render working, compaction, summarization, and retry status in the editor's top border") and `renderTopBorder` (`:36-80`), which draws `── <status> ───`, degrades to the spinner alone (`renderSpinnerInBorder`) when the `↑ N more` overflow label or the width does not fit. `components/status-indicator.ts:24-31` puts `renderInBorder`/`renderSpinnerInBorder` on the `StatusIndicator` base (at v0.85.1 they were on `WorkingStatusIndicator` only). `interactive-mode.ts:603-607` builds the default editor with `embedWorkingStatus: true`, and `showStatusIndicator` (`:2215-2225`) hands the indicator to the editor and adds it to `statusContainer` only when the editor cannot embed it (a custom extension editor). Absent at v0.84.3 (`embedWorkingStatus` → 0); working indicator v0.85.0 (#8799), the other three v0.86.0.
...
**Kind** upstream-drift · **Severity** high · **Effort** S · **Confidence** confirmed static read of both sides; the race itself was not driven live · **CLOSED 2026-09-24** (`0820d97`): `app/tree_nav.rs::begin_tree_navigation` re-checks `is_compacting()` before the branch-summary indicator would replace another operation's and shows pi's error; its stop-the-response step now uses `is_run_active` + `abort_and_settle` (pi's awaited `abort()`). The session half landed with it as `SEAM-124`, so the severity disagreement no longer needs reconciling. Verify: `tests/tree_branch_summary.rs` `tree_navigation_is_refused_while_a_compaction_runs` (red with the check removed).
