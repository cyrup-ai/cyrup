...
| TUI-088 | **high** | not-ported | M | **Ctrl+C does not work in the running TUI** — reported from live use 2026-08-15, not from the backlog. `grep "Char('c')" crates/cyrup-tui/src/` finds a binding only in `selector.rs` (overlay-local) and one constructed `KeyEvent` in `app.rs`; there is **no global Ctrl+C destination in `keymap.rs`**, where every other chord family is declared. Establish pi's behaviour FIRST — upstream distinguishes interrupt-the-turn from exit-the-app and the two are bound differently — then port the mechanism. **This is user-facing and blocks basic operation: a user with a running turn has no way to interrupt it.** Note `TUI-067`/`TUI-068` are the same class (chord parses, destination missing), so check whether this is a third instance of that root cause rather than an isolated binding. — **FILED 2026-08-15 (live use)** |
