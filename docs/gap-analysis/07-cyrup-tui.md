...
| TUI-067 | low | parity-bug | S | `tui.input.copy` migrates correctly and is then silently dropped by `merge_entries`: `EditorAction::from_id` has no arm, so pi's one "do not consume this key" id is inert — **filed 2026-08-14**; named only inside TUI-028's prose (`:692`), never in its Fix, so its closure did not cover it — **2026-08-15 (TUI-092 §4): `TUI-088` was checked as a possible THIRD instance of this class ("chord parses, destination missing") and is confirmed NOT one** — Ctrl+C is bound (`keymap.rs:656`), id-resolvable (`:268`) and wired (`app.rs:2440-2453`) at HEAD — so the class stays these two rows and is **not** re-swept. **Shared root cause with `TUI-068`, stated once:** `merge_entries` silently skips any id `from_id` does not know — `crates/cyrup-tui/src/keymap.rs:128`, `let Some(action) = from_id(&id) else { continue };`, which is upstream's deliberate `if (!(keybinding in this.definitions)) continue` (`packages/tui/src/keybindings.ts:172-179`) — so an id present in a `Default` table but missing from the matching `from_id` is **unbindable and reports nothing**, not even a `KeybindingIssue`. |
| TUI-068 | medium | not-ported | S | `app.session.deleteNoninvasive` is unbound **and** unbindable — Ctrl+Backspace in `/resume` hits a modifier-blind `Backspace` catch-all and does nothing — **filed 2026-08-14**; the chord parses, so this is a missing destination, not a dead spec — **2026-08-15 (TUI-092 §4): `TUI-088` was checked as a possible THIRD instance of this class and is confirmed NOT one** — Ctrl+C is bound (`keymap.rs:656`), id-resolvable (`:268`) and wired (`app.rs:2440-2453`) at HEAD — so the class stays this row and `TUI-067`, and is **not** re-swept. **Shared root cause with `TUI-067`, stated once:** `merge_entries` silently skips any id `from_id` does not know — `crates/cyrup-tui/src/keymap.rs:128`, `let Some(action) = from_id(&id) else { continue };`, which is upstream's deliberate `if (!(keybinding in this.definitions)) continue` (`packages/tui/src/keybindings.ts:172-179`) — so an id present in a `Default` table but missing from the matching `from_id` is **unbindable and reports nothing**, not even a `KeybindingIssue`. |
...
| ~~TUI-088~~ | ~~**high**~~ **CLOSED 2026-08-15 — ALREADY IMPLEMENTED / MIS-DIAGNOSED** | ~~not-ported~~ *(mis-filed — ported at HEAD)* | M | ~~**Ctrl+C does not work in the running TUI**~~ — filed 2026-08-15 from live use — **CLOSED 2026-08-15 (TUI-092 §4): its premise, "there is no global Ctrl+C destination in `keymap.rs`", is FALSE at HEAD, and every layer was re-read line by line.** **Binding** — `crates/cyrup-tui/src/keymap.rs:656`, `(Key::ctrl('c'), Action::Clear)` in `Keymap::default()`. **Id** — `keymap.rs:268`, `"app.clear" => Some(Action::Clear)`, so it is rebindable from `keybindings.json` through `merge_json` (`:760`). **Destination** — `crates/cyrup-tui/src/app.rs:2440-2453` implements pi's `handleCtrlC` verbatim: a second press within 500 ms sets `should_quit` and returns `AppAction::Quit`, otherwise the editor is cleared and `last_sigint` stamped — including the asymmetry that the timestamp is updated **only** on the clear branch. **Reach** — `handle_input`'s `defer_to_editor` match (`app.rs:2059`) special-cases only `Interrupt`/`Quit`/`PageUp`/`PageDown`, so `Action::Clear` falls to `_ => false` (`:2075`) and reaches `apply_action` unconditionally. **Affordance** — `crates/cyrup-tui/src/chrome.rs:81-86` renders `ctrl+c/ctrl+d — clear/exit` in the startup hint bar from the LIVE keymap. **Coverage** — `crates/cyrup-tui/src/tests/keymap.rs:75` and `crates/cyrup-tui/src/tests/keybindings.rs:200` both assert `action_for(ctrl+c) == Action::Clear`. **And it is 1:1 with upstream, so the fix this row asked for would have been a divergence, not a fix:** `pi/packages/coding-agent/src/core/keybindings.ts:66-68` binds `app.interrupt`→`escape`, `app.clear`→`ctrl+c`, `app.exit`→`ctrl+d`, and `modes/interactive/interactive-mode.ts:3797-3805` is `handleCtrlC` with no `isStreaming` branch anywhere — **Ctrl+C does NOT bind to interrupt upstream and never aborts a running turn** (aborting is Escape, `keymap.rs:657` → `app.rs:2383`), and rebinding it would also destroy the double-tap exit `TUI-092` §3 depends on. **Mis-diagnosis cause, in one line:** the probe was `grep "Char('c')"`, but the chord table is built from the `Key::ctrl(..)` / `Key::plain(..)` constructors (`keymap.rs:654-696`), so a `KeyCode::Char` grep is structurally blind to **EVERY** global binding in this codebase — the correct probe is `grep "ctrl('c')"`. **The user-visible symptom this row recorded is real and is REASSIGNED TO `TUI-092`:** the key is bound, wired, tested and advertised, and it stops responding because the run loop is starved or wedged and can service NO input at all. **No keybinding code was written for this closure.** |
| TUI-089 | medium | port-bug | S | **Models added through `~/.cyrup/agent/models.json` are appended to the picker unsorted instead of being merged into its ordering.** Observed live 2026-08-15: the built-in `together` block is ordered (`Qwen…` → `deepseek-ai/…` → `essentialai/…` → `google/…` → `meta-llama/…` → `moonshotai/Kimi-K2.6` → `moonshotai/Kimi-K2.7-Code` → `nvidia/…` → `openai/…`), and a user-declared `moonshotai/Kimi-K3` on the same provider lands at the **bottom of the whole 85-entry list**, far from its provider group and its `moonshotai/` siblings. So a user who adds a model finds it only by scrolling past every other provider — with no indication it is present at all. Establish where pi sorts (catalog assembly vs. picker render) and sort at the same point; a picker-only sort would still leave `--list-models` and any other consumer inconsistent. **Scope check before fixing: confirm whether this affects all `models.json`-declared entries or only ones added to an EXISTING provider**, since a wholly new provider block may take a different path. — **FILED 2026-08-15 (live use)** |
| TUI-090 | **high** | port-bug | M | **A whole screen of blank rows is emitted after every conversation turn, pushing the agent's output above the fold.** Reported from live use 2026-08-15. The user must scroll UP to read the reply to the message they just sent, on every turn — which makes normal use painful and is the worst-rated defect found in a live session so far. **Architectural lead, to be confirmed not assumed:** cyrup's TUI is an *inline, content-sized viewport* (`lib.rs:3`, ADR-0001 commitment #1), not the alternate screen; committed rows leave the render tree for native scrollback via `App::flush_committed` -> `Terminal::insert_before` (`app.rs:1944`), and the viewport is supposed to be re-sized to the height of what remains ACTIVE (`transcript.rs:1135`, "used to content-size the inline viewport"). The symptom is what a viewport that keeps a finished turn's height instead of collapsing to the idle height would produce. Check the height computed on the LAST render of a turn versus the first render after it settles. **Verify in a real terminal, not TestBackend** — a fixed-size `TestBackend` cannot express "the viewport did not shrink", which is very likely why 96 rows of area-07 static analysis and a full assembled-render test suite never caught it. Confirm against pi, whose inline renderer solves the same problem, before changing the sizing rule. — **FILED 2026-08-15 (live use)** |
| TUI-091 | **high** | port-bug | M | **Reasoning blocks never render, though every layer from provider to setting is wired and correct.** Owner report, live use 2026-08-15, `together`/`Kimi-K3` at `defaultThinkingLevel: high`, confirmed still absent after scrolling. **FIVE layers traced and VERIFIED CORRECT at HEAD — do not re-trace them, start at the two candidates below:** (1) **provider** — Together streams real `reasoning_content` deltas for this model (live request returns `"delta":{"reasoning_content":"The"}`), and `first_reasoning_delta` (`cyrup-provider/src/api/openai_completions.rs:2111`) reads it first from `REASONING_FIELDS` (`:1630`), emitting `StreamEvent::ThinkingStart/Delta/End` (`:1891`, `:1999`); (2) **agent** — `decode_stream` sends `StreamEvent::Start` before any delta and `agent.rs:926` sets `started = true` on it, so the `if started` gate at `:949` is already open when reasoning arrives, and the `_ =>` catch-all re-emits EVERY non-terminal event as `AgentEvent::MessageUpdate` carrying the original event boxed (`:948-956`); (3) **TUI route** — `app.rs:5662` calls `ingest_stream_event(assistant_message_event)`; (4) **TUI handler** — `app.rs:6007` matches `StreamEvent::ThinkingDelta` and calls `push_thinking_delta`, which accumulates into `TranscriptView::thinking` (`transcript.rs:658-663`); (5) **setting** — `hide_thinking_block()` is `get_bool("hideThinkingBlock").unwrap_or(false)` (`cyrup-config/src/settings.rs:581`), matching pi's `?? false` (`settings-manager.ts:855`), read into the view at `app.rs:7842`, and the reporter's `settings.json` does not set the key. **Candidate (a) — the facade dropping the inner event — is REFUTED:** `cyrup-session-svc/src/event.rs:292-296` clones `assistant_message_event` straight from `AgentEvent::MessageUpdate` onto `AgentSessionEvent::MessageUpdate` with no projection, and `app.rs:5661` destructures exactly that field. (The `grep 'ThinkingDelta' cyrup-session-svc` miss was a false lead — the facade carries the event *boxed and opaque*, so the variant name never appears there.) **MEASURED AGAINST A LIVE SESSION 2026-08-15 — the model IS thinking and cyrup IS capturing it.** The reporter's active transcript (`~/.cyrup/agent/sessions/--Users-davidmaple-cyrup.ai--/*.jsonl`, 682KB) contains **40 non-empty `"type":"thinking"` content blocks** alongside 106 `text` blocks across 133 messages, with real reasoning prose in each. So reasoning is requested, returned, decoded AND PERSISTED to the session file — the "maybe the TUI shows `high` but never enables thinking" hypothesis is REFUTED, and every layer except rendering is proven live rather than by reading. **ONE CANDIDATE REMAINS, and it is a single-file question in `crates/cyrup-tui/src/transcript.rs`:** the accumulated `thinking` buffer is never included in the painted region — `thinking_lines` (`transcript.rs:1123`) builds the inline viewport's styled lines and its doc says "only the active streaming partial", so check the live block is actually in the render tree. **Confound to eliminate first, cheaply: `TUI-090`** — a screen of whitespace after every turn means a correctly-rendered block can sit above the fold. The reporter has scrolled and still sees nothing, which weakens but does not eliminate it. Verify in a REAL terminal; `TestBackend` cannot distinguish "not rendered" from "rendered off-screen". — **FILED 2026-08-15 (live use)** |
| TUI-092 | **critical** | port-bug | M | **The TUI degrades from smooth to a TOTAL LOCKUP that cannot be exited.** Escalated to critical 2026-08-15: the terminal ends up fully wedged and **`Ctrl+D` does not kill it either** — and with `TUI-088` (Ctrl+C has no global binding) that leaves the user **no in-band way to exit at all**, only killing the process from another terminal. A hang that survives both EOF and the interrupt key is a blocked event loop or a held lock, not slow drawing. Owner report, live use 2026-08-15. **Keystrokes go unresponsive and rendering crawls together** (owner detail) — and that pairing is the most diagnostic fact here: if render cost alone were growing, input would still be READ and the screen would merely lag behind it. Input going dead means **the loop that reads keys is being starved**, so look for render work executing ON the input path, or a lock the input path needs being held across a growing render — not merely for an expensive draw. Progressive rather than a single stall, so the blocking work grows with turn count. **Already ruled out — do not re-check these:** `TranscriptView::pending` is emptied by `drain_committed` via `std::mem::take` (`transcript.rs:553-556`), and `active_tools` is drained at `:899` and `:935`. Neither is the unbounded accumulator. **Candidates, in the order they are cheapest to test:** (1) **`TUI-090` may be the cause, not a sibling** — if a full screen of whitespace is left after every turn and the inline viewport's active region grows with it, per-frame render cost grows linearly with turns, which presents exactly as "fast at first, freezes later". Test by measuring frame time against turn count; if it climbs, fix `TUI-090` first and re-measure before touching anything here. (2) **A per-turn leak** — a subscription, task or channel created per turn and never dropped. This codebase has shipped that class twice (a `DrainLatch` that left the event bus permanently disabled after an abort, and a dedup owner dropped without settling), so audit what a turn creates against what its teardown releases; `active_tools.iter` runs five times per frame, so a retained entry costs every frame thereafter. (3) **Re-parse per frame** — markdown wrapping or image rasterisation recomputed over history rather than cached; `MAX_RASTER_PX` caps a single raster but not their number. **Measure before fixing:** capture frame time and RSS against turn count in a real terminal (`TestBackend` cannot show either), and per `handoff/03-verification.md` instrument and read ONE run rather than re-running to characterise. — **FILED 2026-08-15 (live use)** |
