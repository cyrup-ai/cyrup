...
|---|---|---|---|---|
| `cyrup/` | **`68facb9b`** — last CODE commit as of the 2026-09-14 upstream-tag re-measure (`workflows 16,17`, `main` = `73485aab`). *Superseded: `824a539e` (batch 3), `6cf2cb9f` (batch 2).* **Re-measure before trusting any status: the port moves faster than this directory does.** `824a539e..68facb9b` is **31 code commits, 453 files, +98 179 / −15 880** under `crates/`+`xtask` — the `workflowScript` runtime (`WORKFLOW_1`…`17`) and the net-new `cyrup-workflow-runtime` crate — and **no area file has been re-read against it** | — | — | **23 crates, 885 246 lines of Rust under `crates/`** (re-derived 2026-09-14: `find crates -name '*.rs' -type f \| xargs wc -l`) |
