...
|---|---|---|---|---|
| `cyrup/` | **`6cf2cb9f`** — last CODE commit as of the 2026-09-04 batch-2 (eighth-edition) pass (`fix(subagents): SUBA-087 review fixes …`, branch `claude/beautiful-feynman-odz1v5`, 68 commits off `main` = `a4805955`, 35 of them touching `crates/`/`xtask`; the docs commit that wrote this row is the next one and cannot cite its own sha — the code sha is the one a status is measured against). *Superseded: `275c1f85` (second pass, five code commits off `main`).* **Re-measure before trusting any status: the port moves faster than this directory does.** | — | — | 21 crates, ~786k lines of Rust under `crates/` (`find crates -name '*.rs' \| xargs wc -l \| tail -1` = 786 236 at `6cf2cb9f`; was 714 265 at `275c1f85`; `for d in crates/*/; do [ -f "$d/Cargo.toml" ] && echo "$d"; done \| wc -l` = 21) |
