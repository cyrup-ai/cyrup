...
|---|---|---|---|---|
| `cyrup/` | **`824a539e`** — last CODE commit as of the 2026-09-04→05 batch-3 (ninth-edition) pass (`fix(tui,session-svc,ext,intercom): TUI-046 DRIFT-041 EXT-006 ICOM-054 review fixes`, branch `claude/parity-batch3`, 51 commits off `main` = `3e9633c4`, 23 of them touching `crates/`/`xtask`; the docs commit that wrote this row is a later one and cannot cite its own sha — the code sha is what a status is measured against). *Superseded: `6cf2cb9f` (batch 2), `275c1f85` (second pass).* **Re-measure before trusting any status: the port moves faster than this directory does.** | — | — | 21 crates, ~786k lines of Rust under `crates/` (last measured at `6cf2cb9f`; batch 3 added code and tests across nine crates and the figure was NOT re-derived at `824a539e`) |
...
| `pi-mcp-adapter/` | `6ba7d36` = `v2.32.1-3-g6ba7d36` — clone re-pulled 2026-09-04, and **area 13 HAS now been RE-AUDITED against it** (`11b9994a`, 2026-09-04), superseding the prior "clone re-pulled, area 13 NOT re-audited" note: its files were opened, the counted census re-derived by re-parsing the unit table (**244 implemented / 82 partial / 84 missing / 27 n-a of 437**, a floor rather than an answer), and 40 new units filed including four criticals/highs. *Superseded: `14c0e6c` = `v2.25.0-4-g14c0e6c`.* | **not ported** — area 13 is the plan | **v2.32.1** *(was v2.25.0; the `v2.26.1..v2.32.1` window is now measured — 147 files, +16 014 / −1 001, 72 commits, 68 `--no-merges`)* | 203 paths / 164 `.ts` at the tag, ~24 200 lines; drift to HEAD is 17 files, +543 / −69 |
