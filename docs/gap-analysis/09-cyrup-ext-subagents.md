...
> ### RE-VERIFICATION 2026-09-24 — open rows re-read at cyrup `ea23ca2` against pi-subagents `v0.71.0`
>
> **Pins for THIS pass:** cyrup HEAD **`ea23ca2`** (merge of #152; code commit `df3e9a8`, 2026-09-24).
> `9aeba769..ea23ca2` is 46 commits under `crates/`, most of them in this crate (`background/`,
> `exec/`, `extension/executor/`), the new `cyrup-herdr` crate and `cyrup-it/tests/subagents`.
> Upstream **pi-subagents `v0.71.0`** (`git -C tmp/pi-subagents tag --sort=-v:refname | head -1`;
> clone HEAD is `v0.71.0-10-g6f1027f7`, and that past-the-tag range is not read). New upstream
> citations below use `git -C tmp/pi-subagents show v0.71.0:<path>`. **No older `@v0.43.0` / `@v0.47.1`
> / `@v0.67.0` / `@v0.68.0` citation was rewritten.**
>
> **Open rows re-read: `SUBA-022` and `SUBA-024`. Neither closed.** Both rows carry a
> `RE-READ 2026-09-24` clause. `SUBA-024`'s `Kind` is now `not-ported`, the correction the row had
> recorded as owed since 2026-09-14 (`git cat-file -e v0.43.0:src/runs/shared/agent-contract.ts`
> succeeds). **Closed rows spot-checked port-side, not re-audited in full:** `SUBA-017` (the batching
> sink `background/watch/batch.rs::BatchingHostServicesCompletionSink` is built in production at
> `extension/executor/notices.rs:234`), `SUBA-063` (`exec/runtime_acknowledged_extensions.rs` is read by
> `exec/attempt_runner.rs:227` and its path env is set by `exec/spawn_plan.rs:590-594`; the row says
> "uncommitted at writing", and `git log` shows it landed in **`df3e9a8`**) and `SUBA-026`
> (`registration/slash_commands.rs` registers a bare `subagents`). None had regressed.
>
> **The upstream window this file cannot own.** `v0.57.0..v0.71.0` now belongs to
...
> | `SUBA-063` | still-open · low | **PARTIALLY CLOSED** → **CLOSED 2026-09-23** | zero-budget DECODE and the events-cap override landed; no parent-side writer, no ack path, no truncation marker. *2026-09-23: ack protocol ported, marker closed at `2f4d0a5`, writer rides `SUBA-022` — see the `## Open items` row.* |
...
> **[2026-09-23: ~~both open~~ both CLOSED.** `SUBA-097` at `d00a314` (2026-09-17): the test was
> written and the comment corrected, and this file was never updated. `SUBA-098` at `c92f739`. See
> their rows.**]**
>
...
> **[2026-09-23: `SUBA-097` and `SUBA-098` have both since CLOSED, and so have `SUBA-061` and
> (in `09a`) `SUBA-096`. `scripts/count_open_items.py` is the number to read. It is not restated here.]**
...
| SUBA-012 | **closed** | `tui/fleet.rs` (3040 lines) + `fleet_state/fleet_status/fleet_transcript/fleet_overlay/fleet_theme`; `view`/`lines` are advertised properties; `/subagents-fleet` is in the 16-name table at `registration/slash_commands.rs:127-145`. ~~Keystroke half remains PARITY-GAPS UW-7.~~ **Keystroke half CLOSED 2026-09-22** — `init` subscribes through `InitApi::subscribe_terminal_input` (`extension/host/native_impl.rs:276-284`), `NativeExtension::on_terminal_input` (`:1040`) routes to `extension/host/terminal_input.rs`, and `cyrup-tui`'s input arm folds every key through `ExtensionHost::terminal_input` before the editor sees it (`cyrup-tui/src/app/run_action.rs`, `app/terminal_input.rs`). See PARITY-GAPS UW-7. |
...
| SUBA-017 | **CLOSED 2026-09-23** | Premise corrected 2026-09-22 (`.flux/todo/COMPLETION_BATCHING.md`): the session pump already coalesced queued injections, so the delta was 2–3 turns vs 1, not N vs 1. Closed by the pump re-drain after `wait_for_idle` plus `merge_injection_batch` tests (`8b84484`) and the lead-in batcher `background/watch/batch.rs` on the host arm of `effective_completion_sink`, with the grouped notice and the `completionBatch` key (`b5f6393`). Default window is upstream's plus one results-watcher poll quantum ([CYRUP-DELTA], see the row). |
| SUBA-018 | **closed — citation corrected 2026-09-21 (VL-S12)** | `registration/prompt_workflows.rs` (867 lines). `/prompt-workflow` survives and is the row's whole evidence: variant at `registration/slash_commands.rs:109`, `as_str` arm at `:176`, descriptor at `:271`, dispatched at `extension/host/slash.rs`'s `SlashCommandName::PromptWorkflow` arm, and its `chain:` frontmatter branch (pi `prompt-workflows.ts:271-278` @v0.68.0) still expands a recipe into a multi-step run. **`/chain-prompts` is DELETED** — upstream removed it at v0.41.0 (at v0.68.0 `prompt-workflows.ts:254` registers `prompt-workflow` alone and `"chain-prompts"` survives only in `RESERVED_COMMAND_NAMES` at `:24`), so VL-S12 removed the variant, its descriptor, its dispatch arm, its handler and `split_chain_declaration`. The old `:141-142` citation was stale on both halves before the deletion. |
...
| SUBA-097 | **closed at `d00a314` (2026-09-17)**; ~~new 2026-09-16 — OPEN~~ | low · `background/scheduled_runs/trigger.rs:887` claims an integration suite that does not exist, over the one seam its unit tests stub. Found while verifying `SUBA-016`. |
| SUBA-098 | **closed 2026-09-23 (`c92f739`)**; ~~new 2026-09-16 — OPEN~~ | low · A stray CJK token inside an English doc comment at `extension/tool/scheduled_runs_tests.rs:31`. Found while verifying `SUBA-016`. |
...
> **2026-09-24, cyrup `ea23ca2` / pi-subagents `v0.71.0`:** the two open rows, `SUBA-022` and
> `SUBA-024`, were re-read on both sides. Both are still open, and nothing in this table closed.
> Items from the `v0.57.0..v0.71.0` window (`SUBA-107`…) are filed in `09b`, not here. Run
> `scripts/count_open_items.py` for the count. It is not restated here.
...
> **[2026-09-23: `SUBA-097` (closed at `d00a314`, 2026-09-17), `SUBA-098` (`c92f739`) and `SUBA-061`
> (`e75fb16`) have since CLOSED. `SUBA-063` was re-derived and stays open on part (b) only. Run
> `scripts/count_open_items.py` for the current count.]**
...
| ~~SUBA-017~~ | ~~low~~ **CLOSED 2026-09-23** | not-ported | M | Completion batching — **CLOSED 2026-09-23; premise corrected 2026-09-22** (`.flux/todo/COMPLETION_BATCHING.md`). cyrup never delivered N completions as N turns: since `8de7460` the session pump coalesces every queued injection into one turn (`cyrup-session-svc/src/session/mod.rs`, `merge_injection_batch` in `session/inject.rs`). What was missing, and is now built: **(1)** the pump's re-drain AFTER `wait_for_idle`, so a busy parent spends one turn on everything that arrived while it was busy, not two (helps every producer); **(2)** tests for `merge_injection_batch`, which had none; **(3)** upstream's LEAD-IN debounce (`completion-batcher.ts`, byte-identical v0.43.0→v0.68.0) as `background/watch/batch.rs`: `InlineAnsweredSink(BatchingHostServicesCompletionSink)` on the HOST arm of `effective_completion_sink` only — below the inline-answer decorator (a claimed run never holds its siblings) and at the injector (only a synchronous `inject_message_ack` can order a failure behind the held successes it flushes, both under one lock). Upstream's straggler groups, per-key groups, failure/loss bypass with flush-first, grouped `Background tasks completed (N): …` text (`formatGroupedCompletion`, `notify.ts:472-489`), OR'd `display`/`trigger_turn`, per-member receipts only after the one group ack, ownership re-checked per member at flush, and a `Weak`-holding flusher that skips members whose caller was aborted. **(4)** the `completionBatch` key (raw on the config since the contract commit), resolved per field with upstream's lenient fallback. **[CYRUP-DELTA] the default window:** upstream's 150/1000/75/400 ms assume `fs.watch` latency; cyrup's results watcher polls every `RESULTS_DIR_POLL_INTERVAL` (500 ms), so a 150 ms debounce could never group siblings landing in different polls. Each TIMER defaults to upstream's value plus one poll quantum, derived from that constant (650/1500/575/900 ms; the 2000 ms straggler window is a look-back and is unchanged). A user-set `completionBatch` is honoured exactly. Phase-1 observation still runs BEFORE delivery, so `wait`'s bus wake and wait-subscription reconciles are not delayed by the window (upstream's are). Upstream: `completion-batcher.ts:29-167`, `notify.ts:544-549`, `:692-791`, `:802-817`, key `shared/types.ts:2663`, wiring `extension/index.ts:495` @v0.68.0 (`:376` @v0.43.0). |
| SUBA-022 | low | not-ported | L | Typed extension delegation API unported (**in-baseline**, not drift) — **RE-READ 2026-09-14 at `9aeba769` (verified re-audit): still open, unchanged.** No `api/` module exists in the crate at `9aeba769` (`registration/` holds `authority.rs`, `cost.rs`, `doctor.rs`, `guide.rs`, `mod.rs`, `profiles.rs`, `prompt_workflows.rs`, `resources.rs`, `slash_commands.rs`, `tool_description.rs` and nothing else), and the five `prompt-template:subagent:*` event names are zero-hit crate-wide. Upstream still exports all five at `src/api/delegation.ts:6-10` @v0.67.0, plus the `SubagentDelegationRequest` shape at `:27-45`. `Kind: not-ported` re-confirmed by presence: `git cat-file -e v0.43.0:src/api/delegation.ts` succeeds. **Note for the fixer:** at 116 L @v0.67.0 the file has grown well past a five-constant module — it now carries the typed request/response contract (`toolBudget`, `thinking`, structured-result request, per-launch intercom-bridge config), so `Effort L` still reads correctly. — **RE-READ 2026-09-14 at `d53763b` (post-#134 delta check): still open, unchanged.** #134 added no module outside `extension/executor/`, `extension/tool/`, `missions/` and `workflows/scripted/` — there is still no `api/` in the crate — and `prompt-template:subagent` is still zero-hit crate-wide. — **RE-READ 2026-09-16 at `cc7818b` and upstream `v0.68.0`: still open, unchanged.** Still no `api/` module among the crate's 23 `src/` entries, and `prompt-template:subagent` is still zero-hit workspace-wide; the SCOPE sequence added `background/scheduled_runs/`, `background/completion_replay/`, `background/wait_subscriptions/`, `background/inspect_rpc/` and `background/async_retention/` and no `api/`. Upstream's five constants unchanged at `src/api/delegation.ts:6-10` @v0.68.0. **One line of its header is the design instruction and was not recorded before:** *"This is the established extension-to-extension transport. The structured delegation API intentionally reuses it instead of adding a second event protocol."* — so the port is a typed façade over `cyrup-ext`'s `SharedBus`, which queues emits and passes payloads by value, the same request/response-topic problem `09a` recorded for the v0.64.0 runtime-agent event bridge. Design the two together. — **RE-READ 2026-09-24 at `ea23ca2` and upstream `v0.71.0`: still open.** The crate's `src/` still has no `api/` module. Its top-level entries at `ea23ca2` include the new `herdr/`, `placement/`, `identity/`, `runner/` and `watchdog/`, but no `api/`. `prompt-template:subagent` has zero hits in `crates/`. #152's subject, *"delegation that works end to end"*, is about machine placement and agent keys (`df3e9a8`). It does not port this API, and the row must not be closed on that wording. Upstream still exports the five constants at `src/api/delegation.ts:6-10` @v0.71.0. The file grew from 116 to 131 L in `v0.68.0..v0.71.0`: it adds `SubagentDelegationUpdateUsage` (`input`/`output`/`cacheRead`/`cacheWrite`/`turns`, emitted only when every counter is known) as an optional `usage` on `SubagentDelegationUpdate` (CHANGELOG 0.71.0, #2374). The contract a port has to meet has therefore grown by one field. The row's severity and effort are unchanged. |
...
| SUBA-063 | ~~low~~ **CLOSED 2026-09-23** | ~~not-ported~~ | M | **CLOSED 2026-09-23 (TAIL batch; uncommitted at writing, the conductor's combined commit carries it).** **(b) The runtime-extension acknowledgement protocol is PORTED, end to end.** New module `crates/cyrup-ext-subagents/src/exec/runtime_acknowledged_extensions.rs`: `RUNTIME_EXTENSION_ACK_EVENT = "subagent:acknowledge-extension"`, the id rule (`isRuntimeAcknowledgedExtensionId`, non-empty, ≤128, `[A-Za-z0-9._:@+-]`, no `..`/`/`/`\`), the 32-id projection with `omitted`, the sanitizer (envelope check, re-projection, additive floored `omitted`), the 64 KiB-bounded parent reader and the `0600` child writer (remove-on-empty) — pi `runtime-acknowledged-extensions.ts` (identical logic at v0.64.0 and v0.68.0). **Child side:** `prompt_runtime.rs` `SubagentPromptRuntime::with_runtime_acknowledgements` arms a `RuntimeExtensionAcknowledgements` collector from `CYRUP_SUBAGENT_RUNTIME_ACKNOWLEDGED_EXTENSIONS`; `init` subscribes the bus topic (`InitApi::subscribe_bus`) plus `agent_end`/`session_shutdown`; `on_bus_event` collects; `on_event` finalizes once, FIRST in the `agent_end` arm (pi registers it first, `subagent-prompt-runtime.ts:451` @v0.68.0). **Parent side:** `exec/spawn_plan.rs` writes the env var for EVERY child (`<scratch>/runtime-acknowledged-extensions.json`, pi `pi-args.ts:842-846` @v0.64.0; overwrites an inherited value), `exec/attempt_runner.rs` clears a stale file before the spawn and reads it back at the child's close on every exit path (pi `execution.ts:1386`), `run_sync` publishes the WINNING attempt's value on `SingleResult`. **Carried where upstream publishes it and cyrup has the type:** `StepResult` → `StepStatus` (`runner_main/executor.rs`, `status.rs`; pi `subagent-runner.ts:3834/4253/4738` @v0.68.0), the terminal `ResultFile`'s per-result copy (`settle.rs`; pi `:5068`), a new `RunStatus::runtime_acknowledged_extensions` lifted from a ONE-result run (`finish.rs`; pi `:4887,:5002`, `AsyncStatus` `shared/types.ts:1889`), the in-memory remembered foreground child (`ForegroundHistoryChild`, pi `subagent-executor.ts:815,:895`; NOT persisted, because pi's `compactChild` drops it, `foreground-history.ts:28-63`), the artifact `_meta.json` (`artifacts.rs`, pi `execution.ts:164`), and the RPC `ping` capability (`extension/rpc/ping.rs`, pi `rpc.ts:457`). **Not carried, each for a stated reason:** `Details` (:1474) and `AsyncStartedEvent` (:1693) — no upstream setter at v0.68.0 (`git grep runtimeAcknowledged` shows only the type lines), and a started event precedes any child; `NestedRunSummary`/`NestedStepSummary` (:1602/:1622) — upstream fills them from `nestedSummaryFromAsyncStatus` (`nested-events.ts:991-1060`), which cyrup does not have: its only nested producer is the spawn-time `subagent.nested.started` event (`extension/executor/background.rs`), written before any child ran, so there is nothing to carry until that producer exists. **[CYRUP-DELTA] transport:** upstream moved children in-process at v0.65.0 (`d9bc62f8`) and replaced the env var + file with an in-memory sink (`child-hooks.ts:141-159` @v0.68.0); cyrup's children are OS processes, so it keeps the v0.64.0 file transport with the unchanged v0.68.0 rules. **Tests:** `exec::runtime_acknowledged_extensions::tests::{the_id_rule_is_upstreams, the_projection_dedups_caps_at_32_and_counts_the_rest, the_sanitizer_requires_the_envelope_and_adds_declared_omitted, a_finalized_collector_round_trips_through_the_parents_reader}`; cyrup-it `subagents::runtime_acknowledged_extensions_child_integration::a_cooperating_child_extensions_acknowledgement_is_written_where_the_parent_asked` (real session, real `SharedBus`, a cooperating native emitting); `subagents::exec_run_sync_integration::{run_sync_reads_the_childs_runtime_acknowledgements_back_through_the_sanitizer, a_child_that_acknowledges_nothing_publishes_no_runtime_acknowledgement}` and `subagents::background_runner_main_integration::a_background_runs_status_and_result_carry_the_childs_runtime_acknowledgements` (real child PROCESS writing an untrusted capture); `tests::rpc_bridge_integration::a_host_drives_ping_over_the_inter_extension_bus_and_gets_a_reply` (capability now asserted present). **(c) the `subagent.events.truncated` marker: CLOSED at `2f4d0a5`** — `jsonl::RunEventLog::write_diagnostic_line` (512-byte reserve, one-shot marker), pinned by `jsonl::tests::the_first_overflowing_diagnostic_line_writes_one_marker_and_stops` and `background::runner_main::events` `an_overflowing_child_event_is_replaced_by_one_marker`; the 2026-09-23 TAIL batch also moved the last `events.jsonl` writer that bypassed it (the detached-workflow reconciler) onto `RunEventLog`. **(a) the parent-side zero-tool-budget WRITER is not owed here, re-verified 2026-09-23:** upstream's only producer of `allowZeroToolBudget: true` is `executeDelegated` (`subagent-executor.ts:7455-7475` @v0.68.0, the gate at `:5017`/`:6870`), whose only caller is the prompt-template delegation bridge's `executeStructured` (`extension/index.ts:747-755`, `slash/delegation-adapters.ts:301`). cyrup has neither: `git grep -n "prompt-template:subagent\|execute_delegated\|allow_zero_tool_budget" -- crates` is 0. It lands with `SUBA-022`. *History follows.* — Zero-tool-budget authorisation and the runtime-extension acknowledgement path unported — **RE-DERIVED 2026-09-23 from scratch (`.flux/todo/SILENT_DROPS.md` §4). Still OPEN, on part (b) only.** **(a) Zero-budget authorisation: nothing owed here, because it rides `SUBA-022`.** A public `hard: 0` is refused loudly on every surface cyrup has, which matches upstream: the public tool validates with `minimumHard: 1`. Upstream's **only** producer of the authorisation is the prompt-template delegation bridge (`slash/delegation-request.ts:90`, `slash/delegation-adapters.ts:301` @v0.68.0), and that bridge is `SUBA-022`. Upstream **removed** `PI_SUBAGENT_TOOL_BUDGET_ZERO_AUTH` at v0.67.0, because its children went in-process. cyrup still spawns subprocesses, so it keeps the env var, now marked `[CYRUP-DELTA]` at `exec/tool_budget.rs`. When `SUBA-022` lands, it owes the writer (`spawn_plan.rs`, guarded on "the effective budget is the delegated one", pi `subagent-executor.ts:4121`). **(b) Runtime-extension acknowledgement: OPEN, M, and not a silent drop.** It is a missing feature, and `extension/rpc/ping.rs` says it is not advertised. It needs a child-side collector, a hand-back channel, and carriage from `SingleResult` to `StepStatus` to `RunStatus` to the RPC capability. **(c) `events.jsonl` truncation marker: CLOSED at `2f4d0a5`.** Diagnostics are capped with the 512-byte reserve and a one-shot `subagent.events.truncated` marker, and lifecycle events are never capped. *History follows; its "(c) CLOSED at `9aeba769`" claim was about the cap override, not the marker.* — **RE-READ 2026-09-14 at `9aeba769` (verified re-audit): still open — but BOTH of the row's cyrup evidence lines are now WRONG and the SHAPE of part (a) has changed; recorded here so the next reader does not close this on the grep alone.** (a) `rg -c 'ZERO_AUTH' = 0` is REFUTED: `TOOL_BUDGET_ZERO_AUTH_ENV` exists (`exec/tool_budget.rs:39`), `HardMinimum::{One,Zero}` exists (`:31-72`), and the CHILD side fully honours it (`HardMinimum::from_env` at `:56-64`, exact-string `"1"` equality as upstream; `prompt_runtime.rs:2301,:2598-2680` decodes with it and blocks the first call). But what that code DOES was audited rather than that it exists, and it is a dead end: **no site anywhere writes `TOOL_BUDGET_ZERO_AUTH_ENV` into the child env** — `exec/spawn_plan.rs:1163-1176` writes `TOOL_BUDGET_ENV` and stops — and the parent-side validator `validate_tool_budget_config` (`tool_budget.rs:111-115`) hardcodes `HardMinimum::One`, as do `spawn_plan.rs:3128` and `:3391`, so a declared `hard: 0` is refused parent-side before it can ever be authorised. `HardMinimum::Zero` is reachable only from tests and from the child's own env read. **The defect is therefore now `unwired`, not `not-ported`, and its failure mode has INVERTED relative to `CFG-080`:** an unauthorised zero no longer runs unbudgeted (fail-open) but refuses the child runtime (fail-closed). Upstream has meanwhile grown the parent half considerably — a `delegatedAllowZeroToolBudget` private param and a `delegatedZeroToolBudgets` `WeakSet` (`src/runs/foreground/subagent-executor.ts:7283-7289`, `:4891`) gating `{ minimumHard: 0 }` at `:6688`; see also `:513`, `:3996` and `src/shared/types.ts:2386` (`allowZeroToolBudget`) @v0.67.0. (b) The runtime-extension acknowledgement path is unchanged and genuinely unported, and upstream has grown it well past a path env var into a `subagent:acknowledge-extension` RPC event surfaced on `AsyncStatus` (`src/extension/rpc.ts:457`, `src/runs/background/subagent-runner.ts:1224,:1368`, `async-status.ts:87,:140,:386,:441` @v0.67.0). (c) The row's own "separately and cheaply" residual — the `ASYNC_EVENTS_MAX_BYTES` override — **IS closed**: `background/runner_main/events.rs:9-36` ports it with upstream's `Number()` coercions and one documented divergence. The `subagent.events.truncated` marker (`TRUNCATED_EVENT_TYPE`, `subagent-runner.ts:329` @v0.67.0) is not. — **RE-READ 2026-09-14 at `d53763b` (post-#134 delta check): still open, unchanged.** #134 touched nothing under `exec/` or `background/`: `TOOL_BUDGET_ZERO_AUTH_ENV` (`exec/tool_budget.rs:39`) still has no parent-side writer and `validate_tool_budget_config` (`:115`) still hardcodes `HardMinimum::One`, so part (a) is still the `unwired`, fail-closed dead end this row describes; `acknowledge-extension`/`acknowledge_extension` are still zero-hit crate-wide, so part (b) is untouched. — **RE-READ 2026-09-16 at `cc7818b` and upstream `v0.68.0`: still open, and this pass CONFIRMS the `9aeba769` re-read's sharpest observation by re-running it rather than inheriting it.** Part (a) is still a reader with no writer: `git grep TOOL_BUDGET_ZERO_AUTH` over the WHOLE workspace returns 12 hits, every one in `exec/tool_budget.rs` or `prompt_runtime.rs`; `exec/spawn_plan.rs:1173` writes `TOOL_BUDGET_ENV` and no sibling. **The `unwired` reclassification that re-read proposed is correct and should be applied to the `Kind` cell.** One thing HAS changed in its favour: the row's stated dependency on `SUBA-047` for a caller surface is discharged — `toolBudget` is advertised (`extension/tool/schema.rs:546`) and validated (`extension/tool/routing.rs:736,:825`), and `hard: 0` is refused parent-side with *"toolBudget.hard must be an integer >= 1."* (`routing_tests.rs:604,:1801`), so only the writer and the explicit-zero plumbing are owed. Upstream's parent half at v0.68.0 is where the row said it was, line-shifted: `delegatedZeroToolBudgets` `subagent-executor.ts:4998`, the `{ minimumHard: 0 }` gate `:6870`, `delegatedAllowZeroToolBudget` `:7469-7475`, `allowZeroToolBudget` on the async input `async-execution.ts:319` and `shared/types.ts:2427`. Part (b) unchanged and now has its own upstream module — `src/runs/shared/runtime-acknowledged-extensions.ts` @v0.68.0 beside the `acknowledge-extension` RPC — still zero-hit here. Part (c)'s marker (`TRUNCATED_EVENT_TYPE = "subagent.events.truncated"`, `subagent-runner.ts:320` @v0.68.0, `:266` @v0.43.0 — in-baseline) is still unported; the cap override stays closed. |
| ~~SUBA-097~~ | ~~low~~ **CLOSED 2026-09-17** | cyrup-original | S | `scheduled_runs`' production launcher is claimed to be integration-tested by a file that does not exist — **CLOSED at `d00a314` (2026-09-17, "… prove scheduled runs really fire"); this ledger row was stale until 2026-09-23.** `background/scheduled_runs/trigger.rs:888-899` now names `extension::executor::scheduled_runs::tests::a_scheduled_run_really_spawns_a_process_through_the_production_launcher` (`extension/executor/scheduled_runs.rs:490`). It says `tests/scheduled_runs_integration.rs` never existed. That test drives the real `schedule.create` and `schedule.run` through `install_scheduled_runs`, the only construction site of `ExecutorScheduleLauncher`, with no injected launcher. It then checks three pieces of evidence written by production code: a marker file written by the spawned child, the fired run's terminal `status.json`, and the `ResultFile`'s `scheduleOrigin.id`. *History follows.* — **FILED 2026-09-16** while verifying `SUBA-016`'s closure. `background/scheduled_runs/trigger.rs:884-888` justifies stubbing the launcher in its unit tests with *"the integration suite (`tests/scheduled_runs_integration.rs`) covers the production one"*. **There is no such file:** the crate has no `tests/` directory at all (its tests are at `src/tests/`, 15 files, none about schedules) and `find . -name 'scheduled_runs_integration*'` is empty workspace-wide. The seam it promises is covered — `ExecutorScheduleLauncher` (`extension/executor/scheduled_runs.rs:39-51`), the only thing that actually spawns a `RunMode::Workflow` run — has exactly three references in the crate: its declaration, its `impl ScheduleLauncher`, and its one construction at `:317`. No test names it. The surrounding suite is unusually strong (all nine verbs through `Tool::execute`, 100/100 green), which makes the one gap easier to miss, not harder. **Fix:** write the row the comment promises, or correct the sentence to say the production launcher is not covered — a false coverage claim is worse than an acknowledged gap because it stops the next reader looking. |
| ~~SUBA-098~~ | ~~low~~ **CLOSED 2026-09-23** | cyrup-original | S | A stray CJK token inside an English doc comment — **CLOSED at `c92f739`**: `观察` → `observe` at `extension/tool/scheduled_runs_tests.rs:31`. No test is proportionate: a Han-in-comments lint would false-positive on `cyrup-tui/src/editor/wrap.rs`'s deliberate code-point documentation. The sibling `cyrup-tui/src/tests/selector.rs:41` (`末-position`) is area 07's row and is not touched. *History follows.* — **FILED 2026-09-16** while verifying `SUBA-016`'s closure. `extension/tool/scheduled_runs_tests.rs:31` reads *"a trigger that read it live would观察 a different identity after the swap"*; the verb is replaced by 观察 with no surrounding space, in the comment that justifies `SwappableSessionHost`'s mutability for two session-pinning tests. A workspace Unicode-script sweep (`rg '[\p{Han}\p{Hiragana}\p{Katakana}\p{Hangul}]' crates/ -g '*.rs'`) shows every other match is deliberate test DATA — CJK width fixtures in `cyrup-tui`, `"日"` in the schedule-interval rejection table, `"あ"` in the output-truncation test — with exactly one other accidental instance, `crates/cyrup-tui/src/tests/selector.rs:41` (*"an empty/末-position caret"*), which belongs to **area 07** and is named here only so the sweep is not re-run. Cosmetic; filed because ids are never deleted here, so it can be closed in one line whenever the file is next touched. |
...
## ~~SUBA-017 — Completion batching unported~~ — CLOSED 2026-09-23

> **CLOSED 2026-09-23.** See the row above for what landed (`8b84484` pump re-drain + merge tests;
> `b5f6393` batcher, grouped notice, `completionBatch`). The **Impact** and **Fix** lines below are
> kept as history and were wrong: ten runs finishing together never produced ten turns (the session
> pump merged them); an idle parent spent 2 turns and a busy one up to 3, the first reacting to 1/N of
> the fleet. And the batcher does not belong "between `CompletionWatcher` and `CompletionSink`" — that
> seam is above `InlineAnsweredSink` and cannot order a failure behind a flushed group; it sits at the
> injector, below the decorator (`.flux/todo/COMPLETION_BATCHING.md` §4).
...
## ~~SUBA-061~~ — ~~low~~ **CLOSED 2026-09-23 (`e75fb16`)** — Four documented config keys are silently ignored

> **CLOSED.** `asyncWidget`, `fleetKeybindings` and `inlineToolDisplay` each have a production reader. `legacyChainControls` was deleted upstream and stays struck. The evidence is in the `## Open items` row. The body below is history.
...
## SUBA-063 — Zero-tool-budget authorisation and the runtime-extension acknowledgement path are unported — **CLOSED 2026-09-23** (was: RE-DERIVED 2026-09-23: open on (b) only)

> **CLOSED 2026-09-23 (TAIL batch).** (b) the acknowledgement protocol is ported end to end — child
> collector on the bus, `0600` file hand-back, sanitized parent read-back, carried on `SingleResult`,
> `StepStatus`, `RunStatus`, the `ResultFile`, the remembered foreground child, `_meta.json` and the
> RPC `ping` capability; evidence and tests in the `## Open items` row. (c) closed at `2f4d0a5`. (a)
> rides `SUBA-022` (upstream's only writer is the delegation bridge's `executeDelegated`, absent here).

> **Re-derived from scratch 2026-09-23** (`.flux/todo/SILENT_DROPS.md` §4). (a) is not owed here; it rides `SUBA-022`, because the delegation bridge is upstream's only producer of the authorisation, and `exec/tool_budget.rs` now marks the env var `[CYRUP-DELTA]`, since upstream removed it at v0.67.0. (b) the acknowledgement path is open (M), a missing feature that is not silently dropped. (c) the truncation marker is CLOSED at `2f4d0a5`. The body below is history.
...
## ~~SUBA-097~~ — ~~low~~ **CLOSED at `d00a314` (2026-09-17)** — `scheduled_runs`' production launcher is claimed to be integration-tested by a file that does not exist

> **CLOSED.** The integration test was written (`extension/executor/scheduled_runs.rs:490`, `a_scheduled_run_really_spawns_a_process_through_the_production_launcher`) and the `trigger.rs` comment was corrected to name it. This ledger entry was stale until 2026-09-23. The body below is history.
...
## ~~SUBA-098~~ — ~~low~~ **CLOSED 2026-09-23 (`c92f739`)** — A stray CJK token inside an English doc comment in the scheduled-runs test module

> **CLOSED.** `观察` → `observe`. The body below is history.
