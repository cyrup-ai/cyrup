...
> ### RE-VERIFICATION 2026-09-24 — cyrup `ea23ca2`, pi-subagents `v0.71.0`; the out-of-scope window now has an owner
>
> **Pins for this pass:** cyrup HEAD **`ea23ca2`** (merge of #152; code commit `df3e9a8`).
> pi-subagents **`v0.71.0`** is the newest tag (clone HEAD `v0.71.0-10-g6f1027f7`, and that range
> is not read). **No older tag citation in this file was rewritten.**
>
> **Summary table re-checked.** The one open row, **`SUBA-106`** (agent frontmatter `outputSchema`),
> was re-read on both sides and is **still open**. `exec/agent_config.rs::AgentConfig` has no schema
> field. Upstream parses the key at `src/agents/agents.ts:2220-2224` @v0.71.0; the row's `:2152-2157`
> is correct at v0.68.0. The `SUBA-099`…`SUBA-105` closures from 2026-09-23 were spot-checked
> port-side, not re-audited. Their code is at `ea23ca2`, and `df3e9a8` is their landing commit (the
> rows say "uncommitted at writing").
>
> **Ownership.** `v0.57.0..v0.71.0` now belongs to **`09b-cyrup-ext-subagents-v0.64-drift.md`**,
> which this pass created. It files `SUBA-107`…`SUBA-113`. The `## UNVERIFIED census 2026-09-14`
> block below is still a list of leads. By scope, `09b` owns it now. It was **not** re-verified, and
> it stays here as history. `09b` settles two of those leads: the watchdog `fallbackModels` lead is
...
> **RE-CHECKED 2026-09-16 against cyrup `cc7818b` (area-09/09a reconciliation, after PRs #137/#139/#140)
> — the table's closures stand; ONE accounting defect found and fixed.** This file's rows were all
> struck (closed or partially closed), so `scripts/count_open_items.py` reported **zero open rows for
> `09a`**. That was wrong: **`SUBA-081` is PARTIALLY CLOSED with a live residual and no open id**,
> which is precisely the defect the 2026-09-04 batch-2 review recorded here when it split
> `SUBA-087`/`SUBA-090`'s residuals out as `SUBA-093`/`SUBA-094` — *"the two PARTIALLY CLOSED rows each
> carried a MEDIUM residual that `scripts/count_open_items.py` could not see (struck id ⇒ closed)"*.
> `SUBA-081` was left partial in the same pass without that split. It is now filed as **`SUBA-096`**,
> and the residual is re-measured: **three fields, not four** — `default_provider` landed with
> `SUBA-088` (`ba24e5e5`) and was double-counted, so the row overstated its own remainder by a third.
> `SUBA-081` stays struck; no id was renumbered or deleted. Nothing else in this file changed: the
> confirmed closures were spot-checked port-side and none regressed under the SCOPE sequence.
>
> **This file's scope line still binds it to `v0.47.1..v0.57.0`, and `v0.57.0..v0.68.0` remains owned
> by no file.** Upstream latest re-measured this pass with
> `git -C tmp/pi-subagents tag --sort=-v:refname | head -1` = **v0.68.0**.
...
| ~~SUBA-081~~ | ~~high~~ **PARTIALLY CLOSED 2026-09-04** | M | discovery / settings overrides | Ten `agentOverrides` fields never apply, and a legal `tools: "inherit"` fails the settings load — 6 of 10 landed, 4 fields remain. **Re-measured 2026-09-16 at `cc7818b`: the residual is THREE fields, not four** — `default_provider` landed with `SUBA-088` (`ba24e5e5`) and was double-counted; `AgentOverrideConfig` is now 21 fields and `acceptance_role` / `output_mode` / `fast` are the only ones missing. The residual is **filed as `SUBA-096`**, on this file's own `SUBA-093`/`SUBA-094` precedent — until now it was live work behind a struck id, which is why this file reported zero open rows while carrying some |
...
| ~~SUBA-096~~ | ~~medium~~ **CLOSED 2026-09-23** | S | discovery / settings overrides | **CLOSED at `c7a56ed`**: `acceptanceRole`, `outputMode` and `fast` are on `AgentOverrideConfig`. Each is validated with pi's text, applied in `merge.rs`, and consumed in production. `fast` is ported in full: frontmatter, override, call and step keys on four schemas, the `FAST_MODE_ALLOWED_MODELS` allowlist, the `CYRUP_SUBAGENT_FAST` env hand-off, the child `before_provider_request` hook that sets `service_tier: "priority"`, and the external-CLI refusal. The three v0.68.0 keys this row left out (`machine`, `inheritGlobalContext`, `mutationTools`) are now reported instead of dropped, and are filed as `SUBA-100`…`SUBA-102`. *History:* `SUBA-081`'s residual as its own row: three `AgentOverrideConfig` fields (`acceptanceRole`, `outputMode`, `fast`) are still not on the struct, so each is accepted into `agentOverrides` and silently dropped |
| ~~SUBA-099~~ | ~~medium~~ **CLOSED 2026-09-23** | S | discovery / settings overrides | Filed and closed together; this was the precondition `.flux/todo/SILENT_DROPS.md` §1.6 named. A custom (and package) agent's settings override was gated on "fill-unset-only", pi's rule up to v0.57.0. So a custom agent that declared `model:` kept its frontmatter value over a settings override, silently. Upstream `31562d76` (first tag v0.64.0) made the custom arm `applyBuiltinOverride` unconditionally, and v0.68.0 keeps it that way (`agents.ts:1532`). **Ported at `483cf88`**: `apply_custom_override` delegates to `apply_builtin_override`, and the Package arm now receives its override. |
| ~~SUBA-100~~ | ~~low~~ **CLOSED 2026-09-23** | L | discovery / Herdr placement | **Ported.** *Entry paths:* `machine` is read and validated with pi's `validateOptionalMachine` text (`agents.ts:979-986` @v0.68.0; `placement/resolve.rs`) from agent frontmatter (`:2168`, `KNOWN_FIELDS` + serializer after `mutationTools`, `agent-serializer.ts:129`), `agentOverrides` (`:1118-1121`, `false` clears, applied in `merge.rs`; it left `UNPORTED_OVERRIDE_KEYS`, which is now empty), runtime definitions (`runtime-agent-registry.ts:240,357`; 36 supported fields), `.chain.md`/`.chain.json` steps and a parallel step's own `machine` (`chain-serializer.ts:25`, `async-execution.ts:1269`), and the tool's `machine` param on single, `tasks[]` items, chain steps and the call (schema on all four shapes; a blank or whitespace machine is refused at the tool boundary with `Herdr machine id or label is required.`, upstream's `minLength: 1`, instead of silently moving the call's `cwd` onto no machine); with a machine (the call's, or a single agent's own) the typed `cwd` becomes `machineCwd` and is never resolved locally (`subagent-executor.ts:6176-6195`; workflow children likewise, `:3548-3560`). Management: `config.machine` on create/update (`false`/`""` clears) `[CYRUP-DELTA]` (upstream's `applyAgentConfig` has no such key), `get` renders `Machine: <m> (saved Herdr placement)`, `list` carries `runnerListBadge`'s placed arms (`agent-management.ts:709-718`; the capability-mode rendering is `SUBA-104`'s); `reset` keeps the pin and says `Retained machine placement.` (`:1388`); a profile apply preserves existing pins (`profiles.ts:495-496`). *Launch:* `s.machine ?? call ?? a.machine` is refused per `formatHerdrMachineRunnerUnsupported` (job runner, generic external-cli, managed worktree) and resolved against `herdr machine list --json` + `subagents.machines.<name>.{cwd,env}` (project over user, keyed by selector/label/id; a non-empty env refused) with every upstream sentence, BEFORE any run exists, on the foreground single, async single, and chain/parallel/dynamic graph launches (foreground and background; `placement/resolve.rs`, one production resolver `PlacementResolver`), on `append-step` (the step's own validated `machine` ?? the agent's, its `cwd` a machine path, resolved BEFORE the step is enqueued, `async-execution.ts:990`) and on `revive` (the current agent's `machine`, `:1774`; `extension/executor/control.rs`). The resolved reference rides the step to the detached runner, which refuses a requested-but-unresolved step instead of running it locally. *Execution* (`placement/native.rs`, `placement/external.rs`, `placement/remote_resources.rs`; every Herdr and ssh interaction goes through `cyrup-herdr` — its herdr API client (`workspace.create`/`tab.create`/`agent.start`/`agent.prompt`/`agent.get`/`pane.process_info`), `machine.rs` (catalog), `remote.rs` (upstream's hardened ssh argv/env, `remoteShellCommand`, endpoint discovery, StreamLocal forward, and the long-lived `SshTransport::stream` + `MachineConnection::is_lost`) and the new `relay.rs`): a native child's argv/env contract is serialized with `serializeHerdrPiLaunch`'s refusals (fork/resume, extension paths, nested delegation, permission/tool-budget/MCP-direct/required-tool contracts; any other local path by name) and ships no local system prompt or persona: it names LOGICAL resources (`remoteResources`, `child-launch.ts:309`: the agent name, the skill names, the `--tools` ceiling, `reads`) in `CYRUP_SUBAGENTS_HERDR_RESOURCES`, and the machine's `cyrup` resolves them against ITS checkout when its subagents runtime is built (`resolveRemoteHerdrResources`, `herdr-pi-bridge.ts:34-45`: agent body + memory + skills + reads, upstream's refusal sentences, a failure fails the child launch) and applies them at `BeforeAgentStart` (`:119`: the prompt is replaced with the machine-composed one tagged `<active_agent/>`, active tools set; `prompt_runtime.rs`), writing `configured.json`; the launching side injects no read line or skill block for a placed child (`subagent-executor.ts:3941`). The run is uploaded to a run-private `mktemp` dir (`umask 077`, its run id recorded), started with `sh <rt>/run.sh` in a fresh Herdr-owned pane (deterministic owned workspace `cyrup-subagents-<sha10(cwd)>`, cross-process allocation lock, 20-pane limit, journal), and driven by an in-process `cyrup_herdr::relay::RunRelay` — no local shell wrapper — over ONE ssh stream per connection: a framed protocol (events from a byte offset, stderr tail, exit, loss) with upstream's bounded reconnect (3 attempts / 15 s, resuming from the delivered offset; a foreign run dir is refused; a pane that died without an exit is `lost`); the attempt runner consumes it as `SpawnedChild::relayed` exactly like a local child. The live bridge channels cross the machine both ways through the same relay: the child's `contact_supervisor` requests come down into the parent's supervisor channel and the parent's replies go up (uploaded, then removed locally), the parent's steer inbox goes up and the child's acks and steer capability come down; the placed child always uses the native supervisor tool. Capture/diagnostic/acknowledgement files, the remote Git before/after pair (`execution.ts:554,1502-1503`) and a 4 KiB remote stderr tail are fetched back; the evidence is `SingleResult.nativeMachine` / `StepResult.nativeMachine` / `StepStatus.nativeMachine` (`types.ts:1268`, `subagent-runner.ts:1549,1579`) as well as the mutation evidence; a failed placed child's error gets upstream's remote-failure hint matched against that real stderr tail; the pane is closed and the dir removed on settle/stop/timeout, retained when the machine was lost. Owned claude/codex/cursor profiles run pane-native (`herdr-external-adapters.ts`: preflight identity/version floor/help options, sandbox argv, `agent.start`, `agent.prompt` or the Codex terminal monitor) and, when the herdr transport is lost mid-run, reconnect as `HerdrExternalSession.reconnect` does (`:137-178`: a fresh forward within the same budget, the same session, the same terminal in the same pane, the same canonical native pid/cwd/argv0, each drift refused with upstream's sentence; the prompt is not re-sent); they settle `[best-effort/unverified]` at exit 1 with `execution: { status: "partial" }` (`"stopped"` when stopped; `subagent-runner.ts:900-944`). *Run-level partial* (`subagent-runner.ts:1831-1845,4953-4997`, `partialWithEvidence`): `RunState::Partial` / `StepState::Partial` (`"partial"`, terminal); a step whose result settled partial (and did not time out) is `partial`, a run whose only shortfall is partial evidence settles `partial` (a concrete failure beside it keeps `failed`), with status and the failed/partial steps `needsAttention`, `success: false`; every consumer of the run/step state (status labels and ranks, wait and wait subscriptions, watch, workflow graph/settlement/child summary, retained children, cleanup, active capacity, control import) handles it. The runner status carries the machine (`external-cli-contract.ts:107,131`). *Not ported, with evidence:* upstream's `requestedCwd` has no cyrup counterpart on ANY run (not a placement surface: pi reads it only for `preflightLaunchCwd`, and `AsyncStatus.cwd` is the runner cwd in both); upstream's placed-run `writerChangesNote` output note is unreachable upstream (its only caller sits on the ssh-wrapped path that `subagent-runner.ts:900` returns ahead of), so `placement/hints.rs` keeps only the failure hint. `[CYRUP-DELTA]`s: the local ssh binary is `CYRUP_HERDR_SSH_BIN` (else `ssh` on the hardened PATH) and, with `HERDR_BIN`/`SSH_AUTH_SOCK`, is read through `SubagentExtensionConfig::env_overrides` and forwarded to the detached runner; a placed run writes no recovery descriptor — upstream writes one, but a pane-native run has no session file (`herdr-placed-run.ts:213`) and refuses revival outright (`:90`, "Pane-native remote Pi does not support fork, resume, or revival; use fresh context."), so no revive of a placed run can succeed in either; the remote-failure hint is applied to a placed native failure (upstream's `decorateHerdrMachineResult` is reachable only on its ssh-wrapped path). Tests: `placement/tests.rs` — `the_optional_machine_validator_speaks_upstreams_sentences`, `the_catalog_selects_by_id_then_unique_label_and_refuses_the_rest`, `the_remote_cwd_is_resolved_from_the_step_cwd_and_the_configured_root`, `machine_settings_are_read_from_both_scopes_project_first`, `the_catalog_is_read_from_the_herdr_binary`, `unsupported_runners_and_worktrees_are_refused_before_launch`, `step_beats_call_beats_agent_and_the_cwd_moves_onto_the_machine`, `a_graph_launch_resolves_every_member_and_refuses_the_first_bad_one`, `a_single_run_takes_the_call_machine_over_the_agents`, `a_step_placement_round_trips_the_runner_config_without_its_transport`, `frontmatter_runtime_definitions_and_chain_files_carry_the_machine`, `the_management_surface_creates_shows_and_clears_a_machine`, `the_native_launch_serializes_for_the_machine_or_refuses`, `placed_mutation_evidence_is_the_remote_git_pair`, `a_placed_native_run_executes_in_a_herdr_pane_on_the_machine`, `a_failed_placed_child_reports_the_remote_error_with_the_machine_hint`, `a_placed_run_with_a_local_session_is_refused_before_touching_the_machine`, `a_timed_out_placed_run_closes_its_pane`, `a_placed_childs_supervisor_and_steer_channels_reach_the_parent`, `a_placed_child_resolves_its_agent_skills_and_reads_on_the_machine`, `a_placed_external_run_reconnects_a_lost_transport_and_settles` (fake `agent.start`/`agent.prompt`/`agent.get`/`pane.process_info`, forward killed mid-prompt), `the_remote_external_preflight_holds_upstreams_floor_and_options`, `the_pane_native_launch_argv_is_the_profiles_sandbox`, `placed_terminal_evidence_is_sanitized_and_tagged_unverified`, `the_codex_monitor_settles_on_stable_ready_output_and_retains_on_drift`; `cyrup-herdr` `tests/relay.rs` (`the_relay_streams_events_and_mirrors_both_channel_directions`, `a_dropped_stream_resumes_from_the_delivered_offset`, `a_child_whose_pane_died_is_reported_lost`, `a_lost_transport_is_unknown_and_a_foreign_run_is_refused`) and `relay::tests::headers_parse_and_refuse_garbage`, plus its `machine_remote` tests; `extension/tool/routing_tests.rs::a_blank_machine_is_refused_at_the_tool_boundary`; `extension/executor/control.rs::append_step_places_the_step_or_refuses_it_before_enqueueing`; `extension/executor/background.rs::a_revived_agent_that_names_a_machine_is_placed_there`; `background/runner_main/finish.rs::a_run_whose_only_shortfall_is_placed_partial_evidence_settles_partial`; `background/runner_main/status.rs::a_partial_execution_settles_its_step_partial_with_its_machine_evidence`; `discovery::tests::override_machine_is_read_trimmed_cleared_and_validated`; `cyrup-it` `herdr_machine_placement_integration` (real tool, real detached runner process, fake ssh + fake herdr; blank-machine refusal; the detached step's `nativeMachine`). |
| ~~SUBA-101~~ | ~~low~~ **CLOSED 2026-09-23** | M | discovery / context inheritance | **Ported in full.** `inheritGlobalContext` (`agents.ts:66,97,151` @v0.68.0) is read on every entry path: frontmatter strictly `=== "true"`, default `false` (`agents.ts:2070`; `discovery/frontmatter.rs`, `KNOWN_FIELDS` + serializer `frontmatter_write.rs`, written only when true or preserved, `agent-serializer.ts:90`); `agentOverrides` (validated with pi's text `:1055-1060` in `discovery/mod.rs::validate_override_context_and_mutation_keys`, applied `:1447` in `merge.rs`, recorded in override provenance); management `config.inheritGlobalContext` on create/update (`agent-management.ts:536-539`, `config_parse.rs`, `agent_crud.rs`) and rendered by `get` (`:947`, `render.rs`); the runtime registry now ACCEPTS it (`?? false`, `runtime-agent-registry.ts:341`) and it left `UNPORTED_OVERRIDE_KEYS`/`UNREPRESENTABLE_FIELDS`. Carried on `AgentConfig`/`ResolvedAgentPersona` (every chain/parallel/fanout/background step dispatches from the persona) and handed to the child as `CYRUP_SUBAGENT_INHERIT_GLOBAL_CONTEXT=1|0` beside the project flag (`exec/spawn_plan.rs`; pi `child-launch.ts:261`). Child side: `prompt_runtime.rs::strip_global_context` ports `stripGlobalContext` (`subagent-prompt-runtime.ts:144-181,206-214`): inside `<project_context>` it removes only the `<project_instructions>` blocks whose file is an `agents.md`/`agents.override.md`/`claude.md` in the canonicalized agent dir this child's env resolves, independently of the project flag; env absent => inherit (`?? true`). Recovery descriptor writes it (`async-execution.ts:2024`), a pre-field descriptor takes `inheritProjectContext` (`async-resume.ts:368`, `RecoveryDescriptor::effective_inherit_global_context`), and the overlay/synthesised persona apply it (`:621`, `subagent-executor.ts:1900`); the launch digest binds it (`launch-contract.ts:122`; `recovery_descriptor.rs::launch_binding_projection`). *Finishing pass 2026-09-23:* the same legacy rule now covers the persona hand-off — a `ResolvedAgentPersona` serialized without the key (a `runner-config.json` from a pre-field binary) reads it as its own `inheritProjectContext` instead of `serde(default)`'s `false`, which is also exactly what the pre-field child did (project flag on kept the whole `<project_context>`, off stripped it); `exec/agent_config.rs`'s hand-written `Deserialize` over the derived one. `[CYRUP-DELTA]` pi's legacy `# Project Context` markdown form is not rewritten: cyrup never emits it. Not ported because cyrup has no port of the surface: `api/preflight.ts` launch contract, `herdr-pi-bridge.ts` (SUBA-100), `child-hooks.ts` in-process config validation. Tests: `inherit_global_context_and_mutation_tools_are_typed_frontmatter`, `override_inherit_global_context_and_mutation_tools_are_read_and_validated`, `override_inherit_global_context_and_mutation_tools_apply_through_discovery`, `management_config_inherit_global_context_and_mutation_tools_round_trip`, `inherit_global_context_and_mutation_tools_register_and_land`, `inherit_global_context_and_mutation_tools_survive_every_projection`, `build_attempt_spawn_plan_threads_inherit_global_context_independently`, `strip_global_context_removes_only_agent_dir_context_files`, `strip_global_context_compares_canonical_directories`, `rewrite_applies_the_global_flag_independently_of_the_project_flag`, `the_child_runtime_strips_global_context_when_the_parent_says_so`, `inherit_global_context_false_removes_only_the_global_agents_md_from_the_real_prompt` (real `ContextFileLoader` + `SystemPromptBuilder` through `ExtensionHost`), `a_pre_field_descriptor_takes_the_project_flag_and_the_new_fields_are_validated`, `a_revived_run_replays_fast_global_context_and_mutation_tools`; finishing pass: `a_pre_field_persona_reads_global_context_as_its_project_flag`, `the_launch_digest_binds_global_context_mutation_tools_and_fast` (the projection key, not just the digest — the persona also feeds `definitionDigest`), `an_unrelated_update_keeps_global_context_and_mutation_tools_and_their_lines` (merge keeps an unstated key; a hand-written `inheritGlobalContext: false` is preserved), `a_stated_inherit_global_context_false_is_written_although_the_file_lacked_it` (`:366-369`'s re-add). |
| ~~SUBA-102~~ | ~~low~~ **CLOSED 2026-09-23** | M | discovery / mutation detection | **Ported in full.** `mutationTools` (`agents.ts:80,109,168` @v0.68.0) is read from frontmatter (`parseFrontmatterList`, kept only when non-empty, `:2117,2205`; serializer `agent-serializer.ts:126-127`), `agentOverrides` (`string[] | false`, pi's `parseOverrideStringArrayOrFalse` refusal text and trim/drop-empty, `:947-966,1152`; `false` deletes, `:1459`), management `config.mutationTools` (`false` deletes, `""` empties, CSV otherwise, else pi's refusal — `agent-management.ts:514-518`; rendered `Mutation tools:` `:957`) and the runtime registry (`:356`); it left the unported/unrepresentable lists. Carried on `AgentConfig`/`ResolvedAgentPersona`/the recovery descriptor (written `async-execution.ts:2020`, validated non-empty strings `async-resume.ts:372-375`, overlaid `:617`). Consumed on every path upstream consumes it: `exec/control.rs::is_mutating_tool(name, args, mutation_tools)` checks the list first (`long-running-guard.ts:146`) and is now the ONE classifier — `completion_guard::has_mutation_tool_call` delegates to it (`completion-guard.ts:145,246`; this also brought in upstream's `cursor` arm the private copy lacked); the completion guard reads `agent.mutation_tools` via `completion_guard_projection`; the control loop's failure-streak escalation gets it via `ControlMonitor::with_mutation_tools` (`execution.ts:1059`, and the async runner's `subagent-runner.ts:3042`, which in cyrup is the same `run_sync`); the startup-retry evidence's `observed_mutation_attempt` uses it (`run-child-session.ts:471`). `parallel-utils.ts`/`dynamic-fanout.ts` are runner-step key lists: cyrup's steps dispatch from the persona, which carries it. Tests: `listed_mutation_tools_count_as_mutating`, `an_agent_s_mutation_tools_reach_the_guard_and_the_control_loop` (`run_sync` + scripted child: with the list the guard is satisfied and the streak escalates; without it the guard fails the run), `is_mutating_tool_covers_edit_write_cursor_and_bash`, plus the entry-path and carry tests named in `SUBA-101`; finishing pass: `a_declared_empty_mutation_tools_list_renders_none` (`:957`'s `(none)`), the preserve/merge and launch-digest tests named in `SUBA-101` (`mutationTools` reaches the digest through `definitionDigest`, `launch-contract.ts:62`). **Not part of this closure, filed separately as `SUBA-104` (open):** upstream also SHOWS `mutationTools` in management `list`'s `capabilities: true` rows (`agentCapabilityTools`, `agent-management.ts:812-820`; `AgentCapabilityRow.tools.mutationTools`, `types.ts:1388`) — cyrup has no capabilities mode at all, so that row has no home until the mode is ported. *(2026-09-23: `SUBA-104` is CLOSED — the capability rows carry `tools.mutationTools` (`discovery/management/capabilities.rs::agent_capability_tools`), pinned by `capability_list_renders_upstream_rows_without_system_prompts`; see its row.)* |
| ~~SUBA-103~~ | ~~low~~ **CLOSED 2026-09-23** | S | background revive / recovery descriptor | **Fixed.** `RecoveryDescriptor::for_single_launch` writes the launch's EFFECTIVE `fast` (`step.fast`, already `s.fast ?? params.fast ?? a.fast`; pi `async-execution.ts:2008`) and the revive at `extension/executor/control.rs` relaunches with `fast: descriptor.fast.or(<the agent's CURRENT fast>)` — pi passes `fast: recoveryDescriptor?.fast` (`subagent-executor.ts:2147`) and relaunches with `params.fast ?? agentConfig.fast` (`async-execution.ts:1967`), where `agentConfig` is the freshly DISCOVERED base under `applySteeringRecoveryAgentConfig`'s overlay, which never touches `fast` (`async-resume.ts:604-632`); a synthesised base (agent gone) has none. *Corrected 2026-09-23 (finishing pass):* the first cut used `descriptor.fast` alone, so a descriptor with no `fast` — every launch where no rung set it, and every pre-field descriptor — revived at the standard tier even when the agent file had since turned `fast` on; the revive now resolves the discovered definition (not just the persona) to get that rung. The revived run records the effective value again on its own descriptor; the launch digest binds it (`launch-contract.ts:117`). `[CYRUP-DELTA]`, stated precisely: the descriptor records `step.fast`, the launch's effective `s.fast ?? params.fast ?? a.fast`, which is `None` when no rung set it (recorded as absent, exactly like pi) and `Some(false)` only when a call, step or agent file said `false` — pi drops that `false` (it writes `fast` only when truthy, `:2008`), so its revive of a deliberately-standard launch takes the file's current value and can come back fast; cyrup replays the recorded `false`. The second `fast: None` (`control.rs` append-step) was also a drop: pi builds an appended step through `buildAsyncRunnerSteps` (`subagent-executor.ts:1370`, `s.fast ?? a.fast` at `async-execution.ts:1118`), so it now resolves the step key, else the agent's, and refuses a foreign runner with fast (`:1012`). Launch-time refusal: an async launch of an `external-cli`/`external-job` agent whose effective fast is true is refused in `spawn_background_steps` BEFORE any directory, status or runner exists (`async-execution.ts:1012,1756`), with pi's sentence. Tests: `a_revived_run_replays_fast_global_context_and_mutation_tools` (launch -> settle -> file rewritten without fast -> `action: "resume"`), `a_revive_with_no_recorded_fast_takes_the_agents_current_fast` (no rung set fast -> nothing recorded -> file turns `fast: true` -> the revive runs fast and records it; a hand-recorded `false` stays standard), `the_launch_digest_binds_global_context_mutation_tools_and_fast`, `for_single_launch_projects_every_field_of_the_resolved_launch`, `the_on_disk_form_is_pi_s_and_round_trips`, `append_step_resolves_fast_and_refuses_it_for_a_foreign_runner`, `an_external_cli_agent_with_fast_is_refused_before_any_run_exists` (through the `subagent` tool: call-level, agent-level and chain-step fast; asserts no run dir and no result). |
| ~~SUBA-104~~ | ~~low~~ **CLOSED 2026-09-23** | M | management `list` / capability rows | **Ported.** *Schema/entry:* `capabilities` is on the `subagent` tool schema right after `action`, boolean, description verbatim (`extension/schemas.ts:287` @v0.68.0; `extension/tool/schema.rs`), parsed as `Option<bool>` (`extension/tool/params.rs`) so a non-boolean is refused at the parse as upstream's `Type.Boolean` refuses `"true"` (`schemas.test.ts:313-314`); `route_management_action` passes `capabilities === true` and the live session id to `discovery::management::handle_management_action_with(.., &ListOptions)` and spreads the handler's extra details over `{ mode: "management", results: [] }` (`result`, `agent-management.ts:55-57`). *Listing (`discovery/management/handlers.rs::handle_list`, `:971-1005`):* the session's ceiling (`resolveCurrentSubagentCapabilityCeiling(ctx.currentSessionId)`, via `exec::capability_ceiling::resolve_current_capability_ceiling`, fail-closed on a malformed inherited value) splits the visible agents into executable ones and a `Restricted agents (not executable in this session; capability ceiling: <sources>):` block (`appendRestrictedAgentLines`, `:852-859`) in BOTH modes; with `capabilities: true` the answer is `Executable agents (capabilities):`, upstream's source sections (`formatAgentListSections`, `:885-905`), the restricted block, the invalid-definition diagnostics and the proactive block — no chain block, as v0.68.0 lists none. *Rows (`discovery/management/capabilities.rs`, new):* `formatAgentCapabilitiesLine` (`:735-753`: metadata with `runnerListBadge` incl. the availability-bearing external-cli and placed badges `:708-719`, `Description` previewed to 240, `Tools` `default/ambient`/`none`/list + `; excludes:`, `Model` with the provider prefix for a bare model, `Thinking` `off`/`default`, `; Machine:`, `formatAcceptanceSummary` `:755-780` with `formatAcceptanceDisplayLabel`'s bounded JSON-quoted labels and `formatReviewGateLabel`); `agentCapabilityRow` (`:822-836`, upstream key order: `name`, `description` previewed to 1000, `source`, `executable`, `restrictionSources`, `aliases`, `runner` — `pi` / `external-cli` {`adapter`, `command`, `machine`, `available`, bounded `unavailableReason`, `capabilities` = `runner::status::resolve_external_cli_runner_status(..).capabilities`} / `external-job` {`provider`, `available`, the five-`false` envelope} — `tools` {`ambient`, `names`, `excludeTools`, `mcpDirectTools`, `mutationTools`}, and `presentDetails` `model`/`execution`/`acceptance`/`output`/`extensions`); `agentCapabilitiesSnapshot` (`:838-848`: `agents`, `restrictedCount`, `capabilityCeilingSources`). External-cli availability is the passive `resolveBinary` lookup (`exec::external_cli::preflight::resolve_binary` on `PATH`), a placed agent's is local ssh only (`placement::resolve::ssh_transport_available`) — no process, catalog or machine preflight. *Consumer:* the tool description's safety guidance now points at `{ action: "list", capabilities: true }` and `runner.available` (`AGENT_SELECTION_GUIDANCE`, `tool-description.ts:8`; `registration/tool_description.rs`, pin 1430 → 1510). *Tests (`src/tests/management_capabilities_integration.rs`, all through `SubagentTool::execute`):* `capability_list_renders_upstream_rows_without_system_prompts`, `capability_rows_report_ambient_and_excluded_tool_surfaces`, `capability_rows_render_thinking_false_as_off`, `capability_line_prefixes_a_bare_model_with_its_provider`, `capability_rows_report_bare_and_disabled_acceptance`, `capability_rows_report_optional_and_disabled_review_gates`, `capability_summaries_bound_acceptance_labels_but_keep_structured_values`, `capability_descriptions_are_previewed_for_line_and_row`, `capability_rows_report_passive_external_cli_availability`, `capability_rows_do_not_preflight_herdr_machines`, `capability_rows_report_unregistered_external_job_providers`, `capability_ceiling_moves_disallowed_agents_to_restricted_rows`, `plain_list_splits_restricted_agents_under_the_ceiling`, `capabilities_false_keeps_the_plain_listing`, `a_non_boolean_capabilities_flag_is_refused`, `capability_list_keeps_invalid_agent_diagnostics`, `the_capabilities_key_is_advertised_as_upstreams_boolean`; plus `the_safety_guidance_is_pinned_to_pis_v0_34_0_text`. *Deltas, stated:* cyrup registers no external-job providers, so the registry is present and empty (upstream's answer with nothing registered: `missing`, `available: false`; the `External-job provider registry unavailable` line cannot arise); cyrup has no `packageSourceLabel`, so a package agent reads `package` as the plain listing does; `subagentOnly`/`skills` have no declared-but-empty state in cyrup, so an empty list is omitted. *Not coverable yet:* the `report: …` acceptance modifier is ported but cyrup's acceptance validator still refuses `acceptance.report` (`acceptance frontmatter.report is not supported.`; upstream accepts it, `ACCEPTANCE_CONFIG_KEYS`, `runs/shared/acceptance.ts:53` @v0.68.0) — a separate acceptance-schema drift, not filed by this pass; the malformed-inherited-ceiling refusal needs an env var this `forbid(unsafe)` crate cannot set in-process. *Follow-up 2026-09-23 (SUBA-105 pass):* both "not coverable" items now have tests. The `report: …` modifier is reached from a real agent file: `capability_list_renders_upstream_rows_without_system_prompts` now carries upstream's `report: on` fixture verbatim (`agent-management.test.ts:85,120,134` @v0.68.0), and `capability_rows_report_the_acceptance_report_toggle` covers `off`, a `report`-only policy rendered at `auto`, and a malformed value refused as an invalid definition. The malformed inherited ceiling is refused through the real tool entry on the async launch path: `SubagentExecutor::spawn_background_steps` reads `CYRUP_SUBAGENT_CAPABILITY_CEILING_V1` through `exec::capability_ceiling::resolve_current_capability_ceiling_from` over `SubagentExtensionConfig::env_overrides` (then the process), tested by `extension::executor::background::tests::inherited_capability_ceiling_refusal::a_malformed_inherited_capability_ceiling_refuses_an_async_launch`. *Follow-up 2026-09-23 (PROVE pass):* the two remaining direct process-env reads now go through the same seam. `handle_list` resolves the ceiling with `resolve_current_capability_ceiling_from` over `ListOptions::env_overrides` (built in `extension/tool/routing.rs` from `SubagentExtensionConfig::env_overrides`) — `tests::management_capabilities_integration::list_reads_the_inherited_capability_ceiling_through_the_env_seam` (malformed and wrong-version values refuse the listing, a well-formed one splits it into a `Restricted agents` block under its source, a scrubbed one lists every agent as executable). The foreground spawn preflight `exec::spawn_plan::preflight_capability_ceiling` reads through `RunOptions::parent_env_overrides` (`RunOptions::parent_env_var`), which `run_foreground_impl` fills from `env_overrides`; the detached runner passes none, since its own environment is its parent's hand-off — `extension::executor::background::tests::inherited_capability_ceiling_refusal::an_inherited_capability_ceiling_binds_a_foreground_launch` (malformed → `Invalid inherited capability ceiling: …`, excluding → upstream's `Capability ceiling from … does not allow agent 'worker'. Allowed agents: reviewer.`, scrubbed → unbounded). *History:* **Open — filed 2026-09-23 by the SUBA-102 finishing pass.** Upstream's management `list` takes `capabilities: true` (`extension/schemas.ts:287` @v0.68.0: "list: compact capability rows/details without system prompts.") and then answers `Executable agents (capabilities):` with `formatAgentCapabilitiesLine` rows (`agent-management.ts:735`) and a structured `details.agentCapabilities` snapshot (`agentCapabilitiesSnapshot`/`agentCapabilityRow`, `:822-850`; `AgentCapabilitiesSnapshot`/`AgentCapabilityRow`, `shared/types.ts:1373-1395`): per agent its runner (`pi`, `external-cli` with local/machine availability from `externalCliAvailabilityForAgents` and `resolveExternalCliRunnerStatus(...).capabilities`, `external-job` with provider availability), `tools` (`ambient`, `names`, `excludeTools`, `mcpDirectTools`, `mutationTools` — `agentCapabilityTools`, `:812-820`), model/thinking, execution, acceptance, output and extensions, plus restricted agents under the session's capability ceiling with its sources (`handleList`, `:971-1005`). cyrup's `list` has no `capabilities` parameter and no capability rows or details at all, so the `mutationTools` capability row (SUBA-102) and the placed external-cli availability badge (SUBA-100's list delta) have no home. Not started. |
| ~~SUBA-105~~ | ~~low~~ **CLOSED 2026-09-23** | M | acceptance / structured output | **Ported: `acceptance.report` and the structured-output acceptance report channel** (`runs/shared/acceptance.ts:53,189-197,267-269,404-430,516-563,1397-1464`, `runs/shared/structured-output.ts:11,17-18,64-74,132-148,185-214`, `runs/shared/subagent-prompt-runtime.ts:405-440`, `runs/shared/child-launch.ts:276-283`, `runs/foreground/execution.ts:1452-1461,1769,2019-2020`, `runs/background/subagent-runner.ts:810,863,1286-1292` @v0.68.0). *Validation:* `report` is in `ACCEPTANCE_CONFIG_KEYS` and takes only `on`/`off` (`exec/acceptance/model/validate_input.rs`), so agent frontmatter, runtime agent definitions, chain files and the tool's `acceptance` (every caller of that one validator) all accept it; `resolveAcceptanceReportMode` / `validateAcceptanceReportMode` / the per-site `validateExecutionAcceptance` report half are `exec/acceptance/model/report_mode.rs`. `lower_acceptance_input` carries the mode and the declared-key fact onto `AcceptanceContract::{report_mode, report_declared}` (a `report`-only policy is kept as an inferred-level policy, `explicitAcceptanceRequestsPolicy`). *Runtime:* `run_sync` refuses a declared `report` on a run with no `outputSchema` before spawning (`acceptance.report requires outputSchema.`), creates the structured runtime with `acceptance-report.json` unless the mode is `off` (`create_structured_output_runtime_with`), and hands the child the path and mode (`CYRUP_SUBAGENT_STRUCTURED_OUTPUT_ACCEPTANCE_REPORT[_MODE]`, `exec/spawn_plan.rs`); the child `structured_output` tool advertises `acceptanceReport` (required for `on`), refuses a missing one with `MISSING_STRUCTURED_ACCEPTANCE_REPORT_ERROR` and a malformed one with `Invalid structured output acceptance report: …` before capturing anything, writes the report before the value and clears a stale one (`prompt_runtime.rs`). The parent reads it back after a valid value (`read_structured_output_acceptance_report`) and the live gate (`exec/acceptance/lattice/gate.rs::evaluate_acceptance_with_structured_report`) rejects on a report error and otherwise uses the structured report INSTEAD of prose/file sources. The child is told `Include an \`acceptanceReport\` object in your final \`structured_output\` tool call …` with an unfenced example (`format_acceptance_prompt_with`, `inject_acceptance_contract_for`). Foreground, chain/parallel steps and background hop-2 all reach this through `run_sync`. *Tests:* `exec::acceptance::model::report_mode::tests::{report_mode_follows_upstreams_truth_table, a_declared_report_without_an_output_schema_is_refused, every_dispatch_site_is_checked_against_its_own_output_schema}`, `exec::acceptance::model::validate_input::tests::report_is_a_supported_key_taking_only_on_or_off`, `prompt_runtime::structured_acceptance_report_tests::{a_required_report_is_enforced_before_anything_is_captured, an_optional_report_is_captured_when_sent_and_cleared_when_not, no_report_channel_advertises_no_acceptance_report_parameter}`, `tests::structured_acceptance_report_integration::{a_required_structured_acceptance_report_that_never_arrives_rejects_the_run, a_structured_acceptance_report_satisfies_the_gate_without_any_prose_report, report_off_keeps_the_fenced_prompt_and_the_prose_report, a_declared_report_without_an_output_schema_is_refused_before_spawning, a_report_only_policy_still_arms_the_required_channel}` (real child process through `run_sync`, mirroring `single-execution.part-2.test.ts:1945-2060`), and the capability rows under SUBA-104. *Up-front per-site refusal:* `extension/tool/params.rs::validate_execution_acceptance` (pi `validateExecutionAcceptance`, `acceptance.ts:404-421`) now appends `validate_execution_acceptance_report_modes` after the policy half, so the tool entry (single/`tasks`/`chain`, and the `append-step`/`resume` control actions with their `Cannot append step: `/`Cannot resume: ` prefixes) refuses every site that declares `report` without an `outputSchema` beside it (absent or `false`) before anything runs, with upstream's labels `acceptance`, `tasks[i].acceptance`, `chain[i].acceptance`, `chain[i].parallel[j].acceptance` and `chain[i].parallel.acceptance`, joined by a space — `extension::tool::routing::tests::a_declared_report_without_a_schema_is_refused_per_site_at_the_tool_entry` (through `SubagentTool::execute`). *Deltas, stated:* upstream validates `tasks[i]`/`chain[i]` against the agent's EFFECTIVE schema (`projectEffectiveAcceptanceSchemas`, `subagent-executor.ts:2507-2517` → `resolveEffectiveOutputSchema`, `child-launch-plan.ts:64`), which falls back to the agent's frontmatter `outputSchema` (`agents.ts:2152-2156`); cyrup agents have no frontmatter `outputSchema` at all (an agent-definition drift outside this row, not filed here), so the item's own schema is the effective one. Upstream's `normalizeAcceptanceValue` JSON-string form of `acceptance` is not accepted anywhere in cyrup, so it is not special-cased here either. No agent-contract concept exists in this crate, so `reportOptional` is `false` for every caller, as before; the lattice ledger has no `childReport`/`runtimeChecks`, so a report error lands in `detail`. *History:* filed and ported 2026-09-23 by the SUBA-105 pass; the drift was first noted in SUBA-104's "not coverable yet" list. |
| SUBA-106 | **low** **OPEN — filed 2026-09-24** | M | discovery / agent frontmatter | Agent frontmatter `outputSchema` is unported. Upstream parses it as a JSON object (`agents.ts:2152-2157` @v0.68.0, `assertJsonSchemaObject`), and `projectEffectiveAcceptanceSchemas` falls back to it when a call or step names no `outputSchema`. So an agent can declare its own structured-output contract, and that contract satisfies an `acceptance.report` check. cyrup agents have no frontmatter `outputSchema`, so the SUBA-105 per-site `report` check sees only the call- or step-level schema, and an agent that carries its own schema upstream is refused here. Found by the SUBA-105 proving pass; not in this batch's scope. **RE-READ 2026-09-24 at `ea23ca2` / `v0.71.0`: still open.** `exec/agent_config.rs::AgentConfig` has no schema field, and the only `outputSchema` readers under `discovery/` are chain-step parsers (`discovery/types.rs:1455-1458`, `discovery/management/config_parse.rs:455-460`). Upstream at v0.71.0: `src/agents/agents.ts:2220-2224`, emitted at `:2273`. |
...
>
> **RESIDUAL RE-MEASURED 2026-09-16 at cyrup `cc7818b` — it is THREE fields, not four, and it now has
> its own open id.** `AgentOverrideConfig` (`discovery/types.rs`) carries **21** fields: `description`,
> `output`, `default_reads`, `model`, `fallback_models`, `default_provider`, `thinking`,
> `system_prompt_mode`, `inherit_project_context`, `inherit_skills`, `default_context`, `disabled`,
> `system_prompt`, `skills`, `tools`, `exclude_tools`, `allow_nested_subagents`, `extensions`,
> `subagent_only_extensions`, `completion_guard`, `tool_budget`. **`default_provider` landed with
> `SUBA-088`** (`ba24e5e5`) — that row's own closure says it added `agentOverrides.<name>.defaultProvider`,
> so this residual double-counted a field another row had already closed. `exclude_tools` and
> `allow_nested_subagents` came from `SUBA-092` (`247ff97b`). **Still absent: `acceptance_role`,
> `output_mode`, `fast`** — re-confirmed by enumerating the struct rather than by the grep, which is
> what let the `default_provider` overlap go unnoticed for a pass.
>
> **Filed as `SUBA-096`, per this file's own precedent.** A PARTIALLY CLOSED row with a struck id reads
> as closed to `scripts/count_open_items.py` — the exact defect the batch-2 review recorded here when it
> split `SUBA-087`/`SUBA-090`'s residuals out as `SUBA-093`/`SUBA-094`. `SUBA-081` was left partial
> without that split, so this file has reported **zero open rows while carrying live work** ever since.
> `SUBA-096` closes that hole; `SUBA-081` itself stays struck and is not reopened.
...
## ~~SUBA-096~~ — ~~medium~~ **CLOSED 2026-09-23 (`c7a56ed`)** — Three `agentOverrides` fields are still not on `AgentOverrideConfig`, so each is accepted into a settings file and silently dropped

> **CLOSED at `c7a56ed`.** On all three axes a settings override now changes the resolved `AgentDefinition`, and each value is consumed:
> * **`acceptanceRole`**: `OverrideField<AcceptanceRole>`. Applied in `merge.rs`, where `false` clears it, and read by `infer_level`. Pinned by `acceptance_role_override_sets_and_false_clears`. The discovery fixture's old `acceptanceRole: "reviewer"` is not a valid role at v0.68.0, so it became `"writer"`; commit `c7a56ed` says so.
> * **`outputMode`**: `OverrideOutputMode{Inline,FileOnly}`. It is applied onto the `OutputSpec` without disturbing its path, and a `false`, `"file-and-inline"` or `null` value is refused with pi's text (`override_output_mode_false_and_file_and_inline_are_refused_with_pis_text`). The agent's own mode is honoured at all five sites: foreground single, async single, parallel, chain and slash, through `resolve_effective_output_mode` and `apply_agent_launch_defaults`. Pinned by `output_override_preserves_mode_and_output_mode_override_preserves_path`, `an_agents_own_file_only_mode_is_consulted_on_the_{foreground,async}_single_path` and `agent_output_mode_reaches_every_step_shape_unless_the_step_set_one`.
> * **`fast`**: `OverrideField<bool>`, plus frontmatter `fast:`, step, call and item keys on the top-level schema and four item schemas (`fast_is_advertised_on_the_top_level_and_every_item_schema`). Precedence is step, then call, then agent (`fast_precedence_is_step_then_call_then_agent`). `exec/spawn_plan.rs` `fast_mode_env` enforces pi's `FAST_MODE_ALLOWED_MODELS` and refuses a launch that denies extensions, then writes `CYRUP_SUBAGENT_FAST=1`. The child's `prompt_runtime` hook rewrites the provider request to `service_tier: "priority"` (`fast_child_rewrites_the_provider_request`). An external-CLI agent refuses fast mode with pi's sentence (`external_cli_agent_refuses_fast`).
> * **The census**: `agent_override_config_models_every_v0_68_0_key_or_names_it_unported` pins v0.68.0's 26-key interface against the modelled keys plus `UNPORTED_OVERRIDE_KEYS`, so the next upstream key cannot be dropped silently.
>
> **Residuals, filed rather than left in prose:** `SUBA-100`…`SUBA-102` (the three unported keys) and `SUBA-103` (revive does not replay `fast`). The body below is history. *(2026-09-23: `SUBA-101`, `SUBA-102` and `SUBA-103` are CLOSED — see their rows.)* *(2026-09-23: `SUBA-100` is CLOSED — see its row.)*

**Kind** parity-bug · **Severity** medium · **Effort** S · **Confidence** confirmed
**Subsystem** discovery / merge (settings overrides)
**Window** in-baseline (≤ v0.43.0) for `acceptanceRole`; **v0.47.1..v0.57.0** for `outputMode` and `fast`
*(Filed 2026-09-16 by the area-09/09a reconciliation pass against `cc7818b`. This is `SUBA-081`'s residual given its own id, on this file's own `SUBA-093`/`SUBA-094` precedent — a PARTIALLY CLOSED row with a struck id reads as closed to `scripts/count_open_items.py`, so this file has reported **zero open rows while carrying live work** since 2026-09-04. `SUBA-081` stays struck and is NOT reopened.)*

**upstream** — `git -C tmp/pi-subagents show v0.57.0:src/agents/agents.ts`, `interface BuiltinAgentOverrideConfig` read in full and in order: `description`, `output`, **`outputMode?: OutputMode`**, `defaultReads`, `model`, `defaultProvider`, `fallbackModels`, **`fast?: boolean`**, `thinking`, `systemPromptMode`, `inheritProjectContext`, `inheritSkills`, `defaultContext`, **`acceptanceRole?: AcceptanceRole | false`**, `disabled`, `systemPrompt`, `skills`, `tools`, `extensions`, `subagentOnlyExtensions`, `completionGuard`, `toolBudget` — **22 fields**. `acceptanceRole` is already in the 17-field v0.43.0 shape (`v0.43.0:src/agents/agents.ts:82`), so it was portable at the measured baseline; `outputMode` and `fast` arrive in the v0.47.1..v0.57.0 window. `applyBuiltinOverride` applies each, with `false` meaning delete.

**cyrup** — `crates/cyrup-ext-subagents/src/discovery/types.rs`, `AgentOverrideConfig` enumerated field by field at `cc7818b` (**21**): `description`, `output`, `default_reads`, `model`, `fallback_models`, `default_provider`, `thinking`, `system_prompt_mode`, `inherit_project_context`, `inherit_skills`, `default_context`, `disabled`, `system_prompt`, `skills`, `tools`, `exclude_tools`, `allow_nested_subagents`, `extensions`, `subagent_only_extensions`, `completion_guard`, `tool_budget`. Against v0.57.0 the missing set is exactly **`output_mode`, `fast`, `acceptance_role`** — three, not the four `SUBA-081` recorded: **`default_provider` landed with `SUBA-088` (`ba24e5e5`)** and the residual double-counted a field another row had already closed. (`exclude_tools`/`allow_nested_subagents` are ahead of v0.57.0, from `SUBA-092`/`247ff97b`.) The drop is silent by construction: the struct is `#[serde(rename_all = "camelCase", default)]` (`types.rs:612-613`) with **no `deny_unknown_fields`**, so an unknown key deserializes to nothing and no diagnostic is raised — unlike a malformed agent FILE, which `SUBA-086` made reportable by name.

**Impact** — Three documented override keys are accepted into a settings file and have no effect, with no error. `acceptanceRole` is the sharpest of the three because `SUBA-082` made the role *the primary input* to `infer_level`: an operator who sets `agentOverrides.<name>.acceptanceRole` to re-classify a builtin gets the inferred level instead, and the acceptance gate then runs at a strictness nobody chose — a silent policy divergence rather than a visibly missing feature. `outputMode` and `fast` are the ordinary silent-no-op class. The severity that made `SUBA-081` `high` — the `tools: "inherit"` hard-failure — is fixed and does not carry here; `medium` reflects the `acceptanceRole` policy consequence alone.

**Fix** — Add the three to `AgentOverrideConfig` with the same `OverrideField` treatment their siblings already use (`acceptance_role` and `output_mode` take `false` as `ExplicitClear`; `fast` is a plain `bool` like `disabled`), and apply each in `discovery/merge.rs` beside the fields `SUBA-081` landed. All three target types already exist in the crate — `AcceptanceRole` from `SUBA-082`, `OutputMode` from `SUBA-N03` — so this is a struct + three apply arms, which is why the effort is **S** rather than `SUBA-081`'s M. Enumerate the struct when verifying, not `grep -c`: a substring grep is exactly what let the `default_provider` overlap survive a pass.

**Verify** — `agentOverrides: { reviewer: { acceptanceRole: "…", outputMode: "…", fast: true } }` in the subagents settings file must change the resolved `AgentDefinition` on all three axes; today it changes none. A field-count assertion against v0.57.0's 22 (less `fallbackModels`' cyrup-side rename, plus the two v0.62.0 fields) is the regression that keeps the next field from going missing.

**Out of scope for this row, recorded so it is not re-derived as new:** at **v0.68.0** the same interface is 26 fields — it gained `machine`, `inheritGlobalContext` and `mutationTools` and **dropped `fallbackModels`** (which cyrup still carries). Those four deltas fall in `v0.57.0..v0.68.0`, a window **no file in this directory owns**; `09b` is the structurally correct home, per the census in `09`. They are not folded into this row because doing so would misrepresent an unmeasured window as measured.
...
| Session lease / process-terminal record / owned-process-tree verification | ~~`VL-S3` / `VL-S4` / `SUBA-023`~~ — **ALL THREE CLOSED 2026-09-20** | Filed on both sides, and closed together: `background/session_lease/` + `background/process_terminal/` (15 modules, 5,912 LOC). This row's own additional finding — **the port's KILL path is stronger than upstream's** (process-group negation at `spawn/signal.rs:503-510` vs upstream's single-pid kills) — is what made the owned-process-tree half land ABOVE upstream rather than at it: pi writes `writers[i] = []` for every step because its children run in-process, while cyrup records one real `PiWriterProcessInstanceExit` per launched child carrying a `ProcessTreeTerminal` over that child's whole process group. The `process-tree-unverified` rung is therefore a live verdict here and an unreachable arm upstream. |
