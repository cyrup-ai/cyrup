...
| SUBA-096 | **medium** **OPEN — filed 2026-09-16** | S | discovery / settings overrides | `SUBA-081`'s residual as its own row: three `AgentOverrideConfig` fields (`acceptanceRole`, `outputMode`, `fast`) are still not on the struct, so each is accepted into `agentOverrides` and silently dropped |
...
## SUBA-096 — ~~medium~~ **OPEN — filed 2026-09-16** — Three `agentOverrides` fields are still not on `AgentOverrideConfig`, so each is accepted into a settings file and silently dropped
