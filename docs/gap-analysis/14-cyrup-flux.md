...
diagnostic in warn mode. `cargo check -p cyrup` was attempted and did not complete — the shared
build volume hit ENOSPC mid-build with nine tracks compiling; the change is API-additive (one `pub
const`, no signature change) and the crate's only consumer, `crates/cyrup/src/session_launch.rs`,
names no chord. **Residual — NOT re-filed here (medium; it is `EXT-039`, areas 06/07):** **Fix (1)**
