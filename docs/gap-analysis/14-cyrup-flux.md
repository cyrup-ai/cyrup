...
> **Re-measured 2026-09-24 at cyrup `ea23ca2` against `code_puppy_core_plugins` v0.0.62** (was
> v0.0.50; the clone's HEAD is `v0.0.62-1-g6d4e26a`). **Current pins: ported baseline v0.0.6
> (unchanged), measured-to v0.0.62.** The ported surface is STILL byte-identical: the git tree
> object of `code_puppy_core_plugins/flux_bootstrap` is `4ca4c1a8` and the blob of
> `tests/test_flux_bootstrap.py` is `10f0d458` at v0.0.6 **and at every one of the thirteen tags
> v0.0.50…v0.0.62** (checked tag by tag with `git rev-parse <tag>:<path>`, not by one diff), and
> `git diff --stat v0.0.62..HEAD` over the same two paths is empty. The whole-repo window
> `v0.0.50..v0.0.62` is 65 files, +2 499 / −452, 33 non-merge commits (herdr restore, a new
> `mcp_binding_prompt` plugin, subagent_panel, steer_queue, context_indicator, a `shell` tool rename
> inside `acp/bridge.py`) — none of it under `flux_bootstrap/`, so **no upstream-drift item is filed
> from it.** On the cyrup side `git diff --quiet 9aeba769 ea23ca2 -- crates/cyrup-flux spec/flux.md
> spec/flux docs/guide/extensions/flux.md` holds: nothing this area cites moved in the 46-commit
> window. Unmeasured: `code_puppy` itself (newest tag now **v0.0.867**, was v0.0.819 at the last
> look) past `v0.0.720` — see Blind spots 7.
...
> **RE-AUDITED 2026-09-24 at cyrup `ea23ca2`; no row moved.** `git log 9aeba769..ea23ca2 --
> crates/cyrup-flux` is empty and the crate, `spec/flux.md`, `spec/flux/` and
> `docs/guide/extensions/flux.md` are byte-identical across that window, so the six closures carry
> forward by identity. `FLUX-007`'s three limbs were nonetheless re-run as commands, not inherited:
> `grep -rn 'tmp/code-puppy' crates/cyrup-flux/` = 4 and `spec/flux.md` = 21 (unchanged);
> `grep -rnoE 'FLUX_[0-9]+' crates/cyrup-flux/src/` = 13 against a `spec/flux/` that holds only
> `README.md`; `grep -rn 'v0\.0\.' crates/cyrup-flux/src/ crates/cyrup-flux/Cargo.toml` = 9 and
> `lib.rs:1-16` still names no tag. **Still open, `low`.** `FLUX-001`'s unfiled medium residual (the
> sibling `CARGO_MANIFEST_DIR` resolvers) is also still live and was re-grepped:
> `crates/cyrup-ext-subagents/src/registration/resources.rs:46`,
> `crates/cyrup-ext-subagents/src/extension/executor/paths.rs:499` and
> `crates/cyrup-intercom/src/resources.rs:41` each still fall back to `env!("CARGO_MANIFEST_DIR")`.
> No `FLUX-` id is added: the upstream window `v0.0.50..v0.0.62` does not touch the ported surface
> (see the header).
...
| FLUX-007 | low | tooling | S | The crate still records no upstream baseline version in-source, and its four `tmp/code-puppy` doc-comment links still resolve nowhere — confirmed this pass to be wrong on two axes (directory name and a missing path segment) even now that both upstreams are cloned in-workspace — and 13 `FLUX_NN`/`§5.6` citations still point at spec files that do not exist — **RE-READ 2026-09-14 at `9aeba769` (verified re-audit): still open — two of the three compounding defects are untouched, but limb (1) is now WRONG and this row must be EDITED, not merely re-confirmed.** Limb (2), unchanged, and the two-axis wrongness re-proved by OBJECT TEST rather than by argument: `crates/cyrup-flux/src/state.rs:2`, `render_status.rs:2`, `render_cheatsheet.rs:2` and `render_about.rs:2` still cite `../../../tmp/code-puppy/flux_bootstrap/bundled/scripts/<f>.py`, and `git cat-file -e v0.0.6:code_puppy_core_plugins/flux_bootstrap/bundled/scripts/flux_status.py` SUCCEEDS while the same path WITHOUT the `code_puppy_core_plugins/` package segment is `fatal: ... does not exist in 'v0.0.6'` — the segment is required, and the hyphenated `code-puppy` directory does not exist. `spec/flux.md` still carries 21 `tmp/code-puppy` occurrences. Limb (3), unchanged, count re-taken exactly rather than trusted: **13** `FLUX_NN` occurrences across 11 lines, decomposing FLUX_01×2 / FLUX_06×1 / FLUX_07×3 / FLUX_08×1 / FLUX_09×4 / FLUX_10×2 — identical to the recorded census — while `git ls-tree --name-only 9aeba769 spec/flux/` is `README.md` ONLY, i.e. all twelve `FLUX_NN.md` files its task table links are absent; the §5.6 pair is intact (`state.rs:8` and `spec/flux.md:692`, moved from the cited `:672`, against a file whose only section-5 heading is `## 5. Gaps, risks, decisions` at `:877`). **LIMB (1) IS THE CORRECTION:** this row asserts that `grep -rn 'v0\.0\.' crates/cyrup-flux/src/ Cargo.toml` "still returns 0"; at `9aeba769` it returns **NINE** hits, every one an `@v0.0.40` anchor (`extension.rs:4,:56`, `overlay.rs:4,:38`, `resources.rs:78,:86`, `install.rs:2`, `bundle.rs:6`, `Cargo.toml:35`), with `install.rs:2` naming the fully correct `code_puppy_core_plugins/flux_bootstrap/installer.py`; the crate's tests additionally carry the resolvable `git -C tmp/code_puppy_core_plugins show` citation form (`tests/flux_003_renderers.rs:15`, `tests/flux_003_state_and_status.rs:15`). So this row's own title — "nothing in the crate records which upstream it was ported from" — is no longer LITERALLY true. It remains substantially true in the sense Fix (a) meant: `lib.rs:1-16` is still prose-only ("ported from code-puppy's `flux_bootstrap` plugin"), with no repository, no tag, and nothing anywhere naming `code_puppy @ v0.0.720`. The obvious objection to the new anchors — that they cite `v0.0.40` rather than the ported baseline `v0.0.6` — does NOT bite: `git diff --stat v0.0.6 v0.0.50 -- code_puppy_core_plugins/flux_bootstrap/ tests/test_flux_bootstrap.py` is EMPTY and `git ls-tree -r --name-only` gives 28 files at both tags, so `@v0.0.40` resolves to identical bytes. `Kind: tooling` stays (cyrup bookkeeping, not parity; nothing crossed the ported tag) and severity stays `low` (already the floor), though this row's claim that it "gates re-auditing every other row in this file" is now WEAKER than when written, since the crate's own tests already carry a resolvable citation form. **Rewrite Fix (a) before scheduling:** it should now ask only for the `lib.rs` `//!` anchor and a `code_puppy @ v0.0.720` citation, since the in-source `@v0.0.40` anchors it asked for already landed in nine places. Closes when this row's own Verify measures clean — `grep -rn 'tmp/code-puppy' crates/cyrup-flux/ spec/flux.md` returns 0 (today 4 + 21), `grep -rnE 'FLUX_[0-9]+' crates/cyrup-flux/src/` resolves to files that exist (today 13 citations, 0 targets), and `lib.rs` names a tag `git -C tmp/code_puppy_core_plugins rev-parse` accepts. **RE-CONFIRMED 2026-09-16 at `cc7818b` by identity, and the 2026-09-14 correction is upheld — do NOT re-litigate limb (1).** `crates/cyrup-flux` is byte-identical across `9aeba769..cc7818b`. A fresh reader running this row's TITLE as a grep will find `@v0.0.40` anchors and conclude the row is closed; it is not, and the row body above already explains precisely why (the anchors landed, `lib.rs:1-16` is still prose-only, and limbs (2) and (3) are untouched). **Limbs (2) and (3) re-measured this pass, not carried forward, because both are one command:** the four `tmp/code-puppy` doc links still resolve nowhere — `crates/cyrup-flux/src/{render_about,render_cheatsheet,render_status,state}.rs:2` each cite `../../../tmp/code-puppy/...`, while the clone in this workspace is `tmp/code_puppy` (underscore) and the required package segment `code_puppy_core_plugins/` is still absent from the path, i.e. the row's *two axes* of wrongness are both still live and now BOTH independently checkable against a real directory listing. **This is area 14's only open row** and it remains `tooling`/`low`. **RE-MEASURED 2026-09-24 at `ea23ca2`: still open, unchanged** — crate byte-identical to `9aeba769`; limb (2) = 4 + 21 `tmp/code-puppy` hits, limb (3) = 13 `FLUX_NN` citations against zero `spec/flux/FLUX_NN.md` targets, `lib.rs` still tagless. The `@v0.0.40` anchors still resolve to the same bytes at v0.0.62 (`flux_bootstrap` tree `4ca4c1a8` at both). |
...
8. **`flux_bootstrap/`'s zero-diff finding (`v0.0.6..v0.0.40`, extended 2026-09-24 to `v0.0.62` by tree-hash at every tag) covers only that directory and
