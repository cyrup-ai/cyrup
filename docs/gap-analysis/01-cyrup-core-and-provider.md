...
| PROV-069 | **critical** | port-bug | M | **A `length` stop kills the agent loop; it must not.** Reported from live use 2026-08-15 (owner report, `together`/`Kimi-K3`): a turn emits reasoning, stops at `LENGTH_STOP_NOTICE` (`crates/cyrup-tui/src/app.rs:6096`) and the run ends with no answer. **Owner statement of record: this is not how pi behaves.** **WHAT IS ALREADY VERIFIED — do not re-derive it, and do not "fix" the agent loop:** pi terminates the loop on `stopReason === "error" || "aborted"` ONLY (`packages/agent/src/agent-loop.ts:196-200` — note the path is `packages/agent/`, NOT `packages/coding-agent/`); `"length"` is deliberately absent, and its sole special handling is `failToolCallsFromTruncatedMessage` for tool calls carrying truncated arguments (`:207-216`), after which `turn_end` fires and `prepareNextTurn` runs. **cyrup matches this exactly** — `crates/cyrup-agent/src/agent.rs:593` gates on `StopReason::Error \| StopReason::Aborted`, and `:610` fails truncated tool calls with the upstream citation. **So the kill is NOT in the agent loop and the loop must not be edited.** Also ruled out: the length-stop overflow case (`crates/cyrup-provider/src/utils/overflow.rs:103`) requires `usage.output == 0` AND input ≥99% of the context window, which a 1M-window session will not meet. **The work is to FIND THE ACTUAL KILL SITE** — candidates not yet examined: the session-svc turn wrapper, the TUI's handling of the length notice, and whether `usage.output` is being populated at all when reasoning tokens arrive under `thinkingFormat: "together"`. Instrument per `handoff/03-verification.md` (log to file, run once, read) rather than re-running to characterise. **See `PROV-068`** — the same model is pinned to `high` reasoning because its lower rungs are nulled, which is what makes the budget exhaustible in the first place; the two are likely one user-visible failure. **Do not conclude from the `maxTokens: 131072` observed in this session** — it came from a hand-written `models.json` and was inferred, not measured. — **FILED 2026-08-15 (live use)** |
