...
**Unverified leads (no id):** (1) **the v0.86.0 `TranscriptContext` break** — `ProviderStreams` take a
...
| PROV-078 | medium | upstream-drift | S | **Unknown OpenAI-compatible endpoints are still detected as strict-capable, so every tool carries a `strict` key** — pi v0.87.0 (#9816) defaults runtime detection to `supportsStrictMode: false` and moves the built-in "capable" set into generated metadata. **FILED 2026-09-24**; body below. |
