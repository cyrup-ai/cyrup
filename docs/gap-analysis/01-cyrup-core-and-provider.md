...
| PROV-068 | **high** | port-bug | S | **An explicit `null` in `thinkingLevelMap` is read as UNSUPPORTED, collapsing most reasoning models to two rungs.** Reported from live use 2026-08-15: on a `together` Kimi model, Shift+Tab offers only `off` and `high`. Deterministic, and the mechanism is exact — `get_supported_thinking_levels` (`crates/cyrup-provider/src/collection.rs:794-807`) matches `Some(None) => false, // explicit null -> unsupported`, `Some(Some(_)) => true`, absent => true except `xhigh`/`max`. Together's Kimi rows declare `{minimal: null, low: null, medium: null}`, so those three are filtered out; `off` and `high` survive only because they are ABSENT from the map. `deepseek-ai/DeepSeek-V4-Pro` collapses the same way (only `high` maps to a value). **ESTABLISH PI'S SEMANTICS FIRST — do not fix from this row's hypothesis.** A `null` upstream most likely means *supported, with no provider-specific value to send*, in which case the predicate is inverted and every catalog row that nulls a rung is affected; but the opposite reading would make the current code correct and the CATALOG DATA wrong instead. The two fixes are in different files and only one is right. Note the maps are now regenerated from the pinned `b0c2a90e` (PROV-018/059), so if the data is at fault the fix belongs in the generator, not by hand. **User-visible effect: a paid reasoning model cannot be tuned below `high`, and the levels the docs advertise as available silently are not.** — **FILED 2026-08-15 (live use)** |
| PROV-069 | **critical** | port-bug | S | **The model's `maxTokens` is never sent to the provider, so every response is truncated at the provider's own default output cap.** Owner report 2026-08-15: truncation on EVERY pass, mid-sentence, on short replies, and it does not happen in pi. **ROOT CAUSE, verified:** `GenConfig::max_tokens` is `Option<u64>` (`crates/cyrup-agent/src/proxy.rs:315`), the request body writes the field ONLY when it is `Some` (`crates/cyrup-provider/src/api/openai_completions.rs:373-376`), and **`grep -rn '\.max_tokens(' crates/ | grep -v tests` returns ZERO production callers** — the builder at `crates/cyrup-agent/src/agent.rs:2294` is used only by tests. So `opts.max_tokens` is always `None`, no `max_tokens` key is emitted, and the provider falls back to its own default (small on Together's OpenAI-compatible endpoint). **The catalog's `max_tokens` — ported for all 1087 rows, regenerated from `b0c2a90e`, and pinned by tests — is decorative: it reaches no request.** The wire layer is CORRECT and must not be touched; the missing wiring is between the resolved model and `GenConfig`. **Fix: populate `GenConfig::max_tokens` from the active model's `max_tokens` at session/turn construction, and confirm pi's own value and precedence first** (a `maxTokens` SETTING or `modelOverrides` entry must still beat the catalog). Add a wire-level test asserting the body carries the active model's value — `openai_completions.rs:2697` already proves the field serialises when `Some`, which is why unit coverage never caught this: **every existing test constructs `max_tokens: Some(...)` by hand, so the one thing never exercised is the production path where nothing sets it.** **NOT verified this pass and required before closing:** pi's corresponding send site (`packages/ai/src/apis/openai-completions.ts` did not yield to a `maxTokens` grep — locate it properly). **Supersedes this row's earlier framings:** the agent loop is a faithful port (`agent.rs:593` gates on `Error \| Aborted` exactly as `agent-loop.ts:196-200`) and must not be edited; `overflow.rs:103` is ruled out. **`PROV-068` is a separate defect** and no longer believed to be the cause here. — **FILED 2026-08-15 (live use)** |
...
**Impact** — A benign structural original. Filed for one reason only: so the **35-vs-35 catalog count is not mistaken for a clean one-to-one mapping.** The true mapping is 34 shared names, plus `together` (inline Rust in cyrup, `together.models.ts` in pi, 20/20 exact), plus `openrouter-images`, against pi's 37 `*.models.ts` files. A future sweep that compares directory counts and stops will conclude the catalog set is complete; it is not (`PROV-014` names three providers with no catalog at all).
