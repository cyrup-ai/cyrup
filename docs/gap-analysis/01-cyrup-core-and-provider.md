...
| PROV-069 | **critical** | port-bug | M | **A reasoning turn spends its whole output budget on reasoning and returns no answer — the turn ends on `stop_reason: length` with the visible reply never emitted.** Reported from live use 2026-08-15 on `together` / `moonshotai/Kimi-K3`: the transcript shows the model's reasoning text and then stops at `LENGTH_STOP_NOTICE` (`crates/cyrup-tui/src/app.rs:6096`). **cyrup's NOTICE is correct and is not the defect** — it faithfully reports the provider's `length` stop; the defect is that a turn can be spent entirely on reasoning and deliver nothing, which makes the product unusable on a reasoning model. **Three candidate contributors, all of which must be checked before any is fixed:** (1) **`PROV-068`** — the thinking level is pinned to `high` on this model because every lower rung is nulled in the catalog and read as unsupported, so the user cannot reduce reasoning depth to leave room for an answer; these two may close together. (2) **`max_tokens` provenance** — the `Kimi-K3` entry used in this session came from a hand-written `~/.cyrup/agent/models.json` whose `maxTokens: 131072` was **inferred, not measured** (Together's `/v1/models` exposes no `max_output_tokens`), so the budget cyrup sends may not match the provider's real cap. Establish the true value before concluding anything from an observed truncation. (3) **Reasoning-vs-visible budget accounting** — with `thinkingFormat: "together"` the reasoning block bills against the same output allowance as the answer. **Establish what pi does when a reasoning response stops on `length`** — whether it retries, reserves budget for the visible turn, caps reasoning separately, or simply surfaces it as cyrup does — and port THAT. If pi also just surfaces it, this is not a parity bug and the item must be reclassified as `cyrup-original`/UX rather than fixed by invention. — **FILED 2026-08-15 (live use)** |
