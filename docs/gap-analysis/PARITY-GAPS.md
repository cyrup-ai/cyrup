...
|---|---|---|---|---:|---|
| 1 | **crit** | `AGENT-020` (02) | PB-25 | S | `continue_run` drains the steering queue **before** the run-active check, so `Err(RunActive)` destroys a user-typed message with no error |
...
**PB-25 · `continue_run` drains the steering queue before claiming the run latch, destroying a user-typed message** — ***critical*** **(raised from high this pass)** · area 02 `AGENT-020`
...
6. **The first-run wizard (UW-2).** The standing trap list says it is "deliberately unreachable"; the code says `is_official_distribution()` is **true** for this build and the gate can fire into an empty `if` body. One of the two is wrong. Decide whether the wizard ships (wire `startup.rs:256`) or does not (delete the predicate and the dead function, and correct the trap list) — it cannot stay in this state, because the current shape is the worst of both.
