...
|---|---|---|---|
| `SEAM-112` | 08 | **crit** | `/resume` produces a broken session — nothing renders and bash tool calls repeat endlessly. **The repetition is the load-bearing clue**: a pure render fault runs each command once, so tool results are not reaching the model and this is at minimum a context/session-rebuild fault. |
| `PERM-034` | 10 | **crit** | *(renumbered from `PERM-033` on 2026-08-19 — id collision with area 10's closed forwarding-audit row, which claimed it first.)* "Allow Always" does not stick; the same tool/command is re-prompted inside one session, which makes the permission gate unusable. **Not the dead-seam class** — the write is `cyrup-permission-system/src/extension.rs:1717`/`:2023` (`approve_always`) and the read is `:1464`/`:1634`/`:2211` (`get_rules`), both wired at HEAD. |
| `TUI-092` | 07 | ~~crit~~ **high** | Progressive lockup of the TUI. **De-escalated `critical` → `high` in this batch**, because all three clauses that justified `critical` are false at HEAD: `Ctrl+D` IS bound (`keymap.rs:655` → `app/input.rs:126-129` → `app/run_action.rs:16` returns `RunFlow::Break`), `Ctrl+C` IS bound (`keymap.rs:656` → `app/input.rs:219-231`), and `TUI-088` is CLOSED. |
...
| `TUI-091` | 07 | high | Reasoning blocks never render although every layer is wired — and the row now has **zero live hypotheses**; its last named candidate was refuted 2026-08-19 in the area file. It needs a live re-observation, not more tracing. |
