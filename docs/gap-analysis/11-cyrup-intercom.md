...
| ICOM-042 | medium — **PARTIALLY CLOSED 2026-08-14** | upstream-drift | L | cwd-scoped `send`/`ask` and `openProjectPaneIfMissing` unported — **PARTIALLY CLOSED 2026-08-14** (sweep 2: the cwd-targeting half, `project_target.rs`); **RESIDUAL RE-MEASURED 2026-08-15 (sweep 9), still open and still a DECISION, not work.** The residual is the Herdr half only: `project-agent.ts` is 324 lines, of which ~230 are `HerdrClient` (`:60-225`, spawning `process.env.HERDR_BIN ?? "herdr"` and mapping a six-variant error union whose text names "Herdr 0.7.5+"), `openProjectPane` (`:227-254`) and `waitForProjectSession` (`:255-290`), plus the pane branches inside `index.ts`'s send/ask cases and the three schema params. **cyrup ships no Herdr integration to align with — `grep -rn herdr crates/` finds only `cyrup-ext-subagents`' documented no-inspector divergence (`tui/fleet.rs:58`) — so this is a port of a launcher for a third-party binary cyrup neither ships nor requires. NEEDS one product yes/no.** Note ICOM-004 no longer waits on that answer: the shipped SKILL.md adapts the two Herdr passages under a `[CYRUP-DELTA]` naming the exact upstream lines to restore. |
...
| ICOM-016 | low | not-ported | L | Silent namespaced extension bus: effects absent — **2026-08-15, still open; NOT ATTEMPTED by sweep 9, deliberately and reported rather than omitted**, for the reason sweep 6 gave and one more. Re-measured upstream at v0.10.1: `broker/extension-state.ts` is **197** lines (not 186) and is not a plain map — it is a sha256-checksummed, 64 KiB-capped, optimistically-revisioned store with a `copyFileSync`/`renameSync`/`fsyncSync` atomic-write dance and per-namespace sha256-named files; on top of that sit `handleExtensionPublish` (`broker.ts:1163-1256`), `handleExtensionStateCommit` (`:1258-1356`), `recomputeNamespaceOwners` (`:1080-1155`), the `session.extensions` recording at register (`:329-337`, `:396-410`) and in `extension_capabilities_update` (`:435-470`), `features: [EXTENSION_BUS_FEATURE]` on `Registered` (`:505`), and the client half (`localExtensions` + `emitLocalExtensionEvent`). **~450 upstream lines, and it introduces the crate's first hashing dependency** (`createHash("sha256")` is load-bearing twice: the payload checksum and the state file's name). The current state remains coherent, not broken — the broker answers every bus frame with pi's own not-advertised refusal, so nothing hangs. **NEEDS: a decision on the sha2 dependency, then one agent with the budget to finish it in one pass** — half of this is worse than none, because a broker that advertises `extension-bus-v1` while owner election is missing hangs the clients that believe it. |
