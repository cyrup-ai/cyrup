...
| ICOM-024 | medium | not-ported | M | No `intercom_message` renderer; card frozen at width 80 — **2026-08-14, still open**: sweeps 2 and 6 — blocked behind `ICOM-029` exactly as the item says, and the in-source note at `ui/mod.rs:19` states it: `HostServices::inject_message` carries no `details`, so a live renderer would receive a bare markdown string and hit upstream's `return undefined`. **NEEDS one agent owning `crates/cyrup-ext` (the `HostServices` trait) + `crates/cyrup-session-svc` (its implementation); only then can the live-width card be drawn.** |
| ICOM-029 | medium | not-ported | M | `inject_message` carries no `details`, so a renderer would have nothing to render — **2026-08-14, still open**: sweeps 2 and 6 — blocked OUTSIDE this crate. **FIX SITE: the `HostServices` trait in `crates/cyrup-ext` + its implementation in `crates/cyrup-session-svc`, widened together in one commit. It gates `ICOM-024`.** |
...
| ICOM-015 | low | not-ported | M | Broker listen half of the named-pipe / TCP transport absent — **2026-08-14, still open; REAL SEVERITY LOWERED 2026-08-14 (sweep 6)**: the broker entrypoint hard-codes `UnixListener::bind(&socket_path)` (`broker/mod.rs:1877`) and states at `:1896-1897` that the whole entrypoint is unix-only; `broker_listen_target()` (`transport/target.rs:278`) exists but has no caller on the listen side. **BUT on unix `should_use_windows_tcp_transport` is FALSE unconditionally, so `broker_listen_target` always returns `Socket(broker_socket_path_for(unix, agent_dir))` — the SAME path the hard-coded bind uses. There is NO unix-observable defect at all;** the entire gap is the Windows named-pipe and TCP listen arms plus writing `broker.port.json`. **UNBLOCK CONDITION, concrete: a Windows target available to `cargo check` (`cargo check -p cyrup-intercom --target x86_64-pc-windows-msvc`).** Under the two permitted build commands on a macOS host, `#[cfg(windows)]` code is written blind and never type-checked, and landing only the TCP arm leaves one of three arms wired — the half-landing shape this area file already names. |
| ICOM-016 | low | not-ported | L | Silent namespaced extension bus: effects absent — **2026-08-14, still open — genuinely L, deliberately NOT STARTED by sweep 6, which chose one item finished (`ICOM-017`) over two started.** The current state is **coherent, not broken**: the broker ports pi's VALIDATION prefixes and answers every bus frame with pi's own not-advertised refusal (`broker/mod.rs:803-828`, `:887-908`), deliberately, because `session.extensions` is never recorded — so nothing hangs. **Scope, so the next agent does not under-estimate it:** the effects half needs `broker/extension-state.ts` (186 lines) read in full plus `handleExtensionPublish` (`:1262-1356`) and the `recomputeNamespaceOwners` fan-out; the client half needs `localExtensions` + `emitLocalExtensionEvent`. |
...
| ICOM-053 | medium | test-gap | M | The 16-second workspace gate builds and runs NONE of the broker-socket seam tests, because `cyrup-it` is `required-features = ["it"]` — **FILED 2026-08-14 (sweep 6), open.** Re-filed out of `ICOM-026`, whose string-assertion half closed as refuted: this half is a statement about the **gate**, not about those tests, and must not disappear inside a closed test-defect row. Every broker-socket seam test lives in `crates/cyrup-it`, which the gate does not build, so a change that reds them lands silently and is found only by whoever next runs the gated suite. **The same structure blocks `EXT-025` (area 06) and hides the un-instantiated 0.7 `HOST_WORLD` guest fixture.** Decide whether the gated suite runs on a schedule, in CI, or as a required pre-merge step — this is a process decision, not a port gap. |
