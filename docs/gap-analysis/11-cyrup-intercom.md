...
| ICOM-060 | medium | not-ported | M | **New 2026-09-04.** No guard against a misdirected reply during an ask-triggered turn — upstream `5fe0ee3` (v0.12.1, #119, "fix: guard active intercom replies") adds `ReplyTracker.findActiveReplyTargetMismatch(to)` (`reply-tracker.ts:124-130`): when the current turn's context expects a reply and a `send`'s resolved target differs from that asker's id, the tool refuses with `` `This turn is responding to an intercom ask from "${senderLabel}". Use intercom({ action: "reply", ... }) or set replyTo: "${id}". Refusing non-reply send to "${targetDisplay}" ...` `` (v0.13.0 `index.ts:2350-2360`) instead of sending. cyrup's `ReplyTracker` already carries the needed state (`current_turn_context`, `reply_tracker.rs:96`), but `grep -rn 'active_reply_target_mismatch\|findActiveReplyTargetMismatch' crates/cyrup-intercom/src/` returns nothing, and `tools/intercom/send.rs::action_send` goes straight from the self-target guard (`:120-121`) to the ICOM-037 inferred-reply lookup (`:132-140`) with no mismatch check in between. **Impact:** with ICOM-042 now landed, a turn triggered by peer A's ask can `send` to a *different* target resolved by `cwd` or a roster guess, and cyrup delivers it as an ordinary send rather than refusing — exactly the misdirection upstream's own fix note names ("CWD hierarchy or roster guesses ... misdirecting replies"). **cyrup and upstream agreed at v0.10.1** — this is drift past that tag, not a porting miss at the crate's own baseline. **Fix** — add `find_active_reply_target_mismatch(&self, to: &str, now: u64) -> Option<IntercomContext>` to `ReplyTracker` beside `find_unique_pending_ask_from` (`reply_tracker.rs:220`), consult it in `action_send` before the inferred-reply lookup when `params.reply_to.is_none()`, and port the refusal string. **Verify** — peer A asks B; while B's turn is live, B calls `send` to a *different* target C with no `replyTo`; assert the send is refused naming A, not delivered to C. |
