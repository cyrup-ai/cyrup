...
> |---|---|---|---|
> | `cyrup` | **`ea23ca2`** (2026-09-24) | `9aeba769..ea23ca2` — 46 commits; the code landed in `cyrup-ext-subagents`, the new `cyrup-herdr`, `cyrup-it`, `cyrup-tui` (copy/terminal-input/editor wrap), `cyrup-ext` (`contract.rs`, `native.rs`, terminal keys), `cyrup-session-svc` (`host_services.rs`, `session/inject.rs`) and `crates/cyrup` (subagent inspector/runner). `crates/cyrup-{provider,core,config,agent,session,modes,resources,tools}` and `xtask` are byte-identical to `9aeba769`. Every open row's cyrup locus was re-checked against that diff | — |
> | `pi` | **`v0.87.1`** (2026-09-22) | `v0.85.1..v0.87.1` — **667 files, +79 789 / −22 639**, 201 non-merge commits: `packages/ai` 127 files +4 910/−2 058 (area 01 holds it); `packages/agent` 125 files +36 468/−1 316 (`src/` 53 files +9 910, dominated by a new `harness/pico3/` runtime); `packages/coding-agent` 242 files +18 411/−15 390; `packages/tui` 48 files +2 665/−397; `packages/chord` 26 files +4 228/−529; **a new package `packages/durable`** (27 files +8 533, "Durable conversation, task, and document runtime for Pi"); `packages/evals` 42 files. Read for this area: `packages/coding-agent/CHANGELOG.md` v0.86.0–v0.87.1 in full; `extensions/runner.ts` `emitUserBash` + its two callers; `utils/retry.ts` + `agent-session.ts`'s retry delay + `settings-manager.ts`'s retry settings; `session-export.ts`/`session-share.ts` (for `DRIFT-055`); `packages/telemetry` (for `DRIFT-047`) | everything else in `v0.85.1..v0.87.1` outside `packages/ai` — see `### 2026-09-24 census note`. `v0.84.1..v0.85.1`: the 2026-09-14 census below is still UNVERIFIED in full; nothing in it was promoted by this pass *(superseded by the SECOND PASS paragraph below: its `packages/ai`-side leads are now dispositioned; the harness, coding-agent and tui leads remain with their owners)* |
>
> **Every open row was re-read at these pins; none closed** (stamps in the rows). **Two items filed: `DRIFT-056` (medium) and `DRIFT-057` (low)**, bodies before `## Coverage`. Provider-package drift in the same window is filed in area 01 as `PROV-073` … `PROV-082`.
>
> **SECOND PASS, 2026-09-24 (same pins).** Read for this area: `packages/chord` as a census, not
> line by line: its `README.md` at v0.87.1, the `delta/README.md` and `services/state.ts` diffs in full, the
> rest of the window by diffstat and commit, and its importers at v0.87.1 (`delta/index.ts`'s 1 432 lines of
> churn and `state/{draft,diff,value}.ts` were NOT read line by line — deliberately, because the importer
> census below makes chord a non-target), `packages/{protocol,server,client,
> session-backends,telemetry}` (version bumps only in the window), `packages/evals` (the diffstat and
> `package.json`: private dev tooling), and the 16 non-merge commits `v0.87.1..b45597504` (post-tag leads, in
> the census note). The `packages/ai`-side leads of the 2026-09-14 census are dispositioned (block at the top
> of that section): **two items filed, `DRIFT-058` and `DRIFT-059`**; `DRIFT-057` amended with a third literal;
> `DRIFT-009`'s false sha citation corrected; `DRIFT-027` annotated. Area 01 filed `PROV-083` … `PROV-100` in
> the same pass. **Still unread, by scope rather than by omission:** `packages/agent/src/harness/**` and
> `packages/durable` (another pass owns them), and `packages/coding-agent`/`packages/tui` beyond what the first
> pass read — their 2026-09-14 census bullets outside `packages/ai` remain leads for their owners.

## UNVERIFIED — 2026-09-14 census of the `v0.84.1..v0.85.1` window (leads, not items)

> **2026-09-24, second pass — dispositions for this section's `packages/ai`-side leads** (both sides re-read,
> cyrup `ea23ca2`, pi `v0.87.1`): `ToolCall.namespace` → **`DRIFT-058`**; `AssistantMessage.endTurn` →
> **`DRIFT-059`**; `AssistantMessageFrameEncoder` — **struck as a port target on its own**: at v0.87.1 its
> only references outside `packages/ai` are `packages/agent/docs/**` (no `src/` consumer), so it is SDK surface
> for the harness/durable work another pass owns; `uuidv7` — **struck**: cyrup mints ids with the `uuid`
> crate's `Uuid::now_v7()` (`crates/cyrup-session/src/ids.rs`), monotonic within a millisecond, and pi's
> follower-timestamp form is used only by `packages/agent/src/harness/**`; `ModelRuntime.streamDeferred` →
> growth on area 01's `PROV-040`; `baseten` / `DRIFT-009` sha problem — **resolved**: the registration landed
> in `9b49ad8` ("fix(parity): batch 4 …", `git log -S'"baseten"' -- crates/cyrup-provider/src/providers/fleet.rs`);
> `e4078e3a`/`ccd14981` do not exist, so `DRIFT-009`'s citation is wrong and the row is annotated, and the
> `00-residual-ledger.md` sentence calling baseten unregistered is stale (that file is not this pass's to edit).
> **Not dispositioned here, by scope:** the "harness restructuring" bullets and "Harness telemetry schema
> reshaped" are `packages/agent/src/harness/**`, owned by the harness/durable pass.
...
- **`uuidv7(timestampMs?)` rewritten: follower timestamps, 41-bit monotonic sequence, `crypto`
  ...
  a different name, so the cyrup side is UNVERIFIED.** *(Resolved 2026-09-24: struck — see the disposition block at the top of this section.)* Upstream consumers include the Codex adapter,
...
### 2026-09-24 census note — `v0.85.1..v0.87.1`, outside `packages/ai` (leads, not items)

Read from `packages/coding-agent/CHANGELOG.md` and the diffstat unless stated; **nothing here is an
item**. Filed after a two-sided read: `DRIFT-056` (`user_bash` fail-closed) and `DRIFT-057` (retry
literals + delay cap). **Cleared:** `--mode` missing/invalid now errors (v0.87.1, #9045) — cyrup's
`crates/cyrup/src/cli/args.rs` declares `#[arg(long = "mode", value_enum)]`, so clap already rejects
both (exit code 2 vs pi's 1 was not checked). `DRIFT-047`'s falsifier re-run: `git grep startAiSpan
v0.87.1 -- packages/` still hits only `agent/src/harness/telemetry.ts:138` (definition) and
`agent/src/index.ts:103` (re-export). **Leads, strategic (no id, sizing only):** (1) v0.86.0
transcript-backed mid-conversation system messages (`TranscriptContext`, `SystemMessage.toolsAdded` /
`toolsRemoved` / `sections`) and v0.87.0's canonical `SessionManager` context with append-only
`ContextEditEntry` (`context_edit`, a new `SessionEntry` variant) — a session-format change for area
03 and the provider seam for area 01; (2) `packages/agent/src/harness/pico3/` (~7 000 new lines:
...
census, now configurable); (7) split-turn compaction prompt wording (#9908); `GIF`-prefixed text files
misread as images (#9755); malformed prompt-template frontmatter now a resource warning (#9830);
OSC 52 clipboard fallback and WSL backend (#9688); `--resume`/`--continue` startup ordering.


> **2026-09-24, second pass — package census (every pi package other than `ai`, `agent/src/harness/**`,
> `durable`, `coding-agent`, `tui`), read at v0.87.1:**
>
> | package | window `v0.85.1..v0.87.1` | cyrup counterpart | decision |
> |---|---|---|---|
> | `packages/chord` (7 647 `src` lines) | 26 files, +4 228/−529: delta tracker replaced by an operation log (`2c995acf4`, `c4289b20e`), transactional replicated state — `change(context, mutate)` over copy-on-write drafts with revision diffing (`10d1ad621`: `state/{draft,diff,value}.ts`), `publish()` removed | **none** | Not a port target. Its non-test importers at v0.87.1 are `agent/src/harness/**` (incl. `pico3/`), `durable`, `protocol`/`server`/`client` and `coding-agent/src/experimental/**`; **no production `coding-agent/src` file outside `experimental/` imports it**. cyrup's composition runtime is the WIT/wasm extension host (area 06), a different design. Re-open only if cyrup adopts pico3/durable (harness owner) or pi's remote-session protocol |
> | `packages/protocol` · `server` · `client` | version bumps only (`CHANGELOG.md`, `package.json`) | none (cyrup's remote surfaces are RPC mode and `cyrup-acp`) | experimental CBOR remote-session stack; unchanged in the window; scoping decision unchanged from the 2026-09-14 census |
> | `packages/session-backends` (`sqlite-node`) | version bumps only | none | harness storage backend — harness owner |
> | `packages/telemetry` | version bumps only | none (`DRIFT-047`) | `DRIFT-047`'s falsifier re-run by the first pass; nothing new |
> | `packages/evals` | 42 files, +3 525/−1 974: vitest-evals harness replaced by a Docker-run doc-eval harness (`src/{harness,report,docker,cli,plan}.ts`, `evals/*.docs.eval.ts`) | none needed (`cyrup-it` is cyrup's test harness) | `"private": true` dev tooling that grades pi's own docs with LLM judges; no runtime behaviour to port |
>
> **Post-tag leads (pi v0.87.1..b45597504) — not items.** Non-durable, non-`packages/ai` commits (the ai ones
> are in area 01): **`9a139c62b`** consolidates chord delta tracking into `delta/{diff,draft,tracker}.ts`
> (moving `state/draft.ts`) and **`4bc1a2fe5`** adds two prototype delta engines (`delta/astra`, `delta/cow`,
> +7 322) — chord-internal, no decision change. **`8d897edaa`** (#9863) stops managed git packages installing
> pi peer dependencies and warns when an extension package lists host-provided modules in `dependencies` —
> area 05's package-manager surface. **`36af9dc48`** (#9944) restores `/skill` prefix autocomplete when skill
> names do not contain the letters of `skill` — area 07. **`a8ed49771`** changelog scaffolding. The durable and
> Pico5-doc commits (`b45597504`, `b313731b8`, `898ab8040`, `967246214`, `a32782520`) belong to the
> harness/durable pass. None was read on the cyrup side.
...
> ### RE-AUDIT 2026-09-16 — cyrup `cc7818b`, pi **v0.85.1** — TWO ROWS CLOSED, THREE REFRESHED
>
> **This area's whole premise is a comparison against a pi version, so the version was re-measured
> before any row was believed.** `git -C tmp/pi describe --tags` → **`v0.85.1-84-g509ee2bd0`**;
> `git -C tmp/pi tag --sort=-v:refname | head -1` → **`v0.85.1`**. **The latest tag has NOT moved**
> (the README records the same tag with the clone at `-67-g71dca871b`; only the clone's HEAD advanced,
> and HEAD is past the latest tag and is not citable). **So the recorded window is not stale in the
> direction that would misclassify rows:** nothing filed here as `upstream-drift` has silently become
> in-baseline, and no new pi release has opened an unowned window since the last re-measure. The two
> closures below are cyrup-side, not window-side.
>
> **Scope.** The five open rows and the three trackers, and nothing else. No closed row was re-opened,
> no id was renumbered, no duplicate marker changed.
>
> | row | verdict at `cc7818b` | evidence |
> |---|---|---|
> | `DRIFT-009` | **still open**, medium | catalog dir still 35 entries; the four blocked modules still `data/*.json` re-exports at v0.85.1 |
> | `DRIFT-015` | **CLOSED** | the third half (`outputPad`) landed at `1739fcc`; world `0.8 → 0.9` was that bump |
> | `DRIFT-019` | **CLOSED** | all four providers registered and reachable — `da970e8` (radius + the three Qwen plans), `9b49ad8` (baseten) |
> | `DRIFT-047` | **still open**, low | its own grep still returns zero; upstream's `startAiSpan` still has no production caller at v0.85.1 |
> | `DRIFT-055` | **still open**, low | `/share` still posts the whole-tree export; upstream still linearises, re-read at v0.85.1 |
>
> **`DRIFT-015` and `DRIFT-019` are closed on code that is present, reachable from production AND
> pinned by a test** — this directory's own three-part bar — and each closure names what it does NOT
> cover. `DRIFT-015`'s three halves are all in the WIT world and wired end to end; `DRIFT-019`'s four
> providers are constructed in `providers/all.rs`'s builtin list, and the catalog-floor shortfall that
> row deferred stays with **`DRIFT-009`, which remains open**. Nothing was rounded up: the qwen trio
> ship no embedded rows, and that is exactly the residual `DRIFT-009` owns.
>
> **The three trackers were re-probed, not re-argued, and all three stay trackers** — see their rows.
> No tracker was escalated into the work set, because escalating one requires the two-sided read each
> of them says it has never had, and this pass did not do those reads.
>
> **A hazard this file already records is confirmed again, and it is worth stating at the top:
> commit shas cited in ledger prose are frequently unresolvable in this repository.** `DRIFT-009`
> found `e4078e3a` and `ccd14981` missing; re-checked this pass, `019e4ad6` (cited by `DRIFT-015`),
> `380c713`, `a9000b1`, `04c1ba2` and `9962e0f` (area 11's re-baseline) are all missing too, while
> `9e2bfda5`, `2571969` and `b28d3ff` exist. **Every closure below therefore cites a sha that was
> verified present with `git cat-file -e` and confirmed an ancestor of `cc7818b`**, and says so.
...
| ~~DRIFT-015~~ | ~~medium~~ **CLOSED 2026-09-16** | **not-ported** *(was upstream-drift)* | L | duplicate-of: `EXT-019` | Extension context surface: ~~`scopedModels`~~, ~~`outputPad`~~, ~~markdown transformers~~ ~~still absent~~ — **all three now present** — **2026-08-15, TWO OF THREE HALVES CLOSED; the third is BLOCKED (measured)**: area-12 pass. `scoped-models: func() -> string` is declared at `crates/cyrup-ext/wit/world.wit:732` (EXT-045) and `register-markdown-transformer: func()` at `:476` with its `transform-markdown` export — both landed after this row was written, and the world is now `@0.8` (`HOST_WORLD` in `cyrup-ext/src/manifest.rs:219`), not the `@0.4` this row's Fix names. **KIND CORRECTED:** `MessageRenderOptions { expanded; outputPad }` is declared at **v0.83.0** `core/extensions/types.ts`, so the remaining half is `not-ported`, not `upstream-drift` — the eleventh row in this file to take that correction. **FIX CORRECTED, and this is why it is blocked rather than small:** the Fix asks for "a `render-options` record carrying `output-pad` and **width**", but upstream has no width field and has **three** option records, not one — `MessageRenderOptions { expanded, outputPad }` (custom messages, `custom-message.ts:73`), `EntryRenderOptions { expanded }` (custom entries, `custom-entry.ts:41`) and `ToolRenderResultOptions { expanded, isPartial }` (tool results, `types.ts:411-416`, passed at `:494`). cyrup collapses all three onto ONE `render-call`/`render-result` pair (`world.wit:259`/`:260`), so the work is a decision about that collapse plus a `HOST_WORLD` minor bump and a synchronized edit across `cyrup-ext/wit`, `cyrup-ext-sdk/wit`, `host/live.rs::render`, `facade.rs`, `cyrup-ext-sdk/src/{api,guest}.rs`, the four `MessageRenderer` impls in `example.rs`, `cyrup-it/tests/ext/ergonomic.rs:132` and the `cyrup-tui` caller. `wit_world_sync.rs` fails if only one WIT copy moves. Schedule with `DRIFT-004` — same world, same bump, one agent. — **CLOSED 2026-09-16 at `cc7818b` (landed at `1739fcc`, `feat(ext): EXT-006 give renderers the display options + theme, and re-invoke them when either moves`; sha verified present with `git cat-file -e` and an ancestor of `cc7818b`).** The third and last half — `MessageRenderOptions.outputPad` — is in, and it is in as a WIRED surface rather than as a declaration. **(1) Present.** `events.render-call` and `events.render-result` each take a third parameter, `opts-json` (`crates/cyrup-ext/wit/world.wit:330-331`), and the bag is documented at `:305-321` as the UNION of upstream's three option records — `{"expanded": bool, "outputPad": number, "isPartial": bool, "theme": string\|null}` — citing `MessageRenderOptions.expanded/outputPad` (`extensions/types.ts:1196`, `:1198`), `EntryRenderOptions.expanded` (`:1210`) and `ToolRenderResultOptions.{expanded,isPartial}` (`:415`, `:417`). The Rust carrier is `RenderOptions { expanded, output_pad: u32, is_partial, theme }` (`crates/cyrup-ext/src/render.rs:45-68`, `to_json` at `:85`), and the guest side parses the same key (`crates/cyrup-ext-sdk/src/api.rs:400-428`, reading `"outputPad"`). **(2) Reachable from production.** `crates/cyrup-tui/src/app/events.rs:101-107` builds it live off the view — `self.state.transcript.output_pad()` (`transcript/view.rs:22`, settable mid-session at `:31` for pi's `onOutputPadChange`) — and `App::refresh_extension_renders` re-invokes the guest whenever any input moves, which is what makes it a LIVE input rather than an ingest-time constant. **(3) Pinned by a test.** `crates/cyrup-tui/src/tests/extension_renderers.rs:1257-1292` `staleness_is_derived_from_the_options_the_text_was_produced_under` asserts that moving **outputPad alone** (`RenderOptions::new(false, 2, …)` against a render produced under `(false, 1, …)`) makes the row stale and that the re-invocation carries the live options; `crates/cyrup-it/tests/ext/wasm_renderer_routing.rs` drives the same pair through a real guest. **THE ROW'S OWN "FIX CORRECTED" PARAGRAPH IS WHAT SHIPPED, INCLUDING THE PART IT CALLED BLOCKING.** It said the work was "a decision about that collapse plus a `HOST_WORLD` minor bump and a synchronized edit" across both WIT copies and the SDK: the decision was taken (one export pair carrying the union bag, with the reason written out at `world.wit:308-311` and the theme-as-name CYRUP-DELTA at `:323-329`), and the bump is recorded in the world history at `world.wit:52-58` — **"0.8 -> 0.9 is the renderer display-options batch, and it is an EXPORT re-signing"** — with `HOST_WORLD` now `cyrup:ext@0.10` (`crates/cyrup-ext/src/manifest.rs:253`) after the later `DRIFT-004` bump. **Citations in this row are superseded, and are corrected here rather than left to mislead:** `scoped-models` is `world.wit:855` (row said `:732`), `register-markdown-transformer` is `:586` (row said `:476`), and the world is `@0.10`, not `@0.8`. **What this closure does NOT cover, stated so it is not read as more than it is:** cyrup still routes all three upstream surfaces through ONE export pair, so a guest receives keys that do not apply to its surface — a mechanism difference that costs no behaviour (extra keys are inert, and `is_partial` is overridden per tool row by `TranscriptView::stale_extension_renders`) and is disclosed in-source, which is the bar README sets. **Falsification condition:** if a later pi tag gives one of the three records a field whose MEANING depends on which surface is rendering, the union bag becomes lossy and this row reopens; the check is a diff of `extensions/types.ts`'s three option records against `world.wit:305-321`. |
...
| ~~DRIFT-019~~ | ~~low~~ **CLOSED 2026-09-16** | **not-ported** *(was upstream-drift)* | M | duplicate-of: `PROV-014` | Radius / Qwen Token Plan providers unregistered (the wire API and OAuth landed) — **2026-08-15, BLOCKED on `DRIFT-009` for 3 of 4; the 4th is separable and NOT blocked**: area-12 pass, re-derived at `v0.84.2`. Still absent — `providers/all.rs:38-40` now says so in its own port-status table (`✗ NOT REGISTERED — PROV-014` for `qwen-token-plan`, `qwen-token-plan-cn`, `radius`) and `:377` asserts it, so the row is self-documenting at HEAD. **The three qwen token plans are blocked by `DRIFT-009`'s data block**: `qwenTokenPlanProvider` is `createProvider({ …, models: Object.values(QWEN_TOKEN_PLAN_MODELS) })` (`providers/qwen-token-plan.ts` @v0.84.2), i.e. a STATIC catalog, and that catalog is not in git at any revision. Registering them with an empty catalog is worse than absent — a provider that offers no models. **`radius` is different and was mis-scoped by this row:** `radiusProvider` (`providers/radius.ts` @v0.84.2) has **no static catalog at all** — it is `getModels: () => models` seeded empty plus a `refreshModels(context)` that restores from `context.stored`, imports the pre-ModelsStore legacy cache, then fetches `loadRadiusGatewayConfig(gateway, apiKey, signal)` and publishes. So radius needs the 96-line `radius-config.ts`, a `ModelsStore`-shaped refresh seam and a `builtin_oauth.rs` arm — a real but self-contained slice with no catalog dependency. **Split the item**: radius is schedulable now under `PROV-014`; the qwen half waits on `DRIFT-009`. — **CLOSED 2026-09-16 at `cc7818b` (landed at `da970e8` for `radius` + the three Qwen plans; `9b49ad8`, `fix(parity): batch 4 — six gap rows closed, three narrowed`, added `baseten`, which was never this row's fourth — both shas verified present with `git cat-file -e` and confirmed ancestors of `cc7818b`).** **This row's subject is REGISTRATION, and all four of its providers are now registered, constructed and reachable.** The row's own most load-bearing sentence is what has gone stale: it said *"`providers/all.rs:38-40` now says so in its own port-status table (`✗ NOT REGISTERED — PROV-014` …)"*. At `cc7818b` that table reads the opposite — `crates/cyrup-provider/src/providers/all.rs:39-42`: `qwen-token-plan`, `qwen-token-plan-cn` and `qwen-token-plan-individual` are each **`✓ fleet (dynamic catalog) — PROV-014`** and `radius` is **`✓ super::radius — PROV-014`**. **Present:** the three plans are `FleetSpec` members with `FleetCatalog::Dynamic` and their real base URLs (`providers/fleet.rs:179`, `:183`, `:189`, each carrying its upstream `providers/qwen-token-plan*.ts:6-15` cite and its env var), and `radius` is its own 1 301-line provider module (`providers/radius.rs`) implementing exactly the three things this row said radius needed and called "a real but self-contained slice": the `radius-config.ts` port (`DEFAULT_RADIUS_GATEWAY` `:82`, `sanitize_radius_gateway_config` `:172`, `radius_credential_config` `:190`), the `ModelsStore`-shaped refresh seam (`crate::remote_catalog::RemoteCatalog::load_overlay` + `CatalogOverlay::apply`, `:38-39`, `:1129`) and the OAuth arm (`crate::auth::oauth::radius::RadiusOAuth`, `:56`, wired through `providers/builtin_oauth.rs`). **Reachable from production:** `all.rs:243` `builtin_providers_with` pushes `radius_provider_with(…)` at `:260` and composes the fleet through `fleet_providers_with` (`:111-112`), and `all_providers()` (`:203`) is the production entry — not a test-only constructor. **Pinned by tests:** `fleet.rs:388` `qwen_token_plan_members_match_upstream` asserts all three plans field-for-field against `qwen-token-plan{,-cn,-individual}.ts:6-15` and `env-api-keys.ts:81-83` (including the shared `QWEN_TOKEN_PLAN_API_KEY` for the Individual plan), and `builtin_oauth.rs:127` plus the `radius.rs` test module (`:699` onward, e.g. the gateway-config cases at `:776-892`) pin the radius provider and its gateway-config parsing. **WHAT THIS CLOSURE DOES NOT COVER, and it is the half this row explicitly deferred:** the three plans ship **no embedded catalog** — `FleetSpec::models()` returns an empty vec for a `Dynamic` member (`fleet.rs:206-211`) — so their rows come at runtime from the pi.dev overlay (`crate::remote_catalog`, `/api/models/providers/<id>`) and from `models.json`, and under `--offline`/`CYRUP_OFFLINE` they have no rows at all. **That is `DRIFT-009`'s data block, `DRIFT-009` REMAINS OPEN, and nothing here is rounded up into it** — the row's fear ("registering them with an empty catalog is worse than absent") was answered by giving them a live overlay rather than by fabricating rows, which is the outcome the catalog rules require. **CROSS-FILE, NOT ACTED ON HERE (this pass may edit only areas 08/11/12):** the owning row is area 01's `PROV-014`, which is still `PARTIALLY CLOSED … low` with its residual described as *"two catalog JSON files"* — i.e. `DRIFT-009`'s data block and nothing else, since the registrations this closure verifies are the rest of it. `PARITY-GAPS.md`'s **PB-2** (*"`qwen-token-plan` and `qwen-token-plan-cn` are not registered, but the resolver advertises them"*, medium) is now FALSE as written and points at this row and `PROV-014`. Both need the reconciliation pass README asks for — recorded here so the next reader does not take PB-2's wording as current. **Falsification condition:** if `all_providers()` stops constructing any of the four, or if the pi.dev overlay path is removed leaving a `Dynamic` member with no runtime source, this row reopens — the check is `providers/all.rs`'s builtin list plus `FleetSpec::is_dynamic` call sites. |
...
| DRIFT-047 | low | upstream-drift | L | duplicate-of: `VL-P5` | `packages/telemetry` and the `pi.ai.request` span contract absent — **2026-08-15, BLOCKED (measured), still open**: area-12 pass, re-derived at `v0.84.2`. Still absent — `grep -rnE 'TelemetryContext\|TelemetrySpan\|telemetry_context' crates --include='*.rs'` → **0**. Upstream sizes re-measured: `packages/telemetry/src` is **935 lines** (`index.ts` 357, `testing/conformance.ts` 315, `memory.ts` 219, `noop.ts` 20, `testing/{index,types}.ts` 24) and `packages/agent/src/harness/telemetry.ts` — where the `pi.ai.request` schema actually lives — is **615**. The threading surface in `packages/ai` is still just two lines (`types.ts:123` declares `telemetryContext?`, `api/simple-options.ts:36` forwards it), so the item's "SDK-surface parity today" note holds at v0.84.2 as well. **Why not attempted here rather than deferred silently:** the item's own refuter note says to resolve it by EXTENDING `PARITY-GAPS` VL-P5, not by opening a second L workstream from this ID, and landing only the trait pair would put a `telemetry_context` field on `StreamOptions` with no emitter and no conformance suite behind it — an invented surface, which is the `PROV-061` failure mode. Needs the VL-P5 owner. — **RE-READ 2026-09-14 at `9aeba769` (verified re-audit): still open; nothing moved on either side, the gap is TOTAL, and the classification is correct.** cyrup: this row's own grep still returns zero. Re-run against `crates/*.rs` for `TelemetryContext`, `TelemetrySpan`, `telemetry_context`, `start_span`, `in_span` and `pi.ai.request`, the only hits are unrelated TEST NAMES in `crates/cyrup-tools/src/lock.rs` that contain the English word "spans" — no `TelemetryContext`, no span seam, and no telemetry field on `crates/cyrup-provider/src/stream.rs`'s `StreamOptions`. Upstream @v0.85.1: `packages/telemetry` is present and is genuinely POST-BASELINE by the presence test rather than by date reasoning — `git ls-tree v0.83.0 packages/` lists exactly {agent, ai, coding-agent, evals, server, storage, tui} while `v0.85.1` lists {agent, ai, chord, client, coding-agent, evals, protocol, server, session-backends, telemetry, tui} — so `upstream-drift` is right. **The row's most load-bearing correction survives re-reading and must be kept:** `startSpan<T>(options, callback: (span) => T or Promise<T>): Promise<T>` (`packages/telemetry/src/index.ts:15`) is callback-SCOPED, so a guard-returning Rust port would NOT match pi's nesting; `startAiSpan` (`packages/agent/src/harness/telemetry.ts:138-146`) confirms it, threading the span into a derived `Context` via `withTelemetryContext(span, context)`, which only works because the callback delimits the scope. **The honest scope note also survives:** at `v0.85.1` `startAiSpan` still has NO production caller — `git grep startAiSpan v0.85.1 -- packages/` hits only `harness/telemetry.ts` (the definition), `agent/src/index.ts` (the re-export), two docs files, the README and `test/harness/telemetry.test.ts` — so this remains SDK-surface parity plus a schema, which is why `low` is right; do not upgrade on the strength of the package's size. That scope note IS an argument, and its falsifier is: `git grep startAiSpan <newer tag> -- packages/` acquiring a non-test, non-doc caller inside `packages/ai` or `packages/agent`, which would turn `pi.ai.request` from a declared vocabulary into emitted telemetry an embedder can miss and is the trigger to re-examine `low`. **LINE-CITE REFRESH for `v0.85.1`** (this row is pinned at `v0.84.1` and has drifted): `telemetryContext` is `packages/ai/src/types.ts:124-127` (row says `:122-123`); `startSpan` is `index.ts:15` (row says `:14-16`); `TelemetrySpan` `:19-22`; `TelemetrySpanDefinition`/schema vocabulary `:56-70`; `defineTelemetrySchema` `:72`; the `pi.ai.request` declaration and its four `pi.ai.operation` values `harness/telemetry.ts:42-58`; `startAiSpan` `:138-146` (row says `:138-144`). `packages/ai/src/api/simple-options.ts:36` is still exact. — **RE-READ 2026-09-16 at `cc7818b` — STILL OPEN at low; the gap is still TOTAL and the classification still right, re-derived at the re-checked latest tag.** cyrup: this row's own grep — `grep -rnE 'TelemetryContext\|TelemetrySpan\|telemetry_context\|pi\.ai\.request' crates --include='*.rs'` — returns **ZERO**, unchanged, and `crates/cyrup-provider` (which would carry the `StreamOptions` field) did not change in `9aeba769..cc7818b`. Upstream: `git -C tmp/pi tag --sort=-v:refname \| head -1` is still **v0.85.1**, so the v0.85.1 read this row already carries is current rather than stale — `packages/telemetry` is present at v0.85.1 and absent from `git ls-tree v0.83.0 packages/`, which settles `upstream-drift` by presence rather than by date. **The `low` rating still rests on an argument, and its falsifier is unchanged and was NOT re-run this pass** (it needs a newer tag than v0.85.1 to be meaningful, and there is none): `git grep startAiSpan <newer tag> -- packages/` acquiring a non-test, non-doc caller inside `packages/ai` or `packages/agent` turns `pi.ai.request` from a declared vocabulary into emitted telemetry and is the trigger to re-examine `low`. The v0.85.1 line-cite refresh recorded on 2026-09-14 (`packages/ai/src/types.ts:124-127`, `telemetry/src/index.ts:15`, `:19-22`, `:56-70`, `:72`, `agent/src/harness/telemetry.ts:42-58`, `:138-146`, `ai/src/api/simple-options.ts:36`) needs no further correction at this tag. Still deferred to the `VL-P5` owner for the reason this row gives — landing the trait pair alone would put a `telemetry_context` field on `StreamOptions` with no emitter and no conformance suite behind it. **RE-VERIFIED STILL OPEN (low) 2026-09-24 at `ea23ca2` / pi `v0.87.1`:** cyrup side byte-identical to its last reading (`9aeba769..ea23ca2` touches none of `crates/cyrup-provider`, `cyrup-core`, `cyrup-config`, `cyrup-session`, `cyrup-tools`). The falsifier this row names was finally runnable against a newer tag and did NOT fire: `git grep startAiSpan v0.87.1 -- packages/` hits only `agent/src/harness/telemetry.ts:138` (definition) and `agent/src/index.ts:103` (re-export); `packages/telemetry` changed only `CHANGELOG.md`/`package.json` in `v0.85.1..v0.87.1`. |
...
| DRIFT-055 | low | upstream-drift | S | — | **`/share` uploads the whole session tree with original `parentId`s and the stored header timestamp, where pi uploads the linearised current branch with a fresh one** — `DRIFT-053`'s residual (d), given its own id 2026-09-05 as that row asked. **upstream** `core/session-export.ts:21-38` @v0.84.4 — `exportSessionToJsonl` writes `getBranch()` with `parentId` rewritten into a linear chain and a fresh `new Date().toISOString()` header timestamp. **cyrup** `crates/cyrup-session/src/manager/accessors.rs::export_jsonl` writes the whole tree with parentIds intact and the stored header timestamp, and `/share` posts that. **Impact** a shared artifact carries abandoned branches the sharer did not mean to publish, and its header timestamp is the session's creation time rather than the share time. **Fix** give the share path a linearised export, leaving `cyrup --export`'s whole-tree dump alone. **Verify** share a session with two branches; assert only the current branch's entries are in the artifact and that the header timestamp is the share time — **RE-READ 2026-09-16 at `cc7818b` — STILL OPEN at low, BOTH SIDES READ for the first time since it was filed, and its upstream half is re-pinned from v0.84.4 to the re-checked latest tag v0.85.1 with no change of substance.** **upstream @v0.85.1** `packages/coding-agent/src/core/session-export.ts` — `exportSessionToJsonl` still stamps a fresh header (`const timestamp = new Date().toISOString();` `:21`, used at `:26`) and still linearises: `let parentId: string \| null = null; for (const entry of sessionManager.getBranch()) { lines.push(JSON.stringify({ ...entry, parentId })); parentId = entry.id; }` (`:31-35`). The share path is what consumes it — `modes/interactive/session-share.ts:25-26` `exportSessionForShare` calls `exportSessionToJsonl(session.sessionManager, filePath, …)` and `:53` is the `/share` call site — so the row's upstream claim is confirmed at the tag it must now be measured against. **cyrup** `crates/cyrup-session/src/manager/accessors.rs:20-30` `export_jsonl` still writes `self.header.clone()` (only `cwd` overwritten — the STORED creation timestamp survives) followed by `for e in &self.entries` — the whole append-only store, every abandoned branch included, with `parentId`s untouched; there is no `getBranch()` equivalent on this path. **The `/share` wiring was traced end to end this pass rather than assumed:** `crates/cyrup-tui/src/app/execute_misc.rs:1080-1104` `share_session` takes ONE export via `session.export_to_jsonl(None)` (`:1087`) and feeds it to both arms — `try_share_via_radius(session, &jsonl)` (`:1104`, `:1241`) and the gist arm's `session_jsonl_to_html_with_theme` (`:1107`) — and `export_to_jsonl` is `crates/cyrup-session-svc/src/session/transcript.rs:57-63`, which calls `export_jsonl`. So the defect reaches the shared artifact by both routes. Note `crates/cyrup-tui` DID change in this window (`9aeba769..cc7818b`, TUI-073/TUI-097 work) — the share path was re-read at `cc7818b` because of that, and it is unchanged. Severity `low`, and the Fix stands as written: give the SHARE path a linearised export with a fresh header timestamp, leaving `cyrup --export`'s deliberate whole-tree dump alone. **RE-VERIFIED STILL OPEN 2026-09-24 at `ea23ca2` / pi `v0.87.1`:** `crates/cyrup-session/src/manager/accessors.rs` and `crates/cyrup-session-svc/src/session/transcript.rs` are unchanged since `9aeba769`, and `crates/cyrup-tui/src/app/execute_misc.rs`'s diff in the window is TUI-097 `/copy` work only — `share_session` is untouched. Upstream @v0.87.1 refactored `exportSessionToJsonl` into `serializeSessionBranch` + writer (`packages/coding-agent/src/core/session-export.ts`) and the share path into `createShareTrailingEntries` (`modes/interactive/session-share.ts`); the linearised `getBranch()` walk with rewritten `parentId` and the fresh header timestamp are preserved, so the row stands as written. |
| DRIFT-056 | medium | upstream-drift | S | duplicate-of: `EXT-077` | **`user_bash` fails OPEN: a faulting or malformed extension handler lets the command run on the local shell, where pi v0.86.0 aborts it** — **FILED 2026-09-24**; body below. |
| DRIFT-057 | low | upstream-drift | S | — | **Retry: `520` and "currently experiencing high demand" are not retryable, and the backoff has no `maxAgentDelayMs` cap (pi default 60 s)** — **FILED 2026-09-24**; body below. **GROWTH 2026-09-24 (second pass):** a third missing literal, `"exceeded request buffer limit while retrying upstream"` (pi `utils/retry.ts` @v0.87.1, in `RETRYABLE_PROVIDER_ERROR_PATTERN` after `provider.?returned.?error`; landed v0.84.2), is absent from `crates/cyrup-provider/src/utils/retry.rs`'s `RETRYABLE_PROVIDER_PATTERNS`. Same list, same fix. |
| DRIFT-058 | low | upstream-drift | M | — | **`ToolCall.namespace` is not modelled: a namespaced OpenAI Responses tool call loses its `namespace` on the streamed block, in the JSONL and on same-model replay** — pi v0.84.2 (#7709), still present @v0.87.1 and since v0.86.0 replayed only when the call came from the same model. A cyrup-core wire-format change. **FILED 2026-09-24 (second pass)**; body below. |
| DRIFT-059 | low | upstream-drift | S | — | **`AssistantMessage.endTurn` (Codex's terminal `end_turn`) is not recorded** — pi v0.84.2 (#7766) stores it on the message and in the session file; diagnostics-only upstream, so a cyrup session lacks a field pi's carries. **FILED 2026-09-24 (second pass)**; body below. |
...
| DRIFT-023 | **tracker** · *lead* | tracking | L | duplicate-of: `CFG-020` | Model registry → `ModelRuntime` refactor not absorbed — **evidence unverified** — **RE-PROBED 2026-09-16 at `cc7818b` — STILL A TRACKER, still `lead — evidence unverified`, and deliberately NOT escalated.** Probe: `grep -rn 'ModelRuntime\|model_runtime' --include='*.rs' crates/` hits **doc comments only** (`cyrup-provider/src/auth/store.rs:35`, `unconfigured.rs:12`, `collection.rs:37`, `:223`, `cyrup-tui/src/login_dialog.rs:24`) — cyrup names pi's `ModelRuntime` as the thing it is porting AGAINST, and has no type of that shape. That is consistent with the row as written and refutes nothing. **It stays a tracker because escalating it needs the one thing it has never had — a two-sided read of pi's registry→`ModelRuntime` refactor against `CFG-020`'s owned surface — and this pass did not do that read.** What would escalate it is unchanged: a two-sided read showing a behaviour (not a shape) that cyrup cannot express, at which point the work belongs to `CFG-020`, not to a second id here. **2026-09-24 at `ea23ca2`: tracker, not re-derived;** `crates/cyrup-config` is byte-identical to `9aeba769`, so the 2026-09-16 probe stands. pi v0.86.0 changed `model-runtime.ts` only to `normalizeContext` its stream inputs. |
| DRIFT-032 | **tracker** | **not-ported** *(was upstream-drift)* | L | duplicate-of: `EXT-027` | llama.cpp router integration and Hugging Face model search entirely unported — **RE-PROBED 2026-09-16 at `cc7818b` — STILL A TRACKER, absence re-confirmed.** `grep -rniE 'llamacpp\|llama-server\|router' --include='*.rs' crates/cyrup-provider/src/` returns nothing but OpenRouter (a different thing: the `openrouter` / `openrouter-images` providers), and a workspace grep for a Hugging Face model SEARCH surface finds only the `huggingface` PROVIDER registration (`providers/fleet.rs:166`, `all.rs:26`, `env_api_keys.rs:81`) — the provider is ported, the router integration and the search UI are not, exactly as this row says. **Unchanged, and still proposing no work of its own:** it is `duplicate-of: EXT-027`, so the schedulable form of it lives there. Recorded so the next pass does not re-derive the absence a third time. **2026-09-24 at `ea23ca2`: tracker, not re-derived;** no change in its cyrup locus in `9aeba769..ea23ca2`. |
| DRIFT-040 | **tracker** · *lead* | tracking | L | duplicate-of: `VL-P22` | pi's agent-harness v2 rearchitecture entirely unabsorbed — **evidence unverified** — **RE-PROBED 2026-09-16 at `cc7818b` — STILL A TRACKER, still `lead — evidence unverified`.** `grep -rniE 'harness.?v2\|retainedTail\|retained_tail' --include='*.rs' crates/` returns **zero**, which is what this row and `DRIFT-037`'s withdrawal already imply and is therefore not new evidence. **Not escalated**: pi's agent-harness v2 rearchitecture is a whole-package comparison against `packages/agent` at v0.85.1 that no pass has performed, and filing severity-bearing work off an unread upstream is precisely what this file's repair pass created the tracker class to stop. Escalation condition unchanged — a two-sided read of `packages/agent`'s harness against cyrup's turn loop, owned by `VL-P22`. **2026-09-24 at `ea23ca2`: tracker, not re-derived — UPSTREAM GROWTH recorded:** `v0.85.1..v0.87.1` adds `packages/agent/src/harness/pico3/` (session 1 497, types 1 038, harness 812, generation 641, scheduler 486 lines, …) and a new `packages/durable` (27 files, +8 533). Neither was read; sizing only. |
...
**duplicate-of: `PROV-018`** (tooling half) · see also `PROV-004`, `PROV-039`, `PARITY-GAPS` VL-P25, `PROV-071`
...
## DRIFT-056 — `user_bash` fails OPEN: a faulting or malformed handler lets the command run locally

**Kind** upstream-drift · **Severity** medium · **Effort** S · **Confidence** confirmed · **Filed** 2026-09-24

**cyrup** (at `ea23ca2`) — `crates/cyrup-session-svc/src/session/bash.rs::emit_user_bash_event` runs
`dispatch_block_mutate` and returns `UserBashOutcome::None` — local execution — for anything that is
not `Reduced::Handled` (its own comment: "a block or a pass falls through to normal local execution").
`crates/cyrup-ext/src/dispatch.rs::dispatch_block_mutate` handles a contained handler fault by
`continue` unless `kind.fails_closed()`, and `crates/cyrup-ext/src/event.rs::EventKind::fails_closed`
is `matches!(self, EventKind::ToolCall)` only — so a trapped, panicking or timed-out `user_bash`
handler degrades to `Pass`. A `Handled` value whose `result` does not deserialize as `BashResult` and
whose owner supplies no operations also returns `None`. And
`crates/cyrup-ext/src/facade.rs::ExtensionHost::user_bash_operations` contains a panicking native
supplier as `None` with the doc "the command falls back to the local shell", citing pi's old
`try`/`catch`.

**upstream** — `packages/coding-agent/src/core/extensions/runner.ts` @v0.87.1: `isUserBashEventResult`
(`:136`) requires exactly one of a function-bearing `operations` or a fully-typed `result`;
`emitUserBash` throws on an invalid defined result (`:1162`) and re-throws after reporting a handler
error (`:1177`). The callers do not fall back: `modes/interactive/interactive-mode.ts:6733-6743`
`catch { … Do not fall back to local execution. return; }`, and `modes/rpc/rpc-mode.ts:564` lets the
throw fail the `bash` command. Landed **v0.86.0** (`509ee2bd0`, #9662, fixing #9068) and listed as a
breaking change. Before it, pi's per-handler `catch` continued — cyrup's current behaviour.

**Impact** — an extension that routes user `!` commands to a sandbox, container or remote host can
fault, and the command then runs on the local machine instead — the outcome upstream changed its API
to prevent. The user typed the command, so this is not a model-driven bypass, which is why it is
`medium` rather than rated as a permission bypass; re-rate if a permission extension is found relying
on `user_bash` to block.

**Fix** — make `EventKind::UserBash` fail closed in `dispatch_block_mutate`; in
`emit_user_bash_event`, treat a `Handled` value that is neither a valid `result` nor an
operations-bearing owner as an error that aborts the command; have `user_bash_operations` report a
panicking supplier as an abort, not `None`. The TUI and RPC paths must surface the abort instead of
executing.

**Verify** — with a native extension whose `user_bash` handler errors, `!echo hi` records no bash
execution and reports the extension error; with a handler returning `{"result":{"output":1}}`, the
same; with `undefined`/no subscriber, local execution is unchanged.

## DRIFT-057 — Retry: `520` and "currently experiencing high demand" not retryable; no backoff cap

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed · **Filed** 2026-09-24

**cyrup** (at `ea23ca2`) — `crates/cyrup-provider/src/utils/retry.rs`: `RETRYABLE_PROVIDER_PATTERNS`
has `"524"` but no `"520"` and no `"currently experiencing high demand"`; the per-attempt delay is
`base_delay_ms.saturating_mul(2u64.saturating_pow(attempt - 1))` with no cap. The agent-level retry,
`crates/cyrup-session-svc/src/session/retry.rs::prepare_retry`, computes the same uncapped product, and
no crate knows `maxAgentDelayMs` (`git grep -n 'max_agent_delay\|maxAgentDelayMs' -- crates/` is empty).

**upstream** — `packages/ai/src/utils/retry.ts` @v0.87.1: `:29` `"currently experiencing high
demand"` (Azure peak load, #9669) and `:37` `"520"` (Cloudflare, #9627) in
`RETRYABLE_PROVIDER_ERROR_PATTERN`; `:111` `DEFAULT_MAX_AGENT_RETRY_DELAY_MS = 60_000` and `:113-116`
`retryDelayMs` clamps to `policy.maxAgentDelayMs ?? 60 000`, used at `:209`.
`packages/coding-agent/src/core/agent-session.ts:3393` uses the same `retryDelayMs(settings, …)`, and
`settings-manager.ts:45`/`:932-937` add `retry.maxAgentDelayMs` (default 60 000). All three landed in
**v0.86.0** (#8826 for the cap).

**Impact** — Cloudflare-520 and Azure high-demand failures end the turn instead of retrying; a user
with a large `retry.maxRetries` waits unboundedly doubling delays (2 s base → over 17 min by attempt
10) where pi waits at most a minute.

**Fix** — add the two literals; add `max_agent_delay_ms: Option<u64>` to the provider `RetryPolicy`
and to the session retry settings with the 60 000 default, and clamp both delay sites through one
shared helper.

**Verify** — `is_retryable_assistant_error` on an error message `"520 …"` and on the Azure text returns true;
`retry_delay_ms(base 2000, attempt 10)` returns 60 000; a `retry.maxAgentDelayMs` of 5 000 in
settings caps the `AutoRetryStart.delay_ms` the session emits.

**Growth 2026-09-24 (second pass)** — add `"exceeded request buffer limit while retrying upstream"` too (pi
`utils/retry.ts` @v0.87.1, v0.84.2); cyrup's list stops at `"provider.?returned.?error"`.

## DRIFT-058 — `ToolCall.namespace` is not modelled

**Kind** upstream-drift · **Severity** low · **Effort** M · **Confidence** confirmed · **Filed** 2026-09-24 (second pass)

**cyrup** — `crates/cyrup-core/src/message/tool_call.rs`: `ToolCall { id, name, arguments,
thought_signature }` with a hand-written 4-or-5-field `Serialize`; `grep -rn namespace
crates/cyrup-provider/src/api/openai_responses crates/cyrup-core/src/message` is empty outside tests.

**upstream** — `packages/ai/src/types.ts:393` @v0.87.1 (`namespace?: string`, "for calls to dynamically
loaded or namespaced tools"); `api/openai-responses-shared.ts` reads `item.namespace` at stream start and end
(`:491`, `:512`, `:715`, `:731`) and replays it on `function_call`/`custom_tool_call` only when
`isSameModel` (`:315`, `:324` — v0.86.0 dropped the v0.85 "or it is a deferred tool" arm with
`addedToolNames`). Landed v0.84.2 (#7709).

**Impact** — a call OpenAI returns inside a namespace is replayed without it on the next turn, so the model
sees a call to a tool name that does not resolve in that namespace; the session file drops the field.
Predicted from upstream's `test/openai-responses-namespace.test.ts`, not observed.

**Fix** — optional `namespace` on `ToolCall` (serialized after `thoughtSignature`, pi's key order), captured
in the Responses decoder, replayed under the same-model rule; area 03 owns the session-format note.

**Verify** — the upstream namespace test translated; a JSONL round-trip preserves the key.

## DRIFT-059 — `AssistantMessage.endTurn` is not recorded

**Kind** upstream-drift · **Severity** low · **Effort** S · **Confidence** confirmed · **Filed** 2026-09-24 (second pass)

**cyrup** — `crates/cyrup-core/src/message/assistant.rs` has no `end_turn`; the Codex decoder
(`api/openai_codex_responses/events.rs`) does not read `response.end_turn`.

**upstream** — `packages/ai/src/types.ts:535` @v0.87.1 `endTurn?: boolean`;
`api/openai-codex-responses.ts:748-752` sets it from `response.done|completed|incomplete`. v0.84.2 (#7766),
documented as not affecting agent control flow.

**Impact** — diagnostics only; a cyrup session file lacks a field pi's carries for the same turn.

**Fix** — optional field on `AssistantMessage`, set by the Codex decoder.

**Verify** — a Codex fixture with `"end_turn": false` persists `"endTurn": false`.
