...
| DRIFT-056 | medium | upstream-drift | S | — | **`user_bash` fails OPEN: a faulting or malformed extension handler lets the command run on the local shell, where pi v0.86.0 aborts it** — **FILED 2026-09-24**; body below. |
| DRIFT-057 | low | upstream-drift | S | — | **Retry: `520` and "currently experiencing high demand" are not retryable, and the backoff has no `maxAgentDelayMs` cap (pi default 60 s)** — **FILED 2026-09-24**; body below. |
