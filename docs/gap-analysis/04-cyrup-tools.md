...
| TOOL-017 | low | not-ported | M | `read`'s compact `docs` classification arm is unported (residual only) — **FIX SITE: `crates/cyrup-tui/**`, and it needs a PRODUCT DECISION first.** **2026-08-14, sweep 6 — re-verified still open**: `cyrup-tui/src/transcript.rs:2305` documents the missing arm in-source and names the blocker precisely — cyrup has no decided shipped-docs root for `getPiDocsClassification` to resolve against. Once that root is decided the arm is mechanical. **STILL OPEN 2026-08-15 — re-verified at HEAD `68bbd39`, and the in-source note has MOVED to `crates/cyrup-tui/src/transcript.rs:2350-2356` (the `:2305` cite is stale; it now lands mid-`more_lines_hint`, which is the exact stale-line hazard the batch rules warn about). The two ported arms and `COMPACT_RESOURCE_FILE_NAMES` (`:2326-2327`, five names, `AGENTS.override.md` first) are unchanged. Reported BLOCKED: this row asks for a PRODUCT DECISION — "what is cyrup's shipped-docs root?" — which no agent can take. It is not schedulable as work until that is answered; scheduling it again will produce a fourth pass that re-derives the same blocker.** |
