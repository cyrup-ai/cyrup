...
> **RECOUNTED 2026-08-15 (batch, area 04, fifth edition) — counted set: 0 critical, 0 high, 2 medium, 2 low = 4.** The table carries **34 rows: 30 fully closed, 4 open (2 of them partially closed).** *(Previous edition: 0 / 0 / 2 / 6 = 8, 26 closed.)* Four ids closed: **`TOOL-043`**, **`TOOL-045`** and **`TOOL-044`** (whose residual was a decision — it was TAKEN, in favour of porting `content`, and the reasoning is written into `truncate.rs` so it is not re-litigated), plus **`TOOL-031`**, which needed **no code at all**: its last residual had already landed in batch A (`68bbd39`) and the row was simply stale. All three code changes are inside `crates/cyrup-tools/**`, which is the third pass in a row to contradict the partitioning note below — see the amendment.
>
> **The four that remain are each blocked on something an area agent structurally cannot supply, and that is the useful headline.** `TOOL-042` needs ~300 `cargo nextest` runs, which this batch's rules forbid. `TOOL-017` needs a product decision (what is cyrup's shipped-docs root). `TOOL-022`/`TOOL-015` are one change, and re-verification found it is a **layout** change to the live transcript, not the "add a branch" the sweep-8 note describes — plus two recorded facts about it are now corrected (the WIT/producer half is DONE, and the `cyrup-session-svc` producer limb is probably unnecessary because pi's own component looks the built-in table up locally). Details in the 2026-08-15 blocks under each row. **None of the four should be re-routed to a tools agent; three of them should not be routed to an agent at all.**
>
> **One citation-rot correction from the confirm-at-HEAD pass, recorded because it is exactly the failure mode the batch rules name:** `TOOL-017`'s in-source blocker note is at `cyrup-tui/src/transcript.rs:2350-2356`, not `:2305` — `:2305` still exists and still has code on it (mid-`more_lines_hint`), so trusting the old offset cites the wrong function silently.
>
...
> **RECOUNTED 2026-09-04 (README-mandated re-audit, cyrup HEAD `2571969`, 210 commits past baseline `4fb5e40`) — counted set: 0 critical, 0 high, 2 medium, 1 low = 3.** `git log --oneline 4fb5e40..HEAD -- crates/cyrup-tools` (17 commits, +12514/−3243) was read; every commit that names a symbol this file tracks was checked against the code it actually left, not its message. **One closure this pass: `TOOL-017`** — see its row for the evidence (`cyrup_config::asset_dir()` as the shipped-docs root, `docs_classification` in `crates/cyrup-tui/src/transcript/tool_args.rs`, landed in `2daaa32`, well inside the measured window). **Two re-verified still open, unchanged:** `TOOL-015`/`TOOL-022` — `grep -rln 'ToolRenderKind' crates/*/src` at HEAD `2571969` still returns only `cyrup-core`, `cyrup-ext`, `cyrup-mcp` and `cyrup-tools` itself; zero hits in `cyrup-tui` or `cyrup-session-svc`, so the "layout change, not a lookup" blocker the 2026-08-15 edition described is unchanged and neither row moves. **One left exactly as found, with the reason stated rather than silently skipped:** `TOOL-042` — its own closure bar is "name a holder or name the harness" via ~300 `cargo nextest run -p cyrup-tools` runs, which this documentation-only pass did not execute (out of scope for a docs pass, and its citations `ops/local.rs:1135`/`:1568`/`:1812` are now additionally stale after `5250b8c` split `ops/local.rs` into `crates/cyrup-tools/src/ops/local/`, e.g. the `sleep 1` fixtures now live in `ops/local/tests/exec.rs` and `ops/local/tests/mod.rs` — noted here rather than re-derived, since fixing it needs the same withheld instrument). *(Previous edition: 0/0/2/2 = 4, 30 closed.)* **The version-lag question was also re-run this pass**, not just the closure sweep: `git -C tmp/pi diff --stat v0.84.1..v0.84.4 -- packages/coding-agent/src/core/tools/` touches `bash.ts`, `edit.ts`, `edit-diff.ts`, `find.ts`, `grep.ts`, `index.ts`, the new `powershell.ts`, `read.ts` and `write.ts` (9 files, +186/−35), and every one of those deltas was read and found **already carried at cyrup HEAD**: the `powershell` 8th built-in (`registry.rs` `BUILTIN_NAMES`, `tools/powershell.rs`, third position matching `index.ts`'s literal order), the shared `bash`/`powershell` engine refactor (`tools/bash.rs` `ShellTool`/`ShellToolConfig`, `PS>` vs `$` prompts in `cyrup-tui/src/transcript/tool_builtin.rs:498`), `constrainedSampling` gated on the experimental flag on `read`/`write`/`bash` (tests named `*_declares_constrained_sampling_exactly_when_the_experimental_flag_is_on` in each tool file), the `command` schema wording `"Shell command to execute"`, and `edit`'s new bare-single-edit-object normalization (`normalize_args`/`is_single_edit` in `tools/edit.rs`, citing v0.84.3 directly) — so **no new version-lag item is filed.** The `stripBom`→`splitBom` rename and the `ensureTool(name, true)`→`ensureTool(name)` default-arg drop in `find.ts`/`grep.ts` were read and are cosmetic / out of cyrup's scope (cyrup does not replicate pi's auto-download-tool mechanism) — excluded, not silently skipped.
...
| ~~TOOL-022~~ | ~~medium~~ **CLOSED 2026-09-04** | not-ported | L | `renderShell`, `prepareArguments` and `label` never reach a guest tool's behavior — **CLOSED 2026-09-04**: all three limbs are wired end to end at HEAD, re-read on BOTH sides this pass rather than trusted from the text below. **(1) `prepareArguments`** — the WIT carries the flag (`crates/cyrup-ext/wit/world.wit:128`) plus the `guest.prepare-arguments` export the function-value-across-a-component-boundary inversion forces (`:255-259`); the SDK lowers it (`cyrup-ext-sdk/src/guest.rs:77`), `register_tool` lifts it (`cyrup-ext/src/host/live.rs:126`), `WasmTool::prepare_arguments` (`live.rs::prepare_arguments`, `:2329`) calls the export only when the descriptor set it and degrades a guest fault to the identity, and the agent runs it BEFORE schema validation (`cyrup-agent/src/agent/run/tools/preflight.rs:40-49`) — pi's `prepareToolCallArguments` → `validateToolArguments` order (`packages/agent/src/agent-loop.ts:584-596` @v0.84.4). **(2) `renderShell`** — `render-shell: option<string>` (`world.wit:132`), lowered at `guest.rs:78-84`, lifted at `live.rs:127`, `WasmTool::render_kind` mapping `Some("self")` at `live.rs:2303-2308`; the CONSUMER this row was blocked on landed in `75532cee` (EXT-024, 2026-09-04, branch `claude/beautiful-feynman-odz1v5`, already on `main` at this branch's base `3e9633c4`): `ToolInfo.render_kind` (`cyrup-session-svc/src/tools.rs:41`, filled from `Tool::render_kind` at `:223`, served by `AgentSession::tool_definition`, `session/tools.rs:28`) → `AppState::known_tool_definitions` (`cyrup-tui/src/app/state.rs:403-405`, filled by the live per-tool-start lookup `app/events.rs:289-297` and the bind refresh `app/session_bind.rs:144-160`) → `ToolRun::definition: Option<ToolRenderKind>` (`transcript/entry.rs:403`) → `tool_lines`' `getRenderShell()` port and its `self_rendered_lines` branch (`transcript/tool_render.rs:96-122`, `:140-200`) == `tool-execution.ts:71-79` / `:108-116` / `:237-259` @v0.84.4. **(3) `label`** — `tool-descriptor.label: string` (`world.wit:116`), `WasmTool::label` (`live.rs:2265-2267`, empty ⇒ `None`), delegated by `cyrup-ext/src/wrapper.rs:110-111`; **and the "no consumer" half is PARITY, not a gap** — at v0.84.4 pi copies `label` in `core/tools/tool-definition-wrapper.ts:11`/`:39` onto `AgentTool.label` (`packages/agent/src/types.ts:389`) and then reads it NOWHERE in `packages/coding-agent`, `packages/agent`, `packages/server`, `packages/protocol` or `packages/client` (`ToolExecutionComponent` titles the row from `toolName`, `tool-execution.ts:137-139`), so cyrup having a reachable-but-unread `Tool::label` is exactly upstream's state. A source-text guard now fails the build if any of these overrides is deleted again (`live.rs:2939-2982`, the eleven `fn …(` names). **RESIDUAL, named rather than hidden:** the item's Verify limb (a) — a real WASM fixture guest whose `prepare-arguments` renames a key, asserted through `execute` — still has no test; every link is proven in source and the trait-level ordering is pinned by `cyrup-agent/src/tests/model_boundary.rs::gap18_prepare_arguments_runs_before_execute`, but nothing drives the export across a live component. That is test coverage, not a behavioural gap, and it belongs with the `cyrup-it` wasm fixture suite. **TOOL-015's stated residual ("nothing reads `render_kind`") is discharged by the same commit** — left to that row's owner rather than closed from here. **Citations in this row were re-resolved against HEAD `ba380e58` on 2026-09-05**, by re-finding each SYMBOL rather than shifting an offset: `EXT-006` (`1739fcc4`) and `DRIFT-004` (`8b688401`) rewrote `world.wit` and `host/live.rs`, and `CFG-078` (`69f6c4e1`) moved `app/state.rs`, after this row was written. Every symbol the row names still EXISTS — the substantive REFUTED verdict never depended on the numbers — but the pointers had all rotted, which is the standing hazard `README.md`'s *Work this directory owns* describes. **Superseded text follows.** **FIX SITE: `crates/cyrup-tui/**` + `crates/cyrup-core`, NOT `crates/cyrup-tools`.** **2026-08-14, sweep 6 — re-verified: the PRODUCER half is done** (`WasmTool` overrides `render_kind`/`prompt_guidelines`/`prepare_arguments` at `cyrup-ext/src/host/live.rs:1774-1808`, and `cyrup-ext/src/wrapper.rs:110` delegates `render_kind`). What is missing is a CONSUMER: `grep -rn 'render_kind' crates` shows **zero** sites in `cyrup-tui` that branch on the value. ~~Needs one agent owning cyrup-tui + cyrup-core~~ **CORRECTED 2026-08-14 (sweep 8): needs `cyrup-session-svc` TOO — `ToolRun` carries only `name: String` and no `tool_info`/`tool_catalog`/`set_tools` accessor exists on `App`, so there is nothing for a TUI branch to read.** See the correction block under the body. **STILL OPEN 2026-08-15 — re-verified at HEAD, and TWO of the recorded facts are now corrected; see the 2026-08-15 block under the body. Reported BLOCKED rather than half-landed.** **STILL OPEN 2026-09-04 — re-verified at HEAD `2571969`: `grep -rln 'ToolRenderKind' crates/*/src` returns only `cyrup-core`, `cyrup-ext`, `cyrup-mcp` and `cyrup-tools`; still zero in `cyrup-tui` or `cyrup-session-svc`. No code landed against this row in the `4fb5e40..HEAD` window.** |
...
| ~~TOOL-031~~ | ~~low~~ **CLOSED 2026-08-15 — the residual was already gone at HEAD** | not-ported | S | `bash` stamps no `AI_AGENT` / `PI_CODING_AGENT` into the child environment — **CLOSED 2026-08-15**: re-verified at HEAD `68bbd39` before any code was written, per the confirm-first rule, and **the row's last residual (the subagent re-exec half, PARITY-GAPS PB-5) no longer exists**. `crates/cyrup-ext-subagents/src/exec/mod.rs:1961-1963` writes `PI_CODING_AGENT=true` and `AI_AGENT=cyrup` into the re-exec `env_overlay` unconditionally, with the `[CYRUP-DELTA, value only]` tag on `AI_AGENT` and the PB-5 rationale in-source at `:1947-1960`; pinned by the assertions at `exec/mod.rs:6833-6861`. It landed in batch A (`68bbd39`, "subagents and provider") and no tools pass noticed, exactly as `TOOL-016` closed under `PROV-011`. **No new code was written for this row — it is a stale-row correction, not a fix.** The other two halves were already recorded closed (`cyrup-session-svc/src/bash.rs:149`/`:159`; `cyrup-tools/src/tools/bash.rs:165`/`:168`). |
...
| ~~TOOL-042~~ | ~~medium~~ **CLOSED 2026-09-04** | test-defect | M | An intermittent `LEAK` fails the `cyrup-tools` suite; the source-scan pin closed one real class but **not** the failure — **REOPENED 2026-08-14 (sweep 8), superseding sweep 6's "filed and closed … pinned so it cannot return".** Refuted by measurement, not by reading: **3 LEAK-FAILs in 286 scrubbed runs (~1.0%, down from the historical ~4 in 33 ≈ 12%), on three DIFFERENT tests**, one of them on an idle box. `fail-fast` CANCELS the run (`243 tests run: 242 passed, 1 failed`), so it is a hard gate red. **The stated mechanism is refuted for the instrumented occurrence**: it named `ops::bash_operations_tests::local_bash_operations_forwards_command_cwd_and_env_onto_the_proc_seam`, which drives `RecordingProc` — an in-memory double (`ops/mod.rs:605`) — and whose only possible child (`which bash`, `ops/shell.rs:78-84`) names all three stdio handles and is reaped in-loop, so **no candidate holder of that test process's fd 1/2 exists in its subtree**, and the wait ran the full 500 ms. Corroborating: 69 distinct orphan pipe addresses vs 244 sampled `cargo-nextest` pipe addresses — **zero intersection**. **KEEP the source-scan pin: it closed a real class.** RESIDUAL: a harness-level question. See the body. **STILL OPEN 2026-08-15 — BLOCKED, and the reason is structural rather than a judgement call: the residual is defined as "does `cargo nextest`'s leak detector time out under saturation", and the row's own closure bar is "name a holder or name the harness". Sweep 8 needed 286 scrubbed runs of `cargo nextest run -p cyrup-tools` to see 3 events (~1.0%); this batch's agents are forbidden `cargo test`/`cargo nextest` entirely (the orchestrator owns the gate), so the ONE instrument that can discharge this row was unavailable. Nothing was written for it — in particular `.config/nextest.toml`'s 500 ms tripwire was not touched, since raising it hides the signal and is the user's call. Route to whoever runs the gate, with the run count (≥300) budgeted up front. The separate hygiene lead (the `exec`/`exec_argv` fixtures' backgrounded descendants at `ops/local.rs:1135`, `:1568`, `:1812`) was re-read at HEAD and is NOT actionable as written: `a_dropped_exec_future_kills_the_whole_process_group_not_just_the_direct_child` and its two siblings already `kill_pid(descendant)` before asserting, and the three that deliberately leave a `sleep 1` document that at `ops/local.rs:1061-1092`.** **LEFT OPEN 2026-09-04, unverified by this pass, one sentence why: the row's own closure bar needs ≥300 live `cargo nextest run -p cyrup-tools` runs, which a documentation-only pass does not execute, so the mechanism/rate cannot be re-measured here — the citations are additionally now stale after `5250b8c` split `ops/local.rs` into `crates/cyrup-tools/src/ops/local/{mod,exec,...}.rs` (the `sleep 1` fixtures now live in `ops/local/tests/exec.rs` and `ops/local/tests/mod.rs`), noted rather than re-derived since the exact new line numbers were not the blocking question.** **NARROWED 2026-09-04 (measurement pass, cut short by budget — NOT closed; counts recorded so nobody re-derives them):** the budgeted block was started and the row's instrument finally ran: **110 runs of `cargo nextest run` over the archived `cyrup-tools` test binaries (371 tests each, `--no-fail-fast`, retries 0) as TWO CONCURRENT loops (sweep 8's block-C shape) with the tripwire AMPLIFIED to `leak-timeout = 100ms` via a `--config-file` scratch copy of `.config/nextest.toml` (the committed 500 ms is untouched), on Linux 6.18.44 x86_64 / 4 vCPU at load 5–7, with a 10 ms `/proc` sampler recording every pipe held on fd 1/2 by every process throughout — 0 LEAK, 0 FAIL, 371/371 every run**; plus 4 amplified main-tree runs (0 LEAK) and the prior pass's 34 sequential runs at the stock 500 ms on `1139b1d9` (0 LEAK). Toolchain `rustc 1.98.0`, `cargo-nextest 0.9.143` / `nextest-runner 0.122.1`, cyrup HEAD `0c4d5bf1`. Two facts the row was missing, both load-bearing for whoever finishes the ≥300 block: (1) `2daaa32d` (2026-08-27, AFTER sweep 8's 2026-08-14 numbers) diagnosed the cross-test mechanism — std's non-atomic `pipe(2)`+`FIOCLEX` on macOS lets a concurrently spawned test inherit another test's pipe write end above fd 2, and a fixture's surviving `sleep 1` grandchild holds it past 500 ms naming an arbitrary NON-forking victim, exactly sweep 8's block-E shape — removed the amplifier (`SleeperMarker::reap`, `crates/cyrup-tools/src/ops/local/tests/mod.rs:40-127`, used at `tests/exec_argv.rs:95,130,164`) and pinned it (`fixture_scripts_that_fork_a_sleeper_record_its_pid_so_the_fixture_can_reap_it`, `src/tests/no_inherited_harness_stdio.rs:184`); (2) the harness read from source is clean on Linux: `nextest-runner 0.122.1 src/runner/executor.rs:1482-1551` `detect_fd_leaks` is a `biased` `select!` polling the stdout/stderr readers BEFORE a per-iteration `tokio::time::sleep(leak_timeout)`, so starvation cannot manufacture a LEAK, and the test pipes are `pipe2(O_CLOEXEC)` on Linux (`rust-lang/rust` `library/std/src/sys/pipe/unix.rs:7-29`; macOS takes the `pipe()`+`set_cloexec()` fallback), so on Linux a LEAK requires a real descendant of the NAMED test holding its fd 1/2 for ≥ `leak-timeout` — the spawn shape `every_command_in_this_crate_pins_all_three_stdio_handles` (`:79`) forbids. **Closure recipe (≈25 min at ~20 runs/min):** `cargo nextest archive -p cyrup-tools --archive-file A.tar.zst`, then two concurrent loops of `cargo nextest run --archive-file A.tar.zst --workspace-remap <repo> --no-fail-fast --config-file <copy with [profile.leak100] leak-timeout = { period = "100ms", result = "fail" }> --profile leak100` until ≥300 total, under a ~10 ms sampler of `/proc/<pid>/fd/{1,2}` → `pipe:[ino]` for every process plus every pipe fd of `cargo-nextest`; on any LEAK line, list every process that ever held the named test's (`--exact <test>` argv) fd 1/2 inodes and every `cargo-nextest` read-end closing within [−0.8 s, +0.3 s] — a holder named closes the row by fixing it at source; zero LEAK at ≥300 closes it as not-reproducible-on-Linux with `2daaa32d`'s reap + the three `no_inherited_harness_stdio.rs` pins as the closure and macOS (not measured here) as a stated low residual. **CLOSED 2026-09-04 — NOT REPRODUCIBLE ON LINUX; closure = the row's own "name the harness" bar, discharged by measurement.** The ≥300-run block the row budgeted was finally executed, with the tripwire AMPLIFIED (100 ms, not the stock 500 ms) and under saturation: **300 runs of `cargo nextest run` over the archived `cyrup-tools` test binaries (371 tests each, `--no-fail-fast`, `leak-timeout = 100ms` / `result = "fail"`, retries 0) as two concurrent loops (sweep 8's block-C shape) on Linux 6.18.44 x86_64 / 4 vCPU at load 5–7 — 0 LEAK, 0 FAIL, 371/371 passed every run.** Plus 4 amplified runs in the main tree (0 LEAK, 0 FAIL) and the previous pass's 34 sequential runs at the stock 500 ms on `1139b1d9` (0 LEAK, 0 FAIL). Toolchain `rustc 1.98.0`, `cargo-nextest 0.9.143` (`nextest-runner 0.122.1`), cyrup HEAD `0c4d5bf1`; a 10 ms `/proc` sampler recorded every pipe held on fd 1/2 by every process for the whole block so a LEAK line could be correlated to its holder (`cargo-nextest`'s read-end inodes vs every writer) — there was nothing to correlate. **The harness is named, and it is clean on Linux, from source:** `nextest-runner 0.122.1 src/runner/executor.rs:1482-1551` `detect_fd_leaks` is a `biased` `select!` that polls the stdout/stderr readers BEFORE a fresh `tokio::time::sleep(leak_timeout)` on every iteration, so runtime starvation cannot manufacture a LEAK — EOF wins whenever it has arrived; the test's pipes come from `src/test_command/imp.rs:40-92` (`stdin` null, `stdout`/`stderr` `Stdio::piped()` → std's `pipe2(O_CLOEXEC)` on Linux, `library/std/src/sys/pipe/unix.rs:7-29`, where `linux` is in the atomic-`pipe2` list and macOS is not). So on Linux a LEAK needs a real descendant of the NAMED test holding that test's fd 1/2 for ≥ `leak-timeout`, and `every_command_in_this_crate_pins_all_three_stdio_handles` (`crates/cyrup-tools/src/tests/no_inherited_harness_stdio.rs:79`) forbids the only spawn shape that can produce one. **Why sweep 8 saw 3/286 and this pass sees 0/338 at a five-times-tighter tripwire:** sweep 8 (2026-08-14) predates `2daaa32d` (2026-08-27), which diagnosed the cross-test mechanism the row was missing — std's NON-atomic `pipe(2)`+`FIOCLEX` on macOS lets a concurrently spawned test inherit another test's pipe write end above fd 2, and a fixture's surviving `sleep 1` grandchild kept it open past 500 ms, naming an arbitrary non-forking victim (exactly sweep 8's block-E shape). `2daaa32d` removed the amplifier (`SleeperMarker::reap`, `crates/cyrup-tools/src/ops/local/tests/mod.rs:40-127`, used by the three `exec_argv` fixtures at `src/ops/local/tests/exec_argv.rs:95,130,164`) and pinned it (`fixture_scripts_that_fork_a_sleeper_record_its_pid_so_the_fixture_can_reap_it`, `no_inherited_harness_stdio.rs:184`); the row never recorded either. Both pins are the closure; nothing was weakened — `.config/nextest.toml`'s 500 ms tripwire is untouched, no `#[ignore]`, no retries. **Falsification (what reopens this row):** any `LEAK` line from `cargo nextest run -p cyrup-tools` at or after `2daaa32d` — on Linux at any load, or on macOS — reopens it, and the reopening note must carry the holder: run the suite under a 10 ms `/proc` sampler of fd 1/2 pipe inodes and list every process holding the named test's harness-pipe inode at the LEAK timestamp (the recipe in the body). RESIDUAL (low): macOS was NOT re-measured this pass (no macOS box); its closure rests on `2daaa32d`'s fix + pin, not on a fresh run count. |
...
| ~~TOOL-047~~ | ~~high~~ **CLOSED 2026-09-24** | upstream-drift | S | **NEW 2026-09-24.** A shell command killed by a signal is returned to the model as a SUCCESS; pi v0.86.0 reports `128 + signo` as a failed exit — see the body. — **CLOSED 2026-09-24** (`6a3f162`): `ExitStatus::Signaled` now carries the signal number (`ops/local/proc.rs::exit_from`); `bash`/`powershell` fail a `Signaled(Some(n))` as `Command exited with code {128+n}` with `details.exitCode`, and a guest backend's `Signaled(None)` with pi's `Command terminated without an exit code`; the user `!` bash reports `128+n` too (pi runs it on the same local operations). Verify: `tests/tools.rs` `bash_signal_killed_command_fails_with_128_plus_signo` (137 / 139; red with the old success mapping). |
...
**Kind** upstream-drift · **Severity** high · **Effort** S · **Confidence** confirmed (both sides read 2026-09-24; not run) · **CLOSED 2026-09-24** (`6a3f162`): `ExitStatus::Signaled` now carries the signal number (`ops/local/proc.rs::exit_from`); `bash`/`powershell` fail a `Signaled(Some(n))` as `Command exited with code {128+n}` with `details.exitCode`, and a guest backend's `Signaled(None)` with pi's `Command terminated without an exit code`; the user `!` bash reports `128+n` too (pi runs it on the same local operations). Verify: `tests/tools.rs` `bash_signal_killed_command_fails_with_128_plus_signo` (137 / 139; red with the old success mapping).
...
> (`crates/cyrup-ext/wit/world.wit:128`) and `guest.prepare-arguments: func(name, args-json) ->
...
> `cyrup-ext/src/registry.rs:39`); `WasmTool::prepare_arguments` (`live.rs::prepare_arguments`, `:2329`) round-trips
...
> (`crates/cyrup-agent/src/agent/run/tools/preflight.rs:40-49`), the order pi fixes in
...
> `WasmTool::render_kind` (`live.rs:2303-2308`, `Some("self")` ⇒ `SelfRendered`, anything else ⇒
> `Default` — upstream's own literal test). The CONSUMER that this row, `TOOL-015` and `EXT-024`
> were all blocked on landed in **`75532cee`** (2026-09-04, branch `claude/beautiful-feynman-odz1v5`; `git merge-base --is-ancestor 75532cee 3e9633c4` succeeds, so it reached `main` before `claude/parity-batch3` was cut and is NOT this branch's work):
...
> memoizes it per tool name (`cyrup-tui/src/app/state.rs:384-397`, filled by the live tool-start
> lookup at `app/events.rs:289-297` and the bind-time refresh at `app/session_bind.rs:144-159`),
...
>
> **RE-VERIFIED AT HEAD `68bbd39`, 2026-08-15 — BLOCKED, with the size measured and TWO recorded
> facts corrected. Do not schedule this as "add a branch in cyrup-tui".**
>
> *(1) The producer half is MORE complete than the rows say, and the WIT is no longer the blocker.*
> `crates/cyrup-ext/wit/world.wit` is at `cyrup:ext@0.8.0` and `tool-descriptor` carries
> `render-shell`, `prepare-arguments` and `constrained-sampling`; `impl Tool for WasmTool` overrides
> `prompt_guidelines` (`host/live.rs:1914`), `render_kind` (`:1922-1927`, mapping `Some("self")`)
> and `prepare_arguments` (`:1949+`, with a real guest round trip). The body above still describes
> the 8-field record and "overrides neither" — **that text is stale; only its CONSUMER paragraph
> still holds.**
>
> *(2) The consumer is a LAYOUT change, not a lookup plus a tint — this is the correction that
> matters.* Sweep 8 framed the residual as "give the value a consumer". Read at v0.83.0,
> `ToolExecutionComponent` adds **either** `contentBox = new Box(1, 1, bg)` **or**
> `selfRenderContainer = new Container()` (`modes/interactive/components/tool-execution.ts:67-75`,
> chosen at `:71-75` from `getRenderShell()` at `:105-113`). The two differ in **padding and
> gutter**, not only in background: a self-rendered tool gets *no* Box at all. cyrup's
> `tool_lines` (`cyrup-tui/src/transcript.rs:1312`) always calls
> `finalize_block(block, width, bg)`, whose padding rows and one-column gutter are themselves
> asserted (`transcript.rs:3765` `tool_block_has_tinted_padding_rows_and_a_gutter_on_both_sides`).
> So the fix must split `finalize_block` into a framed and an unframed form and decide what
...
>
> *(4) Why it was not landed here.* It is a visual-layout change to the live transcript, and this
> project's own standing rule is that TUI work is not done until it has been RUN in a real
> terminal — `TestBackend` assertions do not catch assembled-layout regressions. A tools-area agent
> with a check-only gate cannot discharge that. Route to whoever can run the TUI, together with
> `TOOL-015`.
>
...
## ~~TOOL-042~~ — ~~An intermittent nextest `LEAK` fails the `cyrup-tools` suite~~ **CLOSED 2026-09-04 — NOT REPRODUCIBLE ON LINUX** (was: REOPENED 2026-08-14 (sweep 8); PARTIALLY CLOSED)

**Kind** test-defect · **Severity** medium · **Effort** M · **Confidence** confirmed by measurement · **Filed** 2026-08-14 (sweep 6, as closed) · **REOPENED** 2026-08-14 (sweep 8) · **CLOSED** 2026-09-04

> **CLOSED 2026-09-04 — NOT REPRODUCIBLE ON LINUX. The row's own bar was "name a holder or name the harness"; this pass ran the budgeted ≥300 block and names the harness (clean).**
>
> | block | shape | tripwire | runs | LEAK | FAIL |
> |---|---|---|---:|---:|---:|
> | C1 + C2 | two CONCURRENT loops over the archived `cyrup-tools` test binaries (`cargo nextest run --archive-file … --workspace-remap … --no-fail-fast`), 371 tests per run, 10 ms `/proc` sampler running throughout, load 5–7 on 4 vCPU | **100 ms** (`--config-file` copy of `.config/nextest.toml` + `[profile.leak100] leak-timeout = { period = "100ms", result = "fail" }`) | **300** | **0** | **0** |
> | B1 + B2 | main-tree `cargo nextest run -p cyrup-tools --no-fail-fast`, two concurrent loops of two (the four runs the NARROWED note below records) | 100 ms | 4 | 0 | 0 |
> | A | previous pass, sequential, detached worktree of `1139b1d9` | 500 ms (stock) | 34 | 0 | 0 |
> | | | | **338** | **0** | **0** |
>
> Environment: Linux 6.18.44 x86_64 (firecracker, pid 1 is a subreaper), `rustc 1.98.0`, `cargo-nextest 0.9.143` / `nextest-runner 0.122.1`, cyrup HEAD `0c4d5bf1` (binaries archived from that HEAD; `git diff --stat 0c4d5bf1..b36c72e3 -- crates/cyrup-tools .config/nextest.toml` is empty — the commits that landed during and after the block touched only `docs/` and `cyrup-ext-subagents`). The two concurrent loops were chosen over one sequential loop because sweep 8's only under-load event came from its concurrent block C, and the 100 ms tripwire because any holder short enough to slip 500 ms only sometimes should trip 100 ms often — a strictly more sensitive instrument than the gate itself.
>
> **The harness, named from source.** `nextest-runner 0.122.1 src/runner/executor.rs:1482-1551` (`detect_fd_leaks`): `tokio::select! { biased; child_acc.fill_buf() …, sleep …, req_rx.recv() …, else => NoLeak }` with the sleep re-created on every iteration — the readers are polled before the timer, so a starved runtime that wakes late still sees EOF first; starvation alone cannot produce `Leaked`. Pipes: `src/test_command/imp.rs:40-92` (`stdin` `Stdio::null()`, `stdout`/`stderr` `Stdio::piped()`) → std `anon_pipe` → `pipe2(O_CLOEXEC)` on Linux (`rust-lang/rust` `library/std/src/sys/pipe/unix.rs:7-29`: `linux` is in the `pipe2` list; the fallback `pipe()` + `set_cloexec()` is what macOS gets). Each test runs in its own process group (`src/runner/unix.rs:32`). Consequence on Linux: the ONLY way a test can be reported `LEAK` is a real descendant of that same test holding its fd 1/2 for ≥ `leak-timeout`; the crate's `Command::new(` sites all pin all three handles (`ops/shell.rs:102,:503`; `ops/local/command.rs:16,:71`; `ops/local/signal.rs:42,:79,:151`; `ops/local/tests/signal.rs:13` at HEAD) and std's `Command` `dup2`s the piped handles over fd 0/1/2 before `exec`, so bash-tool descendants hold the TOOL's pipes, never the harness's — which is also what sweep 8's "69 orphan pipes vs 244 `cargo-nextest` pipes, zero intersection" was showing.
>
> **Why the 2026-08-14 numbers and today's disagree.** Sweep 8 measured BEFORE `2daaa32d` (2026-08-27). That commit's `SleeperMarker` doc (`crates/cyrup-tools/src/ops/local/tests/mod.rs:40-79`) supplies the mechanism the row lacked: on macOS std's `pipe(2)` and `ioctl(FIOCLEX)` are two calls, nextest creates every test's pipes from a thread pool while spawning other tests, and a spawn landing in that window inherits another test's WRITE end above fd 2; a fixture's surviving `sleep 1` grandchild then holds it past 500 ms and the victim is whichever test owned the pipe — usually one that spawns nothing (sweep 8's block E named an in-memory `RecordingProc` test; `2daaa32d` recorded 16 leaks in ~120 runs naming nine non-forking victims before its fix). The fix reaps the sleeper before the fixture returns (`SleeperMarker::reap`, `mod.rs:115-126`, a `SIGKILL` plus a bounded wait for the pid to be GONE, since `kill(2)` only queues), used by `exec_argv_timeout_preserves_the_real_code_of_a_graceful_sigterm_handler`, `exec_argv_timeout_escalates_to_sigkill_when_the_child_ignores_sigterm` and `exec_argv_cancel_escalates_to_sigkill_when_the_child_ignores_sigterm` (`src/ops/local/tests/exec_argv.rs:95,130,164`), and is pinned by `fixture_scripts_that_fork_a_sleeper_record_its_pid_so_the_fixture_can_reap_it` (`src/tests/no_inherited_harness_stdio.rs:184`) next to sweep 6's `every_command_in_this_crate_pins_all_three_stdio_handles` (`:79`) and `shell_probe_loops_reap_on_the_error_arm_not_just_the_deadline` (`:259`). Those three pins are the closure. The NARROWED note below (written mid-block, at 110 runs) already records both facts; the C1/C2 loops it left running reached 150 runs each with `DONE` and no LEAK/FAIL line, which is what closes the row. Sweep 8's platform is not recorded in the row; the ledger's own repro box is macOS arm64 (`REPRO-LOG.md:375`), which is where that mechanism lives.
>
> **Nothing weakened:** `.config/nextest.toml` is byte-identical (the 100 ms profile lived in a scratch copy passed via `--config-file`), no `#[ignore]`, `retries = 0` untouched, no test changed.
>
> **Falsification condition — what reopens this row.** A single `LEAK` line from `cargo nextest run -p cyrup-tools` at or after `2daaa32d`, on Linux at any load or on macOS, reopens it at medium. The reopening note must name the holder, not the rate: run the suite under a ~10 ms sampler that records, for every process, which pipe inode fd 1 and fd 2 point at (`/proc/<pid>/fd/{1,2}` → `pipe:[ino]`) plus every pipe fd of `cargo-nextest`; timestamp the nextest output; at the LEAK line, take the named test's process (argv contains `--exact <test>`), list every process that ever held its fd 1/2 inodes, and list every `cargo-nextest` read-end that closed within [−0.8 s, +0.3 s] of the line. A holder named → fix it at source in `crates/cyrup-tools`; no holder on Linux → the harness has changed, re-read `detect_fd_leaks` at the nextest version in use. Use `lsof -p`/`/proc`, never `pgrep -f` (see the sweep-6 caveat below).
>
> **RESIDUAL (low, stated):** macOS was not re-measured this pass (no macOS box in this environment). Its closure rests on `2daaa32d`'s reap + pin rather than a fresh run count; the falsification condition above covers it.
>
> **Everything below is the 2026-08-14 state, retained unedited.**

> **NARROWED 2026-09-04 (measurement pass, cut short by budget — NOT closed; counts recorded so nobody re-derives them):** the budgeted block was started and the row's instrument finally ran: **110 runs of `cargo nextest run` over the archived `cyrup-tools` test binaries (371 tests each, `--no-fail-fast`, retries 0) as TWO CONCURRENT loops (sweep 8's block-C shape) with the tripwire AMPLIFIED to `leak-timeout = 100ms` via a `--config-file` scratch copy of `.config/nextest.toml` (the committed 500 ms is untouched), on Linux 6.18.44 x86_64 / 4 vCPU at load 5–7, with a 10 ms `/proc` sampler recording every pipe held on fd 1/2 by every process throughout — 0 LEAK, 0 FAIL, 371/371 every run**; plus 4 amplified main-tree runs (0 LEAK) and the prior pass's 34 sequential runs at the stock 500 ms on `1139b1d9` (0 LEAK). Toolchain `rustc 1.98.0`, `cargo-nextest 0.9.143` / `nextest-runner 0.122.1`, cyrup HEAD `0c4d5bf1`. Two facts the row was missing, both load-bearing for whoever finishes the ≥300 block: (1) `2daaa32d` (2026-08-27, AFTER sweep 8's 2026-08-14 numbers) diagnosed the cross-test mechanism — std's non-atomic `pipe(2)`+`FIOCLEX` on macOS lets a concurrently spawned test inherit another test's pipe write end above fd 2, and a fixture's surviving `sleep 1` grandchild holds it past 500 ms naming an arbitrary NON-forking victim, exactly sweep 8's block-E shape — removed the amplifier (`SleeperMarker::reap`, `crates/cyrup-tools/src/ops/local/tests/mod.rs:40-127`, used at `tests/exec_argv.rs:95,130,164`) and pinned it (`fixture_scripts_that_fork_a_sleeper_record_its_pid_so_the_fixture_can_reap_it`, `src/tests/no_inherited_harness_stdio.rs:184`); (2) the harness read from source is clean on Linux: `nextest-runner 0.122.1 src/runner/executor.rs:1482-1551` `detect_fd_leaks` is a `biased` `select!` polling the stdout/stderr readers BEFORE a per-iteration `tokio::time::sleep(leak_timeout)`, so starvation cannot manufacture a LEAK, and the test pipes are `pipe2(O_CLOEXEC)` on Linux (`rust-lang/rust` `library/std/src/sys/pipe/unix.rs:7-29`; macOS takes the `pipe()`+`set_cloexec()` fallback), so on Linux a LEAK requires a real descendant of the NAMED test holding its fd 1/2 for ≥ `leak-timeout` — the spawn shape `every_command_in_this_crate_pins_all_three_stdio_handles` (`:79`) forbids. **Closure recipe (≈25 min at ~20 runs/min):** `cargo nextest archive -p cyrup-tools --archive-file A.tar.zst`, then two concurrent loops of `cargo nextest run --archive-file A.tar.zst --workspace-remap <repo> --no-fail-fast --config-file <copy with [profile.leak100] leak-timeout = { period = "100ms", result = "fail" }> --profile leak100` until ≥300 total, under a ~10 ms sampler of `/proc/<pid>/fd/{1,2}` → `pipe:[ino]` for every process plus every pipe fd of `cargo-nextest`; on any LEAK line, list every process that ever held the named test's (`--exact <test>` argv) fd 1/2 inodes and every `cargo-nextest` read-end closing within [−0.8 s, +0.3 s] — a holder named closes the row by fixing it at source; zero LEAK at ≥300 closes it as not-reproducible-on-Linux with `2daaa32d`'s reap + the three `no_inherited_harness_stdio.rs` pins as the closure and macOS (not measured here) as a stated low residual.
>
> **Everything below is the 2026-08-14 state, retained unedited.**
...
> **CLOSED 2026-08-15.** Both tags landed in `crates/cyrup-tools/src/tools/bash.rs`'s
> `prompt_guidelines` doc block; neither delta was reverted, as the item asked. The upstream
> offsets below were RE-DERIVED at both tags this pass and are corrected here: the ported-tag
> imperative is **v0.83.0 `bash.ts:330`** and the softened wording is **v0.84.1 `bash.ts:47`**
> (the const hoist moved it). Pinned by `bash_prompt_guideline_deltas_are_tagged_cyrup_delta`.
...
> **CLOSED 2026-08-15 — the decision below was TAKEN, and it went the other way.** `content` is
> ported. The residual paragraph is retained unedited because its cost analysis is correct and a
> later reader is entitled to see what was weighed; what it got wrong is the conclusion. The
> divergence was never *forced* — the size cost is precisely the cost pi pays for the identical
> record — and the port rule permits only forced divergences, so "no reachable consumer" is not a
> ground to keep one. The counter-argument is written into `Truncation`'s doc comment at the source
> so it is inherited rather than re-derived. Superseded text follows.
>
...
> **CLOSED 2026-08-15.** All seven now return `Some(<name>)`. The `Verify` line's first half is
> done and lives in `src/tests/pi_schema.rs`'s `assert_meta` rather than in `pi_tool_semantics.rs`,
> so the assertion runs through the `Arc<dyn Tool>` vtable — the only surface the registry, the
> agent's `ToolDef` build and the prompt builder can reach. Its SECOND half (a `cyrup-ext` wrapper
> assertion that a label DIFFERING from the name survives to the consumer) was already satisfied
> before this pass: `cyrup-ext/src/wrapper.rs:375` asserts `w.label() == Some("Fixed Label")`.
