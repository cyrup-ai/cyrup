...
Fully autonomous development pipeline orchestrator. Executes the full **//flux pipeline** start to finish — pausing only during `/ask` for clarifying questions.
...
- **File path** (`.md`/`.txt`): read with the `read` tool, use full content. Display: `📄 Loaded task from file: <path>`
...
Pass `2`. If QA produces refined tasks, re-run `/exec 2` and `/qa 2` until all tasks clear (max 3 cycles).
