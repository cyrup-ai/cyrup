...
- **Present** → display summary:
  ...
  Use `ask_user_question` — question: "Config looks good. Do you want to update any values?" / header: "Update config" / options: `Yes, update values` | `No, looks good`
...
- `New value - use Other below` → description: `e.g. bun run test:quiet, npm test, pytest -q — or chain with lint: bun run lint && bun run test:quiet`. When chosen, ask a plain-text follow-up for the new value (an empty reply keeps the current value).
