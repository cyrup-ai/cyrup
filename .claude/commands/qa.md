...
- Identify every task file in `$FLUX_BASE/todo/*.md` to delegate
...
Prompt each subagent with the single-task mode steps above as its `task` string, substituting `{{absolute_file_path}}` for that task's file path. Include the stack detection snippet, `$FLUX_BASE` setup, and all constraints (no-git, review criteria).
