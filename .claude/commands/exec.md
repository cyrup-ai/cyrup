...
Use the `subagent` tool — parallel foreground calls only; NEVER background — to execute IN PARALLEL each task in `$FLUX_BASE/todo/*.md`.
...
- Identify every task in `$FLUX_BASE/todo/*.md`
...
- Issue **one** `subagent` call with `tasks: [...]` — one entry per task file, using the subagent prompt template below for each `task` string — and `concurrency: $ARGUMENTS`. The tool's own bounded worker pool runs up to `$ARGUMENTS` tasks at a time and starts the next queued task itself the instant a worker finishes; do NOT loop issuing additional `subagent` calls to "refill" — the call does not return until every task in it has finished, so there is no mid-batch point at which a second call could be issued anyway.

### Subagent prompt template

Use the single-task mode steps above as each subagent's `task` string, substituting `{{absolute_file_path}}` for that task's file path. Include the stack detection snippet and all constraints (no-git, scope guard, no task file modification).
...
Then propose the next step: `/qa` (include arguments if needed).
