...
fn registry_erased_tools_all_carry_a_description_and_snippet() {
    let registry = cyrup_tools::ToolRegistry::with_builtins(
        cwd(),
        Backend::default(),
        cyrup_tools::ToolsOptions::default(),
    );
    let tools = registry.visible(&cyrup_tools::Availability::All);
    assert_eq!(tools.len(), cyrup_tools::BUILTIN_NAMES.len(), "all built-ins visible");
    for t in &tools {
