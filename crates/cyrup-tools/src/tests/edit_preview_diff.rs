...
use crate::tools::edit_diff::compute_edits_diff;
...
async fn failures_carry_the_tools_own_wording() {
    ...
    .unwrap_err();
    assert!(
        noop.starts_with("No changes made to f.txt."),
        "unexpected no-op wording: {noop}"
    );
}
