...
pub struct PromptInputs {
    ...
    /// Tools currently enabled (active set; may change at runtime — R-06-013). Empty = default set
    /// (treated as read-available, mirroring Pi).
...
static DEFAULT_TEMPLATE: PromptTemplate = PromptTemplate {
    ...
    // [CYRUP-DELTA] docs pointer references cyrup docs.
    docs_header: "cyrup documentation (read only when you need it):",
...
impl SystemPromptBuilder {
    ...
    pub fn build(&self, inp: &PromptInputs) -> String {
        ...
        // Empty/unset list => default set => read available (mirror Pi).
        let read_available =
            inp.selected_tools.is_empty() || is_selected(&inp.selected_tools, "read");
    ...
    fn build_default_body(&self, out: &mut String, inp: &PromptInputs) {
        ...
        for c in &inp.tool_contributions {
            if !is_selected(&inp.selected_tools, &c.tool) {
                continue;
            }
        ...
        let has = |n: &str| is_selected(&inp.selected_tools, n);
    ...
    /// Cheap, non-cryptographic fingerprint of the output-affecting inputs (R-06-017). The agent
    /// caches `(fingerprint -> prompt)` for the session and only rebuilds when it changes.
    pub fn inputs_fingerprint(&self, inp: &PromptInputs) -> u64 {
        let mut h = rustc_hash::FxHasher::default();
        opt_str_hash(&mut h, &inp.custom_prompt);
        // selected_tools order is part of identity for tool listing, but membership is what gates;
        // hash a sorted view so a mere reorder doesn't force a rebuild.
        let mut tools: Vec<&str> = inp.selected_tools.iter().map(|s| &**s).collect();
        tools.sort_unstable();
        tools.hash(&mut h);
        for c in &inp.tool_contributions {
            c.tool.hash(&mut h);
...
/// A tool is selected if the active set is empty (default set) or names it.
fn is_selected(selected: &[Arc<str>], name: &str) -> bool {
    selected.is_empty() || selected.iter().any(|t| &**t == name)
}
...
fn emit_docs_section(out: &mut String, t: &PromptTemplate, docs: &DocsPointers) {
    ...
    if let Some(p) = &docs.readme {
        emit_docs_line(out, "README", p);
    }
    if let Some(p) = &docs.docs {
        emit_docs_line(out, "docs", p);
    }
    if let Some(p) = &docs.examples {
        emit_docs_line(out, "examples", p);
    }
}

fn emit_docs_line(out: &mut String, label: &str, p: &Path) {
    out.push_str("- ");
    out.push_str(label);
    out.push_str(": ");
