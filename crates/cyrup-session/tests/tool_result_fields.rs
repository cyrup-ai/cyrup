    ...
    let recovered: Vec<Message> = reimported
        ...
        .filter_map(|e| match e {
            cyrup_session::Entry::Known(cyrup_session::KnownEntry::Message {
                message: cyrup_session::agent_message::AgentMessage::Core(c),
                ..
            }) => Some(c.clone()),
            _ => None,
        })
...
fn new_file_read_by_pre_change_shape_stays_a_known_entry() {
    ...
    let unknown = reimported
        .entries()
        .iter()
        .filter(|e| matches!(e, cyrup_session::Entry::Unknown(_)))
        .count();
    ...
    let tr = reimported
        ...
        .find_map(|e| match e {
            cyrup_session::Entry::Known(cyrup_session::KnownEntry::Message {
                message: cyrup_session::agent_message::AgentMessage::Core(
