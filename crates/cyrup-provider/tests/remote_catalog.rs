...
fn a_none_overlay_is_byte_identical_to_the_pre_drift_007_registry() {
    ...
    let ids = |ps: Vec<Arc<dyn cyrup_provider::Provider>>| -> Vec<(String, Vec<String>)> {
        ps.into_iter()
            .map(|p| {
                (
    ...
    let registry = Arc::new(cyrup_provider::builtin_registry());
    let legacy = ids(cyrup_provider::all_providers_with(
        store.clone(),
        registry.clone(),
    ));
    ...
    let store2: Arc<dyn cyrup_provider::CredentialStore> =
        Arc::new(cyrup_provider::InMemoryCredentialStore::new());
    let empty = CatalogOverlay::default();
    let inert = ids(all_providers_with_overlay(
        store2,
        Arc::new(cyrup_provider::builtin_registry()),
        Some(&empty),
    ));
...
fn the_builtin_manifest_stamp_agrees_with_the_documented_provenance() {
    ...
    assert!(cyrup_provider::BUILTIN_CATALOG_MANIFEST_JSON.contains("91585d9a"));
}
