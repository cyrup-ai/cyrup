...
/// Locate the workspace root from this crate's manifest dir (`crates/cyrup-provider`).
fn workspace_root() -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("crates/cyrup-provider is two levels below the workspace root")
        .to_path_buf()
}
...
fn cargo_tree(extra: &[&str]) -> Result<String, String> {
    ...
    // `tree` only resolves, it does not build, so this takes no target-dir build lock.
    let out = cmd
        .output()
