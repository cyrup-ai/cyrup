...
//! matching pi's `getArtifactPaths` (`shared/artifacts.ts:186-196`). The four filenames and the
...
//! (`shared/artifacts.ts:229-258`, a `.last-cleanup` marker gates re-scanning to once per day and
//! deletes any artifact older than `max_age_days`), [`cleanup_all_artifact_dirs`] fans that sweep
//! across the temp + per-session artifact roots (`shared/artifacts.ts:260-284`), and
//! [`cleanup_old_chain_dirs`] is pi's separate 24h chain-runs sweep (`shared/settings.ts:195-218`).
...
/// The default artifact-cleanup horizon (pi `DEFAULT_ARTIFACT_CONFIG.cleanupDays`, `shared/types.ts:899`).
pub const DEFAULT_CLEANUP_DAYS: u64 = 7;

/// The four artifact paths for one run/agent/index (pi `ArtifactPaths`, `shared/types.ts:464-469`).
///
...
/// `shared/types.ts:471-478`). The [`Default`] impl reproduces pi's `DEFAULT_ARTIFACT_CONFIG`
/// (`shared/types.ts:893-900`) exactly: input/output/metadata on, **jsonl off**, 7-day cleanup.
///
...
/// (`runs/background/subagent-runner.ts:1630-1666,1117-1133`). Every field is `#[serde(default)]`ed
...
impl Default for ArtifactConfig {
    fn default() -> Self {
        // pi `DEFAULT_ARTIFACT_CONFIG` (`shared/types.ts:893-900`).
        Self {
            enabled: true,
            include_input: true,
            include_output: true,
...
/// (`shared/types.ts:961`), keyed per-`cwd` under the SAME `<home>/.cyrup/subagents` root the async/results
...
/// Resolve the artifacts directory for a run (pi `getArtifactsDir`, `shared/artifacts.ts:145-184`):
...
/// The four artifact paths for one run (pi `getArtifactPaths`, `shared/artifacts.ts:186-196`): base is
...
/// Write one artifact file's UTF-8 content (pi `writeArtifact`, `shared/artifacts.ts:202-205`).
///
/// # Errors
...
/// Write a pretty-printed JSON metadata file (pi `writeMetadata`, `shared/artifacts.ts:207-223`,
...
/// `cleanupOldArtifacts`, `shared/artifacts.ts:229-258`).
///
...
/// `shared/artifacts.ts:260-284`): the scoped temp artifacts root for `cwd`, plus each persisted
...
/// `shared/settings.ts:195-218`). Unlike [`cleanup_old_artifacts`] this is unthrottled and operates
