...
impl McpDirs {
    ...
    /// `getAgentDir()` (`CYRUP_AGENT_DIR`/`CYRUP_CODING_AGENT_DIR` with `~` expansion, else
...
mod tests {
    ...
    use super::*;
    // Promoted to `exec::testsupport` so `tool_surface.rs`'s fold tests can build the same
    // hermetic layout: one definition, two consumers.
    use crate::exec::testsupport::{McpFixture, make_fixture, write_json, write_mcp_fixture};
