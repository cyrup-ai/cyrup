...
// =================================================================================================
// /subagents-companions (pi `companion-suggestions.ts`) — regression coverage for the four
// confirmed divergences: `status` returning a hardcoded string instead of the per-package doctor
// report; `hide` persisting an unkeyed marker file nothing reads back instead of the config's
// `companionSuggestions` structure; `show` wiping every scope/workspace instead of only the current
// workspace (preserving other workspaces' dismissals); and `hide`/`show`'s reply text not matching
// pi's exact strings.
// =================================================================================================

/// pi `buildCompanionCommandStatus`/`buildCompanionDoctorLines` (`companion-suggestions.ts:229-242,
/// 324-326`): bare `/subagents-companions` (and `status`) must render the "Companion packages"
/// header followed by a per-package report for BOTH `pi-intercom` and `pi-prompt-template-model`,
/// in that order — never the old fixed "nothing to report" string, which this assertion would fail
/// against (that string contains neither "Companion packages" nor either package name formatted as
/// a doctor-report line).
#[tokio::test]
async fn subagents_companions_status_reports_the_per_package_doctor_lines() {
    let _guard = ENV_MUTATION_LOCK.lock().await;
    let home = tempfile::tempdir().expect("home tempdir");
    // SAFETY: scoped, mutex-serialized env mutation — see this file's module doc.
    unsafe {
    ...
    let ctx = HostCtx::command(ExtMode::Tui, false, dir.path().to_path_buf());

    let out = ext
        .execute_command("subagents-companions", "", &ctx)
        .await
        .expect("status must not error")
        .expect("status returns text");

    assert!(out.starts_with("Companion packages"), "got: {out:?}");
    assert!(out.contains("- pi-intercom: inactive"), "got: {out:?}");
    assert!(out.contains("- pi-prompt-template-model: inactive"), "got: {out:?}");
    assert!(out.contains("install: pi install npm:pi-intercom"), "got: {out:?}");
    assert!(
        out.contains("install: pi install npm:pi-prompt-template-model"),
        "got: {out:?}"
    );
...
/// pi `updateCompanionDismissal`'s user/workspace hide branches plus `handleCompanionCommand`'s
/// exact reply strings (`companion-suggestions.ts:290-322, 347-350`): `hide <pkg> user`/`hide <pkg>
/// workspace` must reply with pi's literal "Hid ... for this user."/"Hid ... for this workspace."
/// text, and the dismissal must land in `status`'s own `... recommendation hidden by config` line —
/// proving the mutation goes through the SAME store `status` reads back, not an inert side file.
#[tokio::test]
async fn subagents_companions_hide_replies_and_suppresses_via_the_shared_config_store() {
    let _guard = ENV_MUTATION_LOCK.lock().await;
    let home = tempfile::tempdir().expect("home tempdir");
    // SAFETY: scoped, mutex-serialized env mutation — see this file's module doc.
    unsafe {
    ...
    let hide_user = ext
        .execute_command("subagents-companions", "hide pi-intercom user", &ctx)
        .await
        .expect("hide must not error")
        .expect("hide returns text");
    assert_eq!(hide_user, "Hid pi-intercom recommendations for this user.");

    let hide_workspace = ext
        .execute_command(
            "subagents-companions",
            "hide pi-prompt-template-model workspace",
            &ctx,
        )
        .await
        .expect("hide must not error")
    ...
    assert_eq!(
        hide_workspace,
        "Hid pi-prompt-template-model recommendations for this workspace."
    );
    ...
    let status = ext
        ...
        .expect("status returns text");
    assert!(
        status.contains("- pi-intercom: inactive recommendation hidden by config"),
        "user-scope hide must be visible to a same-process status call, got: {status:?}"
    );
    assert!(
        status.contains(
            "- pi-prompt-template-model: inactive recommendation hidden by config"
        ),
        "workspace-scope hide must be visible to a same-process status call, got: {status:?}"
    );

    // The on-disk store `hide` wrote to is the SAME `config.json` `status`'s gating reads back
    // (pi's `updateConfig`/`isDismissed` share one file) — not an unkeyed marker file.
    let config_path = home.path().join(".cyrup").join("subagents").join("config.json");
    let on_disk = std::fs::read_to_string(&config_path).expect("config.json must exist");
    assert!(on_disk.contains("companionSuggestions"), "got: {on_disk}");
    assert!(on_disk.contains("\"user\": true"), "got: {on_disk}");
...
/// pi `show`'s exact semantics (`companion-suggestions.ts:299-307, 340-342`): `show <pkg>` clears
/// ONLY the current workspace's dismissal (plus the user-scope flag) and always replies "Showing
/// ... for this workspace again." — a dismissal recorded for a DIFFERENT workspace key must survive
/// untouched. Before the fix, `show` deleted BOTH the `workspace`- and `user`-scope marker files
/// unconditionally (the markers carried no workspace key at all), which would have wiped the other
/// workspace's dismissal too; this test's final assertion is the regression proof.
#[tokio::test]
async fn subagents_companions_show_clears_only_the_current_workspace_and_keeps_others() {
    let _guard = ENV_MUTATION_LOCK.lock().await;
    let home = tempfile::tempdir().expect("home tempdir");
    // SAFETY: scoped, mutex-serialized env mutation — see this file's module doc.
    unsafe {
    ...
    // Seed the shared config store with a pre-existing dismissal for an UNRELATED workspace key,
    // plus a user-scope dismissal, both of which `show` must leave untouched.
    let config_dir = home.path().join(".cyrup").join("subagents");
...
"#,
    ...
    // Hide for THIS workspace too, so both this workspace and the seeded "other" workspace are
    // dismissed before `show` runs.
    ext.execute_command("subagents-companions", "hide pi-intercom workspace", &ctx)
        .await
        .expect("hide must not error");
    ...
    let show = ext
        ...
        .await
        .expect("show must not error")
        .expect("show returns text");
    assert_eq!(show, "Showing pi-intercom recommendations for this workspace again.");

    let on_disk = std::fs::read_to_string(&config_path).expect("config.json must exist");
    assert!(
        on_disk.contains("/some/other/workspace"),
        "show must preserve a dismissal recorded for a DIFFERENT workspace key, got: {on_disk}"
    );
    assert!(
        !on_disk.contains("\"user\": true"),
        "show must clear the user-scope dismissal flag, got: {on_disk}"
    );
    ...
    let status = ext
        ...
        .expect("status returns text");
    assert!(
        status.contains("- pi-intercom: inactive") && !status.contains("hidden by config"),
        "this workspace's dismissal must be cleared by `show`, got: {status:?}"
    );
