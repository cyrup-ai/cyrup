...
// MIGRATION — THE REASON THIS FILE IS IN `cyrup-it` AND NOT IN `cyrup-session-svc/src/tests/`.
//
// Nothing below spawns a process or loads a wasm guest, so by §9.1 these four tests belong in the
// source crate's `#[cfg(test)]` modules. They cannot go there: the version of this file that lived
// in `tests/` established its precondition with a `ScrubbedProviderEnv` guard that called
// `unsafe { std::env::remove_var(..) }` / `set_var(..)` under a file-local `ENV_SCRUB_LOCK`, and
// `crates/cyrup-session-svc/src/lib.rs:17` is `#![forbid(unsafe_code)]`. Its soundness argument was
// also only ever true of a ONE-FILE-ONE-PROCESS harness: the lock is file-local, so the moment the
// file shares a binary with siblings it guards nothing, which is precisely what a consolidated test
// binary is (docs/TEST-ARCHITECTURE.md §4 R2 — 33 such per-file mutexes in the old corpus).
//
// So the guard is GONE and the precondition is now ASSERTED instead of manufactured, via §4 R5's
// layer-3 check in `support::env`. `available_model_catalog()` reaches the process environment
// through `provider_is_configured(.., env: None)` (cyrup-config model.rs:1796-1804 → `env_keys`),
// and that `None` is not injectable from a test — so "openai/anthropic are unconfigured" is a
// property of the environment the suite runs in, and the only non-`unsafe` way to have it is to
// require it and say so loudly when it is missing. Pi has the same requirement and meets it the
// same way, by running its whole suite under `env -i` with an allowlist (`pi/test.sh`).
//
// DELIBERATE BEHAVIOUR CHANGE, recorded rather than hidden: a developer with `ANTHROPIC_API_KEY`
// exported used to get a silent pass here (the guard removed it for the duration); they now get a
// named red naming the variable. The four assertions themselves are byte-for-byte unchanged.
