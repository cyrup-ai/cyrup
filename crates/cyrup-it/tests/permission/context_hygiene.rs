...
use cyrup_permission_system::{EXTENSION_ID, PermissionSystemExtension};
...
async fn denied_tool_is_stripped_from_prompt_and_active_tool_set() {
    ...
    let out = ext
        .on_event(&before_agent_start(system_prompt), &ctx(agent_dir))
        .await;

    // The handler RETURNED the sanitized prompt as a [mutate].
    let mutated = mutate_system_prompt(&out).unwrap_or_else(|| {
        panic!("before_agent_start must return a system-prompt mutate; got {out:?}")
    });

    // The whole "Available tools:" section is gone, and the denied `write` guideline bullet is stripped.
    assert!(
        !mutated.contains("Available tools:"),
        "tools section stripped:\n{mutated}"
    );
    assert!(
    ...
    assert!(
        mutated.contains("use read to examine files"),
        "allowed guideline kept:\n{mutated}"
    );
    ...
    assert_eq!(
        rec.active_tools.len(),
        1,
        "setActiveTools called exactly once"
    );
...
async fn deny_skill_hidden_from_prompt_but_still_gates_skill_read() {
    ...
    assert!(
        mutated.contains("<name>deploy</name>"),
        "allowed skill advertised:\n{mutated}"
    );
    assert!(
    ...
    let read_secret = ext
        .on_event(
            &tool_call("read", json!({ "path": secret_file })),
            &ctx(agent_dir),
        )
        .await;
    assert!(
        block_reason(&read_secret)
            .is_some_and(|r| r.contains("not permitted to access this skill")),
    ...
    let read_deploy = ext
        .on_event(
            &tool_call("read", json!({ "path": deploy_file })),
            &ctx(agent_dir),
        )
        .await;
    assert!(
...
async fn yolo_status_pill_set_on_start_and_cleared_on_shutdown() {
    ...
    write(
        &agent_dir.join("cyrup-permission-system/config.json"),
        r#"{ "yoloMode": true }"#,
    );
