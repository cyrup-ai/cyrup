...
impl HostServices for SharedSink {
    ...
    fn select(&self, _prompt: &str, _options: &Value, _opts: &DialogOptions) -> Option<String> {
        ...
        self.prompts
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .push(("permission-select", start, end));
...
async fn permission_ask_holding_the_shared_lock_blocks_the_intercom_clarify_prompt() {
    ...
    let permission_ext = Arc::new(PermissionSystemExtension::new(
    ...
    assert!(
        Arc::ptr_eq(
            &sink.human_interaction_lock().expect("sink lock"),
            &human_lock
        ),
    ...
    recv_signal(
        &mut append_rx,
        "orchestrator inbound surface of the child ask",
    )
    .await;
    ...
    assert!(
        !clarify_task.is_finished(),
        "the clarify is still WAITING on the shared lock, not resolved"
    );
    ...
    assert_eq!(
        ...
        "the clarify surfaced only AFTER the permission lock freed"
    );
    assert!(
        input_started.load(Ordering::SeqCst),
        "the intercom input prompt fired once the lock freed"
    );
    ...
    assert_eq!(
        prompts.len(),
        2,
        "exactly two human prompts occurred: {prompts:?}"
    );
    let (sel_label, _sel_start, sel_end) = prompts[0];
    let (inp_label, inp_start, _inp_end) = prompts[1];
    assert_eq!(
        sel_label, "permission-select",
        "the permission dialog prompted first"
    );
    assert_eq!(
        inp_label, "intercom-input",
        "the intercom clarify prompted second"
    );
    assert!(
        inp_start >= sel_end,
