...
async fn live_guest_provider_oauth_stream_autocomplete_activetools() {
    ...
    let responses = CannedResponses {
        oauth_prompt: Some("code123".into()),
        ..Default::default()
    };
    ...
    assert!(
        reg.has_stream_simple(),
        "provider supplies a custom streamSimple"
    );
    ...
    let creds = ext
        .provider_login("demo-oauth")
        .await
        .expect("login flow runs");
    assert_eq!(
        creds["access"],
        json!("a-code123"),
        "credentials carry the prompted code"
    );
    ...
    let key = ext
        .provider_get_api_key("demo-oauth", &creds)
        .await
        .expect("get api key");
    ...
    let out = ext
        .provider_modify_models("demo-oauth", &models, &creds)
        .await
        .expect("modify models");
    ...
    assert_eq!(
        stream_events.len(),
        2,
        "guest streamed two events: {stream_events:?}"
    );
    ...
    let suggestions = ext
        .autocomplete_suggest(Some(&base), &query)
        .await
        .expect("autocomplete");
    ...
    assert_eq!(
        suggestions["prefix"],
        json!("dem"),
        "guest used the cursor line as the prefix"
    );
    ...
    let out = host
        .run_command("planmode", "", &cancel)
        .await
        .expect("planmode runs");
    assert_eq!(out.as_deref(), Some("active tools: read"));
    assert_eq!(
        ext.guest().active_tools_restriction().as_deref(),
        Some(&["read".to_string()][..])
    );
    ...
    let probe = {
        ...
        active
            .into_iter()
            .find(|t| t.name() == "signal_probe")
            .expect("signal_probe surfaced")
    };
    ...
    let out = host
        .run_command("signaldemo", "", &cancel)
        .await
        .expect("signaldemo runs");
    assert_eq!(
        out.as_deref(),
        Some("confirmed: false"),
        "dialog bound to an aborted signal cancels"
    );
    assert!(
        ext.guest()
            .aborted_signals()
    ...
    //     export ran).
    let out = host
        .run_command("confirmdemo", "", &cancel)
        .await
        .expect("confirmdemo runs");
    assert_eq!(
        out.as_deref(),
        Some("confirmed: true"),
        "confirmdemo's dialog is not dismissed"
    );
    assert!(
        rec.confirm_messages()
            .iter()
            .any(|m| m == "this is the message body, distinct from the title"),
    ...
    let out = host
        .run_command("inputdemo", "", &cancel)
        .await
        .expect("inputdemo runs");
    assert!(
        out.as_deref().unwrap_or("").starts_with("input:"),
        "inputdemo ran: {out:?}"
    );
    assert!(
        rec.input_placeholders()
            .iter()
            .any(|p| p.as_deref() == Some("e.g. Ada Lovelace")),
    ...
    let out = host
        .run_command("withsessiondemo", "", &cancel)
        .await
        .expect("withsessiondemo runs");
    ...
    assert!(
        rec.control_ops()
            .iter()
            .any(|op| matches!(op, cyrup_ext::ControlOp::NewSession { .. })),
