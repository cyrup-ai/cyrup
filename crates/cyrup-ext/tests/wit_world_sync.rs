...
fn the_cache_key_tracks_the_wit_and_sdk_sources_outside_the_extension_crate() {
    ...
    let files = cyrup_ext::build::abi::abi_source_files(&crates_dir);
    ...
    let recomputed = cyrup_ext::build::abi::hash_abi_sources(&crates_dir);
    assert_eq!(recomputed.len(), 64, "blake3 hex is 64 chars");
    assert_eq!(
        cyrup_ext::build::ABI_FINGERPRINT,
        recomputed,
    ...
    let id = cyrup_ext::build::world_abi_id();
    assert!(id.starts_with(cyrup_ext::HOST_WORLD), "the world identity leads with HOST_WORLD: {id}");
