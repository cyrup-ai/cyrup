...
/// The built demo component. Same contract as `tests/discover_load.rs`: honor a prebuilt artifact
/// from the environment, otherwise build the guest crate for `wasm32-wasip2`. Built at most once per
/// test binary.
fn component_bytes() -> &'static [u8] {
    static BYTES: OnceLock<Vec<u8>> = OnceLock::new();
    BYTES.get_or_init(|| {
        let path = if let Ok(p) = std::env::var("CYRUP_EXT_FIXTURE_COMPONENT") {
            PathBuf::from(p)
        } else {
            ...
            target_dir.join("wasm32-wasip2/debug/cyrup_ext_sdk.wasm")
        };
