...
fn engine_builds_with_epoch_and_pooling() {
    testkit::engine_builds()
        .expect("production engine builds (component-model+async+epoch+pooling)");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn epoch_preemption_surfaces_timeout_host_alive() {
    let contained = testkit::epoch_preemption_demo()
        .await
        .expect("host returned (alive)");
    assert!(
...
fn guest_state() -> Arc<GuestState> {
    Arc::new(GuestState::new(
        ExtensionId::from("drop-guard-probe"),
...
fn a_bound_cancelled_token_is_what_the_guest_poll_reads() {
    ...
    assert!(
        !guest.tool_is_cancelled("call-1"),
        "bound but not fired -> not cancelled"
    );

    cancel.cancel();
    assert!(
        guest.tool_is_cancelled("call-1"),
        "bound AND fired -> the guest poll reads true"
    );
    ...
    assert!(
        guest.tool_is_cancelled("some-other-call"),
        "the binding is not keyed by call-id"
    );
}
...
fn the_binding_is_cleared_when_the_guard_leaves_scope() {
    ...
    assert!(
        !guest.tool_is_cancelled("call-1"),
        "cleared when the guard leaves scope"
    );
}
...
async fn the_binding_is_cleared_when_the_call_future_is_dropped_mid_await() {
    ...
    let call = {
        ...
        async move {
            ...
            assert!(
                guest.tool_is_cancelled("call-1"),
                "bound while the call is in flight"
            );
...
fn emitted_update_chunks_land_in_one_instance_wide_queue() {
    ...
    assert_eq!(
        ...
        0,
        "nothing emitted -> nothing queued"
    );
...
fn the_replay_takes_only_this_calls_chunks() {
    ...
    assert_eq!(
        mine[0].get("details").and_then(|v| v.as_str()),
        Some("mine")
    );
    assert_eq!(
        guest.queued_tool_update_count(),
    ...
    let call = {
        ...
        async move {
            ...
            assert_eq!(
                guest.queued_tool_update_count(),
                1,
                "queued while the call is in flight"
            );
            reached.store(true, std::sync::atomic::Ordering::SeqCst);
            pending::<()>().await;
        }
