...
use crate::config::{ServerEntry, ToolPrefix};
...
fn disabled_call_result(
    ...
    match metadata {
        None => {
            map.insert(
                "server".to_string(),
                Value::String(disabled_server.to_string()),
            );
            map.insert(
...
pub async fn execute_call(
    ...
    if server_name.is_none() && tool_meta.is_none() && prefix_mode != ToolPrefix::None {
        ...
        for (candidate, _) in candidates {
            ...
            let per_server = ctx.with_metadata(|metadata| {
                ...
                let exact: Vec<ToolMetadata> = get_tool_matches(&tools, tool_name, true)
                    .into_iter()
                    .cloned()
                    .collect();
                if exact.len() > 1 {
                ...
                let fallback: Vec<ToolMetadata> = get_tool_matches(&tools, tool_name, false)
                    .into_iter()
                    .cloned()
                    .collect();
                if fallback.len() > 1 {
        ...
        let lazy_matches = if lazy_exact.is_empty() {
            lazy_fallback
        } else {
            lazy_exact
        };
    ...
    let (Some(server_name), Some(mut tool_meta)) = (server_name.clone(), tool_meta.clone()) else {
        ...
        let native = if server_override.is_none() {
            ctx.env.all_tool_names().and_then(|names| {
                names
                    .into_iter()
                    .find(|name| name == tool_name && name != MCP_TOOL_NAME)
            })
        } else {
            None
        };
        if let Some(native) = native {
            ...
            return Ok(text_result(
                format!(
                    "\"{native}\" is a native cyrup tool. Call {native} directly instead of using mcp({{ tool: \"{native}\" }})."
                ),
                map,
            ));
        }

        let hint_server = server_name
            .clone()
            .or_else(|| prefix_matched_server.clone());
        let available = hint_server
            .as_ref()
            .map(|server| ctx.tool_names(server))
            .unwrap_or_default();
    ...
    if connection != Some(ConnectionStatus::Connected) {
        ...
        if !ctx.config().mcp_servers.contains_key(&server_name) {
            ...
            spread(&mut map, &identity);
            return Ok(text_result(
                format!("Server \"{server_name}\" not connected"),
                map,
            ));
        }
        ...
        let reconnected: McpResult<Option<ToolResult>> = async {
            ...
            ctx.state
                .notify_tool_metadata_updated(&server_name, "proxy-call-reconnect");
        ...
        .await
    {
        ...
        outcome => {
            ...
            map.insert(
                "tool".to_string(),
                Value::String(tool_meta.original_name.clone()),
            );
            return Ok(text_result(message, map));
        }
    }
...
async fn invoke(
    ...
    if let Some(uri) = tool_meta.resource_uri.as_ref() {
        return match ctx
            .env
            .read_resource(server_name, uri, recovery, owned)
            .await
        {
            Ok(content) => {
                let content = if content.is_empty() {
                    vec![Content::Text {
                        text: "(empty resource)".into(),
                        text_signature: None,
                    }]
                } else {
                    content
                ...
                let guarded = ctx
                    .env
                    .guard_mcp_output(content, OutputGuardOptions::default())
                    .await;
...
async fn catch_arm(
    ...
    match error {
        ProxyCallError::SessionRecoveryAuthRequired { auth_message, .. } => {
            ...
            map.insert("message".to_string(), Value::String(message.clone()));
            map.insert(
                "autoAuthAttempted".to_string(),
                Value::Bool(latch.attempted()),
            );
        ...
        ProxyCallError::UrlElicitationRequired { error } => {
            let action = ctx
                .env
                .handle_url_elicitation_required(server_name, &error)
                .await;
...
mod tests {
    ...
    async fn call_fails_closed_for_duplicate_unqualified_names() {
        ...
        let (ctx, _) = ctx_with(
            ...
            &[
                (
                    "zeta",
                    vec![ToolMetadata::new("create_issue", "create_issue", "")],
                ),
                (
                    "alpha",
                    vec![ToolMetadata::new("create_issue", "create_issue", "")],
    ...
    async fn an_exact_disabled_match_suppresses_the_fuzzy_scan_entirely() {
        ...
        let result = execute_call(&ctx, "do-it", None, None, &CancelToken::new(), None)
            .await
            .unwrap();
        let details = result.details.clone().unwrap();
    ...
    async fn a_same_named_host_tool_is_reported_as_native_and_none_falls_through() {
        ...
        let (ctx, _) = ctx_with(
            ...
            &[],
            &[],
            FakeEnv::default().with_all_tools(&["read", "mcp"]),
        );
        let result = execute_call(&ctx, "read", None, None, &CancelToken::new(), None)
            .await
            .unwrap();
        assert_eq!(
