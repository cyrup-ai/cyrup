...
use crate::proxy::env::{
    ApprovalOrigin, ApprovalOutcome, CallToolOutcome, ConnectOutcome, ConnectionStatus,
    GuardedOutput, OutputGuardOptions, ProxyCallError, ProxyCtx, ProxyEnv, UrlElicitationAction,
};
...
impl ProxyEnv for FakeEnv {
    ...
    async fn connect(&self, server: &str, _cancel: &CancelToken) -> McpResult<ConnectOutcome> {
        Ok(ConnectOutcome {
            status: self.get_connection(server),
            ..ConnectOutcome::default()
        })
    }
    ...
    async fn ensure_tool_call_approved(
        ...
        self.approval
            .lock()
            .unwrap()
            .unwrap_or(ApprovalOutcome::Approved)
    }
...
pub(crate) fn metadata_with(
    servers: &[(&str, Vec<ToolMetadata>)],
) -> IndexMap<String, Vec<ToolMetadata>> {
