...
//!   only the `"script"` call site disappears. The description's `use mcpScript.` sentence is gone,
...
use crate::config::{
    locale_compare, McpConfig, McpSettings, OAuthGrantType, OAuthSetting, ServerEntry, ToolPrefix,
    ToolResultRendering,
};
...
/// `tool-approval.ts`'s `McpToolApprovalOrigin`.
///
/// The `Script` variant is **Cut 4**; the parameter keeps its shape and its `Proxy` default, leaving
/// `Proxy` and the derived `Resource`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ApprovalOrigin {
    /// A `mcp({tool})` call.
    Proxy,
    /// A resource tool, derived when `toolMeta.resourceUri` is set.
    Resource,
}
...
pub async fn execute_call(
    ...
    let resolved_origin = origin.unwrap_or(if tool_meta.resource_uri.is_some() {
...
// ==================================================================================================
// 15 · Conformance (MCP-195, and the unit assertions 13d names per port unit)
// ==================================================================================================
