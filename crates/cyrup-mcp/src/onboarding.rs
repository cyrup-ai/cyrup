...
//! other's rename.
//!
//! # Who reads and writes this at runtime
//!
//! Upstream's consumers are `commands.ts` (`/mcp`) and `mcp-setup-panel.ts`, and both have
//! counterparts here. [`load_onboarding_state`] is read by `crate::commands`' `/mcp` and
//! `/mcp setup` arms, which hand the state to `McpSetupPanelModel` and to
//! [`crate::panel_host::shared_config_notice`]; [`mark_shared_config_hint_shown`] is stamped by the
//! `/mcp` arm once the panel closes, and [`mark_setup_completed`] by
//! [`crate::panel_host::SetupCallbacks`] after every successful write.
...
pub struct OnboardingState {
    ...
    ///
    /// **Written but never compared.** Both stamps carry a fingerprint forward, and nothing reads
    /// it back: re-arming a hint when the fingerprint changes is a comparison upstream makes and
    /// neither `/mcp` arm does, so today the field only round-trips through the file.
...
///
/// Called by `crate::commands`' `/mcp` panel arm once the panel closes, with the fingerprint
/// [`crate::panel_host::shared_config_notice`] handed back — and only when that call actually
/// produced notice lines, so a notice that never rendered is not retired.
