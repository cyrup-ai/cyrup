...
/// gap-07 #2: a package's DECLARED EXTENSION must actually LOAD into the running session (not merely
/// be collected into `ext_crate_paths` and dropped). This drives the REAL assembled session over a
/// prebuilt wasm component and asserts the guest's `/greet` command is registered in the live host —
/// end-to-end proof the package-tier extension path is wired into the loader (Pi
...
mod wasm_ext {
    ...
    use cyrup_resources::{InstalledPackage, InstalledPackages, PinRef};

    /// Build (or locate) the bundled `cyrup-ext-sdk` demo component (its `example.rs` registers the
    /// `/greet` command) — mirrors tests/wasm_slash_command.rs. `CYRUP_EXT_FIXTURE_COMPONENT` skips
    /// the nested build.
