...
async fn session_dag_flattens_a_real_multi_branch_session() {
    ...
    assert!(
        !anchors.is_empty(),
        "expected a user message to branch from"
    );
    let first_user = anchors[0].entry_id.clone();
    session
        .branch(first_user.clone())
        .await
        .expect("branch to first user message");
    ...
    assert!(
        dag.len() >= 4,
        "expected a multi-node DAG, got {}: {dag:#?}",
        dag.len()
    );
    assert!(
        dag.iter().any(|n| n.depth > 0),
        "no nested node — the DAG was flattened to depth 0"
    );
    ...
    let root = dag
        .iter()
        .find(|n| n.entry_id == first_user)
        .expect("first user node present");
    assert!(
        root.foldable,
        "the fork point must be foldable (has >1 child): {dag:#?}"
    );
    // (parent links) non-root nodes carry a parent id.
    assert!(
        dag.iter().any(|n| n.parent_id.is_some()),
        "no parent links recorded in the flat DAG"
    );
    // (leaf) exactly one node is the current branch leaf (the newest assistant reply).
    assert_eq!(
        dag.iter().filter(|n| n.is_leaf).count(),
        1,
        "exactly one leaf expected: {dag:#?}"
    );
    // (kinds/labels) user + assistant messages are classified and role-labeled.
    assert!(
        dag.iter()
            .any(|n| n.kind == SessionDagKind::Message && n.label.starts_with("user:")),
