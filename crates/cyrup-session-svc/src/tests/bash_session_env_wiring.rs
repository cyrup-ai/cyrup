...
async fn probe(
    ...
    faux.set_responses(vec![
        faux_assistant_message(
            vec![faux_tool_call(
                "bash",
                serde_json::json!({ "command": probe_command(out) }),
            )],
            StopReason::ToolUse,
        ),
    ...
    let _ = session
        .prompt(UserInput::text("probe", InputSource::Sdk))
        .await
        .expect("prompt");
...
fn get<'a>(kv: &'a [(String, String)], key: &str) -> &'a str {
    kv.iter()
        .find(|(k, _)| k == key)
        .map(|(_, v)| v.as_str())
        .unwrap_or_else(|| panic!("the probe did not report {key}; got {kv:?}"))
}
...
async fn bash_children_see_this_sessions_metadata_and_track_model_changes() {
    ...
    assert_eq!(
        get(&kv, "CYRUP_PROVIDER"),
        "acme",
        "model change did not reach the child: {kv:?}"
    );
...
async fn a_fork_republishes_the_session_identity_to_bash_children() {
    ...
    assert_ne!(
        forked.to_string(),
        id_before,
        "fixture: the fork must mint a new session id"
    );
