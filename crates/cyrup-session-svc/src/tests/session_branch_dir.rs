...
async fn finding1_clone_stays_in_same_dir_and_is_listed() {
    ...
    assert_eq!(
        ...
        "Finding 1: branch must stay in the same dir"
    );
    ...
    assert!(
        paths.contains(&file_after),
        "Finding 1: branched session must be visible to --resume"
    );
}
...
async fn finding3_explicit_session_dir_is_literal() {
    ...
    assert_eq!(
        dir, custom,
        "Finding 3: explicit --session-dir must be used literally"
    );
}
