...
async fn valid_args_execute_normally() {
    ...
    let (_faux, sf) = faux_stream_fn(vec![
        faux_assistant_message(
            vec![faux_tool_call("int_tool", json!({ "n": 7 }))],
            StopReason::ToolUse,
        ),
    ...
    assert_eq!(
        executed,
        vec![json!({ "n": 7 })],
        "tool executed with valid args"
    );
    assert!(
        tool_result_errors(&new).is_empty(),
        "no error tool-results: {new:?}"
    );
}
...
async fn invalid_args_yield_error_result_seen_next_turn_without_executing() {
    ...
    assert!(
        ...
        "uncoercible args must not execute the tool"
    );
    ...
    assert_eq!(
        faux.call_count(),
        2,
        "loop continued into a second assistant turn"
    );
    assert!(
...
async fn missing_required_field_yields_error_result_without_executing() {
    ...
    assert!(
        seen.lock().unwrap().is_empty(),
        "missing-required must not execute the tool"
    );
    let errs = tool_result_errors(&new);
    assert_eq!(
        errs.len(),
        1,
        "missing required field yields one isError result"
    );
    assert!(
        errs[0]
            .content
            .iter()
            .any(|c| matches!(c, Content::Text { text, .. } if text.contains("required"))),
    ...
    assert_eq!(
        faux.call_count(),
        2,
        "loop continues after the error result"
    );
}
