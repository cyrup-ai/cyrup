...
///
/// `[CYRUP-DELTA]`: cyrup's agent loop builds a provider-level [`StreamOptions`], which (per the
/// open `thinkingBudgets` blocker) carries no per-level token budgets and only a provider-level
/// `reasoning`. This adapter therefore forwards the unified level it can recover and leaves
/// `thinking_budgets` unset. Apps that need per-level budgets construct [`ProxyStreamOptions`]
/// directly and pass [`stream_proxy`] as their `StreamFn` — exactly as Pi's example does — which
/// DOES forward budgets.
...
impl ProxyStreamFn {
    ...
    /// Map the agent's provider-level [`StreamOptions`] onto [`ProxyStreamOptions`] (see the
    /// `[CYRUP-DELTA]` on the struct re: `thinking_budgets`).
