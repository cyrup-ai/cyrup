...
impl Hooks for GateLateToolHook {
    async fn before_tool_call(
        &self,
        ctx: cyrup_agent::BeforeToolCall<'_>,
        _cancel: CancelToken,
    ) -> Result<cyrup_agent::BeforeOutcome, HookError> {
        if ctx.tool_name == "late" {
            Ok(cyrup_agent::BeforeOutcome::Block { reason: Some("denied by policy".into()) })
        } else {
            Ok(cyrup_agent::BeforeOutcome::Proceed)
        }
    }
}
