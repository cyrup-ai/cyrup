...
use cyrup_agent::{Agent, HookError, Hooks, PostTurn, StreamFn, TurnUpdate};
use cyrup_core::{
...
fn recording(responses: Vec<cyrup_core::AssistantMessage>) -> (Arc<dyn StreamFn>, Requests) {
    ...
    let sf: Arc<dyn StreamFn> =
        Arc::new(Recorder { inner: Arc::new(cyrup_agent::ProviderStreamFn::new(faux)), seen: seen.clone() });
    (sf, seen)
}
