...
//!
//! These are integration tests, so this file is its own crate and inherits no lint config from the
//! library — hence the file-level allow (workspace clippy DENIES `unwrap_used`/`expect_used`/
//! `panic`/`indexing_slicing`).
#![allow(clippy::unwrap_used, clippy::expect_used, clippy::panic, clippy::indexing_slicing)]

use cyrup_test_support::assert_jsonl_roundtrip;
