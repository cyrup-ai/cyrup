...
impl Settings {
    ...
    fn get_nested_str(&self, path: &[&str]) -> Option<String> {
        self.get_path(path).and_then(Value::as_str).map(str::to_string)
    }
}
...
pub fn migrate_settings(settings: &mut Map<String, Value>) {
        ...
        && let Some(Value::Bool(b)) = settings.get("websockets").cloned()
    {
        settings.insert(
            "transport".to_string(),
            Value::String(if b { "websocket".to_string() } else { "sse".to_string() }),
        );
...
fn random_uuid_v4() -> String {
    ...
    for (i, b) in entropy(0).to_le_bytes().into_iter().chain(entropy(1).to_le_bytes()).enumerate() {
...
impl EffectiveSettings {
    ...
    pub fn npm_command(&self) -> Option<Vec<String>> {
        self.merged.get("npmCommand").and_then(Value::as_array).map(|a| {
    ...
    pub fn show_images(&self) -> bool {
        self.merged.get_nested_bool(&["terminal", "showImages"]).unwrap_or(true)
    }
    ...
    pub fn show_terminal_progress(&self) -> bool {
        self.merged.get_nested_bool(&["terminal", "showTerminalProgress"]).unwrap_or(false)
    }

    /// `images.autoResize` (default true; :1107-1109).
    pub fn image_auto_resize(&self) -> bool {
        self.merged.get_nested_bool(&["images", "autoResize"]).unwrap_or(true)
    }

    /// `images.blockImages` (default false; :1120-1122).
    pub fn block_images(&self) -> bool {
        self.merged.get_nested_bool(&["images", "blockImages"]).unwrap_or(false)
    }

    /// `doubleEscapeAction` (default `tree`; :1143-1145).
    pub fn double_escape_action(&self) -> String {
        self.merged.get_str("doubleEscapeAction").unwrap_or_else(|| "tree".to_string())
    }
    ...
    pub fn autocomplete_max_visible(&self) -> i64 {
        self.merged.get_nested_i64(&["autocompleteMaxVisible"]).unwrap_or(5)
    }
    ...
    pub fn show_hardware_cursor(&self, env: &crate::env::EnvVars) -> bool {
        self.merged.get_bool("showHardwareCursor").unwrap_or(env.hardware_cursor)
    }
    ...
    pub fn clear_on_shrink(&self, env: &crate::env::EnvVars) -> bool {
        self.merged.get_nested_bool(&["terminal", "clearOnShrink"]).unwrap_or(env.clear_on_shrink)
    }
    ...
    pub fn provider_retry_timeout_ms(&self) -> Option<i64> {
        self.merged.get_nested_i64(&["retry", "provider", "timeoutMs"])
    }
    ...
    pub fn provider_retry_max_retries(&self) -> Option<i64> {
        self.merged.get_nested_i64(&["retry", "provider", "maxRetries"])
    }
...
impl SettingsManager {
    ...
    pub fn set_image_width_cells(&mut self, width: f64) -> Result<(), ConfigError> {
        ...
        self.set_nested(SettingsScope::Global, &["terminal", "imageWidthCells"], clamped.into())
    }
    ...
    pub fn set_http_idle_timeout_ms(&mut self, timeout_ms: f64) -> Result<(), ConfigError> {
        if !timeout_ms.is_finite() || timeout_ms < 0.0 {
            return Err(ConfigError::Trust(format!("Invalid httpIdleTimeoutMs setting: {timeout_ms}")));
        }
        self.set(SettingsScope::Global, "httpIdleTimeoutMs", timeout_ms.floor() as i64)
    }
    ...
    pub fn set_show_images(&mut self, show: bool) -> Result<(), ConfigError> {
        self.set_nested(SettingsScope::Global, &["terminal", "showImages"], show.into())
    }
    ...
    pub fn set_enable_analytics(&mut self, enabled: bool) -> Result<(), ConfigError> {
        self.store.with_lock(SettingsScope::Global, &mut |current| {
            let mut doc = match current.map(Settings::parse) {
                Some(Ok(s)) => s,
                _ => Settings::default(),
            };
            doc.obj.insert("enableAnalytics".to_string(), Value::Bool(enabled));
            ...
            if enabled && !has_tracking {
                doc.obj.insert("trackingId".to_string(), Value::String(random_uuid_v4()));
            }
            Some(doc.to_pretty())
        })?;
        self.reload_internal();
...
fn set_value_at_path(map: &mut Map<String, Value>, path: &[String], value: Value) {
    ...
    let entry = map.entry(first.clone()).or_insert_with(|| Value::Object(Map::new()));
    if !entry.is_object() {
...
mod tests {
    ...
    fn migrations_applied_on_parse() {
        ...
        assert_eq!(s.get("enableSkillCommands"), Some(&serde_json::json!(false)));
        // retry.maxDelayMs -> retry.provider.maxRetryDelayMs
        let s = Settings::parse(r#"{ "retry": { "maxDelayMs": 5000 } }"#).unwrap();
        assert_eq!(s.get("retry").unwrap()["provider"]["maxRetryDelayMs"], serde_json::json!(5000));
    ...
    fn nested_set_preserves_siblings() {
        ...
        let store = Arc::new(InMemorySettingsStore::new());
        store.seed(SettingsScope::Global, r#"{ "terminal": { "imageWidthCells": 40 } }"#);
        let mut mgr = SettingsManager::load(store.clone(), Settings::new(), false);
        mgr.set_show_images(false).unwrap();
        let raw = store.read(SettingsScope::Global).unwrap().unwrap();
        let s = Settings::parse(&raw).unwrap();
        assert_eq!(s.get("terminal").unwrap()["showImages"], serde_json::json!(false));
        assert_eq!(s.get("terminal").unwrap()["imageWidthCells"], serde_json::json!(40));
    }
    ...
    fn theme_split_get_theme_vs_get_theme_setting() {
        ...
        let s = EffectiveSettings::from_settings(Settings::parse(r#"{ "theme": "pkg/dark" }"#).unwrap());
    ...
    fn hardware_cursor_and_clear_on_shrink_env_fallback() {
        ...
        let s = EffectiveSettings::from_settings(
            Settings::parse(r#"{ "showHardwareCursor": false, "terminal": { "clearOnShrink": false } }"#)
                .unwrap(),
        );
    ...
    fn thinking_budgets_warnings_and_combined_settings() {
        ...
        assert_eq!(
            ...
            ProviderRetrySettings { timeout_ms: None, max_retries: None, max_retry_delay_ms: 60000 }
        );
        assert_eq!(
        ...
        assert_eq!(
            s.thinking_budgets(),
            Some(ThinkingBudgets { minimal: None, low: Some(100), medium: None, high: Some(9000) })
        );
        assert_eq!(s.warnings(), Warnings { anthropic_extra_usage: Some(false) });
        assert_eq!(
            s.branch_summary_settings(),
            BranchSummarySettings { reserve_tokens: 2048, skip_prompt: true }
        );
        assert_eq!(
            s.provider_retry_settings(),
            ProviderRetrySettings {
    ...
    fn compaction_and_retry_combined_getters() {
        ...
        assert_eq!(
            s.retry_settings(),
            RetrySettings { enabled: true, max_retries: 3, base_delay_ms: 2000 }
        );
        ...
        assert_eq!(
            s.compaction_settings(),
            CompactionSettings { enabled: false, reserve_tokens: 100, keep_recent_tokens: 200 }
        );
        assert_eq!(
            s.retry_settings(),
            RetrySettings { enabled: false, max_retries: 9, base_delay_ms: 500 }
        );
    }
    ...
    fn typed_list_getters() {
        ...
        assert_eq!(s.skill_paths(), vec!["/skill/a".to_string(), "/skill/b".to_string()]);
    ...
    fn apply_overrides_deep_merges_onto_effective() {
        ...
        let overrides = Settings::parse(
            r#"{ "compaction": { "reserveTokens": 4096 }, "quietStartup": true }"#,
        )
        .unwrap();
