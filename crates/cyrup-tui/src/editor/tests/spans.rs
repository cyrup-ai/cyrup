...
fn spans_for_segment_cursor_inside_the_accent_zone_splits_it() {
    ...
    assert_eq!(
        ...
        "before-cursor part of the accent zone stays accent"
    );
    assert_eq!(spans[1].style, cursor_style, "the cursor cell is reversed");
    assert_eq!(
        spans[2].style, accent,
        "after-cursor part of the accent zone stays accent"
    );
...
fn spans_for_segment_empty_visual_line_emits_one_empty_base_span() {
    ...
    assert_eq!(
        spans[0].style, base,
        "a blank NON-cursor soft-newline row must not carry a caret"
    );
}
