        ...
        'run: loop {
            ...
            tokio::select! {
                ...
                maybe_in = input.next() => {
                    ...
                    let Some(first) = maybe_in else { break };
                ...
                maybe_ev = events.next() => {
                    let Some(first) = maybe_ev else { continue };
                    ...
                    while let Some(ev) = pending.pop_front() {
                        ...
                        if should_honor_extension_shutdown(&session, settled) {
                            return Ok(());
                        }
                    }
                    self.draw_synchronized()?;
