...
use crate::{
    App, UiTheme, render_markdown, render_markdown_with_hyperlinks, trim_partial_closing_fence,
};
...
fn headings_drop_hash_for_h1_h2_and_keep_it_for_h3_plus() {
    ...
    assert!(
        has_span_mod(&lines, "Plan", Modifier::BOLD),
        "heading not bold"
    );
    ...
    assert!(
        has_span_mod(&h1, "Title", Modifier::UNDERLINED),
        "H1 must be underlined"
    );
    assert!(
        !has_span_mod(&lines, "Plan", Modifier::UNDERLINED),
        "H2 must NOT be underlined"
    );
}
...
fn unordered_and_ordered_lists_render_markers() {
    ...
    assert!(
        ul.iter().any(|r| r.starts_with("- one")),
        "unordered bullet missing:\n{ul:?}"
    );

    // Ordered lists renumber from the list's start, even when the source repeats `1.`.
    let ol = rows(&render_markdown("1. a\n1. b\n", 80, &theme));
    assert!(
        ol.iter().any(|r| r.starts_with("1. a")),
        "ordered #1 missing:\n{ol:?}"
    );
    assert!(
        ol.iter().any(|r| r.starts_with("2. b")),
        "ordered renumber to 2 missing:\n{ol:?}"
    );
}
...
fn blockquote_prefixes_border_and_hr_is_a_rule() {
    ...
    let bq_rows = rows(&bq);
    assert!(
        bq_rows.iter().any(|r| r.starts_with("│ ")),
        "blockquote border missing:\n{bq_rows:?}"
    );
    assert!(
        bq_rows.iter().any(|r| r.contains("note")),
        "blockquote body missing:\n{bq_rows:?}"
    );
    ...
    assert!(
        rows(&hr)
            .iter()
            .any(|r| r.chars().all(|c| c == '─') && !r.is_empty()),
        "hr missing"
    );
}
...
fn inline_code_bold_and_links() {
    ...
    assert!(
        has_span_mod(&bold, "b", Modifier::BOLD),
        "bold span not bold"
    );
    ...
    assert!(
        has_span_mod(&link, "docs", Modifier::UNDERLINED),
        "link text not underlined"
    );
    assert!(
        lt.contains("(https://example.com)"),
        "link url not appended:\n{lt}"
    );
...
fn m14_empty_link_text_still_gets_the_url_suffix() {
    ...
    let theme = UiTheme::dark();
    let plain = rows(&render_markdown_with_hyperlinks(
...
fn unknown_language_code_renders_flat() {
    ...
    assert!(
        line_text(body).starts_with("  some plain text"),
        "code body not 2-space indented"
    );
...
fn trim_partial_closing_fence_keeps_open_block_stable() {
    ...
    assert!(
        !trimmed.contains("``\n") && !trimmed.ends_with("``"),
        "partial fence not trimmed: {trimmed:?}"
    );
...
fn markdown_assistant_entry_reaches_scrollback_multiline() {
    ...
    app.transcript_mut()
        .commit_assistant(Some("## Plan\n\n- step one\n- step two".to_string()));
    app.draw().unwrap();
    let sb = app.scrollback_text();
    assert!(
        !sb.contains("assistant:"),
        "invented assistant label in scrollback:\n{sb}"
    );
    assert!(
        sb.contains("Plan"),
        "heading missing from scrollback:\n{sb}"
    );
    assert!(
        sb.contains("- step one"),
        "list item missing from scrollback:\n{sb}"
    );
    assert!(
        sb.contains("- step two"),
        "second list item missing from scrollback:\n{sb}"
    );
}
...
fn streaming_markdown_partial_renders_in_viewport() {
    ...
    assert!(
        text.contains("Title"),
        "streaming heading missing from viewport:\n{text}"
    );
    assert!(
        text.contains("streaming body"),
        "streaming body missing from viewport:\n{text}"
    );
    ...
    assert!(
        !text.contains('▌'),
        "invented soft streaming cursor:\n{text}"
    );
}
...
fn tables_render_as_a_box_drawing_grid() {
    ...
    assert!(
        text.contains("Name") && text.contains("Role"),
        "header cells:\n{text}"
    );
    assert!(
        text.contains("Ada") && text.contains("logic"),
        "body cells:\n{text}"
    );
...
fn m1_m2_table_frame_and_header_take_body_colour_not_mdhr_or_mdheading() {
    ...
    for glyph in ["┌", "├", "└"] {
        ...
        assert_ne!(
            line_fg(row),
            Some(MD_HR),
            "border row {glyph} is still mdHr grey"
        );
    }
    ...
    assert!(
        ...
        "header cell not bold"
    );
    assert_eq!(
        header_cell.style.fg,
        Some(BODY_FG),
        "header cell must be body-coloured"
    );
    assert_ne!(
        header_cell.style.fg,
        Some(MD_HEADING),
        "header cell is still mdHeading amber"
    );
    ...
    assert!(
        !body_cell.style.add_modifier.contains(Modifier::BOLD),
        "body cell wrongly bold"
    );
    ...
    assert_eq!(
        line_fg(rule),
        Some(MD_HR),
        "the `---` rule must stay mdHr grey"
    );
    let head = render_markdown("## Plan\n", 80, &theme);
    assert!(
        has_span_colored(&head, "Plan", MD_HEADING),
        "heading must stay mdHeading"
    );
    assert!(
        has_span_mod(&head, "Plan", Modifier::BOLD),
...
fn m3_nested_lists_indent_four_columns_per_level() {
    ...
    assert!(
        r.iter().any(|l| l == "- top"),
        "depth 0 must be flush left:\n{r:?}"
    );
    assert!(
        r.iter().any(|l| l == "    - one"),
        "depth 1 must indent 4 columns:\n{r:?}"
    );
    assert!(
        r.iter().any(|l| l == "        - two"),
        "depth 2 must indent 8 columns:\n{r:?}"
    );

    // MIRROR — a flat list is unindented at every item.
    let flat = rows(&render_markdown("- a\n- b\n", 80, &theme));
    assert!(
        flat.iter().any(|l| l == "- a") && flat.iter().any(|l| l == "- b"),
        "{flat:?}"
    );
}
...
fn m4_task_list_items_keep_the_bullet_before_the_box() {
    ...
    assert!(
        r.iter().any(|l| l == "- [ ] todo"),
        "unchecked marker must be `- [ ] `:\n{r:?}"
    );
    assert!(
        r.iter().any(|l| l == "- [x] done"),
        "checked marker must be `- [x] `:\n{r:?}"
    );
    // The bullet and the box are one `listBullet` run, not two differently-styled spans.
    assert!(
        has_span_colored(&lines, "- [x] ", theme.md_list_bullet_style().fg.unwrap()),
        "{r:?}"
    );
...
fn m6_horizontal_rule_is_followed_by_exactly_one_blank_row() {
    ...
    assert_eq!(
        r.get(rule_at + 2).map(String::as_str),
        Some("after the rule"),
        "{r:?}"
    );
    ...
    assert!(
        !spaced.get(at + 2).map(is_blank).unwrap_or(true),
        "doubled blank:\n{sr:?}"
    );
}
...
fn m8_soft_line_break_inside_a_paragraph_stays_a_row_break() {
    ...
    assert!(
        !r.iter().any(|l| l.contains("alpha beta")),
        "soft break collapsed to a space:\n{r:?}"
    );
...
fn m11_table_too_narrow_for_its_grid_falls_back_to_raw_markdown() {
    ...
    let nr = rows(&narrow);
    assert!(
        !nr.iter().any(|l| l.contains('┌')),
        "narrow pane still drew a grid:\n{nr:?}"
    );
    ...
    assert_eq!(
        squashed, source,
        "raw fallback lost or reordered source text:\n{nr:?}"
    );
    assert!(
        nr.iter().any(|l| l.contains("------")),
        "raw delimiter row missing:\n{nr:?}"
    );
    assert!(
        nr.iter().any(|l| l.contains("Ada")),
        "raw body row missing:\n{nr:?}"
    );
    ...
    assert!(
        wide.iter().any(|l| l.contains('┌')),
        "width 9 must still draw the grid:\n{wide:?}"
    );
    assert!(
        !wide.iter().any(|l| l.contains("|------|")),
        "grid must not print raw:\n{wide:?}"
    );
}
...
fn m11_narrow_tables_never_panic_on_column_arithmetic() {
    ...
    for w in 0..=40usize {
        ...
        if w < 3 * 5 + 1 + 5 {
            ...
            assert!(
                ...
                "width {w} drew an oversized grid:\n{r:?}"
            );
        }
    }
    ...
    for l in &render_markdown(md, 13, &theme) {
        assert!(
            l.width() <= 13,
            "row {:?} is {} cells at a 13-cell pane",
            line_text(l),
            l.width()
        );
    }
...
fn m14_inline_url_suffix_is_gated_on_terminal_hyperlink_support() {
    ...
    assert_eq!(
        plain, "See the docs (https://example.com/a/very/long/path) now.",
        "{plain}"
    );
    assert!(
        has_span_mod(&plain_lines, "the docs", Modifier::UNDERLINED),
        "{plain}"
    );
    let cap_lines = render_markdown_with_hyperlinks(src, 200, &theme, true);
    assert!(
        has_span_mod(&cap_lines, "the docs", Modifier::UNDERLINED),
        "{capable}"
    );
    ...
    for hl in [true, false] {
        ...
        assert_eq!(
            ...
            "hyperlinks={hl} duplicated the bare URL"
        );
    }
}
...
fn m16_single_tilde_is_not_strikethrough() {
    ...
    let text = rows(&lines).join("\n");
    assert_eq!(
        text, "open ~/notes~ and a~b~c here",
        "single tildes were eaten:\n{text}"
    );
    assert!(
        !lines
            .iter()
    ...
    assert_eq!(
        st, "this gone stays",
        "double-tilde delimiters must be dropped:\n{st}"
    );
    assert!(
        has_span_mod(&strong, "gone", Modifier::CROSSED_OUT),
        "`~~gone~~` not struck:\n{st}"
    );
    ...
    assert!(
        has_span_mod(&mixed, "b", Modifier::BOLD),
        "bold inside a literal `~…~` lost"
    );
}
...
fn m8_soft_break_inside_a_list_item_indents_past_the_marker() {
    ...
    assert_eq!(
        ol,
        vec!["1. alpha".to_string(), "   beta".to_string()],
        "{ol:?}"
    );
    let wide = rows(&render_markdown(
        "9. alpha\n   beta\n10. gamma\n    delta\n",
        200,
        &theme,
    ));
    ...
    assert!(
        deep.iter().any(|l| l == "    - one"),
        "depth 1 must be indent-only:\n{deep:?}"
    );
    assert!(
        deep.iter().any(|l| l == "        - two"),
        "depth 2 must be indent-only:\n{deep:?}"
    );
...
fn m11_narrow_table_fallback_keeps_the_blockquote_and_list_prefixes() {
    ...
    assert!(
        !r.iter().any(|l| l.contains('┌')),
        "width 8 must not draw a grid:\n{r:?}"
    );
    let body: Vec<&String> = r.iter().filter(|l| !l.trim().is_empty()).collect();
    assert!(
        !body.is_empty(),
        "fallback produced nothing inside a blockquote:\n{r:?}"
    );
    for l in &body {
        assert!(
            l.starts_with("│ "),
            "fallback row escaped the blockquote border:\n{r:?}"
        );
    }
    ...
    assert!(
        lines
            .iter()
            .flat_map(|l| &l.spans)
            .any(|s| s.content == "│ " && s.style.fg == border),
    ...
    assert!(
        !lbody.is_empty(),
        "fallback produced nothing inside a list item:\n{lr:?}"
    );
    assert!(
    ...
    for l in lbody.iter().skip(1) {
        assert!(
            l.starts_with("  "),
            "fallback row fell back to column 0:\n{lr:?}"
        );
    }
    ...
    assert!(
        top.iter().any(|l| l.contains("|---")),
        "raw fallback missing:\n{top:?}"
    );
    assert!(
        !top.iter().any(|l| l.starts_with("│ ")),
        "invented a quote border:\n{top:?}"
    );
}
...
fn fenced_code_block_emits_no_trailing_indent_only_row() {
    ...
    assert_eq!(
        flat,
        vec!["```".to_string(), "  plain".to_string(), "```".to_string()],
        "{flat:?}"
    );
...
fn m17_code_fence_keeps_the_whole_info_string() {
    ...
    let r = rows(&lines);
    assert!(
        r.iter().any(|l| l == "```js title=\"server.ts\""),
...
fn m14_a_link_whose_text_is_the_full_mailto_href_gets_no_suffix() {
    let theme = UiTheme::dark();
    let lines =
        render_markdown_with_hyperlinks("[mailto:a@b.com](mailto:a@b.com)", 80, &theme, false);
    ...
    // suppression.
    let differing =
        render_markdown_with_hyperlinks("[email me](mailto:a@b.com)", 80, &theme, false);
...
fn m5_a_wrapped_list_item_hangs_under_its_own_text() {
    ...
    for row in &text[1..] {
        ...
        assert!(
            row.starts_with("  "),
            "continuation lost the hanging indent: {row:?}"
        );
        assert!(
            !row.starts_with("- "),
            "bullet re-drawn on a continuation row: {row:?}"
        );
    }
    for l in &lines {
        assert!(
            l.width() <= 20,
            "row overflows itemWidth + prefix: {:?}",
            line_text(l)
        );
    }
    ...
    assert!(
        ordered[1].starts_with("   ") && !ordered[1].starts_with("    "),
        "{ordered:?}"
    );
...
fn sys2_a_long_blockquote_redraws_its_border_on_every_row() {
    ...
    assert!(
        lines[1].spans[0].style.fg.is_some(),
        "continuation border unstyled: {:?}",
        lines[1]
    );
}
...
fn m9_the_rule_still_spans_the_full_content_width() {
    ...
    assert_eq!(
        rows(&render_markdown("---", 200, &theme)),
        vec!["─".repeat(80)]
    );
    ...
    assert_eq!(
        rule[0],
        &format!("  {}", "─".repeat(18)),
        "rule not sized/prefixed to the item"
    );
}
...
fn m9_a_table_inside_a_blockquote_is_sized_to_the_quote_not_the_pane() {
    ...
    assert!(
        ...
        "top level should draw a grid: {top:?}"
    );
    ...
    assert!(
        !nested.iter().any(|r| r.contains('┌')),
        "grid drawn too wide: {nested:?}"
    );
    assert!(
        nested.iter().any(|r| r.contains("|---")),
        "no raw fallback: {nested:?}"
    );
...
fn sys2_a_fenced_block_inside_a_list_item_keeps_the_prefix_and_wraps() {
    ...
    assert_eq!(
        code[0], "    let a = 1; let b = 2; let",
        "first code row: {text:?}"
    );
    assert_eq!(
        code[1], "  c = 3; let d = 4;",
        "continuation drops the code indent: {text:?}"
    );
    for l in &lines {
...
fn emit_table_routes_the_grid_through_the_container_prefix() {
    ...
    assert!(
        r.iter().any(|l| l.contains('┌')),
        "no grid drawn at width 30:\n{r:?}"
    );
    for l in r.iter().filter(|l| !l.trim().is_empty()) {
        assert!(
            l.starts_with("│ "),
            "grid row escaped the blockquote border:\n{r:?}"
        );
    }
    ...
    assert!(
        body.len() > 4,
        "expected a full grid inside the item:\n{lr:?}"
    );
    assert!(
        body[0].starts_with("- ┌"),
    ...
    for l in body.iter().skip(1) {
        assert!(
            l.starts_with("  "),
            "grid row fell back to column 0:\n{lr:?}"
        );
        assert!(
            !l.starts_with("- "),
            "bullet re-emitted on a continuation row:\n{lr:?}"
        );
    }
    ...
    assert!(
        two.iter().any(|l| l == "- after"),
        "the NEXT item lost its bullet:\n{two:?}"
    );
    ...
    assert!(
        top.iter().any(|l| l.starts_with('┌')),
        "top-level grid gained a prefix:\n{top:?}"
    );
}
...
fn a_blank_inside_a_blockquote_keeps_the_border() {
    ...
    assert!(
        code.iter().any(|l| l == "│ "),
        "code-block separator lost the border:\n{code:?}"
    );
    assert!(
        !code.iter().any(|l| l.is_empty()),
        "bare blank inside the quote:\n{code:?}"
    );
    ...
    assert_eq!(
        tail,
        vec!["│ a".to_string(), String::new(), "tail".to_string()],
        "{tail:?}"
    );
...
fn the_narrow_table_fallback_strips_blockquote_source_markers() {
    ...
    for width in [8usize, 9, 10] {
        ...
        assert!(
            r.iter().any(|l| l.contains("Name")),
            "width {width}: no fallback body:\n{r:?}"
        );
    }

    // Doubly nested: both levels of marker come off, and both borders stay on.
    let deep = rows(&render_markdown(
        "> > | a | b |\n> > |---|---|\n> > | 1 | 2 |\n",
        10,
        &theme,
    ));
    ...
    let gt = rows(&render_markdown(
        "| a | b |\n|---|---|\n| > x | 2 |",
        8,
        &theme,
    ));
    assert!(
        gt.iter().any(|l| l.contains('>')),
        "stripped a `>` that was content:\n{gt:?}"
    );
}
...
fn the_top_level_rewrap_bounds_every_row_when_the_prefix_eats_the_pane() {
    ...
    for width in 1..=12usize {
        assert_all_rows_fit(
            &render_markdown(deep, width, &theme),
            width,
            "deep nested list",
        );
    }
    ...
    for width in 1..=14usize {
        ...
        assert_all_rows_fit(
            &render_markdown(&src, width, &theme),
            width,
            "quoted deep list",
        );
    }
    ...
    assert_eq!(
        marker_only,
        vec!["- ".to_string()],
        "post-pass trimmed a fitting row"
    );
    let grid = rows(&render_markdown(
        "| a | b |\n|---|---|\n| 1 | 2 |",
        40,
        &theme,
    ));
...
fn m7_inline_formatting_survives_inside_a_table_cell() {
    ...
    let plain_header = find_row(&lines, "Name")
        ...
        .expect("header cell");
    assert!(
        plain_header.style.add_modifier.contains(Modifier::BOLD),
        "header band lost bold"
    );
    ...
    assert_eq!(
        link_span.fg,
        theme.md_link_style().fg,
        "link text lost mdLink inside the cell"
    );
    ...
    assert!(
        cap.iter().any(|l| l.contains("doc")),
        "the capable branch lost the link text:\n{cap:?}"
    );
    assert!(
...
fn m12_inline_math_is_typeset_and_falls_back_to_its_raw_source() {
    ...
    let raw = rows(&render_markdown(
        "unsupported $x + \\unknown{y}$ here\n",
        60,
        &theme,
    ));
    assert_eq!(
...
fn m12_block_math_stacks_in_display_mode() {
    ...
    let in_list = rows(&render_markdown(
        "- item\n\n  $$\\frac{1}{2}$$\n\n- next\n",
        60,
        &theme,
    ));
    ...
    assert!(
        in_list.iter().any(|l| l == "- next"),
        "the next item was swallowed:\n{in_list:?}"
    );
}
...
fn m12_math_is_not_tokenized_inside_code_or_in_prices() {
    ...
    assert!(
        !fenced.iter().any(|l| l.contains('α')),
        "math leaked into a code fence:\n{fenced:?}"
    );
...
fn m12_assistant_math_reaches_scrollback_typeset() {
    ...
    let sb = app.scrollback_text();
    assert!(
        sb.contains("e^(iπ)+1 = 0"),
        "assistant math was not typeset:\n{sb}"
    );
...
fn m7_the_gap_between_a_bold_word_and_a_plain_one_is_not_bold() {
    ...
    assert_eq!(
        bold,
        vec!["alpha"],
        "the separator was swept into the bold run: {row:?}"
    );
}
...
fn incremental_highlight_matches_a_cold_render_at_every_delta() {
    ...
    for (i, line) in body.iter().enumerate() {
        ...
        assert_eq!(
            rows(&render_markdown(&text, 80, &theme)),
            cold(&text),
            "open delta {i}"
        );
    }
    ...
    assert_eq!(
        rows(&render_markdown(&closed, 80, &theme)),
        cold(&closed),
        "the closing frame"
    );
...
fn a_bounded_highlight_equals_the_first_rows_of_a_full_one() {
    ...
    for bound in [1usize, 2, 10, 37, 199, 200] {
        ...
        assert_eq!(
            bounded[..],
            full[..bounded.len()],
            "bound {bound} must match the full prefix"
        );
    }
...
fn a_settled_body_memo_is_keyed_on_everything_that_changes_its_rows() {
    ...
    let as_python =
        crate::markdown::highlight_code_lines(code, "python", &dark, 3).expect("python");
