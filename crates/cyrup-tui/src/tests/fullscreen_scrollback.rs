...
//! `Regular` arm passes the `false` itself. The exit path's `false` (`app/run.rs`) is a literal
//! inside `App::run`'s event loop, which a unit test cannot enter without a live session and event
//! stream — so flipping THAT literal to `true` would not fail either test, while quietly restoring
