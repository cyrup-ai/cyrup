...
fn read_call_shows_path_and_line_range() {
    ...
    assert!(
        text.contains("read src/main.rs:10-14"),
        "read header + range: {text:?}"
    );
    assert!(
        !text.contains("read("),
        "no generic paren-wrapped marker: {text:?}"
    );
...
fn read_result_hidden_until_expanded() {
    ...
    assert!(
        ...
        "body shown when expanded: {expanded:?}"
    );
    assert!(
        expanded.contains("line three"),
        "full body when expanded: {expanded:?}"
    );
}
...
fn bash_call_and_output_tail() {
    ...
    assert!(
        collapsed.contains("$ ls"),
        "bash `$ command` header: {collapsed:?}"
    );
    assert!(
        collapsed.contains("ln25"),
        "tail line previewed: {collapsed:?}"
    );
    assert!(
        collapsed.contains("ln21"),
        "5-line tail starts at ln21: {collapsed:?}"
    );
    assert!(
        !collapsed.contains("ln20"),
        "earlier lines hidden: {collapsed:?}"
    );
    assert!(
        collapsed.contains("20 earlier lines"),
        "hidden-head hint: {collapsed:?}"
    );
    ...
    assert!(
        expanded.contains("ln01"),
        "all lines visible when expanded: {expanded:?}"
    );
}
...
fn powershell_call_renders_with_the_ps_prompt() {
    ...
    view.push_tool_start(
        "powershell",
        json!({ "command": "Get-ChildItem", "timeout": 5 }),
    );
    let body: String = (1..=25)
        .map(|i| format!("ln{i:02}"))
        .collect::<Vec<_>>()
    ...
    assert!(
        collapsed.contains("(timeout 5s)"),
        "timeout suffix: {collapsed:?}"
    );
    assert!(
        collapsed.contains("ln25") && collapsed.contains("ln21"),
        "5-line tail: {collapsed:?}"
    );
    assert!(
        !collapsed.contains("ln20"),
        "earlier lines hidden: {collapsed:?}"
    ...
    assert!(
        expanded.contains("ln01"),
        "expand shows the head: {expanded:?}"
    );
    assert!(
        expanded.contains("PS> Get-ChildItem"),
        "header survives expand: {expanded:?}"
    );
}
...
fn edit_error_shows_error_text_not_diff() {
    ...
    view.push_tool_end(
        "edit",
        true,
        Some(text_result("permission denied", json!(null))),
    );
    let text = render(&mut view, 70, 8);
    assert!(
        text.contains("edit a.txt"),
        "edit header still shows: {text:?}"
    );
    assert!(
        text.contains("permission denied"),
        "error body shown: {text:?}"
    );
    assert!(
        !text.contains('✗'),
        "no cross glyph — state is the bg tint: {text:?}"
    );
}
...
fn grep_call_shows_pattern_and_path() {
    ...
    view.push_tool_start(
        "grep",
        json!({ "pattern": "foo", "path": "src", "limit": 50 }),
    );
    view.push_tool_end(
        "grep",
        false,
    ...
    assert!(
        text.contains("src/a.rs:1:foo()"),
        "match line shown: {text:?}"
    );
}
...
fn list_tool_limit_survives_a_float_spelling() {
    ...
    view.push_tool_start(
        "grep",
        json!({ "pattern": "foo", "path": "src", "limit": 50.0 }),
    );
    view.push_tool_end(
        "grep",
        false,
        Some(text_result("src/a.rs:1:foo()", json!(null))),
    );
    let text = render(&mut view, 70, 8);
    assert!(
        text.contains("limit 50"),
        "grep float limit renders as `limit 50`: {text:?}"
    );
    assert!(
        !text.contains("limit 50.0"),
        "no Rust `Debug` float spelling: {text:?}"
    );

    let mut view = TranscriptView::new();
    view.push_tool_start(
        "find",
        json!({ "pattern": "*.rs", "path": "src", "limit": 20.0 }),
    );
...
fn list_tool_limit_zero_still_renders() {
    ...
    view.push_tool_start(
        "grep",
        json!({ "pattern": "foo", "path": "src", "limit": 0 }),
    );
    view.push_tool_end("grep", false, Some(text_result("", json!(null))));
    let text = render(&mut view, 70, 8);
    assert!(
        text.contains("limit 0"),
        "presence test, not truthiness: {text:?}"
    );
    ...
    view.push_tool_start(
        "find",
        json!({ "pattern": "*.rs", "path": "src", "limit": -0.0 }),
    );
    view.push_tool_end("find", false, Some(text_result("", json!(null))));
    let text = render(&mut view, 70, 8);
    assert!(
        text.contains("(limit 0)"),
        "find negative-zero float limit: {text:?}"
    );
    assert!(
        !text.contains("(limit -0"),
        "`String(-0)` is `0`, never `-0`: {text:?}"
    );
}
