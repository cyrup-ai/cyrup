...
fn spinner_advances_every_80ms() {
    assert_eq!(
        StatusIndicator::spinner_at(Duration::ZERO),
        SPINNER_FRAMES[0]
    );
    assert_eq!(
        StatusIndicator::spinner_at(Duration::from_millis(80)),
        SPINNER_FRAMES[1]
    );
    assert_eq!(
        StatusIndicator::spinner_at(Duration::from_millis(79)),
        SPINNER_FRAMES[0]
    );
    // Wraps after the full frame count (10 * 80 ms = 800 ms).
    assert_eq!(
        StatusIndicator::spinner_at(Duration::from_millis(800)),
        SPINNER_FRAMES[0]
    );
}
...
fn auto_retry_event_renders_pi_exact_copy_with_delay_seconds() {
    ...
    assert!(
        out.contains("Retrying (2/5) in 2s"),
        "retry copy missing/incorrect:\n{out}"
    );
...
fn overflow_compaction_event_prefixes_context_overflow_copy() {
    ...
    app2.ingest_event(&AgentSessionEvent::CompactionStart {
        reason: CompactionReason::Manual,
    });
    app2.draw().unwrap();
    let m = buf_text(&app2);
    assert!(
        m.contains("Compacting context..."),
        "manual compaction copy missing:\n{m}"
    );
    assert!(
        !m.contains("Auto-compacting"),
        "manual must not say Auto-compacting:\n{m}"
    );
}
...
fn compaction_band_renders_its_message() {
    ...
    app.draw().unwrap();
    assert!(
        buf_text(&app).contains("Compacting context..."),
        "compaction band missing"
    );
...
fn a_summarization_retry_shows_the_countdown_then_restores_the_compaction_band() {
    ...
    app.draw().unwrap();
    assert!(
        buf_text(&app).contains("Auto-compacting..."),
        "sanity: the compaction band is up"
    );
    ...
    let out = buf_text(&app);
    assert!(
        out.contains("Retrying (1/3) in 2s"),
        "retry countdown missing during backoff:\n{out}"
    );
    assert_eq!(app.state().indicator.kind(), IndicatorKind::Retry);
    ...
    assert!(
        out.contains("Auto-compacting..."),
        "compaction band not restored:\n{out}"
    );
}
...
fn compaction_and_retry_status_is_band_only_never_scrollback() {
    ...
    assert!(
        !sb.contains("Compacting context"),
        "compaction copy leaked to scrollback:\n{sb}"
    );
    assert!(
        !sb.contains("Retrying"),
        "retry copy leaked to scrollback:\n{sb}"
    );
    assert!(
        !sb.contains('•'),
        "status bullet row leaked to scrollback:\n{sb}"
    );

    // MIRROR: the BAND still shows it — removing the mirror must not remove the affordance.
    let live = buf_text(&app);
    assert!(
        live.contains("Retrying (1/3) in 30s"),
        "retry band missing:\n{live}"
    );
    assert_eq!(app.state().indicator.kind(), IndicatorKind::Retry);
}
...
fn summarization_retry_writes_the_error_but_not_the_countdown() {
    ...
    let sb = app.scrollback_text();
    assert!(
        sb.contains("summarizer overloaded"),
        "the error must still reach the chat:\n{sb}"
    );
    assert!(
        !sb.contains("Retrying"),
        "countdown leaked to scrollback:\n{sb}"
    );
    assert!(
        buf_text(&app).contains("Retrying (2/3)"),
        "retry band missing"
    );
}
...
fn working_indicator_options_resolve_exactly_as_pis_loader_does() {
    use crate::{SPINNER_INTERVAL, WorkingIndicator};
    use serde_json::json;
    ...
    assert!(
        text(&ind, 0).contains(SPINNER_FRAMES[0]),
        "absent `frames` keeps the Braille set"
    );
    assert!(
        text(&ind, 199).contains(SPINNER_FRAMES[0]),
        "…advancing on the extension's clock"
    );
    assert!(
        text(&ind, 200).contains(SPINNER_FRAMES[1]),
        "…not the built-in 80 ms"
    );
    assert_eq!(
        ind.spinner_period(),
        Duration::from_millis(200),
        "the run-loop tick follows too"
    );
...
fn reset_extension_working_state_restores_the_defaults_and_pis_interrupt_suffix() {
    ...
    assert_eq!(
        ind.kind(),
        IndicatorKind::Idle,
        "fixture: hidden while an extension owns it"
    );
