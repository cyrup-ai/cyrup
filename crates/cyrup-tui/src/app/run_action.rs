...
impl App<InlineBackend<Stdout>> {
    ...
    pub(crate) async fn dispatch_run_action(
        ...
        match action {
            ...
            AppAction::InterruptRestoreQueued => {
                ...
                self.dispatch_queue_drain(&ctx.session, QueueDrainReason::Interrupt)
                    .await;
            }
            ...
            AppAction::Dequeue => {
                ...
                self.dispatch_queue_drain(&ctx.session, QueueDrainReason::Dequeue)
                    .await;
            }
    ...
    pub(crate) async fn on_input_event(
        ...
        let Some(first) = maybe_in else {
            return Ok(RunFlow::Break);
        };
