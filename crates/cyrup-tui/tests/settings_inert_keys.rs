...
use cyrup_tui::{App, AppCommand, SelectorKind, UiTheme};
...
async fn disabling_skill_commands_removes_skill_rows_from_the_slash_menu() {
    ...
    app.editor_mut().set_registry(cyrup_tui::CommandRegistry::with_dynamic(
        cyrup_tui::dynamic_commands_from_catalog_gated(&catalog, true),
    ));
    assert!(
