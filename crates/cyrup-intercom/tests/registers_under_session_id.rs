...
/// Point this agent dir's intercom config at the REAL broker binary, so the connect path's own
...
/// Poll until `predicate` holds or `budget` elapses. Generous budgets throughout: this suite runs
/// alongside 240+ others and every wait here is for a real subprocess.
