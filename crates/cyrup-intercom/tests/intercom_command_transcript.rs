...
/// Point this agent dir's config at the REAL broker binary so the connect path's own `ensure_broker`
/// (pi `spawnBrokerIfNeeded`, `index.ts:828`) can never re-exec the test harness — and so a health
/// probe that times out under CPU contention still recovers into a working broker.
