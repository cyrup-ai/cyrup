...
use cyrup_core::{
    CancelToken, Content, TerminateHint, Tool, ToolCallId, ToolError, ToolResult, ToolUpdateSink,
};

use crate::identity::{
    ChildMessageKind, ChildOrchestratorMetadata, format_child_orchestrator_message,
    preferred_supervisor_target,
};
...
impl ContactSupervisorTool {
    ...
    async fn resolve_supervisor(&self, client: &Arc<IntercomClient>) -> Option<String> {
        ...
        match self
            .state
            .resolve_target(client, &self.metadata.orchestrator_target)
            .await
        {
    ...
    async fn dispatch(
        ...
        match params.reason.as_str() {
            "progress_update" => {
                ...
                let text = format_child_orchestrator_message(
                    ChildMessageKind::Update,
                    &self.metadata,
                    &message,
                );
                ...
                if result.delivered {
                    ...
                    Err(ToolError::new(
                        result
                            .reason
                            .unwrap_or_else(|| "update not delivered".to_string()),
                    ))
                }
            }
            "need_decision" => {
            ...
            "interview_request" => {
                let interview_input = params.interview.ok_or_else(|| {
                    ToolError::new("interview_request requires an `interview` object")
                })?;
                let interview = validate_supervisor_interview_request(&interview_input)?;
                let body =
                    format_supervisor_interview_request(&interview, params.message.as_deref());
                let text = format_child_orchestrator_message(
                    ChildMessageKind::Interview,
                    &self.metadata,
                    &body,
                );
                let question_id = uuid::Uuid::new_v4().to_string();
                ...
                let details = match parse_structured_supervisor_reply(&reply, &interview) {
                    ...
                    Some(Err(parse_error)) => {
                        serde_json::json!({ "structuredReplyParseError": parse_error })
                    }
                };
...
fn validate_supervisor_interview_request(
    input: &serde_json::Value,
) -> Result<Interview, ToolError> {
    let obj = input
        .as_object()
        ...
        && !title.is_string()
    {
        return Err(ToolError::new(
            "interview.title must be a string when provided",
        ));
    }
    ...
    {
        return Err(ToolError::new(
            "interview.description must be a string when provided",
        ));
    }
    ...
    for (index, question_input) in raw_questions.iter().enumerate() {
        let question = question_input.as_object().ok_or_else(|| {
            ToolError::new(format!("interview.questions[{index}] must be an object"))
        })?;
        ...
        let id = match question.get("id").and_then(|v| v.as_str()).map(str::trim) {
            ...
            _ => {
                return Err(ToolError::new(format!(
                    "interview.questions[{index}].id must be a non-empty string"
                )));
            }
        };
        if !ids.insert(id.clone()) {
            return Err(ToolError::new(format!(
                "interview question id must be unique: {id}"
            )));
        }
            ...
            .map(str::trim)
        {
            ...
            _ => {
                return Err(ToolError::new(format!(
                    "interview.questions[{index}].question must be a non-empty string"
                )));
            }
        };
        ...
        {
            return Err(ToolError::new(format!(
                "interview.questions[{index}].context must be a string when provided"
            )));
        }
        ...
        if let Some(options_value) = question.get("options") {
            let arr = options_value.as_array().ok_or_else(|| {
                ToolError::new(format!(
                    "interview.questions[{index}].options must be an array when provided"
                ))
            })?;
...
fn format_supervisor_interview_request(interview: &Interview, message: Option<&str>) -> String {
    ...
    if let Some(description) = interview
        .description
        .as_deref()
        .map(str::trim)
        .filter(|d| !d.is_empty())
...
fn validate_supervisor_interview_reply(
    value: &serde_json::Value,
    interview: &Interview,
) -> Result<StructuredReply, String> {
    let obj = value
        .as_object()
    ...
    for (index, response) in responses_input.iter().enumerate() {
        ...
        let question = question_by_id.get(id.as_str()).ok_or_else(|| {
            format!("responses[{index}].id must match a non-info interview question id")
        })?;
        ...
        match question.r#type.as_str() {
            "single" => {
                let s = value.as_str().ok_or_else(|| {
                    format!("responses[{index}].value must be a string for single questions")
                })?;
                let trimmed = s.trim();
                let option_labels: HashSet<&str> = question
                    .options
                    .iter()
                    .flatten()
                    .map(String::as_str)
                ...
                if !option_labels.contains(trimmed) {
                    return Err(format!(
                        "responses[{index}].value must match one of the question options"
                    ));
                }
            ...
            "multi" => {
                let arr = value.as_array().ok_or_else(|| {
                    format!(
                        "responses[{index}].value must be an array of strings for multi questions"
                    )
                })?;
                ...
                if let Some(invalid) = selected
                    .iter()
                    .find(|item| !option_labels.contains(item.as_str()))
                {
                    return Err(format!(
            ...
            other => {
                let s = value.as_str().ok_or_else(|| {
                    format!("responses[{index}].value must be a string for {other} questions")
                })?;
                responses.push(StructuredReplyResponse {
                    id,
...
fn extract_fenced_block(text: &str) -> Option<String> {
    ...
    let looks_like_json_tag = after_open.len() >= 4
        && after_open
            .as_bytes()
            .get(..4)
            .is_some_and(|b| b.eq_ignore_ascii_case(b"json"));
    let after_tag = if looks_like_json_tag {
        after_open.get(4..)?
    } else {
        after_open
    };
...
fn parse_structured_supervisor_reply(
    text: &str,
    interview: &Interview,
) -> Option<Result<StructuredReply, String>> {
...
mod tests {
    ...
    fn rejects_single_question_without_options() {
        ...
        let err = validate_supervisor_interview_request(&input).unwrap_err();
        assert!(
            err.message.contains("options must be a non-empty array"),
            "{}",
            err.message
        );
    }
    ...
    fn rejects_options_on_non_choice_question() {
        ...
        let err = validate_supervisor_interview_request(&input).unwrap_err();
        assert!(
            err.message.contains("only valid for single and multi"),
            "{}",
            err.message
        );
    }
    ...
    fn rejects_malformed_option_entry() {
        ...
        assert!(
            err.message
                .contains("non-empty string or an object with a non-empty label"),
            "{}",
            err.message
        );
    }
    ...
    fn accepts_valid_interview_and_trims_fields() {
        ...
        assert_eq!(
            interview.questions[0].options.as_deref(),
    ...
    fn format_includes_json_reply_instructions_block() {
        let interview =
            validate_supervisor_interview_request(&sample_interview(None)).expect("valid");
    ...
    fn parses_fenced_structured_reply() {
        ...
        let parsed = parse_structured_supervisor_reply(reply_text, &interview)
            .expect("should attempt parse");
        let structured = parsed.expect("should be valid");
        assert_eq!(structured.responses.len(), 1);
        assert_eq!(structured.responses[0].id, "q1");
        assert_eq!(
            structured.responses[0].value,
    ...
    fn rejects_reply_value_not_in_options() {
        ...
        assert!(
            err.contains("must match one of the question options"),
            "{err}"
        );
    }
