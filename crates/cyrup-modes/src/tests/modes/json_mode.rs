...
async fn json_mode_emits_ordered_event_stream() {
    ...
    run_json(
        &runtime,
        [UserInput::text("write a file", InputSource::Cli)],
        &mut out,
    )
    .await
    ...
    assert_eq!(
        type_of(&lines[0]),
        "session",
        "line 1 must be the session header: {lines:?}"
    );
    let events = &lines[1..];
...
async fn json_mode_writes_session_header_as_first_line() {
    ...
    run_json(
        &runtime,
        [UserInput::text("hello", InputSource::Cli)],
        &mut out,
    )
    .await
    .expect("json mode runs");
    ...
    assert!(
        lines.len() >= 2,
        "expected a header line plus events, got {} lines",
        lines.len()
    );
    ...
    assert_eq!(
        lines[0], expected,
        "line 1 must be the byte-exact Pi session header"
    );
    ...
    assert_eq!(
        parsed["cwd"].as_str(),
        Some(fx.cwd.to_string_lossy().as_ref())
    );
    assert!(
        parsed.get("parentSession").is_none(),
        "parentSession must be omitted when unset"
    );
...
async fn json_mode_writes_session_header_exactly_once_across_followups() {
    ...
    run_json(
        &runtime,
        [UserInput::text("first", InputSource::Cli)],
        &mut out,
    )
    .await
    ...
    run_json(
        &runtime,
        [UserInput::text("second", InputSource::Cli)],
        &mut out,
    )
    .await
    .expect("follow-up json run");

    let header_lines = parse_lines(&out)
        .into_iter()
        .filter(|v| type_of(v) == "session")
        .count();
    assert_eq!(
        header_lines, 1,
        "the session header must be emitted exactly once across the run"
    );
}
