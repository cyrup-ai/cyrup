...
All three diagnostics fixed: `agent/run/mod.rs` collapsed to a let-chain, the bare `return;`
removed (it now lives in `proxy/transport.rs` after the wave-4 split), and `area02_backlog.rs`
switched to `expect_err`. Both READMEs carry a strict gate line.

**Two acceptance criteria were wrong as authored and were corrected, not met literally:**

1. `cargo clippy -p cyrup-agent --all-targets -- -D warnings` exits 101, not 0 — and would have
   even at baseline. Flags after `--` propagate to the workspace path dependencies clippy compiles
   on the way in (`cyrup-core` 3 warnings, `cyrup-provider` 33), neither of which is warning-clean.
   The documented gate therefore uses `--no-deps`, which exits 0 and genuinely fails when
   cyrup-agent regresses. The reason is written into both READMEs.
2. `--features test-fixtures` is unrunnable: cyrup-agent declares no `[features]` section at all.
   The flag was omitted from the documented line, with the reason stated.

The criterion `git diff Cargo.toml is empty` also conflicted with BUILDER_MUST_USE in the same
wave, which mandates adding a lint there. Resolved in favour of BUILDER_MUST_USE: this task's
stated intent was that the four `deny` lints stay untouched, and they are — the addition is
warn-level.
