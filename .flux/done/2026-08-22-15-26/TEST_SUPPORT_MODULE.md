...
Clean — all seven acceptance criteria pass. `src/tests/support.rs` is 356 lines, under the
400-line split threshold. `model_ref` went from 12 definitions to 1, with `anthropic_model_ref` as
the explicit variant. The three drifts this task existed to close are closed: `obj_schema` is one
body, `faux_stream_fn` is one signature, and the overloaded `Recorder` name is resolved
(`EventRecorder` for the subscriber, `ToolRequestSpy` for the StreamFn spy). Test count unchanged
at 140.
