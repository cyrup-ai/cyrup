...
`src/proxy.rs` (1,175 lines) is gone, replaced by seven files under `src/proxy/` — `mod.rs`,
`wire.rs`, `builder.rs`, `options.rs`, `transport.rs`, `http_status.rs`, `stream_fn.rs` — split
along the file's own five section banners. The largest is 376 lines, inside the 385-line ceiling the
`agent.rs` decomposition set. `lib.rs`'s five proxy re-exports still resolve, tests move with their
subjects, and the wave-1 `needless_return` fix was carried into `transport.rs` rather than lost.

**One criterion counted the wrong thing:** its clippy grep returns 36, but every one of those is a
`cyrup-core` or `cyrup-provider` diagnostic. cyrup-agent itself is at 0.
