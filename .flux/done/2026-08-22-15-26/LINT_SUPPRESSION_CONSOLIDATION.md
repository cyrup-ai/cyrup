...
13 per-file waivers hoisted to a single one in `tests/mod.rs`; the dead `clippy::print_stdout`
entries dropped (0 print calls existed); all four `#[allow(unused_imports)]` doc-link shims retired.

**The judgement call this task flagged was decided in favour of removing the shims.** The links were
path-qualified instead, and the doc-warning count held at 6 with the same composition — the four
links that were broken by the earlier `agent.rs` split still resolve. The one that remains
unresolved is `super::Agent::run`, which names a method `Agent` has never had and was broken long
before this branch.

**One criterion was invalidated by a later wave:** it requires the waiver at `src/proxy.rs:805` to
remain, but wave 4 (PROXY_MODULE_DECOMPOSITION) split that file away. The equivalent waivers now
live on the inline test modules inside `src/proxy/`.
