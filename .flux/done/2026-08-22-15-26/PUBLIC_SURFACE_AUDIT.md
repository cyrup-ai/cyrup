...
Clean — all nine acceptance criteria pass literally. `StateInner`, `snapshot` and `reduce` are
`pub(crate)`; the `GenerationConfig::transport` doc link was demoted to a plain code span so no new
rustdoc warning appeared; `ToolDef` is gone from `lib.rs`; the eight provider types and eleven core
types that appear in public signatures are re-exported; and both policy comments are in place.
