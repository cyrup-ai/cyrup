...
> **REFRESHED 2026-08-23 after wave 1.** `SESSION_DEAD_HOOK_SEAMS` deleted the three
> `impl CompactionHooks` test doubles and their tests, so `compaction.rs` went 2769 -> 2336 lines,
> 46 -> 43 tests, 25 -> 21 banners. The `area03.rs` section and the `A-05-8`/`A-05-9` banners no
...
```
   1-  35    35 ln   0 tests  (prologue)
  36- 167   132 ln   0 tests  fixtures.rs
 168- 540   373 ln   9 tests  acceptance.rs      A-05-1..A-05-7, A-05-10
 541- 729   189 ln   4 tests  tokens.rs          G-1 tokensBefore, M2, G-3/G-8
 730- 880   151 ln   4 tests  keep_recent.rs     SESS-002
 881-1243   363 ln   8 tests  agent_messages.rs  THEME F, F-1, F-2, F-3
1244-1459   216 ln   3 tests  usage.rs           F-4
1460-1545    86 ln   2 tests  nested.rs          F-5
1546-1979   434 ln   7 tests  streaming.rs       F-6
1980-2337   358 ln   6 tests  guards.rs          StopReason::Pending
                        TOTAL 43 tests (file has 43)
```

Three helpers defined outside the fixtures block cross a seam and must be hoisted into `compaction/fixtures.rs` (line numbers re-verified post-wave-1): `RecordingArc` (def 327, 6 refs), `custom_message_entry` (def 732, 4 refs), `branch_summary_entry` (def 746, 5 refs). Everything else localizes cleanly: `usage_of` (1247) / `UsageSummarizer` (1268) / `compaction_line` (1297) are usage.rs-only; `OptionSpy` (1554) / `f6_session` (1590) / `transient_failure_step` (1572) are streaming.rs-only; `agent_msg_entry` (890) / `bash_msg` (901) are agent_messages-only; `compaction_entry_with_details` (2101) / `prev_details` (2123) are guards-only; `f3_session` (1049) is F-3-only. Note `CapturingHooks` is GONE — wave 1 deleted it with the hook seam; do not look for it.
...
- `fn user(s: &str) -> Message` — md5 `6ef91b2d197ab9f973962d942348e584`, **4 copies**: `compaction.rs:46`, `parity.rs:23`, `sessions.rs:19`, `area03_repairs.rs:20`
